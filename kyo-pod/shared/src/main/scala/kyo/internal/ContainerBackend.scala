...
/** Abstract contract that container runtime backends implement.
  ...
  * `ContainerBackend.detect` which tries Podman first, then Docker. Override with `Container.withBackend` to force a specific runtime.
  *
  * @see
  *   [[ShellBackend]] Shell-based implementation for Docker/Podman CLIs
  * @see
    ...
    def detect()(using Frame): ContainerBackend < (Async & Abort[ContainerException]) =
        Abort.runWith[CommandException](Command("podman", "version").textWithExitCode) {
            case Result.Success((_, exitCode)) if exitCode == ExitCode.Success =>
                new ShellBackend("podman"): ContainerBackend
            case _ =>
                Abort.runWith[CommandException](Command("docker", "version").textWithExitCode) {
                    case Result.Success((_, exitCode)) if exitCode == ExitCode.Success =>
                        new ShellBackend("docker"): ContainerBackend
                    case _ =>
