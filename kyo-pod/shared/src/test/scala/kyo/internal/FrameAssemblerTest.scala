...
class FrameAssemblerTest extends kyo.BasePodTest:
    ...
    "FrameAssembler" - {
        ...
        "UTF-8 multi-byte payload in single complete frame" in {
        ...
        "stream-type byte 0 maps to stdout" in {
            runPipe(span(frame(0, "x".getBytes("UTF-8")))).map(r => assert(r == Chunk(("x", LogEntry.Source.Stdout))))
        }

        // --- cross-chunk: payload split ---

        "frame whose payload is split across two chunks emits exactly once after the second chunk" in {
        ...
        "frame whose 8-byte header is split across two chunks emits exactly once after the second chunk" in {
        ...
        "frame split across three chunks emits exactly once after the third chunk" in {
        ...
        "multiple frames where the last is partial emit complete first, partial after rest arrives" in {
        ...
        "two consecutive frames with partial header straddling chunks" in {
            val a        = stdoutFrame("alpha")
            val b        = stdoutFrame("beta")
            val combined = a ++ b
        ...
        "stderr frame split across chunks preserves source" in {
        ...
        "16 KB payload fed one byte at a time emits exactly one frame at the end" in {
        ...
        "UTF-8 multi-byte char split across chunks reassembles correctly" in {
