...
/** Escape a string for embedding in a Scala double-quoted string literal. */
def kyoTastyEscapeStr(s: String): String =
    s.replace("\\", "\\\\").replace("\"", "\\\"").replace("\r", "").replace("\n", "\\n")

/** Emit a private val declaration, splitting content into 60KB chunks to stay under the JVM
  * class-file string-literal limit.
  */
def kyoTastyEmitVal(varName: String, content: String): String = {
    val chunkSize = 60000
    if (content.length <= chunkSize) {
        s"""    private val $varName: String = "${kyoTastyEscapeStr(content)}""""
    } else {
        val chunks = content.grouped(chunkSize).toSeq
        val parts  = chunks.zipWithIndex.map { case (chunk, i) =>
            s"""    private val ${varName}_$i: String = "${kyoTastyEscapeStr(chunk)}""""
        }.mkString("\n")
        val joined = chunks.indices.map(i => s"${varName}_$i").mkString(" + ")
        parts + s"\n    private val $varName: String = $joined"
    }
}

// Generates EmbeddedText.scala for JS/Native so that TastyTest/OnceCellTest can call
// TestResourceLoader.readText without java.nio.file or ClassLoader (both JVM-only).
// JVM uses classpath resources instead (see resourceGenerators below).
def kyoTastyEmbeddedTextGenerator(platform: String): Def.Initialize[Task[Seq[File]]] = Def.task {
    val base    = (LocalRootProject / baseDirectory).value / "kyo-tasty"
    val srcBase = base / "shared" / "src" / "main" / "scala"
    val outDir  = (Test / sourceManaged).value / "kyo"
    outDir.mkdirs()
    ...
    val cases = filesToEmbed.map { case (varName, (key, _)) =>
        s"""        case "$key" => $varName"""
    }.mkString("\n")
    val src =
        s"""package kyo
// Auto-generated by sbt for $platform platform. Do not edit by hand.
// Embeds text resources that cannot be loaded via ClassLoader on $platform.
object EmbeddedText:
$fields
    def get(resourcePath: String): String =
        resourcePath match
$cases
            case other => throw new RuntimeException(s"$platform: text resource not embedded: $$other")
end EmbeddedText
"""
...
lazy val `kyo-tasty` =
    crossProject(JSPlatform, JVMPlatform, NativePlatform)
        ...
        .jsSettings(
            `js-settings`,
            scalaJSLinkerConfig ~= { _.withModuleKind(ModuleKind.CommonJSModule) },
            Test / sourceGenerators += kyoTastyEmbeddedTextGenerator("JS").taskValue
        )
