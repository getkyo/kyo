...
lazy val `kyo-ai` =
    crossProject(JSPlatform, JVMPlatform, NativePlatform, WasmPlatform)
        ...
        .settings(
            `kyo-settings`,
            // The live-provider narrowing flag is a StaticFlag: it resolves from the FORKED test JVM's
            // properties at class load. Passing it to the sbt launcher alone leaves the matrix un-narrowed,
            // so an intended one-provider run bills every metered backend instead. Forward it explicitly.
            Test / javaOptions ++=
                sys.props.get("kyo.ai.completion.provider").map(value => s"-Dkyo.ai.completion.provider=$value").toSeq
        )
        .jvmSettings(
            mimaCheck(false),
            // The tiktoken rank tables are regenerated by a standalone script, not a build task:
            // see kyo-ai/scripts/TiktokenVocabGen.scala. They are committed sources.
