...
class SqlConfigAddressTest extends Test:
    ...
    "SqlConfig.Address equality" in {
        val a1 = SqlConfig.Address.Network("postgres", "localhost", 5432, "mydb", Present("alice"))
        val a2 = SqlConfig.Address.Network("postgres", "localhost", 5432, "mydb", Present("alice"))
        assert(a1 == a2)
    }

    "SqlConfig.Address inequality on different host" in {
        val a1 = SqlConfig.Address.Network("postgres", "localhost", 5432, "mydb", Present("alice"))
        val a2 = SqlConfig.Address.Network("postgres", "db.example.com", 5432, "mydb", Present("alice"))
        assert(a1 != a2)
    }

    "SqlConfig.Address inequality on different port" in {
        val a1 = SqlConfig.Address.Network("postgres", "localhost", 5432, "mydb", Present("alice"))
        val a2 = SqlConfig.Address.Network("postgres", "localhost", 3306, "mydb", Present("alice"))
        assert(a1 != a2)
    }
    ...
    "SqlConfig.Address inequality between an absent user and a declared one" in {
        val named  = SqlConfig.Address.Network("postgres", "localhost", 5432, "mydb", Present("alice"))
        val absent = SqlConfig.Address.Network("postgres", "localhost", 5432, "mydb", Absent)
        assert(named != absent)
    }
    ...
    "SqlConfig.Address inequality between an absent user and a declared empty one" in {
        val absent = SqlConfig.Address.Network("postgres", "localhost", 5432, "mydb", Absent)
        val empty  = SqlConfig.Address.Network("postgres", "localhost", 5432, "mydb", Present(""))
        assert(absent != empty)
    }

    "SqlConfig.Address usable as Map key" in {
        val a = SqlConfig.Address.Network("postgres", "localhost", 5432, "mydb", Present("alice"))
        val m = Map(a -> 42)
        assert(m(SqlConfig.Address.Network("postgres", "localhost", 5432, "mydb", Present("alice"))) == 42)
    }

    "Render[SqlConfig.Address] renders SqlConfig.Address to scheme://user@host:port/db form" in {
        val addr     = SqlConfig.Address.Network("postgres", "localhost", 5432, "mydb", Present("admin"))
    ...
    "Render[SqlConfig.Address] omits the userinfo entirely for an absent user" in {
        val addr     = SqlConfig.Address.Network("postgres", "localhost", 5432, "mydb", Absent)
        val rendered = Render[SqlConfig.Address].asString(addr)
    ...
    "Render[SqlConfig.Address] keeps the delimiter for a declared empty user" in {
        val addr     = SqlConfig.Address.Network("postgres", "localhost", 5432, "mydb", Present(""))
        val rendered = Render[SqlConfig.Address].asString(addr)
        assert(rendered == "postgres://@localhost:5432/mydb")
    }
    ...
    "Render[SqlConfig.Address] round-trips through Url.parse for all three user spellings" in {
        val addresses = List(
            SqlConfig.Address.Network("postgres", "localhost", 5432, "mydb", Present("alice")),
            SqlConfig.Address.Network("postgres", "localhost", 5432, "mydb", Present("")),
            SqlConfig.Address.Network("postgres", "localhost", 5432, "mydb", Absent)
        )
