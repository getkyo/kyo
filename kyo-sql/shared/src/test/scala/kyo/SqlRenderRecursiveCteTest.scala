...
class SqlRenderRecursiveCteTest extends Test:
    ...
    "WITH RECURSIVE on MySQL 8.0.0+ emits WITH RECURSIVE keyword" in {
        val q       = Sql.commonTablesRecursive(cte)(nodes)
        val backend = kyo.internal.SqlBackend.Mysql.versioned((8, 0, 0))
    ...
    "kyo.internal.SqlBackend.Mysql.versioned(8, 0, 0).supportsRecursiveCte is true" in {
        val backend = kyo.internal.SqlBackend.Mysql.versioned((8, 0, 0))
        assert(backend.supportsRecursiveCte)
    }

    // Leaf 6, supportsRecursiveCte boundary: (5, 7, 44) returns false.
    "kyo.internal.SqlBackend.Mysql.versioned(5, 7, 44).supportsRecursiveCte is false" in {
        val backend = kyo.internal.SqlBackend.Mysql.versioned((5, 7, 44))
        assert(!backend.supportsRecursiveCte)
    }

    // Leaf 7, supportsRecursiveCte: the default Mysql singleton (8.4.0) returns true.
    "kyo.internal.SqlBackend.Mysql.supportsRecursiveCte is true for the default singleton" in {
        assert(kyo.internal.SqlBackend.Mysql.supportsRecursiveCte)
    }
