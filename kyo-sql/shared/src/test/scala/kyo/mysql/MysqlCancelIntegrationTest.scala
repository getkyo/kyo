...
/** Integration tests for MySQL query cancellation via KILL QUERY.
  ...
  *   3. cancellableQuery pattern, connectionId is exposed via handle.connectionId (SqlClient.CancelHandle.Mysql)
            ...
            SqlSharedContainers.withFreshSchema(SqlSharedContainers.Backend.MySQL) { ctx =>
                initClient(ctx, maxConns = 2) { client =>
                    ...
                    (client.cancellableQueryFiber(
                        "SELECT SLEEP(5)"
                    ): (SqlClient.CancelHandle.Mysql, Fiber[Chunk[SqlRow], Abort[SqlException]]) < (Async & Abort[SqlException])).flatMap {
            ...
            SqlSharedContainers.withFreshSchema(SqlSharedContainers.Backend.MySQL) { ctx =>
                initClient(ctx, maxConns = 2) { client =>
                    ...
                    (client.cancellableQuery(
                        sql"SELECT 1"
                    ): (SqlClient.CancelHandle.Mysql, Chunk[SqlRow]) < (Async & Abort[SqlException])).flatMap { case (handle, _) =>
        ...
        Scope.run {
            SqlSharedContainers.withFreshSchema(SqlSharedContainers.Backend.MySQL) { ctx =>
                initClient(ctx, maxConns = 1) { client =>
                    (client.cancellableQuery(
                        sql"SELECT 1"
                    ): (SqlClient.CancelHandle.Mysql, Chunk[SqlRow]) < (Async & Abort[SqlException])).map { case (handle, _) =>
        ...
        Scope.run {
            SqlSharedContainers.withFreshSchema(SqlSharedContainers.Backend.MySQL) { ctx =>
                initClient(ctx, maxConns = 1) { client =>
                    ...
                    // Construct a SqlClient.CancelHandle.Mysql pointing at a non-existent thread.
                    val fakeHandle = SqlClient.CancelHandle.Mysql(client.address, fakeConnectionId)
