...
class SqlClientMetricsTest extends Test:
    ...
    "query_duration_ms histogram records observed duration" in {
        ...
        // Two queries observe the histogram twice. The observed ms values are wall-clock and stay unasserted;
        // how many observations landed is not, and one observation per timedQuery is the property.
        m.timedQuery(successBody).andThen {
            m.timedQuery(successBody).andThen {
