...
object SqlClient:
    ...
    private val local: Local[(Maybe[SqlClient], SqlClientConfig)] =
        Local.init((Absent, defaultConfig))
    ...
    inline def initWith[B, S](rawUrl: String, config: SqlClientConfig)(inline f: PgSqlClient => B < S)(using
        ...
        Abort.get(SqlUrl.parse(rawUrl)).flatMap { url =>
            if url.address.driver != "postgres" then
                ...
                sanitizeTypeNames(config.typeNames).flatMap { _ =>
                    mergeConfig(url, config).flatMap { mergedConfig =>
    ...
    def let[A, S](client: SqlClient)(v: A < S)(using Frame): A < S =
        ...
        local.let((Present(client), client.config))(v)
    end let
    ...
    def metrics(using Frame): kyo.SqlMetrics < (Async & Abort[SqlException]) =
        use { c => c.metrics }
    ...
    extension (self: SqlClient)
        ...
        /** Returns all rows for a parameterised query using the extended protocol.
          *
          * If a transaction is active in the current fiber (via [[txLocal]]), uses the bound connection directly. Otherwise acquires a
        ...
        /** Streams rows from a parameterised query using the Postgres portal protocol.
          *
          * If a transaction is active in the current fiber (via [[txLocal]]), uses the bound connection directly and does NOT release the
        ...
        /** Returns the rows of a parameterised query using the backend-agnostic [[BoundValue]] surface, decoded as `A` via the supplied
          * [[SqlSchema]].
          *
          * Threads the fiber-local [[txLocal]] so calls inside a `transaction { ... }` block use the bound connection. Dispatches the
        ...
        /** Executes a parameterised DML statement using the [[BoundValue]] surface and returns the affected-row count.
          *
          * Threads the fiber-local [[txLocal]] so calls inside a `transaction { ... }` block use the bound connection.
          *
        ...
        /** Runs `body` inside a database transaction.
          *
          * Acquires a single connection from the pool and binds it to the fiber-local [[txLocal]] for the duration of `body`. All
          ...
          * [[txLocal]]), the inner call uses a `SAVEPOINT` instead of a new `BEGIN`:
          *
                ...
                case Present(ctx: TransactionContext.Pg) =>
                    ...
                    Random.nextStringAlphanumeric(20).flatMap { suffix =>
                        ...
                        conn.savepointTransaction(spName).andThen {
                            Abort.run[Any](
                                txLocal.let(Present(innerCtx))(body)
                            ).flatMap {
    ...
    extension (self: SqlClient)
        ...
        /** Executes multiple statements in a single logical batch.
          ...
          *
          * The `body` lambda receives a [[SqlPipelineBuilder]] and registers statements via [[SqlPipelineBuilder.execute]] /
          * [[SqlPipelineBuilder.query]]. When the body returns, all registered statements are dispatched and their responses are read in
          * order.
          *
          ...
          *
          * @param body
          *   lambda that registers statements on the [[SqlPipelineBuilder]]
          * @return
          *   per-statement results in submission order
          */
        def pipeline[S](
            body: SqlPipelineBuilder => Unit < S
    ...
    extension (self: PgSqlClient)

        /** Streams data from `data` into the database using `COPY ... FROM STDIN`.
          *
          * If a transaction is active in the current fiber (via [[txLocal]]), uses the bound connection directly so that the COPY
