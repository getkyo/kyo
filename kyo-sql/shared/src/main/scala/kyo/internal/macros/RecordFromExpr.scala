...
import kyo.*
import kyo.Sql.*
import kyo.internal.FromExprDerived
...
/** Provides `FromExpr[Record[F]]` by TASTy-tree walking of inline-expanded `buildColumns` / `&` expressions.
  ...
  * Record-carrying AST nodes MUST have this given in scope via `import kyo.internal.macros.RecordFromExpr.given`.
  */
    ...
    private class WalkContext(using Quotes):
        ...
        private def walkRecord(t: Term): Option[Record[?]] =
            unwrap(t) match
                ...
                case Apply(TypeApply(Select(_, "buildGroupedView"), _), List(srcExpr, keysExpr)) =>
                    // `import quotes.reflect.*` shadows `kyo.Sql.Term` with the reflection `Term`,
                    // so the AST term type must be fully qualified as `Sql.Term` here.
                    ...
                    given scala.quoted.FromExpr[Sql.Query[?]]       = kyo.FromExpr.derived
                    given scala.quoted.FromExpr[Chunk[Sql.Term[?]]] = kyo.FromExpr.derived
                    for
                        src <- summon[scala.quoted.FromExpr[Sql.Query[?]]]
                            .unapply(srcExpr.asExprOf[Sql.Query[?]])
                        keys <- summon[scala.quoted.FromExpr[Chunk[Sql.Term[?]]]]
                            .unapply(keysExpr.asExprOf[Chunk[Sql.Term[?]]])
        ...
        private def walkDictChain(t: Term): Option[Dict[String, Any]] =
            unwrap(t) match
                ...
                // (`TypeApply(Select(Dict, "empty"), _)`) or wrapped in an `Apply`, and the match handles both shapes.
        ...
        /** Recognises a `Select(qual, "narrowPhantom")` where `qual` refers to `MacroSupport` or its inlined alias.
          *
          * Keyed exactly like [[isColumnNameCall]] and for exactly the same reason: `Join.OnBuilder.on` is an `inline def`, so by the time
          * the walker sees the call the compiler has rewritten the qualifier to `inline$MacroSupport$iN(kyo.internal)`. An `==` test on the
          * symbol name matches the direct form only, which strips nothing.
          */
        private def isNarrowPhantomCall(sel: Term): Boolean =
            sel match
                case Select(qual, "narrowPhantom") =>
                    val symName = qual.symbol.name
                    symName.contains("MacroSupport") || qual.show.contains("MacroSupport")
                case _ => false
        ...
        /** Resolves the `naming` argument of a reconstructed `SqlNameResolver.columnName[T](scalaName, renames, naming)` call to a
          * `Maybe[SqlNaming]`. The argument is a `Maybe` built from a `summonFrom` at the query site, which is not a stable given reference,
          * so this resolves to `Maybe.empty` (no casing) whenever the value is not statically recoverable, matching the runtime fallback's
          * positional match for a casing the static path cannot fold.
          */
        private def resolveNaming(namingArg: Term): Option[Maybe[SqlNaming]] =
            betaUnwrap(namingArg) match
                case Apply(_, List(inner)) =>
                    // `Maybe(<SqlNaming given ref>)`: a casing is in scope. Fold it only if the given is statically
                    // resolvable; otherwise return None so the column does not fold and the whole query falls back to
                    // the runtime render (which applies the casing), never folding a wrong, un-cased name.
                    FromExprDerived.resolveStableGiven[SqlNaming](
                        MacroSupport.expectExpr[SqlNaming](betaUnwrap(inner).asExpr)
                    ).map(n => Maybe(n))
                case _ =>
                    // `Maybe.empty`: no casing in scope; the verbatim / schema-resolved name folds.
                    Some(Maybe.empty)

        private def sqlNameValue(t: Term): Option[String] =
            betaUnwrap(t) match
                case Literal(StringConstant(s)) => Some(s)
                // `SqlNameResolver.columnName[T](scalaNameArg, renamesArg, namingArg)`, the single shape `resolveSqlName[T]` emits.
                // The renames are NOT read from the lifted argument tree: the reconstruction re-reads them from `T`'s own
                // `@column` annotations (the same source the emission's `sqlRenames[T]` macro read), so the folded name and
                // the runtime name agree by construction and no bytecode resolution is involved. A casing that is present but not
                // statically resolvable makes `resolveNaming` None, which makes this whole column None, so the query falls back to
                // the runtime render rather than folding an un-cased name.
                case Apply(TypeApply(sel, List(tpt)), List(scalaNameArg, _, namingArg)) if isColumnNameCall(sel) =>
                    for
                        scalaName <- columnName(scalaNameArg)
        ...
        /** Strips common TASTy wrappers: `Inlined`, `Block`, `Typed`, `asInstanceOf`, `MacroSupport.narrowPhantom`, substitution evidence,
          * and Ident bindings.
          */
        @tailrec
        private def unwrap(t: Term): Term =
            t match
                ...
                // reason. `Join.OnBuilder.on` narrows the merged record to the joined view's field type, because an outer join retypes
                // the NULL-extended side's columns as `Maybe`. Without this arm an `innerJoin ... on ... select` stops folding and the
                // macro reports "query cannot be folded at compile time", which `PostgresDialectNestedRenderTest`'s "Nested in JOIN"
                // scenario exercises through `SqlStaticProbe`.
                case Apply(TypeApply(sel, _), List(inner)) if isNarrowPhantomCall(sel) =>
                    unwrap(inner)
