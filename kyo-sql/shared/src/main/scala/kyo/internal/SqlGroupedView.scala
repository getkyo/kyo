...
import kyo.*
import kyo.Sql.*
import kyo.internal.macros.MacroSupport

/** Construction-time post-`groupBy` view builder.
  *
  * Walks the source's `columns` Record and produces a rewritten `Record[F]` where each leaf is wrapped as [[Sql.GroupedColumn]] (for keys
  * appearing in the grouping set) or [[Sql.UngroupedView]] (for non-key columns). The resulting record's dict is the runtime payload of
  * [[Sql.GroupBy.view]]; the type-level `F` is computed by `RewriteGrouped` at the `groupBy[…]` call site.
  *
  * The single cast (`Record[Any] → Record[F]`) lives at the AST builder seam, where `F` is in scope at the type level.
  *
  * The `case c: Column[?, ?] => c.name` collect below is total over what `groupBy` accepts as a single or tuple key, because the view is
  * rebuilt from the source's columns by name and a key that is not a column has no entry there to rewrite. `groupByGroupingSets` still takes
  * arbitrary terms, which is sound because it promises no rewrite at all: its view is the source's own.
  */
// NOT `private[kyo]`: this object is referenced from the `inline def groupBy` methods in
// `Sql.Table` / `Sql.Where`. A `private[kyo]` modifier drives the Scala 3 compiler to
