// Cast budget:
// - Primitive-lookup: one boundary cast in summonPrimitiveFromExpr.
// - buildDirect needs no invariance dodge: buildMaybe/Chunk/Field/Tilde return FromExpr[A] directly.
...
//
// The recursion guard's casts are the SAME `FromExpr`-invariance category as the "Quoted-list
// invariance" / "type variable mismatch" channel above, not a new category:
...
//     `var` holding macro-expansion-local single-threaded state, never shared across threads.
...
object FromExprDerived:
    ...
    // ---------------------------------------------------------------------
    // Recursive-ADT derivation guard
    // ---------------------------------------------------------------------
    ...
    /** Per-`FromExpr.derived` accumulator that breaks recursive-ADT derivation cycles (`Query` ↔ `Term`).
      ...
      *
      * Strongly typed on the singleton `Quotes` instance `Q` (the `SqlStaticMacro.R` pattern, see `SqlStatic.scala`) so `refs` /
    ...
    /** Test-only: applies the derived FromExpr to a captured `Expr[A]` at macro-expansion time and returns whether the lift matched, as a
      * literal `Boolean`. This sidesteps the production path's `Expr[FromExpr[A]]` packaging, useful for unit testing the matcher logic.
      */
    private[kyo] def applyMatchedImpl[A: Type](value: Expr[A])(using q: Quotes): Expr[Boolean] =
    ...
    /** Same idea as `applyMatchedImpl` but emits the `Option[A].toString` instead of just isDefined. Useful when tests want to verify the
      * lifted *value*.
      */
    private[kyo] def applyReprImpl[A: Type](value: Expr[A])(using q: Quotes): Expr[String] =
        val directFE: scala.quoted.FromExpr[A] = buildDirect[A](new DirectCtx)
        val result                             = directFE.unapply(value)
        ...
        candidates.iterator
            .flatMap { name =>
                try
                    ...
                    // A Scala 3 enum case (`case Num(p, s)` in an `enum`) compiles to an ENUM-flagged class,
                    // and the JVM refuses `Constructor.newInstance` on such a class with
                    // "Cannot reflectively create enum objects". Build it through its companion object's
                    // synthetic `apply` factory instead, an ordinary method the JVM allows. A plain case class
                    // carries no ENUM flag and goes through its constructor. `instantiate` is shared
                    // by the production `deriveProduct` and the test `buildProduct`, so this covers both twins.
                    // (The ENUM access flag, 0x4000, is not surfaced by java.lang.reflect.Modifier, so test it
                    // directly; `Class.isEnum` is false for a Scala 3 enum case, which is not a java.lang.Enum.)
                    if (cls.getModifiers & 0x00004000) != 0 then
                        instantiateEnumCase[A](name, arity, argsRef)
                    else
                        cls.getDeclaredConstructors.find(_.getParameterCount == arity).map { c =>
    ...
    /** Resolves the runtime value of an `Expr[A]` that is a reference to a stable `given` definition (a `given val` / `given def` on a
      * module, e.g. `kyo.Schema.intSchema`). Walks `x` to its symbol, splits the fully-qualified name into owner module +
      ...
      * or reflection fails.
      *
      * Public so kyo-sql's static-SQL lift can delegate here: `ColumnFromExpr` resolves a `Cast` target's codec through this seam,
      * because the render reads the SQL type name out of the instance (a bind's schema, by contrast, is never read at render time
      * and stands in without resolution; see `BindFromExpr.renderStandIn`).
      *
      * ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── WHY
    ...
    def resolveStableGiven[A](using q: Quotes)(x: Expr[A]): Option[A] =
        ...
        // PARAMETERIZED GIVENS ARE REACHED BY CARRYING THEIR ARGUMENTS. `unwrap` keeps every argument list it
        // walks past and the reflection below asks for a method of MATCHING ARITY, so the 22 `SqlSchema`
        // givens that take arguments (`maybe[A](using SqlSchema[A])` and `tuple2` through `tuple22`) resolve
        // exactly like the roughly 300 that do not. Matching an EMPTY argument list only (`case Apply(fun,
        // Nil)`) and asking for a zero-parameter method leaves every parameterized given unresolved, and that
        // is invisible in the DSL: a row of `Long`/`String`/`Int` folds while the same row with one `Maybe`
        // or tuple field does not, and `.run` falls back to the runtime renderer without a word.
        // Parameterized and parameterless givens share this one seam, so a change here moves all of them.
        def unwrap(t: Term, args: List[Term]): (Term, List[Term]) =
            t match
                case Inlined(_, _, inner) => unwrap(inner, args)
        ...
        // Untyped inner resolution, so an ARGUMENT can be resolved by the same code as the head. The cast
        ...
        def resolve(t: Term): Option[Any] =
            ...
            val symName =
                ...
                // error from a pathological tree, a StackOverflowError above all, propagates as itself
                // instead of being reported as "not a stable given".
            ...
            if symName.isEmpty then None
                ...
                if dotIdx < 0 then None
                    ...
                    // Each argument is itself a given reference, so resolving them through this same
                    // function is what makes nesting work: `SqlSchema[Maybe[(Int, String)]]` needs `tuple2`
                    // resolved before `maybe` can be invoked with it.
                    val resolvedArgs =
                        argTerms.foldLeft(Option(List.empty[Any])) { (acc, arg) =>
                            acc.flatMap(done => resolve(arg).map(done :+ _))
                        }
                    resolvedArgs.flatMap(invokeGiven(ownerName, memberName, _))
                end if
            end if
        end resolve
        ...
        def invokeGiven(ownerName: String, memberName: String, argValues: List[Any]): Option[Any] =
            try
                ...
                val ownerCls = ownerCandidates.iterator.flatMap { name =>
                    ...
                    // wrong name, so the real owner is skipped and the failure surfaces later wearing an
                    // unrelated shape.
                ...
                ownerCls.flatMap { cls =>
                    try
                        ...
                        // BY NAME AND ARITY, not by name alone. Type parameters erase, so `maybe[A](using
                        // SqlSchema[A])` is a one-parameter method and `tuple2` a two-parameter one, and
                        // matching on the count is what tells them apart from a parameterless given of the
                        // same name. All 22 parameterized `SqlSchema` given names are distinct, so name plus
                        // count identifies exactly one method.
                        // `getMethods` before `getDeclaredMethods`: the first sees inherited members, the
                        // second sees private ones, and both are needed.
                        val candidates = cls.getMethods.toList ::: cls.getDeclaredMethods.toList
                        candidates
                            .find(m => m.getName == memberName && m.getParameterCount == argValues.size)
                            .flatMap { method =>
                                try
                                    method.setAccessible(true)
                                    // Reflection-seam cast: invoke takes and returns Object.
                                    Some(method.invoke(module, argValues.map(_.asInstanceOf[AnyRef])*))
                                catch
                                    // A THROW HERE IS NOT "this is not a stable given". Everything before this
                                    // point succeeded: the term resolved to a symbol, the owner class was found,
                                    // `MODULE$` was read, and a method matched by name AND arity. So a throw now
                                    // is a fault inside the given's own body, and returning a bare `None` routes
                                    // it down the same runtime-fallback path as a term that was never a given
                                    // reference at all, hiding the fault one layer down.
                                    //
                                    // INFO RATHER THAN WARNING, and that is forced rather than preferred.
                                    // `-Werror` is on for every Scala 3 module (build.sbt:135) and a macro
                                    // `report.warning` IS promoted by it, so a warning here reddens the tree with
                                    // zero real compile errors. This seam also runs under `.run`'s opportunistic
                                    // path, so a warning would turn a documented silent fallback into a compile
                                    // failure for code that is otherwise fine. `report.info` is what survives
                                    // `-Werror` while still putting the cause in the build log.
                                    case e: java.lang.reflect.InvocationTargetException =>
                                        val cause = Option(e.getCause).getOrElse(e)
                                        report.info(
                                            s"static lift: the given `$ownerName.$memberName` was found and invoked, " +
                                                s"and its body threw ${cause.getClass.getName}: ${cause.getMessage}. " +
                                                "This expression falls back to the runtime path. The fault is in the " +
                                                "given, not at the call site.",
                                            x.asTerm.pos
                                        )
                                        None
        ...
        resolve(x.asTerm).map(_.asInstanceOf[A])
    end resolveStableGiven

    /** Beta-reduces an application to a fixpoint. `Term.betaReduce` reduces one application redex per call, but it does so by
    ...
    /** Deeply resolves block-local `val` bindings in a term tree.
      *
    ...
    def resolveBindings(using q: Quotes)(root: q.reflect.Term): q.reflect.Term =
        ...
        // betaReduceFully iterations; termMemo alone does not prevent it.
            ...
            override def transformTerm(t: Term)(owner: Symbol): Term =
                ...
                val result = t match
                    ...
                    case sel @ Select(receiver, fieldName) =>
                        ...
                        // Case-field fold, in two arms. Checking the head symbol alone
                        // (`caseSym.flags.is(Case)` / `caseSym.companionClass.flags.is(Case)`) fails
                        // when the head names an `export` / re-export alias whose `companionClass`
                        // is `NoSymbol`, so the second arm accepts the fold when the field
                        // accessor's owner IS a case class AND the construction's arg count matches
                        // that class's caseFields count, a conservative shape check that keeps the
                        // indexing on the right arg list even when the head symbol is an alias
                        // rather than the case-class companion directly.
                        //
    ...
    /** Type-constructor cycle key. Collapses `Query[T]`, `Query[?]`, `Query[Any]` to one key (`kyo.Sql.Query`) so the cycle is detected
    ...
    /** Saturates a type so it has no free type parameters / wildcards: applies `Any` to every parameter of an unapplied type constructor,
      * and replaces wildcard / abstract-parameter arguments of an applied type with `Any`.
      *
    ...
    private def applyAnyToFreeParams(using q: Quotes)(tpe: q.reflect.TypeRepr): q.reflect.TypeRepr =
        ...
        // raw `TypeBounds`) marks a position the recursion guard must collapse to `Any`. Testing
        // `isClassDef` here instead rejects `Predef.String`, an alias, and collapses `SqlSchema[String]`
        // to `SqlSchema[Any]`.
        ...
        // Saturate a non-concrete argument at applied position `i` of constructor `c`. A bare
    ...
    /** `FromExpr[Any]` for `Any`-positioned AST leaves (`Literal[Any].value`). An arbitrary `Any` is not liftable,
    ...
    private def buildDirect[A: Type](ctx: DirectCtx)(using q: Quotes): scala.quoted.FromExpr[A] =
        ...
        // As in `deriveFor`, the cycle guard must NOT intercept `Maybe` / `Chunk` (themselves
        ...
        val isSpecial =
            isPrimitive[A] || isMaybe(tpe) || isChunk(tpe) || isField(tpe) || isTilde(tpe) ||
                isRecord(tpe) || isSqlCustomLift(tpe)
        ...
        if isProductOrSum then
            // Cycle guard, test path. On re-encounter of an in-progress recursive product/sum type,
            // return the LazyFromExpr placeholder whose `delegate` is populated by the time any
            // `unapply` runs.
            ...
            // `Any`-positioned AST leaves, lift compile-time constant literals (see deriveFor).
        ...
        else if isSqlCustomLift(tpe) then
            // ABORTS on a missing given, matching `deriveSqlCustomLift`. See the two helpers for why the
            // arms differ.
            buildSummonedFromExprOrAbort[A](tpe)
        else if isRecord(tpe) then
            // The test path mirrors `deriveRecord`, summon a `given FromExpr[Record[F]]`
            ...
            // seam `resolveStableGiven` uses). Falls back to `None` when no given
            // is in scope, which is `deriveRecord`'s policy and NOT the custom-lift arm's.
            buildSummonedFromExprOrNone[A]
            ...
            tpe match
                case OrType(left, right) =>
                    // Union-typed AST field, try each branch in turn (see deriveOr).
                ...
                case AndType(_, _) =>
                    // Singleton-narrowed field sites carry `String & Singleton` (and similar), where the
                    // `Singleton` half is a phantom marker with no runtime content. Strip phantom
                    // marker traits from the AndType and recurse on the substantive operand.
    ...
    private def summonPrimitiveFromExpr[A: Type](using q: Quotes): scala.quoted.FromExpr[A] =
        ...
        // rather than T =:= Primitive, so one cast at the outermost return is unavoidable.
    ...
    private def buildChunk[A: Type](using q: Quotes)(ctx: DirectCtx, tpe: q.reflect.TypeRepr): scala.quoted.FromExpr[A] =
        ...
        tpe match
            case AppliedType(_, List(rawInner)) =>
                // Saturate a plain case-class element's free params / wildcards (see `deriveChunk`).
                ...
                val isPlainCaseElem =
                    !isMaybe(rawInnerDealiased) && !isChunk(rawInnerDealiased) && !isField(rawInnerDealiased) &&
                        !isTilde(rawInnerDealiased) &&
                        !isRecord(rawInnerDealiased) && !isSqlCustomLift(rawInnerDealiased) &&
                        ...
                        (new scala.quoted.FromExpr[Chunk[t]]:
                            def unapply(x: Expr[Chunk[t]])(using qctx: Quotes): Option[Chunk[t]] =
                                ...
                                if endsWith(norm, "Chunk.empty") then Some(Chunk.empty[t])
                                    ...
                                    // Also handles `<varargs>.map(<closure>)` (see `deriveChunk`):
    ...
    private def buildProduct[A: Type](using q: Quotes)(ctx: DirectCtx, tpe: q.reflect.TypeRepr): scala.quoted.FromExpr[A] =
        ...
        val fieldFEs: List[scala.quoted.FromExpr[Any]] =
            fields.map { field =>
                // Saturate free parameters / wildcards with `Any` (see deriveProduct).
    ...
    /** Constructs a Scala 3 enum case reflectively via its companion's synthetic `apply`, because the enum
      * case class carries the JVM ENUM flag that blocks `Constructor.newInstance` (see [[instantiate]]).
      * `caseBinaryName` is the case class's binary name (e.g. `pkg.Obj$Enum$Case`); its companion module is
      * `caseBinaryName + "$"`, whose `apply(args)` is what `Enum.Case(args)` calls at the source level.
      */
    private def instantiateEnumCase[A](caseBinaryName: String, arity: Int, argsRef: List[AnyRef]): Option[A] =
        try
            val companion = Class.forName(caseBinaryName + "$")
            val module    = companion.getField("MODULE$").get(null)
            companion.getMethods
                .find(m => m.getName == "apply" && m.getParameterCount == arity)
                .map { m =>
                    m.setAccessible(true)
                    m.invoke(module, argsRef*).asInstanceOf[A]
                }
        catch case _: Throwable => None
    end instantiateEnumCase
        ...
        val childFEs: List[scala.quoted.FromExpr[A]] =
            children.map { child =>
                if child.flags.is(Flags.Module) then
                    ...
                    // Saturate free parameters / wildcards with `Any` (see deriveSum).
    ...
    private def deriveFor[A: Type, Q <: Quotes & Singleton](ctx: DeriveCtx[Q]): Expr[scala.quoted.FromExpr[A]] =
        ...
        val tpe = TypeRepr.of[A].dealias
        val sym = tpe.typeSymbol
        // The cycle guard applies ONLY to case classes / sealed / enum class-defs that are
        ...
        if isProductOrSum then
            // Cycle guard, production path. On first encounter of a recursive
            ...
            // `Any`-typed leaves arise from saturating an AST node's free value parameter
            ...
            tpe match
                case OrType(left, right) =>
                    // A union-typed AST field (`Windowed.inner: Aggregate.Call[A] |
    ...
    /** Derives a `FromExpr` for a union type `L | R`: tries the `FromExpr[L]` matcher, then the `FromExpr[R]` matcher.
      */
    ...
    /** Closes a recursion cycle. On first encounter of a recursive product/sum type, reserves a `lazy val` symbol, registers
    ...
    /** `buildDirect` test-path counterpart of `deriveSqlCustomLift` and `deriveRecord`. Builds a `FromExpr[A]` whose `unapply`
      * summons a `given FromExpr[A]` at the macro use-site (e.g. kyo-sql's `BindFromExpr.fromExprLiteral` or
      * `RecordFromExpr.fromExprRecord`) and delegates to it.
      *
      * WHY THIS PATH REFLECTS AT ALL, when the production path does not. `deriveSqlCustomLift` and `deriveRecord` SPLICE the summoned given
      * into generated code, so they never need its value at expansion time. `buildDirect` is invoked by `applyMatchedImpl` /
      * `applyReprImpl`, which must produce an answer DURING expansion, so the given has to be turned into a real instance here. That is the
      * whole reason `resolveStableGiven` exists, and it is why only this path can be defeated by a given reflection cannot reach.
      *
      * The resolution itself is delegated to `resolveStableGiven` rather than reimplemented: both paths reach a given through one seam, so
      * neither can drift into resolving a narrower set of givens than the other.
      */
    private def summonedFromExpr[A: Type](using q: Quotes)(onMissingGiven: Quotes ?=> Option[A]): scala.quoted.FromExpr[A] =
        new scala.quoted.FromExpr[A]:
            def unapply(x: Expr[A])(using qctx: Quotes): Option[A] =
                Expr.summon[scala.quoted.FromExpr[A]] match
                    case None         => onMissingGiven(using qctx)
                    case Some(feExpr) => resolveStableGiven[scala.quoted.FromExpr[A]](feExpr).flatMap(_.unapply(x))
            end unapply
        end new
    end summonedFromExpr

    /** The custom-lift arm, which ABORTS when the given is missing, matching its production twin `deriveSqlCustomLift`.
      *
      * The two arms of this helper deliberately do NOT share a policy. Were a missing `BindFromExpr` import to make the harness answer a
      * plain `false`, that is also what it answers when the derivation correctly refuses a tree, so a genuine break would present as an
      * ordinary assertion failure. Aborting instead names the exact missing import; the message below is `deriveSqlCustomLift`'s, verbatim.
      *
      * Aborting fails the whole derivation rather than one branch, which matters in the `OrType` case where `buildDirect` builds both
      * branches and `orElse`s them. `deriveFor` builds Or branches the same way and reaches the same abort.
      */
    private def buildSummonedFromExprOrAbort[A: Type](using q: Quotes)(tpe: q.reflect.TypeRepr): scala.quoted.FromExpr[A] =
        val shown = tpe.show
        summonedFromExpr[A] {
            quotes ?=>
                import quotes.reflect.*
                report.errorAndAbort(
                    s"FromExpr.derived: no FromExpr[$shown] found. " +
                        "Ensure the kyo.internal.macros givens are in scope (import kyo.internal.macros.ColumnFromExpr.given and " +
                        "import kyo.internal.macros.BindFromExpr.given)."
                )
        }
    end buildSummonedFromExprOrAbort

    /** The `Record` arm, which stays SILENT when the given is missing, matching its production twin `deriveRecord`.
      *
      * `deriveRecord` deliberately emits a `None`-returning placeholder rather than aborting, for contexts that do not import the kyo-sql
      * given. This arm answers `None` for the same reason and must keep doing so: a context with no kyo-sql on its path is not in error.
      */
    private def buildSummonedFromExprOrNone[A: Type](using q: Quotes): scala.quoted.FromExpr[A] =
        summonedFromExpr[A](None)
    ...
    /** True for the kyo-sql AST nodes that need a custom `FromExpr` rather than the generic product walker.
      *
      * Two groups, for two different reasons. `Column` / `GroupedColumn` / `UngroupedView` arise from `Record` field projections
      * (`selectDynamic` chains), and `Cast` carries a target that is a reference to a summoned `SqlType` given (a summoned given arrives as a
      * reference, not a literal enum), so its custom instance resolves that reference to the enum via `resolveStableGiven`; the product
      * walker reads neither shape. `Literal` / `Fragment.Bind` / `BoundValue` are ordinary constructor applies that the walker COULD read,
      * and must not: their value position holds a runtime user expression, so lifting it would demand the value be a compile-time constant.
      * Their custom instances keep the call-site expression as a hole for the static macro to splice.
      */
    private def isSqlCustomLift(using q: Quotes)(tpe: q.reflect.TypeRepr): Boolean =
        sqlCustomLiftTypeSyms.exists(t => symbolMatches(tpe, t))

    /** The kyo-sql type-member symbols that resolve through a summoned given, resolved at macro-expansion time; empty when kyo-sql is not on
      * the classpath.
      *
      * Two owners because the nodes live at two nesting levels: the projections, `Literal`, and `BoundValue` are members of
      * `object kyo.Sql`, and `Bind` is a member of `object kyo.Sql.Fragment`. Each lookup fails soft to `Symbol.noSymbol`, which
      * `symbolMatches` reads as no match.
      */
    private def sqlCustomLiftTypeSyms(using q: Quotes): List[q.reflect.Symbol] =
        import q.reflect.*
        def module(fqn: String): Symbol =
            try Symbol.requiredModule(fqn)
            catch case _: Throwable => Symbol.noSymbol
        def members(owner: Symbol, names: List[String]): List[Symbol] =
            if !owner.exists then Nil else names.map(owner.typeMember)
        members(module("kyo.Sql"), List("Column", "GroupedColumn", "UngroupedView", "Cast", "Literal", "BoundValue")) ++
            members(module("kyo.Sql.Fragment"), List("Bind"))
    end sqlCustomLiftTypeSyms
    ...
    /** Production-path arm for the custom-lift nodes: delegates to the `given FromExpr` summoned at the macro use-site
      * (`ColumnFromExpr.fromExprColumn` and `BindFromExpr.fromExprLiteral` and friends, when imported in kyo-sql). Mirrors `deriveRecord`.
      */
    private def deriveSqlCustomLift[A: Type](using q: Quotes)(tpe: q.reflect.TypeRepr): Expr[scala.quoted.FromExpr[A]] =
        import q.reflect.*
        Expr.summon[scala.quoted.FromExpr[A]].getOrElse(
            report.errorAndAbort(
    ...
    private def deriveChunk[A: Type, Q <: Quotes & Singleton](
        ...
        tpe match
            case AppliedType(_, List(rawInner)) =>
                // Derive a *case-class* element's `FromExpr` against the SATURATED element type.
                ...
                rawInner.asType match
                    case '[t] =>
                        // Erase the inner FromExpr to `FromExpr[Any]`. When the recursion guard
                            ...
                            new scala.quoted.FromExpr[Chunk[t]]:
                                ...
                                def unapply(x: Expr[Chunk[t]])(using qctx: Quotes): Option[Chunk[t]] =
                                    ...
                                    if endsWith(norm, "Chunk.empty") then
                                        ...
                                        //
                                        // Also handles `<varargs>.map(<closure>)`: the DSL's INSERT
    ...
    private def deriveRecord[A: Type](using q: Quotes)(tpe: q.reflect.TypeRepr): Expr[scala.quoted.FromExpr[A]] =
        ...
        // imported in kyo-sql). Falls back to a `None`-returning placeholder when no given is found, for contexts
        // that do not import the kyo-sql given.
        Expr.summon[scala.quoted.FromExpr[A]].getOrElse('{
            new scala.quoted.FromExpr[A]:
    ...
    private def deriveProduct[A: Type, Q <: Quotes & Singleton](
        ...
        val fieldFromExprs: List[Expr[Any]] =
            fields.map { field =>
                // Saturate any free parameter / wildcard in the field type with `Any` so a
        ...
        '{
            new scala.quoted.FromExpr[A]:
                // `lazy`: defers forcing the field FromExprs until `unapply` runs, so the
    ...
    private def deriveSum[A: Type, Q <: Quotes & Singleton](
        ...
        val childFromExprs: List[Expr[Any]] =
            children.map { child =>
                // A child case class / module may carry its own type arguments (e.g.
        ...
        '{
            new scala.quoted.FromExpr[A]:
                // `lazy`: the mutually-recursive `lazy val` block resolves child refs only
