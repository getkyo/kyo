...
- `ArrowEffect.scala`: the effect protocol, fully inline. `suspend` / `suspendWith` create operations; the handling variants are `handle` (Cont), the two `handleLoop` overloads (Loop and LoopState), their `handleWith` / `handleLoopWith` forms that fuse the continuation in place of the identity exit arrow (the `*With` pattern is a perf path; a With region allocates one object serving as both handler and exit), `handleFirst` (First), which answers one operation and leaves, and `handlePartial`, the eager driver for scheduler integration. `handleCatching` is not a variant of its own: it is `handle` over `Effect.catching` with the clause wrapped in the same guard.
...
- `KyoInternal.scala`: the node shapes (`Suspend`, `Defer`, and the region nodes `Handled`, `HandledState`, `HandledFirst`, each with a diagnostic `toString`), the `Defaulted` mixin, and the `Nested` box that keeps boxed data distinguishable from computations.
- `Handler.scala` / `Handlers.scala`: the four handler kinds as traits with an abstract `apply` (`Cont`, `Loop`, `LoopState`, `First`, all pure logic; a region's state lives in its node and its cell, never in the handler) and the spine, the stack of entered regions: one immutable generic cell per region (`Node` for stateless with the Cont-or-Loop union, `StateNode` carrying the region's current state, `FirstNode` for the region that answers once), linked through `prev` with `Empty` as the empty stack.
...
- `LiftMacro.scala`: the `abortCastUnit` diagnostic macro.
...
4. **When a constructor's result must unify with an expected `< S`, return the currency type, and cast to a type the constructor names itself.** The `Loop.Outcome` constructors are the worked example (`Loop.scala:129-145`). A handler clause expects `Outcome[answer < row, result] < S2`, whose answer slot is pending so a clause may answer effectfully, while most clauses answer with a settled value: with a bare `Outcome` return, inference solves the answer type from the argument instead of from the expectation and 58 sites in this module stop compiling, which is the compile that settled it. The row on the returns is `Any`, which conforms wherever the site's row lands, so the type parameters are the old kernel's. `continue` casts rather than lifting, because a `Continue` is never `Boxed` and the lift's boxing arm cannot fire; `done` keeps the lift, because its payload may be a computation held as data and the box is what keeps the evaluator from reading it as a clause that suspended. The cast in each constructor names that constructor's own result type: a shared helper whose target type comes from inference had that type solved to the `Kyo` member of the currency union at some sites, and the resulting `checkcast` failed only when a loop ran, which is rule 2's footgun reached through `fromKyo` instead of the lift.
