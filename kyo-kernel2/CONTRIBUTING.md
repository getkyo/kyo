...
- `Handler.scala` / `Handlers.scala`: the three handler kinds as abstract classes with an abstract `apply` (`Cont`, `Loop`, `LoopState`, with `LoopState` carrying its state and building successors via `withState`) and the collection `Eval` scans.
