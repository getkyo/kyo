...
- `ArrowEffect.scala`: the effect protocol. `suspend` / `suspendWith` create operations; the four handling variants are `handle` (Cont), the two `handleLoop` overloads (Loop and LoopState), and `partial`, the eager driver for scheduler integration.
