...
Nine files under `shared/src/main/scala/kyo/kernel`:

- `Pending.scala`: the currency. `opaque type <[+A, -S] >: Kyo[A, S] = A | Kyo[A, S]` (`Pending.scala:8`), the implicit lift, and the `map` / `eval` / `evalPartial` extensions.
- `Kyo.scala`: the three node shapes (`Suspend`, `Defer`, `Handled`) and the `Nested` box that keeps boxed data distinguishable from computations.
- `Arrow.scala`: typed continuations. `Arrow.AndThen.step` is the kernel's one stack-safe recursion carrier (`Arrow.scala:76-102`).
- `ArrowEffect.scala`: the effect protocol. `suspend` / `suspendWith` create operations; `handle` and the two `handleLoop` overloads construct region nodes; `resume`, `stop`, `loop`, and `partial` answer at the call site by rotation.
- `Handler.scala` / `Handlers.scala`: the three handler kinds (`Cont`, `Loop`, `LoopState`) and the collection `Eval` scans.
- `Eval.scala`: the evaluator. One flat loop.
- `Loop.scala`: stack-safe user-level iteration; its `Outcome` encoding is also what handler clauses return.
- `Safepoint.scala`: the per-thread depth budget (`Period = 512`, `Safepoint.scala:14`) that bounds fused execution; when it runs out, construction yields a `Kyo.Defer` and `Eval` resets the budget as it steps one (`Eval.scala:163-166`).
...
**Two answering families.** `ArrowEffect.handle` (constructing a `Handler.Cont`) and the two `ArrowEffect.handleLoop` overloads (constructing `Handler.Loop` and `Handler.LoopState`) build `Kyo.Handled` region nodes and run nothing: handling is a value, and answering happens at `eval` (`ArrowEffect.scala:61-101`; pinned by `ArrowEffectTest` "lazy: the handled computation is a value and answers at eval"). The stateful `handleLoop` adapts value state to handler succession with a private `StateHandler` per state (`ArrowEffect.scala:88-99`). `resume`, `stop`, `loop`, and `partial` are the other family: flat `@tailrec` trampolines that answer matching operations at the call site and rotate themselves into foreign suspensions and defers (`ArrowEffect.scala:104-318`). The rotation invariant is that re-entry wraps the continuation's application, never follows it: the append encoding agreed with every existing test and still lost the handler behind one trailing transform, found by static trace and pinned with five red tests before the fix (commit `727e8e6742`; the pins are the `ArrowEffectTest` "stays in force across a foreign crossing with a trailing transform" family).
...
3. **An operation resolves to the innermost matching handler.** `Handlers.indexOf` scans from the innermost end with a `<:<` tag test, so innermost-wins is scan order and a subtype tag resolves a supertype handler (`Handlers.scala:38-44`). A miss throws `IllegalStateException("unhandled suspension: ...")` (`Eval.scala:56`).
...
8. **State is an index update.** A `LoopState` successor replaces its own layer in place, with an identity check skipping the copy when the clause returns the same handler (`Eval.scala:121-124`):
...
The implicit lift (`Pending.scala:12-17`) passes primitives and `String` raw and routes everything else through `Nested.lift`, which boxes only `Boxed` values: `Kyo` nodes and already-nested values (`Kyo.scala:16-19`). Ordinary user values pass raw. This gives four rules:
...
4. **Search the tree for the precedent before inventing.** The final model is `AndThen.step`'s iterative flattening applied to regions, generalized from the one-layer rewriting discipline the trampolines still use. Both were in this module the whole time; effort went into inventing where it should have gone into recognizing. Read `Arrow.AndThen.step` and the `resume` trampoline before designing any new evaluation mechanism, and ask whether your problem is one of them at a different scale.
...
- **Tests reach features through the user-facing surface**: `ArrowEffect.suspend` / `suspendWith` / `handle` / `handleLoop` / `resume` / `stop` / `loop` / `partial`, the `<` extensions, and `Loop.continue` / `Loop.done`. Direct `new Kyo.Handled`, `Handler.*` subclassing, `Handlers`, `Nested`, and `Safepoint` internals appear in a test only when the internal unit is itself under test: `HandlersTest` over the collection, `KyoTest` over the node shapes, `Eval`'s rejection and `evalPartial` pins, and the `Safepoint.Period` boundary tests that would otherwise hard-code 512. The per-test adjudication and the migration map for `EvalTest`'s remaining direct constructions live in [kernel2-test-api-audit.md](../kernel2-test-api-audit.md).
- **Probes are disposable dev artifacts.** An executable spec under `jvm/src/test`, named after the source it probes, with a header stating the model it encodes; no test runner picks it up. Probes made it possible to build competing evaluator models and measure both without one speculative edit to main source, and they are a liability if left behind: the repo rule is that scratch files are deleted before the change ships, their validated assertions folded into the matching `*Test.scala`. `EvalProbe.scala` and `HandlersProbe.scala` are the current dev artifacts, both pending removal; the Handlers probe's capture scenarios are the only coverage of multi-shot capture and belong in `EvalTest` now that `Eval` runs `Cont` regions.
...
- **Properly typed code, with casts only at documented boundaries.** The sanctioned boundaries are: tag-keyed handler recovery after `Handlers.indexOf` (the scan proves the tag, the type system cannot), the erased currency inside `evalLoop`, the effect slot pinned to `Nothing` where a pattern-bound effect type loses its GADT bound, and the lift-avoidance casts of the currency discipline. Each site carries a comment naming its boundary (`Handlers.scala:35-37`, `Eval.scala:211-213`). A cast that fits none of these categories is a design smell to resolve, not a typing convenience.
- **No explicit type parameters unless inference genuinely fails, and then the site says so.** The current examples: `rebuildFrom`'s erased `Kyo.Handled[[B] =>> Any, [B] =>> Any, Nothing, Any, Any, Any]` construction under its lift comment, and the stateful `handleLoop`'s outcome re-typing with its union-encoding comment (`ArrowEffect.scala:95-97`).
