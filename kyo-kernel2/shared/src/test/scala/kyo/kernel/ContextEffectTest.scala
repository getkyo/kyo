...
class ContextEffectTest extends AnyFreeSpec:
    ...
    "resuming" - {
        ...
        "a parked computation resumes with its binding" in {
            val v: Int < Any = ContextEffect.handle(Tag[Count], 21)(count.map(_ * 2))
            val parked       = Eval.partial(v, () => true)
    ...
    "release" - {
        ...
        def held[A, S](value: Int, onRelease: Int => Unit)(v: A < (Count & S)): A < S =
            ContextEffect.handle(Tag[Count], value, (_: Int) => value, release = Maybe((i: Int) => onRelease(i)))(v)
        ...
        "may be a deferred computation" in {
            ...
            val v = ContextEffect.handle(
                ...
                1,
                (_: Int) => 1,
                release = Maybe((_: Int) => Effect.defer { released = true })
            )(count.map(_ + 1))
            assert(Eval(v) == 2)
            assert(released)
        }
