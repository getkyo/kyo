...
class KyoTest extends Test[Any]:
    ...
    // the current kernel renders the full frame snippet; kernel2 renders the node shape with tag
    // and position until the trace round lands (recorded divergence)
    "toString" in {
        val rendered = TestEffect1(1).map(_ + 1).toString
        assert(rendered.startsWith("Continue(Suspend("))
        assert(rendered.contains("TestEffect1"))
    ...
    "combinators replay correctly when a captured continuation runs twice" - {
        ...
        def park[A](v: A < TestEffect1)(using kyo.test.AssertScope): Arrow[Int, A, TestEffect1] =
            ...
            val _ = ArrowEffect.handlePartial(Tag[TestEffect1], v, kyo.kernel.internal.Context.empty)(
                [C] =>
                    (input, cont) =>
