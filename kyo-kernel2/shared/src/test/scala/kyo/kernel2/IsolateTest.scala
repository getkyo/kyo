...
class IsolateTest extends Test[Any]:
    ...
    "isolate application" - {
        "no context effect suspension" in {
            val effect: Context < Any = ContextEffect.runDetached { context =>
                context
            }
        ...
        "allows access to context" in {
            ...
            val effect = ContextEffect.runDetached { context =>
        ...
        "isolates runtime effect" in {
            val isolate = Isolate.derive[TestEffect1, Any, Any]
            val effect: Int < TestEffect1 = ContextEffect.runDetached { context =>
    ...
    "nested isolates" in {
        ...
        val effect: Int < (TestEffect1 & TestEffect2) =
            ContextEffect.runDetached { outerContext =>
                ContextEffect.runDetached { innerContext =>
                    for
    ...
    "with non-context effect" in {
        val isolate = Isolate.derive[TestEffect1, Any, Any]
        val effect: Int < (TestEffect1 & NotContextEffect) = ContextEffect.runDetached { context =>
            for
                x <- ContextEffect.suspend[Int, TestEffect1](Tag[TestEffect1])
    ...
    "preserves outer effects" in {
        ...
        val effect: Int < TestEffect1 =
            for
                outer <- outerEffect
                inner <- ContextEffect.runDetached { context => innerEffect }
            yield outer + inner
    ...
    "residual effects" - {
        ...
        "preserves residual effects after isolate application" in {
            val isolate = Isolate.derive[TestEffect1, ResidualEffect, Any]
            val effect: Int < (TestEffect1 & ResidualEffect) = ContextEffect.runDetached { context =>
        ...
        "supports subclasses of residual effects" in {
            val isolate = Isolate.derive[TestEffect1, ResidualEffect, Any]
            val effect: Int < (TestEffect1 & SubResidualEffect) = ContextEffect.runDetached { context =>
                for
                    x <- ContextEffect.suspend[Int, TestEffect1](Tag[TestEffect1])
            ...
            val context =
                ContextEffect.handle(Tag[IsolatedEffect], 24, _ + 1) {
                    ContextEffect.handle(Tag[NonIsolatedEffect], "test", _.toUpperCase) {
                        ContextEffect.runDetached { context => context }
                    }
                }.eval
