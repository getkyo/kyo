...
final private[kyo] class EffectTrace extends Exception(null, null, false, false):
    ...
    override def getMessage =
        frames.map((op, f) => "at " + EffectTrace.describe(op, f) + "(" + f.position.show + ")")
...
private[kyo] object EffectTrace:
    ...
    def attach(ex: Throwable, op: String, frame: Frame): Unit =
        ex.getSuppressed.collectFirst { case o: EffectTrace => o } match
            case Some(o) =>
    ...
    def install(ex: Throwable): Unit =
        ex.getSuppressed.collectFirst { case o: EffectTrace => o } match
            case Some(o) if o.frames.size > o.installed =>
                ...
                val user = ex.getStackTrace.filterNot(e => e.getClassName.startsWith("kyo.kernel"))
