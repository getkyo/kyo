...
/** The kernel's runtime guard: a per-thread token gating eager execution behind depth accounting and preemption polling.
  ...
  */
sealed abstract private[kyo] class Safepoint(private[kernel] val thread: Thread):
    ...
    private[kernel] def ownedBy(t: Thread): Boolean =
        thread eq t

    private[kernel] def alive: Boolean =
        thread.isAlive
end Safepoint
...
private[kyo] object Safepoint:
    ...
    final private[kernel] class Active private[Safepoint] (thread: Thread, private[Safepoint] val home: Current)
    ...
    final private[kernel] class Preempted private[Safepoint] (
