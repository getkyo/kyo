...
object FleetLedgerDemo extends KyoApp:
    ...
    def flow(using Frame): LedgerSnapshot < (Async & Abort[FileSystemException | JournalError | EventLog.PreparationFailure]) =
        Scope.run {
            Path.run {
                for
    ...
    private def writeAndCount(dir: Path)(using
        Frame
    ): Int < (Async & Abort[FileSystemException | JournalError | EventLog.PreparationFailure]) =
        Scope.run {
            for
    ...
    private def rereadAfterReopen(dir: Path)(using
        Frame
    ): Chunk[Event.Record[FleetEvent]] < (Async & Abort[FileSystemException | JournalError]) =
        Scope.run {
            for
                log     <- EventLog.init(fleetCodecs, journalId)
                backend <- Journal.Backend.file(dir, jsonlConfiguration)
    ...
    private def streamInfoAfterReopen(dir: Path)(using Frame): Boolean < (Async & Abort[FileSystemException | JournalError]) =
        Scope.run {
            for
                backend <- Journal.Backend.file(dir, jsonlConfiguration)
    ...
    run {
        for
            snapshot <- Abort.run[FileSystemException | JournalError | EventLog.PreparationFailure](flow)
