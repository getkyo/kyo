...
class FileJournalTest extends kyo.test.Test[Any]:
    ...
    // PathWrite and adds Abort[FileSystemException]. A temp-dir failure is test-infra breakage, surfaced
    ...
    private def discharge[A](effect: A < (Async & Scope & Abort[JournalError]))(using Frame): A < (Async & Scope) =
        ...
        "opens a .seg backend and round-trips" in {
            ...
            for
                ...
                segPresent <- Sync.Unsafe.defer {
                    (dir / "streams" / fjEventStreamId.value / "00000000000000000000.seg").unsafe.exists().getOrThrow
                }
            yield
    ...
    "MANIFEST-less root" - {
        "a MANIFEST-less root with existing streams/ entries still opens, inferring Binary with default media types" in {
            ...
            yield
                ...
                assert(!(dir / "MANIFEST").unsafe.exists().getOrThrow)
        }
    }
