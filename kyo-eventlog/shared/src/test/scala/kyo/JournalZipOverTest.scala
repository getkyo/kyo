...
class JournalZipOverTest extends kyo.test.Test[Any]:
    ...
    private def migrateAll(
        from: FileSystem.Read[Sync],
        to: FileSystem.Write[Sync],
        root: Path
                    ...
                    case Present(p) =>
                        from.isDirectory(p).map { isDir =>
                            if isDir then Loop.continue(())
                                ...
                                from.readBytes(p).map(bytes =>
                                    to.writeBytes(p, bytes, Path.WriteOptions(createFolders = true)).andThen(Loop.continue(()))
                                )
                        }
                }
            }
