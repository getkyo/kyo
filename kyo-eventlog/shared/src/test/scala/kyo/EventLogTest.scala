...
class EventLogTest extends kyo.test.Test[Any]:
    ...
    "decode corruption on EventLog.read folds into JournalCorruptedError, never a codec-specific error" in {
    ...
    "EventLog.reader reads committed records and the Reader has no append (compile-negative)" in {
        ...
        val sid       = testStreamId
        for
            dir <- Abort.run[FileSystemException](Path.run(Path.tempDir("log-test-reader"))).map {
    ...
    // Unsafe: JVM-only self-audits of the eventlog guides and sources on disk (a dev-time grep, not
    // a runtime capability); mirrors JournalEventTest and DemoValidationTest's repo-root walk,
    // recursive file listing, and bare-token helpers. These leaves are `.onlyJvm` because
    // `user.dir` and the synchronous file reads are JVM-only.
    private def guardRepoRoot(): Path =
        @scala.annotation.tailrec
        def loop(dir: Path): Path =
            if (dir / "build.sbt").unsafe.exists().getOrThrow then dir
            else
                dir.parent match
    ...
    private def guardFilesUnder(dir: Path): List[Path] =
        if !dir.unsafe.exists().getOrThrow then List.empty
        else
    ...
    "eventlog documentation and platform-hygiene guards" - {

        // JVM-only because it reads repo files.
        "the eventlog guide and README reference only current API symbols and reader semantics".onlyJvm in {
            ...
            requiredNames.foreach(name => assert(combined.contains(name), s"the API name '$name' is missing from the eventlog guides"))
        ...
        // JVM-only because it reads repo files.
        "the shared and JS/Wasm sources use no JVM-only file APIs or browser-persistence symbols".onlyJvm in {
            val root = guardRepoRoot()
            val trees = List(
