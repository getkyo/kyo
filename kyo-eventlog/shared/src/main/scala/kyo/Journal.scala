...
object Journal:
    ...
    object Backend:
        ...
        /** Opens a typed Sync file-backed journal over `dir`, running the engine's physical store
          ...
          */
        def fileOver[A](fs: FileSystem.Write[Sync], dir: Path, configuration: FileJournal.Configuration[A])(using
            Frame
        ): FileJournal.Backend[A, Sync] < (Async & Scope & Abort[JournalStorageError]) =
        ...
        /** Opens an existing typed Sync file-backed journal through a read-only filesystem. */
        def fileOver[A](fs: FileSystem.Read[Sync], dir: Path, configuration: FileJournal.Configuration[A])(using
            Frame
        ): FileJournal.Backend[A, Sync] < (Async & Scope & Abort[JournalStorageError]) =
            kyo.internal.FileJournalCore.openOverReadOnly(dir, configuration, fs)
