...
private[kyo] object RouteCoverageMacros:
    ...
    def unionCoverageImpl[A: Type, Covered: Type](using Quotes): Expr[EventLog.RouteCoverage[A, Covered]] =
        ...
        // `using Quotes`, not the outer one captured implicitly: the quotes this builds are spliced
        // into the matcher lambda below, and an Expr built under the enclosing Quotes is out of scope
        // there. -Xcheck-macros rejects exactly that.
        def matchBody(value: Expr[A])(using Quotes): Expr[String] =
