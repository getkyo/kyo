...
class FileJournalNodeLockTest extends kyo.test.Test[Any]:
    ...
    private def freshDir(using Frame): Path < (Sync & Scope) =
        Abort.run[FileSystemException](Path.run(Path.tempDir("fj-nodelock"))).map {
    ...
    "lock acquisition failure matrix" - {
                ...
                ok <- Sync.Unsafe.defer {
                    new NodeSegmentStore().acquireLock(dir) match
                        case Result.Success(lock) =>
                            ...
                            val lockExists = (dir / "LOCK").unsafe.exists().getOrThrow
                            lock.release()
                            // LOCK file must be gone after release
                            val lockGone = !(dir / "LOCK").unsafe.exists().getOrThrow
                            lockExists && lockGone
        ...
        "case 4: LOCK with unparseable content fails closed, no reclaim" in {
            for
                ...
                lockStillPresent <- Sync.Unsafe.defer { (dir / "LOCK").unsafe.exists().getOrThrow }
            yield result match
                case Result.Failure(e: JournalStorageError) =>
