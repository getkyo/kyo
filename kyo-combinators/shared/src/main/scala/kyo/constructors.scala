...
extension (kyoObject: Kyo.type)
    def acquireRelease[A, S](acquire: => A < S)(release: A => Unit < IO): A < (S & Resource) =
        acquire.map(a => Resource.ensure(release(a)).as(a))

    def addFinalizer(finalizer: => Unit < IO): Unit < Resource =
    ...
    def fromAutoCloseable[A <: AutoCloseable, S](closeable: => A < S): A < (S & Resource) =
    ...
    def provideFor[D, SD, A, SA, E, SR](
        ...
        dependency.map(d => Env.run[D, A, SA, E, SR](d)(effect))
