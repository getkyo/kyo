    ...
    "aborts" - {
        "construct" - {
            "should construct from either" in {
                ...
                assert(Abort.run[String](effect).pure == Left("failure"))
            }
            ...
            "should construct from try" in {
                ...
                assert(Abort.run[Throwable](effect).pure.left.toOption.get.getMessage == "failure")
            }
        ...
        "handle" - {
            "should handle" in {
                ...
                val effect2 = Abort.get[Boolean, Int](Right(1))
        ...
        "convert" - {
            ...
            "should convert some aborts to options" in {
                ...
                val handledFailureOptions = Options.run(failureOptions)
                val handledFailureAborts  = Abort.run[Boolean | Double | Int](handledFailureOptions)
                ...
                val handledSuccessOptions = Options.run(successOptions)
                val handledSuccessAborts  = Abort.run[Boolean | Double | Int](handledSuccessOptions)
            ...
            "should convert some aborts to choices" in {
                val failure: Int < Abort[String | Boolean | Double | Int] =
                    Abort.fail("failure")
                val failureSeqs: Int < (Choices & Abort[Boolean | Double | Int]) =
                    failure.someAbortsToChoices[String]
                val handledFailureSeqs   = Choices.run(failureSeqs)
                val handledFailureAborts = Abort.run[Boolean | Double | Int](handledFailureSeqs)
                assert(handledFailureAborts.pure == Right(Seq.empty))
                val success: Int < Abort[String | Boolean | Double | Int] = 23
                val successSeqs: Int < (Choices & Abort[Boolean | Double | Int]) =
                    success.someAbortsToChoices[String]
                val handledSuccessSeqs   = Choices.run(successSeqs)
                val handledSuccessAborts = Abort.run[Boolean | Double | Int](handledSuccessSeqs)
