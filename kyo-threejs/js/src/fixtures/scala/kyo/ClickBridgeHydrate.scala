...
/** The client hydrate entry point for the kyo-threejs interaction-half browser test
  ...
  * either canvas posts a `BackendEvent` the server resolves. The test observes the click's effect
  * through the rendered DOM label, so this entry carries no mount-state sentinel of its own beyond the
  * plain readiness flag.
  */
object ClickBridgeHydrate:
    ...
    private def hydrateAndWait(using Frame): Unit < (Async & Scope) =
        for
            ...
            _ <- Sync.defer(dom.window.asInstanceOf[js.Dynamic].bridgeReady = true)
            _ <- Async.never
        yield ()
