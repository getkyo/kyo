...
private[kyo] object ThreeToImage:
    ...
    /** PNG-encodes an RGBA pixel buffer into the byte array `Image.fromBinary` consumes. */
    def encodePng(pixels: Uint8Array, width: Int, height: Int)(using Frame): Array[Byte] < Sync =
        // Unsafe: encode the RGBA buffer to PNG bytes (a pure byte transform deferred for effect placement).
        Sync.Unsafe.defer(Png.encode(pixels, width, height))
