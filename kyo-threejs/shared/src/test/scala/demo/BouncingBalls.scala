...
/** The scene-graph builder for the bouncing-balls demo: many meshes with per-`onFrame` physics, the
  * frame-loop and many-object showcase. Each ball carries its own `SignalRef[(Three.Vec3, Three.Vec3)]` holding
  ...
  * pattern. `CanEqual` for the `(Three.Vec3, Three.Vec3)` pair derives automatically from `Three.Vec3 derives CanEqual`.
...
object BouncingBallsScene:
    ...
    /** 24 spheres each carrying a `SignalRef[(Three.Vec3, Three.Vec3)]` (position, velocity) and an `onFrame`
    ...
    def scene(using Frame): Three.Ast.Scene < Sync =
        ...
        for
            refs <- Kyo.foreach(Chunk.from(0 until count)) { i =>
                val pos = Three.Vec3(i % 6 - 2.5, 4.0 + i * 0.3, i / 6 - 1.5)
                val vel = Three.Vec3(0, 0, 0)
                Signal.initRef((pos, vel))
            }
            balls = refs.map { stateRef =>
                Three.mesh(
                    Three.Geometry.sphere(0.4),
                    Three.Material.standard(color = Three.Color.cyan)
    ...
    def camera(using Frame): Three.Ast.Camera =
        Three.Camera.perspective(
            position = Three.Vec3(0, 4, 14),
            lookAt = Three.Vec3(0, 2, 0)
        )
    ...
    private def bounce(pos: Three.Vec3, vel: Three.Vec3, dt: Double): (Three.Vec3, Three.Vec3) =
        val vy1 = vel.y + gravity * dt
        val ny  = pos.y + vy1 * dt
        if ny < floor then
            (Three.Vec3(pos.x, floor, pos.z), Three.Vec3(vel.x, -vy1 * restitution, vel.z))
        else
            (Three.Vec3(pos.x, ny, pos.z), Three.Vec3(vel.x, vy1, vel.z))
        end if
    end bounce
