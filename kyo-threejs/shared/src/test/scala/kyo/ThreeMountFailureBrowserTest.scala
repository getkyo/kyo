...
/** Pins that the DIRECT `Three.runMount` entry surfaces a mount-pipeline failure through its typed
  ...
  * scaladoc/README promise the typed channel. The `mountFailureInspect` entry runs `runMount` and
  * projects the OUTCOME (there is no mount to install `ThreeInspect` over); the serving page commits the
  * canvas to a 2D context FIRST, so a WebGL context can never be created on it (deterministic,
  * GPU-independent), and the production `makeRenderer` maps that to the typed `WebGLUnavailable` leaf.
  * The test asserts the outcome is "typed:WebGLUnavailable", never "success".
  *
...
class ThreeMountFailureBrowserTest extends WebGLSceneHarness:
                ...
                swiftshaderLaunch.map { launch =>
                    Browser.run(launch) {
                        for
                            _ <- Browser.goto(url)
                            _ <- awaitProbeError
                            _ <- Browser.waitFor("window.failureOutcome !== undefined && window.failureOutcome !== ''")
                                .handle(diagnoseOnTimeout)
                            outcome <- Browser.eval("String(window.failureOutcome)")
                        yield assert(
    ...
    /** Reads the fixtures bundle, three.js build, and the jsm stack the linked bundle imports, and serves
    ...
    private def awaitProbeError(using Frame): Unit < (Browser & Abort[BrowserReadException]) =
        Browser.eval("window.failureError || ''").map { err =>
    ...
    private def diagnoseOnTimeout(
        ...
        Abort.recover[BrowserReadException] { _ =>
            for
                err     <- Browser.eval("String(window.failureError)")
                outcome <- Browser.eval("String(window.failureOutcome)")
            yield Abort.fail(BrowserScriptErrorException(
                s"runMount outcome probe never reported an outcome: failureError='$err' outcome=$outcome"
            ))
        }(wait)
...
object ThreeMountFailureBrowserTest:
    ...
    /** The probe page: an import map for `three`, the canvas, and the `mountFailureInspect` call.
    ...
    private[kyo] val page: String =
        """<!doctype html>
          ...
          |    const { mountFailureInspect } = await import("/main.js");
          |    mountFailureInspect("#app");
          |} catch (e) {
          |    window.failureError = String(e && e.message ? e.message : e);
          |}
          |</script>
          |</body>
          |</html>""".stripMargin
