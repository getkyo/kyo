...
class ThreeAstTest extends kyo.test.Test[Any]:
    ...
    "a texture handle for a url carries the url" in {
        val url    = "/textures/test.png"
        val handle = Three.Ast.Texture.FromUrl(url)
        assert(handle == Three.Ast.Texture.FromUrl(url))
        handle match
            case Three.Ast.Texture.FromUrl(u) => assert(u == url)
    }
