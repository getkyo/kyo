...
/** Tests for the server-side `onClick` resolution walk (`resolveOnClick`, private on `object Three`)
...
class ThreeBackendEventTest extends ThreeTest:
    ...
    "a malformed encoded payload is a silent no-op: the dispatch completes with no observable side effect" in {
        for
            ...
            _   <- scene.dispatchBackendEvent(Seq("0"), PointerWire.encode(testPointer)) // sentinel: the session survived
            got <- channel.take
