...
class TextureLoaderTest extends ThreeTest:
    ...
    "a texture handle for a url has the correct url" in {
        val handle = Three.Ast.Texture.FromUrl(testUrl)
        assert(handle == Three.Ast.Texture.FromUrl(testUrl))
        handle match
            case Three.Ast.Texture.FromUrl(url) => assert(url == testUrl)
    }
