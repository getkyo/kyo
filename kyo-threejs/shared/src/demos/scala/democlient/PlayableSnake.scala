...
/** The PlayableSnake demo, as a standalone app: it serves its own page and its own assets, and nothing
  * else in the repository has to exist for it to run.
  *
  * A static page: it imports this demo's own `mountPlayableSnake` entry from the linked demos bundle and
  * calls it against the page's `<div>`, so the game runs entirely in the browser through `UI.runMount`.
  * The keyboard is bound at the document level, so the arrow keys steer as soon as the page loads, with no
  * click to focus anything first.
  *
  * Link the browser bundle first (`sbt kyo-threejsJS/demos/fastLinkJS`), then run this launcher on any host
  * platform (`sbt 'kyo-threejsJVM/demos/runMain democlient.PlayableSnake'`, or the same under
  * `kyo-threejsJS/demos` to serve from Node).
  */
object PlayableSnake extends KyoApp:
    ...
    run {
        for
            ...
            _ <- Console.printLine(
                s"PlayableSnake running on http://localhost:${server.port}/  " +
                    "(steer with the arrow keys, WASD, or the on-screen buttons)"
            )
            _ <- server.await
        yield ()
    }
    ...
    private def page: String =
        s"""<!doctype html>
           ...
           |<head>
           |<meta charset="utf-8">
           |<title>kyo-threejs PlayableSnake</title>
           ...
           |<script type="module">
           |try {
           |    const { mountPlayableSnake } = await import("$bundlePath");
           |    mountPlayableSnake("#app");
           |} catch (e) {
    ...
    private val bundlePath: String = "/playablesnake.js"
