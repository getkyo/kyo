...
- **Every GL resource disposed exactly once.** Every GPU resource (geometry, material,
  ...
  browser-required (see the real-GL gate below).
...
a `Maybe` return, typed `loadGltf` failure, keyed reuse, the `position(v)` →
...
- `shared/src/main` holds the pure, FFI-free surface (the AST, `Bound`, `Color`/`Radians`/`Normal`/
  `Vec3`, `Pointer`, `ThreeFrames`, `Asset`, `ThreeException`). This is Node-testable without a GPU.
...
(`Color.scala` → `ColorTest.scala`), and no orphan or scratch test survives a finished change.
