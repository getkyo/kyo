...
`Pointer` and the event's `PointerKind` and walks the node's AST down the relative path
(`resolvePointer`) to run the handler that kind names (`onClick`, `onPointerOver`, `onPointerOut`): a
pure server-side read of the same authoritative signals the structural region drives, no
...
- **A region inside a region is a region.** Structural regions are registered in `Reconciler.record`,
  the one place every `Reactive` / `Foreach` holder is created, and retired in `removeFromLive` when the
  element holding them is disposed. They are NOT discovered once from the live map: a region's content is
  materialized strictly after that region first reconciles, so a `Three.when(visible)(items.foreach(...))`
  built from a once-computed list would leave the inner holder an empty `Group` that nothing fills and
  nothing watches, rendering nothing at all with no error. Two rules follow, and both are load-bearing:
  a freshly materialized element activates the regions it brought (`activateRegions`: fill, then hand to
  the mount's `subscribeRegion` hook), and a region's watcher starts from the value its fill already
  reconciled (`ReactiveRegion.lastDriver`), or every level of nesting would rebuild itself once more than
  the level above it. Guarded by `ReconcilerTest` ("a foreach NESTED inside a when renders", "a nested
  region keeps WATCHING", "a reactive NESTED inside a foreach item renders", "disposing an element
  RETIRES the region nested inside it", "the startup fill and the region's watcher do not BOTH reconcile
  the same value").

- **An op the client cannot apply is reported, never dropped in silence.** A dropped prop freezes one
  attribute of one object while everything else on it keeps animating, so it is invisible unless it says
  so. `applyByKey` returns the REASON it failed, `ThreeBackend` warns once per mount, path and reason,
  and `ThreeMount` warns once per declaration site when a pointer event lands on an object that declares
  a handler no path resolves to. The dedupe set lives on the MOUNT, not on the backend object: pages
  re-hydrate and suites mount scene after scene, and a set shared across mounts would swallow the next
  mount's genuine drop as a duplicate. Guarded by `ThreeBackendTest` ("applyByKey REPORTS the two ways a
  prop can fail to apply", "a drop is reported once per mount, path AND reason") and `ThreeMountTest`
  (the `PointerRoute` leaves).
...
- **Server-side pointer handlers resolve against the authoritative signals.** A client raycast on a
  server-driven scene posts a `BackendEvent` whose `path` the server resolves through
  `dispatchBackendEvent` / `resolvePointer`; a foreach child is re-read fresh at dispatch, a keyed
  child is addressed by key, and a malformed payload is a silent no-op. All three interactions take
  this path, not just clicks: the event carries a `PointerKind`, so a hover on a server-driven page
  runs the session's `onPointerOver` rather than a client-local copy whose signals nothing renders.
  Guarded by `ThreeBackendEventTest`; proven end to end over the WS by `ThreeStructuralBridgeBrowserTest`.
...
## Platform: four platforms, split along the GL boundary

kyo-threejs is a `crossProject(JSPlatform, JVMPlatform, NativePlatform, WasmPlatform)`. WebGL, the DOM,
and three.js exist only in the browser, so the RENDER runs on the client (js/wasm) alone. But the
scene AST a server constructs, SSRs, and drives by server push does NOT need GL, so it is
cross-platform: a JVM (or Native) server builds a `Three.embed` page, SSRs its `<canvas>` placeholder,
and drives the scene through kyo-ui's server push, while the client island renders it. The split is by
source dir, along that GL boundary:

- `shared/src/main` (all four platforms): the pure FFI-free surface, the AST (`Three.scala`), `Bound`,
  `Color`/`Radians`/`Normal`/`Vec3`, `Pointer`, `ThreeFrames`, `ThreeException`, `KeyedEntry`, and the
  `boundPropsOf` / `structuralRegion` server-walk halves. No `scala.scalajs.js` or `org.scalajs.dom`
  here: `shared` must compile on the JVM and Native. `Ast.Custom.build` is typed `In => Any` so the
  escape-hatch node stays cross-platform (the client reconciler casts the result to `js.Dynamic`).
- `js-wasm/src/main` (js + wasm only, the `CrossType.Full` partial-share dir): every three.js / WebGL /
  DOM binding, the facade, the reconciler, `ThreeMount`, `ThreeBackend`, the loaders, `ThreeToImage`,
  `Asset`, and the client-only `Three` surface (the `ThreeRunnerOps` mixin carrying `runMount` /
  `loadGltf` / `toImage` / `testDriver` / `texture` / `custom`, plus the `mount.canvas` /
  `renderer.unsafe` and custom-geometry/material extensions in `ThreeClient`). This dir may freely
  import `scala.scalajs.js` and `org.scalajs.dom`. A new FFI source must compile under both backends
  from one file: use `@JSImport`, never `js.Dynamic.global.require` (`require` is not a global under the
  ESModule the Wasm backend mandates).
- `jvm-native/src/main` (jvm + native only): the stubs the shared surface needs where there is no GL,
  the `ThreeBackendHook` no-op and the empty `ThreeRunnerOps`.

The client runners are companion MEMBERS via the `ThreeRunnerOps` mixin, never top-level extensions: a
top-level `runMount` extension would clash with kyo-ui's `UI.runMount`.

Source and test layout:

- `shared/src/test`: the pure AST/wire suites; they run on all four platforms.
- `js-wasm/src/test`: the GL suites (they drive real three.js on Node and read live GL objects) and the
  GL test fixtures; they run on js and wasm.
- `js/src/test`: the surface that needs a real browser (the WebGL submit and in-browser tests). That is
  the divergence the root's "all platforms, shared tests" rule allows for genuinely platform-specific
  behavior. Do not move a JVM/Native-testable test into a client tree to dodge anything.
...
- **The in-browser WebGL tests** (`ThreeToImageBrowserTest`, `ThreeMountBrowserTest`, `WebGLAcceptanceTest`,
  ...
  `headedSwiftshaderLaunch` / `headless=false` and `--enable-unsafe-swiftshader`. Each of these serves
  the linked `fixtures` configuration's bundle and drives one of its `@JSExportTopLevel` entries by name. The
  build links that bundle for you: it is a prerequisite of `kyo-threejsJS`'s `Test / compile`, so a plain
  `kyo-threejsJS/test` or `testOnly` already runs the browser leaves against the bundle THIS tree just
  produced. Do not link it by hand and do not remove that wiring: the harness reads the bundle off the
  filesystem and cannot tell a stale one from a fresh one, so an unlinked run would go GREEN against code
  the browser never loaded.
...
Each demo (`ControlPanel`, `SceneEditor`, `PlayableSnake`, `ServerViz`, `GltfInspector`, `Flagship`) is
a **fully isolated application**: an `object` in package `democlient` that starts its own `HttpServer` on
Node and serves its own page and its own assets. There is no shared demo infrastructure, and adding some
is not an option: if a demo needs something the library does not offer, that is a library gap to fix in
the library. Scala.js has no `Test/runMain`, so a demo runs by selecting its app as the main of the
`demos` configuration on `kyo-threejsJS` and running it:
...
The `set` picks the app (`democlient.ControlPanel` is the default main, so it can be dropped for that
demo) and `demos/run` serves it on Node, linking the demos configuration's link output the page imports
as a prerequisite. It spells the key as `LocalProject("kyo-threejsJS") / ConfigKey("demos") / mainClass`
rather than `kyo-threejsJS/demos/mainClass` because `set` parses the token as Scala and the hyphens in
the project id break that parse. The app prints `http://localhost:<port>/` to open. There is **no shared
dispatcher**: one `run` launches exactly one demo.

A demo that serves an asset owns that asset. `GltfInspector` serves a real glTF file
(`js/src/demos/models/cube.gltf`) at the same route value it hands the page, so the URL the page asks for
and the route the server answers cannot drift apart, and a missing file fails at startup with a typed
`FileException` rather than as a 404 that renders a black canvas.

The **demos source set** (`kyo-threejs/js/src/demos/scala/`) rides a single Compile-like `demos`
configuration on `kyo-threejsJS`, not a project of its own: the `demo` package holds each scene and its
`@JSExportTopLevel` browser entry, the `democlient` package holds the `KyoApp` launchers. Its one link
output splits both sides of every demo across ESModules: the launcher (the configuration's `mainClass`)
lands in `main.js` as a `KyoApp` that starts the `HttpServer`, and each demo's browser entry is exported
into its OWN module via `@JSExportTopLevel(name, "<demoid>")`. A served page imports only its own demo
module (no `node:*` paths there, so the linker's dead-code elimination drops the HttpServer code); the
launcher's Node code never reaches the browser.

Each demo app reads its browser module from disk at request time, so the build links the `demos`
configuration output as a prerequisite of `demos/run`: launching a demo serves what THIS tree just
produced. That matters because running a demo by hand is how a fix gets checked against a real browser,
and a stale link would make the demo look fixed, or look broken, for reasons unrelated to the code in
front of you.

**The demos configuration carries the six applications and nothing else.** A browser test needs its own
`@JSExportTopLevel` entries (a probe that installs `ThreeInspect` over a mount, a hydrate entry that
rebuilds an SSR'd fixture tree), and those live in a SEPARATE `fixtures` configuration (sources under
`kyo-threejs/js/src/fixtures/scala/`, plus the `fixture` scenes from `shared/src/test/scala/fixture/`).
The build links it as a prerequisite of `kyo-threejsJS`'s `Test / compile`, and the browser suites read
it through `WebGLSceneHarness.readFixtureBundle`. Adding a test entry to the demos configuration is the
mistake this split exists to prevent: it makes the demos read as harness code, and a demo is an
application someone runs.

The **gallery** (`ThumbnailGallery`, `shared/src/test/scala/fixture/ThumbnailGallery.scala`) renders
scenes to PNGs via `Three.toImage` for visual review. It needs a browser WebGL context (a real
software-WebGL Chrome) and is not part of the automated test suite; it is run manually.
