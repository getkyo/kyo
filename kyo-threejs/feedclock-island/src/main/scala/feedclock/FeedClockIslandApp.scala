...
/** The FeedClock per-app island: the self-running
  * client bundle the [[democlient.FeedClock]] launcher links into its page through `head.moduleScript`.
  *
  * The `kyo-threejs-feedclock-island` project links this object as its main module initializer, so
  * loading the bundled ESModule on the page runs [[main]] without a separate bootstrap. The body mounts
  * the FeedClock scene at the page's `#app` canvas via [[DemoMounts.mountFeedClock]], which runs the real
  ...
  */
object FeedClockIslandApp:

    def main(args: Array[String]): Unit =
        DemoMounts.mountFeedClock("#app")

end FeedClockIslandApp
