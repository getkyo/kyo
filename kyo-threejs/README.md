...
`Three.runMount(scene, camera, selector, frames)` resolves the `<canvas>` at `selector`, acquires a `WebGLRenderer` into it under `Scope`, materializes the scene, forks one observe fiber per reactive prop, wires pointer delegation, and runs the frame loop until the scope closes. The whole effect is `Three.Mount < (Async & Scope & Abort[ThreeException])`: it produces a live `Three.Mount` handle (see below), teardown disposes the renderer and every GPU resource, and a missing canvas or absent WebGL context surfaces as a typed `ThreeException` through `Abort`.
...
A mount is worth holding onto, not fire-and-forget: `runMount` yields a live `Three.Mount` rather than `Unit`, and on the embed side `Three.Embedded.mounted: Signal[Maybe[Three.Mount]]` carries the same handle, `Absent` until the embed's `<canvas>` mounts on the client page, then `Present(mount)` for the rest of the mount's lifetime. Observe it to drive render-aware app logic: `renders: Signal[Long]` counts committed frames, `disposed: Signal[Boolean]` fires once when the mount's `Scope` closes, `canvas` and `renderer: Three.Renderer` are the resolved live objects, and `readPixels(x, y, width, height)` reads the live framebuffer as row-major RGBA bytes, typed `Span[Byte] < (Async & Abort[ThreeException])`: a read after `disposed` or out of bounds completes `ThreeException.RenderFailure`, never a throw. `Three.Renderer` itself adds `contextLost: Boolean < Sync` and the sanctioned `unsafe: js.Dynamic` escape.

The example below observes an embedded scene from the mount side (embedding itself is covered in [Embedding as a child node](#embedding-as-a-child-node)):
...
val tracked =
    for
        label <- Signal.initRef("Frames: 0")
        scene    = Three.scene(Three.Light.ambient(), Three.mesh(Three.Geometry.sphere(1.0), Three.Material.standard()))
        camera   = Three.Camera.perspective(position = Three.Vec3(0, 2, 6), lookAt = Three.Vec3.zero)
        embedded = Three.embed(scene, camera)
        _ <- embedded.mounted.observe {
            case Present(m) => m.renders.observe(n => label.set(s"Frames: $n"))
            case Absent     => Kyo.unit
        }
    yield UI.div(embedded, UI.p(label))
```

This is exactly what the Flagship demo does: observe `embed(...).mounted` to drive a frame counter off `renders` and a pixel capture off `readPixels`.
...
The opening sections all run the scene client-side: closures compiled into the browser, the frame loop and raycast local. `Three.embed` also supports a SERVER-DRIVEN mode: the server owns an ordinary `SignalRef` a scene's `.color(signal)`/`.position(signal)` setter binds to, and every `.set` reaches the client over kyo-ui's WebSocket through the SAME bound-prop path a plain kyo-ui element uses. The client still owns the frame loop and the raycast; only the DATA a bound prop observes is server-owned.

That server is any platform kyo-ui runs on. The scene AST, the SSR'd `<canvas>` placeholder, and the Schema-encoded payloads the server pushes carry no WebGL, so a JVM (or Native) backend constructs the page, SSRs it, and drives it under `UI.runHandlers`, while the browser client (the js/wasm island) renders the 3D. WebGL stays on the client; everything the server touches is cross-platform.
...
val serverDriven =
    for
        color <- Signal.initRef(0xff0000)
        cube = Three.mesh(
            Three.Geometry.box(2.0, 2.0, 2.0),
            // The cube's color follows the server-owned signal; a spin keeps the client frame loop running.
...
The cube spins from the local `onFrame` while its color steps whenever a server fiber sets `color`: animation and server-driven reactivity on the same object at once.

A click on the cube is raycast LOCALLY (the client tests its own live scene) and posted to the server addressed by the hit node's path; the server resolves the SAME `onClick` closure the scene was built with and runs it there, so the closure mutates server state directly, with no separate event type or routing id.
...
val clickable =
    for
        ...
        cube = Three.mesh(Three.Geometry.box(2.0, 2.0, 2.0), Three.Material.standard())
            .onClick(_ => clicks.updateAndGet(_ + 1).unit)
    yield cube
```

`UI.runHandlers(basePath, head)(ui)` is the serve entry (the same one every server-push kyo-ui page uses): it returns the HTTP handlers (an SSR page GET and a WebSocket route) composed with your static handlers via `HttpServer.init`. Embedding `Three.embed(scene, camera)` directly in the served tree makes the scene server-driven: the page links a client bundle through `head.moduleScript` that rebuilds the SAME tree (so `data-kyo-path` matches the server's SSR markup) and hydrates the embedded canvas onto it.
...
val served =
    for
        ...
        handlers <- UI.runHandlers(
            "",
            UI.PageHead("3D App", moduleScript = Present("/app.js"), importMap = Seq("three" -> "/three.module.js"))
        ) {
            for
                color <- Signal.initRef(0xff0000)
                // A server fiber cycles the color; each set reaches the client through the ordinary
                // bound-prop path.
                _ <- Fiber.initUnscoped(color.set(0x00ff00))
                cube =
                    Three.mesh(Three.Geometry.box(2.0, 2.0, 2.0), Three.Material.standard().color(color.map(rgb => Three.Color(rgb))))
                scene  = Three.scene(Three.Light.ambient(intensity = 1.0), cube)
                camera = Three.Camera.perspective(position = Three.Vec3(0, 0, 4), lookAt = Three.Vec3.zero)
            yield UI.div(Three.embed(scene, camera).id("app"))
        }
        server <- HttpServer.init(0, "localhost")(handlers*)
    yield server
```

The client half is an ordinary Scala.js bundle: a small `@JSExportTopLevel` entry that rebuilds the SAME tree client-side and hydrates it onto the SSR'd page through the public `UI.runHydrate` entry, so the tree's `Three.embed` node's mount registers on the already-rendered canvas. Link it through `head.moduleScript` and map its bare `three` import through `head.importMap` to a served three module, so a plain `fastLinkJS`/`fullLinkJS` ESModule loads with no separate bundler step. The kyo-threejs demos ship runnable examples: the `ServerViz` app serves a bar chart a server fiber rewrites several times a second, and the `Flagship` app serves a cube that does all four at once (client spin, server-driven color, click-driven scale resolved server-side, and an orbit camera). See "Running the demos" below for how to launch them.
...
Six demos, written to be read in order: each one adds a single idea to the one before it, and each scene's scaladoc says which idea it is. The scenes live in [`js/src/demos/scala/demo`](js/src/demos/scala/demo), part of the `demos` configuration, and are compile-checked on the JS backend by kyo-threejs's own test build. Each demo is a standalone app that serves its own page and its own assets; see [Running the demos](#running-the-demos) for how to launch one.

| Demo | The idea it adds | Scene |
|------|------------------|-------|
| ControlPanel | A kyo-ui control and a 3D prop are two views of ONE signal, so there is no wiring between them: no listener, no update method, no re-render call. | [ControlPanelScene](js/src/demos/scala/demo/ControlPanelScene.scala) |
| SceneEditor | The same signal, driven the other way: click a 3D object and the panel reflects it. Each object owns its own refs, so editing one never touches the other. | [SceneEditorScene](js/src/demos/scala/demo/SceneEditorScene.scala) |
| PlayableSnake | Real input and real game state, with no new primitive: keyboard through kyo-ui's `UIWindow.onKeyDown`, the board stepped by a `Group.onFrame` ticker. | [PlayableSnakeScene](js/src/demos/scala/demo/PlayableSnakeScene.scala) |
| ServerViz | A server fiber writes an ordinary `SignalRef` and the scene follows, across the network. The scene code cannot tell who called `.set`. | [ServerVizScene](js/src/demos/scala/demo/ServerVizScene.scala) |
| GltfInspector | Asynchronous asset loading as a typed, `Scope`-managed effect, plus orbit and zoom as ordinary scene nodes. A failed load renders its own typed error. | [GltfInspectorScene](js/src/demos/scala/demo/GltfInspectorScene.scala) |
| Flagship | Every idea above on one cube, plus the live `Three.Mount` handle: a frame counter reading `mount.renders` and a capture button reading real pixels back off the canvas. | [FlagshipScene](js/src/demos/scala/demo/FlagshipScene.scala) |

### Running the demos

The demos are client (js) apps, and Scala.js has no `Test/runMain`, so `sbt 'kyo-threejsJS/Test/runMain demo.ControlPanel'` does not work. The demos ride a Compile-like `demos` configuration on `kyo-threejsJS`, not a project of their own; each demo is a standalone app: an `object` in package `democlient` that serves its own page and its own assets. Select the app as the configuration's main and run it:

```sh
sbt 'set LocalProject("kyo-threejsJS") / ConfigKey("demos") / mainClass := Some("democlient.ServerViz")' \
    'kyo-threejsJS/demos/run'
```

The `set` picks the app and `kyo-threejsJS/demos/run` serves it on Node, linking the demos configuration's link output (the launcher's `main.js` plus each demo's own browser module) as a prerequisite, so what you open is what you just built. It spells the key as `LocalProject("kyo-threejsJS") / ConfigKey("demos") / mainClass` because `set` parses the token as Scala and the hyphens in `kyo-threejsJS/demos/mainClass` break that parse. `democlient.ControlPanel` is the configuration's default main, so for that demo the `set` can be dropped: `sbt 'kyo-threejsJS/demos/run'`. The app prints a `http://localhost:<port>/` URL; open it. One `run` serves exactly one demo. Swap the `mainClass` for any of the six:

| App main | Renders |
|----------|---------|
| `democlient.ControlPanel` | A cube whose colour, size and spin are driven by kyo-ui controls |
| `democlient.SceneEditor` | A cube and a sphere; click one to select it, then edit it in the panel |
| `democlient.PlayableSnake` | A playable 3D Snake, steered with the arrow keys, WASD, or the on-screen buttons |
| `democlient.ServerViz` | A six-bar chart a server fiber rewrites several times a second |
| `democlient.GltfInspector` | A loaded glTF model you can orbit and zoom |
| `democlient.Flagship` | One cube carrying every idea at once, plus a live frame counter and a pixel capture |

The four client-mounted apps (ControlPanel, SceneEditor, PlayableSnake, GltfInspector) serve a static page and run the whole scene in the browser through `UI.runMount`. The two server-driven apps (ServerViz, Flagship) serve the tree through `UI.runHandlers` instead: the page links a mount shim that imports the demo's hydrate entry and an import map that resolves `three`, so the server drives the scene over the WebSocket while it still animates client-side.

`fixture.ThumbnailGallery` renders scenes to PNGs headless through `Three.toImage`, writing to `runs/thumbnails/`. It needs a browser WebGL context, so it does not run through a demo `run` on Node. The `toImage` primitive is validated by `ThreeToImageBrowserTest` and `WebGLAcceptanceTest` in a real software-WebGL Chrome.
