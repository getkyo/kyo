...
class ThreeEmbedTest extends ThreeTest:
    ...
    // The construction invariants (mounted Absent before mount, the SAME mountedRef surviving an .id()
    // copy, and the frames default/round-trip) are FFI-free and covered cross-platform by
    // ThreeEmbedServerTest in the shared test tree.
