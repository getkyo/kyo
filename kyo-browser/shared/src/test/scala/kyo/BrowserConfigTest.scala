...
class BrowserConfigTest extends BrowserTest:
    ...
    "assertExists with custom retry schedule honors that schedule's maxDuration" in run {
        withBrowser {
            onPage("<div>nothing here</div>") {
    ...
    "executable returns updated launch config" in run {
    ...
    "headless returns updated launch config" in run {
    ...
    "launchTimeout returns updated launch config" in run {
    ...
    "retrySchedule returns updated session config" in run {
    ...
    "loadSchedule returns updated session config" in run {
    ...
    "mutationQuiescenceWindow returns updated session config" in run {
        val cfg = Browser.SessionConfig.default.mutationQuiescenceWindow(25.millis)
        assert(cfg.mutationQuiescenceWindow == 25.millis)
    }

    "mutationSettlementTimeout returns updated session config" in run {
    ...
    "currentConfig returns default session config at outermost scope" in run {
        withBrowser {
    ...
    "withConfig installs session config for the duration of the block" in run {
    ...
    "withConfig restores outer session config after the block" in run {
        withBrowser {
    ...
    "nested withConfig applies innermost session config" in run {
    ...
    "nested withConfig restores outer session config after inner block" in run {
        val outer = Schedule.fixed(30.millis).take(4)
        val inner = Schedule.fixed(10.millis).take(2)
        withBrowser {
            Browser.withConfig(_.retrySchedule(outer)) {
    ...
    "withConfig(f) applies f to current session config, not default" in run {
    ...
    "retrySchedule affects retry count for non-existent element" in run {
        withBrowser {
    ...
    "networkIdleWindow returns updated session config" in run {
    ...
    "mutationFirstMutationGrace returns updated session config" in run {
    ...
    "BrowserSetupException is raised when launch executable is missing" in run {
