...
class CdpBackendTest extends kyo.BaseBrowserTest:
            ...
            mkBackendWithServer(Seq(navMethod)).map { (backend, _) =>
                Abort.run[BrowserReadException](CdpBackend.getNavigationHistory(backend)).map {
                    case Result.Failure(e: BrowserProtocolErrorException) =>
                        ...
                        // A decode failure surfaces as JsonRpcInternalError(Operation.DecodeResult); its message carries
                        // the "result decode" marker. Pinning it catches a regression that mis-routes a server-signalled
                        // coded error (which has a different message) through the same method.
    ...
    "CdpBackend.runtimeEvaluate returns the typed EvalResult" in {
        Scope.run {
            ...
            mkBackendWithServer(Seq(method)).map { (backend, _) =>
                Abort.run[BrowserReadException](CdpBackend.runtimeEvaluate(backend, EvalParams("'hello'"))).map {
                    case Result.Success(env) =>
                        // The engine decodes the reply through Schema[EvalResult]; the value arrives typed.
                        assert(env.result == RemoteObject.`string`(value = "hello"), s"expected string 'hello' but got ${env.result}")
                        assert(env.exceptionDetails == Absent, s"expected no exceptionDetails but got ${env.exceptionDetails}")
                    case other => fail(s"Expected Success but got $other")
                }
            }
        }
            ...
            mkBackendWithServer(Seq(method)).map { (backend, _) =>
                Abort.run[BrowserReadException](CdpBackend.runtimeEvaluate(backend, EvalParams("1+1"))).map {
                    case Result.Failure(e: BrowserProtocolErrorException) =>
                        ...
                        // Server-signalled coded error: the numeric CDP code (-32601, method not found) reaches the caller.
    ...
    // -------------------------------------------------------------------------
    // recoverContextDestroyed (pure): shared Runtime.evaluate-family translator
    // -------------------------------------------------------------------------

    "CdpBackend.recoverContextDestroyed translates a context-destroyed protocol error to BrowserIFrameInvalidException(ContextDestroyed)" in {
        val failing: Int < (Async & Abort[BrowserReadException]) =
            Abort.fail(BrowserProtocolErrorException(
                "Runtime.evaluate",
                s"CDP error: ${CdpErrorStrings.ContextDestroyedErrorMessage}"
            ))
        Abort.run[BrowserReadException](CdpBackend.recoverContextDestroyed(failing)).map {
    ...
    "CdpBackend.recoverContextDestroyed passes a successful value through unchanged" in {
        val ok: Int < (Async & Abort[BrowserReadException]) = 7
        Abort.run[BrowserReadException](CdpBackend.recoverContextDestroyed(ok)).map {
            case Result.Success(v) => assert(v == 7, s"expected value 7 passed through but got $v")
            case other             => fail(s"expected Success(7) but got $other")
        }
    }

    "CdpBackend.recoverContextDestroyed passes an unrelated protocol error through unchanged" in {
        val unrelated = BrowserProtocolErrorException("Runtime.evaluate", "some unrelated protocol failure")
        val failing: Int < (Async & Abort[BrowserReadException]) = Abort.fail(unrelated)
        Abort.run[BrowserReadException](CdpBackend.recoverContextDestroyed(failing)).map {
            case Result.Failure(e: BrowserProtocolErrorException) =>
                assert(e.error == unrelated.error, s"expected unrelated error propagated unchanged but got ${e.error}")
            case other => fail(s"expected the unrelated BrowserProtocolErrorException but got $other")
        }
    }
