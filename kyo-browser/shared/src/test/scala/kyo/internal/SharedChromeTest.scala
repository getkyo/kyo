...
class SharedChromeTest extends BaseBrowserTest:
    ...
    "a launch failure is retried by the next caller instead of failing every later call" in {
        ...
        val attempt = Abort.run[BrowserReadException | BrowserSetupException](shared.withUrl(url => url))
        for
            first    <- attempt
            second   <- attempt
    ...
    "callers that see the same dead Chrome relaunch it once" in {
    ...
    "a failure that does not mean the Chrome is gone is neither retried nor relaunched" in {
        ...
        for
            result <- Abort.run[BrowserReadException | BrowserSetupException] {
                shared.withUrl(_ => Abort.fail(BrowserProtocolErrorException("Page.navigate", "bad request", Absent)))
            }
            launches <- fake.launches
            again    <- shared.withUrl(url => Sync.defer(url))
        yield
