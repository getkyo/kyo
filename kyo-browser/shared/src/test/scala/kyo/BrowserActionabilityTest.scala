...
class BrowserActionabilityTest extends BrowserTest:
    ...
    "click on a visible+enabled+stable+hittable button succeeds" in {
        withBrowser {
            onPage("""<body>
            <button id="target" style="width:80px;height:30px"
    ...
    "click on display:none fails Browser.Reason.Hidden" in {
        withBrowser {
            onPage("""<body>
    ...
    "click on visibility:hidden fails Browser.Reason.Hidden" in {
        withBrowser {
            onPage("""<body>
    ...
    "click on zero-size element fails Browser.Reason.Hidden" in {
        withBrowser {
            onPage("""<body>
    ...
    "click on disabled <button> fails Browser.Reason.Disabled" in {
        withBrowser {
            onPage("""<body>
    ...
    "click on a covered button fails Browser.Reason.Covered" in {
        withBrowser {
    ...
    "click on mid-animation element eventually succeeds once stable" in {
        withBrowser {
            onPage("""<body style="margin:0">
            <button id="t" style="position:absolute;left:0;top:10px;width:80px;height:30px"
                    onclick="window.__kyoClicked=true">Go</button>
            <script>
                ...
                Browser.withConfig(_.retrySchedule(Schedule.fixed(50.millis).maxDuration(5.seconds))) {
                    ...
                    Browser.eval("String(!!window.__kyoClicked)").map { clicked =>
                        assert(clicked == "true", s"expected click handler to fire after retry succeeds but got $clicked")
                    }
                }
            }
        }
    ...
    "click on a detached element fails Reason.NotAttached" in {
        withBrowser {
            onPage("""<body>
    ...
    "exactly one actionability JS round-trip per check" in {
        withBrowser {
            onPage("""<body>
            <button id="t" style="width:80px;height:30px">Go</button>
        </body>""") {
    ...
    "Browser.Reason.NotVisible error message cites 'not visible' and the selector description" in {
        withBrowser {
            onPage("""<body>
    ...
    "Browser.Reason.Disabled matches native <input disabled>" in {
        withBrowser {
            onPage("""<body>
    ...
    "Browser.Reason.Disabled matches <button aria-disabled='true'>" in {
        withBrowser {
            onPage("""<body>
    ...
    "NotAttached short-circuits before Hidden" in {
    ...
    "retry succeeds on transient Unstable once motion stops" in {
        withBrowser {
            onPage("""<body style="margin:0">
            <button id="t" style="position:absolute;left:0;top:10px;width:80px;height:30px"
                    onclick="window.__kyoClicked=true">Go</button>
            <script>
                ...
                Browser.withConfig(_.retrySchedule(Schedule.fixed(50.millis).maxDuration(3.seconds))) {
                    ...
                    Browser.eval("String(!!window.__kyoClicked)").map { clicked =>
                        assert(clicked == "true", s"expected click handler to fire after Unstable retry but got $clicked")
                    }
                }
            }
        }
    ...
    "fill on a <div> target fails Browser.Reason.NotFillable" in {
        withBrowser {
            onPage("""<body>
    ...
    "hover on hidden element fails NotActionable" in {
        withBrowser {
            onPage("""<body>
    ...
    "dragAndDrop fails on target-side Disabled" in {
        withBrowser {
            onPage("""<body style="margin:0">
            ...
            <button id="tgt" disabled style="position:absolute;left:200px;top:0;width:80px;height:30px">T</button>
                ...
                fastConfig {
                    ...
                    case Result.Failure(ex: BrowserElementNotActionableException) =>
                        ex.reason match
                            case Reason.Disabled(Reason.DisabledKind.Attribute) => ()
    ...
    "Actionability.check returns Success on visible and Failure(Reason) on hidden" in {
        withBrowser {
            onPage("""<body>
    ...
    "ActionableRef carries backend node id and non-zero rect" in {
        withBrowser {
            onPage("""<body>
    ...
    "Actionability surfaces failures via Abort[BrowserException], never as a thrown exception" in {
        withBrowser {
            onPage("""<body>
            <button id="t" style="display:none">Go</button>
        </body>""") {
    ...
    "click on a below-the-fold element auto-scrolls into view" in {
        withBrowser {
            onPage("""<body style="margin:0">
    ...
    "click on an element inside a scrolled container auto-scrolls the container" in {
        withBrowser {
            onPage("""<body style="margin:0">
    ...
    "click on an element above the current scroll position scrolls up" in {
        withBrowser {
            onPage("""<body style="margin:0">
            <button id="t" style="width:120px;height:30px"
                    onclick="window.__kyoClicked=true">Go</button>
    ...
    "click on an already-visible element does not scroll" in {
        withBrowser {
            onPage("""<body style="margin:0">
    ...
    "two-RAF sample reports Unstable during a 2px/frame drift and succeeds after motion ends" in {
        withBrowser {
            onPage("""<body style="margin:0">
    ...
    "stability passes when the rect is identical across two RAFs" in {
        withBrowser {
            onPage("""<body style="margin:0">
    ...
    "below-viewport animating element is still flagged Unstable after scroll-into-view" in {
        withBrowser {
            onPage("""<body style="margin:0">
            <div style="height:2000px;background:#eef"></div>
    ...
    "sub-pixel drift passes stability" in {
        withBrowser {
            onPage("""<body style="margin:0">
    ...
    "hover auto-scrolls a below-the-fold element into view" in {
        withBrowser {
            onPage("""<body style="margin:0">
            <div style="height:2000px;background:#eef"></div>
    ...
    "element screenshot after auto-scroll captures non-empty bytes and has moved scrollY" in {
        withBrowser {
            onPage("""<body style="margin:0">
            <div style="height:2000px;background:#eef"></div>
    ...
    "NotActionable(NotVisible) message cites 'not visible' and selector description" in {
        withBrowser {
            onPage("""<body>
            <button id="t" style="display:none">Go</button>
        </body>""") {
    ...
    "NotActionable(Covered) message cites 'covered'" in {
        withBrowser {
            onPage("""<body style="margin:0">
            <button id="t" style="position:absolute;left:10px;top:10px;width:80px;height:30px">Go</button>
    ...
    "NotActionable(Disabled) message distinguishes native disabled from aria-disabled" in {
    ...
    "NotActionable(NotFillable) raised when fill targets a non-INPUT" in {
        withBrowser {
    ...
    "Frame propagates through actionability failures (call site appears in the Kyo frame chain)" in {
    ...
    "click on opacity:0 element fails NotVisible(OpacityZero)" in {
        withBrowser {
            onPage("""<body>
    ...
    "click on element inside <fieldset disabled> fails Disabled(FieldsetDisabled)" in {
        withBrowser {
            onPage("""<body>
            <fieldset disabled>
                <button id="t" style="width:80px;height:30px">Go</button>
    ...
    "OutsideHitTarget.actualHit carries non-empty covering element info" in {
        withBrowser {
            onPage("""<body style="margin:0">
            <button id="t" style="position:absolute;left:10px;top:10px;width:80px;height:30px">Go</button>
            <div id="overlay" style="position:absolute;left:0;top:0;width:300px;height:200px;background:rgba(0,0,0,0.2)"></div>
    ...
    "Reason.NotInViewport carries the element rect and viewport rect" in {
        withBrowser {
    ...
    "NotFillable.tagName carries the correct lowercase tag from the actionability probe" in {
        withBrowser {
            onPage("""<body>
    ...
    "NotVisible failure message embeds the element's bounding rect" in {
        withBrowser {
    ...
    "NotAttached failure message embeds the current page URL" in {
        withBrowser {
