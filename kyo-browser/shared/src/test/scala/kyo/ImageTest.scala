...
class ImageTest extends Test:
    ...
    "Image.fromBinary and Image.fromBase64 resolve via Browser.Image" in run {
    ...
    "Image.fromBase64(s).base64 == s for a fixture base64 string" in run {
        Browser.Image.fromBase64(fixtureBase64) match
            case Result.Success(img) => assert(img.base64 == fixtureBase64)
            case other               => fail(s"expected Success but got $other")
    }

    "Image.fromBase64(s).base64 == s for an empty string" in run {
    ...
    "Image.fromBase64 raises BrowserDecodingException on malformed input" in run {
    ...
    "Image.fromBinary(bytes).binary is byte-identical to bytes" in run {
    ...
    "Image.writeFileBinary(path: String) writes binary; re-reading yields identical bytes" in run {
        Scope.run {
    ...
    "Image.writeFileBinary(path: Path) writes binary; re-reading yields identical bytes" in run {
        Scope.run {
            Path.tempScoped("kyo-image-test", ".bin").map { tmp =>
    ...
    "Image.writeFileBase64(path: String) writes base64; re-reading yields original base64 string" in run {
        Scope.run {
    ...
    "Image.writeFileBase64(path: Path) writes base64; re-reading yields original base64 string" in run {
        Scope.run {
            Path.tempScoped("kyo-image-test", ".b64").map { tmp =>
    ...
    "Image.renderWith(ConsoleType.iterm) starts with ]1337;File=" in run {
    ...
    "Image.renderWith(ConsoleType.kitty) starts with _G" in run {
    ...
    "Image.ConsoleType.detect(Map.empty) returns Absent (no supported terminal)" in run {
    ...
    "Image value equality: two instances built from identical bytes compare equal" in run {
    ...
    "Image value equality: instances with differing bytes do not compare equal" in run {
    ...
    "ConsoleType.detect with TERM_PROGRAM=iTerm.app yields iterm" in run {
    ...
    "ConsoleType.detect with KITTY_WINDOW_ID set and TERM=xterm-kitty yields kitty" in run {
        val env    = Map("TERM" -> "xterm-kitty")
        val result = Browser.Image.ConsoleType.detect(env)
        assert(result == Present(Browser.Image.ConsoleType.kitty))
    }

    "ConsoleType.detect with neither TERM_PROGRAM nor kitty TERM yields Absent" in run {
