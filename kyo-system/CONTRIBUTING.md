...
**The headline invariant:** the negative capability law. A write op left in a program passed to a read-only runner keeps `PathWrite` undischarged, so the ascribed read-only residual does not compile [`Path.scala:194-199`]. Filesystem mutation boundaries are enforced in the type system: read-only contexts reject write programs at the call site, not at runtime [`Path.scala:63-66`]. Internalize this before touching runners, service backends, or overlay combinators.
...
kyo-system/
  shared/src/main/scala/kyo/
    Path.scala                    # opaque type, PathRead/PathWrite capabilities, runners, combinators
    FileSystem.scala             # pluggable backend SPI + CommitStrategy + host/inMemory/overlay factories + nested CommitHandle
    HostFileSystem.scala             # private[kyo] host delegate + root-confined backend
    InMemoryFileSystem.scala         # private[kyo] Auto-strategy virtual backend
    OverlayFileSystem.scala          # private[kyo] Manual-strategy copy-on-write overlay + ForwardingLowerFileSystem
  ...
  shared/src/test/scala/kyo/
    ...
    PathConfinementTest.scala, FileSystemTest.scala, FileSystemCommitTest.scala
    InMemoryFileSystemTest.scala
    OverlayFileSystemTest.scala, OverlayFileSystemCommitTest.scala, OverlayFileSystemRecoveryTest.scala
  ...
  jvm/src/test/scala/kyo/
    ...
    PathConfinementJvmTest.scala  # JVM-only: FileSystem.host(root) symlink escape
```

Virtual filesystem backends (`InMemoryFileSystem`, `OverlayFileSystem`) live in `shared/src/main` as `private[kyo]` objects/classes in dedicated source files, not in platform trees [`InMemoryFileSystem.scala:5`, `InMemoryFileSystem.scala:53`, `OverlayFileSystem.scala:21`, `OverlayFileSystem.scala:371`]. Shared OS directory logic lives in `shared/src/main/internal`; both platform `PathPlatformSpecific` classes extend it [`PathDirectories.scala:5-8`].

### End-to-end read call flow

1. `path.read` suspends `Path.Op.Read` under `Tag[PathRead]` via `ArrowEffect.suspend` [`Path.scala:578-579`].
2. A runner (`Path.run`, `Path.runReadOnly`, or `runWith`) discharges the capability via `ArrowEffect.handle` and routes the op through `dispatch` to the installed `FileSystem` [`Path.scala:217-220`, `Path.scala:347`].
3. `HostFileSystem.read` bridges `path.unsafe.read()` through `Sync.Unsafe.defer(Abort.get(...))` [`HostFileSystem.scala:35-37`].
...
5. At the unsafe tier, `read()` returns `Result[FileReadException, String]` with `(using AllowUnsafe, Frame)` [`Path.scala:1029`].
...
Filesystem I/O is tracked as separate read and write capabilities so mutation boundaries are visible in the effect row before a runner executes anything [`Path.scala:49-54`]. A computation that only reads carries `< PathRead`; a computation that writes carries `< PathWrite`, which subtypes `PathRead` [`Path.scala:63-66`]. The negative capability law is the compile-time gate: if a program contains a write op but the caller ascribes a read-only runner residual, `PathWrite` remains undischarged and the program does not compile [`Path.scala:194-199`, `Path.scala:227-230`].
...
**Safe methods hide Sync and Abort until discharge.** Safe extension methods suspend via `ArrowEffect.suspend` and carry only `PathRead` or `PathWrite` on the row, not `Sync` or per-op `Abort` [`Path.scala:578-579`, `Path.scala:785-786`]. `Sync` and the `Abort[FileException]` umbrella are folded into the capability and become visible only after a runner discharges it [`Path.scala:49-52`].

**Runners discharge and expose backend effects.** Runners call `ArrowEffect.handle`, dispatch each op to the installed `FileSystem`, and expose the backend effect `S` on the residual [`Path.scala:217-220`, `Path.scala:227-230`]:
...
Both default to `FileSystem.host`; residuals carry `Sync & Abort[FileException] & S` [`Path.scala:182-192`, `Path.scala:194-206`].

### When each runner fires

- **`Path.run` / `runWith`:** use when the program may read and write. Discharges both capabilities against the service backend [`Path.scala:182-192`].
- **`Path.runReadOnly` / `runReadOnlyWith`:** use when the program must not mutate the filesystem. Discharges read capability only [`Path.scala:194-206`]. A write op in the program keeps `PathWrite` undischarged and fails to compile (the negative capability law) [`Path.scala:194-199`].
- **`Path.runWith(service)` / `runReadOnlyWith(service)`:** use when testing or running against a non-host backend (`FileSystem.inMemory`, `FileSystem.overlay`, or a custom `FileSystem[S]`) [`Path.scala:217-220`, `InMemoryFileSystemTest.scala:7-10`].
...
| Default service | `FileSystem.host` | Host filesystem backend with `Auto` commit strategy |
| Custom service | `FileSystem.inMemory`, `FileSystem.overlay(lower)`, custom `FileSystem[S]` | Virtual or confined backends; backend effect `S` rides the residual |
| Service commit strategy | `Auto`, `Manual`, `OnSuccess` (reserved) | When staged writes become visible (see FileSystem SPI section) |
...
**Production unsafe bridging:** every call to an `Unsafe` method inside kyo-system production code must be wrapped in `Sync.Unsafe.defer(...)` and annotated with a `// Unsafe:` comment naming the safe-tier contract being bridged [`HostFileSystem.scala:17-18`, `StreamFileExtensions.scala:25`]. Do not introduce `import AllowUnsafe.embrace.danger` in kyo-system sources; all unsafe execution passes through `Sync.Unsafe.defer`. `HostFileSystem` lifts `Result`-returning unsafe ops with `Sync.Unsafe.defer(Abort.get(...))` and pairs each call with a `// Unsafe:` comment [`HostFileSystem.scala:35-37`]. Inspection unsafe ops that return plain values omit `Abort.get` inside `Sync.Unsafe.defer` [`HostFileSystem.scala:17-18`].

**Target convention (not yet universal):** `Command` and `System` safe-tier lifts use `Sync.Unsafe.defer` but do not yet carry `// Unsafe:` comments, unlike `Path.HostFileSystem` [`Command.scala:67-68`, `System.scala:89`]. New lifts on Path backends, `StreamFileExtensions`, `InMemoryFileSystem`, and `OverlayFileSystem` must follow the annotated pattern.
...
| `A < (Sync & Abort[E])` from `Result`-returning unsafe | `Sync.Unsafe.defer(Abort.get(unsafe.op()))` + `// Unsafe:` | `HostFileSystem.read` [`HostFileSystem.scala:35-37`] |
| `A < Sync` from plain-value unsafe | `Sync.Unsafe.defer(unsafe.op())` + `// Unsafe:` | `HostFileSystem.exists` [`HostFileSystem.scala:17-18`] |
...
### FileSystem SPI and factories

`FileSystem[S]` is the pluggable backend SPI, a top-level trait alongside `Path` (not nested under it). It is effect-polymorphic in `S` (the backend's own effect, exposed on a runner residual); methods take no `(using Frame)` because a service captures its `Frame` at construction; the umbrella `Abort[FileException]` is the uniform error row [`FileSystem.scala:19-20`, `FileSystem.scala:22-27`].

| Factory | Class | Commit strategy | Effect `S` | Role |
|---|---|---|---|---|
| `FileSystem.host` | `HostFileSystem` | `Auto` | `Sync` | Default host backend; delegates every op to `Path.Unsafe` [`FileSystem.scala:104`, `HostFileSystem.scala:7-13`] |
| `FileSystem.host(root)` | rooted `HostFileSystem` | `Auto` | `Sync` | Resolves `root.realPath` at construction; rejects ops whose canonical path escapes the confinement root [`HostFileSystem.scala:9-11`, `HostFileSystem.scala:156-158`] |
| `FileSystem.inMemory` | `InMemoryFileSystem` | `Auto` | `Sync` | Immutable node tree keyed by `Path.parts` behind one `AtomicRef`, advanced by optimistic CAS [`FileSystem.scala:118`, `InMemoryFileSystem.scala:13-15`] |
| `FileSystem.overlay(lower)` | `OverlayFileSystem` | `Manual` | inherits `S` from lower | Copy-on-write over `lower`; reads fall through, writes stage in upper layer; scope-managed auto-rollback [`FileSystem.scala:124-125`, `OverlayFileSystem.scala:367-375`] |

**Commit strategy** (`FileSystem.CommitStrategy`) governs when staged bytes become visible [`FileSystem.scala:94-98`]:

| Commit strategy | Contract | Used by |
|---|---|---|
| `Auto` | Each successful write is durable immediately | `FileSystem.host`, `FileSystem.inMemory` |
| `Manual` | Writes stage until explicit `FileSystem.CommitHandle.commit` | `FileSystem.overlay` |
| `OnSuccess` | Writes stage during enclosing run, commit on success | Reserved; not used by shipped factories |

**Overlay internals (summary).** Staged writes are recorded as `WriteOp` journal entries, distinct from the read/write op-family partition in `Path.Op` [`OverlayFileSystem.scala:7-19`]. Upper-layer state uses `Upper` variants: staged `Entry`, deletion `Whiteout`, directory-hiding `OpaqueDir` [`OverlayFileSystem.scala:26-30`]. First observation of a lower path records the full observed `Path.Entry` in the read-set (bytes and stat for a regular file, stat only for a directory, an `Absent` sentinel for an observed-missing path); commit compares the recorded entry against a fresh live read to detect divergence [`OverlayFileSystem.scala:36-43`, `OverlayFileSystem.scala:441-475`]. Overlay commit machinery uses a self-contained binary intent log with no `kyo-eventlog` dependency to avoid circular module edges [`OverlayFileSystem.scala:56-59`].

**Commit strategies** on `FileSystem.CommitHandle[S]` (extends `FileSystem[S]`) [`FileSystem.scala:142-158`]:

| Method | Behavior | Residual includes `Abort[CommitConflict]`? |
|---|---|---|
| `commit` | Validates read-set; aborts `CommitConflict` if the recorded entry diverged; lower untouched on conflict | Yes |
| `commitOverwrite` | Replays unconditionally (last-writer-wins); no conflict check | No [`FileSystem.scala:151`] |
...
`CommitConflict`, `Conflict`, and `Resolution` are public types in `CommitConflict.scala` [`CommitConflict.scala:3-9`, `CommitConflict.scala:28-33`, `CommitConflict.scala:47-52`]. `Conflict.ancestor` carries `Maybe[Path.Entry]`: the read-set records the full observed entry at observation, so the base view is available at commit without re-reading the lower path [`CommitConflict.scala:16-27`]. `Resolution` has four cases: `KeepOurs`, `KeepTheirs`, `Write(entry)`, `Remove` [`CommitConflict.scala:47-52`]. Non-conflicting staged entries replay regardless of per-conflict resolution choices [`CommitConflict.scala:44-45`].

**Overlay traps:**

- Calling `commit` or `rollback` on a virtual overlay after `PathWrite` is discharged leaves path operations unresolved at runtime [`Path.scala:325-327`]. `Path.virtual` commit outside an ambient `PathWrite` scope leaves suspensions unresolved; `evalNow` returns absent [`PathTransactionTest.scala:167-176`].
- A read-set `Absent` entry marks a path that was observed but happened to not exist at that moment; `Maybe.Absent` on `Conflict.ancestor` means the path was never read through the overlay at all [`OverlayFileSystem.scala:458-462`].

### Overlay combinators (transaction, sandbox, virtual)

These combinators share an overlay bootstrap that casts `Sync` off the residual so combinator return rows stay locked without `Sync`, the same way `exists`/`read` hide `Sync` behind suspend/dispatch [`Path.scala:244-253`].

| Combinator | Return row | Disposition on success |
|---|---|---|
| `Path.transaction` | `A < (PathWrite & Abort[CommitConflict] & S)` | Commits overlay; rolls back on `Abort[CommitConflict]` [`Path.scala:288-299`, `Path.scala:276-286`] |
| `Path.sandbox` | `A < (PathWrite & S)` | Always rolls back; never surfaces `CommitConflict` [`Path.scala:301-316`] |
| `Path.virtual` | `(A, FileSystem.CommitHandle[Sync]) < (PathWrite & S)` | Caller must commit or rollback within ambient `PathWrite` scope [`Path.scala:318-336`] |
...
Overlay combinators forward lower I/O back through `ArrowEffect.suspend` so the ambient `PathWrite` handler stays transparent [`OverlayFileSystem.scala:1521-1522`, `OverlayFileSystem.scala:1525-1526`]. `ForwardingLowerFileSystem` re-suspends lower I/O as `PathRead`/`PathWrite` so exceptions propagate through the outer handler, not inside the overlay bootstrap handler [`Path.scala:264-266`, `OverlayFileSystem.scala:1519-1612`].

All three require an ambient `PathWrite` scope (an enclosing `runWith` or `run`) [`Path.scala:276-286`, `Path.scala:301-316`].
...
Safe-tier Path extension methods never call `Sync.Unsafe.defer` directly; they suspend under the capability tag [`Path.scala:1030-1031`]. Lifting happens in `HostFileSystem` and platform `Unsafe` implementations.
...
4. **Add the method to `FileSystem[S]`** with umbrella `Abort[FileException]` [`FileSystem.scala:22-27`].
5. **Wire `HostFileSystem`** to bridge `Path.Unsafe` through `Sync.Unsafe.defer(Abort.get(...))` with a `// Unsafe:` comment [`HostFileSystem.scala:35-37`].
...
9. **Implement on `InMemoryFileSystem`** (optimistic CAS over immutable tree state) [`InMemoryFileSystem.scala:149-170`].
10. **Implement on `OverlayFileSystem`** (stage into upper map and append to journal) [`OverlayFileSystem.scala:700-716`].
...
1. Implement `FileSystem[S]`; declare the correct `CommitStrategy` (`Auto` for immediate backends, `Manual` for staged backends) [`FileSystem.scala:19-20`, `InMemoryFileSystem.scala:53-55`, `OverlayFileSystem.scala:378`].
2. Methods take no `(using Frame)`; capture frame at construction [`FileSystem.scala:19-20`].
3. Every method returns `A < (S & Abort[FileException])` with the umbrella error row.
4. For host-backed variants: bridge each op to `Path.Unsafe` inside `Sync.Unsafe.defer(Abort.get(...))` with `// Unsafe:` comments; register a factory on `FileSystem` [`FileSystem.scala:104`, `HostFileSystem.scala:13-14`].
5. For in-memory backends: immutable state behind `AtomicRef` with optimistic CAS; expose via `FileSystem.inMemory` [`FileSystem.scala:118`, `InMemoryFileSystem.scala:53-55`].
6. For copy-on-write overlays: wrap lower in `OverlayFileSystem.init`, scope-register state for auto-rollback, return `FileSystem.CommitHandle[S]` [`OverlayFileSystem.scala:49-53`, `FileSystem.scala:124-125`].
7. Test with `Path.runWith(service)(...)` [`InMemoryFileSystemTest.scala:7-10`, `InMemoryFileSystemTest.scala:12-17`].

### Resolving overlay commit conflicts with commitWith

1. Seed lower, read through overlay to stamp read-set, stage writes, diverge lower [`OverlayFileSystemCommitTest.scala:168-198`].
...
4. Inside `OverlayFileSystem.commitWith`, each `Resolution` case rebuilds upper state and journal before replay [`OverlayFileSystem.scala:1371-1403`].
...
| `InMemoryFileSystem.scala` | `InMemoryFileSystemTest.scala` | (1:1) |
| `OverlayFileSystem.scala` | `OverlayFileSystemTest.scala` (base COW semantics) | `OverlayFileSystemCommitTest.scala` (conflict/commit machinery), `OverlayFileSystemRecoveryTest.scala` (crash recovery) |
...
Transaction combinator tests use `FileSystem.inMemory` and `Path.runWith(lower)` for deterministic lower inspection; no `Thread.sleep` anywhere [`PathTransactionTest.scala:5-9`]. `PathTransactionTest` locks the public sandbox row positively and proves transaction adds `Abort[CommitConflict]` via `typeCheckErrors` [`PathTransactionTest.scala:90-106`].

Overlay tests build via `FileSystem.inMemory` then `FileSystem.overlay(lower)`, exposing both overlay and lower [`OverlayFileSystemTest.scala:7-17`]. Copy-on-write semantics: write through `Path.runWith(ov)`, inspect lower via `Path.runWith(lower)` before commit [`OverlayFileSystemTest.scala:163-176`]. Stamp correctness avoids mtime flakiness by forcing `File→Absent` conflicts via lower removal, not re-write [`OverlayFileSystemTest.scala:208-212`].

`OverlayFileSystemCommitTest` creates conflicts by observing through overlay, staging writes, then mutating lower out-of-band with different-size content [`OverlayFileSystemCommitTest.scala:5-12`]. `commitOverwrite` must not carry `Abort[CommitConflict]` in its effect row; the test proves this by calling it without `Abort.run[CommitConflict]` [`OverlayFileSystemCommitTest.scala:115-127`]. `WriteOpLog.decode` failure modes are tested as pure `Result` matches without a service fixture [`OverlayFileSystemCommitTest.scala:305-338`].

### Recovery tests

`OverlayFileSystemRecoveryTest` injects crashes via `private[kyo]` hooks on `OverlayFileSystem` (default no-ops) [`OverlayFileSystem.scala:387-398`]. Recovery tests distinguish intentional hook crashes from real file errors using a `SyntheticCrash` exception type and `attemptCrash` [`OverlayFileSystemRecoveryTest.scala:28-50`]. Every crash point has paired `"in-memory:"` and `"host:"` test leaves in `shared/src/test` [`OverlayFileSystemRecoveryTest.scala:7-11`]. Host recovery tests use `FileSystem.host(root)` with a scoped `tempDir` as the rooted lower [`OverlayFileSystemRecoveryTest.scala:65-82`].

### In-memory service tests

`InMemoryFileSystemTest` obtains a fresh service per test via `FileSystem.inMemory.map` [`InMemoryFileSystemTest.scala:7-10`]. Service-level `Abort` from `Path.runWith` propagates outside the inner scope; `Abort.run` must wrap `Path.runWith`, not sit inside it [`InMemoryFileSystemTest.scala:95-100`]. Concurrency tests synchronize fibers with `Latch.init`/`gate.release` instead of sleeps [`InMemoryFileSystemTest.scala:143-156`].
...
1. **New Path operation.** Is there a `Path.Op` case (read-group or write-group)? Is the abstract method on `Path.Unsafe` with `(using AllowUnsafe, Frame)` and the most specific `Result[File*Exception, A]`? Is there a safe-tier extension method that suspends via `ArrowEffect.suspend` (not `Sync.Unsafe.defer`)? Is there a `FileSystem[S]` method and a `dispatch` case? Is `HostFileSystem` wired with `Sync.Unsafe.defer(Abort.get(...))` and a `// Unsafe:` comment? Are there concrete implementations in `jvm-native/` and `js-wasm/`, plus `InMemoryFileSystem` and `OverlayFileSystem`? Is there a cross-platform test leaf in `shared/src/test`?
...
4. **New FileSystem backend.** Does it implement `FileSystem[S]` with the correct `CommitStrategy`? Are methods frame-free (frame captured at construction)? Does every method return `A < (S & Abort[FileException])`? For host backends, is each op bridged through `Sync.Unsafe.defer` with `// Unsafe:` comments? Is there a factory registered on `FileSystem`? Are there tests via `Path.runWith(service)(...)`?
