...
kyo-system/
  shared/src/main/scala/kyo/
    ...
    PathService.scala             # pluggable backend SPI + Disposition + host/inMemory/overlay factories
    CommitHandle.scala            # PathService extension for manual-commit backends
  ...
  jvm/src/test/scala/kyo/
    ...
    PathConfinementJvmTest.scala  # JVM-only: PathService.host(root) symlink escape
```

Virtual filesystem backends (`InMemoryService`, `OverlayService`) live in `shared/src/main` as `private[kyo]` objects/classes in dedicated source files, not in platform trees [`InMemoryService.scala:5`, `InMemoryService.scala:53`, `OverlayService.scala:21`, `OverlayService.scala:371`]. Shared OS directory logic lives in `shared/src/main/internal`; both platform `PathPlatformSpecific` classes extend it [`PathDirectories.scala:5-8`].

### End-to-end read call flow

1. `path.read` suspends `Path.Op.Read` under `Tag[PathRead]` via `ArrowEffect.suspend` [`Path.scala:578-579`].
2. A runner (`Path.run`, `Path.runReadOnly`, or `runWith`) discharges the capability via `ArrowEffect.handle` and routes the op through `dispatch` to the installed `PathService` [`Path.scala:217-220`, `Path.scala:347`].
...
**Runners discharge and expose backend effects.** Runners call `ArrowEffect.handle`, dispatch each op to the installed `PathService`, and expose the backend effect `S` on the residual [`Path.scala:217-220`, `Path.scala:227-230`]:
...
Both default to `PathService.host`; residuals carry `Sync & Abort[FileException] & S` [`Path.scala:182-192`, `Path.scala:194-206`].
...
- **`Path.runWith(service)` / `runReadOnlyWith(service)`:** use when testing or running against a non-host backend (`PathService.inMemory`, `PathService.overlay`, or a custom `PathService[S]`) [`Path.scala:217-220`, `InMemoryServiceTest.scala:7-10`].
...
| Default service | `PathService.host` | Host filesystem backend with `AutoCommit` disposition |
| Custom service | `PathService.inMemory`, `PathService.overlay(lower)`, custom `PathService[S]` | Virtual or confined backends; backend effect `S` rides the residual |
| Service disposition | `AutoCommit`, `ManualCommit`, `CommitOnSuccess` (reserved) | When staged writes become visible (see PathService SPI section) |
...
### PathService SPI and factories

`PathService[S]` is the pluggable backend SPI, a top-level trait alongside `Path` (not nested under it). It is effect-polymorphic in `S` (the backend's own effect, exposed on a runner residual); methods take no `(using Frame)` because a service captures its `Frame` at construction; the umbrella `Abort[FileException]` is the uniform error row [`PathService.scala:19-20`, `PathService.scala:22-27`].

| Factory | Class | Disposition | Effect `S` | Role |
|---|---|---|---|---|
| `PathService.host` | `HostService` | `AutoCommit` | `Sync` | Default host backend; delegates every op to `Path.Unsafe` [`PathService.scala:104`, `HostService.scala:7-13`] |
| `PathService.host(root)` | rooted `HostService` | `AutoCommit` | `Sync` | Resolves `root.realPath` at construction; rejects ops whose canonical path escapes the confinement root [`HostService.scala:9-11`, `HostService.scala:156-158`] |
| `PathService.inMemory` | `InMemoryService` | `AutoCommit` | `Sync` | Immutable node tree keyed by `Path.parts` behind one `AtomicRef`, advanced by optimistic CAS [`PathService.scala:118`, `InMemoryService.scala:13-15`] |
| `PathService.overlay(lower)` | `OverlayService` | `ManualCommit` | inherits `S` from lower | Copy-on-write over `lower`; reads fall through, writes stage in upper layer; scope-managed auto-rollback [`PathService.scala:124-125`, `OverlayService.scala:367-375`] |

**Disposition** (`PathService.Disposition`) governs when staged bytes become visible [`PathService.scala:94-98`]:

| Disposition | Contract | Used by |
|---|---|---|
| `AutoCommit` | Each successful write is durable immediately | `PathService.host`, `PathService.inMemory` |
| `ManualCommit` | Writes stage until explicit `CommitHandle.commit` | `PathService.overlay` |
| `CommitOnSuccess` | Writes stage during enclosing run, commit on success | Reserved; not used by shipped factories |

**Overlay internals (summary).** Staged writes are recorded as `WriteOp` journal entries, distinct from the read/write op-family partition in `Path.Op` [`OverlayService.scala:7-19`]. Upper-layer state uses `Upper` variants: staged `Entry`, deletion `Whiteout`, directory-hiding `OpaqueDir` [`OverlayService.scala:26-30`]. First observation of a lower path records the full observed `Path.Entry` in the read-set (bytes and stat for a regular file, stat only for a directory, an `Absent` sentinel for an observed-missing path); commit compares the recorded entry against a fresh live read to detect divergence [`OverlayService.scala:36-43`, `OverlayService.scala:441-475`]. Overlay commit machinery uses a self-contained binary intent log with no `kyo-eventlog` dependency to avoid circular module edges [`OverlayService.scala:56-59`].

**Commit strategies** on `CommitHandle[S]` (extends `PathService[S]`) [`CommitHandle.scala:18-34`]:
...
| `commitOverwrite` | Replays unconditionally (last-writer-wins); no conflict check | No [`CommitHandle.scala:27`] |
...
- A read-set `Absent` entry marks a path that was observed but happened to not exist at that moment; `Maybe.Absent` on `Conflict.ancestor` means the path was never read through the overlay at all [`OverlayService.scala:458-462`].
...
| `Path.virtual` | `(A, CommitHandle[Sync]) < (PathWrite & S)` | Caller must commit or rollback within ambient `PathWrite` scope [`Path.scala:318-336`] |
...
Overlay combinators forward lower I/O back through `ArrowEffect.suspend` so the ambient `PathWrite` handler stays transparent [`OverlayService.scala:1521-1522`, `OverlayService.scala:1525-1526`]. `ForwardingLowerService` re-suspends lower I/O as `PathRead`/`PathWrite` so exceptions propagate through the outer handler, not inside the overlay bootstrap handler [`Path.scala:264-266`, `OverlayService.scala:1519-1612`].
...
4. **Add the method to `PathService[S]`** with umbrella `Abort[FileException]` [`PathService.scala:22-27`].
...
10. **Implement on `OverlayService`** (stage into upper map and append to journal) [`OverlayService.scala:700-716`].
...
### Adding a new PathService backend

1. Implement `PathService[S]`; declare the correct `Disposition` (`AutoCommit` for immediate backends, `ManualCommit` for staged backends) [`PathService.scala:19-20`, `InMemoryService.scala:53-55`, `OverlayService.scala:378`].
2. Methods take no `(using Frame)`; capture frame at construction [`PathService.scala:19-20`].
3. Every method returns `A < (S & Abort[FileException])` with the umbrella error row.
4. For host-backed variants: bridge each op to `Path.Unsafe` inside `Sync.Unsafe.defer(Abort.get(...))` with `// Unsafe:` comments; register a factory on `PathService` [`PathService.scala:104`, `HostService.scala:13-14`].
5. For in-memory backends: immutable state behind `AtomicRef` with optimistic CAS; expose via `PathService.inMemory` [`PathService.scala:118`, `InMemoryService.scala:53-55`].
6. For copy-on-write overlays: wrap lower in `OverlayService.init`, scope-register state for auto-rollback, return `CommitHandle[S]` [`OverlayService.scala:49-53`, `PathService.scala:124-125`].
...
4. Inside `OverlayService.commitWith`, each `Resolution` case rebuilds upper state and journal before replay [`OverlayService.scala:1371-1403`].
...
Transaction combinator tests use `PathService.inMemory` and `Path.runWith(lower)` for deterministic lower inspection; no `Thread.sleep` anywhere [`PathTransactionTest.scala:5-9`]. `PathTransactionTest` locks the public sandbox row positively and proves transaction adds `Abort[CommitConflict]` via `typeCheckErrors` [`PathTransactionTest.scala:90-106`].

Overlay tests build via `PathService.inMemory` then `PathService.overlay(lower)`, exposing both overlay and lower [`OverlayServiceTest.scala:7-17`]. Copy-on-write semantics: write through `Path.runWith(ov)`, inspect lower via `Path.runWith(lower)` before commit [`OverlayServiceTest.scala:163-176`]. Stamp correctness avoids mtime flakiness by forcing `File→Absent` conflicts via lower removal, not re-write [`OverlayServiceTest.scala:208-212`].
...
`OverlayServiceRecoveryTest` injects crashes via `private[kyo]` hooks on `OverlayService` (default no-ops) [`OverlayService.scala:387-398`]. Recovery tests distinguish intentional hook crashes from real file errors using a `SyntheticCrash` exception type and `attemptCrash` [`OverlayServiceRecoveryTest.scala:28-50`]. Every crash point has paired `"in-memory:"` and `"host:"` test leaves in `shared/src/test` [`OverlayServiceRecoveryTest.scala:7-11`]. Host recovery tests use `PathService.host(root)` with a scoped `tempDir` as the rooted lower [`OverlayServiceRecoveryTest.scala:65-82`].

### In-memory service tests

`InMemoryServiceTest` obtains a fresh service per test via `PathService.inMemory.map` [`InMemoryServiceTest.scala:7-10`]. Service-level `Abort` from `Path.runWith` propagates outside the inner scope; `Abort.run` must wrap `Path.runWith`, not sit inside it [`InMemoryServiceTest.scala:95-100`]. Concurrency tests synchronize fibers with `Latch.init`/`gate.release` instead of sleeps [`InMemoryServiceTest.scala:143-156`].
...
1. **New Path operation.** Is there a `Path.Op` case (read-group or write-group)? Is the abstract method on `Path.Unsafe` with `(using AllowUnsafe, Frame)` and the most specific `Result[File*Exception, A]`? Is there a safe-tier extension method that suspends via `ArrowEffect.suspend` (not `Sync.Unsafe.defer`)? Is there a `PathService[S]` method and a `dispatch` case? Is `HostService` wired with `Sync.Unsafe.defer(Abort.get(...))` and a `// Unsafe:` comment? Are there concrete implementations in `jvm-native/` and `js-wasm/`, plus `InMemoryService` and `OverlayService`? Is there a cross-platform test leaf in `shared/src/test`?
...
4. **New PathService backend.** Does it implement `PathService[S]` with the correct `Disposition`? Are methods frame-free (frame captured at construction)? Does every method return `A < (S & Abort[FileException])`? For host backends, is each op bridged through `Sync.Unsafe.defer` with `// Unsafe:` comments? Is there a factory registered on `PathService`? Are there tests via `Path.runWith(service)(...)`?
