...
[`kyo-ffi` native loader](../kyo-ffi/README.md). Overlay recovery records containing
file contents are stored in a private staging directory.
...
`FileSystem.overlay(lower)` builds the same staging layer over any service directly. With a
`FileSystem.Write[Sync]` lower, it also exposes `FileSystem.Watch`; the generic overload preserves
the lower service's effects in reads and writes. A commit is
...
Host commits require directory synchronization. On Windows, they fail at the staging directory's
parent barrier before publishing any staged target. Reads and discarded writes remain available.
Recovery of an existing intent can apply part of its plan before a directory barrier fails; the
intent stays on disk for another recovery attempt. The in-memory lower supports commit on Windows.
