            ...
            Abort.run[CommitConflict] {
                Path.run {
                    Path.tempDir("staged-absent-").map { dir =>
                        FileSystem.host(dir).map { lower =>
                            val fresh = dir / "created-by-staging.txt"
                            // The Path handler raises lower-service errors, so capture them outside it.
                            Abort.run[FileSystemException](Path.runWith(lower)(Path.commitWritesOnSuccess(fresh.write("value")))).map {
                                result =>
                                    if kyo.internal.Platform.isWindows then
                                        lower.realPath(dir).map { parent =>
                                            assert(
                                                OverlayFileSystemWindowsTest.directoryFailure(result, parent),
                                                s"Directory barrier result: $result"
                                            )
                                            lower.exists(fresh).map(exists => assert(!exists))
                                        }
                                    else
                                        assert(result == Result.unit, s"Staged commit result: $result")
                                        lower.read(fresh).map(value => assert(value == "value"))
                            }
                        }
                    }
                }
                ...
                Path.run {
                    Path.tempDir("staged-absent-parents-").map { dir =>
                        FileSystem.host(dir).map { lower =>
                            ...
                            // The Path handler raises lower-service errors, so capture them outside it.
                            Abort.run[FileSystemException](Path.runWith(lower)(Path.commitWritesOnSuccess(deep.write(
                                "deep",
                                ...
                                if kyo.internal.Platform.isWindows then
                                    lower.realPath(dir).map { parent =>
                                        ...
                                        lower.exists(dir / "a").map(exists => assert(!exists))
                                    }
                                else
                                    assert(result == Result.unit, s"Staged commit result: $result")
                                    lower.read(deep).map(value => assert(value == "deep"))
                            }
                        }
                    }
                }
