...
class OverlayFileSystemRecoveryTest extends kyo.test.Test[Any]:
    ...
    // Host crash hooks after a directory barrier are unreachable on Windows. Explicit Windows
    // cases in OverlayFileSystemWindowsTest verify the initial failure and recovery of existing intents, including partial replay.
    ...
    "host: crash before intent log write leaves lower unchanged".only(!kyo.internal.Platform.isWindows) in {
    ...
    "host: crash after intent log write replays the full plan on recovery".only(!kyo.internal.Platform.isWindows) in {
        withHostTestOverlay { (ov, lower, root) =>
            val a = root / "a.txt"; val d = root / "d"; val old = root / "old.txt"
            primePlan(ov, lower, a, d, old).andThen {
    ...
    for removeDirectory <- Seq(false, true) do
        s"recovery retains applied children after clearing a directory, whiteout: $removeDirectory" in {
            withInMemoryTestOverlay { (overlay, lower) =>
                val parent      = Path("replaced-directory")
                val first       = parent / "a.bin"
                val second      = parent / "b.bin"
                val old         = parent / "old.bin"
                val firstBytes  = Span(1.toByte, 2.toByte)
                val secondBytes = Span(3.toByte, 4.toByte)
                for
                    _ <- lower.writeBytes(old, Span(9.toByte), Path.WriteOptions())
                    _ <- overlay.mkDir(parent)
                    _ <- overlay.writeBytes(first, firstBytes, Path.WriteOptions())
                    _ <- overlay.writeBytes(second, secondBytes, Path.WriteOptions())
                    _ <- (if removeDirectory then lower.removeAll(parent) else (): Unit < (Sync & Abort[FileSystemException]))
                    // Unsafe: stop after the directory and first child have been applied.
                    _ <- Sync.Unsafe.defer {
                        overlay.afterEntryApplyHook = (i, _) =>
                            if i == 2 then throw SyntheticCrash("crash: after first directory child")
                    }
                    _ <- attemptCrash(overlay.commitWith(_ =>
                        if removeDirectory then FileSystem.Resolution.Remove else FileSystem.Resolution.KeepOurs
                    ))
                    applied <- lower.readBytes(first)
                    pending <- lower.exists(second)
                    // Unsafe: recovery runs without further injected crashes.
                    _               <- Sync.Unsafe.defer { overlay.afterEntryApplyHook = (_, _) => () }
                    _               <- overlay.recover()
                    recoveredFirst  <- lower.readBytes(first)
                    recoveredSecond <- lower.readBytes(second)
                    oldExists       <- lower.exists(old)
                yield
                    assert(applied.is(firstBytes))
                    assert(!pending)
                    assert(recoveredFirst.is(firstBytes))
                    assert(recoveredSecond.is(secondBytes))
                    assert(!oldExists)
                end for
            }
        }
    ...
    "host: crash mid-apply after first entry recovers idempotently".only(!kyo.internal.Platform.isWindows) in {
        withHostTestOverlay { (ov, lower, root) =>
            val a = root / "a.txt"; val d = root / "d"; val old = root / "old.txt"
            primePlan(ov, lower, a, d, old).andThen {
                Sync.Unsafe.defer {
    ...
    "host: crash mid-apply after second entry recovers idempotently".only(!kyo.internal.Platform.isWindows) in {
        withHostTestOverlay { (ov, lower, root) =>
            val a = root / "a.txt"; val d = root / "d"; val old = root / "old.txt"
            primePlan(ov, lower, a, d, old).andThen {
                Sync.Unsafe.defer {
    ...
    "host: crash before committed marker replays the plan idempotently".only(!kyo.internal.Platform.isWindows) in {
        withHostTestOverlay { (ov, lower, root) =>
            val a = root / "a.txt"; val d = root / "d"; val old = root / "old.txt"
            primePlan(ov, lower, a, d, old).andThen {
    ...
    final private class RecordingLower(
        ...
        crashAt: Maybe[Path] = Absent
    ) extends FileSystem.Write[Sync]:
        export inner.{syncDirectory as _, *}

        private val crashPending = new java.util.concurrent.atomic.AtomicBoolean(true)

        def syncDirectory(path: Path)(using Frame): Unit < (Sync & Abort[FileWriteException]) =
            Sync.defer {
                if crashAt.contains(path) && crashPending.compareAndSet(true, false) then
                    throw SyntheticCrash("crash: before directory sync")
                discard(synced.updateAndGet(_.appended(path)))
            }.andThen(inner.syncDirectory(path))
    end RecordingLower

    /** Interrupts the commit during its first staged channel write, before sealing the file. */
    final private class InterruptingLower(
        ...
        export inner.{openWriteChannelUnscoped as _, *}
        ...
        private[kyo] def openWriteChannelUnscoped(
            ...
            inner.openWriteChannelUnscoped(path, open, onAcquire, preservePermissionsFrom).map { (channel, release, close) =>
                val interrupted = new Path.WriteChannel[Sync]:
                    export channel.{writeAt as _, *}
                    def writeAt(position: Long, bytes: Span[Byte])(using Frame): Unit < (Sync & Abort[FileWriteException]) =
                        // Unsafe: interruption is injected synchronously inside the acquired channel write.
                        Sync.Unsafe.defer {
                            if calls.incrementAndGet() == at then
                                // Unsafe: interrupt from inside the write, after acquisition has transferred cleanup ownership.
                                self.get().foreach(fiber => discard(fiber.unsafe.interrupt()))
                        }.andThen(channel.writeAt(position, bytes))
                (interrupted, release, close)
            }
    end InterruptingLower

    /** Counts channel ownership and signals after temporary removal, the last cleanup operation. */
    final private class CountingHandleLower(
        ...
        export inner.{durableReplacePreserving as _, openWriteChannelUnscoped as _, tempFileHandle as _, *}

        private[kyo] def durableReplacePreserving(target: Path, bytes: Span[Byte], source: Path)(using
            Frame
        ): Unit < (Sync & Abort[FileReadException | FileWriteException | FileStructureException]) =
            FileSystem.durableReplace[Sync](this, target, bytes, Present(source))
        ...
        private[kyo] def openWriteChannelUnscoped(
            ...
            (Sync & Abort[FileWriteException | FileStructureException]) =
            Sync.defer {
                var acquired: Maybe[Path.ChannelCloseHandle] = Absent
                inner.openWriteChannelUnscoped(
                    path,
                    open,
                    underlying =>
                        discard(opens.incrementAndGet())
                        val once = new java.util.concurrent.atomic.AtomicBoolean(false)
                        val counted = new Path.ChannelCloseHandle:
                            def close()(using AllowUnsafe): Unit =
                                if once.compareAndSet(false, true) then
                                    underlying.close()
                                    discard(closes.incrementAndGet())
                        acquired = Present(counted)
                        onAcquire(counted)
                    ,
                    preservePermissionsFrom
                ).map { (channel, _, _) =>
                    val close = acquired.get
                    // Unsafe: the generic durable protocol owns this channel and invokes its release once.
                    (channel, () => Sync.Unsafe.defer(close.close()), close)
                }
            }
        end openWriteChannelUnscoped

        private[kyo] def tempFileHandle(path: Path)(using Frame): Path.TempFileHandle =
            val underlying = inner.tempFileHandle(path)
            new Path.TempFileHandle:
                def path: Path = underlying.path
                def remove()(using AllowUnsafe): Unit =
                    try underlying.remove()
                    // Unsafe: this signal follows both descriptor closure and temporary removal.
                    finally closed.unsafe.completeUnitDiscard()
            end new
        end tempFileHandle
    ...
    /** Runs `ov2.commit` on its own fiber, hands that fiber to the interrupting lower through
      ...
      * interrupt panic.
      *
      * Returns after the temporary removal has signalled `closed`, so a caller can inspect the
      * completed close-and-remove cleanup. The wait is bounded because the defect these cases exist for is
      ...
      *
      * `at = 1` interrupts the first positioned channel write. The sentinel uses writeBytes,
      * so it cannot consume this injection before the staged replacement begins.
      */
    ...
    "recovery syncs a removed entry's parent before recording the removal as durable" in {
        FileSystem.inMemory.map { inner =>
            val parent = Path("removed-parent")
            val target = parent / "removed.bin"
            val synced = new java.util.concurrent.atomic.AtomicReference(Chunk.empty[Path])
            val lower  = new RecordingLower(inner, synced, Present(parent))
            FileSystem.overlay(lower).map { value =>
                val overlay = value.asInstanceOf[OverlayFileSystem[Sync]]
                for
                    _      <- inner.writeBytes(target, Span(1.toByte), Path.WriteOptions())
                    _      <- overlay.removeAll(target)
                    _      <- attemptCrash(overlay.commit)
                    exists <- inner.exists(target)
                    _      <- Sync.defer(synced.set(Chunk.empty))
                    _      <- overlay.recover()
                    calls  <- Sync.defer(synced.get())
                yield
                    assert(!exists)
                    assert(calls.contains(parent), s"recovery did not persist the prior removal: $calls")
                end for
            }
        }
    }

    for recovering <- Seq(false, true) do
        s"commit syncs every newly created directory ancestor, recovery: $recovering" in {
            FileSystem.inMemory.map { inner =>
                val parent = Path("new-directory")
                val middle = parent / "nested"
                val target = middle / "empty"
                val synced = new java.util.concurrent.atomic.AtomicReference(Chunk.empty[Path])
                val lower  = new RecordingLower(inner, synced)
                FileSystem.overlay(lower).map { value =>
                    val overlay = value.asInstanceOf[OverlayFileSystem[Sync]]
                    for
                        _ <- overlay.mkDir(target)
                        _ <- (if recovering then
                                  // Unsafe: stop after the durable intent and simulate mkdir completing before any directory sync.
                                  Sync.Unsafe.defer {
                                      overlay.afterIntentLogHook = () => throw SyntheticCrash("crash: before directory creation")
                                  }.andThen(attemptCrash(overlay.commit))
                                      .andThen(inner.mkDir(target))
                                      .andThen(Sync.defer(synced.set(Chunk.empty)))
                                      .andThen(overlay.recover())
                              else
                                  // Unsafe: count apply barriers separately from staging directory acquisition.
                                  Sync.Unsafe.defer { overlay.afterIntentLogHook = () => synced.set(Chunk.empty) }
                                      .andThen(overlay.commit)
                        )
                        calls  <- Sync.defer(synced.get())
                        exists <- inner.isDirectory(target)
                    yield
                        assert(exists)
                        val required = Chunk(target, middle, parent, Path())
                        assert(calls.filter(required.contains) == required, s"missing or unordered directory syncs: $calls")
                    end for
                }
            }
        }
    ...
    "recovery syncs each recovered file's parent directory".only(!kyo.internal.Platform.isWindows) in {
    ...
    "an interrupted commit leaves no unfinished file in the staging directory".only(!kyo.internal.Platform.isWindows) in {
        // Staged content now uses durable replacement. Interrupt its channel write, not the later
        // intent-log writeChunk, and await removal of the unfinished sibling temporary before inspection.
        withHostTestOverlay { (_, lower, root) =>
            ...
            Fiber.Promise.init[Unit, Any].map { closed =>
                // The wrapper signals after the complete channel cleanup, including removal.
                val faulty =
                                ...
                                Sync.Unsafe.defer(ov2.stagingDirHandle).map {
                                    ...
                                    case Present(handle) =>
                                        lower.list(handle.path).map { entries =>
                                            val staged = entries.flatMap(_.name).filter(name =>
                                                name.endsWith(".dat") || name.startsWith(".kyo-durable-")
                                            )
                                            assert(
                                                staged.isEmpty,
    ...
    "an interrupted commit closes every handle it opened".only(!kyo.internal.Platform.isWindows) in {
    ...
    "a recovering overlay replays an orphaned staging directory left by a previous process".only(!kyo.internal.Platform.isWindows) in {
    ...
    "a plain overlay leaves an orphaned staging directory alone".only(!kyo.internal.Platform.isWindows) in {
    ...
    "host: crash after committed marker recovers via cleanup only".only(!kyo.internal.Platform.isWindows) in {
        withHostTestOverlay { (ov, lower, root) =>
            val a = root / "a.txt"; val d = root / "d"; val old = root / "old.txt"
            primePlan(ov, lower, a, d, old).andThen {
    ...
    "host: a directory's staged timestamp survives a commit replayed entirely from the intent log".only(
        ...
        //
        // This is the only case that puts a directory's explicit timestamp through the version 6
    ...
    "host: recover is idempotent; second call after completed recovery is a no-op".only(!kyo.internal.Platform.isWindows) in {
        withHostTestOverlay { (ov, lower, root) =>
            val a = root / "a.txt"; val d = root / "d"; val old = root / "old.txt"
            primePlan(ov, lower, a, d, old).andThen {
                Sync.Unsafe.defer {
    ...
    "host: staging dir is removed from root after recover on crash before log write".only(!kyo.internal.Platform.isWindows) in {
        withHostTestOverlay { (ov, lower, root) =>
            val a = root / "a.txt"; val d = root / "d"; val old = root / "old.txt"
            primePlan(ov, lower, a, d, old).andThen {
                Sync.Unsafe.defer {
                    ov.afterStageHook = () => throw SyntheticCrash("crash: before intent log write (cleanup host test)")
                }
                    .asInstanceOf[Unit < (Sync & Abort[FileSystemException])].andThen {
                        attemptCrash(
                            ov.commitWith(_ => FileSystem.Resolution.KeepOurs)
    ...
    "torn intent log is discarded by recoverFromDisk and lower is unchanged" in {
        withHostTestOverlay { (_, lower, root) =>
            ...
            val tornBytes  = Span.from("KYIL".getBytes(StandardCharsets.UTF_8) :+ 0x06.toByte)
    ...
    "fresh overlay recoverFromDisk replays a valid intent log written to disk".only(!kyo.internal.Platform.isWindows) in {
        withHostTestOverlay { (_, lower, root) =>
            ...
            Path.runWith(lower)(old.write("old-content")).andThen {
                ...
                val plan: Chunk[ReplayEntry] = Chunk(
                    ...
                    ReplayEntry.Directory(d.parts, opaque = false, Absent, syncDirectories = Chunk(d.parts, root.parts)),
    ...
    "recoverFromDisk replays two orphaned staging dirs independently".only(!kyo.internal.Platform.isWindows) in {
        withHostTestOverlay { (_, lower, root) =>
                                    ...
                                    Path.runWith(lower)(a1.read).map(c => assert(c == "content1")).andThen {
                                        Path.runWith(lower)(a2.read).map(c => assert(c == "content2")).andThen {
                                            Path.runWith(lower)(old1.exists).map(e => assert(!e, "old1 should be removed")).andThen {
                                                Path.runWith(lower)(old2.exists).map(e =>
                                                    assert(!e, "old2 should be removed")
                                                ).andThen {
    ...
    "staged file is durably written to staging dir before intent log (barrier ordering)".only(!kyo.internal.Platform.isWindows) in {
        withHostTestOverlay { (ov, lower, root) =>
            val a = root / "a.txt"; val d = root / "d"; val old = root / "old.txt"
            primePlan(ov, lower, a, d, old).andThen {
                Sync.Unsafe.defer {
