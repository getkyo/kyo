...
class OverlayFileSystemCommitTest extends kyo.test.Test[Any]:
    ...
    /** Registers `body` once per lower: the in-memory service, and a host service confined to a
      ...
      */
    // Windows rejects the first directory barrier. OverlayFileSystemWindowsTest covers its retained state.
    private def withEachLower(name: String, requiresDirectorySync: Boolean = true)(
        ...
        (name + " (host lower)").only(!kyo.internal.Platform.isWindows || !requiresDirectorySync) in {
    ...
    withEachLower("commit aborts CommitConflict when lower diverges after observation", requiresDirectorySync = false) {
        (ov, lower, base) =>
            val p = base / "p19.txt"
    ...
    withEachLower("commit aborts CommitConflict when lower file is deleted after observation", requiresDirectorySync = false) {
        (ov, lower, base) =>
    ...
    withEachLower("a same-size edit under one timestamp is still a conflict", requiresDirectorySync = false) { (ov, lower, base) =>
    ...
    withEachLower("an existence check still conflicts when the path stops existing", requiresDirectorySync = false) { (ov, lower, base) =>
    ...
    withEachLower("listing a directory conflicts when a child appears", requiresDirectorySync = false) { (ov, lower, base) =>
    ...
    withEachLower("a write after discard is rejected", requiresDirectorySync = false) { (ov, _, base) =>
    ...
    withEachLower("a commit that conflicts leaves the overlay writable", requiresDirectorySync = false) { (ov, lower, base) =>
    ...
    "WriteOpLog preserves private directories and permission sources through recovery serialization" in {
        val bytes  = Span(1.toByte, 2.toByte)
        val source = Path("private-source").parts
        val plan = Chunk(
            ReplayEntry.Directory(
                ...
                Present(42L),
                privateAccess = true,
                syncDirectories = Chunk(Path("staging").parts, Path().parts)
            ),
            ReplayEntry.File(Path("staging", "payload").parts, bytes, Path.PathStat(0L, 2L), Absent, Present(source)),
            ReplayEntry.Directory(
                Path("ordinary").parts,
                opaque = false,
                Absent,
                syncDirectories = Chunk(Path("ordinary").parts, Path().parts)
            ),
            ReplayEntry.File(Path("ordinary", "payload").parts, bytes, Path.PathStat(0L, 2L), Absent, defaultPermissions = true)
        )
        WriteOpLog.decode(Path("intent.kyo"), WriteOpLog.encode(plan)) match
            case Result.Success(Present(decoded)) =>
                val normalized = decoded.map {
                    case entry @ ReplayEntry.File(_, content, _, _, _, _) =>
                        assert(content.is(bytes))
                        entry.copy(bytes = bytes)
                    case entry => entry
                }
                assert(normalized == plan)
            case other => fail(s"Expected all access requirements to survive the log, got $other")
        end match
    }
    ...
    "WriteOpLog.decode returns Success(Absent) for torn log with valid magic but no terminator" in {
        ...
        val torn = Span.from(
            Array[Byte]('K'.toByte, 'Y'.toByte, 'I'.toByte, 'L'.toByte, 0x06.toByte)
        )
        val logPath = Path("intent.kyo")
