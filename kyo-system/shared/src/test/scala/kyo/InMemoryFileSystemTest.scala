...
class InMemoryFileSystemTest extends kyo.test.Test[Any]:
    ...
    private def withInMem[A, S](program: FileSystem[Sync] => A < (Sync & Abort[FileException] & S))
    ...
    "Service.host commitStrategy is Auto" in {
        assert(FileSystem.host.commitStrategy == FileSystem.CommitStrategy.Auto)
    }

    "Service.inMemory commitStrategy is Auto" in {
        FileSystem.inMemory.map { svc =>
            assert(svc.commitStrategy == FileSystem.CommitStrategy.Auto)
        }
    }
