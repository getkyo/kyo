...
class ZipInflateTest extends kyo.test.Test[Any]:
    ...
    "inflateRaw refuses to expand past a declared output size" in {
        import kyo.AllowUnsafe.embrace.danger
        // A zip entry's central directory record states its uncompressed size, so decoding is
        // bounded by it. Without a bound a crafted archive expands without limit inside whatever
        // process reads it, and ZipReadOnlyFileSystem inflates archives it did not produce.
        //
        // The bound is enforced inside the block decode loop rather than between blocks, because a
        // single block can emit without limit: a back-reference expands from output already
        // produced, so block count is no proxy for output size.
        val storedBlock = Array[Byte](
            0x01, // BFINAL=1, BTYPE=00 (stored), padding to byte boundary
            0x05,
            0x00, // LEN=5
            0xfa.toByte,
            0xff.toByte, // NLEN = ~5 & 0xFFFF
            1,
            2,
            3,
            4,
        ...
        val withinBound = ZipInflate.inflateRaw(storedBlock, 0L, -1, 5)
        assert(withinBound.toSeq == Seq[Byte](1, 2, 3, 4, 5), "a stream that fits its declared size must decode")

        val overrun =
            try
                val _ = ZipInflate.inflateRaw(storedBlock, 0L, -1, 2)
                kyo.Maybe.empty[String]
            catch case e: ZipInflate.InflateException => kyo.Present(e.getMessage)
        assert(overrun.isDefined, "decoding past the declared uncompressed size must fail")
        assert(overrun.exists(_.contains("exceeded the declared size")), s"unexpected message: $overrun")
    }
