...
class PathDurabilityTest extends kyo.test.Test[Any]:
    ...
    final private class Recording(
        ...
        maxOpens: Maybe[Int] = Absent,
        cleanupCloseFailure: Maybe[Throwable] = Absent,
        cleanupRemoveFailure: Maybe[Throwable] = Absent,
        releaseFailure: Maybe[Throwable] = Absent,
        openFailure: Maybe[Throwable] = Absent
                ...
                def wrapClose(underlying: Path.ChannelCloseHandle): Path.ChannelCloseHandle =
                    new Path.ChannelCloseHandle:
                        def close()(using AllowUnsafe): Unit =
                            underlying.close()
                            recordClose()
                            cleanupCloseFailure.foreach(throw _)
                        end close
                recorded += Event.Temporary(path)
                recorded += Event.Open(path, open)
                if maxOpens.exists(limit => recorded.count { case Event.Open(_, _) => true; case _ => false } > limit) then
                    throw new IllegalStateException("unexpected retry after parent failure")
                openFailure.foreach(throw _)
                (if fails("open") then Abort.fail(failure(path, FileSystemOperation.Channel))
                 else base.openWriteChannelUnscoped(path, open, close => onAcquire(wrapClose(close)), preservePermissionsFrom))
                    .map { (channel, release, closeHandle) =>
                        ...
                        (
                            wrapped,
                            () =>
                                release().andThen(Sync.defer {
                                    recordClose()
                                    releaseFailure.foreach(throw _)
                                }),
                            wrappedClose
                        )
                    }
        ...
        override private[kyo] def tempFileHandle(temporary: Path)(using Frame): Path.TempFileHandle =
            ...
            new Path.TempFileHandle:
                ...
                def remove()(using AllowUnsafe): Unit =
                    ...
                    handle.remove()
                    cleanupRemoveFailure.foreach(throw _)
    ...
    final private class ClosePanicking(
        ...
        override private[kyo] def openWriteChannelUnscoped(
            ...
            temporary.set(Present(path)).andThen(base.openWriteChannelUnscoped(
                path,
                open,
                close => onAcquire(wrapClose(close)),
    ...
    Chunk(false, true).foreach { sibling =>
        Chunk("", "/child").foreach { suffix =>
            s"${if sibling then "sibling temporary" else "durable replacement"} does not retry a regular-file parent$suffix" in {
                for
                    (base, root) <- hostFileSystem("kyo-durable-parent-file")
                    file   = root / "file"
                    target = Path(file.toString + suffix) / "target.bin"
                    fs     = new Recording(base, maxOpens = Present(1))
                    _ <- base.writeBytes(file, Span(9.toByte), Path.WriteOptions())
                    result <- Abort.run[FileSystemException] {
                        if sibling then Scope.run(Path.runWith(fs)(target.siblingTemporary)).unit
                        else fs.durableReplace(target, Span(1.toByte))
                    }
                    bytes <- base.readBytes(file)
                yield
                    assert(result.isFailure, s"Expected a parent failure, got $result")
                    assert(!result.failure.exists(_.isInstanceOf[FileAlreadyExistsException]))
                    assert(fs.events.collect { case Event.Open(path, _) => path }.size == 1)
                    assert(bytes.is(Span(9.toByte)))
            }
        }
    }

    Chunk(false, true).foreach { sibling =>
        Chunk(false, true).foreach { failLog =>
            s"${
                    if sibling then "sibling temporary" else "durable replacement"
                } reports both cleanup failures and preserves the primary failure when logger throws=$failLog" in {
                for
                    (base, root) <- hostFileSystem("kyo-durable-cleanup-errors")
                    captured     <- AtomicRef.init(Chunk.empty[Throwable])
                    closeError  = new RuntimeException("cleanup close failure")
                    removeError = new RuntimeException("cleanup removal failure")
                    primary     = new RuntimeException("release failure")
                    logError    = new RuntimeException("logger failure")
                    fs = new Recording(
                        base,
                        failOn = if sibling then Absent else Present("write"),
                        cleanupCloseFailure = Present(closeError),
                        cleanupRemoveFailure = Present(removeError),
                        releaseFailure = if sibling then Present(primary) else Absent
                    )
                    sink = new Log.Unsafe:
                        private val underlying = Log.live.unsafe
                        export underlying.{error as _, *}
                        def error(msg: => String)(using Frame, AllowUnsafe): Unit = underlying.error(msg)
                        def error(msg: => String, error: => Throwable)(using Frame, AllowUnsafe): Unit =
                            // Unsafe: the test sink records cleanup reports synchronously.
                            discard(captured.unsafe.getAndUpdate(_ :+ error))
                            if failLog then throw logError
                        end error
                    target = root / "target.bin"
                    result <- Log.let(Log(sink)) {
                    ...
                    errors <- captured.get
                    temporary = fs.events.collectFirst { case Event.Temporary(path) => path }.get
                    remains <- base.exists(temporary)
                yield
                    assert(errors == Chunk(closeError, removeError))
                    val failure = result.failureOrPanic.get
                    assert(Chunk.from(failure.getSuppressed) == Chunk(closeError, removeError))
                    if failLog then
                        assert(Chunk.from(closeError.getSuppressed) == Chunk(logError))
                        assert(Chunk.from(removeError.getSuppressed) == Chunk(logError))
                    if sibling then assert(result.panic.exists(_ eq primary))
                    else
                        result match
                            case Result.Failure(FileIOException(path, FileSystemOperation.Write, cause)) =>
                                assert(path == temporary)
                                assert(cause.getMessage == "injected Write failure")
                            case other => fail(s"Expected the write failure, got $other")
                    end if
                    assert(!remains)
                    assert(fs.events.last == Event.Remove(temporary))
            }
        }
    }

    Chunk(false, true).foreach { sibling =>
        s"${if sibling then "sibling temporary" else "durable replacement"} preserves an acquisition panic" in {
            for
                (base, root) <- hostFileSystem("kyo-durable-open-panic")
                primary = new RuntimeException("acquisition panic")
                fs      = new Recording(base, openFailure = Present(primary))
                result <- Abort.run[FileSystemException] {
                    if sibling then Scope.run(Path.runWith(fs)((root / "target.bin").siblingTemporary)).unit
                    else fs.durableReplace(root / "target.bin", Span(1.toByte))
                }
            yield assert(result.panic.exists(_ eq primary))
        }
    }

    "host temporary cleanup reports a filesystem deletion failure" in {
        for
            handle <- Scope.acquireRelease(FileSystem.host.tempDir("kyo-temp-delete-failure"))(h => Sync.Unsafe.defer(h.remove()))
            directory = handle.path / "nonempty"
            _ <- FileSystem.host.mkDir(directory)
            _ <- FileSystem.host.mkFile(directory / "child")
            result <- Sync.Unsafe.defer {
                Result.catching[Throwable](FileSystem.host.tempFileHandle(directory).remove())
            }
        yield assert(result == Result.fail(FileDirectoryNotEmptyException(directory)))
    }

    "concurrent replacement of a temporary-looking target preserves both writes" in {
        for
            (base, root) <- hostFileSystem("kyo-durable-concurrent-target")
            ready        <- Latch.init(1)
            hold         <- Latch.init(1)
            ...
            recording = new Recording(base)
            fs        = new Interrupting(recording.asInstanceOf[FileSystem.Write[Async]], ready, hold, temporary, closes, cleaned, removals)
            first     = root / "first.bin"
            second    = root / ".kyo-durable-0"
            fiber       <- Fiber.initUnscoped(fs.durableReplace(first, Span(1.toByte)))
            _           <- ready.await
            _           <- Sync.ensure(hold.release)(recording.durableReplace(second, Span(2.toByte)))
            _           <- fiber.get
            firstBytes  <- base.readBytes(first)
            secondBytes <- Abort.run[FileSystemException](base.readBytes(second))
        yield
            assert(firstBytes.is(Span(1.toByte)))
            assert(secondBytes.map(_.toArrayUnsafe.toSeq) == Result.succeed(Seq(2.toByte)))
    }

    "sibling temporary retries an exclusive-create collision without sharing the file" in {
        for
            (base, root) <- hostFileSystem("kyo-sibling-collision")
            paths <- Scope.run {
                for
                    first       <- Random.withSeed(17)(Path.runWith(base)((root / "first.bin").siblingTemporary))
                    second      <- Random.withSeed(17)(Path.runWith(base)((root / "second.bin").siblingTemporary))
                    _           <- base.writeBytes(first, Span(1.toByte), Path.WriteOptions())
                    _           <- base.writeBytes(second, Span(2.toByte), Path.WriteOptions())
                    firstBytes  <- base.readBytes(first)
                    secondBytes <- base.readBytes(second)
                yield
                    assert(first != second)
                    assert(firstBytes.is(Span(1.toByte)))
                    assert(secondBytes.is(Span(2.toByte)))
                    (first, second)
            }
            firstRemains  <- base.exists(paths._1)
            secondRemains <- base.exists(paths._2)
        yield assert(!firstRemains && !secondRemains)
    }
    ...
    Chunk("write", "sync-file", "move").foreach { failedStep =>
        s"failure at $failedStep stops later steps and removes the temporary" in {
            ...
            yield
                val operation = if failedStep == "move" then FileSystemOperation.Move else FileSystemOperation.Write
                result match
                    case Result.Failure(FileIOException(path, actualOperation, cause)) =>
                        assert(path == temp && actualOperation == operation)
                        assert(cause.getMessage == s"injected $operation failure")
                    case other => fail(s"Expected the injected $failedStep failure, got $other; events: ${fs.events}")
                end match
                assert(
                    actual.toArrayUnsafe.sameElements(Array[Byte](9)) &&
                        !exists &&
                        fs.events.last == Event.Remove(temp) &&
                        (moved == (failedStep == "move")) &&
                        !synced
                )
        }
    }
    ...
    "directory synchronization failure reports failure after replacement" in {
        ...
        yield
            result match
                case Result.Failure(FileIOException(path, FileSystemOperation.SyncDirectory, _)) =>
                    assert(path == target.parent.get)
                case other => fail(s"Expected the injected directory synchronization failure, got $other; events: ${fs.events}")
            end match
            assert(actual.is(Span(1.toByte, 2.toByte)), s"Replacement bytes: $actual; events: ${fs.events}")
            assert(fs.events.last == Event.SyncDirectory(target.parent.get))
    }
    ...
    "durable temporary names never expose an absent target before replacement" in {
        for
            ...
            identifier <- Random.withSeed(17)(Random.uuid)
            target = root / s".kyo-durable-$identifier-0"
            fiber   <- Fiber.initUnscoped(Random.withSeed(17)(fs.durableReplace(target, Span(1.toByte))))
            _       <- ready.await
    ...
    "sibling temporary names never create the target itself" in {
        for
            ...
            identifier <- Random.withSeed(17)(Random.uuid)
            target = root / s".kyo-temporary-$identifier-0"
            observed <- Scope.run {
                Random.withSeed(17)(Path.runWith(fs)(target.siblingTemporary)).map { temporary =>
