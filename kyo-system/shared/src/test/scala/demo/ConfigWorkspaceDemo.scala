...
/** A self-contained demo of a configuration deployment workspace: staging app settings on disk,
  ...
  *
  * Host commits require directory synchronization; this demo reports a typed failure on Windows.
  *
  * Dual-purpose: run it to print the workspace trace (`runMain`), and [[ConfigWorkspaceDemoTest]]
  * exercises the same `flow` plus `validate` as a CI guard.
  *
  * Run: `sbt 'kyo-systemJVM/testOnly demo.ConfigWorkspaceDemoTest'`
...
object ConfigWorkspaceDemo extends KyoApp:
    ...
    def flow(using Frame): WorkspaceSnapshot < (Async & Abort[FileSystemException | CommitConflict]) =
        Scope.run {
            Path.run {
                Path.tempDir("config-workspace-").map { workspace =>
                    FileSystem.host(workspace).map { lower =>
                        Path.runWith(lower) {
                            val appConfig    = workspace / "app.toml"
                            val deployConfig = workspace / "deploy.toml"
                            val scratch      = workspace / "scratch.toml"
                            val baseline     = "[app]\nname = demo\nversion = 1"
                            for
