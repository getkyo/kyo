...
/** A persistent trie keyed by path segments, carrying an optional value at every node.
  ...
  */
final private[kyo] case class PathTrie[A](
    value: Maybe[A],
    children: Map[String, PathTrie[A]],
    caseInsensitive: Boolean = false,
    spelling: String = ""
):
    private def key(segment: String): String =
        if caseInsensitive then segment.toLowerCase(java.util.Locale.ROOT) else segment

    def sameKey(first: Chunk[String], second: Chunk[String]): Boolean =
        first.size == second.size && first.indices.forall(index => key(first(index)) == key(second(index)))
    ...
    def nodeAt(parts: Chunk[String]): Maybe[PathTrie[A]] =
        @tailrec def loop(node: PathTrie[A], i: Int): Maybe[PathTrie[A]] =
            if i >= parts.size then Present(node)
                ...
                node.children.get(node.key(parts(i))) match
    ...
    def updated(parts: Chunk[String], a: A, replaceSpelling: Boolean = false): PathTrie[A] =
        def loop(node: PathTrie[A], i: Int): PathTrie[A] =
            if i >= parts.size then node.copy(value = Present(a))
            else
                val seg   = node.key(parts(i))
                val child = node.children.getOrElse(seg, PathTrie.empty[A](caseInsensitive).copy(spelling = parts(i)))
                val named = if replaceSpelling && i == parts.size - 1 then child.copy(spelling = parts(i)) else child
                node.copy(children = node.children.updated(seg, loop(named, i + 1)))
        loop(this, 0)
    end updated
    ...
    def removedSubtree(parts: Chunk[String]): PathTrie[A] =
        if parts.isEmpty then PathTrie.empty[A](caseInsensitive)
        else
            def loop(node: PathTrie[A], i: Int): PathTrie[A] =
                val seg = node.key(parts(i))
        ...
        nodeAt(parts) match
            ...
            case Present(node) =>
                Chunk.from(node.children.toIndexedSeq.collect {
                    case (seg, child) if child.value.isDefined => (child.spelling, child.value.get)
                })
    end childValues
    ...
    private def collectInto(prefix: Chunk[String], includeSelf: Boolean): Chunk[(Chunk[String], A)] =
        ...
        children.toIndexedSeq.sortBy(_._1).foldLeft(self) { case (acc, (seg, child)) =>
            acc.concat(child.collectInto(prefix.append(child.spelling), includeSelf = true))
        }
    end collectInto
...
private[kyo] object PathTrie:
    ...
    def empty[A](caseInsensitive: Boolean): PathTrie[A] =
        if caseInsensitive then PathTrie(Absent, Map.empty, caseInsensitive = true) else empty[A]
