...
private[kyo] object ZipArchive:
    ...
    /** One entry to serialize. ZIP's canonical MS-DOS timestamp has two-second precision and a
      * representable UTC range from 1980-01-01 through 2107-12-31. [[write]] clamps and truncates
      * `lastModifiedMs` to that representation deterministically on every platform.
      */
    final case class WriteEntry(
        name: String,
        isDirectory: Boolean,
        content: Array[Byte],
        lastModifiedMs: Long = 315532800000L
    )
    ...
    // MS-DOS date/time conversion uses pure integer arithmetic (Howard Hinnant's civil calendar,
    // proleptic Gregorian). No java.util.Calendar/Date or platform timezone participates, so parsing
    // and writing use the same deterministic UTC interpretation on all four platforms.
    ...
    private val MinDosEpochSecond = daysFromCivil(1980L, 1L, 1L) * 86400L
    private val MaxDosEpochSecond = daysFromCivil(2107L, 12L, 31L) * 86400L + 86398L

    private def toDosDateTime(epochMs: Long): (Int, Int) =
        val epochSecond        = Math.floorDiv(epochMs, 1000L).max(MinDosEpochSecond).min(MaxDosEpochSecond)
        val epochDay           = Math.floorDiv(epochSecond, 86400L)
        val secondOfDay        = Math.floorMod(epochSecond, 86400L)
        ...
        val dosTime            = (hour.toInt << 11) | (minute.toInt << 5) | (second.toInt / 2)
        val dosDate            = ((year.toInt - 1980) << 9) | (month.toInt << 5) | day.toInt
        (dosTime, dosDate)
    end toDosDateTime
    ...
    def readEntry(bytes: Array[Byte], entry: Entry)(using AllowUnsafe): Array[Byte] =
        ...
        val raw =
            if entry.method == MethodStored then
                ...
                // The central directory declares the uncompressed size, so decoding is bounded by it.
                // Without a bound a crafted archive expands without limit inside a shared JVM.
                try ZipInflate.inflateRaw(bytes, start.toLong * 8L, start + entry.compSize, entry.uncompSize)
                catch
    ...
    /** Serializes `entries` into a STORED-only (uncompressed, zip
    ...
    def write(entries: Chunk[WriteEntry]): Array[Byte] =
        ...
        entries.foreach { entry =>
            ...
            val (dosTime, dosDate)                               = toDosDateTime(lastModifiedMs)
