...
private[kyo] object InMemoryFileSystem:
    ...
    def init(using Frame): (FileSystem.Write[Sync] & FileSystem.Watch) < Sync =
    ...
    private[kyo] def hasFileParent(root: Node, parts: Chunk[String]): Boolean =
        if parts.isEmpty then false
        else if root.file.isDefined then true
        else
            root.children.get(parts.head) match
                case Some(child) => hasFileParent(child, parts.tail)
                case None        => false
...
final private[kyo] class InMemoryFileSystem(state: AtomicRef[InMemoryFileSystem.State])
    extends FileSystem.Write[Sync], FileSystem.Watch:
    ...
    private def modify[E <: FileSystemException, A](op: State => Result[E, (State, A)], onCommit: A => Unit = (_: A) => ())(using
        Frame
    ): A < (Sync & Abort[E]) =
        Loop(()) { _ =>
            state.get.map { cur =>
                Abort.get(op(cur)).map { (next, v) =>
                    // Unsafe: publish resource ownership in the same node as the state mutation.
                    Sync.Unsafe.defer {
    ...
    // does neither, so a mkDir on an existing directory never drops its children.
    ...
    def temp(prefix: String, suffix: String)(using Frame): Path.TempFileHandle < (Sync & Abort[FileStructureException]) =
        now.map { t =>
            val file = Path(prefix + "-" + java.lang.Long.toHexString(t) + "-" + nextTempId.getAndIncrement().toHexString + suffix)
            modify(s => Result.succeed((s.copy(root = upsert(s.root, file.parts, Node.file(Span.empty[Byte], t), t)), ()))).andThen {
                tempFileHandle(file)
            }
        }
    ...
    private def ensureWriteTarget(path: Path, open: FileSystem.WriteOpen, onAcquire: () => Unit = () => ())(using
        Frame
    ): Unit < (Sync & Abort[FileWriteException | FileStructureException]) =
        now.map(t =>
            modify(
                { s =>
                    if hasFileParent(s.root, path.parts) then Result.fail(FileNotADirectoryException(path))
                    else
    ...
    private[kyo] def openWriteChannelUnscoped(
        ...
        Sync.defer {
            ...
            val acquired = unscopedWrite(ops, writeChannel(ops))
            ensureWriteTarget(path, open, () => onAcquire(acquired._3)).andThen(acquired)
        }
    ...
    /** Validates the directory; volatile memory has no persistence boundary to flush. */
    def syncDirectory(path: Path)(using Frame): Unit < (Sync & Abort[FileWriteException]) =
        @scala.annotation.tailrec
        def validate(node: Node, parts: Chunk[String]): Result[FileWriteException, Unit] =
            if node.file.isDefined then Result.fail(FileNotADirectoryException(path))
            else if parts.isEmpty then Result.succeed(())
            else
                node.children.get(parts.head) match
                    case Some(child) => validate(child, parts.tail)
                    case None        => Result.fail(FileNotFoundException(path))
        state.use(s => Abort.get(validate(s.root, path.parts)))
    end syncDirectory
    ...
    private[kyo] def siblingTemporary(target: Path, onAcquire: Path.TempFileHandle => Unit)(using
        ...
        FileSystem.siblingTemporary[Any](this, target, onAcquire)
    ...
    def tryLock(path: Path, mode: Path.LockMode, sentinelSuffix: String)(using
        ...
        if sentinelSuffix.isEmpty || sentinelSuffix.exists(c => c == '/' || c == '\\') then
            ...
            val acquire = Scope.acquireRelease(acquireLock(path, mode, sentinelSuffix))(lock => lock.release(lock.ownership)).map(Maybe(_))
                            ...
                            new Path.Watcher:
                                def events: Stream[PathChange, Async & Scope & Abort[FileWatchException]] =
                                    channel.streamUntilClosed().map { event =>
                                        val handled: Unit < Async = event match
    ...
    private def sentinelParts(path: Path, suffix: String): Chunk[String] = Path(path.unsafe.show + suffix).parts

    private def acquireLock(path: Path, mode: Path.LockMode, sentinelSuffix: String)(using
        Frame
    ): Path.Lock < (Sync & Abort[FileLockException]) =
        val owner = Path.LockOwnership.fresh()
        AtomicBoolean.init(false).map { released =>
            modify { s =>
                s.locks.get(sentinelParts(path, sentinelSuffix)) match
                    case None =>
                        val claim = LockClaim(mode, Chunk(owner))
                        Result.succeed((
                            s.copy(locks = s.locks.updated(sentinelParts(path, sentinelSuffix), claim)),
                            mkLock(path, mode, owner, released, sentinelSuffix)
                        ))
                    case Some(claim) if mode == Path.LockMode.Shared && claim.mode == Path.LockMode.Shared =>
                        ...
                        Result.succeed((
                            s.copy(locks = s.locks.updated(sentinelParts(path, sentinelSuffix), next)),
                            mkLock(path, mode, owner, released, sentinelSuffix)
                        ))
    ...
    private def releaseLock(path: Path, owner: Path.LockOwnership, sentinelSuffix: String)(using
        Frame
    ): Unit < (Sync & Abort[FileLockException]) =
        modify { current =>
            current.locks.get(sentinelParts(path, sentinelSuffix)) match
                case Some(claim) if claim.owners.exists(Path.LockOwnership.same(_, owner)) =>
                    ...
                    val locks = if owners.isEmpty then current.locks - (sentinelParts(path, sentinelSuffix))
                    else current.locks.updated(sentinelParts(path, sentinelSuffix), claim.copy(owners = owners))
                    Result.succeed((current.copy(locks = locks), ()))
                case _ => Result.fail(FileLockOwnershipLostException(path))
        }

    private def owns(path: Path, owner: Path.LockOwnership, sentinelSuffix: String)(using Frame): Boolean < Sync =
        state.use(_.locks.get(sentinelParts(path, sentinelSuffix)).exists(_.owners.exists(Path.LockOwnership.same(_, owner))))

    private def mkLock(
        path: Path,
        grantedMode: Path.LockMode,
        owner: Path.LockOwnership,
        ...
        new Path.Lock:
            ...
            def check(using Frame): Unit < (Sync & Abort[FileLockException]) =
                owns(path, owner, sentinelSuffix).map {
                    case true  => ()
            ...
            def release(candidate: Path.LockOwnership)(using Frame): Unit < (Sync & Abort[FileLockException]) =
                if !Path.LockOwnership.same(owner, candidate) then Abort.fail(FileLockOwnershipLostException(path))
                    ...
                    released.compareAndSet(false, true).map {
                        case true  => releaseLock(path, owner, sentinelSuffix)
                        case false => ()
                    }
    ...
    def read(bytes: Span[Byte])(using Frame): Path.ReadHandle =
        new Path.ReadHandle:
            ...
            def size()(using AllowUnsafe, Frame): Result[FileReadException, Long] =
                // The handle holds the content the open captured, so its size is that content's size. A later
                // write to the same path replaces the tree node and leaves this handle on the bytes it opened,
                // exactly as an unlinked host file stays readable at its old content.
                Result.succeed(bytes.size.toLong)
    ...
    def walk(base: Path, node: Maybe[Node], maxDepth: Int)(using Frame): Path.WalkHandle =
        new Path.WalkHandle:
            ...
            def next()(using AllowUnsafe, Frame): Result[FileReadException | FileStructureException, Maybe[Path]] =
                Result.succeed(if pending.hasNext then Maybe(pending.next()) else Maybe.empty)
            def close()(using AllowUnsafe): Unit = ()
