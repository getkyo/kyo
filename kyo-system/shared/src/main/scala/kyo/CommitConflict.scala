...
/** One read-set divergence detected at commit: what the overlay observed when it first saw the lower
  * path (`ancestor`), the overlay's staged view (`ours`), and the live lower entry (`theirs`).
  *
  * `ancestor` is a [[Conflict.Ancestor]] rather than a full entry because the overlay records what
  ...
  * present a byte array the overlay never saw as though it had. `ours` and `theirs` do carry full
  ...
  * [[Path.commitWritesOnSuccess]] abort with [[CommitConflict]] when any divergence is unacceptable.
  */
...
object Conflict:
    ...
    /** The base view a conflict diverged from, at the strength the overlay observed it.
      *
      * `Unobserved` and `Missing` used to be indistinguishable, both reported as an absent entry, so
      * a caller resolving a conflict could not tell "the overlay never looked at this path" from
      * "the overlay saw that it was not there". Those call for different resolutions.
      */
    enum Ancestor derives CanEqual:
        /** The overlay never read this path. */
        case Unobserved

        /** The overlay saw that the path did not exist. */
        case Missing

        /** The overlay saw that the path was there and of this kind, and nothing more: not its
          * size, not its timestamp, not its contents.
          */
        case Presence(isDirectory: Boolean)

        /** The overlay listed this directory and saw these child names. */
        case DirectoryListing(children: Set[String])

        /** The overlay read this stat and did not read the contents. */
        case Metadata(stat: Path.PathStat, isDirectory: Boolean)

        /** The overlay read this entry in full. */
