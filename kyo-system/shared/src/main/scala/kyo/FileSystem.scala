...
object FileSystem:
    ...
    private[kyo] def awaitLock[S](path: Path, wait: Path.LockWait)(
        ...
        def untilAvailable: Path.Lock < (S & Async & Scope & Abort[FileReadException | FileLockException]) =
            attempt.map {
                ...
                case Absent        => Clock.sleep(lockRetryDelay).andThen(untilAvailable)
            }
        ...
        def until(deadline: Clock.Deadline, timeout: Duration)
            ...
            deadline.isOverdue.map {
                ...
                case false =>
                    attempt.map {
                        case Present(lock) => lock
                        case Absent        => Clock.sleep(lockRetryDelay).andThen(until(deadline, timeout))
                    }
            }
    ...
    trait Read[S]:
        ...
        def realPath(path: Path)(using Frame): Path < (S & Abort[FileReadException])
