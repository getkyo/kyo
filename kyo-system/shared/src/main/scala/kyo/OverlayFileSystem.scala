    ...
    object UpperTrie:
        ...
        extension (self: UpperTrie)
            ...
            def updated(cp: CanonicalPath, entry: Upper): UpperTrie =
                self.updated(cp.parts, entry, replaceSpelling = self.get(cp.parts).contains(Upper.Whiteout))
...
private[kyo] class OverlayFileSystem[S](
    ...
    private[kyo] def openWriteChannelUnscoped(path: Path, open: FileSystem.WriteOpen, onAcquire: Path.ChannelCloseHandle => Unit)(using
        Frame
    ): (Path.WriteChannel[S & Sync], () => Unit < (S & Sync), Path.ChannelCloseHandle) <
        ...
        Sync.defer {
            ...
            ensureWriteChannelTarget(path, open, () => onAcquire(acquired._3)).andThen(acquired)
        }
