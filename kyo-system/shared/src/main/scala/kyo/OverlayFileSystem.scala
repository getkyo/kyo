...
private[kyo] class OverlayFileSystem[S](
    ...
    private def stampFile(parts: Chunk[String], stat: Path.PathStat, bytes: Span[Byte])(using Frame): Unit < S =
        modifyPure { s =>
    ...
    private def stampDir(parts: Chunk[String], stat: Path.PathStat)(using Frame): Unit < S =
        modifyPure { s =>
            if s.readSet.contains(parts) then s
            else
    ...
    private def stampAbsent(parts: Chunk[String])(using Frame): Unit < S =
        modifyPure { s =>
            if s.readSet.contains(parts) then s
    ...
    private def stampLower(parts: Chunk[String])(using Frame): Unit < (S & Abort[FileReadException]) =
    ...
    def isSymbolicLink(path: Path)(using Frame): Boolean < (S & Abort[FileReadException]) = false

    def realPath(path: Path)(using Frame): Path < (S & Abort[FileReadException]) = path
    ...
    override private[kyo] def stableIdentity(path: Path)(using Frame): Maybe[String] < (S & Abort[FileReadException]) =
        withState { s =>
            ...
            if done then result else lower.stableIdentity(Path(current.toSeq*))
        }
                ...
                Abort.run[FileReadException] {
                    ...
                    case Result.Success((false, _)) =>
                        ensureWriteParent(parent, options, errorPath).andThen {
                            stampAbsent(parent.parts).andThen {
                                val stat = Path.PathStat(0L, 0L)
                                modifyPure { current =>
                                    val opaque =
                                        current.upper.get(parent.parts).contains(Upper.Whiteout) || ancestorWhiteout(current, parent.parts)
                                    val entry = if opaque then Upper.OpaqueDir(stat) else Upper.Entry(Path.Entry.Directory(stat))
                                    current.copy(
                                        upper = current.upper.updated(parent.parts, entry),
                                        journal = current.journal.appended(WriteOp.WriteDirectory(parent.parts, opaque))
                                    )
                                }
                            }
                        }
    ...
    def appendBytes(path: Path, value: Span[Byte], options: Path.WriteOptions)(using
        ...
        ensureWriteParent(path, options).andThen(withState { s =>
            s.upper.get(path.parts) match
    ...
    def move(
        ...
        ensureWriteParent(to, Path.WriteOptions(options.createFolders)).andThen(resolveEntry(from).map { resolved =>
            ...
            checkMoveTarget(to, options.replace) {
                resolved match
                    case file: Path.Entry.File =>
                        ...
                        modifyPure { cur =>
                            cur.copy(
                                upper = cur.upper.updated(to.parts, Upper.Entry(copied)),
                                journal = cur.journal.appended(
                                    WriteOp.Copy(from.parts, to.parts, copied, options.copyAttributes)
                                )
                            )
                        }
                    case _: Path.Entry.Directory =>
    ...
    // each node by its relative suffix under `from`. Each node becomes an upper entry plus a Copy
    // op carrying its captured content and metadata, so replay is source-independent and can honor
    // copyAttributes after target materialization.
        ...
        nodes.foldLeft((upper, journal)) { case ((u, j), (nodeParts, entry)) =>
            ...
            entry match
                case Path.Entry.File(bytes, stat) =>
                    ...
                    (
                        u.updated(targetParts, Upper.Entry(Path.Entry.File(bytes, targetStat))),
                        j.appended(WriteOp.Copy(nodeParts, targetParts, Path.Entry.File(bytes, targetStat), copyAttributes))
                    )
                case Path.Entry.Directory(stat) =>
                    val targetStat = if copyAttributes then stat else Path.PathStat(0L, stat.sizeBytes)
                    (
                        u.updated(targetParts, Upper.OpaqueDir(targetStat)),
                        j.appended(WriteOp.Copy(nodeParts, targetParts, Path.Entry.Directory(targetStat), copyAttributes))
                    )
            end match
        }
        ...
        new Path.TempFileHandle:
            ...
            def remove()(using AllowUnsafe): Unit =
                ...
                val _ = state.unsafe.updateAndGet { cur =>
                    ...
                    cur.copy(
                        upper = cur.upper.removed(parts),
                        journal = journal,
                        readSet = cur.readSet.removed(parts)
                    )
                }
                ()
            end remove
    ...
    def tempDir(prefix: String)(using Frame): Path.TempDirHandle < (S & Abort[FileStructureException]) =
        ...
        modifyPure { cur =>
            ...
            new Path.TempDirHandle:
                ...
                def remove()(using AllowUnsafe): Unit =
                    val _ = state.unsafe.updateAndGet(cur => cur.copy(upper = cur.upper.removed(dir.parts)))
                    ()
        }
    end tempDir
                ...
                open match
                    ...
                    case FileSystem.WriteOpen.CreateNew =>
                        ...
                        ensureWriteParent(path, Path.WriteOptions()).andThen {
                            modify { current =>
                                current.upper.get(path.parts) match
                                    case Some(Upper.Entry(_) | Upper.OpaqueDir(_)) => Result.fail(FileAlreadyExistsException(path))
                                    case _ =>
                                        Result.succeed((
                                            current.copy(
                                                upper =
                                                    current.upper.updated(path.parts, Upper.Entry(Path.Entry.File(Span.empty[Byte], stat))),
                                                journal = current.journal.appended(WriteOp.WriteFile(path.parts, Span.empty[Byte], stat))
                                            ),
                                            ()
                                        ))
                            }
    ...
    private def validate(s: OverlayState)(using Frame): Chunk[Conflict] < (S & Abort[FileSystemException]) =
        s.readSet.toIndexedSeq.foldLeft[Chunk[Conflict] < (S & Abort[FileSystemException])](Chunk.empty) {
    ...
    def commitWith(resolve: FileSystem.Conflict => FileSystem.Resolution)(using
        ...
        stateGet.map { s =>
            validate(s).map { conflicts =>
                ...
                conflicts.foldLeft[Map[Chunk[String], Resolution] < (S & Abort[FileSystemException])](Map.empty) {
                    (accKyo, conflict) =>
                        accKyo.map { resolutions =>
                            resolutions.updated(conflict.path.parts, resolve(conflict))
                        }
                }.map { resolutions =>
    ...
    private[kyo] def recoverFromDisk(root: Path)(using Frame): Unit < (S & Sync & Abort[FileSystemException]) =
        lower.list(root).map { entries =>
            entries.foldLeft[Unit < (S & Sync & Abort[FileSystemException])](()) { (acc, entry) =>
                acc.andThen {
                    entry.name match
...
final private[kyo] class WatchableOverlayFileSystem[S, S2](
    ...
    active: AtomicBoolean,
    isolate: Isolate[S, Sync, S2]
) extends OverlayFileSystem[S](lower, state, commitSeq, active), FileSystem.Watch[S & Sync]:
...
final private[kyo] class ForwardingLowerFileSystem extends FileSystem.Write[PathWrite]:
    ...
    def realPath(path: Path)(using Frame): Path < (PathWrite & Abort[FileReadException]) =
