...
private[kyo] enum ReplayEntry derives CanEqual:
    case File(
        ...
        bytes: Span[Byte],
        stat: Path.PathStat,
        explicitMtime: Maybe[Long],
        permissionSource: Maybe[Chunk[String]] = Absent,
        defaultPermissions: Boolean = false
    )
    case Directory(
        path: Chunk[String],
        opaque: Boolean,
        ...
        syncDirectories: Chunk[Chunk[String]] = Chunk.empty
    )
...
private[kyo] object OverlayFileSystem:
    ...
    /** Access requirements that must survive volatile staging until a host materializes the entry.
      * Permission sources are canonical paths, resolved through earlier staged replacements.
      */
    enum Access derives CanEqual:
        case Default
        case PrivateDirectory
        case PreserveFile(source: Chunk[String])
    end Access
    ...
    enum Upper derives CanEqual:
        case Entry(body: Path.Entry, explicitMtime: Maybe[Long], access: Access = Access.Default)
        case Whiteout
        case OpaqueDir(stat: Path.PathStat, explicitMtime: Maybe[Long], access: Access = Access.Default)
        def accessRequirement: Access = this match
            case Entry(_, _, access)     => access
            case OpaqueDir(_, _, access) => access
            case Whiteout                => Access.Default
    end Upper
    ...
    object UpperTrie:
        ...
        def empty(caseInsensitive: Boolean): UpperTrie = PathTrie.empty[Upper](caseInsensitive)

        extension (self: UpperTrie)
            def samePath(first: CanonicalPath, second: CanonicalPath): Boolean = self.sameKey(first.parts, second.parts)
            def get(cp: CanonicalPath): Maybe[Upper]                           = self.get(cp.parts)
            def updated(cp: CanonicalPath, entry: Upper): UpperTrie =
                val retained = (entry, self.get(cp.parts)) match
                    case (Upper.Entry(body: Path.Entry.File, mtime, Access.Default), Present(Upper.Entry(_: Path.Entry.File, _, access))) =>
                        Upper.Entry(body, mtime, access)
                    case (
                            Upper.Entry(body: Path.Entry.Directory, mtime, Access.Default),
                            Present(Upper.Entry(_: Path.Entry.Directory, _, access))
                        ) =>
                        Upper.Entry(body, mtime, access)
                    case (Upper.Entry(body: Path.Entry.Directory, mtime, Access.Default), Present(Upper.OpaqueDir(_, _, access))) =>
                        Upper.Entry(body, mtime, access)
                    case (Upper.OpaqueDir(stat, mtime, Access.Default), Present(Upper.Entry(_: Path.Entry.Directory, _, access))) =>
                        Upper.OpaqueDir(stat, mtime, access)
                    case (Upper.OpaqueDir(stat, mtime, Access.Default), Present(Upper.OpaqueDir(_, _, access))) =>
                        Upper.OpaqueDir(stat, mtime, access)
                    case _ => entry
                self.updated(cp.parts, retained, replaceSpelling = self.get(cp.parts).contains(Upper.Whiteout))
            end updated
    ...
    /** The layout of one commit's staging directory.
      ...
      * root, and the names inside it. Each of those names has a writer and a reader in
    ...
    object StagingDir:
        ...
        extension (self: StagingDir)
            ...
            def applied(index: Int)(using Frame): Path = self / s"applied-$index.marker"
    ...
    /** Lower observations recorded on first read, keyed by canonical path.
      ...
      */
    opaque type ReadSet = PathTrie[Observed]

    object ReadSet:
        import CanonicalPath.parts

        val empty: ReadSet                           = PathTrie.empty
        def empty(caseInsensitive: Boolean): ReadSet = PathTrie.empty[Observed](caseInsensitive)

        extension (self: ReadSet)
            def get(cp: CanonicalPath): Maybe[Observed] = self.get(cp.parts)
            def contains(cp: CanonicalPath): Boolean    = self.get(cp.parts).isDefined
            ...
            def entries: Chunk[(CanonicalPath, Observed)] =
                self.entries.map((parts, v) => (CanonicalPath.wrap(parts), v))
        end extension
    end ReadSet
    ...
    final case class OverlayState(
        ...
        phase: Phase,
        casePolicy: Maybe[Glob.CaseSensitivity] = Absent
    ):
        ...
        def replayPlan: Chunk[ReplayEntry] =
            upper.entries.map {
                ...
                case (parts, Upper.OpaqueDir(_, mtime, access)) =>
                    ReplayEntry.Directory(parts, opaque = true, mtime, access == Access.PrivateDirectory)
                case (parts, Upper.Entry(Path.Entry.Directory(_), mtime, access)) =>
                    ReplayEntry.Directory(parts, opaque = false, mtime, access == Access.PrivateDirectory)
                case (parts, Upper.Entry(Path.Entry.File(bytes, stat), mtime, access)) =>
                    val source = access match
                        case Access.PreserveFile(source) => Present(source)
                        case _                           => Absent
                    ReplayEntry.File(parts, bytes, stat, mtime, source)
            }
    end OverlayState
    ...
    def init[S, S2](lower: FileSystem.Write[S])(using
        ...
        FileSystem.StagedChanges[S & Sync & Abort[FileSystemException]] & FileSystem.Write[S & Sync]
    ) < (Sync & Scope) =
        build(lower)

    def initSync(lower: FileSystem.Write[Sync])(using Frame): SyncOverlayFileSystem < (Sync & Scope) =
        Scope.acquireRelease(AtomicRef.init(OverlayState.empty))(_.set(OverlayState.terminated)).map { ref =>
            AtomicLong.init(0L).map(sequence => new SyncOverlayFileSystem(lower, ref, sequence))
        }
...
//
// Version 6 also records the directory sync chain captured before creation, so recovery persists
// every newly created ancestor even when a previous process stopped between mkdir and fsync.
// Decode rejects other versions rather than replaying without their access-control requirements.
// kyo-system has not been released, so there are no released logs to migrate.
private[kyo] object WriteOpLog:
    ...
    private def encodeRecord(entry: ReplayEntry): Array[Byte] =
        ...
        entry match
            case ReplayEntry.File(parts, bytes, stat, explicitMtime, permissionSource, defaultPermissions) =>
                buf += OpFile
                ...
                permissionSource match
                    ...
                    case Present(source) =>
                        buf += 0x01.toByte
                        encodeParts(buf, source)
                end match
                buf += (if defaultPermissions then 0x01.toByte else 0x00.toByte)
            case ReplayEntry.Directory(parts, opaque, explicitMtime, privateAccess, syncDirectories) =>
                buf += OpDirectory
                encodeParts(buf, parts)
                buf += (if opaque then 0x01.toByte else 0x00.toByte)
                buf += (if privateAccess then 0x01.toByte else 0x00.toByte)
                wOptI64(buf, explicitMtime)
                wI32(buf, syncDirectories.size)
                syncDirectories.foreach(encodeParts(buf, _))
    ...
    private def decodeRecord(arr: Array[Byte], offset: Int, len: Int): Maybe[ReplayEntry] =
        ...
        if opcode == OpFile then
                                ...
                                readOptI64() match
                                    ...
                                    case Present(explicitMtime) =>
                                        if pos >= end then Absent
                                            ...
                                            val tag = arr(pos); pos += 1
                                            val source: Maybe[Maybe[Chunk[String]]] =
                                                if tag == 0x00.toByte then Present(Absent)
                                                else if tag == 0x01.toByte then readParts().map(Present(_))
                                                else Absent
                                            source.flatMap { permissionSource =>
                                                if pos + 1 != end then Absent
                                                else
                                                    val defaults = arr(pos)
                                                    if defaults != 0x00.toByte && defaults != 0x01.toByte then Absent
                                                    else
                                                        Present(ReplayEntry.File(
                                                            parts,
                                                            bytes,
                                                            Path.PathStat(lm, sz),
                                                            explicitMtime,
                                                            permissionSource,
                                                            defaults == 0x01.toByte
                                                        ))
                                                    end if
                                            }
                                end match
            ...
            readParts() match
                case Absent => Absent
                case Present(parts) =>
                    if pos + 2 > end then Absent
                    else
                        val opaque        = arr(pos) != 0x00.toByte; pos += 1
                        val privateAccess = arr(pos) != 0x00.toByte; pos += 1
                        readOptI64().flatMap { explicitMtime =>
                            if pos + 4 > end then Absent
                            else
                                val count = rI32(arr, pos); pos += 4
                                if count <= 0 || count > 65536 then Absent
                                else
                                    val directories = new scala.collection.mutable.ArrayBuffer[Chunk[String]](math.min(count, 64))
                                    var i           = 0
                                    var valid       = true
                                    ...
                                    if !valid || pos != end then Absent
                                    else
                                        Present(ReplayEntry.Directory(
                                            parts,
                                            opaque,
                                            explicitMtime,
                                            privateAccess,
                                            Chunk.from(directories.toIndexedSeq)
                                        ))
                                    end if
                                end if
                        }
...
/** Copy-on-write overlay service. Reads check the upper layer first; writes stage in the upper
  ...
  * process finds and replays it. Persistent lowers must support directory synchronization:
  * Windows host commits fail at the staging-parent barrier, before changing staged targets.
  * Recovery of an existing intent can partially apply the plan before a directory barrier fails;
  * the intent remains available for retry.
  *
...
private[kyo] class OverlayFileSystem[S](
            ...
            state.get.map { cur =>
                if cur.phase != Phase.Open then Abort.fail(terminated(path, operation))
                    ...
                    Abort.get(op(cur)).map { (next, v) =>
                        // Unsafe: transfer cleanup ownership in the same node that commits the staged entry.
                        Sync.Unsafe.defer {
                            val committed = state.unsafe.compareAndSet(cur, next)
                            if committed then onCommit(v)
                            committed
                        }.map {
    ...
    private def ensureCasePolicy(using Frame): Unit < S =
        stateGet.map { current =>
            if current.casePolicy.isDefined then ()
            else
                lower.defaultCaseSensitivity.map { policy =>
                    val insensitive = policy == Glob.CaseSensitivity.Insensitive
                    modifyPure { s =>
                        if s.casePolicy.isDefined then s
                        else
                            s.copy(
                                upper = UpperTrie.empty(insensitive),
                                readSet = ReadSet.empty(insensitive),
                                provenance = PathTrie.empty[Provenance](insensitive),
                                casePolicy = Present(policy)
                            )
                    }
                }
        }
    ...
    private def ancestorWhiteout(s: OverlayState, cp: CanonicalPath): Boolean =
        s.upper.nearestAncestorValue(cp).exists {
            ...
            case Upper.OpaqueDir(_, _, _) => true
            case _                        => false
        }
    ...
    def isDirectory(path: Path)(using Frame): Boolean < (S & Abort[FileReadException]) =
        canonical(path).map { cp =>
            ...
            withState { s =>
                s.upper.get(cp) match
                    case Present(Upper.Whiteout)                             => false
                    case Present(Upper.OpaqueDir(_, _, _))                   => true
                    case Present(Upper.Entry(Path.Entry.Directory(_), _, _)) => true
                    case Present(Upper.Entry(Path.Entry.File(_, _), _, _))   => false
                    case Absent =>
                        if ancestorWhiteout(s, cp) then false
    ...
    def isRegularFile(path: Path)(using Frame): Boolean < (S & Abort[FileReadException]) =
        canonical(path).map { cp =>
            ...
            withState { s =>
                s.upper.get(cp) match
                    case Present(Upper.Whiteout)                             => false
                    case Present(Upper.OpaqueDir(_, _, _))                   => false
                    case Present(Upper.Entry(Path.Entry.Directory(_), _, _)) => false
                    case Present(Upper.Entry(Path.Entry.File(_, _), _, _))   => true
                    case Absent =>
                        if ancestorWhiteout(s, cp) then false
    ...
    def readBytes(path: Path)(using Frame): Span[Byte] < (S & Abort[FileReadException]) =
        canonical(path).map { cp =>
            ...
            withState { s =>
                s.upper.get(cp) match
                    case Present(Upper.Entry(Path.Entry.File(bytes, _), _, _)) => bytes
    ...
    def stat(path: Path)(using Frame): Path.PathStat < (S & Abort[FileReadException]) =
        canonical(path).map { cp =>
            ...
            withState { s =>
                s.upper.get(cp) match
                    case Present(Upper.Entry(Path.Entry.File(_, ps), _, _))   => ps
                    case Present(Upper.Entry(Path.Entry.Directory(ps), _, _)) => ps
                    case Present(Upper.OpaqueDir(ps, _, _))                   => ps
                    case Present(Upper.Whiteout)                              => Abort.fail(FileNotFoundException(path))
                    case Absent =>
                        if ancestorWhiteout(s, cp) then Abort.fail(FileNotFoundException(path))
                        else
        ...
        walkCollect(path, maxDepth).map { paths =>
            new Path.WalkHandle:
                ...
                def next()(using AllowUnsafe, Frame): Result[FileReadException | FileStructureException, Maybe[Path]] =
                    Result.succeed(if it.hasNext then Maybe(it.next()) else Maybe.empty)
    ...
    def list(path: Path)(using Frame): Chunk[Path] < (S & Abort[FileReadException | FileStructureException]) =
        canonical(path).map { cp =>
            ...
            withState { s =>
                s.upper.get(cp) match
                    case Present(Upper.Whiteout)                           => Abort.fail(FileNotFoundException(path))
                    case Present(Upper.Entry(Path.Entry.File(_, _), _, _)) => Abort.fail(FileNotADirectoryException(path))
                    case maybeOpaque =>
                        ...
                        def segmentKey(segment: String): String =
                            if s.casePolicy.contains(Glob.CaseSensitivity.Insensitive) then segment.toLowerCase(java.util.Locale.ROOT)
                            else segment
                        val upperSegs: Set[String] = upperChildren.map(entry => segmentKey(entry._1)).toSet
                                    ...
                                    if !exists then Chunk.empty[Path]
                                        ...
                                        lower.list(lp).map { children =>
                                            ...
                                            observe(cp, Observed.DirectoryListing(children.flatMap(_.name).toSet)).andThen {
                                                ...
                                                children.collect {
                                                    case c if !upperSegs.contains(segmentKey(c.parts.last)) => path / c.parts.last
                                                }
                                            }
                                        }
                                }
    ...
    def appendBytes(path: Path, value: Span[Byte], options: Path.WriteOptions)(using
        ...
        ensureWriteParent(path, options).andThen(canonical(path).map { cp =>
            ...
            transact[FileReadException | FileWriteException, Unit](path, FileSystemOperation.Write) { s =>
                s.upper.get(cp) match
                    case Present(Upper.Entry(Path.Entry.File(existing, _), _, _)) =>
    ...
    def truncate(path: Path, size: Long)(using Frame): Unit < (S & Abort[FileReadException | FileWriteException]) =
        canonical(path).map { cp =>
            ...
            transact[FileReadException | FileWriteException, Unit](path, FileSystemOperation.Write) { s =>
                s.upper.get(cp) match
                    case Present(Upper.Entry(Path.Entry.File(bytes, _), _, _)) =>
    ...
    def setLastModified(path: Path, epochMs: Long)(using Frame): Unit < (S & Abort[FileReadException | FileWriteException]) =
        canonical(path).map { cp =>
            ...
            transact[FileReadException | FileWriteException, Unit](path, FileSystemOperation.Write) { s =>
                ...
                s.upper.get(cp) match
                    case Present(Upper.Entry(Path.Entry.File(bytes, stat), _, _)) => (stagedFile(s, bytes, stat), ())
                    case Present(Upper.Entry(Path.Entry.Directory(stat), _, _))   => (stagedDir(s, stat, opaque = false), ())
                    case Present(Upper.OpaqueDir(stat, _, _))                     => (stagedDir(s, stat, opaque = true), ())
                    case Present(Upper.Whiteout)                                  => Abort.fail(FileNotFoundException(path))
                    case Absent =>
                        if ancestorWhiteout(s, cp) then Abort.fail(FileNotFoundException(path))
                        else
    ...
    def mkDir(path: Path)(using Frame): Unit < (S & Abort[FileReadException | FileStructureException]) =
        canonical(path).map { cp =>
            ...
            transact[FileReadException | FileStructureException, Unit](path, FileSystemOperation.Create) { s =>
                ...
                s.upper.get(cp) match
                    case Present(Upper.OpaqueDir(_, _, _))                   => (s, ()) // already opaque dir
                    case Present(Upper.Entry(Path.Entry.Directory(_), _, _)) => (s, ()) // already a dir in upper
                    case _                                                   =>
                ...
                canonical(to).map { toCp =>
                    ...
                    transact[FileReadException | FileWriteException | FileStructureException, Unit](
                        ...
                        resolveEntry(from).map { resolved =>
                            if s.upper.samePath(fromCp, toCp) then (s, ())
                            else
                                checkMoveTarget(to, options.replace) {
                                    resolved match
                                        case file: Path.Entry.File =>
                                            ...
                                            // reference keeps the committed bytes independent of later source writes.
                                            // Its permission source is retained separately until the commit stages it.
                                            ...
                                            val moved = s.copy(
                                                upper = s.upper
                                                    ...
                                                    .updated(toCp, Upper.Whiteout)
                                                    .updated(
                                                        toCp,
                                                        Upper.Entry(
                                                            file,
                                                            Present(file.stat.lastModifiedMs),
                                                            fileSourceAccess(s.upper, fromCp)
                                                        )
                                                    )
                                            )
                                        ...
                                        case _: Path.Entry.Directory =>
                                            ...
                                            collectSubtree(from).map { nodes =>
                                                val upperT = stageSubtree(s.upper, fromCp, toCp, nodes, preserveSource = true)
                            ...
                            checkMoveTarget(to, options.replace) {
                                resolved match
                                    case file: Path.Entry.File =>
                                        ...
                                        val copiedState = s.copy(upper =
                                            s.upper.updated(
                                                toCp,
                                                Upper.Entry(
                                                    copied,
                                                    mtime,
                                                    s.upper.get(fromCp).map(_.accessRequirement).getOrElse(Access.Default)
                                                )
                                            )
                                        )
                                    ...
                                    case _: Path.Entry.Directory =>
                                        ...
                                        collectSubtree(from).map { nodes =>
                                            val upperT = stageSubtree(s.upper, fromCp, toCp, nodes, options.copyAttributes)
                                            (
    ...
    // A moved file retains its original access controls even when its new name has never existed
    // in lower. Resolve an earlier move or durable replacement to the same original source.
    private def fileSourceAccess(upper: UpperTrie, source: CanonicalPath): Access =
        upper.get(source).map(_.accessRequirement) match
            case Present(access: Access.PreserveFile) => access
            case _                                    => Access.PreserveFile(source.parts)

    // Materialize a preorder subtree (root-first) captured by collectSubtree under `to`. Each node
    // becomes an upper entry carrying its captured bytes and timestamp. File permissions retain
    // their source path so staging can read them before commit applies any removals.
    //
    ...
    private def stageSubtree(
        ...
        nodes.foldLeft(upper) { case (u, (suffix, entry)) =>
            ...
            val access   = upper.get(sourceCp).map(_.accessRequirement).getOrElse(Access.Default)
            ...
            entry match
                case Path.Entry.File(bytes, stat) =>
                    ...
                    val fileAccess = if preserveSource then fileSourceAccess(upper, sourceCp) else access
                    u.updated(targetCp, Upper.Entry(Path.Entry.File(bytes, targetStat), mtime, fileAccess))
                case Path.Entry.Directory(stat) =>
                    val targetStat = if copyAttributes then stat else Path.PathStat(0L, stat.sizeBytes)
                    val mtime      = if copyAttributes then Present(stat.lastModifiedMs) else Absent
                    u.updated(targetCp, Upper.OpaqueDir(targetStat, mtime, access))
            end match
        }
    end stageSubtree
    ...
    private def resolveEntry(path: Path)(using Frame): Path.Entry < (S & Abort[FileReadException | FileStructureException]) =
        canonical(path).map { cp =>
            ...
            withState { s =>
                s.upper.get(cp) match
                    case Present(Upper.Entry(e, _, _))        => e
                    case Present(Upper.OpaqueDir(stat, _, _)) => Path.Entry.Directory(stat): Path.Entry
                    case Present(Upper.Whiteout)              => Abort.fail(FileNotFoundException(path))
                    case Absent =>
                        if ancestorWhiteout(s, cp) then Abort.fail(FileNotFoundException(path))
                        else
        ...
        ensureWriteParent(path, options).andThen(canonical(path).map { cp =>
            ...
            withState { s =>
                val upperSeed: Maybe[Span[Byte]] =
                    if append then
                        s.upper.get(cp) match
                            case Present(Upper.Entry(Path.Entry.File(bytes, _), _, _)) => Present(bytes)
                            case _                                                     => Absent
    ...
    private[kyo] def privateTempDir(prefix: String)(using Frame): Path.TempDirHandle < (S & Sync & Abort[FileStructureException]) =
        tempDir(prefix).map { handle =>
            canonical(handle.path).map { cp =>
                stage(handle.path, FileSystemOperation.Create) { current =>
                    current.copy(upper =
                        current.upper.updated(
                            cp,
                            Upper.Entry(
                                Path.Entry.Directory(Path.PathStat(0L, 0L)),
                                Absent,
    ...
    private[kyo] def privateMkDir(path: Path)(using Frame): Unit < (S & Abort[FileReadException | FileStructureException]) =
        canonical(path).map { cp =>
            transact[FileReadException | FileStructureException, Unit](path, FileSystemOperation.Create) { current =>
                exists(path).map { found =>
                    val valid: Boolean < (S & Abort[FileReadException]) = if found then isDirectory(path) else true
                    valid.map { directory =>
                        if !directory then Abort.fail(FileNotADirectoryException(path))
                        else
                            val entry = current.upper.get(cp) match
                                case Present(Upper.Entry(body, mtime, _))     => Upper.Entry(body, mtime, Access.PrivateDirectory)
                                case Present(Upper.OpaqueDir(stat, mtime, _)) => Upper.OpaqueDir(stat, mtime, Access.PrivateDirectory)
                                case _ => Upper.Entry(Path.Entry.Directory(Path.PathStat(0L, 0L)), Absent, Access.PrivateDirectory)
                            (current.copy(upper = current.upper.updated(cp, entry)), ())
                    }
                }
            }
        }
    ...
    def temp(prefix: String, suffix: String)(using Frame): Path.TempFileHandle < (S & Sync & Abort[FileStructureException]) =
        // Unsafe: monotone counter increment for a unique staged file name. Same reasoning as tempDir:
        // hashing the prefix collides whenever two calls pass the same interned literal, and each
        // handle's removal deletes the shared upper entry.
        Sync.Unsafe.defer(uniqueSeq.unsafe.getAndIncrement()).map { seq =>
            val file = Path(prefix + "-overlay-" + instanceSeed + "-" + seq.toHexString + suffix)
            val stat = Path.PathStat(0L, 0L)
            canonical(file).map { cp =>
                val token = stagedToken("write")
                stage(file, FileSystemOperation.Create) { cur =>
                    withProvenance(
                        cur.copy(upper = cur.upper.updated(cp, Upper.Entry(Path.Entry.File(Span.empty[Byte], stat), Absent))),
                        cp,
                        Provenance.Staged(token)
                    )
                }.andThen {
    ...
    private def ensureWriteChannelTarget(path: Path, open: FileSystem.WriteOpen, onAcquire: () => Unit = () => ())(using
        Frame
            ...
            found =>
                open match
                    ...
                    case FileSystem.WriteOpen.CreateNew =>
                        val stat  = Path.PathStat(0L, 0L)
                        val token = stagedToken("channel")
                        ensureWriteParent(path, Path.WriteOptions()).andThen {
                            canonical(path).map { cp =>
                                modify(path, FileSystemOperation.Channel)(
                                    { current =>
                                        current.upper.get(cp) match
                                            case Present(Upper.Entry(_, _, _) | Upper.OpaqueDir(_, _, _)) =>
                    ...
                    case FileSystem.WriteOpen.Create if !found =>
                        Abort.recover[FileAlreadyExistsException](_ =>
                            ensureWriteChannelTarget(path, FileSystem.WriteOpen.Existing, onAcquire)
                        )(
                            ensureWriteChannelTarget(path, FileSystem.WriteOpen.CreateNew, onAcquire)
                        )
                    case _ => Sync.defer(onAcquire())
        }
    ...
    private[kyo] def openWriteChannelUnscoped(
        ...
        Sync.defer {
            val ops      = new ChannelOps(path)
            val acquired = unscopedWriteChannel(ops, writeChannel(ops))
            val access: Access < (S & Abort[FileWriteException]) = preservePermissionsFrom match
                case Absent => Access.Default
                case Present(source) =>
                    canonical(source).map { cp =>
                        withState { current =>
                            current.upper.get(cp).map(_.accessRequirement) match
                                case Present(access: Access.PreserveFile) => access
                                case _                                    => Access.PreserveFile(cp.parts)
                        }
                    }
            access.map { requirement =>
                ensureWriteChannelTarget(path, open, () => onAcquire(acquired._3)).andThen {
                    canonical(path).map { cp =>
                        stage(path, FileSystemOperation.Channel) { current =>
                            val upper = current.upper.get(cp) match
                                case Present(Upper.Entry(body, mtime, _)) =>
                                    current.upper.updated(cp, Upper.Entry(body, mtime, requirement))
                                case _ => current.upper
                            current.copy(upper = upper)
                        }.andThen(acquired)
                    }
                }
            }
    ...
    /** Validates the staged directory; volatile staging has no persistence boundary to flush. */
    def syncDirectory(path: Path)(using Frame): Unit < (S & Abort[FileWriteException]) =
        def validate(candidate: Path): Unit < (S & Abort[FileReadException | FileWriteException]) =
            exists(candidate).map {
                case true => isDirectory(candidate).map { directory =>
                        if !directory then Abort.fail(FileNotADirectoryException(path))
                        else if candidate == path then ()
                        else Abort.fail(FileNotFoundException(path))
                    }
                case false => candidate.parent match
                        case Present(parent) => validate(parent)
                        case Absent          => Abort.fail(FileNotFoundException(path))
            }
        Abort.recover[FileReadException] {
            case error: (FileNotFoundException | FileAccessDeniedException | FileIsADirectoryException |
                    FileInvalidPathException | FileOutsideRootException | FileIOException) => Abort.fail(error)
        }(validate(path))
    end syncDirectory
    ...
    private[kyo] def siblingTemporary(target: Path, onAcquire: Path.TempFileHandle => Unit)(using
        ...
        FileSystem.siblingTemporary[S](this, target, onAcquire)
    ...
    def durableReplace(target: Path, bytes: Span[Byte])(using
        ...
        FileSystem.durableReplace[S & Sync](this, target, bytes)

    private[kyo] def replacementNeedsDefaultPermissions(path: Path)(using Frame): Boolean < (S & Abort[FileReadException]) =
        canonical(path).map { cp =>
            withState { current =>
                val source = fileSourceAccess(current.upper, cp) match
                    case Access.PreserveFile(source) => source
                    case _                           => cp.parts
                lower.replacementNeedsDefaultPermissions(pathFrom(source))
            }
        }
    ...
    private[kyo] def durableReplacePreserving(target: Path, bytes: Span[Byte], permissionSource: Path)(using
        ...
        FileSystem.durableReplace[S & Sync](this, target, bytes, Present(permissionSource))
    ...
    def tryLock(path: Path, mode: Path.LockMode, sentinelSuffix: String)(using
        ...
        lower.tryLock(path, mode, sentinelSuffix)
    ...
    def lock(path: Path, mode: Path.LockMode, wait: Path.LockWait, sentinelSuffix: String)(using
        ...
        lower.lock(path, mode, wait, sentinelSuffix)
    ...
    private def upperToEntry(u: Upper): Maybe[Path.Entry] =
        u match
            case Upper.Entry(e, _, _)      => Present(e)
            case Upper.OpaqueDir(st, _, _) => Present(Path.Entry.Directory(st))
    ...
    private def stagePlan(staging: StagingDir, indexed: Chunk[(ReplayEntry, Int)])(using
        Frame
    ): Chunk[ReplayEntry] < (S & Sync & Abort[FileSystemException]) =
        indexed.foldLeft[Chunk[ReplayEntry] < (S & Sync & Abort[FileSystemException])](Chunk.empty) { case (acc, (entry, i)) =>
            acc.map { prepared =>
                entry match
                    case file @ ReplayEntry.File(parts, bytes, _, _, permissionSource, _) =>
                        val source = pathFrom(permissionSource.getOrElse(parts))
                        lower.replacementNeedsDefaultPermissions(source).map { defaults =>
                            lower.durableReplacePreserving(staging.entry(i), bytes, source)
                                .andThen(prepared.appended(file.copy(defaultPermissions = defaults)))
                        }
                    case directory @ ReplayEntry.Directory(parts, _, _, _, _) =>
                        val target = pathFrom(parts)
                        val parent = target.parent.getOrElse(Path())
                        val chain: Chunk[Chunk[String]] < (S & Abort[FileReadException]) =
                            if parent == target then Chunk(target.parts)
                            else directoriesToSync(parent).map(target.parts +: _)
                        chain.map(paths => prepared.appended(directory.copy(syncDirectories = paths)))
                    case _ => prepared.appended(entry)
            }
        }

    // Capture before any directory creation. Persisting this chain in the intent also covers a
    // process that stops after mkdir: recovery must not mistake unsynced new parents for durable ones.
    private def directoriesToSync(path: Path)(using Frame): Chunk[Chunk[String]] < (S & Abort[FileReadException]) =
        lower.exists(path).map { exists =>
            val parent = path.parent.getOrElse(Path())
            if exists || parent == path then Chunk(path.parts)
            else directoriesToSync(parent).map(path.parts +: _)
        }
    ...
    private def applyDestructiveEntry(staging: StagingDir, index: Int, idempotent: Boolean)(
        mutate: => Unit < (S & Sync & Abort[FileSystemException])
    )(using Frame): Unit < (S & Sync & Abort[FileSystemException]) =
        val applied = staging.applied(index)
        val pending: Boolean < (S & Abort[FileSystemException]) =
            if idempotent then lower.exists(applied).map(found => !found) else true
        pending.map { pending =>
            if !pending then ()
            else
                mutate.andThen(writeSealed(applied, Span.empty[Byte]))
                    .andThen(lower.syncDirectory(staging.path))
        }
    end applyDestructiveEntry

    // Unlike overlay mkDir, replaying an already-present directory must retain its children.
    private def ensureReplayDirectory(target: Path, privateAccess: Boolean)(using
        Frame
    ): Unit < (S & Abort[FileSystemException]) =
        if privateAccess then lower.privateMkDir(target)
        else
            lower.exists(target).map { found =>
                if !found then lower.mkDir(target)
                else lower.isDirectory(target).map(directory => if directory then () else Abort.fail(FileNotADirectoryException(target)))
            }
    ...
    private def applyReplayEntry(staging: StagingDir, i: Int, entry: ReplayEntry, idempotent: Boolean)(using
        ...
        entry match
            case ReplayEntry.Whiteout(parts) =>
                val target = pathFrom(parts)
                applyDestructiveEntry(staging, i, idempotent) {
                    lower.removeAll(target).andThen(syncExistingAncestor(target.parent.getOrElse(Path())))
                }
            case ReplayEntry.Directory(parts, opaque, _, privateAccess, syncDirectories) =>
                val target = pathFrom(parts)
                ...
                val create = ensureReplayDirectory(target, privateAccess)
                val sync   = Kyo.foreachDiscard(syncDirectories)(parts => lower.syncDirectory(pathFrom(parts)))
                if opaque then
                    applyDestructiveEntry(staging, i, idempotent) {
                        lower.removeAll(target).andThen(create).andThen(sync)
                    }.andThen(create)
                else create.andThen(sync)
                end if
            case ReplayEntry.File(parts, bytes, _, mtime, _, defaultPermissions) =>
                val target = pathFrom(parts)
                val staged = staging.entry(i)
                val place: Unit < (S & Abort[FileSystemException]) =
                    if defaultPermissions then lower.durableReplace(target, bytes).andThen(lower.removeAll(staged))
                    else moveInto(staged, target)
                val materialize =
                    if idempotent then
                        lower.exists(staged).flatMap(has => if has then place else ())
                    else place
    ...
    private def restampDirectories(plan: Chunk[ReplayEntry])(using Frame): Unit < (S & Abort[FileSystemException]) =
        val dirs = plan.collect {
            case ReplayEntry.Directory(parts, _, Present(mtime), _, _) => (parts, mtime)
        }
    ...
    private def applyResolved(staging: StagingDir, plan: Chunk[ReplayEntry])(using
        ...
        stagePlan(staging, indexed).map { prepared =>
            runHook(afterStageHook).andThen { // crash point 1
                writeIntentLog(staging, prepared).andThen {
                    runHook(afterIntentLogHook).andThen { // crash point 2
                        prepared.zipWithIndex
                            .foldLeft[Unit < (S & Sync & Abort[FileSystemException])](()) { case (acc, (entry, i)) =>
                                acc.andThen {
                            ...
                            .andThen {
                                restampDirectories(prepared).andThen {
    ...
    private def withCommit(plan: Chunk[ReplayEntry])(using Frame): Unit < (S & Sync & Abort[FileSystemException]) =
        ...
        Sync.Unsafe.defer(uniqueSeq.unsafe.getAndIncrement())
            ...
            .flatMap { seq =>
                ...
                lower.privateTempDir(StagingDir.dirName(instanceSeed, seq)).map { handle =>
    ...
    // A retry may find the removed entry already absent. Sync the nearest surviving ancestor
    // anyway, since the prior process may have stopped before persisting that removal.
    private def syncExistingAncestor(path: Path)(using Frame): Unit < (S & Abort[FileSystemException]) =
        lower.exists(path).map { exists =>
            val parent = path.parent.getOrElse(Path())
            if exists || parent == path then lower.syncDirectory(path)
            else syncExistingAncestor(parent)
        }
    ...
    // Resolutions are path-by-path. A retained child still needs its private ancestor even when
    // the caller keeps or removes the conflicting directory entry itself.
    private def retainPrivateAncestors(original: UpperTrie, resolved: UpperTrie)(using
        Frame
    ): Result[FileNotADirectoryException, UpperTrie] =
        original.entries.foldLeft[Result[FileNotADirectoryException, UpperTrie]](Result.succeed(resolved)) {
            case (acc, (parts, entry)) =>
                acc.flatMap { upper =>
                    if entry.accessRequirement != Access.PrivateDirectory then Result.succeed(upper)
                    else
                        val cp = CanonicalPath.wrap(parts)
                        val retainedChildren = upper.descendantValues(cp).exists {
                            case (_, Upper.Whiteout) => false
                            case _                   => true
                        }
                        if !retainedChildren then Result.succeed(upper)
                        else
                            upper.get(cp) match
                                case Present(Upper.Entry(_: Path.Entry.File, _, _)) =>
                                    Result.fail(FileNotADirectoryException(pathFrom(parts)))
                                case Present(Upper.Whiteout) =>
                                    Result.succeed(upper.updated(
                                        cp,
                                        Upper.OpaqueDir(Path.PathStat(0L, 0L), Absent, Access.PrivateDirectory)
                                    ))
                                case Absent =>
                                    Result.succeed(upper.updated(
                                        cp,
            ...
            stateGet.map { s =>
                validate(s).map { conflicts =>
                    ...
                    conflicts.foldLeft[Chunk[(CanonicalPath, Resolution)] < (S & Abort[FileSystemException])](Chunk.empty) {
                        ...
                        Abort.get(retainPrivateAncestors(s.upper, newUpper)).map { guardedUpper =>
                            val resolved = s.copy(upper = guardedUpper)
                            modifyPure(_.copy(upper = guardedUpper)).andThen {
...
private[kyo] class WatchableOverlayFileSystem[S, S2](
    lower: FileSystem.Write[S],
    state: AtomicRef[OverlayFileSystem.OverlayState],
    uniqueSeq: AtomicLong,
    isolate: Isolate[S, Sync, S2]
) extends OverlayFileSystem[S](lower, state, uniqueSeq):
...
final private[kyo] class SyncOverlayFileSystem(
    lower: FileSystem.Write[Sync],
    state: AtomicRef[OverlayFileSystem.OverlayState],
    uniqueSeq: AtomicLong
) extends WatchableOverlayFileSystem[Sync, Sync](lower, state, uniqueSeq, Isolate.derive[Sync, Sync, Sync]), FileSystem.Watch
...
final private[kyo] class ForwardingLowerFileSystem extends FileSystem.Write[PathWrite]:
    ...
    private[kyo] def privateMkDir(path: Path)(using Frame): Unit < (PathWrite & Abort[FileReadException | FileStructureException]) =
        ArrowEffect.suspend(Tag[PathWrite], Path.Op.PrivateMkDir(path))
    private[kyo] def privateTempDir(prefix: String)(using Frame): Path.TempDirHandle < (PathWrite & Abort[FileStructureException]) =
        ArrowEffect.suspend(Tag[PathWrite], Path.Op.PrivateTempDir(prefix))
    ...
    def temp(prefix: String, suffix: String)(using Frame): Path.TempFileHandle < (PathWrite & Abort[FileStructureException]) =
        ArrowEffect.suspend(Tag[PathWrite], Path.Op.Temp(prefix, suffix))
    private[kyo] def siblingTemporary(target: Path, onAcquire: Path.TempFileHandle => Unit)(using
        Frame
    ): Path.TempFileHandle < (PathWrite & Abort[FileWriteException | FileStructureException]) =
        ArrowEffect.suspend(Tag[PathWrite], Path.Op.SiblingTemporary(target, onAcquire))
    def durableReplace(target: Path, bytes: Span[Byte])(using
        Frame
    ...
    private[kyo] def replacementNeedsDefaultPermissions(path: Path)(using Frame): Boolean < (PathWrite & Abort[FileReadException]) =
        ArrowEffect.suspend(Tag[PathWrite], Path.Op.ReplacementNeedsDefaultPermissions(path))
    private[kyo] def durableReplacePreserving(target: Path, bytes: Span[Byte], permissionSource: Path)(using
        Frame
    ): Unit < (PathWrite & Abort[FileReadException | FileWriteException | FileStructureException]) =
        ArrowEffect.suspend(Tag[PathWrite], Path.Op.DurableReplacePreserving(target, bytes, permissionSource))
    ...
    def tryLock(path: Path, mode: Path.LockMode, sentinelSuffix: String)(using
        ...
        Path.suspendTryLock(path, mode, sentinelSuffix)

    def lock(path: Path, mode: Path.LockMode, wait: Path.LockWait, sentinelSuffix: String)(using
        Frame
    ): Path.Lock < (PathWrite & Async & Scope & Abort[FileLockException]) =
        Path.suspendLock(path, mode, wait, sentinelSuffix)
