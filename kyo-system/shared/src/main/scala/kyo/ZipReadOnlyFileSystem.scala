...
final private[kyo] class ZipReadOnlyFileSystem(
    ...
    private def acquireLock(path: Path, mode: Path.LockMode, sentinelSuffix: String)(using
        ...
        AtomicBoolean.init(false).map { released =>
            modifyLocks { current =>
                current.get(Path(path.unsafe.show + sentinelSuffix).parts) match
                    case None =>
                        val next = current.updated(Path(path.unsafe.show + sentinelSuffix).parts, LockClaim(mode, Chunk(owner)))
                        Result.succeed((next, mkLock(path, mode, owner, released, sentinelSuffix)))
                    case Some(claim) if mode == Path.LockMode.Shared && claim.mode == Path.LockMode.Shared =>
                        val next =
                            current.updated(Path(path.unsafe.show + sentinelSuffix).parts, claim.copy(owners = claim.owners.append(owner)))
                        Result.succeed((next, mkLock(path, mode, owner, released, sentinelSuffix)))
        ...
        modifyLocks { current =>
            current.get(Path(path.unsafe.show + sentinelSuffix).parts) match
                case Some(claim) if claim.owners.exists(Path.LockOwnership.same(_, owner)) =>
                    ...
                    val next = if owners.isEmpty then current - Path(path.unsafe.show + sentinelSuffix).parts
                    else current.updated(Path(path.unsafe.show + sentinelSuffix).parts, claim.copy(owners = owners))
                    Result.succeed((next, ()))
    ...
    private def owns(path: Path, owner: Path.LockOwnership, sentinelSuffix: String)(using Frame): Boolean < Sync =
        locks.use(_.get(Path(path.unsafe.show + sentinelSuffix).parts).exists(_.owners.exists(Path.LockOwnership.same(_, owner))))
