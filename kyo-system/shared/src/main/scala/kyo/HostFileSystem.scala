...
private[kyo] object HostFileSystem:
    ...
    final class HostFileSystem extends FileSystem.Write[Sync], FileSystem.Watch:
        ...
        private[kyo] def tempFileHandle(temporary: Path)(using Frame): Path.TempFileHandle =
            new Path.TempFileHandle:
                ...
                // Unsafe: this handle removes only the sibling reserved by its creating service.
                def remove()(using AllowUnsafe): Unit = discard(temporary.unsafe.remove().getOrThrow)
        ...
        private[kyo] def openWriteChannelUnscoped(
            ...
            preservePermissionsFrom match
                case Present(target) =>
                    kyo.internal.DurableFileChannel.validatePaths(target, path).andThen {
                        val parent = path.parent.getOrElse(Path())
                        val prepareParent: Unit < (Sync & Abort[FileStructureException]) =
                            if parent.parts.isEmpty then () else mkDir(parent)
                        Abort.run[FileStructureException](prepareParent).map {
                            case Result.Success(_) => kyo.internal.DurableFileChannel.open(target, path, onAcquire)
                            case Result.Failure(_: FileAlreadyExistsException) => Abort.fail(FileNotADirectoryException(parent))
                            case Result.Failure(error)                         => Abort.fail(error)
                            case Result.Panic(error)                           => Abort.panic(error)
                        }
                    }
                case Absent => openOrdinaryWriteChannelUnscoped(path, open, onAcquire)

        private def openOrdinaryWriteChannelUnscoped(
            path: Path,
            open: FileSystem.WriteOpen,
            onAcquire: Path.ChannelCloseHandle => Unit
        )(using
            ...
            (Sync & Abort[FileWriteException | FileStructureException]) =
            // Unsafe: register cleanup ownership in the same node that acquires the host descriptor.
            Sync.Unsafe.defer {
                Abort.get(path.unsafe.openWriteChannelRaw(open).map { raw =>
    ...
    final class RootConfinedHostFileSystem(rootReal: Path) extends FileSystem.Write[Sync], FileSystem.Watch:
        ...
        private def confined(path: Path, operation: FileSystemOperation)(using
            ...
            Sync.Unsafe.defer(Abort.get(path.unsafe.exists(followLinks = false))).map {
        ...
        private def nearestExistingParent(path: Path)(using
            ...
            path.parent match
                ...
                case Present(p) =>
                    ...
                    Sync.Unsafe.defer(Abort.get(p.unsafe.exists(followLinks = false))).map {
        ...
        def openWalk(path: Path, maxDepth: Int, followLinks: Boolean)(using
            ...
            confined(path, FileSystemOperation.Walk).andThen {
                if !followLinks then host.openWalk(path, maxDepth, followLinks = false)
                else
                    host.realPath(path).andThen(Sync.defer {
                        require(maxDepth >= 0, "maxDepth must be non-negative")
                        new Path.WalkHandle:
                            private var pending = Chunk((path, 0, Chunk.empty[Path]))

                            def next()(using AllowUnsafe, Frame): Result[FileReadException | FileStructureException, Maybe[Path]] =
                                // Unsafe: evaluate one checked traversal step, preserving typed failures.
                                Sync.Unsafe.evalOrThrow(Abort.run[FileReadException | FileStructureException] {
                                    if pending.isEmpty then Absent
                                    else
                                        val (entry, depth, ancestors) = pending.head
                                        pending = pending.tail
                                        confined(entry, FileSystemOperation.Walk).map { real =>
                                            host.isDirectory(real).map { directory =>
                                                if directory && ancestors.contains(real) then
                                                    Abort.fail(FileIOException(
                                                        entry,
                                                        FileSystemOperation.Walk,
                                                        new IOException("Symbolic link cycle")
                                                    ))
                                                else if directory && depth < maxDepth then
                                                    host.list(real).map { children =>
                                                        val nextAncestors = ancestors :+ real
                                                        pending = children.map(child =>
                                                            (entry / child.name.get, depth + 1, nextAncestors)
                                                        ) ++ pending
                                                        Present(entry)
                                                    }
                                                else Present(entry)
                            ...
                            def close()(using AllowUnsafe): Unit = pending = Chunk.empty
                        end new
                    })
            }
        def write(path: Path, value: String, options: Path.WriteOptions)(using Frame): Unit < (Sync & Abort[FileWriteException]) =
        ...
        private def validateTemporaryName(value: String)(using Frame): Unit < Abort[FileInvalidPathException] =
            if value.exists(c => c == '/' || c == '\\' || c == 0.toChar) then
        ...
        def tempDir(prefix: String)(using Frame): Path.TempDirHandle < (Sync & Abort[FileStructureException]) =
            validateTemporaryName(prefix).andThen(Path.tempDirUnscoped(rootReal, prefix)).map { dir =>
                new Path.TempDirHandle:
                    def path: Path = dir
                    // Unsafe: removes only the directory atomically created within rootReal.
                    def remove()(using AllowUnsafe): Unit = discard(dir.unsafe.removeAll())
            }

        def temp(prefix: String, suffix: String)(using Frame): Path.TempFileHandle < (Sync & Abort[FileStructureException]) =
            validateTemporaryName(prefix).andThen(validateTemporaryName(suffix))
                .andThen(Path.tempUnscoped(rootReal, prefix, suffix)).map(host.tempFileHandle(_))
        ...
        private[kyo] def openWriteChannelUnscoped(
            ...
            (Sync & Abort[FileWriteException | FileStructureException]) =
            confined(path, FileSystemOperation.Channel)
                .andThen(preservePermissionsFrom match
                    case Present(target) => confined(target, FileSystemOperation.Channel).unit
                    case Absent          => ())
                .andThen(host.openWriteChannelUnscoped(path, open, onAcquire, preservePermissionsFrom))
        private[kyo] def openReadWriteChannelUnscoped(path: Path, open: FileSystem.WriteOpen)(using
            Frame
        ): (
            Path.ReadWriteChannel[Sync],
        ...
        def tryLock(path: Path, mode: Path.LockMode, sentinelSuffix: String)(using
            ...
            if sentinelSuffix.isEmpty || sentinelSuffix.exists(c => c == '/' || c == '\\') then
                ...
                confined(path, FileSystemOperation.Lock).map { resolved =>
                    val sentinel = Path(resolved.unsafe.show + sentinelSuffix)
                    confined(sentinel, FileSystemOperation.Lock).andThen(host.tryLock(path, mode, sentinelSuffix))
                }
        def lock(path: Path, mode: Path.LockMode, wait: Path.LockWait, sentinelSuffix: String)(using
            Frame
        ): Path.Lock < (Sync & Async & Scope & Abort[FileReadException | FileLockException]) =
