...
private[kyo] object HostFileSystem:
    ...
    final class HostFileSystem extends FileSystem.Write[Sync], FileSystem.Watch[Sync]:
        ...
        def defaultCaseSensitivity(using Frame): Glob.CaseSensitivity < Sync =
            if Platform.isWindows then Glob.CaseSensitivity.Insensitive else Glob.CaseSensitivity.Sensitive
        ...
        def realPath(path: Path)(using Frame): Path < (Sync & Abort[FileReadException]) =
                ...
                def release(candidate: Path.LockOwnership)(using Frame): Unit < (Sync & Abort[FileLockException]) =
                    if !Path.LockOwnership.same(owner, candidate) then Abort.fail(FileLockOwnershipLostException(path))
                        ...
                        Loop(()) { _ =>
                            state.get.map {
                                case 2 => Loop.done(())
                                case 1 => Loop.continue(())
                                case _ =>
                                    state.compareAndSet(0, 1).map {
                                        case false => Loop.continue(())
                                        case true =>
                                            Abort.run[FileLockException](Sync.Unsafe.defer(Abort.get(raw.release()))).map {
                                                case Result.Success(_)     => state.set(2).andThen(Loop.done(()))
                                                case Result.Failure(error) => state.set(0).andThen(Abort.fail(error))
                                                case Result.Panic(error)   => state.set(0).andThen(Abort.panic(error))
                                            }
                                    }
                            }
                        }
    ...
    final class RootConfinedHostFileSystem(rootReal: Path) extends FileSystem.Write[Sync], FileSystem.Watch[Sync]:
        ...
        def defaultCaseSensitivity(using Frame): Glob.CaseSensitivity < Sync = host.defaultCaseSensitivity
