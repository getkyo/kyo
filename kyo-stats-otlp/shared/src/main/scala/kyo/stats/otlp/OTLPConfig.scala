...
object OTLPConfig:
    ...
    /** Returns None if OTEL_EXPORTER_OTLP_ENDPOINT is not set — export is disabled. */
    private[otlp] def loadIfEnabled()(using AllowUnsafe): Option[OTLPConfig] =
        val endpoint = java.lang.System.getenv("OTEL_EXPORTER_OTLP_ENDPOINT")
        if endpoint == null then None
        else
            Some(OTLPConfig(
                endpoint = endpoint,
