...
class OTLPClientTest extends Test:
        ...
        "includes resource attributes" in run {
            withCollector { (config, traceCh, _) =>
                ...
                yield
                    ...
                    assert(attrs.exists(kv => kv.key == "service.name" && kv.value.stringValue == Some("test-client")))
                    assert(attrs.exists(kv => kv.key == "telemetry.sdk.name" && kv.value.stringValue == Some("kyo")))
                    assert(attrs.exists(kv => kv.key == "env" && kv.value.stringValue == Some("test")))
            }
        }
        ...
        "includes span attributes and events" in run {
            withCollector { (config, traceCh, _) =>
                ...
                yield
                    ...
                    assert(s.attributes.exists(kv => kv.key == "http.method" && kv.value.stringValue == Some("GET")))
        ...
        "handles partial success with rejected spans" in run {
            for
                ...
                traceHandler = mkTraceRoute.handler { req =>
                    traceCh.put(req.fields.body).andThen {
                        HttpResponse.ok.addField("body", ExportTraceResponse(
                            partialSuccess = Some(TracePartialSuccess(
                                rejectedSpans = 3,
        ...
        "sends counter metrics" in run {
            withCollector { (config, _, metricCh) =>
                ...
                yield
                    ...
                    assert(metrics.head.sum.get.dataPoints.head.asInt == Some("42"))
        ...
        "sends gauge metrics" in run {
            withCollector { (config, _, metricCh) =>
                ...
                yield
                    ...
                    assert(metrics.head.gauge.get.dataPoints.head.asDouble == Some(1073741824.0))
            }
        }
        ...
        "handles partial success with rejected data points" in run {
            for
                ...
                metricHandler = mkMetricRoute.handler { req =>
                    metricCh.put(req.fields.body).andThen {
                        HttpResponse.ok.addField("body", ExportMetricsResponse(
                            partialSuccess = Some(MetricsPartialSuccess(
    ...
    "buildResource" - {
        ...
        "includes service name and SDK metadata" in {
            ...
            assert(attrs.exists(kv => kv.key == "service.name" && kv.value.stringValue == Some("test-client")))
            assert(attrs.exists(kv => kv.key == "telemetry.sdk.name" && kv.value.stringValue == Some("kyo")))
            assert(attrs.exists(kv => kv.key == "telemetry.sdk.language" && kv.value.stringValue == Some("scala")))
            assert(attrs.exists(kv => kv.key == "telemetry.sdk.version" && kv.value.stringValue == Some("1.0.0")))
        }

        "includes custom resource attributes" in {
            val config = testConfig(0)
            val resource = OTLPClient.buildResource(config)
            assert(resource.attributes.exists(kv => kv.key == "env" && kv.value.stringValue == Some("test")))
        }
    }
