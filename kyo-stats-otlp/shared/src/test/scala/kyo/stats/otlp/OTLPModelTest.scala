...
class OTLPModelTest extends Test:
    ...
    "KeyValue" - {
        ...
        "with different value types" in run {
            ...
            assert(kvStr.value.stringValue == Present("hello"))
            assert(kvInt.value.intValue == Present("42"))
            assert(kvDbl.value.doubleValue == Present(3.14))
            assert(kvBool.value.boolValue == Present(true))
        }
    }
    ...
    "OTLPClient.buildResource" - {
        ...
        "includes SDK metadata" in run {
            ...
            assert(resource.attributes.exists(kv =>
                kv.key == "telemetry.sdk.language" && kv.value.stringValue == Present("scala")
            ))
            assert(resource.attributes.exists(kv =>
                kv.key == "telemetry.sdk.version" && kv.value.stringValue == Present("1.0.0")
            ))
        }
        ...
        "includes resource attributes" in run {
            ...
            assert(resource.attributes.exists(kv =>
                kv.key == "version" && kv.value.stringValue == Present("1.0")
            ))
        }
