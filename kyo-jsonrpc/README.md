...
The architecture is three layers stacked under one entry point. At the top, `JsonRpcHandler` owns the dispatch loop and exposes typed `call`, `notify`, `callWithProgress`, and `cancel` operations. Below it, `JsonRpcTransport` ferries one `JsonRpcEnvelope` at a time, in either direction. Below that, `JsonRpcWireTransport` + `JsonRpcFramer` + a `Schema[JsonRpcEnvelope]` together turn raw byte streams into envelopes, so that a custom transport (TCP socket, UDS, content-length stdio) only has to plug in at the byte level. Handlers fail with a typed `JsonRpcError` hierarchy organized by pipeline stage (parse, dispatch, execution, application), and user-domain errors register through `.error[E2](code, message)` on the route to flow back to peers as wire error responses.
...
The same constructor is used on both sides. A peer that only initiates requests passes no routes; a peer that only answers requests does not call `.call` on the handler. The snippet below builds both sides over a paired in-memory transport, then issues one round-trip:
...
`callPartialResults[In, T]` is for protocols where the peer streams *typed* partial results of the same shape; there is no separate "final" value; the result is a `Stream[T, ...]` that completes when the peer sends the closing frame:
...
The framer and codec default to `JsonRpcFramer.lineDelimited` and the strict `Schema[JsonRpcEnvelope]` given; override either argument for protocols that frame differently.
...
val uds: Unit < (Async & Scope & Abort[Throwable]) =
    JsonRpcTransport.unixDomain(Path("/tmp/myrpc.sock")).map { transport =>
...
A `Schema[JsonRpcEnvelope]` encodes and decodes `JsonRpcEnvelope` values to and from structural `Structure.Value` trees. Two instances ship on the `JsonRpcEnvelope` companion:

- the `given Schema[JsonRpcEnvelope]` (strict JSON-RPC 2.0): the `"jsonrpc":"2.0"` field is emitted on encode and non-standard `extras` are ignored. The default; summon it with `summon[Schema[JsonRpcEnvelope]]`.
- `JsonRpcEnvelope.lenientSchema`: omits the `"jsonrpc"` version field on encode and flattens `extras` to the top level on both encode and decode.

Set the active schema via `JsonRpcHandler.Config.codec` or pass it directly to `JsonRpcTransport.fromWire` and `JsonRpcTransport.stdio`. Encoding is pure but throws a `JsonRpcError` for the rare unencodable cases (a Malformed envelope, or a lenient `extras` key that collides with a reserved key); the transport catches that and logs it.

> **Note:** `JsonRpcEnvelope.lenientSchema` is asymmetric. Decoding tolerates a missing `jsonrpc` version field, but encoding still validates that user-supplied `extras` do not collide with reserved keys (`jsonrpc`, `id`, `method`, `params`, `result`, `error`); collisions raise `JsonRpcInvalidRequestError`.
...
val handshakeGate: JsonRpcMessageGate < Sync =
...
Extensions on `JsonRpcId`: `fold(ifLong, ifString)`, `isLong`, `isString`, `toLongMaybe`, `toStringMaybe`. A `Schema[JsonRpcId]` and a `CanEqual[JsonRpcId, JsonRpcId]` are provided.
