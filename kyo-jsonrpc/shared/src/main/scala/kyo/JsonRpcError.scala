...
sealed abstract class JsonRpcError(
    code0: Int,
    val message: String,
    data0: Maybe[Structure.Value],
    cause: String | Throwable = ""
)(using Frame)
    extends KyoException(message, cause):
    // `code`/`data` are members (not val parameters) so leaf case classes can supply them as their own
    // `override val` case fields; overriding a `val` parameter is rejected under -Werror.
    def code: Int                    = code0
    def data: Maybe[Structure.Value] = data0
end JsonRpcError
