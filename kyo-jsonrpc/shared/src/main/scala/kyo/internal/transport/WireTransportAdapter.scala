...
final private[kyo] class WireTransportAdapter(
    ...
    def send(env: JsonRpcEnvelope)(using Frame): Unit < (Async & Abort[Closed | JsonRpcError]) =
        ...
        // message, or a Lenient reserved-extras key); Abort.catching reifies that into this method's
        // declared JsonRpcError row, so an envelope that never reached the wire cannot read as sent.
