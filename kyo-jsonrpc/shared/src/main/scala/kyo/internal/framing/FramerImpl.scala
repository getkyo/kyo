...
private[kyo] object FramerImpl:
    ...
    private def splitOnLf(buf: Chunk[Byte]): (Chunk[Chunk[Byte]], Chunk[Byte]) =
        // Hot-path frame splitter: an imperative scan over the primitive byte array with single-owner
        // local state (cursors + frame buffer); a @tailrec rewrite would be neither clearer nor faster.
    ...
    private def splitContentLength(buf: Chunk[Byte]): (Chunk[Chunk[Byte]], Chunk[Byte]) =
        // Hot-path frame drain: imperative loop with single-owner local state (remaining buffer + done flag).
        val builder = scala.collection.mutable.ArrayBuffer.empty[Chunk[Byte]]
        var rest    = buf
        var done    = false
        while !done do
    ...
    private def parseContentLengthHeader(headers: String): Maybe[Int] =
        // Header scan: imperative loop over header lines with single-owner local state, stopping at the match.
    ...
    private def indexOf(haystack: Array[Byte], needle: Array[Byte]): Int =
        // Hot-path byte-substring search over primitive arrays with single-owner cursors; the early
        // return exits on first match.
        var i     = 0
