...
object Requests:
    ...
    /** Sends an HTTP request using a function to modify the basic request
      ...
      */
    def apply[E, A](f: BasicRequest => Request[Either[E, A], Any])(using Frame, Tag[Abort[E]]): A < (Async & Abort[FailedRequest | E]) =
    ...
    /** Sends an HTTP request
      ...
      *   The response body wrapped in an effect
      */
    def request[E, A](req: Request[Either[E, A], Any])(using Frame, Tag[Abort[E]]): A < (Async & Abort[FailedRequest | E]) =
