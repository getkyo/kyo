...
object MonadLawsTest extends ZIOSpecDefault {
  ...
  type Myo[+T] = T > (Defers | Futures)

  val listGenF: GenF[Any, Myo] =
    new GenF[Any, Myo] {
      def apply[R, A](gen: Gen[R, A])(implicit trace: Trace) =
        Gen.oneOf(
            gen.map(v => (v: A > (Defers | Futures))),
            gen.map(v => Defers(v)),
            gen.map(v => Futures.fork(v)),
            gen.map(v => Defers(Futures.fork(v))),
            gen.map(v => Futures.fork(Defers(v)))
        )
    }
      ...
      override def derive[A: Equal]: Equal[Myo[A]] =
        new Equal[Myo[A]] {
          protected def checkEqual(l: Myo[A], r: Myo[A]): Boolean =
            def run(m: Myo[A]): A = Futures.block((m < Defers)(_.run()), Duration.Inf)
            run(l) == run(r)
        }
    }
