...
object Resource:
    ...
    /** Ensures that the given effect is executed when the resource is released, with information about the computation's outcome.
      ...
      * exception. The finalizer receives a `Maybe[Throwable]` which will be `Absent` if the computation succeeded, or `Present` if it
      * failed.
      *
      * @param f
      ...
      *   A unit value wrapped in Resource and IO effects.
      */
    inline def ensure(inline f: Maybe[Throwable] => Any < (Async & Abort[Throwable]))(using frame: Frame): Unit < (Resource & IO) =
    ...
    sealed abstract class Finalizer:
        def ensure(v: Maybe[Throwable] => Any < (Async & Abort[Throwable]))(using Frame): Unit < IO

    object Finalizer:
        sealed abstract class Awaitable extends Finalizer:
            def close(ex: Maybe[Throwable])(using Frame): Unit < IO
        ...
        object Awaitable:
            object Unsafe:
                def init(parallelism: Int)(using frame: Frame, u: AllowUnsafe): Awaitable =
                    new Awaitable:
                        val queue = Queue.Unbounded.Unsafe.init[Maybe[Throwable] => Any < (Async & Abort[Throwable])](
