...
object Retry:
    ...
    /** Retries an operation using the default retry schedule.
      ...
      */
    def apply[E: ConcreteTag](using Frame)[A, S](v: => A < (Abort[E] & S))(using Tag[Abort[E]]): A < (Async & Abort[E] & S) =
    ...
    /** Retries an operation using a custom policy builder.
      ...
      *   The result of the operation, or an abort if all retries fail.
      */
    def apply[E: ConcreteTag](using
        Frame
    )[A, S](schedule: Schedule)(v: => A < (Abort[E] & S))(using Tag[Abort[E]]): A < (Async & Abort[E] & S) =
        Abort.run[E](v).map {
