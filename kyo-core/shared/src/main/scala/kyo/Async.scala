...
object Async:
    ...
    /** Runs an asynchronous computation and returns a Fiber.
      ...
      */
    def run[E, A: Flat, S](
        using scope: Scope.Contextual[S]
    )(v: => A < (Abort[E] & Async & S))(using Frame): Fiber[E, A] < (IO & S) =
        scope.runInternal((trace, context) =>
    ...
    /** Runs an asynchronous computation with interrupt masking.
      ...
      */
    def mask[E, A: Flat, S](
        using scope: Scope.Stateful[S]
    )(v: => A < (Abort[E] & Async & S))(
        using frame: Frame
    ): A < (Abort[E] & Async & S) =
        scope.use { state =>
            Async.run(scope.resume(state, v)).map(_.mask.map(_.use(scope.restore(_, _))))
        }
    ...
    def timeout[E, A: Flat, S](
        ...
        scope.use { state =>
            Async.run(scope.resume(state, v)).map { (task: Fiber[E | Timeout, (scope.State, A)]) =>
                Clock.use { clock =>
                    IO.Unsafe {
    ...
    def race[E, A: Flat, S](
        ...
        if seq.isEmpty then seq(0)
            ...
            scope.use { state =>
                Fiber.race(seq.map(scope.resume(state, _))).map(_.use(scope.restore(_, _)))
            }
    ...
    def gather[E, A: Flat, S](
        ...
        scope.use { state =>
            val x = Fiber.gather(max)(seq.map(scope.resume(state, _)))
            x.map(_.use(chunk => Kyo.collect(chunk.map(scope.restore(_, _)))))
        }
    ...
    def parallelUnbounded[E, A: Flat, S](
        ...
        seq.size match
            ...
            case _ =>
                scope.use { state =>
                    Fiber.parallelUnbounded(seq.map(scope.resume(state, _)))
                        .map(_.use(r => Kyo.collect(r.map(scope.restore(_, _)))))
                }
        end match
    ...
    def parallel[E, A: Flat, S](
        ...
        seq.size match
            ...
            case n =>
                scope.use { state =>
                    Fiber.parallel(parallelism)(seq.map(scope.resume(state, _)))
                        .map(_.use(r => Kyo.collect(r.map(scope.restore(_, _)))))
                }
