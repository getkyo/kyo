...
object System:
    ...
    def apply(u: Unsafe): System =
        new System:
            def env[E, A](name: String)(using p: Parser[E, A], frame: Frame, tag: Tag[Abort[E]]): Maybe[A] < (Abort[E] & Sync) =
                Sync.Unsafe {
                    u.env(name) match
                        case Absent     => Absent
                        case Present(v) => Abort.get(p(v).map(Maybe(_)))
                }
            def property[E, A](name: String)(using p: Parser[E, A], frame: Frame, tag: Tag[Abort[E]]): Maybe[A] < (Abort[E] & Sync) =
                Sync.Unsafe {
    ...
    def env[A](using
        Frame
    )[E](name: String)(using parser: Parser[E, A], reduce: Reducible[Abort[E]], tag: Tag[Abort[E]]): Maybe[A] < (reduce.SReduced & Sync) =
