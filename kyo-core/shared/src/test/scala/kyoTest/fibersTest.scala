...
class fibersTest extends KyoTest:
    ...
    def loop(ref: AtomicInt, r: Double): Unit < IOs =
        ref.incrementAndGet.map(_ => loop(ref, r + Math.pow(Math.random(), Math.random())))

    "load" in run {
        for
            a <- Atomics.initInt(0)
            b <- Fibers.parallelFiber(Seq.fill(100)(loop(a, 1)))
            c <- b.get
        yield succeed
    }
