...
class TraceTest extends KyoTest:
    ...
    "no param" in {
        def test(using t: Trace) = t.sourceCode
        assert(test == "test")
    }

    "one pram" in {
        def test(i: Int)(using t: Trace) = t.sourceCode
        assert(test(42) == "test(42)")
    }

    "two params" in {
        def test(a: Int, b: String)(using t: Trace) = t.sourceCode
    ...
    "infix method" in pendingUntilFixed {
        object Test:
            infix def test(v: Int)(using t: Trace) = t.sourceCode
    ...
    "default parameter" in {
        def test(a: Int = 0)(using t: Trace) = t.sourceCode
        assert(test() == "test()")
        assert(test(1) == "test(1)")
    }

    "implicit parameter" in {
        def test(a: Int)(implicit b: String, t: Trace) = t.sourceCode
        implicit val str: String                       = "hello"
        assert(test(1) == "test(1)")
    }

    "type parameter" in {
        def test[T](a: T)(using t: Trace) = t.sourceCode
        assert(test(1) == "test(1)")
    ...
    "varargs" in {
        def test(a: Int*)(using t: Trace) = t.sourceCode
    ...
    "method with backticks" in {
        def `test-method`(a: Int)(using t: Trace) = t.sourceCode
        assert(`test-method`(1) == "`test-method`(1)")
    }

    "parameter with backticks" in {
        def test(`a-param`: Int)(using t: Trace) = t.sourceCode
        assert(test(1) == "test(1)")
    }

    "type parameter with backticks" in {
        def test[`T-Type`](a: `T-Type`)(using t: Trace) = t.sourceCode
        assert(test(1) == "test(1)")
    }

    "mixed backticks and regular identifiers" in {
        def `test-method`(`a-param`: Int, bParam: String)(using t: Trace) = t.sourceCode
        assert(`test-method`(1, "hello") == "`test-method`(1, \"hello\")")
    }

    "unicode identifiers" in {
        def αβγ(δεζ: Int)(using t: Trace) = t.sourceCode
        assert(αβγ(1) == "αβγ(1)")
    }

    "operator method" in {
        def ++(a: Int)(using t: Trace) = t.sourceCode
        assert(++(1) == "++(1)")
    }

    "method with keyword identifiers" in {
        def `val`(`def`: Int, `type`: String)(using t: Trace) = t.sourceCode
        assert(`val`(1, "hello") == "`val`(1, \"hello\")")
    }

    "nested method calls" in {
        def outer(a: String)(using t: Trace) = t.sourceCode
        def inner(b: String)(using t: Trace) = t.sourceCode
    ...
    "method with default and non-default parameters" in {
        def test(a: Int, b: String = "world")(using t: Trace) = t.sourceCode
        assert(test(1) == "test(1)")
        assert(test(1, "hello") == "test(1, \"hello\")")
    }
    ...
    "method with a type parameter bounded by a complex type" in {
        def test[T <: Either[String, Int]](a: T)(using t: Trace) = t.sourceCode
    ...
    "identation" in {
        def test(f: Int)(using t: Trace) = t.sourceCode
        assert(
            test {
                1 + 2
            } ==
