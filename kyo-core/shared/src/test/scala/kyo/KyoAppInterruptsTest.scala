...
class KyoAppInterruptsTest extends kyo.test.Test[Any]:
    ...
    "an ordinary failure still propagates, rendered to stderr rather than stdout" in {
        ...
        // escape before the captured streams were read, and the stdout assertion would hold vacuously
        // whatever the runner printed.
        val runner                   = new Recording
        ...
        assert(out.isEmpty, s"nothing may reach stdout; got: $out")
        assert(err.contains("boom"), s"the failure must be reported on stderr; got: $err")
    }
