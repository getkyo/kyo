...
class ExitCodeTest extends Test:
    ...
    "ExitCode(0) is ExitCode.Success" in run {
        assert(ExitCode(0) == ExitCode.Success)
    }

    "ExitCode(1) is ExitCode.Failure(1)" in run {
        assert(ExitCode(1) == ExitCode.Failure(1))
    }

    "ExitCode(130) is ExitCode.Signaled(2) (SIGINT)" in run {
    ...
    "ExitCode.Success.toInt returns 0" in run {
        assert(ExitCode.Success.toInt == 0)
    }

    "ExitCode.Failure(42).toInt returns 42" in run {
        assert(ExitCode.Failure(42).toInt == 42)
    }

    "ExitCode.Signaled(15).toInt returns 143" in run {
    ...
    "ExitCode.Success.isSuccess is true" in run {
        assert(ExitCode.Success.isSuccess == true)
    }

    "ExitCode.Failure(1).isSuccess is false" in run {
        assert(ExitCode.Failure(1).isSuccess == false)
    }

    "ExitCode.Signaled(15).isSuccess is false" in run {
    ...
    "ExitCode(128) is Signaled(0) per POSIX encoding" in run {
        // 128 = 128 + 0, so it is Signaled(0) per POSIX convention
        assert(ExitCode(128) == ExitCode.Signaled(0))
    }

    "ExitCode(129) is Signaled(1) (SIGHUP)" in run {
    ...
    "ExitCode(128) should be Signaled(0) per POSIX convention" in run {
        // POSIX: exit codes >= 128 encode signals (128 + N)
        // ExitCode(128) should be Signaled(0), not Failure(128)
        assert(ExitCode(128) == ExitCode.Signaled(0))
    }

    "ExitCode boundary: 127 is Failure, 128 and above are Signaled" in run {
        assert(ExitCode(127) == ExitCode.Failure(127))
        assert(ExitCode(128) == ExitCode.Signaled(0))
        assert(ExitCode(129) == ExitCode.Signaled(1))
    }

    "ExitCode.toInt round-trips for signal numbers 0 through 31" in run {
        (0 to 31).foreach { n =>
    ...
    "ExitCode with negative integer produces Failure" in run {
        assert(ExitCode(-1) == ExitCode.Failure(-1))
    }
    ...
    "signalName returns Present for named signals" in run {
    ...
    "signalName returns Absent for unknown signal numbers" in run {
        assert(ExitCode.Signaled(42).signalName == Absent)
        assert(ExitCode.Signaled(99).signalName == Absent)
    }

    "signalName returns Absent for non-Signaled exit codes" in run {
    ...
    "ExitCode(255) is Signaled(127)" in run {
        // 255 > 128, so 255 - 128 = 127
        assert(ExitCode(255) == ExitCode.Signaled(127))
    }

    "ExitCode.Signaled(0).toInt returns 128" in run {
        assert(ExitCode.Signaled(0).toInt == 128)
    }

    "signalName returns Absent for unknown signal number" in run {
        assert(ExitCode.Signaled(100).signalName == Absent)
    }

    "signalName returns Present for known signals" in run {
        assert(ExitCode.Signaled(9).signalName == Present("SIGKILL"))
        assert(ExitCode.Signaled(15).signalName == Present("SIGTERM"))
    }

    "ExitCode with values above 255 are Signaled" in run {
