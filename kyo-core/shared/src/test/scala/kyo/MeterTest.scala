...
class MeterTest extends kyo.test.Test[Any]:
    ...
    "REPRO: sustained contention must not re-enter a spent scope".notJs.notWasm in {
        val permits    = 2
        val callers    = 20
        val iterations = 1000
        for
        ...
        yield assert(
            outcome.isSuccess,
            s"re-entered a released scope after $count of ${callers * iterations} acquisitions: $outcome"
        )
        end for
    }
    ...
    //         // Isolated reproduction of the panic the stranding test above trips over. That test reports a
    //         // stranded waiter because it only asks whether the run settled; what actually ends it is a
    //         // Finalizer.Spent thrown from Stack.put, which refuses to install a scope whose release already
    //         // ran. Reported here on its own so the failure names the invariant it broke: one acquisition,
    //         // one scope, entered once.
    //         "REPRO: sustained contention must not re-enter a spent scope".notJs.notWasm in {
    //             val permits    = 2
    //             val callers    = 8
