    ...
    "close" - {
        ...
        "non-empty" in runJVM {
            for
                ...
                r <- c.close
                t <- Abort.run(c.empty)
            yield assert(r == Seq(1, 2) && t.isFail)
        }
        "pending take" in runJVM {
            for
                ...
                d <- f.getResult
                t <- Abort.run(c.full)
            yield assert(r == Seq() && d.isFail && t.isFail)
        }
        "pending put" in runJVM {
            ...
            yield assert(r == Seq(1, 2) && d.isFail && e.isFail)
        }
    ...
    "concurrency" - {
        ...
        "concurrent close attempts" in run {
            (for
                ...
                closeFiber <- Async.run(
                    latch.await.andThen(Async.parallel((1 to 100).map(_ => Abort.run(channel.close))))
                )
                _        <- latch.release
                offered  <- offerFiber.get
                backlog  <- closeFiber.get
        ...
        "offer, poll, put, take, and close" in run {
            ...
            yield
                ...
                assert(totalOffered - totalTaken == backlog.size)
                assert(isClosed)
            )
                .pipe(Choice.run).unit
                .repeat(repeats)
