    ...
    "ensure with Maybe[Throwable]" - {
        ...
        "receives Present with exception on failure" in run {
            ...
            io.handle(
                ...
                assert(result.panic.exists(_ eq exception))
            }
        }
        ...
        "with nested ensures passes correct exception to each handler" in run {
            ...
            io.handle(
                ...
                assert(result.panic.exists(_ eq testException))
            }
        }
        ...
        "can use exception information for recovery" in run {
            ...
            val io = Resource.ensure { t =>
                recoveryAction = t match
                    case Present(_: IllegalArgumentException) => "IllegalArgument"
                    case Present(_: IllegalStateException)    => "IllegalState"
