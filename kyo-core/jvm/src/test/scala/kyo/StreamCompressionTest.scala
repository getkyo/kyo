...
class StreamCompressionTest extends Test:
    ...
    private def jdkDeflate(bytes: Chunk[Byte], deflater: Deflater): Chunk[Byte] =
        ...
        val dif       = new DeflaterInputStream(new ByteArrayInputStream(bytes.toArray), deflater)
        val read      = dif.read(bigBuffer, 0, bigBuffer.length)
        Chunk.fromNoCopy(Array.copyOf(bigBuffer, read))
    end jdkDeflate
    ...
    private def jdkInflate(bytes: Chunk[Byte], inflater: Inflater): Chunk[Byte] =
        ...
        @tailrec def loop(acc: Chunk[Byte]): Chunk[Byte] =
            ...
            if read <= 0 then acc
            else loop(acc.concat(Chunk.fromNoCopy(bigBuffer.take(read))))
        end loop
    ...
    private def jdkGzip(bytes: Chunk[Byte], syncFlush: Boolean): Chunk[Byte] =
        ...
        gzos.finish()
        gzos.flush()
        Chunk.fromNoCopy(baos.toByteArray())
    end jdkGzip
    ...
    "deflate/inflate" - {
        "inflate please wrap" in run {
            for
                ...
                bytes     <- outStream.run.map(_.toArray)
        ...
        "short stream" in run {
            for
                ...
                bytes          <- inflatedStream.run.map(_.toArray)
                string         <- IO(new String(bytes, StandardCharsets.UTF_8))
        ...
        "inflate input (deflated larger than inflated)" in runJVM {
            for
                ...
                expected <- IO {
                    ...
                    ios.write(byteChunk.toArray, 0, byteChunk.length)
                    ios.flush()
                    ios.finish()
                    val inflatedBytes = Chunk.fromNoCopy(bos.toByteArray)
                    bos.close()
                    ios.close()
                    inflatedBytes
                }
        ...
        "fail early if header is corrupted" in run {
            Abort.run(
                ...
                result match
                    case Fail(_: DataFormatException) => assert(true)
                    case _                            => assert(false)
        }
    ...
    "gzip/gunzip" - {
        "short stream" in run {
            val shortTextChunk = Chunk.fromNoCopy(getBytes(shortText))
            for
                jdkGzippedStream <- IO {
                ...
                string          <- IO(new String(gunzippedChunk.toArray, StandardCharsets.UTF_8))
            yield assert(string == shortText)
            end for
        }
        ...
        "stream of two gzipped inputs" in run {
            for
                ...
                bytes           <- gunzippedStream.run.map(_.toArray)
                string          <- IO(new String(bytes, StandardCharsets.UTF_8))
            yield assert(string == shortText ++ otherShortText)
        }

        "long stream, no sync flush" in run {
            val longTextChunk = Chunk.fromNoCopy(getBytes(longText))
            for
                jdkGzippedStream <- IO {
        ...
        "with extras" in run {
            ...
            val totalHeader = Chunk.fromNoCopy(header ++ headerExtra ++ comment ++ fileName ++ crc16)
            for
                jdkGzippedStream <- IO {
                    val gzipByte        = jdkGzip(Chunk.fromNoCopy(getBytes(shortText)), syncFlush = false)
