...
object StreamCompression:
    ...
    final class StreamCompressionException(message: String) extends Exception(message)
    ...
    extension [Ctx](stream: Stream[Byte, Ctx])
        inline def deflate(
            bufferSize: Int = 1 << 15,
            ...
            deflateStream(stream, bufferSize, compressionLevel, strategy, flushMode, noWrap)
        end deflate
        ...
        inline def inflate(
            ...
            tag: Tag[Emit[Chunk[Byte]]],
            frame: Frame
        ): Stream[Byte, IO & Resource & Ctx & Abort[DataFormatException | StreamCompressionException]] =
            inflateStream(stream, bufferSize, noWrap)
        end inflate
        ...
        inline def gzip(
            ...
            gzipStream(stream, bufferSize, compressionLevel, strategy, flushMode)
        end gzip
        ...
        inline def gunzip(
            ...
            gunzipStream(stream, bufferSize)
        end gunzip
    end extension
    ...
    private def deflateStream[Ctx](
        ...
        enum DeflateState derives CanEqual:
            ...
            case DeflateInput(delfater: Deflater, emit: Ack < (Emit[Chunk[Byte]] & Ctx)) extends DeflateState
            case PullDeflater(
                delfater: Deflater,
                maybeEmitFn: Maybe[Ack => Ack < (Emit[Chunk[Byte]] & Ctx)],
                chunk: Chunk[Byte]
            ) extends DeflateState
            case EmitDeflated(delfater: Deflater, maybeEmitFn: Maybe[Ack => Ack < (Emit[Chunk[Byte]] & Ctx)], chunk: Chunk[Byte])
                extends DeflateState
        end DeflateState
        ...
        def emit(
            ...
            Tag[Emit[Chunk[Byte]]],
            Frame
        ): Ack < (Resource & IO & Emit[Chunk[Byte]] & Ctx) =
            val local = Local.init(new Array[Byte](bufferSize))
            ...
            Loop(DeflateState.Initialize):
                ...
                case DeflateState.DeflateInput(deflater, emit) =>
                    Emit.runFirst(emit).map {
                        case Present(bytes) -> cont =>
                            IO(deflater.setInput(bytes.toArray)).andThen(Loop.continue(DeflateState.PullDeflater(
                                deflater,
                                Present(cont),
                                Chunk.empty[Byte]
                            )))
                    ...
                    local.use: buffer =>
                        IO(deflater.deflate(buffer, 0, buffer.length, flushMode.value))
                            .map:
                                ...
                                case size =>
                                    IO(chunk.concat(Chunk.fromNoCopy[Byte](java.util.Arrays.copyOf(buffer, size))))
                ...
                case DeflateState.EmitDeflated(deflater, maybeEmitFn, chunk) =>
                    Emit.andMap(chunk): ack =>
                        maybeEmitFn match
                            case Present(emitFn) => Loop.continue(DeflateState.DeflateInput(deflater, emitFn(ack)))
                            case Absent          => IO(deflater.end()).andThen(Loop.done(Ack.Stop))
        end emit
    ...
    private def gzipStream[Ctx](
        ...
        enum GZipState derives CanEqual:
            ...
            case DeflateInput(delfater: Deflater, crc32: CRC32, emit: Ack < (Emit[Chunk[Byte]] & Ctx)) extends GZipState
            case PullDeflater(
                delfater: Deflater,
                crc32: CRC32,
                maybeEmitFn: Maybe[Ack => Ack < (Emit[Chunk[Byte]] & Ctx)],
            ...
            Loop(GZipState.Initialize):
                ...
                case GZipState.SendHeader(deflater, crc32) =>
                    IO {
                        ...
                        Emit.andMap(header)(_ => Loop.continue(GZipState.DeflateInput(deflater, crc32, stream.emit)))
                    }
                case GZipState.DeflateInput(deflater, crc32, emit) =>
                    Emit.runFirst(emit).map {
                        case Present(bytes) -> cont =>
                            IO {
                                crc32.update(bytes.toArray)
                    ...
                    local.use: buffer =>
                        IO(deflater.deflate(buffer, 0, buffer.length, flushMode.value))
                            .map:
                                ...
                                case size => IO(chunk.concat(Chunk.fromNoCopy[Byte](java.util.Arrays.copyOf(buffer, size))))
                ...
                case GZipState.EmitDeflated(deflater, crc32, maybeEmitFn, chunk) =>
                    Emit.andMap(chunk): ack =>
                        maybeEmitFn match
                            case Present(emitFn) => Loop.continue(GZipState.DeflateInput(deflater, crc32, emitFn(ack)))
                ...
                case GZipState.SendTrailer(deflater, crc32) =>
                    IO {
                        ...
                        Emit.andMap(trailer)(_ => Loop.done(Ack.Stop))
                    }
        end emit
    ...
    private def inflateStream[Ctx](
        ...
        enum InflateState derives CanEqual:
            ...
            case InflateInput(inflater: Inflater, emit: Ack < (Emit[Chunk[Byte]] & Ctx)) extends InflateState
            case PullInflater(inflater: Inflater, maybeEmitFn: Maybe[Ack => Ack < (Emit[Chunk[Byte]] & Ctx)], bytes: Chunk[Byte])
        ...
        def emit(
            ...
            Tag[Emit[Chunk[Byte]]],
            Frame
        ): Ack < (Resource & IO & Abort[DataFormatException | StreamCompressionException] & Ctx & Emit[Chunk[Byte]]) =
            Loop(InflateState.Initialize):
                ...
                case InflateState.InflateInput(inflater, emit) =>
                    Emit.runFirst(emit).map:
                        case Present(bytes) -> emitFn =>
                            IO(inflater.setInput(bytes.toArray))
                                .andThen(
                ...
                case InflateState.PullInflater(inflater, maybeEmitFn, bytes) =>
                    if inflater.finished then
                        ...
                        Emit.andMap(leftOver): ack =>
                            maybeEmitFn match
                                case Present(emitFn) =>
                                    Resource
                                        .acquireRelease(IO(new Inflater(noWrap)))(inflater => IO(inflater.end()))
                                        .map: inflater =>
                                            Loop.continue(InflateState.InflateInput(inflater, emitFn(ack)))
                                case Absent =>
                                    Loop.done(Ack.Stop)
                    else if inflater.needsInput then
                        Emit.andMap(Chunk.empty[Byte]): ack =>
                            maybeEmitFn match
                                case Present(emitFn) => Loop.continue(InflateState.InflateInput(inflater, emitFn(ack)))
                                case Absent          => Loop.done(Ack.Stop)
                    else
                        local.use: buffer =>
                            Abort
                                .catching[DataFormatException | StreamCompressionException](inflater.inflate(buffer))
                                .map(read => Chunk.fromNoCopy[Byte](Array.copyOf(buffer, read)))
                                .map(inflated =>
                                    Emit.andMap(inflated)(_ => Loop.continue(InflateState.PullInflater(inflater, maybeEmitFn, bytes)))
                                )
                    end if
        end emit
    ...
    private def gunzipStream[Ctx](
        ...
        enum GunzipState derives CanEqual:
            ...
            case ParseHeader(bytes: Chunk[Byte], headerCrc32: CRC32, emit: Ack < (Emit[Chunk[Byte]] & Ctx)) extends GunzipState
            case ParseHeaderExtra(
                bytes: Chunk[Byte],
                headerCrc32: CRC32,
                checkCrc16: Boolean,
                commentsToSkip: Int,
                emit: Ack < (Emit[Chunk[Byte]] & Ctx)
            ) extends GunzipState
            case SkipComments(
                bytes: Chunk[Byte],
                headerCrc32: CRC32,
            ...
            case InflateInput(leftOver: Chunk[Byte], inflater: Inflater, contentCrc32: CRC32, emit: Ack < (Emit[Chunk[Byte]] & Ctx))
                extends GunzipState
            case PullInflater(
                inflater: Inflater,
                contentCrc32: CRC32,
        ...
        def nextState(
            ...
            using Frame
        ): (Chunk[Byte], CRC32, Ack < (Emit[Chunk[Byte]] & Ctx)) => Loop.Outcome[GunzipState, Ack] < (Resource & IO) =
            (bytes: Chunk[Byte], headerCrc32: CRC32, emit: Ack < (Emit[Chunk[Byte]] & Ctx)) =>
                if hasExtra then
            ...
            Loop(GunzipState.Initialize):
                ...
                case GunzipState.ParseHeader(accBytes, headerCrc32, emit) =>
                    if accBytes.length < fixedHeaderLength then
                        Emit.runFirst(emit).map:
                            case Present(bytes) -> emitFn =>
                                Loop.continue(GunzipState.ParseHeader(accBytes.concat(bytes), headerCrc32, emitFn(Ack.Continue())))
                            case Absent -> _ =>
                ...
                case GunzipState.ParseHeaderExtra(accBytes, headerCrc32, checkCrc16, commentsToSkip, emit) =>
                    if accBytes.length < 12 then
                        ...
                        if accBytes.length < headerWithExtraLength then
                            ...
                            headerCrc32.update(headerWithExtra.drop(fixedHeaderLength).toArray)
                    ...
                    if zeroIdx == -1 then
                        Emit.runFirst(emit).map:
                            case Present(nextBytes) -> emitFn =>
                                headerCrc32.update(bytes.toArray)
                        ...
                        headerCrc32.update(upToZero.toArray)
                ...
                case GunzipState.CheckCrc16(accBytes, headerCrc32, emit) =>
                    if accBytes.length < 2 then
                        Emit.runFirst(emit).map:
                            case Present(bytes) -> emitFn =>
                                Loop.continue(GunzipState.CheckCrc16(accBytes.concat(bytes), headerCrc32, emitFn(Ack.Continue())))
                            case Absent -> _ =>
                                Abort
                                    .fail(new StreamCompressionException("Invalid GZip header"))
                                    .andThen(Loop.done(Ack.Stop))
                ...
                case GunzipState.InflateInput(leftOver, inflater, contentCrc32, emit) =>
                    if leftOver.isEmpty then
                        ...
                        IO(inflater.setInput(leftOver.toArray))
                            .andThen(
                                Loop.continue(GunzipState.PullInflater(inflater, contentCrc32, Present(_ => emit), leftOver))
                            )
                    end if
                case GunzipState.PullInflater(inflater, contentCrc32, maybeEmitFn, bytes) =>
                    if inflater.finished then
                        ...
                        Emit.andMap(Chunk.empty): ack =>
                            maybeEmitFn match
                                case Present(emitFn) =>
                                    Loop.continue(GunzipState.InflateInput(Chunk.empty, inflater, contentCrc32, emitFn(ack)))
                                case Absent =>
                                    Loop.continue(GunzipState.CheckTrailer(Chunk.empty, inflater, contentCrc32, Absent))
                    else
                        local.use: buffer =>
                            Abort
                                .catching[DataFormatException](inflater.inflate(buffer))
                                .map(read => Chunk.fromNoCopy[Byte](Array.copyOf(buffer, read)))
                                .map(inflated =>
                                    IO(contentCrc32.update(inflated.toArray)).andThen(
                                        Emit.andMap(inflated)(_ =>
                ...
                case GunzipState.CheckTrailer(accBytes, inflater, contentCrc32, maybeEmitFn) =>
                    if accBytes.length < 8 then
                        maybeEmitFn match
                            case Present(emitFn) =>
                                Emit.runFirst(emitFn(Ack.Continue())).map:
                        ...
                        val crc32         = readInt(trailerBytes.take(4).toArray)
                        val isize         = readInt(trailerBytes.drop(4).toArray)
                        val expectedCrc32 = contentCrc32.getValue()
                        val expectedIsize = inflater.getBytesWritten
                        if expectedCrc32.toInt != crc32 then
                            Abort
                                .fail(new StreamCompressionException("Invalid CRC32"))
                                .andThen(Loop.done(Ack.Stop))
                        else if expectedIsize.toInt != isize then
                            Abort
                                .fail(new StreamCompressionException("Invalid ISIZE"))
                                .andThen(Loop.done(Ack.Stop))
                        else
                            maybeEmitFn match
                                case Present(emitFn) =>
                                    Resource.acquireRelease(IO(new CRC32()))(headerCrc32 => IO(headerCrc32.reset()))
                                        .map: headerCrc32 =>
                                            Loop.continue(GunzipState.ParseHeader(rest, headerCrc32, emitFn(Ack.Continue())))
                                case Absent =>
                                    Emit.andMap(rest)(_ => Loop.done(Ack.Stop))
                        end if
                    end if
        end emit
