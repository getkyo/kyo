...
object StreamCompression:
    ...
    private def inflateStream[Ctx](
        ...
        def emit(
            ...
            Tag[Emit[Chunk[Byte]]],
            Frame
        ): Ack < (Resource & IO & Abort[StreamCompressionException] & Ctx & Emit[Chunk[Byte]]) =
            Loop(InflateState.Initialize):
                ...
                case InflateState.PullInflater(inflater, maybeEmitFn, bytes) =>
                    if inflater.finished then
                        ...
                        local.use: buffer =>
                            Abort
                                .catching[DataFormatException](inflater.inflate(buffer))(dfe => StreamCompressionException(dfe))
