...
//           */
//         def spawn(clazz: Class[?], args: List[String] = Nil)(using Trace): Process < Fibers =
...
//           */
//         def spawn(using Trace): Process < Fibers

//         /** Spawns a new process and returns its output as a String
//           */
//         def text(using Trace): String < Fibers =
...
//           */
//         def stream(using Trace): InputStream < Fibers =
//             spawn.map(_.stdout)

//         def exitValue(using Trace): Int < Fibers =
//             spawn.map(_.exitValue)

//         def waitFor(using Trace): Int < Fibers =
//             spawn.map(_.waitFor)

//         def waitFor(timeout: Long, unit: TimeUnit)(using Trace): Boolean < Fibers =
...
//         ) extends Command:
//             self =>
//             override def spawn(using Trace): Process < Fibers =
//                 for
//                     process <- IOs {
...
//             self =>
//             def spawnAll(using Trace): List[Process] < Fibers =
