...
class DoltLiteClientTest extends Test:
    ...
    private def withDolt[A](f: DoltClient => A < (Async & Abort[SqlException] & Scope & DB))(using
        Frame
    ...
    "a diff names the rows that changed" in {
        withDolt { dolt =>
            ...
            yield assert(changed.exists(_.kind == DoltDiff.Kind.Added), s"the inserted row must show as added, got $changed")
            end for
        }
    }
