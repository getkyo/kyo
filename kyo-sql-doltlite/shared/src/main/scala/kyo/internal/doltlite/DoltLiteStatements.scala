...
private[kyo] object DoltLiteStatements:
    ...
    def hashOf(client: SqlClient, ref: String)(using Frame): Dolt.CommitHash < (Async & Abort[SqlException]) =
        call(client, "dolt_hashof", Chunk(ref)).map(Dolt.CommitHash.apply)
    ...
    private def readCommit(client: SqlClient, row: SqlRow)(using Frame): Dolt.Commit < (Async & Abort[SqlException]) =
        lifted {
            for
    ...
    /** Parents come from a side table rather than a log column. They must be read: an empty `parents` is what
      * [[Dolt.Commit.isRoot]] and `isMerge` are defined in terms of.
      */
    private def parentsOf(client: SqlClient, hash: String)(using
        Frame
    ): Chunk[Dolt.CommitHash] < (Async & Abort[SqlException]) =
        query(
            client,
            ...
            decoded(rows)(_.decode[String](0)).map(_.map(Dolt.CommitHash.apply))
        }
    ...
    def add(client: SqlClient, tables: Chunk[String])(using Frame): Unit < (Async & Abort[SqlException]) =
        // Naming no table means every changed one, which dolt_add spells `-A`, the same as the server backend.
    ...
    def createBranch(client: SqlClient, name: String, from: Dolt.Ref)(using
        Frame
    ...
    def revert(client: SqlClient, ref: Dolt.Ref)(using Frame): Dolt.Commit < (Async & Abort[SqlException]) =
        callUnit(client, "dolt_revert", Chunk(ref.render)).andThen {
            hashOf(client, Dolt.Ref.Head.render).map(hash => commitAt(client, hash))
        }
    ...
    private def readTag(row: SqlRow)(using Frame): Dolt.Tag < Abort[SqlDecodeException] =
        ...
        yield Dolt.Tag(name, Dolt.CommitHash(hash), tagger, email.getOrElse(""), date, message.filter(_.nonEmpty))
    ...
    private def readRemote(row: SqlRow)(using Frame): Dolt.Remote < Abort[SqlDecodeException] =
        ...
        yield Dolt.Remote(name, url, specs.fold(Chunk.empty[String])(jsonStringArray), params.filter(p => p.nonEmpty && p != "{}"))
