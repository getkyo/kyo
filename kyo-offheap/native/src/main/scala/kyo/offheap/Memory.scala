// kyo-offheap/native/src/main/scala/kyo/Memory.scala
package kyo.offheap
...
object Memory:
    ...
    def init[A: Layout](size: Int)(using Frame): Memory[A] < Arena =
    ...
    extension [A: Layout](self: Memory[A])
        ...
        def view(from: Int, len: Int)(using Frame): Memory[A] < Arena =
            IO.Unsafe {
                require(from >= 0 && len >= 0 && from + len <= size,
                    s"Invalid view range [$from, ${from+len}) of $size elements")
        ...
        def copyTo(target: Memory[A], srcPos: Int, destPos: Int, len: Int)(using Frame): Unit < Arena =
            IO.Unsafe {
                require(srcPos >= 0 && destPos >= 0 && len >= 0 &&
