...
| 34 | `Bench.actionableJit` has a 6-line docstring documenting a vocabulary "verified against a capture", and no caller and no test | **fixed** (`1c34a79c15`): wired to gate the stopped-inlining falsifier, which is its exact purpose (a force-inline flag cannot take on a megamorphic or klass-not-linked refusal, so proposing it wastes a session); a megamorphic regression now earns no futile experiment, tested |
...
Fifty-two entries: **45 fixed**, 3 open (9, 30, 37-pending-0e), 1 won't-fix (39), 1 bounded (23), 1 an observation (7),
1 superseded (5).
