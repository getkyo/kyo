...
| 31 | no decodable jit data exists: the two jit-bearing runs fail to load on a missing `osrTasks`, and their entries are the old `JitEntry` shape | **fixed** (decode, `c4275e761e`): `osrTasks`/`runtimeDeopts`/`plantedTraps`/`madeNotEntrant` defaulted so the runs decode; the old flat `JitEntry` shape (`inlined: Boolean`) is a representation change no default bridges, isolated in `StoreSchemaTest` and flagged as an open decision (leave lossy, or a custom decoder) |
| 32 | `StoreSchemaTest` covers an *added* field but not a **changed field shape**, which is how 31 got in | **fixed** (`c4275e761e`): `StoreSchemaTest` decodes the two real stored runs and isolates the changed `jit` field shape it cannot bridge, so the shape change is exercised, not silently missed |
| 33 | `Ingest.run` hardcodes `evidence = Timing` and `jit = Chunk.empty`, so the ingest path cannot build a jit-bearing fixture at all | **resolved by design** (`01cbecb712`): a json carries no inlining, so a json-only ingest is correctly Timing with empty jit; `Ingest.attachJit`/`BenchJit` fill jit/jit_metrics/deopts/morphism from a compilation log afterward, so a jit-bearing ingested run is now buildable |
| 34 | `Bench.actionableJit` has a 6-line docstring documenting a vocabulary "verified against a capture", and no caller and no test | **open**, a Gate-A wire-or-delete decision for the owner: `actionableJit` (refusals worth chasing, excluding megamorphic and klass-not-linked) overlaps the now-wired near-budget (item 3), partial-inlining (item 1) and jitShift sections; whether it earns a fourth jit section or is deleted is a judgment call, flagged rather than guessed |
| 35 | `diffVerdicts` and `Bench.jitShift` are two implementations of one question, over different types, in different modules; the tested one is unwired | **fixed** (`6d96a0ed23`): `diffVerdicts` re-typed to `Chunk[InlineSites]` (what `Run` carries) and wired into the report to name partial inlining moves; jitShift names the decisive flips, so the two now cover different cases instead of duplicating one |
| 36 | `--drift-row` is inert: `openSession` never references the parameter | **fixed** (`2e140e2d79`, 0d): `openSession` lost the dead `driftRow` parameter and the CLI its `--drift-row` option and the help text that promised a measurement |
| 37 | `driftPercent` is hardcoded `0.0`, so "drift measured this session" is unreachable and every report prints "4.0% assumed" | **open**, this is ruling 0e: `driftPercent` is now intentionally 0.0 because a bracket estimates its spread from its own replicate legs, not from a session-open measurement (defect 36/0d). The single-pair `band` still falls back to the 4.0 `DriftBand` and the header still says "assumed, not measured", which is honest for a single pair. Recorded default: leave it; whether the field and the single-pair band go is the owner's call |
| 38 | `QaEndToEnd` asserts `driftPercent > 0.0` and therefore fails by construction | **fixed** (`2e140e2d79`, 0f): the check now asserts `driftPercent == 0.0` (a session claims no drift; the spread is the bracket's own replicate legs), and `QaEndToEnd.check` throws so it can actually fail |
| 39 | `Delta.flatButUnbounded` can never be true: both producers always attach a `Resolution` to a `Flat` verdict | **won't fix** (confirmed dead value, retained as a defence): both producers do attach a `Resolution` to `Flat` (compare uses a df-0 one), so it is always false today; its one use is a defensive `||` fallback in the unresolved-rows count and it is guarded by the invariant test "no flat row is unbounded", which would catch a producer that forgot the resolution. Removing it removes that guard for no gain; the mislabelled warning it was blamed for was defect 50, fixed |
| 40 | `Stats.commonMode` is computed on every replicated comparison, bound to `common`, and never read | **fixed** (`ce348805cf`, item 10): `commonMode` is carried on `Comparison`, each `Delta` carries its residual from it, and the report states the session's drift and names the rows that depart from it |
| 41 | the `-f N is diagnostic` note is keyed on fork count, so it fires on every replicated bracket that has a real threshold | **fixed**: `forksNote` is keyed on `earnedThreshold` (`c.deltas.exists(_.resolution.exists(_.df > 0))`), so it fires only when no real threshold was estimated, not on fork count; a replicated bracket with a threshold is silent |
| 42 | `Run.alloc` and `Run.cpu` cost one extra JMH invocation each and surface as one integer and one percentage printed only above 25% | **addressed**: cpu surfaces as the three-way `cpuPartition` run-level note (always, item 4) and per-row `cpuByRow`; alloc as `allocByMethod` sites, the conservation blocker (item 9), and the flat/collapsed delta, not one integer and a 25%-gated percentage |
| 43 | the unexplained-row line tells the operator to "check the inlining log" on a **Timing** run, whose `jit` is empty by construction: it names evidence the run provably does not contain | **fixed** (`59c9c2f31f`): on a Timing pair the advice names the absent evidence and the command to collect it (`--evidence full`); on a Full pair it points at the inlining and allocation evidence already in the report |
| 44 | `mode` and `unit` are captured and never checked, in **both** classifiers (`Bench.scala:615` and `Stats.classify` `Stats.scala:171`), so a negative delta is `Faster` unconditionally: a **`thrpt` row is classified backwards, replicated comparisons included**, and legs in different units are off by the unit ratio. `mode` is display-only; `Row.unit` has zero readers | **fixed** (2026-08-18): `Row.lowerIsBetter` (`thrpt` is the one mode where up is better), `Stats.Replicated.lowerIsBetter` read off the control row and used by `classify`, `Bench.compare` the same; `Row.comparableWith` (same mode and unit) gates both classifiers to `BelowResolution` and `Report.blockers` names the mismatch with both sides, since more legs cannot cure it; the table prints the unit beside the mode. Tests: `StatsTest` (the real A/A series `6.10, 6.12, 6.14` vs `6.40, 6.42` read as thrpt is a win, as avgt a regression, a flat row flat either way), `BenchTest` (single-pair and replicated thrpt, the mismatch and its blocker) |
| 45 | `Run.coverage` (`Model.scala:307`) has **no schema default**, so defect 17's *original* class is live in the tree right now | **fixed** (`c4275e761e`, 0b): `coverage` and the other additive evidence fields defaulted, so a stored run recorded before them decodes |
| 46 | `StoreSchemaTest` **does not exist**; the check is inline at `BenchTest.scala:498-511` covering `allocByMethod` only, and a stale comment at `Model.scala:323` names the phantom | **fixed** (`c4275e761e`, 0b): `StoreSchemaTest` exists and decodes the two real stored runs against the additive-field defaults |
| 47 | `Ingest` fabricates `forks = 1` while the json carries the truth (**three readers**, including `stillCompiling`'s measured-window denominator) and **discards `jvmArgs` entirely**, so an ingested configuration pair calls the difference unrecoverable while holding the file that states it | **fixed**: `Ingest` reads `forks` from the json's fork count and `jvmArgs` via `jvmArgsOf`, so an ingested configuration pair carries what distinguishes it; `stillCompiling` also no longer depends on the fabricated iteration count (item 11, `169f16a93e`) |
| 48 | multi-row allocation break: `parseAlloc` reads one flat table per benchmark so `Run.alloc` carries duplicate `cls` rows, and `apportion` takes a last-wins numerator against a denominator summed across all benchmarks | **fixed** (`74d5c1b05b`): `parseAlloc` folds a class repeated across benchmark sections into one leg-level row (bytes and samples summed, encounter order kept), so `apportion`'s numerator and the conservation check both see the whole leg's figure for the class |
| 49 | the JIT table mixes scopes: `msInWindow`/`msTotal` summed across all rows from `-prof comp`, task census from the single surviving fork's log, printed as one table headed "a property of the design" | **fixed** (rendering, `c2c3451d21`): on a multi-row leg the table states that compile time is summed across the rows while the task census is one row's fork only, so the two halves no longer read as one census; the collection half is defect 30 (measurement path, 0a) |
| 50 | "N flat rows carry no resolution" is **reachable** via `bracket --legs 2` and **mislabelled**: it fires on rows that are not flat. I first recorded it as unreachable dead code, which would have deleted a real defect | **fixed** (2026-08-18): the text says what it counts, "N rows could not be resolved: one leg per arm gives no spread to estimate a threshold from", and names the cure; pinned by a one-leg-per-arm replicated comparison in `BenchTest` |
| 51 | `QaEndToEnd.check` only **prints** (`:17-18`), unlike `BenchTest.check` which throws (`:41-43`), so a QA main's checks cannot fail the run: "fails by construction" means "prints FAIL and exits 0" | **fixed** (2026-08-18): throws like `BenchTest.check`; the stale "drift measured, not assumed" check it would then have failed on is replaced by the true statement (a session claims no drift; the spread is the bracket's own legs); `openSession` lost its dead `driftRow` parameter and the CLI its option and the help text that promised a measurement (0d) |
| 52 | item 4 as shipped: the note now fires on **every** Full leg (the `n < 25.0` gate became unreachable in the other direction), and its two-way partition classifies `ProtoKernelBench$$anon$95` as immovable although that allocation is candidate C3's entire subject | **fixed** (`b9446afa89`): the two-way always-firing note is retired; the run-level note is the three-way `cpuPartition` (kernel/benchmark/other) stated as data, with the benchmark's own closures named as their own share, not lumped immovable |

Fifty-two entries: **44 fixed**, 4 open (9, 30, 34, 37-pending-0e), 1 won't-fix (39), 1 bounded (23), 1 an observation (7),
1 superseded (5).
