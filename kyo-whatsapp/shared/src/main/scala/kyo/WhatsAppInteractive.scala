...
/** Closed ADT of the six interactive message sub-shapes supported by the WhatsApp Cloud API.
  * A `WhatsAppInteractive` value is embedded in a `WhatsAppMessage.OfInteractive` and sent via
...
sealed trait WhatsAppInteractive derives CanEqual:
    ...
    def header: Maybe[WhatsAppInteractive.Header]
...
object WhatsAppInteractive:
    ...
    object Header:
        ...
        final case class Media(source: WhatsAppMedia.Source, kind: MediaKind)                     extends Header derives CanEqual
        final case class Document(source: WhatsAppMedia.Source, filename: Maybe[String] = Absent) extends Header derives CanEqual
    end Header
