...
/** A template message value for `WhatsApp.sendTemplate`. Templates are the only message
  ...
  *
  * A `Template` carries the template name, a BCP-47 language code, and an optional sequence
  * of `Component` values that fill the template's variable slots at send time.
  *
  * Component types:
  ...
  * `CopyCode`, and `Flow`.
  */
final case class Template(name: String, language: String, components: Chunk[Template.Component] = Chunk.empty)
    derives CanEqual
...
object Template:
    ...
    object Parameter:
        ...
        final case class Image(source: Media.Source)                                      extends Parameter derives CanEqual
        final case class Document(source: Media.Source, filename: Maybe[String] = Absent) extends Parameter derives CanEqual
        final case class Video(source: Media.Source)                                      extends Parameter derives CanEqual
