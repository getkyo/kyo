...
/** Inbound webhook namespace for the WhatsApp Cloud API notification surface. Covers the
  ...
  * `Notification` values, and a fused POST handler for the common case.
  *
  * Operations:
  ...
  *   - `decode` parses the verified raw body into a `Chunk[Notification]`, aborting with
  *     `WhatsAppError.DecodeError` only on a structurally unparseable envelope. Unknown
...
object Webhook:
    ...
    /** X-Hub-Signature-256 verification. Total and pure: Success(()) on a matching
      ...
      */
    def verifySignature(appSecret: String, signatureHeader: Maybe[String], body: Span[Byte]): Result[WhatsAppError.SignatureError, Unit] =
    ...
    /** Typed decode of the raw verified body into one Notification per entry[].changes[].
      * An unknown type/status/field decodes degenerate, never an Abort; a
      * structurally-unparseable envelope aborts with DecodeError.
      */
    def decode(body: Span[Byte])(using Frame): Chunk[Notification] < Abort[WhatsAppError.DecodeError] =
        Abort.get(Codec.decodeNotifications(body))

    /** Fused POST handler: reads bodyBinary (byte-exact for HMAC), verifies the signature
      * (403 on failure), decodes, invokes `f` per Notification, returns 200. An unparseable
      * change is logged-and-skipped so a structurally-broken-but-delivered POST is acked,
      * not retried by Meta.
      */
    def handler(appSecret: String)(using
        Frame
    )(f: Notification => Unit < (Async & Abort[WhatsAppError])): HttpHandler["body" ~ Span[Byte], Any, WhatsAppError] =
