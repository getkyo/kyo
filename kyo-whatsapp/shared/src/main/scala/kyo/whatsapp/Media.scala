...
import kyo.*
import kyo.whatsapp.internal.Codec as MediaCodec
...
/** Media-API namespace for the WhatsApp Cloud API media upload and download surface.
  ...
  *     acknowledgment
  *
  * All operations require a bound `WhatsApp.Config` (via `WhatsApp.let`) and abort
...
object Media:
    ...
    object Source:
        final case class ById(id: Id.MediaId) extends Source derives CanEqual
    ...
    final case class MediaInfo(id: Id.MediaId, url: String, mimeType: String, sha256: String, fileSize: Long)
        derives CanEqual

    private def mediaUrl(c: WhatsApp.Config, id: Id.MediaId): String =
        s"${c.baseUrl}/${c.apiVersion}/${id.value}"

    private def uploadUrl(c: WhatsApp.Config): String =
        s"${c.baseUrl}/${c.apiVersion}/${c.phoneNumberId.value}/media"

    private def bearer(c: WhatsApp.Config): Seq[(String, String)] =
    ...
    /** Uploads a media asset via a three-part multipart POST and returns the assigned media
      ...
      * `Id.MediaId` from the `{id}` response field. Aborts with a typed `WhatsAppError` leaf
      * on Graph API errors, transport failures, or a response that does not carry an `id`
      * field.
      */
    def upload(bytes: Span[Byte], mediaType: MediaType, filename: Maybe[String] = Absent)(
        using Frame
    ): Id.MediaId < (Async & Abort[WhatsAppError]) =
        WhatsApp.use { c =>
            ...
            val parsedUrl: Result[WhatsAppError, HttpUrl] = HttpUrl.parse(uploadUrl(c)) match
                ...
                case Result.Failure(e) => Result.fail(MediaCodec.mapError(e))
            ...
            Abort.get(parsedUrl).map { uploadHttpUrl =>
                ...
                Abort.runWith[HttpException](
                    ...
                    case Result.Success(respBytes) => Abort.get(MediaCodec.decodeMediaId(respBytes))
                    case Result.Failure(e)         => Abort.fail(MediaCodec.mapError(e))
                    case Result.Panic(ex)          => Abort.get(MediaCodec.mapTransportPanic(ex))
                }
            }
        }
    ...
    /** Retrieves the temporary download URL and metadata for a media id. Issues a GET to
      ...
      */
    def resolveUrl(id: Id.MediaId)(using Frame): MediaInfo < (Async & Abort[WhatsAppError]) =
        WhatsApp.use { c =>
            Abort.runWith[HttpException](HttpClient.getBinary(mediaUrl(c, id), headers = bearer(c))) {
                case Result.Success(respBytes) => Abort.get(MediaCodec.decodeMediaInfo(respBytes))
                case Result.Failure(e)         => Abort.fail(MediaCodec.mapError(e))
                case Result.Panic(ex)          => Abort.get(MediaCodec.mapTransportPanic(ex))
            }
        }
    ...
    /** Resolves the temporary url then downloads the authenticated bytes in a single call,
      ...
      */
    def download(id: Id.MediaId)(using Frame): Span[Byte] < (Async & Abort[WhatsAppError]) =
    ...
    /** Deletes a previously uploaded media asset. Issues a DELETE to the media endpoint at
      ...
      * decode failures.
      */
    def delete(id: Id.MediaId)(using Frame): Unit < (Async & Abort[WhatsAppError]) =
        WhatsApp.use { c =>
            Abort.runWith[HttpException](HttpClient.deleteBinary(mediaUrl(c, id), headers = bearer(c))) {
                case Result.Success(respBytes) => Abort.get(MediaCodec.decodeSuccess(respBytes))
                case Result.Failure(e)         => Abort.fail(MediaCodec.mapError(e))
                case Result.Panic(ex)          => Abort.get(MediaCodec.mapTransportPanic(ex))
            }
        }
