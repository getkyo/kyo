...
class WhatsAppMediaTest extends BaseWhatsAppTest:
    ...
    "Source.ById carries a MediaId, ByLink carries a String" in {
        val byId: WhatsAppMedia.Source   = WhatsAppMedia.Source.ById(WhatsAppId.MediaId("M1"))
        val byLink: WhatsAppMedia.Source = WhatsAppMedia.Source.ByLink("https://x/y.jpg")
        byId match
            case WhatsAppMedia.Source.ById(id) => assert(id.value == "M1")
            case _                             => assert(false)
        byLink match
            case WhatsAppMedia.Source.ByLink(link) => assert(link == "https://x/y.jpg")
            case _                                 => assert(false)
    }

    "id-XOR-link is type-level exclusive (cannot set both)" in {
        typeCheckFailure("""WhatsAppMedia.Source.ById(WhatsAppId.MediaId("m"), link = "l")""")
    }

    "every MediaType case exposes its documented MIME" in {
        assert(WhatsAppMedia.MediaType.ImageJpeg.mime == "image/jpeg")
        assert(WhatsAppMedia.MediaType.ImagePng.mime == "image/png")
        assert(WhatsAppMedia.MediaType.AudioAac.mime == "audio/aac")
        assert(WhatsAppMedia.MediaType.AudioAmr.mime == "audio/amr")
        assert(WhatsAppMedia.MediaType.AudioMp3.mime == "audio/mpeg")
        assert(WhatsAppMedia.MediaType.AudioMp4.mime == "audio/mp4")
        assert(WhatsAppMedia.MediaType.AudioOgg.mime == "audio/ogg")
        assert(WhatsAppMedia.MediaType.VideoMp4.mime == "video/mp4")
        assert(WhatsAppMedia.MediaType.Video3gp.mime == "video/3gp")
        assert(WhatsAppMedia.MediaType.DocumentPdf.mime == "application/pdf")
        assert(WhatsAppMedia.MediaType.DocumentText.mime == "text/plain")
        assert(WhatsAppMedia.MediaType.StickerWebp.mime == "image/webp")
        assert(WhatsAppMedia.MediaType.Other("application/zip").mime == "application/zip")
    }

    "MediaInfo carries all five retrieve-url fields" in {
        val info = WhatsAppMedia.MediaInfo(WhatsAppId.MediaId("M"), "https://lookaside/...", "image/jpeg", "abc123", 12345L)
    ...
    "MediaInfo decodes from the documented response with string file_size" in {
        ...
        kyo.internal.whatsapp.Codec.decodeMediaInfo(bytes) match
            case Result.Success(info) =>
                assert(info.id == WhatsAppId.MediaId("<MEDIA_ID>"))
    ...
    "upload POSTs to the versioned /media endpoint with bearer".notNative in {
        ...
        withUploadServer(mediaIdResponse) { (port, _, reqCapture) =>
            val cfg = makeConfig(port)
            WhatsApp.let(cfg) {
                WhatsAppMedia.upload(Span.from(bytes), WhatsAppMedia.MediaType.ImageJpeg, Present("f.jpg")).map { _ =>
    ...
    "upload uses a default filename when none is given".notNative in {
        ...
        withUploadServer(mediaIdResponse) { (port, partsCapture, _) =>
            val cfg = makeConfig(port)
            WhatsApp.let(cfg) {
                WhatsAppMedia.upload(Span.from(bytes), WhatsAppMedia.MediaType.ImagePng).map { _ =>
                    partsCapture.take.map { parts =>
                        val file = parts.find(_.name == "file").get
    ...
    "upload maps a 131053 error to MediaError.UploadFailed".notNative in {
        ...
        withUploadServer(graphErrorBody(131053), statusOk = false) { (port, _, _) =>
            ...
            Abort.run[WhatsAppError](
                WhatsApp.let(cfg) {
                    WhatsAppMedia.upload(Span.from(bytes), WhatsAppMedia.MediaType.ImageJpeg, Present("f.jpg"))
                }
            ).map { result =>
                result match
    ...
    "resolveUrl GETs the media endpoint and decodes MediaInfo".notNative in {
        ...
        withGetServer(s"v25.0/${reqMediaId.value}", mediaInfoBody("\"204800\"")) { (port, reqCapture) =>
            val cfg = makeConfig(port)
            WhatsApp.let(cfg) {
                WhatsAppMedia.resolveUrl(reqMediaId).map { info =>
                    reqCapture.take.map { case (path, auth) =>
            ...
            HttpServer.init(0, "localhost")(bytesHandler).map { s =>
                ...
                HttpServer.init(0, "localhost")(metaHandler).map { metaS =>
                    HttpClient.let(httpClient) {
                        ...
                        WhatsApp.let(cfg) {
                            WhatsAppMedia.download(reqMediaId).map { result =>
            ...
            HttpClient.init().map { httpClient =>
                HttpServer.init(0, "localhost")(bytesHandler).map { s =>
                    HttpClient.let(httpClient) {
                        ...
                        val info = WhatsAppMedia.MediaInfo(mediaId, s"http://localhost:${s.port}/bytes", "image/jpeg", "h", 4L)
                        WhatsApp.let(cfg) {
                            WhatsAppMedia.downloadFrom(info).map { result =>
    ...
    "downloadFrom maps a 131052 error to MediaError.DownloadFailed".notNative in {
        ...
        HttpClient.init().map { httpClient =>
            HttpServer.init(0, "localhost")(errHandler).map { s =>
                HttpClient.let(httpClient) {
                    val cfg  = makeConfig(s.port)
                    val info = WhatsAppMedia.MediaInfo(mediaId, s"http://localhost:${s.port}/bad", "image/jpeg", "h", 4L)
                    Abort.run[WhatsAppError](
                        WhatsApp.let(cfg) {
                            WhatsAppMedia.downloadFrom(info)
                        }
                    ).map { result =>
                        result match
    ...
    "download returns bytes byte-identical to what the server serves".notNative in {
        val reqMediaId = WhatsAppId.MediaId("M")
    ...
    "delete DELETEs the media endpoint and consumes success body as Unit".notNative in {
        ...
        withDeleteServer(s"v25.0/${reqMediaId.value}", """{"success":true}""") { (port, reqCapture) =>
            val cfg = makeConfig(port)
            WhatsApp.let(cfg) {
                WhatsAppMedia.delete(reqMediaId).map { result =>
                    reqCapture.take.map { case (path, auth) =>
                        assert(path.endsWith(s"/v25.0/${reqMediaId.value}"))
                        assert(auth == s"Bearer $token")
                        assert(result == ())
    ...
    "upload effect row is exactly Async & Abort[WhatsAppError]" in {
        typeCheck("""
            ...
            val _: WhatsAppId.MediaId < (Async & Abort[WhatsAppError]) =
                WhatsAppMedia.upload(bytes, WhatsAppMedia.MediaType.ImageJpeg)
        """)
    }
        ...
        HttpClient.init().map { httpClient =>
            Scope.run(
                ...
                HttpClient.let(httpClient) {
                    ...
                    val info = WhatsAppMedia.MediaInfo(mediaId, s"http://localhost:$deadPort/bytes", "image/jpeg", "h", 4L)
                    Abort.run[WhatsAppError](
                        WhatsApp.let(cfg) {
                            WhatsAppMedia.downloadFrom(info)
                        }
    ...
    "upload of Other media type uses its custom mime and bin default name".notNative in {
        ...
        withUploadServer(mediaIdResponse) { (port, partsCapture, _) =>
            val cfg = makeConfig(port)
            WhatsApp.let(cfg) {
                WhatsAppMedia.upload(Span.from(bytes), WhatsAppMedia.MediaType.Other("application/zip")).map { _ =>
                    partsCapture.take.map { parts =>
                        val tp   = parts.find(_.name == "type").get
                        val file = parts.find(_.name == "file").get
    ...
    "resolveUrl result id is taken from the response body not the request id".notNative in {
        val reqId  = WhatsAppId.MediaId("REQ_ID")
        val respId = mediaId
        val respBody =
        ...
        withGetServer(s"v25.0/${reqId.value}", respBody) { (port, _) =>
            val cfg = makeConfig(port)
            WhatsApp.let(cfg) {
                WhatsAppMedia.resolveUrl(reqId).map { info =>
    ...
    "delete with a broken success body surfaces DecodeError".notNative in {
        val reqMediaId = WhatsAppId.MediaId("M4")
