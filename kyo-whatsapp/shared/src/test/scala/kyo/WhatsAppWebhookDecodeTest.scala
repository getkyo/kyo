...
class WhatsAppWebhookDecodeTest extends BaseWhatsAppTest:
    ...
    "multiple entry[] and changes[] in one POST all decode" in {
        Abort.run[WhatsAppError.DecodeError](
            WhatsAppWebhook.decode(Span.from(multiEntryJson.getBytes("UTF-8")))
        ).map { result =>
            result match
                case Result.Success(notifications) =>
                    assert(notifications.size == 4)
                    assert(notifications(0).isInstanceOf[WhatsAppNotification.InboundMessage])
                    assert(notifications(1).isInstanceOf[WhatsAppNotification.StatusUpdate])
                    assert(notifications(2).isInstanceOf[WhatsAppNotification.InboundMessage])
                    assert(notifications(3).isInstanceOf[WhatsAppNotification.StatusUpdate])
                    val msg0 = notifications(0).asInstanceOf[WhatsAppNotification.InboundMessage]
                    assert(msg0.content == WhatsAppNotification.Content.Text("Hello"))
                    val su1 = notifications(1).asInstanceOf[WhatsAppNotification.StatusUpdate]
                    assert(su1.status == WhatsAppNotification.Status.Delivered)
                case other =>
    ...
    "an unknown change field decodes to Unsupported, NOT an Abort" in {
        Abort.run[WhatsAppError.DecodeError](
            WhatsAppWebhook.decode(Span.from(unknownChangeJson.getBytes("UTF-8")))
        ).map { result =>
            result match
                case Result.Success(chunk) =>
                    assert(chunk.size == 1)
                    chunk.head match
                        case WhatsAppNotification.Unsupported(_, "account_update", raw) =>
                            assert(raw.nonEmpty)
    ...
    "a structurally-unparseable envelope aborts with DecodeError" in {
        Abort.run[WhatsAppError.DecodeError](
            WhatsAppWebhook.decode(Span.from(truncatedJson.getBytes("UTF-8")))
        ).map { result =>
            result match
    ...
    "a valid-but-empty entry array decodes to an empty Chunk" in {
        Abort.run[WhatsAppError.DecodeError](
            WhatsAppWebhook.decode(Span.from(emptyEntryJson.getBytes("UTF-8")))
        ).map { result =>
            result match
    ...
    "decode is Abort[DecodeError] only, never Async" in {
        typeCheck("""
            ...
            val _: Chunk[WhatsAppNotification] < Abort[WhatsAppError.DecodeError] = WhatsAppWebhook.decode(body)
        """)
    }
