...
class WhatsAppWebhookSignatureTest extends BaseWhatsAppTest:
    ...
    "a known-good sha256= header verifies" in {
        val result = WhatsAppWebhook.verifySignature(appSecret, Present(knownGoodHeader), Span.from(bodyBytes))
        assert(result == Result.unit)
    }

    "a missing header yields Failure(Missing)" in {
        val result = WhatsAppWebhook.verifySignature(appSecret, Absent, Span.from(bodyBytes))
    ...
    "a header without the sha256= prefix yields Failure(Malformed)" in {
        val result = WhatsAppWebhook.verifySignature(appSecret, Present("abc123"), Span.from(bodyBytes))
        assert(result == Result.fail(WhatsAppError.SignatureError.Malformed()))
    }

    "a non-hex remainder yields Failure(Malformed)" in {
        val result = WhatsAppWebhook.verifySignature(appSecret, Present("sha256=zzzz"), Span.from(bodyBytes))
        assert(result == Result.fail(WhatsAppError.SignatureError.Malformed()))
    }
    ...
    "a wrong digest yields Failure(Mismatch)" in {
        ...
        val result      = WhatsAppWebhook.verifySignature(appSecret, Present(wrongHeader), Span.from(bodyBytes))
    ...
    "a one-byte-tampered body yields Failure(Mismatch)" in {
        ...
        val result = WhatsAppWebhook.verifySignature(appSecret, Present(knownGoodHeader), Span.from(tampered))
        assert(result == Result.fail(WhatsAppError.SignatureError.Mismatch()))
    }

    "an uppercase-hex header still matches" in {
        val upperHeader = s"sha256=${knownGoodHex.toUpperCase}"
        val result      = WhatsAppWebhook.verifySignature(appSecret, Present(upperHeader), Span.from(bodyBytes))
        assert(result == Result.unit)
    }

    "verifySignature is pure and never throws" in {
        val r1 = WhatsAppWebhook.verifySignature(appSecret, Absent, Span.empty)
        val r2 = WhatsAppWebhook.verifySignature(appSecret, Present("no-prefix"), Span.empty)
        val r3 = WhatsAppWebhook.verifySignature(appSecret, Present("sha256=abc"), Span.empty)
    ...
    "verifySignature accepts the full webhook body unchanged" in {
        ...
        val result    = WhatsAppWebhook.verifySignature(secret, Present(header), Span.from(bytes))
        assert(result == Result.unit)
    }
