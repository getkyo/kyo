...
import kyo.*
import kyo.whatsapp.internal.Codec

class NotificationTest extends BaseWhatsAppTest:
    ...
    def decodeFirst(json: String)(using Frame): Notification =
    ...
    val expectedMeta = Notification.Metadata("15550783881", Id.PhoneNumberId("106540352242922"))
    ...
    "verbatim inbound text webhook decodes to InboundMessage with sender profile name" in {
        Codec.decodeNotifications(Span.from(textWebhookJson.getBytes("UTF-8"))) match
            case Result.Success(chunk) =>
                assert(chunk.size == 1)
                chunk.head match
                    case msg: Notification.InboundMessage =>
                        assert(msg.metadata == expectedMeta)
                        assert(msg.from == Id.WhatsAppId("16505551234"))
                        assert(msg.id == Id.MessageId("wamid.HBgLMTY1MDM4Nzk0MzkVAgASGBQzQTRBNjU5OUFFRTAzODEwMTQ0RgA="))
                        assert(msg.timestamp == 1749416383L)
                        assert(msg.content == Notification.Content.Text("Does it come in another color?"))
    ...
    "an image message decodes to Content.Media with its metadata" in {
        ...
        n match
            case msg: Notification.InboundMessage =>
                assert(msg.content == Notification.Content.Media(
                    Media.MediaType.Other("image"),
                    Id.MediaId("MEDIA123"),
                    "image/jpeg",
                    "abc123",
                    Present("nice photo"),
    ...
    "a location message decodes to Content.Location" in {
        ...
        n match
            case msg: Notification.InboundMessage =>
                assert(msg.content == Notification.Content.Location(
                    37.483307,
                    122.148981,
    ...
    "a reaction message decodes to Content.Reaction" in {
        ...
        n match
            case msg: Notification.InboundMessage =>
                assert(msg.content == Notification.Content.Reaction(Id.MessageId("wamid.ORIG"), "😀"))
            case other => assert(false, s"expected InboundMessage, got: $other")
        end match
    }
    ...
    "button and interactive replies decode to the correct Content cases" in {
        ...
        buttonN match
            case msg: Notification.InboundMessage =>
                assert(msg.content == Notification.Content.Button("PAYLOAD_VALUE", "Click me"))
            case other => assert(false, s"expected InboundMessage, got: $other")
        end match
        ...
        btnReplyN match
            case msg: Notification.InboundMessage =>
                assert(msg.content == Notification.Content.ButtonReply("BTN1", "Yes"))
            case other => assert(false, s"expected InboundMessage, got: $other")
        end match
        ...
        listReplyN match
            case msg: Notification.InboundMessage =>
                assert(msg.content == Notification.Content.ListReply("ITEM1", "Option A", Present("First option")))
            case other => assert(false, s"expected InboundMessage, got: $other")
        end match
    }
    ...
    "contacts, order, and system messages decode to the correct Content cases" in {
        ...
        contactsN match
            case msg: Notification.InboundMessage =>
                msg.content match
                    case Notification.Content.Contacts(cs) =>
                        assert(cs.size == 1)
        ...
        orderN match
            case msg: Notification.InboundMessage =>
                assert(msg.content == Notification.Content.Order("CAT123", Chunk("PROD1", "PROD2")))
            case other => assert(false, s"expected InboundMessage, got: $other")
        end match
        ...
        systemN match
            case msg: Notification.InboundMessage =>
                assert(msg.content == Notification.Content.System("User changed their number"))
            case other => assert(false, s"expected InboundMessage, got: $other")
        end match
    }
    ...
    "a message with a context decodes the reply context" in {
        ...
        n match
            case msg: Notification.InboundMessage =>
                assert(msg.context == Present(Notification.Context(Id.WhatsAppId("16505551234"), Id.MessageId("wamid.ORIG123"))))
            case other => assert(false, s"expected InboundMessage, got: $other")
        end match
    }
    ...
    "verbatim status webhook decodes to StatusUpdate" in {
        Codec.decodeNotifications(Span.from(statusWebhookJson.getBytes("UTF-8"))) match
            case Result.Success(chunk) =>
                assert(chunk.size == 1)
                chunk.head match
                    case su: Notification.StatusUpdate =>
                        assert(su.metadata == expectedMeta)
                        assert(su.id == Id.MessageId("wamid.HBgLMTY1MDM4Nzk0MzkVAgARGBI3MTE5MjVBOTE3MDk5QUVFM0YA"))
                        assert(su.status == Notification.Status.Delivered)
                        assert(su.timestamp == 1750263773L)
                        assert(su.recipientId == Id.WhatsAppId("16505551234"))
                        assert(su.conversation == Present(Notification.Conversation(
                            "6ceb9d929c9bdc4f90e967a32f8639b4",
                            Absent,
                            "service"
                        )))
                        assert(su.pricing == Present(Notification.Pricing(true, "CBP", "service", Absent)))
    ...
    "each documented status value decodes to its typed case" in {
        Seq("sent", "delivered", "read", "failed", "deleted").zip(
            ...
            val n    = decodeFirst(json)
            n match
                case su: Notification.StatusUpdate => assert(su.status == expected, s"for $statusStr")
    ...
    "a failed status carries the Cloud errors" in {
        val json = makeStatusChange(
            "failed",
            """, "errors": [{"code": 131026, "type": "OAuthException", "message": "Message undeliverable"}]"""
        )
        val n = decodeFirst(json)
        n match
            case su: Notification.StatusUpdate =>
                assert(su.status == Notification.Status.Failed)
        ...
        Codec.decodeNotifications(Span.from(json.getBytes("UTF-8"))) match
            case Result.Success(chunk) =>
                ...
                chunk.head match
                    case msg: Notification.InboundMessage =>
                        msg.content match
                            case Notification.Content.Unknown("future_unknown_type", raw) =>
                                assert(raw.nonEmpty)
    ...
    "an unknown status decodes to Status.Other, NOT an Abort" in {
        ...
        Codec.decodeNotifications(Span.from(json.getBytes("UTF-8"))) match
            case Result.Success(chunk) =>
                ...
                chunk.head match
                    case su: Notification.StatusUpdate =>
                        assert(su.status == Notification.Status.Other("future_status"))
                    case other => assert(false, s"expected StatusUpdate, got: $other")
                end match
            case other =>
                assert(false, s"expected Success (no Abort), got: $other")
