...
import kyo.whatsapp.Interactive.*

class InteractiveTest extends BaseWhatsAppTest:

    "every Interactive variant exposes body/header/footer" in {
    ...
    "Header is a sealed union of text/media/document" in {
        ...
        val mh: Header = Header.Media(Media.Source.ById(Id.MediaId("m")), Header.MediaKind.Image)
        val dh: Header = Header.Document(Media.Source.ByLink("l"), Present("menu.pdf"))
        th match
    ...
    "Flow.Ref is flow_id-XOR-flow_name exclusive" in {
        ...
        byName match
            ...
            case _                  => assert(false)
        typeCheckFailure("""Interactive.Flow.Ref.ById(flowId = "F", flowName = "N")""")
    }
    ...
    "list encodes to the expected list JSON shape" in {
        ...
        val listMenu = Interactive.ListMenu(
            "Shipping Options",
            Chunk(
                Interactive.Section(
                    "I want it ASAP!",
                    Chunk(
                        Interactive.Row("priority_express", "Priority Mail Express", Present("Next Day to 2 Days")),
                        Interactive.Row("priority_mail", "Priority Mail", Present("1-3 Days"))
                    )
                ),
                Interactive.Section(
                    "I can wait a bit",
                    Chunk(
                        Interactive.Row("usps_ground_advantage", "USPS Ground Advantage", Present("2-5 Days"))
                    )
                )
            ),
            Present("Which shipping option do you prefer?"),
            Present(Interactive.Header.Text("Choose Shipping Option")),
            Present("Lucky Shrub: Your gateway to succulents!")
        )
        val msg  = Message.OfInteractive(listMenu)
    ...
    "buttons encodes action.buttons[].reply with type:reply" in {
        val to = Id.WhatsAppId("PHONE_NUMBER")
        val buttons = Interactive.Buttons(
            Chunk(
                Interactive.ReplyButton("track_shipment", "Track shipment"),
                Interactive.ReplyButton("contact_support", "Contact support")
            ),
            Present("Would you like to track your shipment?"),
            Present(Interactive.Header.Text("Your order is confirmed")),
            Present("Lucky Shrub")
        )
        val msg  = Message.OfInteractive(buttons)
        val json = new String(kyo.whatsapp.internal.Codec.encodeSend(to, msg, Absent).toArray, "UTF-8")
    ...
    "product encodes action.catalog_id+product_retailer_id with no header" in {
        val to      = Id.WhatsAppId("PHONE_NUMBER")
        val product = Interactive.Product("CATALOG_ID", "ID_TEST_ITEM_1", Present("optional body text"), Present("optional footer text"))
        val msg     = Message.OfInteractive(product)
        val json    = new String(kyo.whatsapp.internal.Codec.encodeSend(to, msg, Absent).toArray, "UTF-8")
    ...
    "product_list encodes required text header and sections with product_items" in {
        val to = Id.WhatsAppId("PHONE_NUMBER")
        val pl = Interactive.ProductList(
            "CATALOG_ID",
            "Our top picks for you",
            "Check out these items",
            Chunk(
                Interactive.ProductSection("Succulents", Chunk("SKU_1001", "SKU_1002")),
                Interactive.ProductSection("Planters", Chunk("SKU_2001"))
            ),
            Present("Sale ends Sunday")
        )
        val msg  = Message.OfInteractive(pl)
        val json = new String(kyo.whatsapp.internal.Codec.encodeSend(to, msg, Absent).toArray, "UTF-8")
    ...
    "flow encodes flow_id branch with flow_message_version 3" in {
        val to = Id.WhatsAppId("PHONE_NUMBER")
        val flow = Interactive.Flow(
            "RANDOM_FLOW_TOKEN",
            Interactive.Flow.Ref.ById("YOUR_FLOW_ID"),
            "Start survey",
            Interactive.Flow.Action.Navigate("SURVEY_START", Present("{}")),
            Interactive.Flow.Mode.Published,
            Present("Please fill the form"),
            Present(Interactive.Header.Text("Feedback")),
            Present("Thank you!")
        )
        val msg  = Message.OfInteractive(flow)
        val json = new String(kyo.whatsapp.internal.Codec.encodeSend(to, msg, Absent).toArray, "UTF-8")
    ...
    "flow encodes flow_name branch with data_exchange and no payload" in {
        ...
        val flow = Interactive.Flow(
            "tok2",
            Interactive.Flow.Ref.ByName("feedback_survey"),
            "Start",
            Interactive.Flow.Action.DataExchange,
            Interactive.Flow.Mode.Draft
        )
        val msg  = Message.OfInteractive(flow)
        val json = new String(kyo.whatsapp.internal.Codec.encodeSend(to, msg, Absent).toArray, "UTF-8")
