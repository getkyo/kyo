...
1. Classpath presence of `MachineStatFactory` triggers construction: JVM/Native
   ...
   page half in a real browser: the export still fires, and no
   `MachineStatFactory` is registered under `ExporterFactory`. What is under
   test there is the gate inside the initializer, not whether it ran.
...
- **Real-host leaves run only where the host matches.**
