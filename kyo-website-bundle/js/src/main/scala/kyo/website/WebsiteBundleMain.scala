...
/** SPA bundle entry-point.
  ...
  * `<link rel=canonical>` href to match the new route, using the SAME title/canonical string formats
  * `WebsiteGenerator` emits at build time, so the in-browser head never diverges from what crawlers
  * indexed.
  *
...
object WebsiteBundleMain:
    ...
    private def build()(using Frame): UI < (Sync & Async & Scope) =
        ...
        // Two landing visuals draw themselves on scroll-into-view: the gap chart (`#gap-chart`) and the
        // platforms "one codebase -> four cards" connector (`#pf-connect`). The mount replaces
        // document.body's HTML, so each SSR node is swapped for a freshly-rendered one; capture the SSR
        // nodes here (before mount) so the reveal fibers can wait for the swap and observe the LIVE node,
        // not the about-to-be-detached SSR one. `null` off the landing route (neither present there).
        val ssrGapChart  = dom.document.getElementById("gap-chart")
        val ssrPfConnect = dom.document.getElementById("pf-connect")
        for
            ...
            _ <- wireCodeCopy
            // Arm each landing visual's scroll-revealed line draw. Forked because the poll must observe the
            // post-mount node and so cannot complete until after this build returns the view to mount.
            _          <- Fiber.init(wireScrollDraw(ssrGapChart, "gap-chart"))
            _          <- Fiber.init(wireScrollDraw(ssrPfConnect, "pf-connect"))
            ...
            view <- SiteApp.view(
                ...
                toggleTheme,
                // The version selector navigates with a full browser load to the chosen version's docs
                // tree (`/latest/` or `/<tag>/`): a hard navigation, not a client-side push, so the new
                // version's bundle + islands boot fresh under their own prefix.
                uri => UILocation.assign(uri),
                content
            )
        yield view
        end for
    ...
    /** The bounded poll budget for arming a scroll-revealed draw: up to [[GapMaxAttempts]] lookups
      * [[GapPollInterval]] apart. The mount swaps `document.body`'s HTML once at boot; the poll yields the
      * event loop between attempts so it observes the freshly-mounted node rather than the SSR node it
      * replaces. Exhausting the budget is a clean no-op (the visual simply stays fully drawn).
      */
    private val GapMaxAttempts: Int       = 25
    private val GapPollInterval: Duration = 20.millis

    /** Arm a landing visual's scroll-revealed line draw, by element id (`gap-chart` or `pf-connect`). The
      * visual sits below the fold, so animating its `stroke-dashoffset` on load would play the draw unseen.
      * Instead each line renders fully drawn by default and the keyframe is gated behind a `.chart-drawn`
      * class on the visual's root; this observes that root and adds the class the first time it scrolls
      * into view.
      *
      * `ssrNode` is the server-rendered root captured BEFORE mount. The mount replaces `document.body`'s
      * HTML, so the live root is a different node; this polls (bounded, yielding the event loop) until an
      * element with `id` distinct from the SSR node is present, then observes that mounted node. Off the
      * landing route the element is absent, so the poll cleanly exhausts to a no-op. Reduced motion needs
      * no branch here: the `.chart-drawn` rule is itself gated behind `prefers-reduced-motion:
      * no-preference`, so adding the class is inert when motion is not wanted.
      */
    private def wireScrollDraw(ssrNode: dom.Element, id: String)(using Frame): Unit < (Async & Scope) =
        Loop[Int, Unit, Async & Scope](0) { attempt =>
            Sync.defer(dom.document.getElementById(id)).map { live =>
                ...
                if live != null && !live.eq(ssrNode) then
                    UIWindow.onIntersectById(id)(addChartDrawn(id)).andThen(Loop.done)
                else if attempt >= GapMaxAttempts - 1 then Loop.done
                else Async.sleep(GapPollInterval).andThen(Loop.continue(attempt + 1))
            }
        }

    /** Add the `.chart-drawn` class to the live element with `id`, which (motion allowed) runs its
      * line-draw keyframe. A no-op if the node has since gone.
      */
    private[website] def addChartDrawn(id: String)(using Frame): Unit < Sync =
        Sync.defer {
    ...
    private def showContentRoute(
        ...
        for
            ...
            // new page starts at its heading instead of inheriting the previous page's scrollY.
            _ <- scrollToHashOrTop()
        yield ()
