...
object Client:
     ...
     *
     * First attempts graceful shutdown by calling [[io.grpc.ManagedChannel.shutdown]] and waits up to `timeout` for
     * termination. If the server doesn't terminate within the timeout, forces shutdown with
     * [[io.grpc.ManagedChannel.shutdownNow]] and then waits up to 1 hour for it to terminate (there is no indefinite
     * wait).
     *
     * @param channel
