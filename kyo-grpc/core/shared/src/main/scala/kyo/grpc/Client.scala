...
object Client:
    ...
    /** Attempts an orderly shut down of the [[ManagedChannel]] within a timeout.
      *
      * First attempts graceful shutdown by calling [[ManagedChannel.shutdown]] and waits up to `timeout` for termination. If the
      * server doesn't terminate within the timeout, forces shutdown with [[ManagedChannel.shutdownNow]] and then waits up to 1 hour
    ...
    /** Creates a managed gRPC channel with automatic resource cleanup.
      *
      * Creates a [[ManagedChannel]] that is automatically acquired and released via the [[Resource]] effect. The channel is
      * configured using the provided `configure` function and will be properly shut down when the resource is released.
      * 
      * The returned channel can be passed to the `client` method on service companion objects generated by kyo-grpc-code-gen
      * to create typed gRPC clients. Alternatively, you can use the `managedClient` method on service companion objects which
      * combines channel creation and client creation in a single step.
      *
      * @example
      *   {{{
      ...
      * @param configure
      *   A function to configure the [[ManagedChannelBuilder]] before building the channel. This allows customization of
      *   channel settings like TLS, keepalive, retry policies, etc.
      * @param shutdown
