...
class ResponseStreamObserverTest extends Test:
    ...
    private def initObserver: (StreamChannel[String, GrpcResponse.Errors], ResponseStreamObserver[String]) < IO =
        for
            channel  <- StreamChannel.init[String, GrpcResponse.Errors]
            observer <- IO.Unsafe(ResponseStreamObserver[String](channel))
        yield (channel, observer)

    "ResponseStreamObserver" - {
        "onNext" - {
            "should put a Success value to the channel" in run {
                for
                    (channel, observer) <- initObserver
                    _                   <- IO(observer.onNext("test"))
                    result              <- channel.take
                    completedValue      <- channel.completed
                yield
            ...
            "should handle backpressure when channel is full" in runNotJS {
                for
                    ...
                    // Don't forget that there can only be a single consumer and a single producer.
                    fiber <- Async.run {
                        for
                          _ <- Loop.repeat(StreamChannel.Capacity)(IO(observer.onNext("test")))
                          _ <- IO(observer.onNext("full"))
                        yield ()
                    }
