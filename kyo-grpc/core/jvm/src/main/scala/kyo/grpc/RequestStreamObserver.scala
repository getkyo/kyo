 ...
 *
 * @param f a function that takes a stream of requests and returns a single response
 * @param requestChannel a channel for receiving requests
 * @param responseObserver the observer that will receive the response
...
object RequestStreamObserver:
     ...
     *
     * When the pending [[IO]] is run, it will start the background processing of requests and sending of the response.
     * It will complete when the response has been sent to the `responseObserver`, and it has been completed. It will
     * abort if an error occurs while processing the requests or sending the response and the `responseObserver` will
     * receive the error.
     *
     * @param f the function that processes the stream of requests and produces a single response
     * @param responseObserver the observer that will receive the response
     * @tparam Request the type of the request messages
     * @tparam Response the type of the response message
     * @return an instance of `RequestStreamObserver` pending [[IO]]
     */
