...
private[grpc] class UnaryServerCallHandler[Request, Response](f: GrpcHandlerInit[Request, Response])(using Frame)
    ...
    override def sent(call: ServerCall[Request, Response], handler: GrpcHandler[Request, Response], promise: Promise[Request, Abort[Status]]): Status < (Grpc & Emit[Metadata]) =
        Abort.merge:
            for
                request <- promise.get
