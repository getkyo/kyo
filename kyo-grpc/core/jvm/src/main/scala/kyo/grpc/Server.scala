...
object Server:
    ...
    def waitForInterrupt(using Frame): Unit < Async =
        for
            promise <- Promise.init[Nothing, Unit]
            complete = promise.complete(Result.success(())).unit
            _ <- IO(Signal.handle(new Signal("INT"), _ => IO.run(complete).eval)).unit
            _ <- IO(Signal.handle(new Signal("TERM"), _ => IO.run(complete).eval)).unit
            _ <- promise.get
        yield ()
