...
def apply_all():
    ...
    for edit in SPEC["edits"]:
        ...
        if edit["kind"] == "create":
            ...
            continue
        if edit["kind"] == "delete":
            # the file exists at the base and not at the tip; nothing to apply, and no
            # digest entry exists for it, so verify checks it by absence from the spec
            files.pop(path, None)
            continue
...
def listing():
    for edit in SPEC["edits"]:
        old = 0 if edit["kind"] in ("create", "delete") else len(edit["old"].split("\n"))
