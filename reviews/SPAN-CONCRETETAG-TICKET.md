...
Prerequisite, and the real work in this item: `ConcreteTagMacro` refuses exactly the element types Span is routinely used with. Applied types hard-error on `tpe.typeArgs.nonEmpty` ("has type parameters"), and opaque or abstract types fail the earlier `isClassDef` check ("is not a class"), so `ConcreteTag[Maybe[Any]]` is refused twice over while `Span[Maybe[Any]]` is an ordinary element type. ClassTag accepts all of these by erasure, which is why Span works today. The migration therefore needs an erasure arm in the derivation: when the strict analysis refuses, derive `fromClass(<erased class>)` instead. None of these summons compile today, so the arm adds capability without changing any existing resolution, and the macro's final arm already emits a `classOf` literal, so the implementation is contained. The design decision to settle first is the semantics of `accepts` for erased tags: erasure-based (exactly `ClassTag`/`isInstanceOf` semantics, where a tag derived at `List[Int]` accepts any `List`), or kept strict with the erased arm usable for array allocation only.
