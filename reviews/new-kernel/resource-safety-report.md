...
- C4 (`Async.uninterruptible`): reproduced. `AsyncTest`, "a value the shielded body produces is not stranded when the caller is interrupted at the join", pending: the shielded body completes with its value into a promise the caller joins, and a caller interrupted while parked on that join never registers the release. The caller's own finalizer does run, pinned green beside it ("interrupting the caller of uninterruptible runs the caller's finalizer while the shielded body completes").
- C6 (`Async.timeout` spawn): reproduced, a window the finding did not name. `AsyncTest`, "an interrupt landing at the timeout's spawn reaches the guarded computation", pending: the timeout wires the child and the sleep in the step the child's fiber arrives and then joins the child; a stop requested during that step is observed in front of the join, before the join has linked the child, so the abandonment interrupts nothing and the child runs on unowned. A 10 microsecond sweep from the step before the timeout lands it.
- C5 (`Queue.close`): pinned green. `QueueTest`, "an interrupted close still closes the queue", 100 rounds.
- C3 (`Channel.take`): pinned green. `ChannelTest`, "take hands the element to its continuation, which a pending stop can drop whole": a dropped element is never both released and left in the channel.
...
- D18 (`Command.spawn`): reproduced. `CommandTest`, "an interrupt landing during spawn does not orphan the process", pending on the JVM (three of three runs) and on Native (two of two). The fork is a fraction of a millisecond on Native and a few on the JVM, below what a timer lands in, and a fiber woken by a latch the spawner releases is queued behind the fork on the same worker, so timer-driven and latch-driven interrupts reproduced it unreliably or never. The leaf's own fiber now spins on a flag the spawner sets in the step before the fork, spins on to a staggered 100 microsecond offset, and requests the stop directly.
- DOC1: fixed in `kyo-kernel/CONTRIBUTING.md`, rewritten to the current walk.
- `Fiber.internal.raceFirst` loser left running: one JVM run on macOS, under heavy machine load, ended with the fork's fiber probe reporting a worker still running `FiberTest` "raceFirst" › "n"'s losing loop long after the race had completed, which is a stop the loser never observed. `FiberTest`, "interrupts a losing computation that never parks", races an immediate winner against a spinning loser that owes a finalizer, 500 rounds per run, and is green in three JVM runs, one Native and one JS run; the observation is not reproduced and stays open.
...
- T5 (`Hub.initUnscopedWith`): no observable for the orphan itself, which parks on a channel nothing else references. `HubTest`, "interrupting Hub.use around its spawn settles the caller with the interrupt", sweeps the spawn at 25 microsecond offsets and pins what can be seen: the caller always settles with the interrupt.
...
- T27: pinned green on the JVM and Native. `FiberTest`, "a fatal thrown in the body releases the fiber's finalizers before the promise settles with the panic". Not on JS: the task rethrows the fatal after completing the promise, and on Node the rethrow reaches the event loop and ends the test process, which the spec did not anticipate.
...
- T1 (K1), core companion: not reproduced. `ScopeInterruptTest`, "a peeled remainder running on a child fiber is not released under it when the peeling scope ends", is green: an `Emit.runFirst` remainder holding a `Sync.ensure` region, handed to a child fiber and parked inside its use while the peeling `Scope.run` ends, completes with the resource still held. The kernel-level shape is green too: `BracketTest`, "a peeled remainder parked in a nested eval is not released under it when the peeling region ends", parks the remainder inside its use in a nested `Eval.partial` on the same thread, ends the peeling region, and resumes it: the use is either refused with `Closed` or completes on a resource that was not released under it.
- D11, server half: reproduced. `HttpServer.init` wraps `initUnscoped` in `Scope.acquireRelease`, whose `ensureMap` covers only the acquire's last step; the acquire itself joins the listen fiber and maps the bound server in a step of its own, and a stop landing there abandons the listener bound (`HttpServer.scala:135-148`). Timer-driven rounds never hit that step on macOS (40 of 40 green) and hit it on Linux under `podman-ci`, where the leaf failed on the held port and the fork's descriptor probe reported the listener (`socket:[...] [LISTEN local:40451 remote:0]`). `HttpServerTest`, "an interrupt landing as the listener binds leaves no listener behind", now spins to sub-millisecond offsets from the step before `init`, as the `Command.spawn` leaf does, and is pending. The client half has a leaf as well, `HttpServerTest`, "an interrupt landing as the client's connection completes leaves no connection behind", which reads the operating system's socket table (`lsof`) for connections to the server's port after the client's scope closed: reproduced, pending, a connection still established on both ends after a stop one millisecond into the request. Its first runs failed for an unrelated reason that became its own finding: the fiber's `getResult`, a `Result.Panic` carried as a value, collapsed into the run's own panic when it crossed `Abort.run` (next item).
- `Result` collapses a success carrying a Panic: `Result.Success.apply` (`kyo-data/shared/src/main/scala/kyo/Result.scala:190-194`) boxes a `Failure` into `SuccessError` and lets a `Panic` through unboxed, so `Success(Panic(x))`, `Result.succeed(Result.panic(x))`, a `map` producing a Panic as a value, and `Abort.run` over a body that yields a Panic as a value all read as `Panic(x)` at the outer level. `ResultTest`, "a success carrying a Panic" (four leaves), and `AbortTest`, "a Result.Panic the body produces as a value is not taken as the run's own panic", pending. Pre-existing by reading; it surfaces wherever a fiber's result is handed on as data.
...
- D16 (`SpawnBackend.init`): reproduced. `SpawnBackendTest`, "an interrupt landing before the kill is armed does not orphan the worker JVM", pending. The window between the spawn returning and the kill being armed is a few milliseconds wide, so the rounds sweep the first 12 ms in quarter-millisecond steps; three consecutive runs each left a worker JVM running past a 10 s bound (the leaf kills it afterwards). With 4 ms steps one run in two missed the window.
- D1 (`closeAll` extraction): reproduced. `SqlClientInterruptTest`, "an interrupt landing as close extracts the idle ring strands no session", pending: with rounds whose stop landed before the close began excluded (they close the client themselves) and a 10 microsecond sweep, the JVM lands the one-poll window at round 1 and Native at round 0, and the extracted connections stay open on the server. A 50 microsecond sweep had missed it on the JVM.
- D2 and the reservation beside it: reproduced, pending on the JVM (three of three runs) and Native. `SqlClientInterruptTest`, "leases stopped at staggered offsets leave a pool that still serves and closes clean": after two hundred back-to-back leases stopped at 10 microsecond offsets, the pool of two refuses every acquire with `SqlConnectionAcquireTimeoutException`. Both paths into the pool take ownership one step after the slot is handed over: the reservation taken in the step that finds the ring empty is released two steps later in the `resolvingOnce` around the connect (`SqlConnectionPool.scala:577`, `:490-493`), and the pooled path registers the exit one step before the custody take (`:911-913`). A statement between the stops masks the loss by re-pooling what each round released, and a fresh pool per round never reaches the pooled path, which is why two intermediate shapes of the leaf passed.
- D5 (browser launcher spawn): not reproduced. `BrowserLauncherJvmTest`, "a launch stopped around its spawn leaves no Chrome behind", green over 40 rounds: the temp directory's removal is registered before the spawn and sweeps by the directory's unique name, so a Chrome the stop orphaned is found and killed at scope close.
- D6 (dialog drainer): the drainer has no observable, parked on a queue nothing else references. `CdpBackendInterruptTest`, "an interrupt landing at the version probe closes the endpoint the init built", pins the scope-bound half green over an in-memory CDP wire the suite plays the browser side of.
- D7 (browser contexts): reproduced. `CdpBackendInterruptTest`, "an interrupt landing at the context creation reply still disposes the context", pending: the disposal is registered in the step after the creation reply, and a stop at that reply's join leaves the context undisposed.
- D8 (page state through a round trip): reproduced. `CdpBackendInterruptTest`, "an interrupt landing at the viewport override reply still restores the viewport", pending, the same shape on `withViewport`.
- D9 (`Topic` hand-off, the K3 instance): reproduced. `AeronTransportTest`, "a publication the add hands on under a stop is closed by someone", pending: over a fake transport that confirms every add on its first poll, stops swept at 25 microsecond offsets across the add left one publication of fifteen open with nobody to close it.
- D10 (aeron native runtimes): not reproduced. `AeronClientTest`, "connects stopped at staggered offsets leave a driver that still closes", green: 40 connects against the embedded driver stopped at 100 microsecond offsets, every client the rounds did receive closed, and the driver closed within its bound. The abandoned native client has no Scala observable.
- D12 (http fiber trios and the chunked decoder): no leaf; the spawns sit inside the request dispatch with no observation point from either end of the wire.
- D14 (jsonrpc engine): not reproduced. `JsonRpcHandlerTest`, "closing the endpoint while a request is being dispatched interrupts its handler", green over 60 rounds: every handler known to have started was interrupted by the endpoint's close.
- D17 (flow supervision): not reproduced. `FlowEngineLifecycleTest`, "closing the engine while it spawns a supervision leaves no supervision renewing the claim", green over 30 rounds: the claim's expiry stops moving once the engine has closed.
...
- **Confirmed on Linux.** Run under `scripts/build.sh --env podman-ci` (the CI container), the kyo-jsonrpc JVM leg with the UDS
  leaf reports exactly the leak the leaf reproduces: `file-descriptor leak (1): socket:[...] [unix /tmp/kyo-jsonrpc-uds-.../test.sock]`,
  and the fork fails on it even though the leaf itself is pending. So the check works where it runs; it had nothing to report before
  because no leaf interrupted inside the window, and it never runs on the macOS machines the branch was validated on. Until
  `UdsBackend.connect` registers the listener's release in the step that delivers it, that leg is red on Linux. The kyo-system leg with the `Command.spawn` leaf passes its probes: the leaf
  reproduces the orphan and kills it, and the process reaper closes the pipes, so nothing reaches the end of the fork. The kyo-http leg
  reports the listener the `HttpServer.init` leaf leaks (`socket:[...] [LISTEN local:40451 remote:0]`), the D11 finding above. The
  kyo-core leg (every core suite touched here), kyo-combinators, kyo-actor and kyo-compiler pass their probes with their pending
  leaves reproducing; in particular the aeron client the `SpawnBackend.init` leaf abandons trips neither the thread nor the
  descriptor category. So two Linux JVM legs, kyo-jsonrpc and kyo-http, stay red on the descriptor probe until `UdsBackend.connect`
  and `HttpServer.initUnscoped` register their listener's release in the step that delivers it.
