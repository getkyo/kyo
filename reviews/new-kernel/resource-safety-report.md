...
- D1 (`closeAll` extraction): not reproduced. `SqlClientInterruptTest`, "an interrupt landing as close extracts the idle ring strands no session", is green over 40 rounds of stops at 50 microsecond offsets once rounds whose stop landed before the close began are excluded (they close the client themselves); its first failure was that case, not the window. The window, one poll between the ring drain and the force-close's installation, stands by reading.
- D2 and the reservation beside it: reproduced, with a mechanism the finding did not name. `SqlClientInterruptTest`, "leases stopped at staggered offsets leave a pool that still serves and closes clean", pending: after sixty leases stopped at sub-millisecond offsets, the pool of two refuses every acquire with `SqlConnectionAcquireTimeoutException`. A lease that finds the ring empty takes a slot reservation in that step (`SqlConnectionPool.scala:577`) and registers its release two steps later in the `resolvingOnce` around the connect (`:490-493`); a stop between them holds the reservation for good.
