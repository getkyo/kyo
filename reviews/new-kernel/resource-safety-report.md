...
- D11, server half: reproduced. `HttpServer.init` wraps `initUnscoped` in `Scope.acquireRelease`, whose `ensureMap` covers only the acquire's last step; the acquire itself joins the listen fiber and maps the bound server in a step of its own, and a stop landing there abandons the listener bound (`HttpServer.scala:135-148`). Timer-driven rounds never hit that step on macOS (40 of 40 green) and hit it on Linux under `podman-ci`, where the leaf failed on the held port and the fork's descriptor probe reported the listener (`socket:[...] [LISTEN local:40451 remote:0]`). `HttpServerTest`, "an interrupt landing as the listener binds leaves no listener behind", now spins to sub-millisecond offsets from the step before `init`, as the `Command.spawn` leaf does, and is pending. The client half (`poolWithImpl`, `connectWebSocket`, `connectRaw`) has no leaf: a leaked client connection is observable from neither side without a seam.
