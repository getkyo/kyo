...
- **Confirmed on Linux.** Run under `scripts/build.sh --env podman-ci` (the CI container), the kyo-jsonrpc JVM leg with the UDS
  ...
  reproduces the orphan and kills it, and the process reaper closes the pipes, so nothing reaches the end of the fork. The other
  legs are recorded below as they complete.
