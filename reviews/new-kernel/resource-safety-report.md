...
- D18 (`Command.spawn`): reproduced. `CommandTest`, "an interrupt landing during spawn does not orphan the process", pending. Timer-driven interrupts reproduced it in three of three JVM runs with millisecond delays from the fiber start and in none of three once aligned to a latch with half-millisecond delays, and never on Native, where the fork takes a fraction of a millisecond; the leaf now spins a second fiber to a 100 microsecond offset from the latch and requests the stop directly, and its reproduction rate per platform is recorded with the run results.
