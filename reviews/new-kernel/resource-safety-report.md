...
- D1 (`closeAll` extraction): reproduced. `SqlClientInterruptTest`, "an interrupt landing as close extracts the idle ring strands no session", pending: a stop landing between the ring drain and the force-close's installation leaves the extracted connections open on the server; rounds whose stop landed before the close began close the client themselves and prove nothing.
