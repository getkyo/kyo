...
| `SqlClientInterruptTest`, "an interrupt landing as close extracts the idle ring strands no session" (D1) | `closeAll` drains the ring in one step and installs the force-close in the next: `SqlConnectionPool.scala:215-222` | JVM, real Postgres | the cell-filled-in-the-same-step shape `db/Connection.scala:395-407` uses |
...
`mapPar` element fibers, `PubSub` subscriptions, the sql warm-up handover and advisory lock grant, the aeron native client,
the http fiber trios and decoder (no observation point), `Hub.use`'s orphaned publisher (no observation point).
