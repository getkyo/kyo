...
JIT-eliminated, precisely: zio-blocks has no interpreter, so after C2 inlines a whole row its body
is plain arithmetic. Where the row's result does not depend on any runtime input (the per-level
chain values are discarded, the recursion's final value is a constant), C2 dead-code-eliminates
the entire computation and the method returns a precomputed constant; the ~2ns reported is the
empty-harness floor. The in-column contrast proves the mechanism: after the seed fix,
deepRecursionPaysRescuesOnly became real (1ns to 3.05us, 10,000 actual frames) while the
seed-independent rows stayed at ~2ns. Every other library's machinery is opaque to the JIT, so
their cells always measure real execution. Eliminated cells are excluded from the verdict and from
best-of-row.

Verdict and bold: on each row, 🟢 = kernel2 better than every other measured column (oldkernel
included), 🔴 = worse than at least one; **bold** marks the best measured cell of the row.
JIT-eliminated cells take no part in either. Allocation cells under 0.5 B/op print as 0; bold
marks the smallest raw value, so one of several printed zeros carries it.
...
| row | kernel2 | oldkernel | zio | ce | zioblocks | turbolift |
|---|---|---|---|---|---|---|
| continuationBodiesFuse | 🔴 64,136 | **56,072 (0.87x)** | 425,342 (6.63x) | 369,398 (5.76x) | 16 (JIT-eliminated) | 561,407 (8.75x) |
...
| fusionAfterSuspension | 🔴 712,785 | 408,437 (0.57x) | 353,238 (0.50x) | **329,361 (0.46x)** | 16 (JIT-eliminated) | 513,358 (0.72x) |
| fusionAfterSuspensionRunOnly | 🔴 1,240 | **0 (0.00x)** | 860 (0.69x) | 1,572 (1.27x) | 0 (JIT-eliminated) | 2,008 (1.62x) |
| fusionAllocatesNothing | 🟢 **0** | 0 | 10,992 | 10,800 | 0 (JIT-eliminated) | 17,984 |
| fusionPastBudgetPaysRescuesOnly | 🟢 **664** | 1,128 (1.70x) | 321,195 (483.51x) | 297,290 (447.52x) | 0 (JIT-eliminated) | 536,844 (808.13x) |
...
| statefulAnswersPaySuccessor | 🟢 **643,273** | 1,040,140 (1.62x) | 2,719,210 (4.23x) | 1,359,154 (2.11x) | 16 (JIT-eliminated) | 1,038,950 (1.62x) |
| suspensionBaseline | 🔴 640,137 | 560,081 (0.87x) | 961,031 (1.50x) | 721,123 (1.13x) | 16 (JIT-eliminated) | **320,856 (0.50x)** |
| suspensionFusesContinuation | 🔴 240,097 | **240,050 (1.00x)** | 640,536 (2.67x) | 721,103 (3.00x) | 16 (JIT-eliminated) | 400,867 (1.67x) |
| trailingMapsStayLinear | 🔴 2,320,956 | 1,601,202,469 (689.89x) | 1,492,358 (0.64x) | 1,264,982 (0.55x) | **160,032 (0.07x)** | 960,947 (0.41x) |
| uncachedValuesPayBoxingOnly | 🔴 155,376 | **141,776 (0.91x)** | 459,894 (2.96x) | 436,013 (2.81x) | 16 (JIT-eliminated) | 683,558 (4.40x) |
