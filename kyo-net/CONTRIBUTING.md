...
The public package `kyo.net` exposes eight types. Three of them, `Transport`, `Connection`, and `Listener`, are the surface; the rest are data (`NetAddress`, `NetTlsConfig`, `NetConfig`) plus the platform entry point (`NetPlatform`). The error vocabulary is the sealed `NetException` hierarchy, rooted directly in `KyoException` and disjoint from `Closed`, so a `Closed` handler never catches a transport failure and vice versa. Its leaves include `NetConnectException`, `NetConnectTimeoutException`, `NetDnsResolutionException`, `NetBindException`, `NetTlsHandshakeException`, `NetConnectionClosedException`, and the STARTTLS leaves. `NetConnectTimeoutException(host, port, timeout)` is the typed leaf the transport's own connect-deadline produces (see "Connect deadline" below); kyo-http maps it to its own connect-timeout type, so a regression that drops the typed leaf or substitutes the generic `NetConnectException` breaks that mapping.

`NetConfig` is a plain `case class`: it has no per-field setter methods, so tune it with `copy(...)`. Collections on the internal selection surface are `Chunk`, not `Seq`: `IoBackend.select` / `selectAndBuild` and `TlsProvider.selectFor` take `Chunk`, and `Connection.detachForUpgrade` returns `Maybe[Chunk[Span[Byte]]]`.
...
- `Transport` exposes `connect`, `connectUnix`, `stdio`, `listen`, `listenUnix`, `upgradeToTls`, `close`. The connecting and listening methods return `Fiber.Unsafe[Connection, Abort[NetException]]` / `Fiber.Unsafe[Listener, Abort[NetException]]` (`NetException` is disjoint from `Closed`, so a transport failure and a genuine channel/resource close are distinct, never-overlapping error channels).
...
- A **Transport** (`PosixTransport` shared, `NioTransport` JVM, `JsTransport` JS/Wasm) owns connection setup: DNS resolution, socket creation, the accept loop, the TLS handshake drive, and connection wiring. It produces `Connection` values and starts their pumps.
- An **IoDriver** (`internal/transport/IoDriver.scala`, the contract) does completion-based I/O: it performs the read internally and completes the caller's `Promise.Unsafe` with the bytes, rather than signalling readiness and letting the caller read. The contract methods are `start`, `awaitRead`, `awaitWritable`, `awaitConnect`, `awaitAccept`, `write`, `cancel`, `closeHandle`, `close`, and `submitEngineOp`, plus `isPeerClosed` (an optional override defaulting to `false`, reporting whether a backpressured connection's peer has closed so the `ReadPump`'s grace timer can reclaim an abandoned connection). Implementations: `PollerIoDriver` (shared; epoll on Linux, kqueue on macOS/BSD), `IoUringDriver` (shared; Linux io_uring), `BlockingReaderDriver` (shared; the regular-file / stdin-from-file fallback for fds that cannot be polled), `NioIoDriver` (JVM NIO selector floor), `JsIoDriver` (JS/Wasm Node floor). The three posix drivers live in `shared/` and run on every platform, including JS and Wasm over koffi, where they rank above the Node floor.
- A **TlsEngine** wraps the platform TLS implementation; the transport drives the handshake and the read/write encrypt/decrypt steps through it. On JVM the engine is `JdkSslEngine` (`javax.net.ssl`). On JVM, Native, and JS/Wasm (loaded over koffi on JS/Wasm) the native engine is the single `NativeSslEngine[B <: SslLibBindings]`: one 1:1 forwarding wrapper that drives any memory-BIO backend through the shared `SslLibBindings` type. The two concrete backends, `BoringSslBindings` (bundled BoringSSL, the primary on every posix platform) and `OpenSslBindings` (system OpenSSL, the Native fallback), declare the same 21-method intersection and bind their own prefixed C symbols. There is no per-backend engine class; an engine method that one backend cannot honor is a design defect, since the shared type is the cross-backend denominator.

### Backend and TLS selection

Both the I/O backend and the TLS provider are chosen at startup through one shared `IoBackend.select`: a forced `-D` property name wins if its entry is available (and fails with `NetBackendUnavailableException`/`NetTlsProviderUnavailableException` if forced-but-unavailable, never a silent fall-through), otherwise the highest-priority available entry wins.
...
- **Connect** on the readiness path uses `connectNow` (the non-blocking synchronous overload of `connect`, alongside `sendNow` / `recvNow` / `acceptNow`) on JS and Wasm: the `@Ffi.blocking connect` hands back a fiber whose inline result JS cannot peek, so a blind readiness-armed connect would hang until the connect deadline. JVM and Native inline-complete the `@Ffi.blocking connect` and never reach the synchronous overload.
...
- `shared/`: the public surface, `PosixTransport`, the poll/reap drivers (`PollerIoDriver`, `IoUringDriver`, `BlockingReaderDriver`), the native TLS stack (the unified `NativeSslEngine`, the shared `SslLibBindings` type with its `BoringSslBindings` / `OpenSslBindings` backends, the `SslLibProvider` base, and the `BoringSslProvider` / `SystemOpenSslProvider` providers), `HostResolver`, the `TlsProvider` selection logic, and the FFI binding traits whose impls every platform generates. The posix drivers and native TLS run on every platform, including JS and Wasm over koffi.
...
- `jvm-native/`: `NetPlatformTransportBase` (the JVM/Native transport base) plus the JVM/Native-only test infrastructure (the recording decorators and the native-TLS / io_uring / poller suites that need a real bundled native). The native TLS stack and the poll/reap drivers used to live here; they moved to `shared/` when the posix transport and BoringSSL TLS became available on JS and Wasm over koffi.
...
naming that backend, so those cells need nothing. A cancel says a backend was unavailable, not what an operation
returns, so `DeferredTransportTest` asserts that half in a page: one `connect` through `NetPlatform.transport`, failing
`NetBackendUnavailableException` on its own channel. The suites that reach `NetPlatform.transport` directly carry their
own `NotBrowser` filter, and the `Js*` transport suites reach Node's modules through `process.getBuiltinModule` at the
call, so nothing a page cannot evaluate runs while the suite is being constructed.
...
These injections live entirely in the test tree, over real components; they are not affordances on production code. A handshake-engine free count is NOT among them: `HandshakeEngineFreeTest` injects no handshake engine (there is no production engine-override seam). It asserts the observable consequence of correct teardown, that file descriptors do not accumulate across a soak of many real failing/succeeding handshakes (lowest-free-fd probe), with a real BoringSSL/OpenSSL engine on every path so a double-free would crash the process.
