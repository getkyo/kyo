...
- An **IoDriver** (`internal/transport/IoDriver.scala`, the contract) does completion-based I/O: it performs the read internally and completes the caller's `Promise.Unsafe` with the bytes, rather than signalling readiness and letting the caller read. The contract methods are `start`, `awaitRead`, `awaitWritable`, `awaitConnect`, `awaitAccept`, `write`, `cancel`, `closeHandle`, `close`, and `submitEngineOp`, plus `isPeerClosed` (an optional override defaulting to `false`, reporting whether a backpressured connection's peer has closed so the `ReadPump`'s grace timer can reclaim an abandoned connection). Implementations: `PollerIoDriver` (jvm-native; epoll on Linux, kqueue on macOS/BSD), `IoUringDriver` (jvm-native; Linux io_uring), `BlockingReaderDriver` (jvm-native; the regular-file / stdin-from-file fallback for fds that cannot be polled), `NioIoDriver` (JVM NIO selector floor), `JsIoDriver` (JS Node).
...
- `shared/`: the public surface, `PosixTransport`, the poller backends, `HostResolver`, the `TlsProvider` selection logic, and the FFI binding traits whose impls every platform generates.
...
- `jvm-native/`: the native TLS stack shared by JVM and Native but not JS (the unified `NativeSslEngine`, the shared `SslLibBindings` type with its `BoringSslBindings` / `OpenSslBindings` concrete backends, the `SslLibProvider` base, and the `BoringSslProvider` / `SystemOpenSslProvider` providers), and the poll/reap drivers (`PollerIoDriver`, `IoUringDriver`, `BlockingReaderDriver`).
