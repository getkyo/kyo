...
`TransportConfig` is a plain `case class`: it has no per-field setter methods, so tune it with `copy(...)`. Collections on the internal selection surface are `Chunk`, not `Seq`: `IoBackend.select` / `selectAndBuild` and `TlsProvider.selectFor` take `Chunk`, and `Connection.detachForUpgrade` returns `Maybe[Chunk[Span[Byte]]]`.
