...
- An **IoDriver** (`internal/transport/IoDriver.scala`, the contract) does completion-based I/O: it performs the read internally and completes the caller's `Promise.Unsafe` with the bytes, rather than signalling readiness and letting the caller read. The contract methods are `start`, `awaitRead`, `awaitWritable`, `awaitConnect`, `awaitAccept`, `write`, `cancel`, `closeHandle`, `close`, and `submitEngineOp`. Implementations: `PollerIoDriver` (jvm-native; epoll on Linux, kqueue on macOS/BSD), `IoUringDriver` (jvm-native; Linux io_uring), `BlockingReaderDriver` (jvm-native; the regular-file / stdin-from-file fallback for fds that cannot be polled), `NioIoDriver` (JVM NIO selector floor), `JsIoDriver` (JS Node).
