...
private[net] object IoBackendPlatform:
    ...
    /** Build the selected Native transport. Selection honors `-Dkyo.net.backend` (a forced-unavailable name aborts [[kyo.net.NetException]]);
      * with no forced name it walks the priority gradient and builds the first backend that constructs, falling back to the next when a
