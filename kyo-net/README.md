...
That is the whole setup, and it needs no OpenSSL on the machine: the shim compiles to nothing and the library supplies its entry points. The library is staged beside the linked binary, and what a deployment carries is those two files together, the way a JVM application carries its jars; the directory `nativeLink` writes into holds build output besides them, so copy the binary and its `lib<id>.<ext>` rather than the directory. `sbt kyoNativesReport` names each library, the artifact it came from and the directory it was staged in. The same two lines serve the JVM, where they add the per-os-arch classifier jars to the runtime classpath in place of the two `classifier` dependencies above. There they carry the build host's architecture unless `kyoNativesTargets` names another, so an image built on one OS and run on another needs that target named; the report says so.
...
Both live under `target/`, which is where `sbt run` and `sbt test` resolve them from. A deployed application resolves them the same way Node resolves any package, by walking up from the linked output, so `node_modules/@kyo/ffi-native` and `node_modules/koffi` travel with `main.js`. A project that sets its own `jsEnv` keeps it and folds in `kyoNativesNodeEnv`, which carries the `NODE_PATH` the default one gets.
