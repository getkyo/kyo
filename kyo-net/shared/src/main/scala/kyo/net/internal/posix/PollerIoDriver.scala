...
final private[net] class PollerIoDriver private[posix] (
    ...
    // JS only: the cycle task of a chain that parked instead of polling, Absent while the chain runs.
    //
    // On Node the poll is a `koffi.callAsync` request on a libuv worker, and Node does not exit while one is outstanding, so a chain that
    // re-arms with nothing registered keeps an idle application alive forever. JVM and Native park a thread the process owns, which holds
    // nothing open, so they always re-arm.
    //
    ...
    // thread, and the JS scheduler always defers to the macrotask queue, so the store and the read in `triggerWake` cannot interleave.
    ...
    // avoids the Native TLS deadlock: an ephemeral spawned task can be lost under a scheduler strand, leaving the FIFO undrained forever, but the
    // poll loop cannot be stranded that way because it is the one carrier running for the driver's whole life, so a submitted op is always drained within one poll cycle.
    ...
    def awaitRead(handle: PosixHandle, promise: Promise.Unsafe[ReadOutcome, Abort[Closed]])(using AllowUnsafe, Frame): Unit =
        ...
        if terminal.get() then
            val closed = Closed(label, Frame.internal, s"fd=${handle.readFd} driver closed")
        ...
        Maybe(pendingAccepts.remove(fd)) match
            case Present(handle) =>
                ...
                if isStaleId(handle.readFd, handle.id) then
                    ...
                    promise.foreach(_.completeDiscard(Result.fail(Closed(
                        s"listener ${handleLabel(handle)}",
                        handle.createdAt,
                        "stale accept event: fd recycled"
                    ))))
                else
    ...
    /** Send as much of the handle's pending ciphertext tail as the socket buffer accepts, then either reset the tail (fully drained) or arm a
      ...
      * send count it advances `pendingCipherSent` (strictly-positive progress, so the loop terminates), the defining difference from a
      * busy-spin that would retry the same bytes up to 4096 times on EAGAIN. On EAGAIN (or a 0-count send) it BREAKS without
                ...
                Maybe(pendingReads.get(fd)).foreach { h =>
                    if (h.id.packed & 0xffffffffL) == staleId then
                        ...
                        h.pendingReadPromise.getAndSet(Absent).foreach(_.completeDiscard(Result.fail(
                            Closed(s"connection ${handleLabel(h)}", h.createdAt, "stale event: fd recycled")
                        )))
                }
                Maybe(pendingAccepts.get(fd)).foreach { h =>
                    if (h.id.packed & 0xffffffffL) == staleId then
                        ...
                        h.pendingAcceptPromise.foreach(_.completeDiscard(Result.fail(
                            Closed(s"listener ${handleLabel(h)}", h.createdAt, "stale event: fd recycled")
                        )))
                        h.pendingAcceptPromise = Absent
                }
    ...
    private def dispatchError(fd: Int)(using AllowUnsafe, Frame): Unit =
        ...
        val err = soError(fd)
        if err != 0 then
            // A pending SO_ERROR (RST, a failed connect) means the peer/OS killed this connection: a connection I/O failure, surfaced on the
            // read and writable ops as a Closed naming the connection and its creation frame (the read handle from pendingReads, the writable
            // handle carried on the pending entry).
            Maybe(pendingReads.remove(fd)).foreach { h =>
                h.pendingReadPromise.getAndSet(Absent).foreach(_.completeDiscard(Result.fail(
                    Closed(s"connection ${handleLabel(h)}", h.createdAt, s"error event errno=$err")
                )))
            }
            Maybe(pendingWritables.remove(fd)).foreach { entry =>
                entry.promise.completeDiscard(Result.fail(
                    Closed(s"connection ${handleLabel(entry.handle)}", entry.handle.createdAt, s"error event errno=$err")
                ))
            }
    ...
    private def finishDispatch(
        ...
        if handle.endDispatch() then
            promise.completeDiscard(Result.fail(Closed(s"connection ${handleLabel(handle)}", handle.createdAt, "read on closed handle")))
        else
    ...
    private def dispatchReadPlain(
        ...
        if n > 0 then
            ...
            finishDispatch(
                ...
                handle,
                promise,
                Result.succeed(ReadOutcome.Failed(NetConnectionIoException(
                    s"connection ${handleLabel(handle)}",
        ...
        Maybe(pendingWritables.remove(fd)) match
            case Present(entry) =>
                if isStaleId(fd, entry.id) then
                    ...
                    entry.promise.completeDiscard(Result.fail(Closed(
                        s"connection ${handleLabel(entry.handle)}",
                        entry.handle.createdAt,
                        "stale writable event: fd recycled"
                    )))
    ...
    /** Submit an interest change command to the change FIFO and return immediately. The packed `long` command is appended to the unboxed FIFO (no
      ...
      * the drain runs on the always-running poll-loop carrier, never on a per-burst `Fiber.Unsafe.init` drain task that a scheduler strand could
      * lose, leaving the FIFO undrained forever (the Native TLS deadlock). The poll loop cannot be stranded that way, so routing the drain through
      * it makes the delivery self-healing while keeping the offload contract (this returns immediately; the drain runs on the separate poll-loop carrier).
      *
      ...
      * registered with the kernel at all: the poll parks indefinitely (timeoutMs=-1) and never times out on its own. Gating the wake behind a
      * `wakePending` CAS flag (to dedup a burst of submits to at most one wake per poll cycle) would race: that flag is
        ...
        Maybe(takeRegistration(fd, kind)) match
            case Present(reg) =>
                ...
                if handle.isClosing() then
                    ...
                    val closed = Closed(res, handle.createdAt, "register on closing handle")
                    kind match
                        case RegKind.Read =>
                            handle.pendingReadPromise.getAndSet(Absent).foreach(_.completeDiscard(Result.fail(closed)))
                        case RegKind.Accept =>
        ...
        if op == OpRegisterRead then
            ...
            if accept then
                ...
                if id != PollScratch.IdNoCheck then
                    ...
                    if rc < 0 then
                        Maybe(pendingAccepts.remove(fd)).foreach { h =>
                            h.pendingAcceptPromise.foreach(_.completeDiscard(Result.fail(
                                Closed(s"listener ${handleLabel(h)}", h.createdAt, "register failed")
                            )))
                            h.pendingAcceptPromise = Absent
                        }
                    end if
                ...
                if id != PollScratch.IdNoCheck then
                    ...
                    if rc < 0 then
                        Maybe(pendingReads.remove(fd)).foreach { h =>
                            h.pendingReadPromise.getAndSet(Absent).foreach(_.completeDiscard(Result.fail(
                                Closed(s"connection ${handleLabel(h)}", h.createdAt, "register failed")
                            )))
                        }
                    else
            ...
            if id != PollScratch.IdNoCheck then
                ...
                if rc < 0 then
                    Maybe(pendingWritables.remove(fd)).foreach { entry =>
                        entry.promise.completeDiscard(Result.fail(
                            Closed(s"connection ${handleLabel(entry.handle)}", entry.handle.createdAt, "register failed")
                        ))
                    }
                end if
            end if
    ...
    /** Submit a TLS engine op to the engine FIFO and return immediately. The poll-loop carrier runs the thunk in submission order on its next cycle
      ...
      * pure offer with no spawn, for the same reason as [[submitChange]]: the drain runs on the always-running poll loop, never on a
      * `Fiber.Unsafe.init` drain task that a scheduler strand could lose, leaving engine ops undrained forever (the Native TLS deadlock); the poll
      * loop cannot be stranded that way. The recv/send syscalls that surround engine ops stay outside the FIFO.
      *
    ...
    /** Drain both per-driver FIFOs once, on the carrier running the poll cycle. Every cycle calls this exactly once, making it the single
      ...
      * it cannot be lost the way a per-burst `Fiber.Unsafe.init` drain task could be lost by a scheduler strand, leaving the FIFO undrained
      * forever. Draining on the poll-loop carrier (not the submitting carrier) keeps the offload contract: a submit
...
private[net] object PollerIoDriver:
    ...
    /** A pending writable registration: the waiter's promise paired with the arming handle's monotonic id. The id lets `dispatchWritable`
      ...
      */
    final private case class PendingWritable(promise: Promise.Unsafe[Unit, Abort[Closed | NetException]], id: HandleId, handle: PosixHandle)
