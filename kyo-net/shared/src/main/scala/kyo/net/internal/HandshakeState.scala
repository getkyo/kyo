...
/** Named state for one step of a TLS handshake, replacing raw integer codes in the posix engine
  ...
  *
  *   - [[HandshakeState.WantRead]]: engine code `0`, more ciphertext needed from the peer before the next step.
  *   - [[HandshakeState.WantWrite]]: engine code `-1`, ciphertext has been queued; drain and send it, then re-step.
  *   - [[HandshakeState.Done]]: engine code `1`, attach the engine and start the pumps.
  *   - [[HandshakeState.Failed]]: engine code `-2`, the engine returned a fatal-error status.
  */
...
private[kyo] object HandshakeState:
    /** Map a raw integer code from [[kyo.net.internal.TlsEngine.handshakeStep]] to a typed state.
      ...
      * value is treated as [[HandshakeState.WantRead]] (safe: the engine will emit a definite terminal code on the next step).
      */
...
/** Why a handshake failed, carried in [[HandshakeState.Failed]] so the failure is a typed value the
  ...
  *
  *   - [[HandshakeFailure.EngineError]]: the engine returned `-2` (a fatal-error status, normal alert-record outcome).
  *   - [[HandshakeFailure.DeadlineReaped]]: the handshake deadline fired before completion (slowloris guard).
  *   - [[HandshakeFailure.EngineThrew]]: the engine threw an unexpected exception during a handshake step.
  */
