...
final private[kyo] class ReadPump[Handle](
    ...
    private def teardown()(using AllowUnsafe, Frame): Unit =
        ...
        // caller has no way to re-deliver, so those bytes would be lost. `closeAwaitEmpty` marks
        ...
        // the unread stderr frame as the close() backlog, so only the stdout frame is delivered to
