...
/** A pool shutdown never races a driver's own event-loop teardown with an external interrupt. `IoDriverPool.close` calls
  ...
  * reached the code that reclaims a still in-flight handle's fd -- exactly the CLOSE_WAIT leak this test exists to rule out.
  *
        ...
        Sync.defer {
            ...
            assert(
                ...
                s"pool.close() must never interrupt a driver's own loop fiber (racing that interrupt against a driver's carrier-confined " +
                    s"deferred-close drain is the CLOSE_WAIT leak class this test guards), got $seq"
            )
        }
    }
