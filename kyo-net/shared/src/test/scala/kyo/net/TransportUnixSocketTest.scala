...
/** Cross-backend Unix-domain-socket round-trip for the public [[Transport]] surface.
  ...
  * check here. The gate reads it through [[NetPlatform.loadedTransport]], because on Scala.js the backend, and so the answer, exists only once
  * the transport has loaded it.
  */
