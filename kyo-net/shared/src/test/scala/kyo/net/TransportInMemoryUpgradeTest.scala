...
/** A STARTTLS upgrade on a connection that is not socket-backed (the in-memory connection) must abort typed on every backend, not throw. The
  ...
  * documented contract (Connection.inMemory) is that it aborts a `NetException`. Asserted over every registered backend via [[eachBackend]]: the
...
class TransportInMemoryUpgradeTest extends Test:
    ...
    "upgradeToTls on a non-upgradable in-memory connection aborts typed, not thrown" - eachBackend { transport =>
