...
/** Round-robin rotation across the `next()` AtomicLong wrap boundary.
  ...
  * negative, and the old `Math.abs(idx)` could not recover (`Math.abs(Long.MinValue)` is still `Long.MinValue`, an overflow), so the index
  * skewed or went out of bounds exactly at the wrap. The fix masks the sign bit before the modulo. This seeds the pool's counter near
  ...
  * boundary. Pure arithmetic over a seeded counter, no timing. FAILs before the fix (the `Math.abs` path throws ArrayIndexOutOfBounds or skews).
  */
class IoDriverPoolWrapRotationTest extends Test:
    ...
    "the returned slot matches the masked-modulo contract for every counter value across the wrap" in {
        // n = 3 is not a power of two, so the wrap discriminates the fix from the old Math.abs: at counter Long.MinValue the old code returned
        // Math.abs(Long.MinValue % 3) = Math.abs(-2) = slot 2 (a mirror that re-hits a slot just visited), while the fix's (c & MaxValue) % 3 = 0.
        val n       = 3
        ...
        // The fix's exact contract for the k-th call is ((seed + k) & Long.MaxValue) % n. Assert the returned slot (by identity) equals it for
        // every call straddling the wrap. Before the fix the slot at the wrap (k=3, counter Long.MinValue) is the Math.abs mirror (2), not 0.
    ...
    "every returned slot stays in [0, N) across the wrap (no negative / out-of-bounds index)" in {
        ...
        // two) is the discriminating case: at counter Long.MinValue the old (Long.MinValue % 3 = -2).toInt path fed a NEGATIVE index into
        // drivers(Math.abs(idx)); the abs masked it here for n = 3, but the negative intermediate is exactly the latent out-of-bounds the fix
        // removes. This asserts the post-fix invariant directly: every slot is a valid array index.
        val n       = 3
        val drivers = mkDrivers(n)
    ...
    "the first next() at counter Long.MinValue lands on slot 0 (not the Math.abs mirror)" in {
        // n = 3 again so Long.MinValue % 3 = -2 discriminates: the old Math.abs(-2) = slot 2, the fix's (MinValue & MaxValue) % 3 = 0.
        val n       = 3
        val drivers = mkDrivers(n)
