...
object PosixTestSockets:
    ...
    /** Cancel the test if io_uring is unavailable at the PRODUCTION ring depth. See [[UringGate.assumeUring]], which every platform's suite
      * shares so the probe cannot drift from the driver's own init.
      */
