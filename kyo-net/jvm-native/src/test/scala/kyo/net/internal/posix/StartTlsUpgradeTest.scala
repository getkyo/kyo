...
class StartTlsUpgradeTest extends Test:
    ...
    "STARTTLS on a non-upgradable in-memory connection aborts NetNotUpgradableException" in {
        ...
        Sync.ensure(Sync.defer(driver.close())) {
            ...
            Abort.run[NetException | Closed](transport.upgradeToTls(inMem, clientTls, transportConfig.channelCapacity).safe.get).map {
                case Result.Failure(c: NetNotUpgradableException) =>
