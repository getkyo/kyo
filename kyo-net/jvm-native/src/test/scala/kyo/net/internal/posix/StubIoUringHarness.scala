...
/** Ringless stubs for [[IoUringDriver]]: no real io_uring, no kernel, no platform gate, so a driver built over them runs the real reap-cycle
  * code on every platform the suite compiles for rather than being cancelled off Linux.
  *
  * Shared by the tests that drive the driver's own logic (cross-tail send ordering, crash containment). Kept here rather than inside one test
  * so a second test reusing the harness does not fork a second copy that drifts from this one.
  */
// StubSocketBindings: all syscalls are no-ops; only shutdown and close matter at driver teardown.
object StubSocketBindings extends SocketBindings:
...
/** Monitor-based io_uring stub: no real ring, CQE injection via injectRawCqe, send counting.
  ...
  */
class StubIoUringBindings extends IoUringBindings:

    // Unsafe: the barrier promise is a test fixture completed from the stub's own syscall shims, which are the unsafe boundary this harness
    // exists to stand in for; every method below already takes AllowUnsafe, and only this field initializer sits outside one.
    ...
    val throwOnWait: java.util.concurrent.atomic.AtomicBoolean = new java.util.concurrent.atomic.AtomicBoolean(false)

    // Teardown observations. queue_exit releasing the ring is what a crashed cycle must still reach.
    val queueExitCount: AtomicInteger    = new AtomicInteger(0)
    val eventfdCloseCount: AtomicInteger = new AtomicInteger(0)
    ...
    override def kyo_uring_submit_and_wait_timeout(ring: Buffer[Byte], cqePtr: Buffer[Long], timeoutNs: Long)(using
        ...
        if throwOnWait.compareAndSet(true, false) then
            throw new RuntimeException("injected reap wait failure (crash-containment guard)")
        monitor.synchronized {
