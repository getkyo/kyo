...
class DoltFileSystemTest extends Test:
    ...
    private def withFiles[A](f: (Dolt, DoltFileSystem) => A < (Async & Abort[SqlException | FileSystemException] & Scope & DB))(
        ...
        Scope.run {
            SqlClient.init("doltlite://:memory:", SqlConfig(maxConnections = 1)).map { client =>
                DB.run(client) {
                    Dolt.use(dolt => DoltFileSystem.init(dolt).map(files => f(dolt, files)))
                }
            }
        }
    end withFiles
    ...
    "a diff reports which files a branch changed, and how" in {
        withFiles { (dolt, files) =>
            for
                ...
                changed <- dolt.diff(Dolt.Ref.Branch("main"), Dolt.Ref.Branch("work"), "vfs_node")
    ...
    "a merge brings another branch's files onto this one" in {
        withFiles { (dolt, files) =>
            ...
            for
                ...
                _      <- dolt.merge(Dolt.Ref.Branch("feature"))
    ...
    "a large file reads back byte for byte, including a positioned read at its end" in {
        withFiles { (_, files) =>
            // Comfortably past any buffer the hex slicing path might round to, and not a round number.
            val size    = 132306
            ...
            yield
                ...
                assert(back.is(content), "content did not survive the round trip")
                assert(tail.is(content.slice(size - 10, size)), "a positioned read at the end returned the wrong bytes")
            end for
        }
    }

    "content that is not valid UTF-8 survives writing, reading and a positioned read" in {
        withFiles { (_, files) =>
            // The case a text-flavoured path corrupts rather than refuses: an embedded NUL, a lone 0xFF and
            // 0xFE, and a truncated multi-byte sequence. Slicing goes through hex precisely so these survive.
            val content = Span.fromUnsafe(
                Array[Byte](0x41, 0x00, 0xff.toByte, 0xfe.toByte, 0x42, 0x80.toByte, 0x43, 0x00, 0x44, 0xc3.toByte)
            )
            val path = Path("data", "binary.bin")
            for
                _     <- files.writeBytes(path, content, options)
                back  <- files.readBytes(path)
            ...
            yield
                assert(back.is(content), "binary content did not survive the round trip")
                assert(slice.is(content.slice(2, 6)), "a positioned read into binary returned the wrong bytes")
            end for
        }
    }
