...
/** A positioned channel over a stored file. Holds no content: the channel contract, unlike the handle contract,
  * carries the backend's effect in its own signatures, so every read and write is a query that slices the stored
  * value at the offset it wants. The `open` flag is what makes an operation after scope exit fail rather than quietly keep working against
