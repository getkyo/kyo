...
Compaction is consulted at one seam between the context read and request assembly, on both the `gen` and `stream` paths. Below the occupancy trigger (the effective high watermark, `min(highWatermark * window, contextCeiling)`), nothing changes: the fast path re-serves the current view as the same value, and the request stays cache-warm. Once occupancy crosses the trigger, a boundary render runs: units are ranked by relevance and demoted along a four-level detail ladder (verbatim, summary, terse, pointer) in ascending relevance order until the view fits the lower target occupancy (the hysteresis band between the low and high watermarks). A demoted span can carry an extractive summary produced by a cheap-tier fill model; below a size threshold a demoted unit renders as a short, mechanically assembled marker instead. Occupancy anchors on the provider's own reported token total once one is available.

Once occupancy crosses the hard window fraction (`hardLimit`, a fraction of the model window less the output reservation), a deterministic forced path takes over: it omits the least-live non-root units. If even the unshrinkable roots (the system head, the first and latest user turn, any unresolved tool-call pair, and the recent tail) cannot fit, `render` aborts with `AIContextOverflowException` rather than send an over-limit request to the provider.
...
The shipped compactor ranks units structurally, from message adjacency and identifier references, optionally augmented by dependency and relatedness edges a lightweight model analysis pass emits. There is no embeddings layer anywhere in the module.
...
Tuning lives on the ambient `kyo.ai.Config`, in a grouped `compaction` record adjusted through `config.compaction(_.highWatermark(...))`. The knobs: `highWatermark` (the boundary trigger, a fraction of the model window), `contextCeiling` (an absolute clamp on that trigger, with `noContextCeiling` to drop it), `lowWatermark` (the render-down target, a fraction of the effective high), `prepareWatermark` (where speculative compaction arms, `1.0` to turn it off), `hardLimit` (the overflow backstop, a fraction of the window less the output reservation), `driftThreshold` (a relevance trigger, with `noDriftThreshold` for size-only), `summarizer` (a pinned `Config` for the summary fill; unset uses the provider's cheap tier degraded), and `rawRetentionCap` (the raw-transcript memory backstop). The occupancy axis these derive is validated at `Config` construction, so an override that reorders it fails with the violated inequality named, never at a boundary. The token accountant is set with `config.tokenizer(...)`, taking a `kyo.ai.Tokenizer` (defaulting to a bundled offline tiktoken tokenizer), and cleared with `noTokenizer`. The remaining ranking and graph constants (edge weights, seed shares, presentation thresholds) are private constants of the default compactor, documented at their point of use in source, never public config.
...
Each provider also exposes a `.small` catalog entry, its cheap tier. The compactor's summary fill route uses it as the degraded default model when `compaction.summarizer` is unset, and it is generally useful for cost-sensitive sub-generations.
...
The compaction design is validated by a replay-based test that scores the real `Compactor.render` path against a simple baseline over synthetic sessions on deterministic metrics (task success, token cost, re-fetch rate); see `CompactorReplayTest.scala`.
