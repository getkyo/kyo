...
- `Prompt`, `Tool`, `Thought`, `Mode`, `Compactor`, `Observe`, the composable enablement kinds (`AI.Enablement`).
...
`Tool.init[In][Out, S]` builds a tool from a name, optional description, prompt, and run function, and says NOTHING about compaction ([`Tool.scala:59-70`]). Supersession is a separate constructor, `Tool.initSuperseding`, taking a `Tool.Supersession(kind, key)` ([`Tool.scala:86-97`]); both delegate to one private canonical `build` so the `Info` shape is written once ([`Tool.scala:99-118`]). `Tool.Kind` is a closed two-case enum, `Read` and `Write` ([`Tool.scala:35-36`]). The kind and the key travel as ONE value because a kind without a key decides nothing: the kind is consulted only for a call that produced a key, so `Info.supersessionKeyFor` returns the pair or `Absent` and a caller cannot pick up one without the other ([`Tool.scala:52`]). A tool built through `init` never supersedes and is never superseded. `ToolTest` pins that a plain tool carries no declaration ([`ToolTest.scala:269-277`]), that the declaration reaches `Info` and reads back as a pair ([`ToolTest.scala:279-293`]), that an undecodable argument payload yields `Absent` rather than failing ([`ToolTest.scala:295-306`]), that an extractor declining one call keeps the opt-in per declaration and the key per call ([`ToolTest.scala:308-320`]), and the closed two-case enum ([`ToolTest.scala:322-326`]). The internal `resultTool` is the dynamic `result_tool` the eval loop adds to every request: its run is a no-op, and the eval loop extracts the call's arguments directly (no capturing run, no ref); it is built through plain `init`, so the result tool never supersedes ([`Tool.scala:236-274`], [`LLM.scala:557`]). Tool-call dispatch contains ANY throw from user code as a tool message and never lets it escape the eval loop: `decodeAndRun`'s `Abort.run[Throwable]` wraps the run body ([`Tool.scala:113-121`]), and `handle` routes a run-body failure to the generic tool-failure message, distinct from the schema-repair message reserved for a malformed-arguments decode failure ([`Tool.scala:152-207`]).
...
trait Compactor[-S] extends AI.Enablement[S]:
    ...
    def compact(ctx: Context, state: State)(using Frame): Compactor.Decision < (LLM & Async & Abort[AIGenException] & S)
    def tools(ai: AI)(using Frame): Chunk[Tool[LLM]] = Chunk.empty
    def release(state: State)(using Frame): Unit < Sync = Kyo.unit
```

The boundary is deliberately this small: ONE decision method, one opaque state cell, and two defaulted hooks. A compactor that needs no ephemeral state sets `type State = Unit` and inherits everything else.

`compact` takes the WHOLE `Context` (so it can read `raw` for grouping and scoring) and answers with a `Decision` rather than a view, which is what makes byte-stability structural: `Decision.Unchanged` has no channel through which to return modified bytes, so "the served bytes do not move between boundaries" cannot be violated by a buggy strategy. `Decision.Compacted(updated)` does carry a whole `Context`, so `raw` is protected by a CHECK rather than by the type: the seam verifies `raw` is preserved (same length, each entry identical or replaced by an origin-carrying marker) and aborts the turn with `AICompactionException` otherwise. See "The LLM seam" below.

`State` is a per-compactor TYPE MEMBER, not a type parameter, so the trait stays usable as `Compactor[Any]` in the env slot while each implementation keeps its own state type. The framework never inspects it: it calls `initState` once per session, threads the value back into every `compact`, and calls `release` at teardown. It is carried as a dependent `Seated` pair so no cast is needed to reunite a state with its compactor.

`init` is a NULLARY, side-effect-free accessor, `def init: Compactor[Any] = Default` ([`Compactor.scala:103`]), and `object Default extends Default(Tuning(), Calibration())` so it returns a shared singleton by reference. Allocation moved to `initState`, which is why `init` is still not `< Sync` the way `AI.init`/`Config.default` are.

### The boundary rule: what the rest of kyo-ai may know about compaction

Compaction previously leaked across the module: strategy-shaped fields sat on types that are not about compaction, and the seam held a trigger that only one implementation's policy justified. Both are fixed, and this rule is what keeps them fixed.

**Outside the compaction sources, kyo-ai may know exactly these compaction facts and no others:**

1. `Compactor[-S]` itself, with `Decision`, `Tuning`, `Mechanism`, and the `State` type member.
2. `Context`'s persisted compaction cell, reached only as an opaque whole.
3. `AISession`'s ephemeral state handle, likewise opaque.
4. `Tool`'s supersession opt-in (`Tool.initSuperseding`).
5. `Message.origin`, which is how a demoted region stays addressable.
6. One usage hook, so occupancy can anchor on provider-reported input tokens.

**The test for a new field is: would a DIFFERENT compaction strategy need it?** If the answer is no, it belongs inside the strategy, not on a shared type. A watermark, a level, a span, a score, a weight, and a summary budget all fail that test: they are the default strategy's vocabulary. A compactor that ranked by embeddings, or truncated, or asked the model what to drop, would use none of them and must not be forced to carry them.

Two consequences worth stating outright, because both were violated before:

- **The seam holds no trigger.** It does not read a watermark, compare an occupancy, or decide that a boundary is due. It calls `compact` on every turn and the strategy decides, which is the only arrangement under which "when to compact" is genuinely the strategy's choice.
- **No compaction knob goes on `kyo.ai.Config`.** Everything a caller sets lives on `Compactor.Tuning`; everything measured lives in `Compactor.internal.Calibration` with no public accessor.

Adding a compaction-shaped field to `Config`, `Context`, `AISession`, `Message`, or `Tool` is a design-surface change. Raise it; do not slip it in.

`Compactor` composes with the other four enablement kinds exactly the same way, including the sanctioned erased-carrier discharge every kind uses (`Mode.scala` has the identical `this.asInstanceOf[Mode[Any]]` pattern): because `Compactor` is contravariant in `S`, widening a user's `Compactor[S]` to the `AIEnv`/`AISession` slot's `Compactor[Any]` needs one cast, applied at the enablement boundary, never inside `compact` itself:
...
`raw` is append-only under compaction, with two narrow exceptions, and it is worth being exact about how that is enforced now. A `Compactor` answers with a whole `Context`, so the type no longer excludes a `raw` edit; the seam CHECKS it instead, requiring the same length and each entry either identical or replaced by an origin-carrying marker, and aborting the turn with `AICompactionException` otherwise. Within the default compactor, eviction is the only step that writes `raw` at all, replacing a forgotten run's messages in place with a coarse head plus content-freed tombstones that all carry the band's `Origin` (which is exactly the shape the check admits). The second exception, documented as such and never a precedent to extend casually: tool dispatch reconciles the transient `"Processing tool call: <name>"` placeholder in the tail of BOTH lists symmetrically, replacing it with the real result (or, on a decode failure, removing it and the failing call from the assistant message) ([`kyo/ai/Context.scala:9-12`], [`Tool.scala:152-207`], specifically the three `ctx.copy(raw = ..., compacted = ...)` sites at [`Tool.scala:165-183`], [`Tool.scala:185-191`], [`Tool.scala:192-204`]). No other path rewrites `raw`.
...
### `Compactor.Tuning` and the `Tokenizer` seam

Tuning lives on the COMPACTOR, not on the ambient `kyo.ai.Config`. It used to be a `Config.Compaction`
record the seam read before any compactor was involved, which meant the framework held a policy that
was not its own and a custom compactor could not change when it ran. Ten fields, all policy:

```scala
final case class Tuning(
    mechanisms: Set[Mechanism] = Mechanism.all,    // the disable set; see below
    keepShare: Double = 1.35,                      // verbatim floor, as a multiple of the uniform share 1/N
    summaryOutputCap: Int = 512,                   // token ceiling on one fill's output
    trigger: Double = 0.5,                         // boundary trigger: fraction of the window
    contextCeiling: Maybe[Int] = Present(128_000), // absolute clamp on the trigger; Absent = pure fraction
    target: Double = 0.6,                          // render-down depth: fraction of the effective trigger
    prepare: Double = 0.8,                         // prepare line; 1.0 = no speculative compaction
    hardLimit: Double = 0.9,                       // overflow backstop vs window - output reservation
    summarizer: Maybe[Config] = Absent,            // Absent = warm route degraded; Present = pinned fills
    rawRetentionCap: Maybe[Int] = Absent           // Absent = several window-widths; raw-memory backstop
)
```

- **The fraction ordering validates at `Compactor.init(tuning)`; the projection does not.** `Tuning.validated` requires
  `target < prepare <= 1.0` and `hardLimit > 0`, which are properties of the numbers alone. The rest of
  the axis is a projection onto a window this record does not know, and one `Tuning` serves many models,
  so `Compactor.internal.axis(tuning, config)` computes `high`/`low`/`prepare`/`hard` where they are used.
  The projection CLAMPS rather than refusing: it raises the prepare line to the first integer above the
  target when truncation collapses two distinct fractions onto one, and falls back to the conservative
  output reservation when a model's declared maximum would reorder the axis.
- **`hardLimit` is a fraction of the model window less the once-counted output reservation**, not of the
  trigger, so it can never be built away to disable the forced-path guard.
- **`keepShare` is a SHARE, not a score.** Liveness is PageRank over a seed vector summing to 1.0, so
  scores are shares of one unit of mass over N regions and the mean is `1/N`. An absolute floor is
  arithmetically certain to exceed every typical score once N passes its reciprocal, which closed the
  verbatim-pin channel on any long session. The floor also does NOT read the budget: pinning asks whether
  content is live, a property of the content, while the budget is pass 2's job.
- **`mechanisms` is the disable set**, replacing five scattered booleans and covering three mechanisms
  that previously had no lever at all (`Preparation` was reachable only by setting a watermark to 1.0,
  `ContentReferences` only by zeroing a weight while its whole admission scan still ran, `Recall` by
  nothing). Each member's guard is scoped to its own work, never to "no calls and no fibers": a fired
  boundary legitimately forks for whatever remains enabled.
- **`summarizer` selects the summary fill model.** `Absent` runs the degraded warm route on
  `provider.small`; `Present` pins a fill `Config`.

The measured constants behind the ranking (edge weights, seed shares, admission gates, presentation byte
budgets) are `Compactor.internal.Calibration`, deliberately not on this surface: they are properties of
one ranking design rather than choices, and an ablation reaches them through `internal`. The escape hatch
for different mechanics is implementing `Compactor`, never a further knob.
...
7. **Forced path.** When `project`'s result still exceeds `hardLimitTokens`, a deterministic `forced` omit pass runs over non-root candidates; if even the unshrinkable roots cannot fit, the projection aborts with `AIContextOverflowException` rather than send an over-limit request ([`Compactor.scala:474-480`], [`Compactor.scala:1131`], [`AIException.scala:198`], [`CompactorTest.scala:466-482`]).

**Recall.** The default contributes exactly ONE tool, `recall(id)`, bound to the calling instance so it resolves against only that instance's own `raw`, never another session's ([`Compactor.scala:485`], [`CompactorTest.scala:486-525`]). Each recall is recorded in `Compaction.State` stamped with the boundary counter, contributing a geometrically decaying seed to its region so a re-fetched region climbs back toward the tail ([`Compactor.scala:709-713`]).

**The summarizer and analysis passes** are what make a boundary's inputs richer over time; they run as background fibers armed below a boundary, not inside the `compact` call that later consumes their results. The `Summary` level's bytes come from a cheap-tier fill generation (`provider.small` degraded, or `compaction.summarizer` when pinned) written once per span into `Compaction.State.summaries` ([`Compactor.scala:1414-1421`], [`kyo/ai/Compaction.scala`]). A per-region analysis generation emits `DependsOn`/`Relates`/`Supersedes` relations, frozen write-once by ordinal into `Compaction.State.analyses`, feeding the `Dependency`/`Relatedness` edges and the analyzed supersession ([`kyo/ai/Compaction.scala`]). Both are model calls; both are decoded through `Schema` over model-controlled output, so a malformed member routes to a typed drop, never a throw.
...
- **`Present(c)` (any compactor).** ONE dispatch, not a branch on which strategy it is. The seam seats
  the session's compaction state on first use, hands the context and that state to `compact`, and
  installs what comes back. `Decision.Unchanged` re-serves the input BY REFERENCE, so the served bytes
  are stable between boundaries and the provider prompt cache survives; that is structural, since a
  compactor reporting `Unchanged` has no channel through which to return a modified view.
  `Decision.Compacted(updated)` installs the returned context after the one check a returned `Context`
  can fail: `raw` preserved, each entry identical or an origin-carrying marker, length unchanged. A
  violation aborts the turn with `AICompactionException` rather than continuing against a corrupted
  transcript. The seam holds NO trigger of its own, so when a boundary fires is the strategy's decision.
- **`Absent`.** No compactor: `Kyo.lift(ctx)`, byte-identical to the default-off path.

`Compactor.internal.occupancy(ctx)` and `activeTokenizer(config)` supply the occupancy read ([`Compactor.scala:236`], [`Compactor.scala:297`]). Below the trigger, `ctx` is returned literally unchanged, so two consecutive below-trigger requests are byte-identical because they carry the exact same value, not merely an equal one. At a boundary, `compact` answers `Decision.Compacted` with a rebuilt `compacted`, and the seam admits it only after the raw-preservation check above. Both `eval` and `streamAgainst` resolve the compactor with the same instance-over-scope precedence `genLoop` applies to config (`session.env.compactor.orElse(scopeEnv.compactor)`), so a named instance compacts identically on `ai.gen` and `ai.stream`. Every provider backend (HTTP and command-harness) builds its outbound request over `ctx.compacted`, never `raw`.

`LLMTest` pins the seam is a true no-op on the Absent path against a committed golden request byte string ([`LLMTest.scala:674-695`]), that it mints no new `Op` and never shrinks the `raw` slot ([`LLMTest.scala:697-738`]), the instance-over-scope precedence both in isolation ([`LLMTest.scala:741-790`]) and end to end against a live outbound request ([`LLMTest.scala:792-828`]). `CompactorTest` pins that the boundary projection is pure: two passes over identical frozen inputs produce a byte-identical view, with no wall-clock or seed nondeterminism ([`CompactorTest.scala:78`]), and that span formation is likewise seed-free, two formations of the same content agreeing on member sets and ranges ([`CompactorTest.scala:495`]). Region bookkeeping is re-derived rather than cached, so feeding a pass's own output back as `compacted` reproduces the same decisions.

**What actually guarantees the hard limit, and what does not.** The limit is tested in exactly one place, inside the boundary projection, where a rendered view over `hard` is re-rendered by the forced path and aborts with `AIContextOverflowException` if even that will not fit ([`Compactor.scala:419-425`]). Below the trigger no projection runs, so nothing is tested at all, and the limit is unreachable there only because the axis puts `high` strictly below `hard` ([`Model.scala:39-56`]), which `CompactorTest` pins along with the rest of the ordering `low < prepare <= high < hard`, including the branch where a model's declared output maximum would otherwise reorder it ([`CompactorTest.scala:1434`], [`CompactorTest.scala:1492`]). The guarantee is those two facts together, not either alone: every generation consults `compact`, and the occupancy that gates the trigger is read over `ctx.compacted`, the same list that gets served ([`Model.scala:335-352`]), so a view that has not crossed `high` is under `hard` without being measured, and one that has crossed it is measured. Content appended between consults is caught by the next consult's occupancy read; the compactor cannot itself grow the view between boundaries, since `Unchanged` re-serves by reference.

What is bounded is therefore the SERVED VIEW, not the outbound request. The result directive and `Prompt.internal.enrichedContext` are added after the render and are deliberately outside the anchor basis, so they are neither counted by the occupancy that gated the trigger nor seen by the hard check ([`LLM.scala:271-280`]); they ride in the headroom `hardLimit` leaves plus the once-counted output reservation. Anything that adds to the view after a consult has passed inherits that headroom and nothing else, and it breaks the limit silently, because there is no later check to fail.
...
- **Compaction.** Nothing. Compaction policy lives on `Compactor.Tuning`, not on `Config`; the config keeps only the separate `tokenizer(Tokenizer)` / `noTokenizer` and the model facts the projection reads. See [`Compactor.Tuning`](#compactortuning-and-the-tokenizer-seam) above.
...
`Completion.Reply(messages: Chunk[Message], usage: Maybe[Usage])` widens the bare-`Chunk[Message]` result with provider-reported usage; `Completion.Usage(inputTokens, outputTokens, cachedInputTokens)` carries the counts ([`kyo/ai/completion/Completion.scala:199-211`]). Every HTTP backend decodes and populates `usage` on every reply. The compaction seam READS it: occupancy anchors on the provider's own reported input total once one is available (`Compaction.State.withUsage`), rather than trusting the offline tokenizer estimate alone. `usage`/`cachedInputTokens` stay `Absent` when a backend does not report them; both fields are also a public, tested surface for external telemetry.
...
Self-contained demo command shape, one per provider, assuming API keys and command harness auth
...
The same command shape runs `demo.ToolCallDemo` for tool calling and `demo.StreamingDemo` for both
streaming modes.
...
This module has no `AllowUnsafe` sites and no `import AllowUnsafe.embrace.danger` in its sources. Ids come from the run's threaded `State.nextId`, never a process-global counter ([`LLM.scala:102-107`]), so there is no module-level mutable state to bridge. The only `java.lang.ref` use is `internal.AIRef extends WeakReference[AI]`, a GC-reclaimability mechanism for `LLM.State.instances` keys, not an unsafe-tier API ([`LLM.scala:629-640`]). A `Compactor` INSTANCE holds no mutable field: `Default` carries only its `Tuning` and `Calibration`, and the projection is a deterministic function of the `Context`, the active `Config`, and the state handed to it. Bookkeeping lives in exactly two declared places, never a hidden cell: persisted state on `Compaction.State`, which rides the `Context` and survives a snapshot, and ephemeral per-session state in the compactor's own opaque `State`, which the framework allocates via `initState`, threads into every `compact`, and drops at `release`. That cell is interior-mutable by design (it is how a background fill fiber publishes its result), but it is reached only through `Async` primitives, so there is nothing to bridge with `AllowUnsafe` here either. When threading new state, put it in one of those two places, in `LLM.State`, or in the `Op` GADT; do not introduce a process-global counter, an `AllowUnsafe` parameter, or a new stored cache the transcript alone could reproduce.

## The public-surface lock: what's locked, what stays `private[kyo]`

What IS public: the enablement itself (`Compactor[-S]`, `Compactor.init`, `Compactor.none`, and the boundary `compact`/`release`/`tools` with its `Compactor.State` type member, `Compactor.Decision`, `Compactor.Tuning` and `Compactor.Mechanism`), the token accountant (`kyo.ai.Tokenizer`, `Tokenizer.tiktoken`, `Tokenizer.Encoding`, `Config.tokenizer`/`noTokenizer`), the two-list `Context(raw, compacted)` shape and `Context.Origin`, the `Message` enrichment fields (`tokens`, `origin`; `Context.TokenStamp` is `private[kyo]`), the env carrier (`AIEnv.compactor`), the cheap-tier selector (`Config.Provider.small`), the tool supersession opt-in (`Tool.initSuperseding`, `Tool.Supersession`, `Tool.Kind`), usage capture (`Completion.Usage`, `Completion.Reply`), and the compaction `AIException` leaf `AIContextOverflowException`. What stays `private[kyo]`: everything `Compactor.internal` carries (`group`, `deriveGraph`, `score`, `cut`, `forced`, `project`, `occupancy`, `activeTokenizer`, `evict`, `demotedOrigins`, the `Region`/`Span`/`Graph`/`Edge`/`EdgeKind`/`Level`/`Default` types, and the internal tuning constants) ([`Compactor.scala:75-116`]); `Compaction.State`, `Context.coreEq` (used only by `merge` and the default `Compactor`, not a lock symbol a custom `Compactor` is obligated to honor). There is no embeddings surface anywhere in the module. There is no `Compactor.Config` type, and no compaction knob on `kyo.ai.Config` at all: everything beyond `Compactor.Tuning`'s ten fields is a calibrated constant in `Compactor.internal.Calibration` with no public accessor. A change that promotes one of these internals to a public return type is a design-surface change: raise it, do not slip it in. `Compactor.init` (nullary) and `Compactor.init(Tuning)` are the only constructor surfaces.
...
([`LLMTest.scala:18-30`]). The eval loop always extracts from `result_tool`, so script the structured reply by wrapping it (`{"resultValue": <value>}` for a plain field, with `openingThoughts`/`closingThoughts` keys when thoughts are enabled) ([`LLMTest.scala:28-30`], [`Thought.scala:142-143`]). `CompactorTest`'s own `renderWith` helper drives the default compactor's boundary under `LLM.run(config)` with NO server at all, since the projection is model-free ([`CompactorTest.scala:53`]).
...
Never widen the effect row of a `< LLM` computation to include `Async`; `Async` belongs only on `LLM.run`'s residual (the `Gen`/`Stream` interpretation) ([`LLM.scala:125-142`]). A new op goes in `LLM.internal.Op`, gets a `runWith` arm that introduces no `Async` for any op but `Gen`/`Stream`, and is suspended via `ArrowEffect.suspend` ([`LLM.scala:409-432`]). Compaction adds NO new op; a new compaction-adjacent capability should extend `Compactor`'s own `compact`/`tools` methods (still `< (LLM & Async & Abort[AIGenException] & S)` at most, matching `compact`'s own row) rather than mint an `Op`, unless it genuinely needs to thread through `LLM.State` itself. A `Mode.apply`'s `gen` parameter row is exactly `Maybe[A] < (LLM & Async & Abort[AIGenException])`, not wider ([`Mode.scala:20-22`]).
...
11. **New `Observe` fire site or `AIStats` field.** Does the site fire once per resolved completion call, before any adjudication that can abort the turn? Does a new `AIStats` field follow the subset convention (shared suffix, `Maybe` when a wire may not report it)? Do all four backends map their wire's fields, with a fixture test each? [`LLM.scala:709-715`, `AIStats.scala:15-27`]
