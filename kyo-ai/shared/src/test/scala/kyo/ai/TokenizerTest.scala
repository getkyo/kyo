...
import kyo.ai.tokenizer.O200kRanks
...
class TokenizerTest extends kyo.test.Test[Any]:
    ...
    private def tiktokenSources: Chunk[(String, String)] =
        // Paths rather than bare names: the generated rank tables live in the tokenizer impl package
        // while the trait stays in kyo.ai, so one directory no longer covers all three.
        val paths = Chunk(
            "kyo/ai/Tokenizer.scala",
            "kyo/ai/tokenizer/O200kRanks.scala",
            "kyo/ai/tokenizer/Cl100kRanks.scala"
        )
        paths.map(path => (path.substring(path.lastIndexOf('/') + 1), readModuleSource(path)))
    end tiktokenSources

    private def readModuleSource(fileName: String): String =
        val relative = s"shared/src/main/scala/$fileName"
