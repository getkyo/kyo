...
class ConfigTest extends kyo.test.Test[Any]:
    ...
    "an over-large pin on a stand-in maximum reserves the conservative default so the axis stays constructible" in {
        ...
        val standIn = Config.Anthropic.default
            ...
            .maxTokens(999999)
        assert(
            standIn.effectiveMaxOutput == Config.defaultMaxOutputReservation,
            s"a stand-in maximum drives the conservative reservation, got ${standIn.effectiveMaxOutput}"
        )
        assert(
            standIn.effectiveHigh < standIn.hardLimitTokens,
            s"the axis stays coherent: effectiveHigh ${standIn.effectiveHigh} < hardLimit ${standIn.hardLimitTokens}"
        )
    }
