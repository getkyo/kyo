...
/** The default compactor's core pipeline, end to end and step by step.
  *
  * This is the 1:1 file for `Compactor.scala` and the widest of the compaction suites, because the pipeline
  * has to be pinned at every joint: the boundary's determinism and the below-trigger `Unchanged`, occupancy
  * and its usage anchor, region fusion, edge derivation and propagation, span formation and its caps, and
  * the three detail levels with the two demotion passes. The sibling files each take one narrower question
  * (`CompactorAblationTest` per-mechanism effects, `CompactorLivenessTest` whether being referenced is what
  * keeps content verbatim, `CompactorPreparationTest` the background fiber and its validity gate,
  * `CompactorConformanceTest` the properties ANY compactor must satisfy).
  *
  * Two conventions run through the file. Assertions are on CONCRETE values, so a test states the number or
  * the level it expects rather than that something changed. And the pipeline is exercised at the grain the
  * production code uses: `group`, `formSpans`, `deriveGraph`, `score`, `cut` and `project` are called
  * directly with frozen inputs, which is what makes these deterministic and free of any wire, while the
  * cross-turn behaviour that only appears through the serving seam belongs in `CompactorContractTest`.
  */
class CompactorTest extends kyo.test.Test[Any]:
    ...
    def renderWith(ctx: Context, config: Config, compactor: Compactor[Any] = Compactor.init)(using
        ...
        LLM.run(config) {
            compactor.initState.map { st =>
                compactor.compact(ctx, st).map {
                    case Compactor.Decision.Unchanged          => ctx.compacted
                    case Compactor.Decision.Compacted(updated) => updated.compacted
                }
            }
        }
    ...
    "below the trigger the compactor reports Unchanged, so the seam re-serves the identical Context" in {
        // The property is unchanged; its observable moved. It used to count calls to `render`, because the
        // seam decided when to call one. The compactor owns the trigger now, so `compact` runs every turn
        // by design and a call count would only measure that. What must be zero is the number of times it
        // decides to COMPACT, which is what actually rewrites the prefix and invalidates the cache.
        AtomicInt.init(0).map { rewrites =>
            val probe = new Compactor[Any]:
                // Delegates to the default, which is the shape a wrapper takes: it keeps the default's own
                // state as its own, so the delegation is one line and the wrapper adds no cell of its own.
                type State = Preparation
                def initState(using Frame): Preparation < Sync = Preparation.init
                def compact(c: Context, state: Preparation)(using Frame): Compactor.Decision < (LLM & Async & Abort[AIGenException]) =
                    Default.compact(c, state).map {
                        case d @ Compactor.Decision.Compacted(_) => rewrites.incrementAndGet.andThen(d)
                        case d                                   => Kyo.lift(d)
                    }
            ...
            TestCompletionServer.run { server =>
                server.enqueueBody(genBody("1")).andThen(server.enqueueBody(genBody("2"))).andThen {
                    LLM.run(config.apiUrl(server.baseUrl)) {
                        ...
                        rewrites.get.map { count =>
                            // Nothing is rewritten below the trigger, so the served view carries through
                            // untouched (an immutable Chunk re-served is eq).
                            val served1 = small.compacted
                            val served2 = small.compacted
                            assert(count == 0, s"the compactor must rewrite ZERO times below the trigger, got $count")
                            assert(occupancy(small) < axisOf(config).high, "REGIME: the probe context sits below the trigger")
    ...
    "the summary slot is write-once (a second write is discarded)" in {
        val state = Compaction.State().withSummary(3, 7, "first").withSummary(3, 7, "second")
    ...
    "the keep floor is a SHARE of the uniform liveness, and does not read the budget" in {
        // Two properties, and the second is what lets one constant work at every occupancy.
        //
        // SCALE. Liveness is Personalized PageRank over a seed vector summing to 1.0, so a score is a share
        // of one unit of mass spread over N regions. An absolute floor is arithmetically certain to exceed
        // every typical score once N passes roughly its reciprocal, which closed the verbatim-pin channel
        // on any long session.
        //
        // BUDGET-FREE. Pinning answers "is this content live", which is a property of the content; the
        // budget is pass 2's job and pass 2 already descends levels until the view fits. An earlier form
        // scaled this floor by pressure as well, and then no single constant could hold: a value that
        // pinned a referenced region at pressure 2 demoted it at pressure 4, which is the same failure the
        // share form exists to remove, with pressure as the driver instead of N.
        assert(eps(keepFloor(10), keepShare / 10.0), "the floor is the keep share over the region count")
        assert(eps(keepFloor(100), keepShare / 100.0), "and it tracks N, so a longer session does not close the channel")
        assert(eps(keepFloor(0), keepShare), "a degenerate zero-region context reads the bare share rather than dividing by zero")
        // The property the absolute floor could not have, in both directions: whether a region pins depends
        // only on the MULTIPLE of the uniform share it holds, identically at every session length.
        assert(
            List(2, 10, 50, 200, 1000).forall(n => (keepShare + 0.1) / n >= keepFloor(n)),
            "a region above the keep share of the uniform mass is pinned at every session length"
        )
        assert(
            List(2, 10, 50, 200, 1000).forall(n => (keepShare - 0.1) / n < keepFloor(n)),
            "and one below it is demotable at every session length, so the rule is scale-free in both directions"
        )
    }
            ...
            server.enqueueBody(genBody("1")).andThen {
                LLM.run(config) {
                    ...
                    server.captured.map { cap =>
                        assert(occupancy(ctx) >= axisOf(config).high, "the seeded context sits above the boundary trigger")
                        assert(
                            cap.size == 1,
    ...
    "a custom Compactor's own render drives the served view, bypassing the Default pipeline" in {
        ...
        // preparation/eviction machinery, so no summarizer/analysis completion fires.
        AtomicInt.init(0).map { renders =>
            ...
            val custom = new Compactor[Any]:
                ...
                def compact(c: Context, state: Unit)(using Frame): Compactor.Decision < (LLM & Async & Abort[AIGenException]) =
                    renders.incrementAndGet.andThen(
                        Kyo.lift(Compactor.Decision.Compacted(c.copy(compacted = Chunk[Message](UserMessage(sentinel, Absent)))))
                    )
            val ctx = demotable()
            TestCompletionServer.run { server =>
                val base   = cfg(16384).apiUrl(server.baseUrl)
    ...
    "the summarizer route knob selects warm / provider.small / pinned" in {
        ...
        assert(Compactor.Tuning().summarizer.isEmpty, "the default summarizer knob is Absent (the warm route)")
        ...
        val set    = Compactor.Tuning(summarizer = Present(pinned))
        assert(set.summarizer.exists(_ eq pinned), "Present(pinnedConfig) resolves to the pinned fill model")
    }
    ...
    "occupancy = last reported total + offline suffix estimate" in {
        ...
        val ctx    = Context(msgs).withCompaction(Compaction.State(lastUsage = Present(50000), lastUsageRawSize = 8))
    ...
    "the next reported total replaces the suffix estimate" in {
        ...
        val first  = Context(msgs).withCompaction(Compaction.State(lastUsage = Present(50000), lastUsageRawSize = 8))
    ...
    "a mid-session provider switch re-anchors in the new provider's units" in {
        val msgs      = Chunk.from((0 until 10).map(i => sm(s"message number $i")))
        val anchoredA = Context(msgs).withCompaction(Compaction.State(lastUsage = Present(50000), lastUsageRawSize = 8))
    ...
    "maxOutputTokens is counted once, on the hard-limit side only" in {
        ...
        assert(axisOf(base).hard == (0.9 * (200000 - 10000)).toInt, "hard == 0.9*(window - maxTokens) == 171000")
        assert(axisOf(base).hard == 171000, s"the hard-limit bound is 171000, got ${axisOf(base).hard}")
        ...
        assert(axisOf(more).hard < axisOf(base).hard, "a larger output reservation lowers only the hard-limit bound")
    }
    ...
    "the boundary's threaded facts are invariant under adoption, which is what makes threading them exact" in {
        // A fired boundary runs its decision pipeline twice in the foreground, and the second run now
        // reuses the transcript-derived half of the first instead of recomputing it. That is a pure
        // removal of repeated work ONLY while every threaded field is blind to what adoption writes
        // between the two runs, which is summaries and analyses and nothing else. Nothing else pins
        // that: a later edit teaching, say, the seed vector to read summaries would silently turn the
        // reuse into stale input, and every existing test would still pass because they each run the
        // pipeline once. This asserts the property directly, over facts made non-degenerate on every
        // field: several regions of real text (the token index), a recall record and a boundary count
        // (the seed), a demotion marker in the served view (the previous levels), and a usage anchor
        // (the occupancy).
        val raw = Chunk[Message](
            tok(sm("system prompt"), 5),
            tok(um("investigate the poolMax setting"), 7),
            tok(am("the poolMax config is 8"), 9),
            tok(am("the second line mentions poolMax again"), 11),
            tok(am("an unrelated fourth line"), 13),
            tok(am("a fifth line about retry budget"), 15)
        )
        val compacted = raw.take(3).append(demotionMarker(3, 4, raw.size)).concat(raw.drop(4))
        val state = Compaction.State()
            .tickBoundary.tickBoundary
            .withRecall(2)
            .withUsage(60, 4)
        val ctx    = Context(raw, compacted).withCompaction(state)
        val config = cfg()
        val before = Default.boundaryFacts(ctx, config, Chunk.empty)

        assert(before.units.size > 1, s"REGIME: the facts must span several regions, got ${before.units.size}")
        assert(before.index.perUnit.exists(_._2.contains("poolMax")), "REGIME: the token index must carry a real identifier")
        assert(before.seed.get(2).isDefined, "REGIME: the recalled region must carry a seed entry")
        assert(before.prevLevels.nonEmpty, "REGIME: the served view must carry a demotion, or prevLevels proves nothing")
        assert(before.occupied > 0, s"REGIME: occupancy must be non-zero, got ${before.occupied}")

        // Exactly the writes adoption performs between the two runs: write-once summaries and analyses.
        val adopted = ctx.withCompaction(
            ctx.compactionState
                .withSummary(2, 4, "a summary adopted at the join")
                .withSummary(4, 6, "another summary adopted at the join")
                .withAnalysis(RegionAnalysis(2, Chunk(Compaction.Relation(3, Compaction.RelationKind.DependsOn))))
                .withAnalysis(RegionAnalysis(4, Chunk.empty[Compaction.Relation]))
        )
        val after = Default.boundaryFacts(adopted, config, Chunk.empty)

        assert(adopted.compactionState.summaries != ctx.compactionState.summaries, "REGIME: adoption must actually have written summaries")
        assert(adopted.compactionState.analyses != ctx.compactionState.analyses, "REGIME: adoption must actually have written analyses")

        // Field by field, and every field, because the carrier's whole contract is that each one is
        // invariant. Dict is opaque and offers no CanEqual, so its fields compare through `is`.
        assert(after.units == before.units, "grouping is blind to adoption")
        assert(after.spans == before.spans, "span formation is blind to adoption")
        assert(after.keys.is(before.keys), "supersession keys are blind to adoption")
        assert(after.index.ordered == before.index.ordered, "the index's region order is blind to adoption")
        assert(after.index.perUnit == before.index.perUnit, "the identifier extraction is blind to adoption")
        assert(after.index.introducer.is(before.index.introducer), "the introducer index is blind to adoption")
        assert(after.index.mentions == before.index.mentions, "the mention counts are blind to adoption")
        assert(after.index.adjacency == before.index.adjacency, "the adjacency edges are blind to adoption")
        assert(after.index.perUnitContent == before.index.perUnitContent, "the content-word extraction is blind to adoption")
        assert(after.index.contentDf == before.index.contentDf, "the content document frequencies are blind to adoption")
        assert(after.index.contentIntro.is(before.index.contentIntro), "the content introducer index is blind to adoption")
        assert(after.index.dfMax == before.index.dfMax, "the document-frequency cutoff is blind to adoption")
        assert(after.seed.is(before.seed), "the seed vector is blind to adoption (it reads recalls and the boundary count only)")
        assert(after.prevLevels.is(before.prevLevels), "the previously demoted origins are blind to adoption")
        assert(after.since == before.since, "the raw watermark is blind to adoption")
        assert(after.occupied == before.occupied, "the occupancy reading is blind to adoption")
    }
    ...
    "pass 1 is unconditional and relevance-complete" in {
        ...
        val dem    = Default.cut(Context(raw), units, spans, scores, 100, 1000, raw.size, Dict.empty)
    ...
    "any at-or-above-keep member pins the whole span verbatim" in {
        // 27 regions, so the uniform share is 0.037 and the floor is 1.35 times that, 0.05. The fixture's
        // scores read as the shares they are: 0.21 is a hot region at roughly five and a half times uniform,
        // and 0.02/0.03 sit below it. The point here is the ANY-MEMBER aggregation, not the arithmetic.
        val regions = 27
        val floor   = keepFloor(regions)
        assert(math.abs(floor - 0.05) < 0.001, s"the floor over $regions regions is keepShare/27, about 0.05, got $floor")
        val hot    = Span(0, 2, Chunk(0, 1))
        val scores = Dict[Int, Double]((0, 0.21), (1, 0.03))
        assert(
            Default.spanMaxLiveness(hot, scores) >= floor,
            "the hottest member (0.21 >= floor) pins the span verbatim (not demotable)"
        )
        val cold    = Span(2, 4, Chunk(2, 3))
        val scores2 = Dict[Int, Double]((2, 0.02), (3, 0.03))
        assert(
            Default.spanMaxLiveness(cold, scores2) < floor,
            "a span with every member below the floor IS demotable (any-member aggregation, not a mean)"
        )
    }
    ...
    "a span with an at-or-above-keep member is pinned verbatim while an all-below-keep sibling demotes" in {
        ...
        val units = Default.group(raw)
        val spans = Chunk(Span(0, 1, Chunk(0)), Span(1, 2, Chunk(1)))
        // Two regions, so the uniform share is 0.5 and the scores are written as the shares they must be:
        // under a normalized floor a synthetic 0.21 over two regions is BELOW average and correctly
        // demotable, which the old absolute floor of 0.03 hid.
        val scores = Dict[Int, Double]((0, 0.9), (1, 0.001))
        val floor  = keepFloor(units.size)
        assert(Default.spanMaxLiveness(spans(0), scores) >= floor, "span A's hottest member is at or above the floor, so it pins verbatim")
        assert(Default.spanMaxLiveness(spans(1), scores) < floor, "span B has every member below the floor, so it is demotable")
        // occupied > low forces pass 2, but the pass-1 demotion of B already fits, so B holds at Summary.
        val dem = Default.cut(Context(raw), units, spans, scores, 200, 100, raw.size, Dict.empty)
    ...
    "terse renders the marker + a fixed prefix of the summary bytes (descent only)" in {
        ...
        val state     = Compaction.State().withSummary(3, 7, longBytes)
    ...
    "a blob-less span steps verbatim->pointer, never terse; no fill is bought" in {
        ...
        val dem = Default.cut(Context(raw), units, spans, scores, 8000, 1000, raw.size, Dict.empty)
    ...
    "role 1 the fixed-size substitute elision at the summary level" in {
        ...
        val units = Default.group(raw)
        val state = Compaction.State() // empty slot => substitute elision
    ...
    "the forced path aborts AIContextOverflowException only when even that cannot fit" in {
        ...
        Abort.run[AIGenException](LLM.run(cfg(16384))(Default.renderView(Context(raw), cfg(16384)))).map { r =>
            assert(
                r match
    ...
    "a forced-path pointer descriptor carries the compaction key" in {
        ...
        val view       = Default.forced(raw, units, spans, scores, 100, raw.size, prevLevels, keys)
    ...
    "the fill config caps summary output at summaryOutputCap over both the default and a user-summarizer path" in {
        ...
        val userTuning     = Compactor.Tuning(summarizer = Present(userSummarizer))
        val userResolved   = Compactor.internal.Default(userTuning, Calibration()).resolveFillConfig(cfg())
        assert(
    ...
    "the rendered view is held at or under the hard limit" in {
        ...
        assert(occupancy(ctx) > axisOf(config).low, "the fixture occupancy sits well above effectiveLow so the demotion loop runs")
        TestCompletionServer.run { server =>
            LLM.run(config.apiUrl(server.baseUrl))(Default.renderView(ctx, config)).map { view =>
                assert(
                    Default.viewTokens(view) <= axisOf(config).hard,
                    s"the demotion loop holds the rendered view at or under the hard limit, got ${Default.viewTokens(view)} > ${axisOf(config).hard}"
                )
                assert(
    ...
    "raw is append-only up to the cap: below the high watermark eviction is a no-op" in {
        ...
        val state     = Compaction.State().withSummary(2, 3, "s2").withAnalysis(RegionAnalysis(2, Chunk.empty[Compaction.Relation]))
        val ctx       = Context(raw, compacted).withCompaction(state)
        val capped    = Compactor.internal.Default(Compactor.Tuning(rawRetentionCap = Present(100)), Calibration())
        val config    = cfg()
        assert(rawSumOf(raw) == 80, s"the fixture's raw stamp sum is 80, below the high watermark 90, got ${rawSumOf(raw)}")
        val evicted = capped.evict(ctx, config)
    ...
    "wholesale forget: past the high watermark the oldest frozen+demoted middle is forgotten down to the low watermark, replaced by one coarse band with no recall id" in {
        ...
        val state = Compaction.State()
            ...
            .withAnalysis(RegionAnalysis(2, Chunk.empty[Compaction.Relation]))
            .withAnalysis(RegionAnalysis(4, Chunk.empty[Compaction.Relation]))
            .withAnalysis(RegionAnalysis(5, Chunk.empty[Compaction.Relation]))
        val ctx     = Context(raw, compacted).withCompaction(state)
        val capped  = Compactor.internal.Default(Compactor.Tuning(rawRetentionCap = Present(100)), Calibration())
        val config  = cfg()
        val evicted = capped.evict(ctx, config)
    ...
    "survivors' ordinals are unchanged: the sequence gains a gap, never renumbers, and slot keys never alias" in {
        ...
        val state = Compaction.State()
            ...
            .withAnalysis(RegionAnalysis(1, Chunk.empty[Compaction.Relation]))
            .withAnalysis(RegionAnalysis(2, Chunk.empty[Compaction.Relation]))
            .withAnalysis(RegionAnalysis(4, Chunk.empty[Compaction.Relation]))
            .withAnalysis(RegionAnalysis(5, Chunk.empty[Compaction.Relation]))
        val ctx     = Context(raw, compacted).withCompaction(state)
    ...
    "the fixed head band and the tail band are never forgotten even under maximal pressure" in {
        ...
        val ctx     = Context(raw, compacted)
        val capped  = Compactor.internal.Default(Compactor.Tuning(rawRetentionCap = Present(20)), Calibration())
        val config  = cfg()
        val evicted = capped.evict(ctx, config)
        assert(evicted.raw.size == ctx.raw.size, "raw.size is unchanged")
    ...
    "the pre-anchor suffix rides in tokenizer units, not char/3" in {
        ...
        val ctx = Context(Chunk[Message](marker, s1, s2))
            .withCompaction(Compaction.State(lastUsage = Present(50000), lastUsageRawSize = 1))
    ...
    "the no-usage path applies the distinct widened margin, the anchored path does not" in {
        ...
        val anchored = Context(Chunk[Message](m1, m2)).withCompaction(Compaction.State(lastUsage = Present(40000), lastUsageRawSize = 2))
        assert(
    ...
    "pass 1 demotes on RELEVANCE even with budget to spare, which is what a budget-free floor means" in {
        // The behavioural consequence of taking the budget out of the pin floor, pinned here because it is
        // the one place the change is visible to a caller and no fixture covered it.
        //
        // Pass 1 is unconditional: it demotes every span below the floor whether or not the view needs to
        // shrink (pass 2, the size pass, is the conditional one). While the floor scaled with pressure it
        // was ALSO the idle bar, and at occupancy below the target that bar sat under the uniform share, so
        // an idle session stayed verbatim-rich almost by accident. Budget-free, the same floor applies at
        // every occupancy, so a span nobody references is demoted even with room to spare.
        //
        // That is the intended reading of "relevance is a property of the content, not of the budget", and
        // it is a deliberate trade: a demoted span costs a summary fill, so idle demotion spends to shrink
        // a view that already fits. Whichever way it is decided, it should be decided rather than inherited
        // from a pressure term, and this test is where the decision lives.
        val raw   = demotable().raw
        val units = Default.group(raw)
        val spans = Default.formSpans(units, raw, cfg(16384))
        val seed  = Default.seedVector(units, raw, Compaction.State())
        val sc    = Default.score(units, Default.deriveGraph(units, raw, Dict.empty[Int, Int]), Dict.empty[Int, Int], seed)
        // occupancy WELL BELOW the target, so the size pass has nothing to do and every demotion is pass 1
        val low      = 100000
        val occupied = 10
        val idle     = Default.cut(Context(raw), units, spans, sc, occupied, low, raw.size, Dict.empty)
        assert(occupied <= low, "REGIME: the fixture must sit under the target, so pass 2 is inert by construction")
        val floor      = keepFloor(units.size)
        val belowFloor = spans.toList.filter(sp => Default.spanMaxLiveness(sp, sc) < floor).map(_.start).toSet
        assert(belowFloor.nonEmpty, s"REGIME: some span must sit below the floor $floor, or there is nothing to demote")
        assert(
            idle.toMap.keySet == belowFloor,
            s"with room to spare, exactly the below-floor spans demote and nothing else: demoted=${idle.toMap.keySet} " +
                s"belowFloor=$belowFloor"
        )
        assert(
            idle.toMap.values.forall(_ == Level.Summary),
            s"and pass 1 stops at the summary level: descending further is pass 2's job, got ${idle.toMap.values.toSet}"
        )
    }

    // ==== the occupancy axis, which is the compactor's policy now ====

    "the axis projects the tuning's fractions onto the model's window, in order" in {
        // These moved here from ConfigTest with the axis itself. They used to check Config.validatedAxis,
        // a construction-time check on a record Config owned; the fractions are the compactor's policy now
        // and the projection needs a window, which is a per-model fact one Tuning serves many of. So the
        // ordering is checked where the projection happens rather than where the fractions are stored.
        //
        // Default tuning on a 200000-token window with maxTokens 10000:
        //   high  = min(0.5*200000, 128000) = 100000
        //   low   = 0.6*100000 = 60000 ; prepare = 0.8*100000 = 80000
        //   hard  = 0.9*(200000-10000) = 171000
        val config = Config.OpenAI.default.model(Config.OpenAI, "m", 200000).maxTokens(10000)
        val a      = Compactor.internal.axis(Compactor.Tuning(), config)
        assert(a.high == 100000, s"high: ${a.high}")
        assert(a.low == 60000, s"low: ${a.low}")
        assert(a.prepare == 80000, s"prepare: ${a.prepare}")
        assert(a.hard == 171000, s"hard: ${a.hard}")
        assert(
            a.low < a.prepare && a.prepare <= a.high && a.high < a.hard,
            s"the full ordering must hold: low ${a.low} < prepare ${a.prepare} <= high ${a.high} < hard ${a.hard}"
        )
    }

    "the fraction ordering is rejected at Tuning construction, naming the inequality" in {
        // The half of the old axis validation that holds WITHOUT a window, so it can still be checked at
        // construction. A caller who inverts target and prepare gets told which inequality they broke.
        val bad = intercept[IllegalArgumentException](Compactor.Tuning(target = 0.9, prepare = 0.5).validated)
        assert(bad.getMessage.contains("target"), s"the message must name the violated inequality, got: ${bad.getMessage}")
        val shipped = Compactor.Tuning()
        assert(shipped.validated eq shipped, "and the shipped fractions validate to the same value, untouched")
    }

    "Compactor.init(tuning) is where that rejection actually reaches a caller" in {
        // The check above exercises `validated` directly, which for a while was the ONLY thing that did:
        // the method was private[kyo] with no call site in main, so an inverted pair reached the axis and
        // was silently corrected there (prepare is raised to low + 1 during projection). A fraction typo
        // therefore became a policy the caller never chose, and the guard was dead code that read as live.
        // This pins the wiring rather than the method: the public entry that accepts a Tuning rejects it.
        val bad = intercept[IllegalArgumentException](Compactor.init(Compactor.Tuning(target = 0.9, prepare = 0.5)))
        assert(bad.getMessage.contains("target"), s"the message must name the violated inequality, got: ${bad.getMessage}")
        // And the ordinary paths stay usable: the shipped tuning, and anything the builders produced.
        assert(Compactor.init(Compactor.Tuning()) ne null, "the shipped tuning must construct")
        assert(
            Compactor.init(Compactor.Tuning().target(0.5).prepare(0.9)) ne null,
            "a builder-built tuning must construct, since the builders already keep prepare above target"
        )
    }

    "per-field clamps hold each knob in its own range" in {
        val t = Compactor.Tuning()
        assert(t.trigger(1.5).trigger == 1.0, "trigger clamps to 1.0")
        assert(t.hardLimit(0.0).hardLimit > 0.0, "hardLimit clamps above 0 so the overflow guard can never be built away")
        val prep = t.prepare(0.1)
        assert(prep.prepare > t.target && prep.prepare <= 1.0, s"prepare clamps into (target, 1.0], got ${prep.prepare}")
        assert(t.noContextCeiling.contextCeiling == Absent, "noContextCeiling gives a pure-fraction trigger")
        assert(t.contextCeiling(7).contextCeiling == Present(7), "and a ceiling is taken as given")
    }

    "the projection never collapses the prepare line onto the target" in {
        // The integer-truncation hazard the old validatedAxis carried a repair search for: two distinct
        // fractions can project onto the same integer (target 0.6 and the next representable double above
        // it both hit 76800 at high=128000). The projection raises prepare to the first integer above the
        // target instead, so a caller whose fractions were already correctly ordered gets a usable axis
        // rather than a failure.
        val config = Config.OpenAI.default.model(Config.OpenAI, "m", 256000)
        val tight  = Compactor.Tuning(target = 0.6, prepare = math.nextUp(0.6))
        val a      = Compactor.internal.axis(tight, config)
        assert(a.low < a.prepare, s"prepare must clear the target after truncation: low ${a.low} prepare ${a.prepare}")
        assert(a.prepare <= a.high, s"and must not exceed the trigger: prepare ${a.prepare} high ${a.high}")
    }

    "an output maximum large enough to reorder the axis falls back to the conservative reservation" in {
        // This judgement used to live on Config, which meant Config had to know the watermarks to make it.
        // It belongs to the projection, the only thing that knows both the reservation and the fractions.
        val huge = Config.OpenAI.default.model(Config.OpenAI, "m", 16384).maxTokens(16000)
        val a    = Compactor.internal.axis(Compactor.Tuning(), huge)
        assert(a.high < a.hard, s"the axis stays coherent whatever the model claims: high ${a.high} hard ${a.hard}")
    }

    // ==== the two properties the seam owns, each guarded before the boundary is reshaped ====

    "the seam serves the ENABLED INSTANCE's tuning, not the shipped singleton's" in {
        // The seam resolves a compactor and must drive THAT value. Nothing else in the suite can see this:
        // the ablation tests call the scoring functions on an instance directly, so they would still pass if
        // the seam quietly reverted to the singleton, and every served view would silently ignore a caller's
        // tuning. The observable is chosen to be one only the instance can explain: with fills off, a
        // demoted span renders the fixed-size substitute elision, whose byte budget is the instance's own
        // substituteElisionChars. Two instances differing ONLY in that number must serve different bytes.
        //
        // DIFFERENCE is the assertion, not direction. A bigger budget does not monotonically mean a bigger
        // view: the substitute is used only while it renders smaller than the content it replaces, and past
        // that point a degenerate note replaces the whole span instead, so a generous budget can serve
        // fewer bytes than a tight one. Asserting a direction here would encode this fixture's regime
        // rather than the property, which is that the served bytes depend on the instance at all.
        // Measured on the view the seam INSTALLED, not on a captured wire body. Reading the first captured
        // request is a race: a boundary arms a background preparation fiber, and whether its request or the
        // generation's lands first is not determined, so `captured.head` is sometimes the wrong request.
        // The installed `compacted` list is the value under test and is deterministic.
        def serveWith(c: Compactor[Any])(using Frame): Int < (Async & Abort[Any] & Scope) =
            TestCompletionServer.run { server =>
                val base   = cfg(16384).apiUrl(server.baseUrl)
                val config = base
                Kyo.foreachDiscard(0 until 20)(_ => server.enqueueBody(genBody("1"))).andThen {
                    LLM.run(config) {
                        AI.enable(c) {
                            AI.init.map(ai =>
                                ai.setContext(demotable()).andThen(ai.gen[Int]).handle(Abort.run[Any])
                                    .andThen(ai.context.map(ctx => contentLen(ctx.compacted)))
                            )
                        }
                    }
                }
            }
        val tight = Compactor.internal.Default(
            Compactor.Tuning(mechanisms = Compactor.Mechanism.all - Compactor.Mechanism.SummaryFills),
            Compactor.internal.Calibration(substituteElisionChars = 60)
        )
        val generous = Compactor.internal.Default(
            Compactor.Tuning(mechanisms = Compactor.Mechanism.all - Compactor.Mechanism.SummaryFills),
            Compactor.internal.Calibration(substituteElisionChars = 2000)
        )
        serveWith(tight).map { tightLen =>
            serveWith(generous).map { generousLen =>
                assert(
                    tightLen != generousLen,
                    s"the seam must serve each instance's own tuning; both arms served $tightLen chars, " +
                        "which is what a seam running the singleton would do"
                )
            }
        }
    }

    "a reply reporting no usage does NOT seat the occupancy anchor" in {
        // The re-anchor gate. Its failure mode is silent and total: seating the anchor at 0 makes occupancy
        // read ~0 forever, so no boundary ever fires again and compaction is disabled without an error
        // anywhere. A synthetic repair reply and a wire that reports nothing both arrive as zero usage, so
        // the gate is on the normal path, not an edge case.
        //
        // Both arms are asserted. Checking only that a zero reply is refused would pass an implementation
        // that never seats the anchor at all, which is the same silent failure by another route.
        def anchorAfter(promptTokens: Int)(using Frame): Maybe[Int] < (Async & Abort[Any] & Scope) =
            TestCompletionServer.run { server =>
                val config = cfg(16384).apiUrl(server.baseUrl)
                server.enqueueBody(genBody("1", promptTokens)).andThen {
                    LLM.run(config) {
                        AI.init.map(ai => ai.userMessage("hello").andThen(ai.gen[Int]).andThen(ai.context))
                    }.handle(Abort.run[Any]).map {
                        case Result.Success(ctx) => ctx.compactionState.lastUsage
                        case other               => Maybe.empty[Int]
                    }
                }
            }
        anchorAfter(0).map { unreported =>
            anchorAfter(4321).map { reported =>
                assert(unreported == Absent, s"a reply carrying no usage must leave the anchor unseated, got $unreported")
                assert(reported == Present(4321), s"a reply carrying usage must seat the anchor at the reported total, got $reported")
            }
        }
    }
