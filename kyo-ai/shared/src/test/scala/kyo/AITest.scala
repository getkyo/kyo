...
class AITest extends kyo.test.Test[Any]:
    ...
    "session context enriches with the ambient scope prompt, the instance prompt, and the default guidance" in {
        ...
        LLM.run {
            ...
            val systems    = messages.collect { case SystemMessage(content, _, _) => content }
            ...
            assert(messages.exists { case UserMessage(c, _, _, _) => c.contains("hello"); case _ => false }, "the raw exchange stays")
        }
    }
