                                            ...
                                            sseStream.map { event =>
                                                ...
                                                Log.trace(s"kyo-ai stream event ${config.provider.name} ${elideBody(event.data)}").andThen {
                                                    ...
                                                    if event.data.trim.isEmpty then ""
                                                        ...
                                                        // Record the provider's stream-end usage into the sink: OpenAI's
                                                        // include_usage final chunk and Anthropic's message_start both
                                                        // carry it on an event that projects to Delta.Skip, so recording
                                                        // is orthogonal to fragment projection. AtomicRef.set is Sync.
                                                        val record =
