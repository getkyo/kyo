...
private[completion] object AnthropicCompletion extends Completion:
    ...
    private def read(config: Config, response: Response)(using Frame): Completion.Reply < (LLM & Sync & Abort[AIGenException]) =
        ...
        val surfaced =
            stop match
                ...
                case _ => Kyo.unit
        surfaced.andThen(Completion.Reply(Chunk(AssistantMessage(textOf(response), calls)), stop, usage = toUsage(response.usage)))
    end read

    // Converts the wire usage object to the public Completion.Usage. Anthropic does not report cached
    // input tokens on this response shape, so cachedInputTokens stays Absent.
    private def toUsage(usage: Maybe[internal.Usage]): Maybe[Completion.Usage] =
        usage.map(u => Completion.Usage(inputTokens = u.input_tokens, outputTokens = u.output_tokens, cachedInputTokens = Absent))
    ...
    // Anthropic reports usage with no opt-in: the message_start event carries message.usage.input_tokens
    // (the request total the anchor needs); message_delta carries only a cumulative output_tokens. Decode
    // the message_start usage and convert the wire usage to the public Completion.Usage (no cached tokens);
    // any other event yields Absent.
    ...
    private object internal:
        ...
        // The streaming usage shape: message_start carries message.usage.input_tokens; fields are Maybe
        // to tolerate the non-usage event types.
