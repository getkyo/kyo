...
case class Context(
    ...
    compaction: Maybe[Compaction.State] = Absent
    ...
    def merge(that: Context): Context =
        ...
        copy(
            raw = raw.concat(tail),
            compacted = compacted.concat(tail),
            compaction = Compaction.merge(compaction, that.compaction)
        )
    end merge
    ...
    /** The compaction state seat, defaulting to a fresh empty state. The Compactor reads and
      ...
      */
    private[kyo] def compactionState: Compaction.State =
        compaction.getOrElse(Compaction.State())

    private[kyo] def withCompaction(state: Compaction.State): Context =
