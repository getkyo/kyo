...
sealed trait TestTrace[+A]:
    ...
    final def withSpan(span: Option[Span] = None): TestTrace[A] =
        if span.isDefined then
            self match
                case node: TestTrace.Node[?]        => node.copy(span = span)
    ...
    def withCustomLabel(customLabel: Option[String]): TestTrace[A] =
        if customLabel.isDefined then
            self match
                case node: TestTrace.Node[A] =>
                    if node.customLabel.isDefined then node
                ...
                case and: TestTrace.And =>
                    TestTrace.And(and.left.withCustomLabel(customLabel), and.right.withCustomLabel(customLabel)).asInstanceOf[TestTrace[A]]
                case or: TestTrace.Or =>
                    TestTrace.Or(or.left.withCustomLabel(customLabel), or.right.withCustomLabel(customLabel)).asInstanceOf[TestTrace[A]]
                case not: TestTrace.Not =>
    ...
    final def withCompleteCode(completeCode: Option[String]): TestTrace[A] =
        self match
            ...
            case and: TestTrace.And =>
                TestTrace.And(and.left.withCompleteCode(completeCode), and.right.withCompleteCode(completeCode)).asInstanceOf[TestTrace[A]]
            case or: TestTrace.Or =>
                TestTrace.Or(or.left.withCompleteCode(completeCode), or.right.withCompleteCode(completeCode)).asInstanceOf[TestTrace[A]]
            case not: TestTrace.Not =>
    ...
    final def withGenFailureDetails(genFailureDetails: Option[GenFailureDetails]): TestTrace[A] =
        self match
            ...
            case and: TestTrace.And =>
                TestTrace.And(
                    and.left.withGenFailureDetails(genFailureDetails),
                    and.right.withGenFailureDetails(genFailureDetails)
                ).asInstanceOf[TestTrace[A]]
            case or: TestTrace.Or =>
                TestTrace.Or(
                    or.left.withGenFailureDetails(genFailureDetails),
                    or.right.withGenFailureDetails(genFailureDetails)
                ).asInstanceOf[TestTrace[A]]
            case not: TestTrace.Not =>
    ...
    def getGenFailureDetails: Option[GenFailureDetails] =
        self match
            case node: TestTrace.Node[?]        => node.genFailureDetails
    ...
    final def annotate(annotation: TestTrace.Annotation*): TestTrace[A] =
        self match
            case node: TestTrace.Node[?]     => node.copy(annotations = node.annotations ++ annotation.toSet)
            case TestTrace.AndThen(_, right) => right.annotate(annotation*)
            case other                       => other
...
object TestTrace:
    ...
    private[test] case class Node[+A](
        ...
        annotations: Set[TestTrace.Annotation] = Set.empty,
    ...
    def die(throwable: Throwable): TestTrace[Nothing] =
        Node(Result.panic(throwable), message = ErrorMessage.throwable(throwable))
end TestTrace
