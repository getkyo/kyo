...
sealed trait TestTrace[+A]:
    ...
    /** Apply the metadata to the rightmost node in the trace.
      */
    final def withSpan(span: Option[Span] = None): TestTrace[A] = if span.isDefined then
        self match
    ...
    /** Apply the parent span to every node in the tree.
      */
    def withParentSpan(span: Option[Span]): TestTrace[A] = if span.isDefined then
        self match
    ...
    /** Apply the location to every node in the tree.
      */
    def withLocation(location: Option[String]): TestTrace[A] = if location.isDefined then
        self match
            case node: TestTrace.Node[?] =>
