...
type Exit[R, E] = Result[E, R]
object Exit:
    val Failure: Exit[Nothing, String] = Result.Failure("Failure")
    val Success: Exit[String, Nothing] = Result.Success("Success")
end Exit
...
final case class Assertion[-A](arrow: TestArrow[A, Boolean]):
    ...
    private[test] def withCode(code: String, arguments: Arguments*): Assertion[A] =
        Assertion(arrow.withCode(code, arguments*))
end Assertion
...
object Assertion extends AssertionVariants:
    ...
    private[test] def smartAssertKyo[S, A](
        expr: => A < S
    )(assertion: Assertion[A])(implicit trace: Trace): TestResult < S =
        lazy val value0 = expr
    ...
    /** Makes a new assertion that requires an Iterable contain the specified element. See [[Assertion.exists]] if you want to require an
      * Iterable to contain an element satisfying an assertion.
      */
        ...
        def loop(iterator: Iterator[A]): Boolean = iterator.hasNext match
            ...
            case true =>
                ...
                if iterator.hasNext then
                    val y = iterator.next()
                    if ord.lteq(x, y) then loop(Iterator(y) ++ iterator) else false
                else
                    true
                end if
        ...
        Assertion {
            TestArrow
                .make[Boolean, Boolean] { boolean =>
                    TestTrace.boolean(boolean)(M.pretty(boolean) + M.was + M.value("true"))
                }
                .withCode("isTrue")
        }
    ...
    def isFalse: Assertion[Boolean] =
        Assertion {
            TestArrow
                .make[Boolean, Boolean] { boolean =>
                    TestTrace.boolean(!boolean)(M.pretty(boolean) + M.was + M.value("false"))
                }
                .withCode("isFalse")
        }
            ...
            TestArrow
                .make[A, Boolean] { value =>
                    TestTrace.boolean(values.exists(_ == value)) {
                        M.pretty(value) + M.was + M.value(Text(values.mkString(", ")))
                    }
                }
        ...
        Assertion[Unit](
            TestArrow
                .make[Unit, Boolean] { value =>
                    TestTrace.boolean(true)(M.value(Text("unit")) + M.was + M.value(Text("unit")))
                }
                .withCode("isUnit")
        )
    ...
    /** Makes a new assertion that requires an Iterable contain an element satisfying the given assertion. See [[Assertion.contains]] if you
      * only need an Iterable to contain a given element.
      */
    ...
    /** Makes a new assertion that requires an Iterable to have the same distinct elements as the other Iterable, though not necessarily in
      * the same order.
      */
    ...
    /** Makes a new assertion that requires an Iterable to have the same elements as the specified Iterable, though not necessarily in the
      * same order.
      */
    ...
    val isDistinct: Assertion[Iterable[Any]] =
        ...
        def loop(iterator: Iterator[Any], seen: Set[Any]): Boolean = iterator.hasNext match
            ...
            case true =>
                val x = iterator.next()
                if seen.contains(x) then false else loop(iterator, seen + x)
    ...
    def isJustInterrupted: Assertion[Exit[Any, Any]] =
        Assertion[Exit[Any, Any]](
            TestArrow
                .make[Exit[Any, Any], Boolean] {
                    case Result.Failure(Fiber.Interrupted(_)) =>
