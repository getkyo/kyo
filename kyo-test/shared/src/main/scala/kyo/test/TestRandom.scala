...
/** `TestRandom` allows for deterministically testing effects involving randomness.
  *
  * `TestRandom` operates in two modes. In the first mode, `TestRandom` is a purely functional pseudo-random number generator. It will
  * generate pseudo-random values just like `scala.util.Random` except that no internal state is mutated. Instead, methods like `nextInt`
  * describe state transitions from one random state to another that are automatically composed together through methods like `flatMap`. The
  * random seed can be set using `setSeed` and `TestRandom` is guaranteed to return the same sequence of values for any given seed. This is
  * useful for deterministically generating a sequence of pseudo-random values and powers the property based testing functionality in ZIO
  * Test.
  *
  * In the second mode, `TestRandom` maintains an internal buffer of values that can be "fed" with methods such as `feedInts` and then when
  * random values of that type are generated they will first be taken from the buffer. This is useful for verifying that functions produce
  * the expected output for a given sequence of "random" inputs.
  *
  * {{{
  * import zio.Random
  ...
  * }}}
  *
  * `TestRandom` will automatically take values from the buffer if a value of the appropriate type is available and otherwise generate a
  * pseudo-random value, so there is nothing you need to do to switch between the two modes. Just generate random values as you normally
  * would to get pseudo-random values, or feed in values of your own to get those values back. You can also use methods like `clearInts` to
  * clear the buffer of values of a given type so you can fill the buffer with new values or go back to pseudo-random number generation.
  */
...
object TestRandom extends Serializable:
    ...
    final case class Test(randomState: Ref.Atomic[Data], bufferState: Ref.Atomic[Buffer]) extends TestRandom:
        ...
        /** Feeds the buffer with specified sequence of booleans. The first value in the sequence will be the first to be taken. These
          * values will be taken before any values that were previously in the buffer.
          */
        def feedBooleans(booleans: Boolean*)(implicit trace: Trace): UIO[Unit] =
            bufferState.update(data => data.copy(booleans = booleans.toList ::: data.booleans))

        /** Feeds the buffer with specified sequence of chunks of bytes. The first value in the sequence will be the first to be taken.
          * These values will be taken before any values that were previously in the buffer.
          */
        def feedBytes(bytes: Chunk[Byte]*)(implicit trace: Trace): UIO[Unit] =
            bufferState.update(data => data.copy(bytes = bytes.toList.flatten ::: data.bytes))

        /** Feeds the buffer with specified sequence of characters. The first value in the sequence will be the first to be taken. These
          * values will be taken before any values that were previously in the buffer.
          */
        def feedChars(chars: Char*)(implicit trace: Trace): UIO[Unit] =
            bufferState.update(data => data.copy(chars = chars.toList ::: data.chars))

        /** Feeds the buffer with specified sequence of doubles. The first value in the sequence will be the first to be taken. These values
          * will be taken before any values that were previously in the buffer.
          */
        def feedDoubles(doubles: Double*)(implicit trace: Trace): UIO[Unit] =
            bufferState.update(data => data.copy(doubles = doubles.toList ::: data.doubles))

        /** Feeds the buffer with specified sequence of floats. The first value in the sequence will be the first to be taken. These values
          * will be taken before any values that were previously in the buffer.
          */
        def feedFloats(floats: Float*)(implicit trace: Trace): UIO[Unit] =
            bufferState.update(data => data.copy(floats = floats.toList ::: data.floats))

        /** Feeds the buffer with specified sequence of integers. The first value in the sequence will be the first to be taken. These
          * values will be taken before any values that were previously in the buffer.
          */
        def feedInts(ints: Int*)(implicit trace: Trace): UIO[Unit] =
            bufferState.update(data => data.copy(integers = ints.toList ::: data.integers))

        /** Feeds the buffer with specified sequence of longs. The first value in the sequence will be the first to be taken. These values
          * will be taken before any values that were previously in the buffer.
          */
        def feedLongs(longs: Long*)(implicit trace: Trace): UIO[Unit] =
            bufferState.update(data => data.copy(longs = longs.toList ::: data.longs))

        /** Feeds the buffer with specified sequence of strings. The first value in the sequence will be the first to be taken. These values
          * will be taken before any values that were previously in the buffer.
          */
        def feedStrings(strings: String*)(implicit trace: Trace): UIO[Unit] =
            bufferState.update(data => data.copy(strings = strings.toList ::: data.strings))

        /** Feeds the buffer with specified sequence of UUIDs. The first value in the sequence will be the first to be taken. These values
          * will be taken before any values that were previously in the buffer.
          */
        def feedUUIDs(uuids: UUID*)(implicit trace: Trace): UIO[Unit] =
        ...
        /** Takes a double from the buffer if one exists or else generates a pseudo-random, uniformly distributed double between 0.0 and
          * 1.0.
          */
        ...
        /** Takes a double from the buffer if one exists or else generates a pseudo-random double from a normal distribution with mean 0.0
        ...
        /** Takes an integer from the buffer if one exists or else generates a pseudo-random integer between 0 (inclusive) and the specified
          * value (exclusive).
          */
        ...
        /** Takes a long from the buffer if one exists or else generates a pseudo-random long between 0 (inclusive) and the specified value
          * (exclusive).
          */
        ...
        override val unsafe: UnsafeAPI =
            new UnsafeAPI:
                ...
                private def getOrElseChunk[A](
                    ...
                    if buffered.length == length then buffered
                    ...
                    randomState.unsafe.modify { case Data(seed1, seed2, queue) =>
                        ...
                        case None =>
                            ...
                            def loop: (Double, Double, Double) =
                                ...
                                val y      = 2 * d2 - 1
                                val radius = x * x + y * y
                                if radius >= 1 || radius == 0 then loop else (x, y, radius)
                            end loop
                            loop match
                                case (x, y, radius) =>
    ...
    /** Constructs a new `TestRandom` with the specified initial state. This can be useful for providing the required environment to an
      * effect that requires a `Random`, such as with `ZIO#provide`.
      */
    ...
    /** Constructs a new `Test` object that implements the `TestRandom` interface. This can be useful for mixing in with implementations of
      * other interfaces.
      */
    ...
    /** Accesses a `TestRandom` instance in the environment and saves the random state in an effect which, when run, will restore the
      * `TestRandom` to the saved state.
      */
    def save(implicit trace: Trace): UIO[UIO[Unit]] =
