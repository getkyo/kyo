...
/** `TestConsole` provides a testable interface for programs interacting with the console by modeling input and output as reading from and
  * writing to input and output buffers maintained by `TestConsole` and backed by a `Ref`.
  *
  * All calls to `print` and `printLine` using the `TestConsole` will write the string to the output buffer and all calls to `readLine` will
  * take a string from the input buffer. To facilitate debugging, by default output will also be rendered to standard output. You can enable
  * or disable this for a scope using `debug`, `silent`, or the corresponding test aspects.
  *
  * `TestConsole` has several methods to access and manipulate the content of these buffers including `feedLines` to feed strings to the
  * input buffer that will then be returned by calls to `readLine`, `output` to get the content of the output buffer from calls to `print`
  * and `printLine`, and `clearInput` and `clearOutput` to clear the respective buffers.
  *
...
object TestConsole extends Serializable:
    ...
    case class Test(
        ...
        /** Runs the specified effect with the `TestConsole` set to debug mode, so that console output is rendered to standard output in
          * addition to being written to the output buffer.
          */
        def debug[R, E, A](zio: ZIO[R, E, A])(implicit trace: Trace): ZIO[R, E, A] =
            debugState.locally(true)(zio)

        /** Writes the specified sequence of strings to the input buffer. The first string in the sequence will be the first to be taken.
          * These strings will be taken before any strings that were previously in the input buffer.
          */
        def feedLines(lines: String*)(implicit trace: Trace): UIO[Unit] =
        ...
        /** Returns the contents of the error output buffer. The first value written to the error output buffer will be the first in the
          * sequence.
          */
        def outputErr(implicit trace: Trace): UIO[Vector[String]] =
        ...
        /** Runs the specified effect with the `TestConsole` set to silent mode, so that console output is only written to the output buffer
          * and not rendered to standard output.
          */
        def silent[R, E, A](zio: ZIO[R, E, A])(implicit trace: Trace): ZIO[R, E, A] =
    ...
    /** Constructs a new `Test` object that implements the `TestConsole` interface. This can be useful for mixing in with implementations of
      * other interfaces.
      */
    ...
    /** Accesses a `TestConsole` instance in the environment and runs the specified effect with the `TestConsole` set to debug mode, so that
      * console output is rendered to standard output in addition to being written to the output buffer.
      */
    def debug[R, E, A](zio: ZIO[R, E, A])(implicit trace: Trace): ZIO[R, E, A] =
    ...
    /** Accesses a `TestConsole` instance in the environment and writes the specified sequence of strings to the input buffer.
      */
    def feedLines(lines: String*)(implicit trace: Trace): UIO[Unit] =
        testConsoleWith(_.feedLines(lines*))
    ...
    /** Accesses a `TestConsole` instance in the environment and saves the console state in an effect which, when run, will restore the
      * `TestConsole` to the saved state.
      */
    def save(implicit trace: Trace): UIO[UIO[Unit]] =
        testConsoleWith(_.save)

    /** Accesses a `TestConsole` instance in the environment and runs the specified effect with the `TestConsole` set to silent mode, so
      * that console output is only written to the output buffer and not rendered to standard output.
      */
    def silent[R, E, A](zio: ZIO[R, E, A])(implicit trace: Trace): ZIO[R, E, A] =
