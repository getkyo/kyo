...
/** A sample is a single observation from a random variable, together with a tree of "shrinkings" used for minimization of "large" failures.
  */
final case class Sample[-S, +A](value: A, shrink: Stream[Sample[S, A], S])(using trace: Trace):
    self =>
    ...
    /** Filters this sample by replacing it with its shrink tree if the value does not meet the specified predicate and recursively
      ...
      */
    def filter(f: A => Boolean)(using trace: Trace): Stream[Sample[S, A], S] =
    ...
    def flatMap[S1, B](f: A => Sample[S1, B])(using trace: Trace): Sample[S & S1, B] =
        val sample = f(value)
        Sample(sample.value, sample.shrink.concat(shrink.map(_.flatMap(f))))

    def foreach[S1, B](f: A => (B < S1))(using trace: Trace): Stream[Sample[S & S1, B], S & S1] =
        f(value).map(b => Sample(b, shrink.map(_.foreach(f))))
    ...
    /** Converts the shrink tree into a stream of shrinkings by recursively searching the shrink tree, using the specified function to
      ...
      */
    def shrinkSearch(f: A => Boolean)(using trace: Trace): Stream[Sample[S, A], S] =
        if !f(value) then Stream(Emit.value(Chunk(this)))
        else Stream(Emit.value(Chunk(this))).concat(shrink.takeUntil(v => f(v.value)).flatMap(_.shrinkSearch(f)))
...
object Sample:
    ...
    def unfold[S, A, T](s: T)(f: T => (A, Stream[Sample[S, A], T]))(using trace: Trace): Sample[S, A] =
