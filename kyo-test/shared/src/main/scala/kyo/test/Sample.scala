...
/** A sample is a single observation from a random variable, together with a tree of "shrinkings" used for minimization of "large" failures.
  */
final case class Sample[-R, +A](value: A, shrink: KStream[R, Nothing, Sample[R, A]])(using trace: Trace):
    self =>

    /** A symbolic alias for `zip`. */
    def <*>[R1 <: R, B](that: Sample[R1, B])(using zippable: Zippable[A, B], trace: Trace): Sample[R1, zippable.Out] =
        self.zip(that)
    ...
    /** Filters this sample by replacing it with its shrink tree if the value does not meet the specified predicate and recursively
      ...
      */
    def filter(f: A => Boolean)(using trace: Trace): KStream[R, Nothing, Sample[R, A]] =
        if f(value) then KStream(Sample(value, shrink.flatMap(_.filter(f))))
        else shrink.flatMap(_.filter(f))

    def flatMap[R1 <: R, B](f: A => Sample[R1, B])(using trace: Trace): Sample[R1, B] =
        val sample = f(value)
        Sample(sample.value, sample.shrink ++ shrink.map(_.flatMap(f)))

    def foreach[R1 <: R, B](f: A => (B < Env[R1]))(using trace: Trace): (Sample[R1, B] < Env[R1]) =
        f(value).map(b => Sample(b, shrink.mapZIO(_.foreach(f))))

    def map[B](f: A => B)(using trace: Trace): Sample[R, B] =
    ...
    /** Converts the shrink tree into a stream of shrinkings by recursively searching the shrink tree, using the specified function to
      ...
      */
    def shrinkSearch(f: A => Boolean)(using trace: Trace): KStream[R, Nothing, A] =
        if !f(value) then KStream(value)
        else KStream(value) ++ shrink.takeUntil(v => f(v.value)).flatMap(_.shrinkSearch(f))

    /** Composes this sample with the specified sample to create a cartesian product of values and shrinkings. */
    def zip[R1 <: R, B](that: Sample[R1, B])(using zippable: Zippable[A, B], trace: Trace): Sample[R1, zippable.Out] =
    ...
    /** Composes this sample with the specified sample to create a cartesian product of values and shrinkings with the specified function.
      */
    def zipWith[R1 <: R, B, C](that: Sample[R1, B])(f: (A, B) => C)(using trace: Trace): Sample[R1, C] =
...
object Sample:
    ...
    def unfold[R, A, S](s: S)(f: S => (A, KStream[R, Nothing, S]))(using trace: Trace): Sample[R, A] =
