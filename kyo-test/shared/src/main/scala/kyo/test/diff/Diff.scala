...
trait DiffInstances extends LowPriDiff:
    ...
    /** The default String renderer attempts to clever render the resulting diff according to its inputs.
      ...
      *   - Single line strings are determined (based on word to length ratio) to be either structured word-based sentences or less
      *     structured dat.
...
trait LowPriDiff:
    ...
    def anyDiff[A]: Diff[A] = new Diff[A]:
        override def diff(x: A, y: A): DiffResult =
            if x == y then DiffResult.Identical(x)
