...
object TestAspect extends TimeoutVariants:
            ...
            def some[R <: R0, E](spec: Spec[R, E])(using trace: Trace): Spec[R, E] =
                Spec.scoped[R](
                    Ref.make(false).flatMap { failure =>
                        Kyo.acquireRelease(unit)(_ => f.whenZIO(failure.get)) *>
            ...
            def some[R <: R0, E](spec: Spec[R, E])(using trace: Trace): Spec[R, E] =
                Spec.scoped[R](
                    Ref.make(true).flatMap { success =>
                        Kyo.acquireRelease(unit)(_ => f.whenZIO(success.get)) *>
    ...
    /** An aspect that applies the provided kyo aspect to each sample of all checks in the test. (Implementation for checksZIO is partially
      * shown)
      */
    def checks(aspect: CheckAspect): TestAspectPoly =
        checksZIO(Kyo.succeed(aspect)(using Trace.empty))

    def checksZIO[R, E](makeAspect: Any < Env[R] & Abort[E]): TestAspect[Nothing, R, E, Any] =
