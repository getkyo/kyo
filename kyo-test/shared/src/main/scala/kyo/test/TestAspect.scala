...
object TestAspect extends TimeoutVariants:
    ...
    /** An aspect that applies the provided zio aspect to each sample of all checks in the test. The aspect will be constructed from the
      * provided effect before each test is run.
      */
    def checksZIO[R, E](
    ...
    /** An aspect that runs each test on a separate fiber and prints a fiber dump if the test fails or has not terminated within the
      * specified duration.
      */
    ...
    /** An aspect that runs each test with the `TestConsole` instance in the environment set to debug mode so that console output is
      * rendered to standard output in addition to being written to the output buffer.
      */
    ...
    val exceptJS: TestAspectPoly =
        if TestPlatform.isJS then ignore else identity
    ...
    def exceptJS[LowerR, UpperR, LowerE, UpperE](
        ...
        if TestPlatform.isJS then identity else that

    /** An aspect that runs tests on all platforms except the JVM.
      */
    val exceptJVM: TestAspectPoly =
        if TestPlatform.isJVM then ignore else identity
    ...
    def exceptJVM[LowerR, UpperR, LowerE, UpperE](
        ...
        if TestPlatform.isJVM then identity else that

    /** An aspect that runs tests on all platforms except ScalaNative.
      */
    val exceptNative: TestAspectPoly =
        if TestPlatform.isNative then ignore else identity
    ...
    /** An aspect that makes a test that failed for the specified failure pass. Note that the test will fail for other failures and also if
      * it passes correctly.
      */
    def failing[E0](assertion: TestFailure[E0] => Boolean): TestAspect[Nothing, Any, Nothing, E0] =
        new TestAspect.PerTest[Nothing, Any, Nothing, E0]:
            def perTest[R, E <: E0](test: ZIO[R, TestFailure[E], TestSuccess])(implicit
                ...
                test.foldZIO(
                    failure =>
                        if assertion(failure) then ZIO.succeed(TestSuccess.Succeeded())
    ...
    /** An aspect that records the state of fibers spawned by the current test in [[TestAnnotation.fibers]]. Applied by default in
      * [[ZIOSpecAbstract]]. This aspect is required for the proper functioning of `TestClock.adjust`.
      */
    ...
    def js[LowerR, UpperR, LowerE, UpperE](
        ...
        if TestPlatform.isJS then that else identity

    /** An aspect that only runs tests on ScalaJS.
      */
    val jsOnly: TestAspectPoly =
        if TestPlatform.isJS then identity else ignore
    ...
    def jvm[LowerR, UpperR, LowerE, UpperE](
        ...
        if TestPlatform.isJVM then that else identity

    /** An aspect that only runs tests on the JVM.
      */
    val jvmOnly: TestAspectPoly =
        if TestPlatform.isJVM then identity else ignore
    ...
    def os(f: System.OS => Boolean): TestAspectPoly =
        if f(System.os) then identity else ignore
    ...
    def native[LowerR, UpperR, LowerE, UpperE](
        ...
        if TestPlatform.isNative then that else identity

    /** An aspect that only runs tests on ScalaNative.
      */
    val nativeOnly: TestAspectPoly =
        if TestPlatform.isNative then identity else ignore
    ...
    /** An aspect that restores a given [[zio.test.Restorable Restorable]]'s state to its starting state after the test is run. Note that
      * this is only useful when repeating tests.
      */
    ...
    /** An aspect that restores the [[zio.test.TestClock TestClock]]'s state to its starting state after the test is run. Note that this is
      * only useful when repeating tests.
      */
    def restoreTestClock: TestAspectPoly =
        restore(testClock(Trace.empty))

    /** An aspect that restores the [[zio.test.TestConsole TestConsole]]'s state to its starting state after the test is run. Note that this
      * is only useful when repeating tests.
      */
    def restoreTestConsole: TestAspectPoly =
        restore(testConsole(Trace.empty))

    /** An aspect that restores the [[zio.test.TestRandom TestRandom]]'s state to its starting state after the test is run. Note that this
      * is only useful when repeating tests.
      */
    def restoreTestRandom: TestAspectPoly =
        restore(testRandom(Trace.empty))

    /** An aspect that restores the [[zio.test.TestSystem TestSystem]]'s state to its starting state after the test is run. Note that this
      * is only useful when repeating tests.
      */
    def restoreTestSystem: TestAspectPoly =
        restore(testSystem(Trace.empty))

    /** An aspect that restores all state in the standard provided test environments ([[zio.test.TestClock TestClock]],
      * [[zio.test.TestConsole TestConsole]], [[zio.test.TestRandom TestRandom]], and [[zio.test.TestSystem TestSystem]]) to their starting
      * state after the test is run. Note that this is only useful when repeating tests.
      */
    ...
    def scala2[LowerR, UpperR, LowerE, UpperE](
        ...
        if TestVersion.isScala2 then that else identity
    ...
    def scala212[LowerR, UpperR, LowerE, UpperE](
        ...
        if TestVersion.isScala212 then that else identity
    ...
    def scala213[LowerR, UpperR, LowerE, UpperE](
        ...
        if TestVersion.isScala213 then that else identity
    ...
    /** An aspect that runs each test with the [[zio.test.TestConsole TestConsole]] instance in the environment set to silent mode so that
      * console output is only written to the output buffer and not rendered to standard output.
      */
    ...
    /** As aspect that runs each test with the default console logger removed so that logs are only written to the output buffer and not
    ...
    object PerTest:
        ...
        /** A `PerTest.Poly` is a `TestAspect.PerTest` that is completely polymorphic, having no requirements ZRTestEnv on error or
          * environment.
          */
