...
// In many cases, we use Kyo's error management via Abort.fail in place of Kyo.die.
import kyo.*
import kyo.Random.*
// Replace Kyo with our Kyo effect type.
// A A < Env[S] & Abort[E] is converted to a computation of type A < Env[S] & Abort[E]
...
final case class Gen[-S, +A](sample: Stream[Sample[S, A], S]):
    ...
    /** Concatenates the specified deterministic generator with this deterministic generator, resulting in a deterministic generator that
      ...
      */
    def concat[S1, A1 <: A](that: Gen[S1, A1])(implicit trace: Trace): Gen[S & S1, A1] =
        Gen[S & S1, A1](self.sample.concat(that.sample))
