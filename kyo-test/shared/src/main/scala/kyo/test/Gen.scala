...
final case class Gen[-R, +A](sample: ZStream[R, Nothing, Sample[R, A]]):
    ...
    /** Concatenates the specified deterministic generator with this determinstic generator, resulting in a deterministic generator that
    ...
    /** Filters the values produced by this generator, discarding any values that do not meet the specified effectual predicate. Using
      * `filterZIO` can reduce test performance, especially if many values must be discarded. It is recommended to use combinators such as
    ...
    def filterZIO[R1 <: R](f: A => ZIO[R1, Nothing, Boolean])(implicit trace: Trace): Gen[R1, A] =
        self.flatMap(a => Gen.fromZIO(f(a)).flatMap(p => if p then Gen.const(a) else Gen.empty))
...
object Gen extends GenZIO with FunctionVariants with TimeVariants:
    ...
    /** A generator of big decimals inside the specified range: [start, end]. The shrinker will shrink toward the lower end of the range
      * ("smallest").
      *
    ...
    /** A generator of [[java.math.BigDecimal]] inside the specified range: [start, end]. The shrinker will shrink toward the lower end of
      * the range ("smallest").
      *
      * The values generated will have a precision equal to the precision of the difference between `max` and `min`.
      * @see
    ...
    /** A generator of big integers inside the specified range: [start, end]. The shrinker will shrink toward the lower end of the range
      * ("smallest").
      */
    def bigInt(min: BigInt, max: BigInt)(implicit trace: Trace): Gen[Any, BigInt] =
        Gen.fromZIOSample {
            if min > max then ZIO.die(new IllegalArgumentException("invalid bounds"))
            else if min == max then ZIO.succeed(Sample.noShrink(min))
            else
    ...
    /** A generator of [[java.math.BigInteger]] inside the specified range: [start, end]. The shrinker will shrink toward the lower end of
      * the range ("smallest").
      * @see
    ...
    def int(min: Int, max: Int)(implicit trace: Trace): Gen[Any, Int] =
        Gen.fromZIOSample {
            if min > max then ZIO.die(new IllegalArgumentException("invalid bounds"))
                ...
                val effect =
                    if max < Int.MaxValue then nextIntBetween(min, max + 1)
                    else if min > Int.MinValue then nextIntBetween(min - 1, max).map(_ + 1)
                    else nextInt
                effect.map(Sample.shrinkIntegral(min))
        }
    ...
    def long(min: Long, max: Long)(implicit trace: Trace): Gen[Any, Long] =
        Gen.fromZIOSample {
            if min > max then ZIO.die(new IllegalArgumentException("invalid bounds"))
                ...
                val effect =
                    if max < Long.MaxValue then nextLongBetween(min, max + 1L)
                    else if min > Long.MinValue then nextLongBetween(min - 1L, max).map(_ + 1L)
                    else nextLong
                effect.map(Sample.shrinkIntegral(min))
        }
