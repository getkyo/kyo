...
private[test] trait SpecVersionSpecific[-R, +E]:
    self: Spec[R, E] =>

    /** Automatically assembles a layer for the spec, translating it up a level.
      */
    inline def provide[E1 >: E](inline layer: ZLayer[?, E1, ?]*): Spec[Any, E1] =
    ...
    /** Automatically constructs the part of the environment that is not part of the `TestEnvironment`, leaving an effect that only depends
      * on the `TestEnvironment`. This will also satisfy transitive `TestEnvironment` requirements with `TestEnvironment.any`, allowing them
      * to be provided later.
      *
      * {{{
      * val zio: Unit < Env[OldLady with Console] & Abort[Nothing] = ???
      * val oldLadyLayer: ZLayer[Fly, Nothing, OldLady] = ???
      * val flyLayer: ZLayer[Blocking, Nothing, Fly] = ???
      *
      * // The TestEnvironment you use later will provide both Blocking to flyLayer and
      * // Console to zio
      * val zio2 : Unit < Env[TestEnvironment] & Abort[Nothing] =
      *   zio.provideCustom(oldLadyLayer, flyLayer)
      * }}}
      */
    @deprecated("use provide", "2.0.2")
    inline def provideCustom[E1 >: E](inline layer: ZLayer[?, E1, ?]*): Spec[TestEnvironment, E1] =
    ...
    /** Automatically assembles a layer for the spec, sharing services between all tests.
      */
    inline def provideShared[E1 >: E](inline layer: ZLayer[?, E1, ?]*): Spec[Any, E1] =
        ${ SpecLayerMacros.provideSharedImpl[Any, R, E1]('self, 'layer) }

    /** Automatically constructs the part of the environment that is not part of the `TestEnvironment`, leaving an effect that only depends
      * on the `TestEnvironment`, sharing services between all tests.
      *
    ...
    inline def provideCustomShared[E1 >: E](inline layer: ZLayer[?, E1, ?]*): Spec[TestEnvironment, E1] =
        ${ SpecLayerMacros.provideSharedImpl[TestEnvironment, R, E1]('self, 'layer) }
end SpecVersionSpecific

final class ProvideSomePartiallyApplied[R0, -R, +E](val self: Spec[R, E]) extends AnyVal:
    inline def apply[E1 >: E](inline layer: ZLayer[?, E1, ?]*): Spec[R0, E1] =
