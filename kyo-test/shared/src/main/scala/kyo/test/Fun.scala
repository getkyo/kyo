...
/** A `Fun[A, B]` is a referentially transparent version of a potentially effectual function from `A` to `B`. Each invocation of the
  * function will be memoized so the function is guaranteed to return the same value for any given input. The function should not involve
  * asynchronous effects.
  */
final private[test] case class Fun[-A, +B] private (private val f: A => B, private val hash: A => Int)
    extends (A => B):
...
private[test] object Fun:
    ...
    /** Constructs a new `Fun` from an effectual function and a hashing function. This is useful when the domain of the function does not
