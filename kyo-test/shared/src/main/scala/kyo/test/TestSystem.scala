...
import kyo.System
import kyo.Unsafe
import kyo.internal.Tracer

trait TestSystem extends System with Restorable:
    def putEnv(name: String, value: String)(implicit trace: Frame): Unit < IO
    def putProperty(name: String, value: String)(implicit trace: Frame): Unit < IO
    def setLineSeparator(lineSep: String)(implicit trace: Frame): Unit < IO
    def clearEnv(variable: String)(implicit trace: Frame): Unit < IO
    def clearProperty(prop: String)(implicit trace: Frame): Unit < IO
end TestSystem
...
object TestSystem extends Serializable:
    ...
    final case class Test(systemState: Ref.Atomic[TestSystem.Data]) extends TestSystem:
        ...
        def env(variable: => String)(implicit trace: Frame): Option[String] < (Env[Any] & Abort[SecurityException]) =
            unsafe.env(variable)(Unsafe.unsafe)

        def envOrElse(variable: => String, alt: => String)(implicit trace: Frame): String < (Env[Any] & Abort[SecurityException]) =
        ...
        def envs(implicit trace: Frame): Map[String, String] < (Env[Any] & Abort[SecurityException]) =
            unsafe.envs()(Unsafe.unsafe)

        def lineSeparator(implicit trace: Frame): String =
            unsafe.lineSeparator()(Unsafe.unsafe)

        def properties(implicit trace: Frame): Map[String, String] < (Env[Any] & Abort[Throwable]) =
            unsafe.properties()(Unsafe.unsafe)

        def property(prop: => String)(implicit trace: Frame): Option[String] < (Env[Any] & Abort[Throwable]) =
            unsafe.property(prop)(Unsafe.unsafe)

        def propertyOrElse(prop: => String, alt: => String)(implicit trace: Frame): String < (Env[Any] & Abort[Throwable]) =
        ...
        def save(implicit trace: Frame): UIO[UIO[Unit]] =
            for
    ...
    def live(data: Data): Layer[Nothing, TestSystem] =
        ...
        Layer.scoped {
            for
                ref <- Ref.unsafe.make(data)(Unsafe.unsafe)
                test = Test(ref)
