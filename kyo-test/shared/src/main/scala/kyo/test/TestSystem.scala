...
/** `TestSystem` supports deterministic testing of effects involving system properties. Internally, `TestSystem` maintains mappings of
  * environment variables and system properties that can be set and accessed. No actual environment variables or system properties will be
  * accessed or set as a result of these actions.
  *
  * {{{
  * import zio.system
...
object TestSystem extends Serializable:
    ...
    /** The default initial state of the `TestSystem` with no environment variable or system property mappings and the system line separator
      * set to the new line character.
      */
    val DefaultData: Data = Data(Map(), Map(), "\n")

    /** Constructs a new `TestSystem` with the specified initial state. This can be useful for providing the required environment to an
      * effect that requires a `Console`, such as with `ZIO#provide`.
      */
    ...
    /** Accesses a `TestSystem` instance in the environment and saves the system state in an effect which, when run, will restore the
      * `TestSystem` to the saved state
      */
    def save(implicit trace: Trace): UIO[UIO[Unit]] =
