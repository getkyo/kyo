    ...
    def missingLayersError(
        ...
        val message =
            if indices.size > 1 then
                s"Please provide layers for the following ${indices.size.toString.underline} types:"
            else
...
"""
    ...
    def missingLayersCustomError(value: Set[String], effectType: String, titleString: String): String =
        ...
        val missingLayersString = missingLayers.zipWithIndex.map { case (layer, index) =>
            val i = s"${index + 1}.".grey
            s"$i $layer"
        }.mkString("\n").indent(3)

        val message =
            if missingLayers.size > 1 then
                s"Your $effectType requires services that are not in the environment.\n".bold +
                    s"Please provide layers for the following ${missingLayers.size.toString.underline} types:"
            else
    ...
    def unusedProvideSomeLayersError(types: List[String]) =
       ...
       |
       |""".stripMargin
    end unusedProvideSomeLayersError
    ...
    def ambiguousLayersError(duplicates: List[(String, List[String])]): String =
        ...
        val message =
            if duplicates.size > 1 then
                s"You have provided more than one layer for the following ${duplicates.size.toString.underline} types:"
            else
...
""".stripMargin
    ...
    def pluralize(n: Int, singular: String, plural: String): String =
        if n == 1 then singular else s"$n $plural"
    ...
    end LayerWiringError
end TerminalRendering
