...
trait TimeVariants:
    ...
    final def dayOfWeek(implicit trace: Frame): Gen[Any, DayOfWeek] =
        Gen.elements(
            DayOfWeek.MONDAY,
            DayOfWeek.TUESDAY,
            DayOfWeek.WEDNESDAY,
    ...
    /** A generator of finite `kyo.Duration` values. Shrinks toward `Duration.Zero`. */
    final def finiteDuration(implicit trace: Frame): Gen[Any, Duration] =
        Gen.long(0L, Long.MaxValue).map(Duration.Finite(_))

    /** A generator of finite `kyo.Duration` values inside the specified range: [min, max]. Shrinks toward min. */
    final def finiteDuration(min: Duration, max: Duration)(implicit trace: Frame): Gen[Any, Duration] =
        Gen.long(min.toNanos, max.toNanos).map(Duration.Finite(_))

    /** A generator of `java.time.Instant` values. Shrinks toward `Instant.MIN`. */
    final def instant(implicit trace: Frame): Gen[Any, Instant] =
    ...
    final def instant(min: Instant, max: Instant)(implicit trace: Frame): Gen[Any, Instant] =
        ...
        def genNano(min: Instant, max: Instant, second: Long): Gen[Any, Long] =
            val minNano = if min.getEpochSecond == second then min.getNano.toLong else 0L
            val maxNano = if max.getEpochSecond == second then max.getNano.toLong else 1000000000L
    ...
    final def localDate(implicit trace: Frame): Gen[Any, LocalDate] =
    ...
    final def localDate(min: LocalDate, max: LocalDate)(implicit trace: Frame): Gen[Any, LocalDate] =
    ...
    final def localDateTime(implicit trace: Frame): Gen[Any, LocalDateTime] =
    ...
    final def localTime(min: LocalTime, max: LocalTime)(implicit trace: Frame): Gen[Any, LocalTime] =
    ...
    final def localTime(implicit trace: Frame): Gen[Any, LocalTime] =
        localTime(LocalTime.MIN, LocalTime.MAX)

    /** A generator of `java.time.Month` values. Shrinks toward `Month.JANUARY`. */
    final def month(implicit trace: Frame): Gen[Any, Month] =
        Gen.elements(
            Month.JANUARY,
            Month.FEBRUARY,
            Month.MARCH,
    ...
    final def monthDay(implicit trace: Frame): Gen[Any, MonthDay] =
        for
    ...
    final def offsetDateTime(implicit trace: Frame): Gen[Any, OffsetDateTime] =
    ...
    final def offsetDateTime(min: OffsetDateTime, max: OffsetDateTime)(implicit trace: Trace): Gen[Any, OffsetDateTime] =
        ...
        def genOffset(min: OffsetDateTime, max: OffsetDateTime, actual: LocalDateTime): Gen[Any, ZoneOffset] =
            ...
            val minOffsetSeconds = if minLocalDate == actualLocalDate then min.getOffset.getTotalSeconds else zoneOffsetMin
            val maxOffsetSeconds = if maxLocalDate == actualLocalDate then max.getOffset.getTotalSeconds else zoneOffsetMax
            if minOffsetSeconds > maxOffsetSeconds then
                ...
                Gen.int(
                    Math.max(secondsAfterMin.toInt, zoneOffsetMin),
                    Math.min(secondsBeforeMax.toInt, zoneOffsetMax)
    ...
    /** A generator of `java.time.OffsetTime` values. Shrinks toward `OffsetTime.MIN`. */
    final def offsetTime(implicit trace: Frame): Gen[Any, OffsetTime] =
        for
            time   <- localTime
            offset <- zoneOffset
    ...
    final def period(implicit trace: Frame): Gen[Any, Period] =
        for
    ...
    final def yearMonth(implicit trace: Frame): Gen[Any, YearMonth] =
        for
            year  <- year
            month <- Gen.int(1, 12)
    ...
    final def yearMonth(min: YearMonth, max: YearMonth)(implicit trace: Frame): Gen[Any, YearMonth] =
    ...
    final def zonedDateTime(implicit trace: Frame): Gen[Any, ZonedDateTime] =
        for
    ...
    final def zonedDateTime(min: ZonedDateTime, max: ZonedDateTime)(implicit trace: Frame): Gen[Any, ZonedDateTime] =
        for
    ...
    final def zoneId(implicit trace: Frame): Gen[Any, ZoneId] =
        Gen.elements(ZoneId.getAvailableZoneIds.asScala.map(ZoneId.of).toList*).noShrink

    /** A generator of `java.time.ZoneOffset` values. Shrinks toward `ZoneOffset.MIN`. */
    final def zoneOffset(implicit trace: Frame): Gen[Any, ZoneOffset] =
