...
sealed case class Summary(
    ...
    final def add(executionEvent: ExecutionEvent)(implicit trace: Frame): Summary =
    ...
    final def add(that: Summary): Summary =
        Summary(
            ...
            self.failureDetails +
                (if that.failureDetails.trim.isEmpty then
                     ""
                 else
                     "\n" + that.failureDetails),
    ...
    final def status: Summary.Status =
        if failureDetails.trim.isEmpty then
            Summary.Success
        else
            Summary.Failure
...
object Summary:
        ...
        final def <>(that: Interval): Interval =
            (self, that) match
                ...
                case (Interval.Finite(start1, end1), Interval.Finite(start2, end2)) =>
                    val start = if start1.isBefore(start2) then start1 else start2
                    val end   = if end1.isAfter(end2) then end1 else end2
                    Interval.Finite(start, end)
    ...
    object Interval:
        ...
        final private case class Finite(start: Instant, end: Instant) extends Interval
    end Interval
