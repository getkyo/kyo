...
/** `TestClock` makes it easy to deterministically and efficiently test effects involving the passage of time.
  *
  * Instead of waiting for actual time to pass, `sleep` and methods implemented in terms of it schedule effects to take place at a given
  * clock time. Users can adjust the clock time using the `adjust` and `setTime` methods, and all effects scheduled to take place on or
  * before that time will automatically be run in order.
  *
  ...
  * }}}
  *
  * Note how we forked the fiber that `sleep` was invoked on. Calls to `sleep` and methods derived from it will semantically block until the
  * time is set to on or after the time they are scheduled to run. If we didn't fork the fiber on which we called sleep we would never get
  * to set the time on the line below. Thus, a useful pattern when using `TestClock` is to fork the effect being tested, then adjust the
  * clock time, and finally verify that the expected effects have been performed.
  *
  ...
  * }}}
  *
  * Here we verify that no effect is performed before the recurrence period, that an effect is performed after the recurrence period, and
  * that the effect is performed exactly once. The key thing to note here is that after each recurrence the next recurrence is scheduled to
  * occur at the appropriate time in the future, so when we adjust the clock by 60 minutes exactly one value is placed in the queue, and
  * when we adjust the clock by another 60 minutes exactly one more value is placed in the queue.
  */
...
object TestClock extends Serializable:
    ...
    final case class Test(
        ...
        /** Increments the current clock time by the specified duration. Any effects that were scheduled to occur on or before the new time
        ...
        /** Sets the current clock time to the specified `Instant`. Any effects that were scheduled to occur on or before the new time will
          * be run in order.
          */
        def setTime(instant: Instant)(implicit trace: Trace): UIO[Unit] =
            warningDone *> run(_ => instant)

        /** Sets the time zone to the specified time zone. The clock time in terms of nanoseconds since the epoch will not be adjusted and
          * no scheduled effects will be run as a result of this method.
          */
        def setTimeZone(zone: ZoneId)(implicit trace: Trace): UIO[Unit] =
            clockState.update(_.copy(timeZone = zone))

        /** Semantically blocks the current fiber until the clock time is equal to or greater than the specified duration. Once the clock
          * time is adjusted to on or after the duration, the fiber will automatically be resumed.
          */
        ...
        /** Captures a "snapshot" of the identifier and status of all fibers in this test other than the current fiber. Fails with the
          * `Unit` value if any of these fibers are not done or suspended. Note that because we cannot synchronize on the status of multiple
          * fibers at the same time this snapshot may not be fully consistent.
          */
        ...
        private def suspended(implicit trace: Trace): IO[Unit, Map[FiberId, Fiber.Status]] =
            freeze.zip(delay *> freeze).flatMap { case (first, last) =>
                if first == last then ZIO.succeed(first)
                else ZIO.fail(())
            }
    ...
    /** Constructs a new `Test` object that implements the `TestClock` interface. This can be useful for mixing in with implementations of
      * other interfaces.
      */
    def live(
        data: Data
    ...
    /** Accesses a `TestClock` instance in the environment and increments the time by the specified duration, running any actions scheduled
    ...
    /** Accesses a `TestClock` instance in the environment and saves the clock state in an effect which, when run, will restore the
      * `TestClock` to the saved state.
      */
    def save(implicit trace: Trace): UIO[UIO[Unit]] =
        testClockWith(_.save)

    /** Accesses a `TestClock` instance in the environment and sets the clock time to the specified `Instant`, running any actions scheduled
      * for on or before the new time in order.
      */
    def setTime(instant: => Instant)(implicit trace: Trace): UIO[Unit] =
        testClockWith(_.setTime(instant))

    /** Accesses a `TestClock` instance in the environment, setting the time zone to the specified time zone. The clock time in terms of
      * nanoseconds since the epoch will not be altered and no scheduled actions will be run as a result of this effect.
      */
    def setTimeZone(zone: => ZoneId)(implicit trace: Trace): UIO[Unit] =
    ...
    /** `Sleep` represents the state of a scheduled effect, including the time the effect is scheduled to run, a promise that can be
      * completed to resume execution of the effect, and the fiber executing the effect.
      */
    final case class Sleep(duration: Duration, promise: Promise[Nothing, Unit], fiberId: FiberId)

    /** `WarningData` describes the state of the warning message that is displayed if a test is using time by is not advancing the
      * `TestClock`. The possible states are `Start` if a test has not used time, `Pending` if a test has used time but has not adjusted the
      * `TestClock`, and `Done` if a test has adjusted the `TestClock` or the warning message has already been displayed.
      */
    ...
    object WarningData:
        ...
        /** State indicating that a test has used time but has not adjusted the `TestClock` with a reference to the fiber that will display
          * the warning message.
          */
    ...
    object SuspendedWarningData:
        ...
        /** State indicating that a test has adjusted the clock but a fiber is still running with a reference to the fiber that will display
          * the warning message.
          */
