...
// Import Kyo effect system modules (assumed names)
import kyo.*
import kyo.concurrent.*
import kyo.test.internal.*
import kyo.time.*
...
trait TestClock extends Clock with Restorable:
    def adjust(duration: Duration)(using Trace): Unit < IO
    def adjustWith[R, E, A](duration: Duration)(effect: A < (Env[R] & Abort[E]))(using Trace): A < (Env[R] & Abort[E])
    def setTime(instant: Instant)(using Trace): Unit < IO
    def setTimeZone(zone: ZoneId)(using Trace): Unit < IO
...
object TestClock:
    ...
    final case class Test(
        ...
        def javaClock(using trace: Trace): java.time.Clock < IO =
            ...
            case class JavaClock(clockState: Ref.Atomic[TestClock.Data], zoneId: ZoneId) extends java.time.Clock:
        ...
        private def suspendedWarningDone(using trace: Trace): Unit < IO =
            suspendedWarningState.updateSomeZIO { case SuspendedWarningData.Pending(fiber) =>
