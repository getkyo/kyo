...
/** Scala.js reflective instantiation for the self-contained runner.
  *
  ...
  * `@EnableReflectiveInstantiation`; `kyo.test.internal.KyoTestReflect` (mixed into `TestBase`) carries it on JS, so all concrete suites
  * are reflectively instantiable. The reflective `newInstance` returns `Any`; a typed pattern match (not a cast) narrows it to `TestBase[?]`.
  */
                ...
                .getOrElse(
                    throw new ClassNotFoundException(
                        s"kyo-test: cannot reflectively instantiate '$fqn'. " +
                            ...
                            "On Scala.js, the kyo.test base carries @EnableReflectiveInstantiation via KyoTestReflect, " +
