...
- **Check, don't assume.** Before naming what a problem is ("slow", "hung", "perf bug", "race", "config issue", "fixture missing"), use a tool to verify. The symptom rarely tells you which: a test that times out could be slow, hung, deadlocked, infinite-looping, or have wrong timeout config; a wrong result could be a decoder bug, a serializer bug, an index miss, or stale state. Concrete diagnostic tools to reach for first: `jstack` for a stuck or looping JVM (a RUNNABLE thread with hundreds of seconds of CPU time in a recognisable hot loop is your answer); `Log` lines and existing timing instrumentation already in the code (often `Log.info(s"init.ms: $ms")` style); a direct one-off measurement when none exists; profiler / async-profiler for hot paths; `git log -L` / `git blame` for "what actually changed"; `strace` / `dtrace` for syscalls. Carrying a hypothesis forward from a prep doc, a code comment, a previous agent's framing, or your own earlier guess IS assuming, even when it sounds reasonable. The most expensive bugs are the ones where someone "knew" the cause and ran with it for an hour before measuring. One `jstack` is seconds; one wrong-direction fix is a phase commit and a revert.
...
- **Cross-platform by default; Kyo is the exception you must internalize.** Source and tests target JVM, JS, and Native, and **Kyo provides full-functionality cross-platform implementations of things you may instinctively assume are JVM-only**. File IO, HTTP clients, HTTP servers, sockets, environment variables, system properties, process management, concurrent primitives, atomic refs, channels, scheduling, time, random, logging, JSON: every one of these has a working JS and Native implementation in some kyo-* module (`kyo-core`, `kyo-http`, `kyo-cache`, ...). The mental model "this needs a JVM" is wrong for Kyo and will produce wrong architecture, wrong test locations (`jvm/src/test` when `shared/` is correct), and wrong scope decisions ("can't be cross-platform because it does X" when X is in fact cross-platform). When in doubt, search for the API in `shared/src/main` of the relevant module before classifying it as JVM-only. Cross-platform consistency is a primary project goal; an API that works on all three platforms is the design target, not a stretch goal. Genuine JVM-only surfaces exist (a dev-only tool wrapping `jstack`, a benchmark harness using JMH, a test that asserts mmap behavior itself, JPMS / `jrt:/` module loading): these are rare exceptions with named per-leaf rationale, not the default. New work goes in `shared/src/main` and `shared/src/test` unless a concrete cross-platform blocker is identified and named; never move a test into a `jvm/` / `js/` / `native/` folder to dodge platform-specific infra cost.

  **Reach for the Kyo primitive, not the raw Java one.** When you would have written `java.nio` or `java.lang.*` or `j.u.c.*` on JVM, the cross-platform answer almost always already exists. Use this lookup before classifying anything as JVM-only:

  | You might reach for                                            | Use this Kyo primitive (works on JVM, JS, Native)                                                  | Module       |
  | -------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- | ------------ |
  | `java.nio.file.{Files, Path, Paths}`                           | `kyo.Path` (`read`, `write`, `list`, `walk`, `remove`, `exists`, `mkDir`, `move`, `copy`)          | `kyo-core`   |
  | `java.lang.System.getenv` / `getProperty` / `lineSeparator`    | `kyo.System` (`env`, `property`, `args`, `lineSeparator`)                                          | `kyo-core`   |
  | `ProcessBuilder` / `Runtime.exec`                              | `kyo.Process` (spawn, pipe, redirect; structured stdout/stderr streams)                            | `kyo-core`   |
  | `OkHttp` / `HttpClient` / `URLConnection`                      | `kyo.http.Http` (client AND server, same API on all three platforms; no HTTP/2 or WebSockets)      | `kyo-http`   |
  | `java.util.concurrent.atomic.*`                                | `kyo.AtomicInt`, `AtomicLong`, `AtomicBoolean`, `AtomicRef`, `LongAdder`                           | `kyo-core`   |
  | `j.u.c.{Channels, BlockingQueue}` / async backpressure         | `kyo.Channel`, `kyo.Queue`, `kyo.Hub`, `kyo.Fiber`                                                 | `kyo-core`   |
  | `j.u.c.{CountDownLatch, Semaphore}` / coordination             | `kyo.Latch`, `kyo.Barrier`, `kyo.Meter`                                                            | `kyo-core`   |
  | `Thread.sleep`, `Instant.now`, time + clock                    | `kyo.Clock` (`now`, `sleep`, `deadline`, `withTimeControl`)                                        | `kyo-core`   |
  | `java.util.Random` / `ThreadLocalRandom`                       | `kyo.Random` (seeded or context-bound; testable)                                                   | `kyo-core`   |
  | SLF4J, Log4j, `println`-as-log                                 | `kyo.Log` (`trace`/`debug`/`info`/`warn`/`error`, level-controlled, structured)                    | `kyo-core`   |
  | `System.console`, `readLine`, `print`                          | `kyo.Console` (`readLine`, `print`, `printLine`, `printErr`)                                       | `kyo-core`   |
  | `Jackson` / `Circe` / `play-json`                              | `derives Schema` + `Json.encode` / `Json.decode`                                                   | `kyo-schema` |
  | Streams / `java.nio.channels`                                  | `kyo.Stream`, `kyo.Pipe`, `kyo.Sink` (chunked push/pull, effect-tracked)                           | `kyo-prelude`|
  | Browser DOM automation (Chrome DevTools)                       | `kyo.browser.*` (cross-platform browser driving)                                                   | `kyo-browser`|
  | Container / Docker / Podman API                                | `kyo.pod.*` (Docker AND Podman, JVM+JS+Native)                                                     | `kyo-pod`    |
  | Test base + platform-gated leaves                              | `extends kyo.test.Test[Any]` + `.onlyJvm` / `.onlyJs` / `.onlyNative` / `.onlyWasm` filters         | `kyo-test`   |

  The README's "Modules" tables ([`README.md`](README.md#modules)) are the source-of-truth for which modules ship JVM ✅ / JS ✅ / Native ✅. If a module has three ticks, every API it exposes is cross-platform; assume the cross-platform answer exists and search `shared/src/main` for it before writing raw `java.*` calls in `shared/src/main`.

- **Platform filters on test leaves are for locking platform-specific MECHANICS, NEVER for dodging a JS / Native contract gap.** The `kyo-test` framework exposes phantom-typed compile-excluding filters: `"name".onlyJvm in { ... }`, `.onlyJs`, `.onlyNative`, `.onlyWasm`, plus negative forms (`.notJvm`, `.notWasm`, ...). They exist for leaves whose purpose IS to assert platform-specific semantics: a `jstack`-driven probe, a JPMS module enumeration, a JMH performance baseline on the JVM heap, a test that mmap actually mmaps. Those are legitimate uses. **Illegitimate uses**: a contract test (e.g. "X converts decoder failures to `TastyError.MalformedSection`") whose JVM run is green and JS run crashes ; gating that with `.onlyJvm` masks an IMPL contract gap on JS. The right fix is to harden the impl so the contract holds on all platforms, then leave the test cross-platform. Before writing `.onlyJvm` / `.onlyJs` / `.onlyNative`, ask: "does this leaf test platform-specific MECHANICS (yes -> filter ok) or a documented CROSS-PLATFORM CONTRACT (no -> fix the impl)?"
...
- **Do not read documentation files or source files as test inputs.** Tests verify library behavior. Documentation (`*.md`) and source files (`*.scala`, `*.java`, ...) are project artifacts under separate review, not test inputs. Loading them via classloader / `EmbeddedText` / `getResourceAsStream` and asserting "this phrase appears" or "this regex does not match" creates a process-artifact test: it does not exercise library behavior, it tracks documentation hygiene. Documentation drift is caught by docs review and by `sbt doctest` (which compiles fenced README blocks); API surface drift is caught by `compiletime.testing.typeCheckErrors`; CI markdown lints catch what those do not. Likewise: no test should read a baseline JSON, a CHANGELOG, an inventory file, or another committed artifact and assert structural properties of its contents. If a test wants to verify "the docstring says X", the docstring belongs in scaladoc that ships with the API, and the verification belongs in `typeCheckErrors` against the API itself, not a regex against the file's text. **The matching corollary on infrastructure**: do not build cross-platform plumbing (resource generators, `EmbeddedText` source generation, custom `TestResourceLoader` text paths) to support process-artifact tests. If you find yourself wiring it up, the test should not exist.
