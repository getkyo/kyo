...
- Actions: `test`, `testDiff`, `compile`, `link`, `linkCheck` plus one or more platforms (`JVM JS Native Wasm Browser BrowserWasm all`), or `sbt <raw command>` to run one arbitrary sbt command in the selected env. `Browser` and `BrowserWasm` run the JS projects' suites in Chrome, which is the row that says whether a page can do the thing, as distinct from Node. `linkCheck` links small representative programs the way an application does and checks their output size, data content, and plain-`node` launch (`project/LinkCheck.scala`); run it after changing dependencies or anything a minimal program links.
