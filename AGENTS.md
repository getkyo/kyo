...
- Actions: `test`, `testDiff`, `compile`, `link`, `linkCheck` plus one or more platforms (`JVM JS Native Wasm all`), or `sbt <raw command>` to run one arbitrary sbt command in the selected env. `linkCheck` links small representative programs the way an application does and checks their output size, data content, and plain-`node` launch (`project/LinkCheck.scala`); run it after changing dependencies or anything a minimal program links.
