...
class AsyncTest extends Test:
    ...
    "run" - {
        ...
        //         val b: Fiber[Int] < Resources = Async.run(a)
    ...
    "runAndBlock" - {
        ...
        "timeout" in runJVM {
            Async.sleep(1.day).andThen(1)
                ...
                .map {
                    case Failure(Timeout(_)) => succeed
                    case v                   => fail(v.toString())
                }
        }

        // "block timeout" in runJVM {
        //     IO.toTry(Async.runAndBlock(10.millis)(
        //         Async.sleep(1.day).andThen(1)
        //     )).map {
        //         case Failure(Async.Interrupted) => succeed
        //         case v                           => fail(v.toString())
        //     }
        // }
        ...
        //     IO.toTry(Async.runAndBlock(10.millis)(
        //         Seqs.fill(100)(Async.sleep(1.milli)).unit.andThen(1)
        //     )).map {
        //         case Failure(Async.Interrupted) => succeed
        //         case v                           => fail(v.toString())
        //     }
    ...
    //     Async.parallelTraverse(Seq(1, 2))(_ + 1).map { r =>
    //         assert(r == Seq(2, 3))
    //     }
    // }
    ...
    //         val io1: (JAtomicInteger & Closeable, Set[Int]) < (Resources & Async) =
    //             for
    ...
    //     "inner" in run {
    //         val resource1 = new Resource
    //         Async.run(Resources.run(Resources.acquire(resource1).map(_.incrementAndGet())))
    //             .map(_.get).map { r =>
    //                 assert(r == 1)
    ...
    //         val resource1 = new Resource
    //         val resource2 = new Resource
    //         val io1: Set[Int] < (Resources & (Async)) =
    //             for
    //                 r  <- Resources.acquire(resource1)
    //                 v1 <- IO(r.incrementAndGet())
    //                 v2 <- Async.run(r.incrementAndGet()).map(_.get)
    ...
    //             yield Set(v1, v2, v3)
    //         Resources.run[Set[Int], Async](io1).map { r =>
    //             assert(r == Set(1, 2))
    //             assert(resource1.get() == -1)
    //             assert(resource2.get() == -1)
    //         }
    ...
    "stack safety" in run {
        def loop(i: Int): Assertion < (IO & Async) =
            if i > 0 then
