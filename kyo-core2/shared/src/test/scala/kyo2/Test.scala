...
abstract class Test extends AsyncFreeSpec with BaseKyoTest[IO & Abort[Any] & Async] with NonImplicitAssertions:
    ...
    def run(v: Future[Assertion] < (IO & Abort[Any] & Async)): Future[Assertion] =
        val a = Async.run(Abort.run(v).map(_.fold(e => throw new IllegalStateException(s"Test aborted with $e"))(identity)))
