...
class ConsoleTest extends Test:
    ...
    "readln" in {
        ...
        val io: String < IO = Console.run(testConsole)(Console.readln)
        assert(IO.run(io).eval == "readln")
    }
    "print" in {
        val testConsole = new TestConsole
        IO.run(Console.run(testConsole)(Console.print("print"))).eval
        assert(testConsole.prints == List("print"))
    }
    "printErr" in {
        val testConsole = new TestConsole
        IO.run(Console.run(testConsole)(Console.printErr("printErr"))).eval
        assert(testConsole.printErrs == List("printErr"))
    }
    "println" in {
        val testConsole = new TestConsole
        IO.run(Console.run(testConsole)(Console.println("println"))).eval
        assert(testConsole.printlns == List("println"))
    }
    "printlnErr" in {
        val testConsole = new TestConsole
        IO.run(Console.run(testConsole)(Console.printlnErr("printlnErr"))).eval
