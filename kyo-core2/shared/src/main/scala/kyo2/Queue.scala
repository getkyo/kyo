...
class Queue[T] private[kyo2] (private[kyo2] val unsafe: Queue.Unsafe[T]):
    ...
    def offer(v: T)(using Frame): Boolean < IO  = op(unsafe.offer(v))
    def poll(using Frame): Option[T] < IO       = op(Option(unsafe.poll()))
    def peek(using Frame): Option[T] < IO       = op(Option(unsafe.peek()))
    def drain(using Frame): Seq[T] < IO         = op(unsafe.drain())
    def isClosed(using Frame): Boolean < IO     = IO(unsafe.isClosed())
    def close(using Frame): Option[Seq[T]] < IO = IO(unsafe.close())

    protected inline def op[T, S](inline v: => T < (IO & S)): T < (IO & S) =
        IO {
...
object Queue:
    ...
    private[kyo2] val closed = IO.fail("Queue closed!")

    abstract private[kyo2] class Unsafe[T]
    ...
    class Unbounded[T] private[kyo2] (unsafe: Queue.Unsafe[T]) extends Queue[T](unsafe):
        def add[S](v: T < S)(using Frame): Unit < (IO & S) =
            op(v.map(offer).unit)

    def init[T](capacity: Int, access: Access = Access.Mpmc): Queue[T] < IO =
        IO {
            capacity match
                case c if (c <= 0) =>
                    new Queue(
    ...
    def initUnbounded[T](access: Access = Access.Mpmc, chunkSize: Int = 8): Unbounded[T] < IO =
        IO {
            access match
                case Access.Mpmc =>
    ...
    def initDropping[T](capacity: Int, access: Access = Access.Mpmc): Unbounded[T] < IO =
    ...
    def initSliding[T](capacity: Int, access: Access = Access.Mpmc): Unbounded[T] < IO =
        init[T](capacity, access).map { q =>
            val u = q.unsafe
            val c = capacity
            new Unbounded(
