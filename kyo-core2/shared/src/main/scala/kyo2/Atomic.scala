...
object Atomic:
    ...
    def initInt(v: Int)(using Frame): OfInt < IO             = IO(OfInt(new AtomicInteger(v)))
    def initLong(v: Long)(using Frame): OfLong < IO          = IO(OfLong(new AtomicLong(v)))
    def initBoolean(v: Boolean)(using Frame): OfBoolean < IO = IO(OfBoolean(new AtomicBoolean(v)))
    def initRef[T](v: T)(using Frame): OfRef[T] < IO         = IO(OfRef(new AtomicReference(v)))
