...
abstract class Channel[T]:
    ...
    def offerUnit(v: T)(using Frame): Unit < (Abort[Closed] & IO)

    def poll(using Frame): Maybe[T] < (Abort[Closed] & IO)
    ...
    def put(v: T)(using Frame): Unit < (Abort[Closed] & Async & IO) =
        putFiber(v).map(_.get)

    def take(using Frame): T < (Abort[Closed] & Async & IO) =
        takeFiber.map(_.get)
