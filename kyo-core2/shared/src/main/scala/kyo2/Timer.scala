...
abstract class Timer:
    ...
    def schedule(delay: Duration)(f: => Unit < (IO & Async))(using Frame): TimerTask < IO

    def scheduleAtFixedRate(
        initalDelay: Duration,
        period: Duration
    )(f: => Unit < (IO & Async))(using Frame): TimerTask < IO
...
object Timer:
    ...
    def apply(exec: ScheduledExecutorService): Timer =
        new Timer:
            ...
            private def eval(f: => Unit < (IO & Async)): Unit =
                discard(IO.run(Async.run(f)).eval)

            def schedule(delay: Duration)(f: => Unit < (IO & Async))(using Frame) =
