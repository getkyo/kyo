...
import kyo2.internal.FiberPlatformSpecific
import kyo2.kernel.*
...
object Async:
    ...
    def timeout[E, A, Ctx](d: Duration)(v: => A < (Abort[E | Timeout] & Async & Ctx))(
        ...
        Async.run(v).map { f =>
            Timer.schedule(d)(f.interruptUnit(Result.Panic(Timeout(frame)))).map { t =>
    ...
    private[kyo2] def use[E, A, B, S](v: IOPromise[E, A])(f: A => B < S)(
        using
        reduce: Reducible[Abort[E]],
        frame: Frame
