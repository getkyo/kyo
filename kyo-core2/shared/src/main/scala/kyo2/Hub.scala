...
object Hubs:
    ...
    class Listener[T] private[kyo2] (hub: Hub[T], child: Channel[T]):
        ...
        def take(using Frame): T < (Abort[Closed] & Async & IO) = child.take
...
class Hub[T] private[kyo2] (
    ...
    def put(v: T)(using Frame): Unit < (Abort[Closed] & IO & Async) = ch.put(v)

    def isClosed(using Frame): Boolean < IO = ch.isClosed

    def close(using frame: Frame): Maybe[Seq[T]] < IO =
        fiber.interruptUnit(Result.Fail(Closed("Hub", initFrame, frame))).andThen {
            ch.close.map { r =>
                IO {
