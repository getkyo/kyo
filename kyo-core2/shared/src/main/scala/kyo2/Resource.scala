...
object Resource:
    ...
    case class Finalizer(createdAt: Frame, queue: Queue.Unbounded[Unit < (Async & IO)])

    def ensure(v: => Unit < (Async & IO))(using frame: Frame): Unit < (Resource & IO) =
    ...
    def run[T, S](v: T < (Resource & S))(using frame: Frame): T < (Async & IO & S) =
