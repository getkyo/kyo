...
class McpContentTest extends Test:
    ...
    "ResourceLink round-trip preserves fields" in {
        val uri     = McpResourceUri("file:///a")
        val content = McpContent.ResourceLink(uri, "a-file", Absent, Absent, McpContent.Annotations.empty)
        assert(roundtrip[McpContent](content) == content)
    }
    ...
    "McpContent.embedded constructs an EmbeddedResource" in {
        val uri     = McpResourceUri("file:///r")
        val rc      = McpHandler.ResourceContents.text(uri, "x")
        val content = McpContent.embedded(rc)
        content match
            case er: McpContent.EmbeddedResource => assert(er.resource == rc)
            case _                               => fail("expected EmbeddedResource")
    }
