...
class McpCapabilityGateTest extends Test:
    ...
    // LogOnly: emits Log.warn for a missing-capability method (verified via Log.let + Log.flush).
    "LogOnly mode: emits Log.warn when capability is absent" in {
        // Unsafe: AtomicBoolean to observe whether warn was called from the gate's Log.warn side-effect.
        val warnCalled = AtomicBoolean.Unsafe.init(false)(using AllowUnsafe.embrace.danger)

        // Extend ConsoleLogger; override emit (not warn) to intercept the daemon dispatch path.
        // The daemon calls event.sink.emit(event), not the per-level methods directly.
        class CapturingLog extends Log.Unsafe.ConsoleLogger("test", Log.Level.warn):
            override def emit(event: Log.Event)(using allow: AllowUnsafe): Unit =
                if event.level == Log.Level.warn then
                    warnCalled.set(true)(using allow)
        end CapturingLog
        ...
        Log.let(Log(new CapturingLog)) {
            for
                dec <- gate.beforeDispatch(requestEnv("tools/list"))
                _   <- Log.flush
                saw = warnCalled.get()(using AllowUnsafe.embrace.danger)
            yield
                assert(dec == JsonRpcMessageGate.Decision.Allow, s"LogOnly must Allow, got $dec")
                assert(saw, "LogOnly must emit Log.warn for unadvertised capability")
            end for
        }
    }

    // Off mode: missing-capability request yields Allow with no Log.warn emitted.
    // Contrast arm for the LogOnly test: Off is JsonRpcMessageGate.noop (no warn, no reject).
    "Off mode: admits missing-capability request with no warn" in {
        // Unsafe: AtomicBoolean to confirm no warn is emitted when the gate is Off.
        val warnCalled = AtomicBoolean.Unsafe.init(false)(using AllowUnsafe.embrace.danger)
        ...
        Log.let(Log(new CapturingLog)) {
            for
                ...
                saw = warnCalled.get()(using AllowUnsafe.embrace.danger)
            yield
                assert(dec == JsonRpcMessageGate.Decision.Allow, s"Off must Allow, got $dec")
                assert(!saw, "Off must not emit Log.warn")
            end for
        }
    }
