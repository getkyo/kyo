...
class McpResourceListReadTest extends Test:
    ...
    private val resRoute = McpHandler.resource(fileUri, "a-file") {
        Chunk(McpHandler.ResourceBody.text("content-a"))
    }
    private val tmplRoute = McpHandler.resourceTemplate(tmplUri, "file-tmpl") { _ =>
        Chunk(McpHandler.ResourceBody.text("content-tmpl"))
    }
    ...
    "readResource returns Text contents for the registered URI" in {
        JsonRpcTransport.inMemory.flatMap { (ts, tc) =>
            Async.zip[McpException, McpServer, McpClient, Any](
                ...
                client.readResourceRaw(fileUri).flatMap { contents =>
                    for
                        _ <- srv.closeNow
                        _ <- client.closeNow
                    yield
