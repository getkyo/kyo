...
class McpUserDefinedErrorTest extends Test:
    ...
    "tool handler aborts with user-defined error type via .error[E2]" in {
        val readTool = McpHandler
            ...
            .error[FsError](code = -40001, message = "filesystem-error")
        ...
        JsonRpcTransport.inMemory.flatMap { (ts, tc) =>
            Async.zip[McpException, McpServer, McpClient, Any](
                ...
                Abort.run[McpException](
                    client.callToolRaw[ReadReq]("read", ReadReq("/etc/passwd"))
                ).flatMap { result =>
                    for
                        _ <- srv.closeNow
                        _ <- client.closeNow
                    yield
                        ...
                        result match
                            case Result.Failure(err) =>
                                assert(err.code == -40001, s"expected code -40001, got ${err.code}")
                                assert(
    ...
    "tool handler happy path still works alongside .error[E2]" in {
        ...
        JsonRpcTransport.inMemory.flatMap { (ts, tc) =>
            Async.zip[McpException, McpServer, McpClient, Any](
                ...
                client.callToolRaw[ReadReq]("read", ReadReq("hello.txt")).flatMap { outcome =>
                    for
                        _ <- srv.closeNow
                        _ <- client.closeNow
                    yield
