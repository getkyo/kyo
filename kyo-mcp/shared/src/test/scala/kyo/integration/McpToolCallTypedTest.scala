...
class McpToolCallTypedTest extends Test:
    ...
    private val addRoute = McpHandler.toolMulti[AddIn]("add") { in =>
    ...
    "callToolTyped[In, Out] returns decoded Out when structuredContent is Present" in {
        JsonRpcTransport.inMemory.flatMap { (ts, tc) =>
            Async.zip[McpException | Closed, McpServer, McpClient, Any](
                ...
                client.callToolTyped[AddIn, Sum]("add", AddIn(2, 3)).flatMap { result =>
                    for
                        _ <- srv.closeNow
                        _ <- client.closeNow
    ...
    "callToolTyped[In, Out] returns correct value for multiple calls" in {
        JsonRpcTransport.inMemory.flatMap { (ts, tc) =>
            Async.zip[McpException | Closed, McpServer, McpClient, Any](
                McpServer.initUnscoped(ts, addRoute),
                McpClient.initUnscoped(tc, McpInfo("typed"), McpCapabilities.Client())
            ).flatMap { (srv, client) =>
                client.callToolTyped[AddIn, Sum]("add", AddIn(10, 20)).flatMap { r1 =>
                    client.callToolTyped[AddIn, Sum]("add", AddIn(0, 0)).flatMap { r2 =>
                        for
                            _ <- srv.closeNow
                            _ <- client.closeNow
                        yield
