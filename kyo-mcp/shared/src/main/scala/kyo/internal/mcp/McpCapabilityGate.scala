                    ...
                    def beforeDispatch(env: JsonRpcEnvelope)(using Frame): JsonRpcMessageGate.Decision < Sync =
                        env match
                            case req: JsonRpcRequest =>
                                missingCapability(req.method, serverCaps) match
                                    case Present(capName) =>
                                        Log.warn(
                                            s"Admitting '${req.method}' despite unadvertised capability '${capName.toString.toLowerCase}' (LogOnly gate)."
                                        )
                                            .andThen(JsonRpcMessageGate.Decision.Allow)
                                    case Absent =>
