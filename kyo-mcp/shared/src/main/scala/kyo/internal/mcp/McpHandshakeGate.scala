...
private[kyo] object McpHandshakeGate:
    ...
    def server(order: McpConfig.HandshakeOrder): JsonRpcMessageGate =
        // Unsafe: AtomicBoolean for thread-safe flags shared across handler fibers.
        ...
        new JsonRpcMessageGate:
            def beforeDispatch(env: JsonRpcEnvelope)(using Frame): JsonRpcMessageGate.Decision < Sync =
                env match
                    case req @ JsonRpcRequest(_, "initialize", _, _) =>
                        initRequestSeen.getAndSet(true).map { alreadySeen =>
                            if alreadySeen then
                                val err = McpHandshakeAlreadyInitializedException(req.method)
