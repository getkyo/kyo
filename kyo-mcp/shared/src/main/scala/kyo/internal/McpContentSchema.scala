...
/** Hand-rolled discriminator-key Schemas for `McpContent` and `McpHandler.ResourceContents`.
  ...
  *
  * The `annotations` field uses the empty pattern: the wire encoder omits the field when the
  * runtime value equals `McpContent.Annotations.empty`. The wire decoder restores `empty` when
...
private[kyo] object McpContentSchema:
    ...
    val contentSchema: Schema[McpContent] = new Schema[McpContent](Seq.empty):
        ...
        @publicInBinary private[kyo] def serializeWrite(c: McpContent, w: Codec.Writer): Unit =
            c match
                case McpContent.Text(text, annotations) =>
                    val emitAnn    = annotations != McpContent.Annotations.empty
        ...
        @publicInBinary private[kyo] def serializeRead(reader: Codec.Reader): McpContent =
            ...
            var resource: Maybe[McpHandler.ResourceContents] = Absent
            var annotations: McpContent.Annotations          = McpContent.Annotations.empty
            ...
            while i < n do
                ...
                if reader.matchField("type".getBytes("UTF-8")) then
                    ...
                    resource = Present(summon[Schema[McpHandler.ResourceContents]].serializeRead(reader))
                ...
                case Structure.Value.Record(fields) =>
                    ...
                    val annotationsResult: Result[DecodeException, McpContent.Annotations] =
                        m.get("annotations") match
                            ...
                            case scala.None => Result.Success(McpContent.Annotations.empty)
                    m.get("type") match
