...
A server is built from `McpHandler` values: one per tool, resource, resource template, prompt, or completion endpoint. You hand a varargs list of handlers to `McpServer.init` and the engine takes care of the rest: the `initialize` handshake, capability auto-derivation from registered handlers, protocol-version negotiation, capability gating, progress and cancellation routing, and lifting each handler into a `JsonRpcRoute`. Inside a handler body the live peer is reachable through the `Mcp.*` accessors (`Mcp.server` for reverse-direction calls back to the client, `Mcp.progress` for monotonic progress notifications, `Mcp.cancelled` for the cancellation promise). The same `McpHandler` type also drives the client side for reverse-direction methods (`sampling/createMessage`, `roots/list`, `elicitation/create`).
...
Each cursor-paginated list returns `McpClient.Page[A](items, nextCursor, meta)` so the page boundary is named, not the tuple-positional `(Chunk[A], Maybe[String])` shape. The `meta: Maybe[Structure.Value]` field is the spec's advisory `_meta` carve-out; the same `_meta` passthrough rides on `ToolOutcome` and `PromptOutcome` and is the only place the typed API hands you the raw JSON shape.
...
> **Note:** `tool` and `toolMulti` have different contracts. `tool` returns one `Out <: McpContent` leaf and the engine wraps it in `ToolOutcome(content = Chunk(out))`; `toolMulti` returns a full `ToolOutcome` directly. Pick `tool` when one content leaf suffices; reach for `toolMulti` when the tool emits a mixed bag (text plus an image, say) or wants to populate the typed `structuredContent` slot that `callToolTyped[In, Out]` decodes against.

> **Caution:** `tool` and `toolMulti` are declared `inline def` specifically so `Json.jsonSchema[In]` expands at the user's call site against a concrete `In`. Calling them in generic code with an abstract `In` produces a tool whose wire `inputSchema` is missing properties and required fields. If you wrap the factories in your own helper, keep the helper `inline` and propagate `[In]` to the call site rather than fixing it inside the wrapper.

`ToolAnnotations.noop` is the empty-record default; the factory detects equality with `.noop` and translates it to wire-`Absent` so the field is omitted from the JSON envelope. `ResourceAnnotations.noop` follows the same noop-omit pattern.
...
The wire codecs for these four types use a private `fromWire` constructor that admits any value without validation, so peer-supplied strings come through unchecked. Your `parse` call at the boundary is the only filter.
...
`McpCapabilityNotAdvertisedException` carries a `capabilityName: McpCapabilities.Name` field; the enum's eight cases (`Tools`, `Resources`, `Prompts`, `Sampling`, `Roots`, `Logging`, `Completions`, `Elicitation`) let handler code pattern-match on which capability the peer required.

### JSON-RPC defaults

MCP layers four protocol-specific policies on top of the bare JSON-RPC handler config; the result lives at `McpConfig.defaultJsonRpcConfig`, an embedded `JsonRpcHandler.Config` pre-populated with: `McpCancellationPolicy.default` (the MCP `notifications/cancelled` shape), `McpProgressPolicy.default` (the MCP `notifications/progress` shape with `_meta.progressToken` extraction), `McpUnknownMethodPolicy.default` (the strict preset), and `JsonRpcCodec.Strict2_0`. Override individual slots via `.jsonRpc(...)`; for example, to allow long-running tools with no per-request timeout, the default already sets `Duration.Infinity`.
