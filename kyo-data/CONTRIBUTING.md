...
`Span`, `Dict`, `OrderedDict`, `Duration`, `Instant`, `TypeMap`, `Tag`, `Record`, ...).
...
`Dict` and `OrderedDict` are opaque types whose runtime representation is one of two things,
chosen by size:

```scala
opaque type Dict[K, V]       = Span[K | V] | HashMap[K, V]        // Dict.scala:66
opaque type OrderedDict[K, V] = Span[K | V] | TreeSeqMap[K, V]     // OrderedDict.scala:70
```
...
(`OrderedDict`) [kyo-data/shared/src/main/scala/kyo/Dict.scala:8-12,
kyo-data/shared/src/main/scala/kyo/OrderedDict.scala:7-13]. The threshold is a single named
constant in each companion, not a literal at use sites
[kyo-data/shared/src/main/scala/kyo/Dict.scala:70,
kyo-data/shared/src/main/scala/kyo/OrderedDict.scala:74]:

```scala
private[kyo] val threshold = 8
```

`Dict.scala` and `OrderedDict.scala` are structurally parallel. When you change one, read the
...
[kyo-data/shared/src/main/scala/kyo/OrderedDict.scala:150,
kyo-data/shared/src/main/scala/kyo/OrderedDict.scala:175,
...
[kyo-data/shared/src/main/scala/kyo/OrderedDict.scala:214], and removal closes the gap with an
`arraycopy` over the value half [kyo-data/shared/src/main/scala/kyo/OrderedDict.scala:252].
...
kyo-data/shared/src/main/scala/kyo/OrderedDict.scala:621-626]:
...
### Adding an operation to `Dict` or `OrderedDict`
...
- Respect the threshold in both directions. A result of size `<= threshold` should be in the
  ...
  [kyo-data/shared/src/main/scala/kyo/OrderedDictBuilder.scala:60-64].
- Keep the tuple-free shape. Iteration, transformation, and lookup take separate key and value
  ...
  kyo-data/shared/src/main/scala/kyo/OrderedDict.scala:20-21]. A new method that takes or returns
...
`Dict` and `OrderedDict` deliberately provide no `CanEqual` instance. Structural equality is the
`is` method [kyo-data/shared/src/main/scala/kyo/Dict.scala:55,
kyo-data/shared/src/main/scala/kyo/Dict.scala:586,
kyo-data/shared/src/main/scala/kyo/OrderedDict.scala:58,
kyo-data/shared/src/main/scala/kyo/OrderedDict.scala:595]:
...
## `OrderedDict` order is a contract

The order semantics are specified, not incidental to the backing collection
[kyo-data/shared/src/main/scala/kyo/OrderedDict.scala:15-18]:
...
unspecified [kyo-data/shared/src/main/scala/kyo/OrderedDict.scala:17-18]. That difference is the
reason both types exist. A change to `OrderedDict` that preserves entries but not their order is a
breaking change even though every `is` comparison still passes.

### `is` does not compare order

`OrderedDict.is` is order-independent by specification: it checks size and then per-key value
equality [kyo-data/shared/src/main/scala/kyo/OrderedDict.scala:592-600]. So `is` cannot witness the
contract above. An assertion about order must go through an order-bearing projection: `toChunk`,
`keys`, or `values` [kyo-data/shared/src/test/scala/kyo/OrderedDictTest.scala:236,
kyo-data/shared/src/test/scala/kyo/OrderedDictTest.scala:291]. `foreach`, `foreachKey`, and
`foreachValue` likewise traverse in insertion order
[kyo-data/shared/src/main/scala/kyo/OrderedDict.scala:275,
kyo-data/shared/src/main/scala/kyo/OrderedDict.scala:289,
kyo-data/shared/src/main/scala/kyo/OrderedDict.scala:303].

## Builders pool their buffers in a `ThreadLocal`

`ChunkBuilder`, `DictBuilder`, and `OrderedDictBuilder` each cache their scratch buffer in a
...
kyo-data/shared/src/main/scala/kyo/OrderedDictBuilder.scala:77]:
...
- The buffer is cleared on release, not on acquire
  [kyo-data/shared/src/main/scala/kyo/OrderedDictBuilder.scala:83-85]. A builder that releases a
  buffer it still references hands live data to the next acquirer.
- `result()` releases the buffer and drops the reference
  [kyo-data/shared/src/main/scala/kyo/OrderedDictBuilder.scala:67-69]. A builder is not reusable
  after `result()`, and a new builder method must preserve that.

## Tests

Tests extend the module's test base, `kyo.test.Test[Any]`
[kyo-data/shared/src/test/scala/kyo/OrderedDictTest.scala:3]. Tests for the public data types live
...
1. **Insert each fixture in an order that differs from both the sorted order and the hash order of
   ...
   `Seq(3, 1, 4, 0, 9, 2, 6, 5, 8, 7)` [kyo-data/shared/src/test/scala/kyo/OrderedDictTest.scala:5],
...
2. **Let the assertion follow the fixture rather than repeat it**, so the two cannot drift apart:
   ...
   [kyo-data/shared/src/test/scala/kyo/OrderedDictTest.scala:236,
   kyo-data/shared/src/test/scala/kyo/OrderedDictTest.scala:239].
...
[kyo-data/shared/src/test/scala/kyo/OrderedDictBuilderTest.scala:72].
...
on `Dict` or `OrderedDict` needs coverage at both sides of the threshold, and the promotion and
demotion boundaries are where the bugs are
[kyo-data/shared/src/test/scala/kyo/OrderedDictBuilderTest.scala:63-75].
...
- [ ] A new `Dict` / `OrderedDict` operation dispatches through `reduce` and handles both arms.
...
- [ ] No `==` on `Dict`, `OrderedDict`, or `Span`; use `is`.
- [ ] A structural change to `Dict.scala` or `OrderedDict.scala` was considered for its parallel.
