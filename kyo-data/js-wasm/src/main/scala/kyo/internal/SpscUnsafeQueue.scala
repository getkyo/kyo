...
/** Bounded SPSC queue — JS single-threaded simplification of JCTools SpscAtomicArrayQueue.
  *
  * Uses a plain ring buffer with head/tail indices. No atomics needed on JS. Minimum capacity is 4 (matches JVM/Native).
  */
final private[kyo] class SpscUnsafeQueue[A](requestedCapacity: Int) extends UnsafeQueue[A]:

    private val actualCapacity = UnsafeQueue.roundToPowerOfTwo(Math.max(4, requestedCapacity))
