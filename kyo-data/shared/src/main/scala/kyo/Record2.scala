...
/** A type-safe, immutable record that maps string field names to values using intersection types.
  *
  * Record2 encodes its schema as an intersection of `Name ~ Value` pairs in the type parameter `F`. For example,
  * `Record2["name" ~ String & "age" ~ Int]` describes a record with a `name` field of type `String` and an `age` field of type `Int`.
  * Fields are created with the `~` extension on `String` and combined with `&`. Case classes can also be converted via `fromProduct`.
  *
  * =Subtyping via Implicit Conversion=
  * The type parameter `F` is '''invariant''', but an implicit `widen` conversion in the companion object allows a `Record2[A]` to be used
  * wherever a `Record2[B]` is expected, provided `A <: B`. Since `"name" ~ String & "age" ~ Int <: "name" ~ String` by Scala's intersection
...
object Record2:
    ...
    /** An empty record with type `Record2[Any]`, which is the identity element for `&`. */
    val empty: Record2[Any] = new Record2(Dict.empty[String, Any])

    /** Implicit conversion that enables structural subtyping for Record2. Since `F` is invariant, this conversion allows a `Record2[A]` to
      * be used where a `Record2[B]` is expected whenever `A <: B`. This is safe because the underlying `Dict` storage is read-only, and the
    ...
    /** Creates a record from a case class or other `Product` type. Each product element becomes a field whose name matches the element
      * label. The return type is a `Record2` with the appropriate field intersection, inferred transparently by the macro. Fails at compile
