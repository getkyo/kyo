...
/** Whether a throwable is one kyo refuses to carry as a value, and lets escape instead.
  ...
  * the program is wrong rather than the runtime, and kyo delivers interrupts itself. The distinction matters because a fatal is re-thrown
  * out of the fiber into the scheduler worker, ending it along with every release that computation still owed.
  *
  ...
  *
  * `ThreadDeath` is absent because `Thread.stop` is gone from the platform. Everything else becomes a `Panic`: the fiber ends, its
