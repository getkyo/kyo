...
class OrderedDictBuilderTest extends kyo.test.Test[Any]:

    "add" - {
        "accumulates fields in insertion order" in {
            val b = OrderedDictBuilder.init[String, String]
    ...
    "pool reentrancy" - {
        "nested/interleaved builders do not alias" in {
            val b1 = OrderedDictBuilder.init[String, Int]
            val b2 = OrderedDictBuilder.init[String, Int]
            b1.add("a", 1)
            b2.add("x", 9)
            b1.add("b", 2)
            b2.add("y", 8)
    ...
    "initTransform" - {
        "used as a foreachEntry sink preserves source order" in {
            ...
            val sink   = OrderedDictBuilder.initTransform[String, Int, String, Int]((b, k, v) => discard(b.add(k, v)))
            source.foreachEntry(sink)
            assert(sink.result().toChunk.map(_._1) == Chunk("mike", "yankee", "delta"))
        }
    }

end OrderedDictBuilderTest
