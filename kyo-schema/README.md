...
`maxLineBytes` (default `Json.Lines.DefaultMaxLineBytes`, 16 MiB) bounds a single record. Without it a byte stream that never contains a newline would grow the framer's residual without bound, and neither `maxDepth` nor `maxCollectionSize` covers that, since both operate inside one document. A breach surfaces as `Framed.error`, and in `decodeAllResults` as a final `LimitExceededException` failure after every record decoded before it. It is the one failure that cannot be skipped past, because no record boundary was found to skip to.
