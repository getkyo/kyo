...
The Scala type the instant kind carries is `kyo.Instant`, not `java.time.Instant`, and that is load bearing rather than a preference: a `java.time.Instant` builds its text through `DateTimeFormatter`, which reaches `java.util.Locale` and the CLDR tables, so a model that named it put a locale database into every JavaScript program with a schema in it. `Schema[java.time.Instant]` still exists and still writes the same wire primitive, built with `Schema.init` like every other `java.time` schema here so that it reports its own Scala type and keeps its absent default. Keep new time-like primitives on kyo's own types for the same reason.
