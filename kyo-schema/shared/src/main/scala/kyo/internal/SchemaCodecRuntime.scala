...
private[kyo] object SchemaCodecRuntime:
    ...
    /** Construct a `Schema[A]` for a derived sealed trait.
      ...
      */
    /** Builds a sealed-type Schema with tied-knot recursion.
      *
      * The `schemasBuilder` parameter takes the in-flight parent `Schema[A]` as an argument so
      * recursive variants (e.g. `Schedule.AndThen(a: Schedule, b: Schedule)`) can close over the
      * parent at emission time instead of looping back through `summonInline[Schema[A]]`. The macro
      * emits the lambda body referring to its parameter for parent-typed variant fields.
      */
    def buildSumSchema[A](
        meta: SumVariantsMeta,
        matchVariant: A => Int,
        schemasBuilder: Schema[A] => Array[Schema[Any]],
        sourceFields: Seq[kyo.Field[?, ?]],
        tag: kyo.Tag[Any],
        ...
        new Schema[A](Seq.empty, sourceFields = sourceFields):
            import scala.annotation.publicInBinary
            private lazy val schemas: Array[Schema[Any]] = schemasBuilder(this)
            @publicInBinary private[kyo] def serializeWrite(value: A, writer: Writer): Unit =
