...
@publicInBinary private[kyo] object FocusMacro:
    ...
    private def emitProductSchema[A: Type](using
        ...
        // Breaks the recursive-variant cycle when a sealed parent's variant has a field whose type
        // equals the parent (e.g. `case class AndThen(a: Schedule, b: Schedule)`). The variant field
        // schema closes over the parent's in-flight Schema instead of re-entering the derivation via
        // `summonInline[Schema[parent]]`, which would loop.
        ...
        val schemasArrayBuilderExpr: Expr[() => Array[Schema[Any]]] =
            val schemaExprs: List[Expr[Schema[Any]]] = effectiveSchemaTypes.map { effective =>
                // Recursive-variant cycle break: when this field's type is the sealed parent currently
                // being emitted, close over the parent's in-flight Schema via `parentSelf` instead of
                // emitting `summonInline[Schema[parent]]` (which would re-enter `Schema.derived` and
                // loop). Restores the pre-campaign "any sealed type, no source-side derives" behavior.
                parentSelf match
                    case Some((parentTpe, selfRef)) if effective =:= parentTpe =>
                        '{ ${ selfRef.asExprOf[Schema[?]] }.asInstanceOf[Schema[Any]] }
                    case _ =>
    ...
    /** Emits `Schema[A]` for a sealed trait or enum.
      ...
      * leaves without their own `derives Schema` resolve correctly. The recursive-variant cycle is
      * broken by emitting the whole Schema as a `lazy val self: Schema[A] = ...` block and threading
      * a `parentSelf` reference into the variant emission: variant fields whose type equals the parent
      * close over `self` instead of re-entering the derivation via `summonInline[Schema[parent]]`.
      * This lets the macro derive recursive sealed types (e.g. `Schedule`) at the call site without
      * the source needing to declare `derives Schema` itself.
      */
    private def emitSealedSchema[A: Type](using
        ...
        // Tied-knot lambda producing the variant Schema array. The `parent` parameter is the
        // in-flight `Schema[A]` (passed by `buildSumSchema` at first use); variants whose fields
        // reference the parent type close over `parent` instead of re-entering the derivation via
        // `summonInline[Schema[parent]]`. Built via `Lambda(...)` at the Term level so the parameter
        // Ref and the variant emission share the macro's Quotes context (a nested `'{ ${...} }` quote
        // would put them in different contexts and trip a type-mismatch on the threaded `parentSelf`).
        // Restores the pre-campaign "derive any sealed type without requiring source-side
        // `derives Schema`" behavior.
        val lambdaType = MethodType(List("parent"))(
            _ => List(TypeRepr.of[Schema[A]]),
            _ => TypeRepr.of[Array[Schema[Any]]]
        )
        val lambdaTerm: Term = Lambda(
            Symbol.spliceOwner,
            lambdaType,
            (methSym, paramRefs) =>
                val parentTerm                           = paramRefs.head.asInstanceOf[Term]
                val parentSelf: Option[(TypeRepr, Term)] = Some((tpe, parentTerm))
                val schemaExprs: List[Expr[Schema[Any]]] = children.map { child =>
                    ...
                    childType.asType match
                        case '[t] =>
                            val vSchema = emitVariantSchema[t](childType, child, parentSelf)
                            '{ $vSchema.asInstanceOf[Schema[Any]] }
                    end match
                }
                '{ Array[Schema[Any]](${ Varargs(schemaExprs) }*) }.asTerm.changeOwner(methSym)
        )
        val schemasArrayBuilderExpr: Expr[Schema[A] => Array[Schema[Any]]] =
            lambdaTerm.asExprOf[Schema[A] => Array[Schema[Any]]]
