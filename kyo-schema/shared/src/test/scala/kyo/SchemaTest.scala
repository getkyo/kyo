...
class SchemaTest extends Test:
    ...
    "derivation errors" - {
        ...
        // After Phase 11 of the kyo-schema followup, the `Schema.apply` (metaApply) path no longer silently demotes
        // case classes whose fields have no `given Schema[FieldType]` to a no-serialization Schema. Derivation now
        // always emits the serialization path and fails loudly at compile time (the surfaced message comes from the
        // per-field summon's recursive `Schema.derived` call, which rejects the opaque type as not derivable).
        "metaApply: case class with non-derivable field fails loudly" in {
            typeCheckFailure(
                "case class TWithOpaque(x: kyo.MTOpaque); kyo.Schema[TWithOpaque]"
            )("kyo.MTOpaque")
        }

        "Structure.of: case class with Instant field still compiles (regression)" in {
            val s = kyo.Structure.of[kyo.MTPerson]
            assert(s.isInstanceOf[kyo.Structure.Type.Product])
        }
    }
    ...
    "intersection types" - {
        ...
        "two traits with disjoint fields round-trip via anonymous synthesis" in {
            given Schema[MTIxFoo & MTIxBar] = Schema.derived[MTIxFoo & MTIxBar]
            val value: MTIxFoo & MTIxBar = new MTIxFoo with MTIxBar:
                def a = 1
                def b = "hi"
            val encoded = Json.encode(value)
            assert(encoded.contains("\"a\":1"))
            assert(encoded.contains("\"b\":\"hi\""))
            val decoded = Json.decode[MTIxFoo & MTIxBar](encoded).getOrThrow
            assert(decoded.a == 1)
            assert(decoded.b == "hi")
        }

        "case class intersected with empty marker trait delegates to case-class Schema" in {
            given Schema[MTIxUser & MTIxEmptyMarker] = Schema.derived[MTIxUser & MTIxEmptyMarker]
            val user: MTIxUser & MTIxEmptyMarker     = new MTIxUser("Alice", 30) with MTIxEmptyMarker
            val encoded                              = Json.encode(user)
            assert(encoded.contains("\"name\":\"Alice\""))
            assert(encoded.contains("\"age\":30"))
            val decoded = Json.decode[MTIxUser & MTIxEmptyMarker](encoded).getOrThrow
            assert(decoded.name == "Alice")
            assert(decoded.age == 30)
        }

        "three-way intersection round-trips all fields" in {
            given Schema[MTIxT1 & MTIxT2 & MTIxT3] = Schema.derived[MTIxT1 & MTIxT2 & MTIxT3]
            val value: MTIxT1 & MTIxT2 & MTIxT3 = new MTIxT1 with MTIxT2 with MTIxT3:
                def a = 42
                def b = "hello"
                def c = true
            val encoded = Json.encode(value)
            assert(encoded.contains("\"a\":42"))
            assert(encoded.contains("\"b\":\"hello\""))
            assert(encoded.contains("\"c\":true"))
            val decoded = Json.decode[MTIxT1 & MTIxT2 & MTIxT3](encoded).getOrThrow
            assert(decoded.a == 42)
            assert(decoded.b == "hello")
            assert(decoded.c == true)
        }

        "field name collision with the same type is deduped" in {
            given Schema[MTIxHasX1 & MTIxHasX2] = Schema.derived[MTIxHasX1 & MTIxHasX2]
            val value: MTIxHasX1 & MTIxHasX2 = new MTIxHasX1 with MTIxHasX2:
                def x = 7
            val encoded = Json.encode(value)
            // "x" appears once on the wire
            val occurrences = encoded.split("\"x\":", -1).length - 1
            assert(occurrences == 1)
            val decoded = Json.decode[MTIxHasX1 & MTIxHasX2](encoded).getOrThrow
            assert(decoded.x == 7)
        }

        "field name collision with different types is a compile error" in {
            typeCheckFailure(
                """
                trait MTIxHasXInt { def x: Int }
                trait MTIxHasXStr { def x: String }
                kyo.Schema.derived[MTIxHasXInt & MTIxHasXStr]
                """
            )("x")
        }

        "refinement passthrough reduces to parent Schema" in {
            given Schema[MTIxWrapped] = Schema.derived[MTIxWrapped]
            // Schema.derived on a refined parent should peel the refinement and reuse the parent Schema.
            val s: Schema[MTIxWrapped] =
                Schema.derived[MTIxWrapped { type Focused = "value" ~ Int }].asInstanceOf[Schema[MTIxWrapped]]
            val value   = MTIxWrapped(99)
            val encoded = Json.encode(value)(using s)
            assert(encoded.contains("\"value\":99"))
            val decoded = Json.decode[MTIxWrapped](encoded)(using summon, s, summon).getOrThrow
            assert(decoded == MTIxWrapped(99))
        }
    ...
    // =========================================================================
    // nested transform composition (folded from NestedTransformTest)
    // =========================================================================
    ...
    // =========================================================================
    // composition matrix (folded from CompositionMatrixTest)
    // =========================================================================
    ...
    "composition matrix" - {
        ...
        val c1Int: Int                          = 42
        val c2Str: String                       = "hello"
        val c3Instant: java.time.Instant        = java.time.Instant.parse("2024-01-15T10:30:00Z")
        val c4ListInt: List[Int]                = List(1, 2, 3)
        val c5MaybeInt: Maybe[Int]              = Present(42)
        val c6EitherStrInt: Either[String, Int] = Right(42)
        val c7TupIS: (Int, String)              = (1, "hi")
        val c8Shape: CMShape                    = CMShape.CMCircle(5.0)

        // --- Round-trip helper ---
        def rt[T](value: T)(using schema: Schema[T]): T =
        ...
        "Sweep A" - {
            ...
            "P3 Maybe wrapped — C3 Instant" in {
                assert(rt[Maybe[java.time.Instant]](Present(c3Instant)) == Present(c3Instant))
            }
            ...
            "P4 Either Right leg — C3 Instant" in {
                val v: Either[String, java.time.Instant] = Right(c3Instant)
                assert(rt(v) == v)
            }
            ...
            "P6 Tuple slot — C3 Instant" in {
                val v: (java.time.Instant, Boolean) = (c3Instant, true)
                assert(rt(v) == v)
            }
