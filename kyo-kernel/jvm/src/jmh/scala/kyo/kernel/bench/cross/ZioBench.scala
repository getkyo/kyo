...
class ZioBench:
    ...
    def deepRecursionNoRescue: Int =
        def loop(i: Int): UIO[Int] =
            ZIO.unit.flatMap(_ => if i > 400 then ZIO.succeed(i) else loop(i + 1))
        runSync(loop(seed - 1))
    ...
    def deepRecursionOneRescue: Int =
        def loop(i: Int): UIO[Int] =
            ZIO.unit.flatMap(_ => if i > 600 then ZIO.succeed(i) else loop(i + 1))
        runSync(loop(seed - 1))
    ...
    def pureIterationViaLoop: Int =
        def go(i: Int): UIO[Int] =
            if i > Depth then ZIO.succeed(i) else ZIO.succeed(i + 1).flatMap(go)
        runSync(go(seed - 1))
    ...
    def effectfulIterationViaLoop: Int =
        def go(i: Int): UIO[Int] =
            if i > Depth then ZIO.succeed(i) else ask.get.flatMap(a => go(i + a))
        runSync(go(seed - 1))
    ...
    def nestedPayloadsUnwrapInMaps: Int =
        def loop(i: Int, acc: Int): UIO[Int] =
            if i > NarrowDepth then ZIO.succeed(acc)
            else ZIO.succeed(ZIO.succeed(acc)).flatten.flatMap(_ => loop(i + 1, acc + i))
        runSync(loop(0, seed))
