        ...
        "immediate" - {
            ...
            "use the interceptor" in {
                ...
                val interceptor = new TestInterceptor:
                    def ensure(f: Maybe[Throwable] => Unit): Unit = ()
    ...
    "ensure" - {
        ...
        "with interceptor" - {
            ...
            class TestInterceptor extends Safepoint.Interceptor:
                var ensuresAdded: List[Maybe[Throwable] => Unit]                = Nil
                var ensuresRemoved: List[Maybe[Throwable] => Unit]              = Nil
                override def addFinalizer(f: Maybe[Throwable] => Unit): Unit    = ensuresAdded = f :: ensuresAdded
                override def removeFinalizer(f: Maybe[Throwable] => Unit): Unit = ensuresRemoved = f :: ensuresRemoved
            ...
            "interceptor can call the ensure function multiple times but it evaluates once" in {
                ...
                val interceptor = new Safepoint.Interceptor:
                    override def addFinalizer(f: Maybe[Throwable] => Unit): Unit =
                        interceptorCalls += 1
                        f(Absent)
                        f(Absent)
                    end addFinalizer

                    override def removeFinalizer(f: Maybe[Throwable] => Unit): Unit = {}
                    def enter(frame: Frame, value: Any): Boolean                    = true
                    def exit(): Unit                                                = {}
        ...
        "Maybe[Throwable] parameter" - {
            "receives Absent on normal completion" in {
                var receivedValue: Maybe[Throwable] = null
            ...
            "receives Present with exception on failure" in {
                ...
                assert(receivedValue.get eq exception)
            }

            "with nested ensures passes correct exception to each handler" in {
                var outerException: Maybe[Throwable] = null
                var innerException: Maybe[Throwable] = null
                ...
                assert(innerException.get eq testException)
                assert(outerException.isDefined)
                assert(outerException.get eq testException)
            }
        }
    }
