...
/** The marker a handler clause's continuation carries, and what it forbids.
  ...
  *
  * `ArrowEffect.handleFirst` is the deliberate opposite: its clause receives an unmarked continuation.
  *
...
object Region:
    ...
    /** Removes the marker from a value the region has taken back: the clause's answer, entering the region it belongs to.
      *
      * The value is consumed on this fiber, inside the scope that owns what the continuation carries. The row is phantom, so this is a
      * representation assertion on the row alone.
      */
    ...
    /** Takes a continuation out of its confinement, making the caller responsible for it.
      *
      * Ordinary code never needs this: a clause hands its continuation back to its region, and a peel receives an unmarked one. It exists
      * for the kernel's own tests, which reach runtime behaviour their types would otherwise forbid; the explicit name keeps those sites
      * one grep away. What the caller takes on is what the marker stated: a continuation carried past its region is refused when it
      * re-enters a bracket the region already released (`kyo.Closed`), and one carried to another fiber can have that bracket released
      * under it.
      */
