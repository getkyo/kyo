...
object ArrowEffect:
    ...
    /** Transforms both the input and the output of an arrow effect as it is handled.
      *
      ...
      */
    inline def transform[I[_], O[_], E <: ArrowEffect[I, O], A, S, S2](
        inline effectTag: Tag[E],
        v: A < (E & S)
    )(
    ...
    /** Transforms only the output of an arrow effect as it is handled.
      *
      ...
      */
    inline def transformOutput[I[_], O[_], E <: ArrowEffect[I, O], A, S, S2](
        inline effectTag: Tag[E],
        v: A < (E & S)
    )(
        inline mapOutput: [C] => (I[C], O[C]) => O[C] < S2
        ...
        inline _frame: Frame,
        safepoint: Safepoint
    ): A < (S & E & S2) = transform(effectTag, v)(mapInput = [C] => i => i, mapOutput = mapOutput)

    /** Transforms only the input of an arrow effect as it is handled.
      *
      ...
      */
    inline def transformInput[I[_], O[_], E <: ArrowEffect[I, O], A, S, S2](
        inline effectTag: Tag[E],
        v: A < (E & S)
    )(inline mapInput: [C] => I[C] => I[C] < S2)(using
        inline _frame: Frame,
        safepoint: Safepoint
    ): A < (S & E & S2) = transform(effectTag, v)(mapInput = mapInput, mapOutput = [C] => (_, o) => o)
