...
final private[kyo] class Trace(
    private[kernel2] val frames: Array[Frame],
    private[kernel2] var index: Int
)

private[kyo] object Trace:

    private[kernel2] def init: Trace = Trace(new Array[Frame](maxTraceFrames), 0)

    abstract private[kernel2] class Owner extends TracePool.Local:
        final private var frames = new Array[Frame](maxTraceFrames)
        final private var index  = 0

        private[kernel2] inline def pushFrame(frame: Frame): Unit =
            val idx = this.index
            frames(idx & (maxTraceFrames - 1)) = frame
            this.index = idx + 1
        end pushFrame

        final private[kernel2] def saveTrace(): Trace =
            val newTrace  = borrow()
        ...
        private[kernel2] inline def withTrace[A](trace: Trace)(inline f: => A)(using frame: Frame): A =
        ...
        private[kernel2] inline def withNewTrace[A](inline f: => A)(using Frame): A =
        ...
        final private[kernel2] def enrich(ex: Throwable): Unit =
