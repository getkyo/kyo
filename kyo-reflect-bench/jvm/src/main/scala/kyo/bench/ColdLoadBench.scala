...
/** Standalone cold-load profiling harness for kyo-reflect (W11).
  *
  * Opens the kyo-bench compiled TASTy directory via Reflect.Classpath.open, runs 5 warmup + 10 measurement iterations, and prints
  * median/p95 to stdout. Designed to be run with async-profiler attached via -agentpath.
  *
  * Run with: sbt -J-agentpath:/opt/homebrew/lib/libasyncProfiler.dylib=start,event=cpu,file=/tmp/cold-load-flame.html \
  * 'kyo-reflect-bench/runMain kyo.bench.ColdLoadProfile'
  */
object ColdLoadProfile:
    ...
    def main(args: Array[String]): Unit =
        ...
        val times = bench("W11 cold-load kyo-bench (enumerate top-level classes)", warmupIter, measureIter):
            val _ = runSync:
                Scope.run:
                    Reflect.Classpath.open(Seq(root)).map(_.topLevelClasses.size)

        java.lang.System.out.println()

        // Also run snapshot-write timing
        val tmpDir = Files.createTempDirectory("kyo-reflect-profile").toString
        val snapshotTimes = bench("W11b cold-load kyo-bench + snapshot write", warmupIter, measureIter):
            val _ = runSync:
                Scope.run:
                    Reflect.Classpath.openCached(Seq(root), tmpDir).map(_.topLevelClasses.size)
