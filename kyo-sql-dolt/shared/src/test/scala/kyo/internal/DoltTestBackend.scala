...
final class DoltTestBackend extends SqlTestBackend:
    ...
    /** The Dolt server fixture.
      *
      * `DOLT_ROOT_HOST=%` is load-bearing: without it the image creates `root@localhost` only, measured, and every connection from outside
      * the container arrives from the podman gateway and is refused with 1045 before any test runs.
      */
    private val image = "dolthub/dolt-sql-server:2.3.4"

    private val rootPassword = "test"

    def containerConfig: Maybe[Container.Config] =
        Present(
            Container.Config(ContainerImage(image))
                .env("DOLT_ROOT_HOST", "%")
                .env("DOLT_ROOT_PASSWORD", rootPassword)
                .port(3306, 0)
                .requireService(true)
                .initProcess(true)
        )
