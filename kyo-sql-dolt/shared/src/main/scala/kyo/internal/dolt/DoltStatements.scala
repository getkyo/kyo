...
private[kyo] object DoltStatements:
    ...
    private def readCommit(row: SqlRow)(using Frame): Dolt.Commit < Abort[SqlDecodeException] =
        for
        ...
        yield Dolt.Commit(
            ...
            splitList(parents).map(Dolt.CommitHash.apply),
            splitList(refs),
    ...
    private def commitAt(client: SqlClient, ref: String)(using Frame): Dolt.Commit < (Async & Abort[SqlException]) =
    ...
    def add(client: SqlClient, tables: Chunk[String])(using Frame): Unit < (Async & Abort[SqlException]) =
        // Naming no table means every changed one, which DOLT_ADD spells `-A`. Calling it with no argument at all is an
        // error on the server rather than a stage-everything, so the flag is supplied here.
        callUnit(client, "DOLT_ADD", if tables.isEmpty then Chunk("-A") else tables)

    def commit(client: SqlClient, message: String, all: Boolean)(using Frame): Dolt.Commit < (Async & Abort[SqlException]) =
    ...
    def log(client: SqlClient, ref: Dolt.Ref, limit: Maybe[Int])(using Frame): Chunk[Dolt.Commit] < (Async & Abort[SqlException]) =
    ...
    def status(client: SqlClient)(using Frame): Chunk[Dolt.Change] < (Async & Abort[SqlException]) =
        query(client, "SELECT table_name, staged, status FROM dolt_status ORDER BY table_name, staged").map { rows =>
            decoded(rows) { row =>
                ...
                yield Dolt.Change(table, staged, state)
            }
        }

    def diff(client: SqlClient, from: Dolt.Ref, to: Dolt.Ref, table: String)(using
        Frame
        ...
        query(client, s"SELECT * FROM DOLT_DIFF(${literal(from.render)}, ${literal(to.render)}, ${literal(table)})").map { rows =>
            decoded(rows) { row =>
                ...
                yield Dolt.Diff(
                    ...
                    Dolt.Diff.Kind.parse(kind).getOrElse(Dolt.Diff.Kind.Modified),
    ...
    private def readBranch(row: SqlRow)(using Frame): Dolt.Branch < Abort[SqlDecodeException] =
        for
    ...
    def branches(client: SqlClient)(using Frame): Chunk[Dolt.Branch] < (Async & Abort[SqlException]) =
        query(client, s"SELECT $branchColumns FROM dolt_branches ORDER BY name").map(rows => decoded(rows)(readBranch))

    def branchNamed(client: SqlClient, name: String)(using Frame): Dolt.Branch < (Async & Abort[SqlException]) =
    ...
    def createBranch(client: SqlClient, name: String, from: Dolt.Ref)(using Frame): Dolt.Branch < (Async & Abort[SqlException]) =
    ...
    private def interpretMerge(client: SqlClient, rows: Chunk[SqlRow], what: String)(using
        ...
        one(rows, what) { row =>
            ...
            else if hash.isEmpty then Dolt.Merge.UpToDate(message)
            else
                commitAt(client, hash).map { commit =>
                    if fastForward then Dolt.Merge.FastForward(commit, message) else Dolt.Merge.Merged(commit, message)
                }
        }
    ...
    def reset(client: SqlClient, to: Dolt.Ref, mode: Dolt.ResetMode)(using Frame): Unit < (Async & Abort[SqlException]) =
        callUnit(client, "DOLT_RESET", mode.flag.fold(Chunk(to.render))(flag => Chunk(flag, to.render)))

    def revert(client: SqlClient, ref: Dolt.Ref)(using Frame): Dolt.Commit < (Async & Abort[SqlException]) =
        ...
        client.transaction {
            callUnit(client, "DOLT_REVERT", Chunk(ref.render)).andThen(commitAt(client, Dolt.Ref.Head.render))
        }

    def cherryPick(client: SqlClient, ref: Dolt.Ref)(using Frame): Dolt.Commit < (Async & Abort[SqlException]) =
        client.transaction {
    ...
    private def readTag(row: SqlRow)(using Frame): Dolt.Tag < Abort[SqlDecodeException] =
        for
            ...
            date    <- row.decode[Instant]("date")
            message <- row.decode[Maybe[String]]("message")
        yield Dolt.Tag(name, Dolt.CommitHash(hash), tagger, email, date, message.filter(_.nonEmpty))

    def tag(client: SqlClient, name: String, ref: Dolt.Ref, message: Maybe[String])(using
        Frame
    ...
    def tags(client: SqlClient)(using Frame): Chunk[Dolt.Tag] < (Async & Abort[SqlException]) =
    ...
    private def readRemote(row: SqlRow)(using Frame): Dolt.Remote < Abort[SqlDecodeException] =
        for
            name   <- row.decode[String]("name")
    ...
    def remotes(client: SqlClient)(using Frame): Chunk[Dolt.Remote] < (Async & Abort[SqlException]) =
        query(client, "SELECT name, url, fetch_specs, params FROM dolt_remotes ORDER BY name").map { rows =>
            decoded(rows)(readRemote)
        }

    def addRemote(client: SqlClient, name: String, url: String)(using Frame): Dolt.Remote < (Async & Abort[SqlException]) =
