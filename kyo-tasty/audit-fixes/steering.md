...
- Phase 23a audit WARN (2026-05-30): JS has zero coverage of the depth-guard at `internRec` because 1023-level Rec overflows the JS stack BEFORE the guard fires. The bound check itself runs on every platform but is only tested on JVM. ROUTE: add a shallow (~20-level) version of the depth-boundary test using a reduced TypeArena.MaxDepth-equivalent (or instrumented internRec depth) to close the JS coverage gap. Defer to a follow-up if MaxDepth is hard-coded.
- Phase 24a audit WARN: TypeArena T7 label overclaims. Per-fiber arenas + sequential merge verifies the production contract but does not exercise the concurrent-intern code path (because canonical() is not thread-safe). The test correctly pins the production model; the "T7 concurrency" framing is the substance compromise. NOTE: real concurrency for the type-canonicalization path would require a multi-threaded TypeArena impl, which is a design change beyond the test scope.
