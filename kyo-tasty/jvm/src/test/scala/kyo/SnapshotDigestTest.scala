...
class SnapshotDigestTest extends kyo.test.Test[Any]:
    ...
    "DigestComputer.compute on jar root returns same digest for two successive calls".onlyJvm in {
        ...
        Abort.run[TastyError] {
            DigestComputer.compute(Seq(jarPath)).map { d1 =>
                DigestComputer.compute(Seq(jarPath)).map { d2 =>
                    (d1, d2)
                }
            }
        }.map {
    ...
    "DigestComputer.compute for two different jar paths produces different digests".onlyJvm in {
        ...
        Abort.run[TastyError] {
            DigestComputer.compute(Seq(jarPath1)).map { d1 =>
                DigestComputer.compute(Seq(jarPath2)).map { d2 =>
                    (d1, d2)
                }
            }
        }.map {
    ...
    "DigestComputer.compute on mixed jar+directory roots is root-order independent".onlyJvm in {
        ...
        Abort.run[TastyError] {
            DigestComputer.compute(Seq(jarPath, tastyDir)).map { d1 =>
                DigestComputer.compute(Seq(tastyDir, jarPath)).map { d2 =>
                    (d1, d2)
                }
            }
        }.map {
    ...
    "snapshot serialization is digest-stable across two calls" in {
        ...
        val bytes1 = SnapshotWriter.serializeToBytes(classpath, digest)
        val bytes2 = SnapshotWriter.serializeToBytes(classpath, digest)
