...
/** JVM platform: walk the JAR central directory to compute a content-addressed digest.
  ...
  * digest is stable across mtime changes and machine boundaries (INV-003). The CRC32 values stored in the CEN are computed by the jar
  * writer and reflect the actual entry byte content.
  *
  * On missing or corrupt jars, JarCentralDirectory.read propagates IOException rather than returning Chunk.empty (BLOCKER-1 fix:
...
private[kyo] object PlatformDigest:
    ...
    /** Compute the content-addressed digest for a jar root on JVM.
      ...
      * DigestComputer.digestForJar. Propagates IOException if the jar is missing or corrupt (BLOCKER-1 fix).
      *
    ...
    def digestForJarRoot(jarPath: String): Long =
        ...
        // IOException propagates on missing/corrupt jar (BLOCKER-1 fix: do not return 0L for absent jars).
