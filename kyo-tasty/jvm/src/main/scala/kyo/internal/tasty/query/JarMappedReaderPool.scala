...
/** Thread-safe pool of JarMappedReader instances, one per JAR path.
  ...
  *
  * Lifecycle: installed by ZipHandlePlatform.withPool at the start of a ClasspathOrchestrator.init call and released (map cleared) at
...
final private[kyo] class JarMappedReaderPool:
    ...
    /** Clear the pool, releasing all JarMappedReader references.
      *
      * Called by the Scope.ensure finalizer registered in JarMappedReaderPool cleanup. Does not explicitly unmap MappedByteBuffers; GC
      * handles reclamation.
      */
    def closeAll(): Unit =
        cache.clear()
