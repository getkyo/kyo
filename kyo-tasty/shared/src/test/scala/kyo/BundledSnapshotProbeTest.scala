    ...
    "remap-at-merge shifts partial ids by existing classpath size" in {
        ...
        val existing = Tasty.Classpath(
            ...
            indices = Tasty.Classpath.Indices(
                byFullName = Dict.from(existingSyms.toSeq.zipWithIndex.map((s, i) => s"ex$i" -> Tasty.SymbolId(i)).toMap),
        ...
        val partial = Tasty.Classpath(
            symbols = partialSyms,
            indices = Tasty.Classpath.Indices(
                byFullName = Dict.from(partialSyms.toSeq.zipWithIndex.map((s, i) => s"partial$i" -> Tasty.SymbolId(i)).toMap),
                bySimpleName = Dict.empty,
                packageIndex = Dict.empty,
                subclassIndex = Dict.empty,
                companionIndex = Dict.empty,
        ...
        assert(
            merged.indices.byFullName.get("partial1").contains(Tasty.SymbolId(101)),
            "byFullName entry for 'partial1' must point to id=101"
        )
    }
    ...
    "two partials with overlapping ids produce disjoint ids after merge" in {
        def makePartial(prefix: String, n: Int): Tasty.Classpath =
            ...
            Tasty.Classpath(
                symbols = syms,
                indices = Tasty.Classpath.Indices(
                    byFullName = Dict.from(syms.toSeq.zipWithIndex.map((s, i) => s"$prefix$i" -> Tasty.SymbolId(i)).toMap),
                    bySimpleName = Dict.empty,
                    packageIndex = Dict.empty,
                    subclassIndex = Dict.empty,
                    companionIndex = Dict.empty,
