...
/** Tests for BinaryName.compute using synthetic Classpath fixtures.
  ...
  * and trailing '$' for top-level objects. Package symbols use dotted fully-qualified name names matching the
...
class BinaryNameTest extends kyo.test.Test[Any]:
    ...
    "top-level class: example.Top maps to example/Top" in {
        buildTopLevelFixture.map { classpath =>
            classpath.findClass("example.Top") match
                case Maybe.Present(symbol) =>
                    val result = classpath.binaryName(symbol)
    ...
    "top-level object: example.MyObj maps to example/MyObj$" in {
        buildTopLevelFixture.map { classpath =>
            classpath.findObject("example.MyObj") match
                case Maybe.Present(symbol) =>
                    val result = classpath.binaryName(symbol)
    ...
    "nested class: example.Outer.Inner maps to example/Outer$Inner" in {
        buildNestedFixture.map { classpath =>
            classpath.findClass("example.Outer.Inner") match
                case Maybe.Present(symbol) =>
                    val result = classpath.binaryName(symbol)
    ...
    "nested object: example.Outer.InnerObj maps to example/Outer$InnerObj$" in {
        buildNestedFixture.map { classpath =>
            classpath.findObject("example.Outer.InnerObj") match
                case Maybe.Present(symbol) =>
                    val result = classpath.binaryName(symbol)
                    assert(
    ...
    "deeper nesting: a.b.c.Outer.Mid.Inner maps to a/b/c/Outer$Mid$Inner" in {
        buildDeepFixture.map { classpath =>
            classpath.findClass("a.b.c.Outer.Mid.Inner") match
                case Maybe.Present(symbol) =>
                    val result = classpath.binaryName(symbol)
                    assert(
