...
class PackageTypedAccessorsTest extends Test:
    ...
    // When: pkg.memberIds.map(cp.symbol)
    ...
    "Leaf 85: members-untyped: Package.members returns Chunk[Symbol] with all 4 direct members" in run {
        buildFixture.map: (pkg, cp) =>
            ...
            val ms                = pkg.memberIds.map(cp.symbol)
    ...
    // Given: same fixture
    // When: pkg.memberIds.map(cp.symbol).collect { case c: Tasty.Symbol.Class => c }
    // Then: Chunk[Symbol.Class] size 1
    // Pins: INV-005
    "Leaf 86: classes-typed: Package.classes returns Chunk[Class] size 1" in run {
        buildFixture.map: (pkg, cp) =>
            given Tasty.Classpath = cp
            val cs                = pkg.memberIds.map(cp.symbol).collect { case c: Tasty.Symbol.Class => c }
    ...
    // Given: same fixture
    // When: pkg.memberIds.map(cp.symbol).collect { case t: Tasty.Symbol.Trait => t }
    // Then: Chunk[Symbol.Trait] size 1
    // Pins: INV-005
    "Leaf 87: traits-typed: Package.traits returns Chunk[Trait] size 1" in run {
        buildFixture.map: (pkg, cp) =>
            given Tasty.Classpath = cp
            val ts                = pkg.memberIds.map(cp.symbol).collect { case t: Tasty.Symbol.Trait => t }
    ...
    // Given: same fixture
    // When: pkg.memberIds.map(cp.symbol).collect { case o: Tasty.Symbol.Object => o }
    // Then: Chunk[Symbol.Object] size 1
    // Pins: INV-005
    "Leaf 88: objects-typed: Package.objects returns Chunk[Object] size 1" in run {
        buildFixture.map: (pkg, cp) =>
            given Tasty.Classpath = cp
            val os                = pkg.memberIds.map(cp.symbol).collect { case o: Tasty.Symbol.Object => o }
    ...
    // When: pkg.memberIds.map(cp.symbol).collect { case cl: Tasty.Symbol.ClassLike => cl }
    // Then: Chunk[Symbol.ClassLike] size 3
    // Pins: INV-005
    "Leaf 89: classLike-typed: Package.classLike returns Chunk[ClassLike] of size 3" in run {
        buildFixture.map: (pkg, cp) =>
            given Tasty.Classpath = cp
            val cl                = pkg.memberIds.map(cp.symbol).collect { case cl: Tasty.Symbol.ClassLike => cl }
    ...
    // When: pkg.memberIds.map(cp.symbol).collect { case sp: Tasty.Symbol.Package => sp }
    // Then: Chunk[Symbol.Package] size 1
    // Pins: INV-005
    "Leaf 90: subpackages-typed: Package.subpackages returns Chunk[Package] size 1" in run {
        buildFixture.map: (pkg, cp) =>
            given Tasty.Classpath = cp
            val sp                = pkg.memberIds.map(cp.symbol).collect { case sp: Tasty.Symbol.Package => sp }
