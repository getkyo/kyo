...
class PackageTypedAccessorsTest extends kyo.test.Test[Any]:
    ...
    private def buildFixture(using Frame) =
        ...
        Tasty.Classpath.fromPicklesWithSymbols(Chunk(cls, trt, obj, subPkg, pkg)).map { classpath =>
            (pkg, classpath)
        }
    end buildFixture

    "members-untyped: Package.members returns Chunk[Symbol] with all 4 direct members" in {
        buildFixture.map { (pkg, classpath) =>
            given Tasty.Classpath = classpath
            val ms                = pkg.memberIds.flatMap(id => classpath.symbol(id).toChunk)
    ...
    "classes-typed: Package.classes returns Chunk[Class] size 1" in {
        buildFixture.map { (pkg, classpath) =>
            given Tasty.Classpath = classpath
            val cs                = pkg.memberIds.flatMap(id => classpath.symbol(id).toChunk).collect { case c: Tasty.Symbol.Class => c }
    ...
    "traits-typed: Package.traits returns Chunk[Trait] size 1" in {
        buildFixture.map { (pkg, classpath) =>
            given Tasty.Classpath = classpath
            val ts                = pkg.memberIds.flatMap(id => classpath.symbol(id).toChunk).collect { case t: Tasty.Symbol.Trait => t }
    ...
    "objects-typed: Package.objects returns Chunk[Object] size 1" in {
        buildFixture.map { (pkg, classpath) =>
            given Tasty.Classpath = classpath
            val os                = pkg.memberIds.flatMap(id => classpath.symbol(id).toChunk).collect { case o: Tasty.Symbol.Object => o }
    ...
    "classLike-typed: Package.classLike returns Chunk[ClassLike] of size 3" in {
        buildFixture.map { (pkg, classpath) =>
            given Tasty.Classpath = classpath
            val cl = pkg.memberIds.flatMap(id => classpath.symbol(id).toChunk).collect { case cl: Tasty.Symbol.ClassLike => cl }
    ...
    "subpackages-typed: Package.subpackages returns Chunk[Package] size 1" in {
        buildFixture.map { (pkg, classpath) =>
            given Tasty.Classpath = classpath
            val sp = pkg.memberIds.flatMap(id => classpath.symbol(id).toChunk).collect { case sp: Tasty.Symbol.Package => sp }
