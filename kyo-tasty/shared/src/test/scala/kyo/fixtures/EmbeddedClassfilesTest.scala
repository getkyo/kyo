...
/** EmbeddedClassfiles behavioral tests.
  *
  * Verifies that EmbeddedClassfiles produces byte-identical output, that lazy
  * decoding is single-init, that dispatch parity holds, and that every accessor
  * is cross-platform.
  */
class EmbeddedClassfilesTest extends kyo.test.Test[Any]:
    ...
    // Expected FNV-1a 64-bit hashes against the canonical JDK 25.0.3 class bytes.
    "byte equality for all 8 classes via FNV-1a hash" in {
