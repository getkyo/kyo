...
class AnnotationArgListTest extends kyo.test.Test[Any]:
    ...
    "arguments is the decoded arg chunk" in {
        ...
        val annotation = Tasty.Annotation(tpe, Chunk(lit), Tasty.Name(""))
    ...
    "arguments holds a single-element Chunk for a non-Apply tree" in {
        ...
        val annotation = Tasty.Annotation(tpe, Chunk(ident), Tasty.Name(""))
        val result     = annotation.arguments
    ...
    "arguments is empty when the annotation carries no arg trees" in {
        val tpe        = Tasty.Type.Named(SymbolId(0))
        val annotation = Tasty.Annotation(tpe, Chunk.empty, Tasty.Name(""))
        assert(annotation.arguments.isEmpty, "arguments must be empty when no arg trees were decoded")
        succeed
    }
