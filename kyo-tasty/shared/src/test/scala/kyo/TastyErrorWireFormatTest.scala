...
class TastyErrorWireFormatTest extends kyo.test.Test[Any]:
    ...
    "round-trip every TastyError variant through the new string-tag wire format" in {
        val allErrors: Chunk[TastyError] = Chunk(
            ...
            TastyError.InconsistentClasspath(
                "a.tasty",
                Tasty.Uuid.unsafeWrap(new java.util.UUID(0L, 1L).toString),
                Tasty.Uuid.unsafeWrap(new java.util.UUID(2L, 3L).toString)
            ),
            TastyError.FullNameCollisionError("com.example.Foo"),
            ...
            TastyError.InvalidFullName("", "fullName must be non-empty"),
    ...
    "unknown wire tag falls back to TastyError.NotImplemented" in {
        ...
        Abort.run[TastyError] {
            SnapshotReader.readFromBytes(patchedSnap, "cache/unknown-tag-test.krfl").map { classpath =>
                classpath.errors
            }
        }.map {
    ...
    "snapshot with minorVersion=9 is rejected with SnapshotVersionMismatch" in {
        ...
        Abort.run[TastyError] {
            SnapshotReader.readFromBytes(patchedBytes, "cache/minor9-reject-test.krfl")
        }.map {
    ...
    "cross-platform round-trip: all 19 TastyError variants serialize and deserialize identically" in {
        ...
        val errors: Chunk[TastyError] = Chunk(
            ...
            TastyError.InconsistentClasspath(
                "c.tasty",
                Tasty.Uuid.unsafeWrap(new java.util.UUID(10L, 20L).toString),
                Tasty.Uuid.unsafeWrap(new java.util.UUID(30L, 40L).toString)
            ),
            TastyError.FullNameCollisionError("cross.Foo"),
            ...
            TastyError.InvalidFullName("bad.fullName!", "invalid character"),
        ...
        Abort.run[TastyError] {
            SnapshotReader.readFromBytes(snapshotBytes, snapPath).map { classpath =>
                classpath.errors
            }
        }.map {
            case Result.Success(loaded) =>
