...
class SymbolFieldPreservationTest extends kyo.test.Test[Any]:
    ...
    ".id is accessible and correct on all 14 subtypes" in {
        ...
        for (expectedId, symbol) <- syms do
            assert(
                symbol.id == SymbolId(expectedId),
                s"Expected id=${expectedId} for ${symbol.getClass.getSimpleName} but got ${symbol.id.value}"
            )
        end for
        succeed
    }
    ...
    "Flags(Flag.Synthetic) is preserved on all 14 subtypes" in {
        ...
        for symbol <- subtypesWithFlags do
            assert(
                symbol.flags.contains(Tasty.Flag.Synthetic),
                s"Expected Flag.Synthetic on ${symbol.getClass.getSimpleName} but not found in flags=${symbol.flags.bits}"
            )
        end for
        succeed
    }
