...
class PositionsUnpicklerTest extends kyo.test.Test[Any]:
    ...
    /** Create a minimal LoadingSymbol.Materialising for testing with a unique sequential id.
      *
      * the LongMap rotation in PositionsUnpickler keys by symbol.id.toLong, so each symbol
    ...
    // addrMap has {5 -> symbol} so header = (5<<3)|4 = 44 (addrDelta=5, hasStart=true), start_delta=17.
    ...
    // ── IntMap addrMap with 10,000 entries, correctness at scale ──

    // PositionsUnpickler.readSync with an IntMap addrMap of 10,000 entries returns
    ...
    //   addrMap has {i+1 -> symbol(i)} for i in [0, 10000)
    ...
    "PositionsUnpickler.readSync with IntMap addrMap of 10,000 entries returns correct positions" in {
        ...
        // Build 10,000 symbols, one per address index 1.N
        val syms: Array[LoadingSymbol.Materialising] =
            Array.tabulate(N)(i => makeTestSymbol(s"symbol$i"))
        ...
        PositionsUnpickler.read(view, addrMap, Present("scale-test.scala")) match
            case Result.Success(result) =>
                ...
                for i <- checks do
                    val symbol = syms(i)
                    assert(result.contains(symbol.id.toLong), s"Expected symbol$i.id to have a position entry")
                    val pos = result(symbol.id.toLong)
                    ...
                    assert(
                        ...
                        s"symbol$i expected line=${expectedLine} but got ${pos.line}"
                    )
                    assert(
                        pos.column == 1,
                        s"symbol$i expected column=1 but got ${pos.column}"
                    )
                    assert(
                        pos.sourceFile == "scale-test.scala",
                        s"symbol$i expected sourceFile=scale-test.scala but got ${pos.sourceFile}"
                    )
                end for
                succeed
            case Result.Failure(e) =>
    ...
    // ── Int overflow on lineStarts cumulation is detected ────────

    // When the cumulative sum of lineSizes overflows Int, PositionsUnpickler must
    ...
    // ── Normal 200-line file decodes lineStarts correctly ─

    // A 200-line synthetic file where line k has size (100 + k) decodes correctly.
    // lineStarts(0) = 0
    ...
    "PositionsUnpickler: 200-line file with varying line sizes decodes lineStarts correctly" in {
        ...
        val symbol  = makeTestSymbol("B9Baseline")
        val addrMap = IntMap(1 -> symbol)
        val view    = ByteView(payload)
        ...
        PositionsUnpickler.read(view, addrMap, Present("test.scala")) match
            case Result.Success(result) =>
                ...
                assert(pos.column == 1, s"Expected column=1 but got ${pos.column}")
                // Verify spot-check on lineStarts(10) = 1055 via a new read with symbol at that offset
                succeed
            case Result.Failure(e) =>
                fail(s"Expected success but got failure: $e")
            case Result.Panic(t) =>
    ...
    // ── lineStarts(10) value correctness for 200-line file ─────────

    // Confirms the exact value of lineStarts(10) for a 200-line file where line k has size (100+k).
    ...
    "PositionsUnpickler: lineStarts(10) equals 1055 for 200-line file with sizes 100+k" in {
        ...
        val symbol  = makeTestSymbol("B9LineStarts10")
        val addrMap = IntMap(1 -> symbol)
        val view    = ByteView(payload)
        // Use Present sourceFile so position entries are built, not the absent-SOURCEFILE silent skip.
        PositionsUnpickler.read(view, addrMap, Present("test.scala")) match
