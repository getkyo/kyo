...
/** Ported regression tests for closed TASTy-reader bugs from scalacenter/tasty-query
  ...
  *
  * Every fixture fully-qualified name exercised here is embedded in `kyo.fixtures.Embedded` and
...
class PortedTastyBugTest extends kyo.test.Test[Any]:
    ...
    // TASTy level.
    // Post-fix behavior: `findClass` returns Absent for the fully-qualified name; only
    // `findObject` resolves it.
    "tasty-query#74: object fully-qualified name exposes only the Object symbol, not a Class" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            val fullName = s"$FixturePkg.PortedBug74Object"
            val asClass  = classpath.findClass(fullName)
            val asObject = classpath.findObject(fullName)
            assert(asObject.isDefined, s"findObject($fullName) must resolve to the Object symbol")
            assert(asClass.isEmpty, s"findClass($fullName) must be Absent (no class artifact at TASTy level), got $asClass")
            succeed
        }
    }
    ...
    // both reachable by fully-qualified name.
    "tasty-query#71/#72: nested package member and inner object both resolve by fully-qualified name" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            val pkgFullName    = s"$FixturePkg.portedBug71Outer.portedBug71Inner"
            val markerFullName = s"$pkgFullName.Marker"
            val pkg            = classpath.findPackage(pkgFullName)
            val marker         = classpath.findObject(markerFullName)
            assert(pkg.isDefined, s"findPackage($pkgFullName) must resolve the nested package")
            assert(marker.isDefined, s"findObject($markerFullName) must resolve the object inside the nested package")
            succeed
        }
    }
    ...
    "tasty-query#193: subtype query on path-dependent inner class terminates" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            val outer = classpath.findClass(s"$FixturePkg.PortedBug193Outer")
            val sup   = classpath.findClass(s"$FixturePkg.PortedBug193SuperClass")
            (outer, sup) match
                case (kyo.Maybe.Present(o), kyo.Maybe.Present(s)) =>
                    val outerTpe = Tasty.Type.Named(o.id)
                    val supTpe   = Tasty.Type.Named(s.id)
                    // Outer is unrelated to SuperClass: must return a definitive verdict, never loop.
                    classpath.isSubtypeOf(outerTpe, supTpe) match
                        case Result.Success(verdict) =>
                            assert(
                                verdict == Tasty.SubtypeVerdict.NotSub || verdict == Tasty.SubtypeVerdict.Indeterminate,
                                s"unrelated types must be NotSub or Indeterminate, not $verdict"
                            )
                        case other => fail(s"subtype query must return a verdict, got $other")
                    end match
                case _ =>
    ...
    "tasty-query#195: parent val survives findMember even when child has private ctor param of same name" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            val parent = classpath.findClass(s"$FixturePkg.PortedBug195$$.Parent")
                .orElse(classpath.findClass(s"$FixturePkg.PortedBug195$$Parent"))
            parent match
                case kyo.Maybe.Present(p) =>
                    val yMember = Maybe.fromOption(p.declarationIds.flatMap(id => classpath.symbol(id).toChunk).find(_.simpleName == "y"))
                    assert(yMember.isDefined, s"Parent.y must be findable; got $yMember")
                    succeed
                case kyo.Maybe.Absent =>
                    fail(s"PortedBug195.Parent not on classpath under either fully-qualified name form")
            end match
        }
    }
    ...
    "tasty-query#263: class and top-level defs in same file produce both class and package-object" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            val cls = classpath.findClass(s"$FixturePkg.PortedBug263ClassAndPackageObjectSameName")
            ...
            val methods = classpath.allMethods.filter(_.name.asString == "portedBug263TopLevelMethod")
    ...
    "tasty-query#380: enum extending java.lang.Enum[Self] decodes without raising" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findClassLike(s"$FixturePkg.PortedBug380Foo") match
                case kyo.Maybe.Present(symbol) =>
                    // Touching declarations forces decode of constructor / parent.
                    val decls = symbol.declarationIds.flatMap(id => classpath.symbol(id).toChunk)
    ...
    "tasty-query#415: refinement-introduced anonymous type member does not raise on decode" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findObject(s"$FixturePkg.PortedBug415Holder") match
                case kyo.Maybe.Present(holder) =>
                    val methods =
                        holder.declarationIds.flatMap(id => classpath.symbol(id).toChunk).filter(_.isInstanceOf[Tasty.Symbol.Method])
                    assert(
    ...
    "tasty-query#428: value class user method is findable as a declared member" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findClass(s"$FixturePkg.PortedBug428ValueClass") match
                case kyo.Maybe.Present(cls) =>
                    val doubled =
                        Maybe.fromOption(cls.declarationIds.flatMap(id => classpath.symbol(id).toChunk).find(_.simpleName == "doubled"))
    ...
    // application loads without raising; `classpath.errors` carries no MalformedSection
    // attributable to this fixture.
    "tasty-query#134: match-type declared type decodes without raising" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findObject(s"$FixturePkg.PortedBug134") match
                case kyo.Maybe.Present(holder) =>
                    val v = Maybe.fromOption(holder.declarationIds.flatMap(id => classpath.symbol(id).toChunk).find(_.simpleName == "v"))
    ...
    "tasty-query#187: both overloads of foo are visible in declarations" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findClass(s"$FixturePkg.PortedBug187OverloadedApply") match
                case kyo.Maybe.Present(cls) =>
                    val foos = cls.declarationIds.flatMap(id => classpath.symbol(id).toChunk).filter(_.name.asString == "foo")
                    assert(
                        foos.length >= 2,
    ...
    "tasty-query#125: ContextFunction reference resolves to Type.ContextFunction" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findObject(s"$FixturePkg.PortedBug125") match
                case kyo.Maybe.Present(holder) =>
                    val v = holder.declarationIds.flatMap(id => classpath.symbol(id).toChunk).find(_.name.asString == "asContextFn")
    ...
    "tasty-query#192: generic identity call decodes without raising" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findObject(s"$FixturePkg.PortedBug192").isDefined match
                case true  => succeed
    ...
    "tasty-query#224: super-select inside override decodes without raising" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findClass(s"$FixturePkg.PortedBug224C") match
                case kyo.Maybe.Present(cls) =>
                    val m = Maybe.fromOption(cls.declarationIds.flatMap(id => classpath.symbol(id).toChunk).find(_.simpleName == "m"))
    ...
    "tasty-query#357: qualified TypeTest patterns decode without raising" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findObject(s"$FixturePkg.PortedBug357") match
                case kyo.Maybe.Present(holder) =>
                    val classify =
                        Maybe.fromOption(holder.declarationIds.flatMap(id => classpath.symbol(id).toChunk).find(_.simpleName == "classify"))
    ...
    "tasty-query#412: top-level def is reachable; fixture decode is consistent" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            val methods = classpath.allMethods.filter(_.name.asString == "portedBug412topLevel")
    ...
    "tasty-query#414: enum.valueOf call decodes without raising" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findObject(s"$FixturePkg.PortedBug414") match
                case kyo.Maybe.Present(holder) =>
                    val parsed =
                        Maybe.fromOption(holder.declarationIds.flatMap(id => classpath.symbol(id).toChunk).find(_.simpleName == "parsed"))
    ...
    "tasty-query#424: inline def returning path-dependent type decodes" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findObject(s"$FixturePkg.PortedBug424") match
                case kyo.Maybe.Present(holder) =>
                    val proxy =
                        Maybe.fromOption(holder.declarationIds.flatMap(id => classpath.symbol(id).toChunk).find(_.simpleName == "proxy"))
    ...
    "tasty-query#464: scala.collection.immutable.Map reference decodes without raising" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findObject(s"$FixturePkg.PortedBug464") match
                case kyo.Maybe.Present(holder) =>
                    val m = Maybe.fromOption(holder.declarationIds.flatMap(id => classpath.symbol(id).toChunk).find(_.simpleName == "m"))
    ...
    "tasty-query#401: recursive match type holder loads without looping" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findObject(s"$FixturePkg.PortedBug401") match
                case kyo.Maybe.Present(holder) =>
                    val x = Maybe.fromOption(holder.declarationIds.flatMap(id => classpath.symbol(id).toChunk).find(_.simpleName == "x"))
    ...
    "tasty-query#108: polymorphic method with <: Any bound decodes" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findClass(s"$FixturePkg.PortedBug108") match
                case kyo.Maybe.Present(cls) =>
                    val poly = Maybe.fromOption(cls.declarationIds.flatMap(id => classpath.symbol(id).toChunk).find(_.simpleName == "poly"))
    ...
    "tasty-query#7: type lambda parameter references decode" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findObject(s"$FixturePkg.PortedBug7") match
                case kyo.Maybe.Present(holder) =>
                    val inst = holder.declarationIds.flatMap(id => classpath.symbol(id).toChunk).find(_.name.asString == "Inst")
    ...
    "tasty-query#213: refinement type alias decodes" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findObject(s"$FixturePkg.PortedBug213") match
                case kyo.Maybe.Present(holder) =>
                    val r = Maybe.fromOption(holder.declarationIds.flatMap(id => classpath.symbol(id).toChunk).find(_.simpleName == "r"))
    ...
    "tasty-query#172: Outer#Inner method signature decodes" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findClass(s"$FixturePkg.PortedBug172Outer") match
                case kyo.Maybe.Present(cls) =>
                    val make = Maybe.fromOption(cls.declarationIds.flatMap(id => classpath.symbol(id).toChunk).find(_.simpleName == "make"))
    ...
    "tasty-query#403: path-dependent opaque type alias decodes" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findClass(s"$FixturePkg.PortedBug403Container") match
                case kyo.Maybe.Present(cls) =>
                    val wrap = Maybe.fromOption(cls.declarationIds.flatMap(id => classpath.symbol(id).toChunk).find(_.simpleName == "wrap"))
    ...
    "tasty-query#116: IArray[String] method signature decodes" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findTrait(s"$FixturePkg.PortedBug116IArraySig") match
                case kyo.Maybe.Present(trt) =>
                    val from = Maybe.fromOption(trt.declarationIds.flatMap(id => classpath.symbol(id).toChunk).find(_.simpleName == "from"))
    ...
    "tasty-query#405: parametric value class decodes with raw field visible" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findClass(s"$FixturePkg.PortedBug405ParamValueClass") match
                case kyo.Maybe.Present(cls) =>
                    val ctors =
                        cls.declarationIds.flatMap(id => classpath.symbol(id).toChunk).collect {
                            case m: Tasty.Symbol.Method if m.simpleName == "<init>" => m
                        }
    ...
    "tasty-query#178: java.util.Map.Entry reference decodes" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findObject(s"$FixturePkg.PortedBug178") match
                case kyo.Maybe.Present(holder) =>
                    val getEntry =
                        Maybe.fromOption(holder.declarationIds.flatMap(id => classpath.symbol(id).toChunk).find(_.simpleName == "getEntry"))
    ...
    "tasty-query#80: java.util.List[String] field does not trigger transitive forcing crash" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findClass(s"$FixturePkg.PortedBug80UsesRawAware") match
                case kyo.Maybe.Present(cls) =>
                    val ctors =
                        cls.declarationIds.flatMap(id => classpath.symbol(id).toChunk).collect {
                            case m: Tasty.Symbol.Method if m.simpleName == "<init>" => m
    ...
    "scala3#11075: abstract+concrete inline def pair decodes" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            val abs = classpath.findTrait(s"$FixturePkg.PortedBug11075A")
            val con = classpath.findClass(s"$FixturePkg.PortedBug11075B")
            (abs, con) match
                case (kyo.Maybe.Present(a), kyo.Maybe.Present(c)) =>
                    val aMethod = Maybe.fromOption(a.declarationIds.flatMap(id => classpath.symbol(id).toChunk).find(_.simpleName == "a"))
                    val cMethod = Maybe.fromOption(c.declarationIds.flatMap(id => classpath.symbol(id).toChunk).find(_.simpleName == "a"))
    ...
    "scala3#16843: typed-pattern bind in match decodes" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findObject(s"$FixturePkg.PortedBug16843") match
                case kyo.Maybe.Present(holder) =>
                    val go = Maybe.fromOption(holder.declarationIds.flatMap(id => classpath.symbol(id).toChunk).find(_.simpleName == "go"))
    ...
    "scala3#7022: overloaded foo retains both alternatives after decode" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findClass(s"$FixturePkg.PortedBug7022C") match
                case kyo.Maybe.Present(c) =>
                    val foos = c.declarationIds.flatMap(id => classpath.symbol(id).toChunk).filter(_.name.asString == "foo")
    ...
    "scala3#12704: case class with default param value decodes and exposes the default-arg method" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            val obj = classpath.findObject(s"$FixturePkg.PortedBug12704CaseClass")
            val cls = classpath.findClass(s"$FixturePkg.PortedBug12704CaseClass")
            ...
            obj match
                case kyo.Maybe.Present(o) =>
                    ...
                    val defaultMethod =
                        o.declarationIds.flatMap(id => classpath.symbol(id).toChunk).find(_.name.asString.contains("default"))
                    assert(
                        defaultMethod.isDefined,
                        s"default-arg method must be visible on companion; got ${o.declarationIds.flatMap(id => classpath.symbol(id).toChunk).map(_.name.asString)}"
                    )
                    succeed
    ...
    "scala3#25801: refinement with higher-kinded type member decodes" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findObject(s"$FixturePkg.PortedBug25801") match
                case kyo.Maybe.Present(holder) =>
                    val aux       = holder.declarationIds.flatMap(id => classpath.symbol(id).toChunk).find(_.name.asString == "Aux")
                    val mapResTrt = holder.declarationIds.flatMap(id => classpath.symbol(id).toChunk).find(_.name.asString == "MapRes")
    ...
    "tasty-query#284: bounded type alias with refined usage decodes" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            classpath.findObject(s"$FixturePkg.PortedBug284") match
                case kyo.Maybe.Present(holder) =>
                    val v = Maybe.fromOption(holder.declarationIds.flatMap(id => classpath.symbol(id).toChunk).find(_.simpleName == "v"))
