...
/** Tests for Symbol id-and-kind equality override on the sealed trait.
  ...
  *
  * Cases:
  *   non-id fields do not affect equality (id discriminant).
  *   id is the equality key.
  *   class-vs-trait at same id is not equal (kind discriminant prevents cross-kind collisions).
  *   sentinel id (-1) is never equal to itself or any other symbol (guard).
  *   equal symbols have equal hashCodes; different-kind symbols at same id differ.
  */
class SymbolEqualityTest extends kyo.test.Test[Any]:
    ...
    "same id different fields equal" in {
    ...
    "different ids same fields not equal" in {
    ...
    "Class vs Trait at same id not equal" in {
    ...
    "sentinel id not equal to self or peer" in {
    ...
    "hashCode consistency with equality" in {
        ...
        // hashCode is id.value only, so Class(42) and Trait(42) have the SAME hashCode but are not equal.
