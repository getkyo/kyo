...
/** Tests for Classpath.symbolsAnnotatedWith using a fixture with both annotated and non-annotated symbols.
  *
  * Fixture: Class "deprecated" (fullName "scala.deprecated"; not annotated), Method "m1" annotated with
...
class ClasspathAnnotatedTest extends kyo.test.Test[Any]:
    ...
    private def makeAnnotClass(id: Int, name: String, fullNameName: String): Tasty.Symbol.Class =
        Tasty.Symbol.Class(
            SymbolId(id),
            Tasty.Name(name),
            Tasty.Flags.empty,
    ...
    private def buildFixture(using Frame): Tasty.Classpath < Sync =
        Sync.defer {
            ...
            //   2 -> Method "m1" (ownerId = 4, annotated with annotation referencing id 1)
            //   3 -> Val "v1" (ownerId = 4, annotated with annotation referencing id 1)
            ...
            val annotatedMethod = makeMethod(2, "m1", ownerId = 4, Chunk(annotation))

            // id 3: val annotated with @deprecated
            val annotatedVal = makeVal(3, "v1", ownerId = 4, Chunk(annotation))
            ...
            Tasty.Classpath.make(
                ...
                fullNameIndex = Dict("scala.deprecated" -> SymbolId(1), "PlainA" -> SymbolId(4)),
