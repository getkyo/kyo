...
class ClasspathAnnotatedTest extends Test:
    ...
    private def buildFixture(using Frame): Tasty.Classpath < Sync =
        Sync.defer:
            ...
            val scalaPkg = Tasty.Symbol.Package(SymbolId(0), Tasty.Name.fromString("scala"), Tasty.Flags.empty, SymbolId(-1), Chunk.empty)
            ...
            Tasty.Classpath.make(
                ...
                fqnIndex = Map("scala.deprecated" -> SymbolId(1), "PlainA" -> SymbolId(4)),
                packageIndex = Map("scala" -> SymbolId(0)),
