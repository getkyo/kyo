...
class SealedAdtCompletenessTest extends kyo.test.Test[Any]:
    ...
    // ── ADT-001: TastyError (enum - 24 variants) ─────────────────────────────

    // Update EXPECTED_TASTY_ERROR_COUNT whenever a new TastyError variant is added.
    private inline val EXPECTED_TASTY_ERROR_COUNT = 24
    ...
    private val tastyErrorCoveredByTest: Map[String, String] = Map(
        ...
        "FullNameCollisionError"  -> "CollisionFidelity2Test",
        ...
        "InvalidFullName"         -> "TastyErrorTest",
        "InvalidUuid"             -> "TastyUuidParseTest",
        "DigestMismatch"          -> "TastyErrorMaybeTest",
        "UnhandledSubtypingCase"  -> "IsSubtypeOfTest",
        "UnresolvedReference"     -> "TastyErrorMaybeTest",
        "UnknownType"             -> "TypeAliasOpaqueTypedAccessorsTest",
        "MissingDeclaredType"     -> "TypeAliasOpaqueTypedAccessorsTest"
    )

    // ADT-001: TastyError - all 24 variants covered by tests.
    "TastyError - all 24 variants are covered by at least one named test" in {
