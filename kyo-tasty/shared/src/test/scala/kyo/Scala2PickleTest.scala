...
/** Tests for Scala 2 pickle reader.
  ...
  * at test time requires a Scala 2 compiler.
  *
...
class Scala2PickleTest extends kyo.test.Test[Any]:
    ...
    private def readPickleDirect(pickleBytes: Array[Byte])(using Frame): Scala2PickleResult < (Sync & Abort[TastyError]) =
        Sync.Unsafe.defer(Abort.get(Scala2PickleReader.readRaw(pickleBytes, new TypeArena)))
    ...
    "Scala 2 case class: CLASSsym with CASE flag -> kind=Class and Flag.Case" in {
        ...
        readPickleDirect(pickleBytes).map { result =>
            ...
            val symbol = caseClass.get
            assert(symbol.kind == SymbolKind.Class, s"Expected kind=Class, got ${symbol.kind}")
            assert(symbol.flags.contains(Tasty.Flag.Scala2), "Expected Flag.Scala2")
        }
    }
    ...
    "Scala 2 method: VALsym with METH flag -> declaredType is Type.Function" in {
        ...
        readPickleDirect(pickleBytes).map { result =>
            ...
            // declaredType for Scala2 method symbols may be Absent or Type.Function.
            (method match
    ...
    "Java-only classfile (no ScalaSig attribute) -> Flag.Scala2 is absent" in {
        val result =
            Sync.Unsafe.defer {
                ...
                Abort.get(ClassfileUnpickler.read(kyo.fixtures.Embedded.throwsFixtureClass, new TypeArena))
            }
        result.map { r =>
            val symbol = r.classSymbol
            assert(!symbol.flags.contains(Tasty.Flag.Scala2), s"Expected no Flag.Scala2 on Java classfile; got flags=${symbol.flags.bits}")
    ...
    "Scala 2 class symbol has at least one parent type (AnyRef placeholder)" in {
        ...
        readPickleDirect(pickleBytes).map { result =>
            ...
            // Name check (AnyRef) deferred to once classpath.symbol(id).name is resolvable.
    ...
    "EXTref decodes to Unresolved symbol with fully-qualified name com.example.Foo" in {
        ...
        readPickleDirect(pickleBytes).map { result =>
            ...
            assert(
                fooSym.isDefined,
                s"Expected symbol with fully-qualified name 'com.example.Foo'; got: ${unresolved.map(_.name.asString).mkString(", ")}"
            )
            assert(fooSym.get.flags.contains(Tasty.Flag.Scala2), "Expected Flag.Scala2 on EXTref symbol")
        }
    }

    "EXTMODCLASSref appends $ and decodes to Unresolved symbol with fully-qualified name com.example.Foo$" in {
        // Layout (entry 3 is EXTMODCLASSref):
    ...
    "malformed EXTref owner-cycle terminates safely without StackOverflow" in {
        ...
        // resolveExtFullName is called with entryIdx=1 and visited=Set.empty.
        //   Entry 1 is an EXTref, not in visited. It reads ownerRef=1.
        //   nextVisited = Set(1). Recursive call: resolveExtFullName(1., Set(1)).
        //   visited.contains(1) is true -> returns "".
        //   ownerFullName is Absent (filtered empty), so leafName "Foo" is used.
        //   The owner resolution returns "Foo".
        // The outer decodeExtRef then builds fullName = "Foo" + "." + "Foo" = "Foo.Foo".
        ...
        readPickleDirect(pickleBytes).map { result =>
            ...
            // The cycle is detected after one recursion step; the fully-qualified name is finite (not an infinite string).
            ...
            assert(
                fooSym.isDefined,
                s"Expected symbol with finite fully-qualified name 'Foo.Foo' (cycle cut after one step); got: ${unresolved.map(_.name.asString).mkString(", ")}"
            )
        }
    }
