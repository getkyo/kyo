...
class CollectionInvariantsTest extends kyo.test.Test[Any]:
    ...
    "classpath.allClassLike.size >= classpath.topLevelClasses.size" in {
    ...
    "classpath.topLevelClasses is a subset of classpath.allClassLike" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
    ...
    "extension methods are found via isExtension on embedded fixtures" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
