...
class ClasspathPureDataTest extends kyo.test.Test[Any]:
    ...
    "Classpath.empty round-trips through Schema" in {
        ...
        decoded match
            case Result.Success(classpath) =>
                assert(classpath == Classpath.empty, s"Decoded Classpath != Classpath.empty: $classpath")
                succeed
    ...
    "non-empty Classpath round-trips through Schema" in {
        ...
        val indices = Classpath.Indices.empty.copy(
            byFullName = Dict("test.Foo" -> SymbolId(0))
        )
        ...
        decoded match
            case Result.Success(decoded) =>
                ...
                assert(decoded.indices.byFullName.contains("test.Foo"), s"byFullName missing test.Foo")
                succeed
            case Result.Failure(e) =>
                fail(s"Schema round-trip failed: $e")
            case Result.Panic(t) =>
    ...
    "Binding.empty wraps Classpath.empty and has Absent decodeCtx" in {
        val b = Binding.empty
        assert(b.classpath == Classpath.empty, s"Binding.empty.classpath should equal Classpath.empty")
