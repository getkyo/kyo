...
class EnumCaseFidelity2Test extends Fidelity2TestBase:
    ...
    "classpath has at least one EnumCase child (value-form or class-form)" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            val enumCases = classpath.symbols.count(_.isInstanceOf[Tasty.Symbol.EnumCase])
            assert(
                enumCases > 0,
                s"Expected at least 1 EnumCase symbol. " +
                    s"Total symbols: ${classpath.symbols.size}. " +
        ...
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            ...
            def hasEnumCase(e: Tasty.Symbol.ClassLike): Boolean =
                ...
                then
                    ...
                    e.declarationIds.flatMap(id => classpath.symbol(id).toChunk).exists { decl =>
                        decl match
                            case companion: Tasty.Symbol.Object =>
                                companion.declarationIds.flatMap(id => classpath.symbol(id).toChunk).exists(
                                    _.isInstanceOf[Tasty.Symbol.EnumCase]
                                )
                            case _ => false
                    }
                end if
            ...
            val names = enumsWithNoCases.take(5).map(e => classpath.fullName(e).asString)
            assert(
    ...
    "total EnumCase count > 0 after reclassification (at least embedded Color + Shape cases)" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            val enumCaseCount = classpath.symbols.count(_.isInstanceOf[Tasty.Symbol.EnumCase])
            assert(
                enumCaseCount > 0,
        ...
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            ...
            if knownValueFormCases.isEmpty then
                ...
                val wrongOwner = knownValueFormCases.filter { e =>
                    classpath.symbol(e.ownerId) match
                ...
                assert(
                    ...
                    s"${wrongOwner.size} value-form EnumCase symbols have non-Object owner: " +
                        wrongOwner.take(3).map { e =>
                            val ownerKind = classpath.symbol(e.ownerId).getClass.getSimpleName
            ...
            target match
                case Maybe.Present(enumClass) =>
                    val permSubEnumCases =
                        enumClass.permittedSubclassIds.map(_.flatMap(id => classpath.symbol(id).toChunk)).getOrElse(Chunk.empty).collect {
                            case e: Tasty.Symbol.EnumCase => e
                        }
                    val name = classpath.fullName(enumClass)
                    assert(
                        permSubEnumCases.nonEmpty,
                        s"${name.asString} has no EnumCase permitted subclasses " +
                            s"(regression: permittedSubclasses = ${enumClass.permittedSubclassIds.map(_.map(id =>
                                    classpath.symbol(id).toChunk
                                )).getOrElse(Chunk.empty)})"
                    )
                    succeed
                case Maybe.Absent =>
                    // Fall back: search for any class-form enum case (at least Shape cases in embedded set)
                    val classFormCases = classpath.symbols.collect {
    ...
    coldWarmEquiv("EnumCase count is equal across cold and warm") { classpath =>
        classpath.symbols.count(_.isInstanceOf[Tasty.Symbol.EnumCase])
    }
    ...
    "embedded fixture EnumCase count > 0 on all platforms" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            val count = classpath.symbols.count(_.isInstanceOf[Tasty.Symbol.EnumCase])
            assert(
                count > 0,
