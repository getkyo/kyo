...
class PortableInflateTest extends kyo.test.Test[Any]:
    ...
    "BitStream reads single bits LSB-first" in {
        ...
        val buffer = Array(0xb4.toByte)
        val stream = new PortableInflate.BitStream(buffer, 0L)
    ...
    //   symbol 5 (F), len 2: 00
    //   symbol 0 (A), len 3: 010
    //   symbol 1 (B), len 3: 011
    ...
    "HuffmanTree decodes RFC 1951 §3.2.2 example (F then A)" in {
        import AllowUnsafe.embrace.danger
    ...
    // Tree with lengths [2, 2]: symbol 0 -> code 00, symbol 1 -> code 01 (maxBits=2).
    ...
    "decodeFixedHuffmanBlock decodes AAA" in {
        import AllowUnsafe.embrace.danger
