...
class TypeUnpicklerTest extends kyo.test.Test[Any]:
    ...
    "decoding TYPEREFsymbol returns Named(intSymbol)" in {
        ...
        Abort.run[TastyError](decodeType(bytes, addrMap, names)).map {
            case Result.Success(t) =>
                t match
                    // TYPEREFsymbol encodes address as SymbolId(PHASE_B_ADDR_OFFSET + address).
                    case Tasty.Type.Named(id) => assert(id.value >= 0, s"Expected address-encoded positive id, got ${id.value}")
    ...
    "decoding BYNAMEtype wrapping a TYPEREFsymbol returns ByName(Named(symbol))" in {
        val symbol     = makeSym("Int")
        val symAddr    = 10
        val addrMap    = IntMap(symAddr -> symbol)
        val names      = Array(Tasty.Name("scala"))
            ...
            case Result.Success(t) =>
                t match
                    case Tasty.Type.ByName(Tasty.Type.Named(id)) =>
                        assert(id.value >= 0, s"Expected address-encoded id in ByName, got ${id.value}")
            ...
            case Result.Success(t) =>
                t match
                    case Tasty.Type.Repeated(Tasty.Type.Named(id)) =>
                        assert(id.value >= 0, s"Expected address-encoded id in Repeated, got ${id.value}")
            ...
            case Result.Success(t) =>
                t match
                    case Tasty.Type.Applied(Tasty.Type.Named(ls), args) =>
                        assert(ls.value >= 0, s"Expected address-encoded listSym id but got ${ls.value}")
                        assert(args.length == 1)
                        args(0) match
                            case Tasty.Type.Named(s) => assert(s.value >= 0, s"Expected address-encoded stringSym id but got ${s.value}")
    ...
    // Uses a real SHAREDtype byte stream: first type decoded at address 0, second is SHAREDtype(0).
    ...
    "decoding SHAREDtype reference returns the same reference (no duplication)" in {
        ...
        // SHAREDtype (61) = category 2: tag + Nat. Nat = 0 = address of first type in the view.
    ...
    "decoding ANNOTATEDtype eagerly populates Annotation.arguments as Chunk[Tree]" in {
        ...
        Abort.run[TastyError](decodeType(bytes, addrMap, names)).map {
            case Result.Success(t) =>
                t match
                    case Tasty.Type.Annotated(_, annotation) =>
                        assert(
                            annotation.arguments.nonEmpty,
                            s"Expected non-empty arguments but got ${annotation.arguments}"
                        )
                        succeed
                    case other =>
    ...
    "Annotation(type, Chunk.empty).arguments is empty without any effect" in {
        val annotation = Tasty.Annotation(Tasty.Type.Named(Tasty.SymbolId(makeSym("Foo").id)), Chunk.empty, Tasty.Name(""))
            ...
            case Result.Success(t) =>
                t match
                    case Tasty.Type.MatchType(_, scrutinee, cases) =>
                        ...
                        // Assert scrutinee is the named type parameter X.
                        scrutinee match
                            case Tasty.Type.Named(id) =>
                                // address-encoded ID; original symbol.id=-1, now SymbolId(PHASE_B_ADDR_OFFSET+address).
                                assert(
                                    id.value >= 0,
                                    s"scrutinee id should be address-encoded positive but was ${id.value}"
                                )
                            case other =>
    ...
    "decoding RECtype with RECthis back-reference completes without stack overflow" in {
        ...
        //   RECtype(100) [parent = RECthis(66) back-ref to address 0]
        // Layout: [address 0] RECtype(100) [parent bytes]
        // RECthis(66) = category 2: tag + recType_ASTRef Nat. recType_ASTRef = 0 (address of RECtype).
        // RECthis bytes: [66, 0]
        // RECtype is category 3: tag(100) + parent_Type (no length prefix in cat3!).
        // So bytes = [100, 66, 0]
        // In dotty TASTy: last byte of Nat has 0x80 SET. For address 0: encodeNat(0) = (0 | 0x80) = 0x80.
        val bytes = Array(
            TastyFormat.RECtype.toByte, // 100 at position 0
            TastyFormat.RECthis.toByte, // 66 at position 1
            (0 | 0x80).toByte           // Nat 0 with termination bit = references address 0 (the RECtype itself)
        )
    ...
    // TYPEREFin with unknown fully-qualified name: cross-file references resolve to synthetic unresolved symbols directly.
    "decoding TYPEREFin with unknown fully-qualified name returns Named(unresolved) type" in {
    ...
    // session.annotationDecodeErrors accumulates errors from truncated annotation arg bytes.
    // ANNOTATEDtype with a 1-byte payload (only UNITconst; no annotation term) produces an empty
    // pickle for the term slice. TreeUnpickler.readTree on an empty ByteView throws AIOOBE, which
    // is caught and converted to a MalformedSection error accumulated in the session.
    ...
    // A known category-1 tag (UNITconst) does NOT fire any log output.
    // Pins negative path.
