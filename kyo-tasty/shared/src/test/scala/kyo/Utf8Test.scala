        ...
        val failures = (0 until trials).flatMap { i =>
            ...
            while j < len do
                if rng.nextInt(10) == 0 then
                    ...
                    val classpath = 0x10000 + rng.nextInt(0x10ffff - 0x10000 + 1)
                    sb.append(new String(Character.toChars(classpath)))
                    j += 1
                else
                    // BMP code point in range U+0020.U+D7FF (avoids surrogates and control codes)
                    val classpath = 0x0020 + rng.nextInt(0xd7ff - 0x0020 + 1)
                    sb.appendCodePoint(classpath)
                    j += 1
                end if
            end while
            val s      = sb.toString
