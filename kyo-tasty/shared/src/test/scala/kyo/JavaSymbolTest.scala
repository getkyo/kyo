...
/** Tests for the unified Java symbol surface using Embedded fixture classfiles and EmbeddedClassfiles. */
class JavaSymbolTest extends kyo.test.Test[Any]:
    ...
    "symbol.fullName for ArrayRecord returns a non-empty string" in {
        import kyo.Tasty.SymbolId
        val bytes = kyo.fixtures.Embedded.arrayRecordClass
        Abort.run[TastyError] {
            readClassfile(bytes, new TypeArena).map { cr =>
                val symbol = toFinalSym(cr.classSymbol, idOverride = 0, ownerOverride = 0)
                Tasty.Classpath.fromPicklesWithSymbols(Chunk(symbol)).map { classpath =>
                    val s = classpath.symbols(0)
                    Sync.defer(classpath.computeFullName(s).asString).map { fullName =>
                        assert(fullName.nonEmpty || s.name.asString.nonEmpty, s"Expected non-empty name for ArrayRecord symbol")
                    }
                }
            }
        }
    ...
    "kyo.internal.tasty.symbol.BinaryName.compute(symbol, classpath) for ArrayRecord returns a non-empty string" in {
        import kyo.Tasty.SymbolId
        val bytes = kyo.fixtures.Embedded.arrayRecordClass
        Abort.run[TastyError] {
            readClassfile(bytes, new TypeArena).map { cr =>
                ...
                Tasty.Classpath.fromPicklesWithSymbols(Chunk(symbol)).map { classpath =>
                    val s  = classpath.symbols(0)
                    val bn = kyo.internal.tasty.symbol.BinaryName.compute(s, classpath)
    ...
    // Binary name "com/example/Foo$Bar" with no InnerClasses entry; dollar sign preserved literally.
    "top-level class with literal $ in binary name preserves $ in fullName" in {
        ...
        import kyo.Tasty.SymbolId
        Abort.run[TastyError] {
            readClass(fooBarClassBytes).map { cr =>
                val symbol = toFinalSym(cr.classSymbol, idOverride = 0, ownerOverride = 0)
                Tasty.Classpath.fromPicklesWithSymbols(Chunk(symbol)).map { classpath =>
                    val s  = classpath.symbols(0)
                    val bn = kyo.internal.tasty.symbol.BinaryName.compute(s, classpath)
    ...
    "symbol.isJava: true for Java classfile, false for TASTy" in {
    ...
    "symJavaMetadata(symbol): Present for Java, Absent for TASTy" in {
        val bytes = kyo.fixtures.Embedded.arrayRecordClass
        for
            javaResult <- readClass(bytes)
            tastySym   <- firstClassSymbolFromTasty("PlainClass.tasty")
    ...
    // Java records are automatically final; pointRecordClass exercises the ACC_FINAL bit.
    "JavaMetadata.accessFlags has ACC_FINAL bit set for a final class" in {
        ...
        readClass(bytes).map { result =>
            ...
            assert(symbol.flags.contains(Tasty.Flag.Final), "Expected Flag.Final for PointRecord (records are final)")
            val meta = symJavaMetadata(symbol)
    ...
    "Java record class produces Flag.JavaRecord and non-empty recordComponents" in {
        ...
        readClass(bytes).map { result =>
            val symbol = result.classSymbol
            assert(
                symbol.flags.contains(Tasty.Flag.JavaRecord),
                s"Expected Flag.JavaRecord for PointRecord, flags=${symbol.flags.bits}"
            )
            val meta = symJavaMetadata(symbol)
            assert(meta.isDefined, "Expected javaMetadata Present for PointRecord")
    ...
    // java.lang.Deprecated is annotated with @Retention(RetentionPolicy.RUNTIME) and @Documented.
    "JavaMetadata.annotations for java.lang.Deprecated includes @Retention" in {
        ...
        readClass(bytes).map { result =>
            ...
            val hasRetention = annotations.exists { annotation =>
                annotation.annotationClass.name.asString.contains("Retention")
            }
            assert(
                hasRetention,
    ...
    // ClassfileUnpickler stores enclosing-method data in ClassfileResult.enclosingMethodData.
    "JavaMetadata.enclosingMethod data is Present for an anonymous class inside enclosingMethodFixture" in {
        ...
        readClass(bytes).map { result =>
            ...
            val (encFullName, methodName) = result.enclosingMethodData.get
            assert(
                methodName == "enclosingMethodFixture",
                s"Expected enclosingMethod name 'enclosingMethodFixture', got '$methodName'"
            )
            assert(
                encFullName.nonEmpty,
                "Expected non-empty enclosing class fully-qualified name"
            )
        }
    }
