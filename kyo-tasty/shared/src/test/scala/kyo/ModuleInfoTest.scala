...
class ModuleInfoTest extends kyo.test.Test[Any]:
    ...
    private def u1(buffer: ArrayBuffer[Byte], v: Int): Unit =
        buffer += (v & 0xff).toByte

    /** Append a u2 (big-endian) to a buffer. */
    private def u2(buffer: ArrayBuffer[Byte], v: Int): Unit =
        buffer += ((v >> 8) & 0xff).toByte
        buffer += (v & 0xff).toByte

    /** Append a u4 (big-endian) to a buffer. */
    private def u4(buffer: ArrayBuffer[Byte], v: Int): Unit =
        buffer += ((v >> 24) & 0xff).toByte
        buffer += ((v >> 16) & 0xff).toByte
        buffer += ((v >> 8) & 0xff).toByte
        buffer += (v & 0xff).toByte
    end u4
    ...
    private def buildModuleAttributePayload(
        ...
        u2(buffer, moduleNameIdx)
        u2(buffer, 0) // module_flags
        u2(buffer, moduleVerIdx)
        u2(buffer, requires.length)
        for (reqIdx, flags, verIdx) <- requires do
            u2(buffer, reqIdx)
            u2(buffer, flags)
            u2(buffer, verIdx)
        end for
        u2(buffer, exports.length)
        for (pkgIdx, flags, targets) <- exports do
            u2(buffer, pkgIdx)
            u2(buffer, flags)
            u2(buffer, targets.length)
            for t <- targets do u2(buffer, t)
        end for
        u2(buffer, opens.length)
        ...
        u2(buffer, uses.length)
        for u <- uses do u2(buffer, u)
        u2(buffer, provides.length)
        for (svcIdx, impls) <- provides do
            u2(buffer, svcIdx)
            u2(buffer, impls.length)
            for i <- impls do u2(buffer, i)
        end for
        buffer.toArray
    ...
    private def buildModuleInfoClass(
        ...
        u4(buffer, 0xcafebabe)   // magic
        u2(buffer, 0)            // minor_version
        u2(buffer, 55)           // major_version (Java 11 = 55 >= 53)
        u2(buffer, poolCount)    // constant_pool_count
        buffer ++= pool          // constant pool entries
        u2(buffer, 0x8000)       // access_flags ACC_MODULE
        u2(buffer, thisClassIdx) // this_class
        u2(buffer, 0)            // super_class
        u2(buffer, 0)            // interfaces_count
        u2(buffer, 0)            // fields_count
        u2(buffer, 0)            // methods_count
        u2(buffer, 1)            // attributes_count = 1
        // Module attribute
        u2(buffer, attrNameIdx)        // attribute_name_index
        u4(buffer, attrPayload.length) // attribute_length
        buffer ++= attrPayload         // attribute body
