...
class TypeAliasOpaqueTypedAccessorsTest extends kyo.test.Test[Any]:
    ...
    "TypeAlias.body field holds the alias body as a Type" in {
        ...
        Tasty.Classpath.fromPicklesWithSymbols(Chunk(intSymbol, typeAlias)).map { classpath =>
            given Tasty.Classpath = classpath
            typeAlias.body match
                case Maybe.Present(Tasty.Type.Named(sid)) =>
                    import Tasty.Name.asString
                    val resolved = classpath.symbol(sid)
                    assert(
    ...
    "TypeAlias.typeParams returns Chunk[TypeParam] size 2" in {
    ...
    "OpaqueType.body holds the underlying Type" in {
        ...
        Tasty.Classpath.fromPicklesWithSymbols(Chunk(longSymbol, opaqueType)).map { classpath =>
            given Tasty.Classpath = classpath
    ...
    "OpaqueType.bounds field exposes TypeBounds with correct upper type" in {
        ...
        Tasty.Classpath.fromPicklesWithSymbols(Chunk(intSymbol, opaqueType)).map { classpath =>
            given Tasty.Classpath    = classpath
    ...
    "OpaqueType.typeParams returns Chunk[TypeParam] size 1 name A" in {
    ...
    "TypedSymbolFactory.from accumulates UnknownType under SoftFail with absent declaredType" in {
        ...
        val symbol = TypedSymbolFactory.from(d, Tasty.ErrorMode.SoftFail, Maybe.Present(accErrors), "soft.tasty", 0L)
