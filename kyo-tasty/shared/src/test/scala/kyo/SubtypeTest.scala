...
/** Tests for Subtype checking and type comparison.
  ...
  * test uses makeTestClasspath to register symbols at their id.value indices so classpath.symbol(id) resolves correctly.
  */
class SubtypeTest extends kyo.test.Test[Any]:
    ...
    /** Build a Symbol with a unique id and the given fully-qualified name / parents. */
    ...
    /** Wire a base-class symbol with the given type params.
      ...
      */
    private def wireTypeParams(symbol: Tasty.Symbol.Class, params: Chunk[Tasty.Symbol]): Tasty.Symbol.Class =
        symbol.copy(typeParamIds = params.map(_.id))

    /** Build a test Classpath populated with the given symbols, indexed by their id.value.
      *
      * All symbols must have distinct non-negative id.value to be reachable via classpath.symbol(id). Any gap in the id range is
    ...
    "Named(A).isSubtypeOf(Named(A)) returns Sub (reflexivity)" in {
        ...
        makeTestClasspath(Chunk(intSym)).map { classpath =>
            assert(classpath.isSubtypeOf(intType, intType) == Result.Success(Tasty.SubtypeVerdict.Sub))
        }
    }
    ...
    "Named(String).isSubtypeOf(Named(Object)) returns Sub via parent chain" in {
        ...
        makeTestClasspath(Chunk(objectSym, stringSym)).map { classpath =>
            assert(classpath.isSubtypeOf(stringType, objectType) == Result.Success(Tasty.SubtypeVerdict.Sub))
        }
    }
    ...
    "Named(String).isSubtypeOf(Named(Int)) returns NotSub" in {
        ...
        val intType    = Tasty.Type.Named(intSym.id)
        val stringType = Tasty.Type.Named(stringSym.id)
        makeTestClasspath(Chunk(intSym, stringSym)).map { classpath =>
            assert(classpath.isSubtypeOf(stringType, intType) == Result.Success(Tasty.SubtypeVerdict.NotSub))
        }
    }
    ...
    "AndType(A, B).isSubtypeOf(A) returns Sub" in {
        ...
        makeTestClasspath(Chunk(symA, symB)).map { classpath =>
            assert(classpath.isSubtypeOf(andType, typeA) == Result.Success(Tasty.SubtypeVerdict.Sub))
        }
    }
    ...
    "A.isSubtypeOf(OrType(A, B)) returns Sub" in {
        ...
        makeTestClasspath(Chunk(symA, symB)).map { classpath =>
            assert(classpath.isSubtypeOf(typeA, orType) == Result.Success(Tasty.SubtypeVerdict.Sub))
        }
    }
    ...
    "Applied(List[String]).isSubtypeOf(Applied(List[AnyRef])) Sub when List is covariant" in {
        ...
        makeTestClasspath(Chunk(anyRefSym, stringSym, tParam, listSym)).map { classpath =>
            assert(classpath.isSubtypeOf(listString, listAnyRef) == Result.Success(Tasty.SubtypeVerdict.Sub))
        }
    }
    ...
    "Named(Nothing).isSubtypeOf(any type) returns Sub (bottom)" in {
        ...
        makeTestClasspath(Chunk(nothingSym, anySym)).map { classpath =>
            assert(classpath.isSubtypeOf(nothingType, anyType) == Result.Success(Tasty.SubtypeVerdict.Sub))
        }
    }
    ...
    "TypeLambda([T], C[T]).isSubtypeOf(TypeLambda([U], C[U])) Sub (alpha-equiv)" in {
        ...
        makeTestClasspath(Chunk(cSym, tSym, uSym)).map { classpath =>
            assert(classpath.isSubtypeOf(lambda1, lambda2) == Result.Success(Tasty.SubtypeVerdict.Sub))
        }
    }
    ...
    "Rec type with RecThis back-reference terminates (budget exhaustion safety)" in {
        ...
        makeTestClasspath(Chunk(cSym)).map { classpath =>
            classpath.isSubtypeOf(rec, rec) match
                ...
                case other => fail(s"expected a subtype verdict but got $other")
        }
    }
    ...
    "Int <: Any returns Sub" in {
        ...
        val intType = Tasty.Type.Named(intSym.id)
        val anyType = Tasty.Type.Named(anySym.id)
        makeTestClasspath(Chunk(intSym, anySym)).map { classpath =>
            assert(classpath.isSubtypeOf(intType, anyType) == Result.Success(Tasty.SubtypeVerdict.Sub))
        }
    }
    ...
    "String <: Int returns NotSub" in {
        ...
        val stringType = Tasty.Type.Named(stringSym.id)
        val intType    = Tasty.Type.Named(intSym.id)
        makeTestClasspath(Chunk(stringSym, intSym)).map { classpath =>
            assert(classpath.isSubtypeOf(stringType, intType) == Result.Success(Tasty.SubtypeVerdict.NotSub))
        }
    }
    ...
    "budget exhaustion returns Indeterminate" in {
        ...
        val intType    = Tasty.Type.Named(intSym.id)
        makeTestClasspath(Chunk(stringSym, intSym)).map { classpath =>
            val result = kyo.internal.tasty.type_.Subtyping.isSubtype(stringType, intType, classpath, budget = 0)
            assert(result == Result.Success(Tasty.SubtypeVerdict.Indeterminate))
            succeed
        }
    }
    ...
    "real 66-deep Rec chain exhausts default budget=64 and returns Indeterminate" in {
        ...
        makeTestClasspath(Chunk(leafSym)).map { classpath =>
            assert(
                ...
                "Expected Indeterminate from real 66-deep Rec chain (budget=64)"
            )
        }
    }
    ...
    "empty parent chain returns NotSub" in {
        ...
        makeTestClasspath(Chunk(fooSym, barSym)).map { classpath =>
            assert(
                classpath.isSubtypeOf(fooType, barType) == Result.Success(Tasty.SubtypeVerdict.NotSub),
    ...
    // Named(X).isSubtypeOf(Type.Any) returns Sub via ADT sentinel arm
    "Named(X).isSubtypeOf(Type.Any) == Sub (ADT sentinel)" in {
        nextId = 0
        val intSym  = makeSym("scala.Int")
        val intType = Tasty.Type.Named(intSym.id)
        makeTestClasspath(Chunk(intSym)).map { classpath =>
            assert(classpath.isSubtypeOf(intType, Tasty.Type.Any) == Result.Success(Tasty.SubtypeVerdict.Sub))
        }
    }

    // Type.Nothing.isSubtypeOf(Named(X)) returns Sub via ADT sentinel arm
    "Type.Nothing.isSubtypeOf(Named(X)) == Sub (ADT sentinel)" in {
        nextId = 0
        val stringSym  = makeSym("java.lang.String")
        val stringType = Tasty.Type.Named(stringSym.id)
        makeTestClasspath(Chunk(stringSym)).map { classpath =>
            assert(classpath.isSubtypeOf(Tasty.Type.Nothing, stringType) == Result.Success(Tasty.SubtypeVerdict.Sub))
        }
    }

    // Type.Nothing.isSubtypeOf(Type.Any) returns Sub (both ADT sentinels)
    "Type.Nothing.isSubtypeOf(Type.Any) == Sub (both ADT sentinels)" in {
        nextId = 0
        makeTestClasspath(Chunk.empty).map { classpath =>
            assert(classpath.isSubtypeOf(Tasty.Type.Nothing, Tasty.Type.Any) == Result.Success(Tasty.SubtypeVerdict.Sub))
        }
    }

    // Type.Any.isSubtypeOf(Named(X)) returns NotSub (Any is not sub of random type)
    "Type.Any.isSubtypeOf(Named(X)) == NotSub (negative pinning)" in {
        nextId = 0
        val intSym  = makeSym("scala.Int")
        val intType = Tasty.Type.Named(intSym.id)
        makeTestClasspath(Chunk(intSym)).map { classpath =>
            assert(classpath.isSubtypeOf(Tasty.Type.Any, intType) == Result.Success(Tasty.SubtypeVerdict.NotSub))
        }
    }

    // Named(X).isSubtypeOf(Type.Nothing) returns NotSub (Nothing is bottom type)
    "Named(X).isSubtypeOf(Type.Nothing) == NotSub (negative pinning)" in {
        nextId = 0
        val stringSym  = makeSym("java.lang.String")
        val stringType = Tasty.Type.Named(stringSym.id)
        makeTestClasspath(Chunk(stringSym)).map { classpath =>
            assert(classpath.isSubtypeOf(stringType, Tasty.Type.Nothing) == Result.Success(Tasty.SubtypeVerdict.NotSub))
        }
    }
