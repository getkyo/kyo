...
/** ScalaCheck-style property tests that verify kyo-tasty decoder stability under adversarial input.
  ...
  *   PROP-PB-003: idempotency - loading same fixture N times produces same symbol count and fully-qualified name set
  *
...
class TastyPropertyBasedTest extends kyo.test.Test[Any]:
    ...
    /** Attempt to load bytes as a TASTy classpath; return the error class name if a panic occurs.
      ...
      */
    private def tryDecode(bytes: Array[Byte]): Option[String] < (Async) =
        val pickle = Tasty.Pickle("probe", Tasty.Version(28, 3, 0), Span.from(bytes))
    ...
    // and equal fully-qualified name index sizes. Guards against non-deterministic initialization.
    "loading fixture classpath 5 times produces equal symbol counts and fully-qualified name index sizes" in {
        val plainPickle = Tasty.Pickle("plain-class", Tasty.Version(28, 3, 0), Span.from(kyo.fixtures.Embedded.plainClassTasty))
        def loadOnce(using Frame): (Int, Int) < (Async & Abort[TastyError]) =
            Tasty.withPickles(Chunk(plainPickle)) {
                Tasty.classpath.map { classpath =>
                    (classpath.symbols.size, classpath.indices.byFullName.size)
                }
            }
        end loadOnce
        def go(n: Int, acc: List[(Int, Int)]): List[(Int, Int)] < (Async & Abort[TastyError]) =
            if n == 0 then acc
            else
                loadOnce.map { pair =>
                    go(n - 1, acc :+ pair)
        ...
        Abort.run[TastyError](go(5, Nil)).map {
            case Result.Success(pairs) =>
                ...
                val fullNameCounts = pairs.map(_._2).distinct
                assert(
                    symbolCounts.size == 1,
                    s"PROP-PB-003: symbol counts differ across 5 loads: $symbolCounts"
                )
                assert(
                    fullNameCounts.size == 1,
                    s"PROP-PB-003: fullNameIndex sizes differ across 5 loads: $fullNameCounts"
                )
                succeed
    ...
    "bit-flipped fixture either fails cleanly or reports errors (seed=0xdeadbeef)" in {
        ...
        def tryOnce(offset: Int): Option[String] < Async =
            val flipped = flipOneByte(offset)
            val p       = Tasty.Pickle(s"flipped-$offset", Tasty.Version(28, 3, 0), Span.from(flipped))
            Abort.run[TastyError] {
                Tasty.withPickles(Chunk(p)) {
                    Tasty.classpath
                }
            }.map {
                case Result.Success(classpath) =>
                    if classpath.symbols.size > 0 && classpath.errors.isEmpty then
                        None
                    else
                        None
                case Result.Failure(_) => None
