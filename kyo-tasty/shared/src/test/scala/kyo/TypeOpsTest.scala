...
class TypeOpsTest extends kyo.test.Test[Any]:
    ...
    private def makeNamedSym(fullName: String): Tasty.Type.Named =
        val leafName = fullName.split("\\.").last
        val symbol   = Tasty.Symbol.Package(Tasty.SymbolId(-1), Tasty.Name(leafName), Tasty.Flags.empty, Tasty.SymbolId(-1), Chunk.empty)
        Tasty.Type.Named(symbol.id)
    end makeNamedSym
