...
class BodyTreeErrorChannelTest extends kyo.test.Test[Any]:
    ...
    private def bindingWithBody(symbol: Tasty.Symbol, body: SymbolBody): Binding =
        val classpath = Tasty.Classpath(
            symbols = Chunk(symbol),
            indices = Tasty.Classpath.Indices.empty,
            errors = Chunk.empty,
            modules = Chunk.empty,
            rootSymbolId = Tasty.SymbolId(symbol.id.value)
        )
        val ctx = DecodeContext.fresh()
        ctx.bodyStore.put(symbol.id, body)
        Binding(classpath, Maybe.Present(ctx))
    end bindingWithBody
    ...
    "cached decode failure re-serves as MalformedSection on second bodyTree call" in {
        ...
        val binding = bindingWithBody(symbol, body)
        for
            r1 <- Abort.run[TastyError](Tasty.bindingLocal.let(Maybe.Present(binding))(Tasty.bodyTree(symbol)))
            r2 <- Abort.run[TastyError](Tasty.bindingLocal.let(Maybe.Present(binding))(Tasty.bodyTree(symbol)))
        yield
            r1 match
