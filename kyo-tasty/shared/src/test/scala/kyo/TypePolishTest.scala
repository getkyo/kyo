...
class TypePolishTest extends kyo.test.Test[Any]:
    ...
    "symbol returns Present(symbol) for Type.Named" in {
        ...
        Tasty.Classpath.fromPicklesWithSymbols(Chunk(classSym)).map { classpath =>
            val t: Tasty.Type = Tasty.Type.Named(SymbolId(0))
            val result        = classpath.typeSymbol(t)
            assert(result.isDefined, "symbol must return Present for Named")
            result match
                case Maybe.Present(symbol: Tasty.Symbol.Class) =>
    ...
    "symbol returns Absent for non-Named types" in {
        Tasty.withPickles(Chunk.empty) {
            Tasty.classpath.map { classpath =>
                ...
                assert(!classpath.typeSymbol(t).isDefined, "symbol must return Absent for non-Named type")
            }
        }
    }
    ...
    "foreach visits nodes in pre-order" in {
        ...
        val buffer  = ArrayBuffer.empty[Tasty.Type]
        applied.foreach(t => buffer += t)
        assert(buffer.length == 3, s"Expected 3 nodes in pre-order, got ${buffer.length}")
        assert(buffer(0) == applied, "First node must be self (Applied)")
        assert(buffer(1) == n1, "Second node must be base (Named(1))")
        assert(buffer(2) == n2, "Third node must be arg (Named(2))")
        succeed
    }
    ...
    "show returns non-empty String for every Type case" in {
        ...
        Tasty.Classpath.fromPicklesWithSymbols(Chunk(classSym)).map { classpath =>
            val cases: Chunk[Tasty.Type] = Chunk(
                ...
                Tasty.Type.Annotated(n, Tasty.Annotation(n, Chunk.empty, Tasty.Name(""))),
            ...
            cases.foreach { t =>
                assert(classpath.typeShow(t).nonEmpty, s"show returned empty string for ${t.getClass.getSimpleName}")
            }
            succeed
        }
    }
