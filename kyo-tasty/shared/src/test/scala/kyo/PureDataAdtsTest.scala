...
class PureDataAdtsTest extends kyo.test.Test[Any]:
    ...
    "removed Symbol methods are off the surface" in {
    ...
    "Symbol.EnumCase is NOT a subtype of Symbol.Class" in {
        ...
        assert(__tcErrors5 > 0, "Assigning EnumCase to Symbol.Class must be a type error (EnumCase is not a subtype of Class)")
        succeed
    }
