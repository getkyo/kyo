...
/** Test helper: cold-vs-warm field-equivalence of Tasty.Classpath pairs.
  *
  * Compares a cold-loaded Classpath against a warm (snapshot-loaded) Classpath on axes that are expected to be equal:
  *
  *   symbols.size
  *   fullNameIndex.size
  *   packageIds.size
  ...
  * final symbols. The warm-load (SnapshotReader) reconstructs it from per-symbol fully-qualified name presence and kind, which may include nested class
  ...
  * The axis that matters is fullNameIndex.size.
  *
...
private[kyo] object SnapshotEquivalence:
    ...
    private[kyo] def warmColdEquivalent(cold: Tasty.Classpath, warm: Tasty.Classpath): EquivResult =
        check("symbols.size", cold.symbols.size, warm.symbols.size)
            .orElse(check("fullNameIndex.size", cold.indices.byFullName.size, warm.indices.byFullName.size))
    ...
    private[kyo] def countUnresolvedRefs(classpath: Tasty.Classpath): Int =
