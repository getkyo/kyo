...
class DecoderFidelity5ExplorationTest extends kyo.test.Test[Any]:
    ...
    /** Load corrupt TASTy bytes via withPickles with SoftFail. */
    ...
    "truncate at size/4 produces clean error, no panic" in {
        ...
        loadCorrupt("PlainClassTrunc4.tasty", bytes).map { classpath =>
            assert(
                classpath.errors.nonEmpty,
                s"Expected errors for tasty truncated at size/4; got empty errors. classpath.symbols=${classpath.symbols.length}"
            )
            val acceptable = classpath.errors.forall {
            ...
            assert(
                acceptable,
                s"Unexpected error variants for truncated file: ${classpath.errors}"
            )
            succeed
        }
    }
    ...
    "truncate at size/2 produces clean error, no panic" in {
        ...
        loadCorrupt("PlainClassTrunc2.tasty", bytes).map { classpath =>
            assert(
                classpath.errors.nonEmpty,
                s"Expected errors for tasty truncated at size/2; got empty. classpath.symbols=${classpath.symbols.length}"
            )
            val acceptable = classpath.errors.forall {
                case _: TastyError.MalformedSection => true
                case _: TastyError.CorruptedFile    => true
            ...
            assert(acceptable, s"Unexpected error variants: ${classpath.errors}")
            succeed
        }
    }
    ...
    "truncate at size-1 produces clean result or clean error, no panic" in {
        ...
        loadCorrupt("PlainClassTrunc1.tasty", bytes).map { classpath =>
            val errorsAreKnownTypes = classpath.errors.forall {
                case _: TastyError.MalformedSection => true
                case _: TastyError.CorruptedFile    => true
                case _: TastyError.FileNotFound     => true
                case _                              => false
            ...
            assert(
                errorsAreKnownTypes,
                s"Unexpected error variants after size-1 truncation: ${classpath.errors}"
            )
            succeed
        }
    }
    ...
    "bit-flip magic byte produces clean CorruptedFile error, no panic" in {
        ...
        loadCorrupt("PlainClassFlipped.tasty", bytes).map { classpath =>
            assert(
                classpath.errors.nonEmpty,
                s"Expected errors after bit-flipping magic byte; got empty. classpath.symbols=${classpath.symbols.length}"
            )
            val hasCorruptedOrMalformed = classpath.errors.exists {
                case _: TastyError.CorruptedFile    => true
                case _: TastyError.MalformedSection => true
                case _                              => false
            }
            assert(
                ...
                s"Expected CorruptedFile or MalformedSection for flipped-magic file; got: ${classpath.errors}"
            )
            succeed
        }
    }
    ...
    "corrupt last 16 bytes with 0xFF produces clean result or clean error" in {
        ...
        loadCorrupt("PlainClassCorruptTail.tasty", bytes).map { classpath =>
            val errorsAreKnownTypes = classpath.errors.forall {
                case _: TastyError.MalformedSection => true
                case _: TastyError.CorruptedFile    => true
                case _: TastyError.FileNotFound     => true
            ...
            assert(
                errorsAreKnownTypes,
                s"Unexpected error variants after tail corruption: ${classpath.errors}"
            )
            succeed
        }
    }
    ...
    "findClass empty string returns Absent, no exception" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            val result = classpath.findClass("")
            assert(
                result == Maybe.Absent,
    ...
    "findClass very-long fully-qualified name returns Absent, no exception" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            val longFullName = "a" * 1000
            val result       = classpath.findClass(longFullName)
            assert(
                result == Maybe.Absent,
    ...
    "findClass(null) returns Absent or specific error, no NPE" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            ...
            try
                result = classpath.findClass(null)
            catch
            ...
            result match
                ...
                case Result.Success(classpath) =>
                    Sync.defer {
                        ...
                        try
                            val _syms   = classpath.symbols.length
                            val _errors = classpath.errors.length
                            val _absent = classpath.findClass("kyo.DoesNotExist")
    ...
    "snapshot with wrong magic produces SnapshotFormatError, no panic" in {
        ...
        Abort.run[TastyError](SnapshotReader.readFromBytes(badBytes, snapPath)).map {
    ...
    "truncated KRFL snapshot produces clean error, no panic" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            ...
            val fullBytes = SnapshotWriter.serializeToBytes(classpath, digest)
            val halfBytes = fullBytes.take(fullBytes.length / 2)
            val snapPath  = "mem/truncated.krfl"
            Abort.run[TastyError](SnapshotReader.readFromBytes(halfBytes, snapPath)).map {
    ...
    "class with most methods decodes all methods without Named(-1)" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            ...
            val classWithMostMethods = classpath.allClassLike.toIndexedSeq.maxByOption { cl =>
                cl.declarationIds.count { id =>
                    classpath.symbol(id).isInstanceOf[Tasty.Symbol.Method]
                }
            }
            classWithMostMethods match
                ...
                case Some(cls) =>
                    ...
                    assert(
                        methods.nonEmpty || classpath.allClassLike.isEmpty,
