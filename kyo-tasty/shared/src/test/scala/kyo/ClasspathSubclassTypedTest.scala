...
class ClasspathSubclassTypedTest extends kyo.test.Test[Any]:
    ...
    private def directSubclassFixture(using Frame): Tasty.Classpath < Sync =
        Sync.defer {
            ...
            Tasty.Classpath.make(
                ...
                fullNameIndex = Dict("A" -> SymbolId(0), "B" -> SymbolId(1)),
    ...
    private def implementationsFixture(using Frame): Tasty.Classpath < Sync =
        Sync.defer {
            ...
            Tasty.Classpath.make(
                ...
                packageIds = Chunk.empty,
                fullNameIndex =
                    Dict("T" -> SymbolId(0), "B" -> SymbolId(1), "C" -> SymbolId(2), "AbsA" -> SymbolId(3), "CFromA" -> SymbolId(4)),
                packageIndex = Dict.empty,
                subclassIndex = Dict(
    ...
    "directSubclassesOf returns Chunk[ClassLike] with direct subclasses" in {
        directSubclassFixture.map { classpath =>
            classpath.findClass("A") match
                case Maybe.Present(a) =>
                    val direct: Chunk[Tasty.Symbol.ClassLike] = classpath.directSubclassesOf(a)
    ...
    "implementationsOf returns only concrete Class instances transitively" in {
        implementationsFixture.map { classpath =>
            classpath.findClassLike("T") match
                case Maybe.Present(t) =>
                    val impls: Chunk[Tasty.Symbol.Class] = classpath.implementationsOf(t)
