...
class SymbolAdtVariantCoverageTest extends kyo.test.Test[Any]:
    ...
    "every Symbol subtype carries sourcePosition as Maybe[Position]" in {
        ...
        for symbol <- withPresent do
            symbol.sourcePosition match
                case Maybe.Present(p) =>
                    assert(
                        p.sourceFile == "T.scala",
                        s"${symbol.getClass.getSimpleName}: expected sourceFile='T.scala' but got '${p.sourceFile}'"
                    )
                case Maybe.Absent =>
                    fail(s"${symbol.getClass.getSimpleName}: expected Present sourcePosition but got Absent")
            end match
        end for
    ...
    "unresolvedCaseDeleted: Symbol.Unresolved is not part of the ADT" in {
        // Symbol.Unresolved is not a member of Symbol; the typeCheckErrors probe must return non-empty.
        val __tcErrors6 = compiletime.testing.typeCheckErrors("(??? : kyo.Tasty.Symbol.Unresolved)").length

        assert(__tcErrors6 > 0, "Symbol.Unresolved must not exist")
        succeed
    }
    ...
    "makePlaceholderDeleted: Symbol.makePlaceholder is not available" in {
        // Symbol.makePlaceholder is not on the surface; the typeCheckErrors probe must return non-empty.
        val __tcErrors7 = compiletime.testing.typeCheckErrors("kyo.Tasty.Symbol.makePlaceholder(???, ???, ???)").length

        assert(__tcErrors7 > 0, "Symbol.makePlaceholder must not exist")
        succeed
    }
