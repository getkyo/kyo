...
class RealClasspathFidelity2Test extends Fidelity2TestBase:
    ...
    "no unknown-tag warnings on clean classpath load" in {
        TestClasspaths2.loadEmbeddedWithSink.map { (classpath, sink) =>
            val unknownTagCount = sink.unknownTagCount
            assert(
                unknownTagCount == 0,
                s"Expected 0 unknown-tag warnings, found $unknownTagCount. " +
    ...
    "embedded-fixture classpath produces non-zero symbol count" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            assert(
                ...
                s"Expected > 0 symbols from embedded-fixture classpath; got ${classpath.symbols.size}"
            )
            succeed
        }
    }
