...
class VarintTest extends kyo.test.Test[Any]:
    ...
    "readNat decodes 6-byte non-minimal encoding" in {
    ...
    "writeNat then readNat round-trips 1234" in {
        ...
        Varint.writeNat(buffer, 1234)
        val view = viewOf(buffer.toArray)
    ...
    "writeLongNat then readLongNat round-trips 9_999_999_999L" in {
        import AllowUnsafe.embrace.danger
        val buffer = scala.collection.mutable.ArrayBuffer.empty[Byte]
        Varint.writeLongNat(buffer, 9_999_999_999L)
        val view = viewOf(buffer.toArray)
    ...
    "writeLongNat then readLongNat round-trips 100 seeded random non-negative Longs" in {
        ...
        val failures = (0 until trials).flatMap { _ =>
            ...
            val buffer = scala.collection.mutable.ArrayBuffer.empty[Byte]
            Varint.writeLongNat(buffer, v)
            val result = Varint.readLongNat(viewOf(buffer.toArray))
