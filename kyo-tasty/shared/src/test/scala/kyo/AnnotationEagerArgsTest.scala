...
/** Phase 08 pinning tests for INV-006: Annotation.args eagerly decoded; failures accumulate in cp.errors.
  *
  * Leaf 1: annotation args populated at open time -- accessing Annotation.args is a plain Maybe[Tree] field access with no effect row. Leaf
  * 2: decode failure stores Absent and accumulates error -- a corrupt annotation pickle produces args = Absent. Leaf 3: no _decodeCtx field
  * on Annotation -- structural reflection confirms the pure case class shape. Leaf 4: Annotation case-class equality is structural.
  *
  * Pins: INV-006.
  */
class AnnotationEagerArgsTest extends Test:
    ...
    // Leaf 1 (INV-006): annotation args populated at open time.
    // After Phase 08, Annotation.args is a plain Maybe[Tree] field -- no effect row needed to access it.
    // This test verifies that a UNITconst annotation term eagerly decoded in ANNOTATEDtype produces
    // args = Present(Literal(UnitConst)) accessible without any Kyo effect.
    "INV-006 leaf 1: Annotation.args populated at open time as plain Maybe[Tree]" in run {
        val sym        = makeSym("Int")
        val symAddr    = 3
        ...
        Abort.run[TastyError](decodeType(bytes, addrMap, names)).map {
            case Result.Success(Tasty.Type.Annotated(_, ann)) =>
                // INV-006: args is a plain field -- access requires no effect row at all.
                ann.args match
    ...
    // Leaf 2 (INV-006): decode failure stores Absent and accumulates error.
    // A corrupt pickle (unknown tag below firstASTtag=60) causes a DecodeException.
    // The ANNOTATEDtype branch catches this, stores args = Absent, and accumulates the error
    // in DecodeSession.annotationDecodeErrors (which flows to cp.errors at file level).
    // This test verifies args = Absent directly; the full cp.errors path is covered by
    // the TypeUnpicklerTest Session interaction.
    "INV-006 leaf 2: decode failure produces args = Absent" in run {
        val sym        = makeSym("Int")
        val symAddr    = 3
        val addrMap    = IntMap(symAddr -> sym)
        val names      = Array(Tasty.Name("scala"))
        ...
        Abort.run[TastyError](decodeType(bytes, addrMap, names)).map {
            case Result.Success(Tasty.Type.Annotated(_, ann)) =>
                // INV-006: decode error produces args = Absent, not a Kyo failure.
    ...
    // Verify at compile time that _decodeCtx and argsPickle are not members of Annotation.
    // assertDoesNotCompile works on all three platforms (JVM, JS, Native).
    "INV-006 leaf 3: Annotation case class has no _decodeCtx or argsPickle field" in {
    ...
    // Two Annotation values with equal annotationType and equal args compare equal via ==.
    "INV-006 leaf 4: Annotation case-class equality is structural" in {
        val sym  = makeSym("Foo")
        val tpe  = Tasty.Type.Named(sym.id)
        val ann1 = Tasty.Annotation(tpe, Maybe.Absent)
        val ann2 = Tasty.Annotation(tpe, Maybe.Absent)
