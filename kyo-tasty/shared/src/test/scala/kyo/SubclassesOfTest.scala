...
class SubclassesOfTest extends kyo.test.Test[Any]:
    ...
    private def ladderClasspath(using Frame): Tasty.Classpath < Sync =
        ...
        Sync.defer {
            ...
            Tasty.Classpath.make(
                ...
                fullNameIndex = Dict("A" -> SymbolId(0), "B" -> SymbolId(1), "C" -> SymbolId(2), "D" -> SymbolId(3)),
    ...
    private def traitClasspath(using Frame): Tasty.Classpath < Sync =
        ...
        Sync.defer {
            ...
            Tasty.Classpath.make(
                ...
                topLevelClassIds = Chunk(SymbolId(0), SymbolId(1), SymbolId(2), SymbolId(3)),
                packageIds = Chunk.empty,
                fullNameIndex = Dict("T" -> SymbolId(0), "Impl1" -> SymbolId(1), "Impl2" -> SymbolId(2), "AbsImpl" -> SymbolId(3)),
                packageIndex = Dict.empty,
    ...
    // subclassesOf is transitive
    "subclassesOf returns transitive closure in BFS order" in {
        ladderClasspath.map { classpath =>
            import Tasty.Name.asString
            val a          = classpath.findClass("A").get
            val subclasses = classpath.subclassesOf(a)
    ...
    // implementationsOf excludes abstract classes and traits
    "implementationsOf excludes abstract subclasses" in {
        traitClasspath.map { classpath =>
            import Tasty.Name.asString
            val t     = classpath.findClassLike("T").get
            val impls = classpath.implementationsOf(t)
    ...
    // directSubclassesOf returns one-level subclasses
    "directSubclassesOf returns exactly one hop" in {
        ladderClasspath.map { classpath =>
            import Tasty.Name.asString
            val a      = classpath.findClass("A").get
            val direct = classpath.directSubclassesOf(a)
    ...
    "subclassesOf returns empty chunk for leaf class" in {
        ladderClasspath.map { classpath =>
            val d   = classpath.findClass("D").get
            val sub = classpath.subclassesOf(d)
