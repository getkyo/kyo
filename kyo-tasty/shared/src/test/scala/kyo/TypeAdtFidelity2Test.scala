...
/** Structural Type ADT fidelity tests.
  ...
  *   scala.reflect.Manifest$ and scala.reflect.Manifest fullNameIndex keys documented
  *
  * Note on JS/Native behavior: the embedded fixture set is small (internal fixtures compiled classes). Many count-positive assertions
  ...
  * example, AndType from APPLIEDtype(scala.&) may not appear in the embedded set.
  */
class TypeAdtFidelity2Test extends Fidelity2TestBase:
    ...
    //   APPLIEDtype(scala.&, [A, B]) require scala-library to resolve scala.& fully-qualified name; the embedded set
    ...
    //   APPLIEDtype(scala.|, [A, B]) or ORtype. Without scala-library in the embedded set, the fully-qualified name
    ...
    "TypeOps.applied(scala.& base, 2 args) collapses to AndType" in {
        ...
        val result  = TypeOps.applied(base, args, Maybe.Present(TypeOps.AndFullName))
        assert(
            result == Tasty.Type.AndType(intT, stringT),
            s"Expected TypeOps.applied with scala.& fullName to produce AndType; got $result"
        )
        succeed
    }
    ...
    "allMethods.count(isTransparentInline) >= 0 (structural non-negative guard)" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            val count = classpath.allMethods.count(_.isTransparentInline)
            assert(
                count >= 0,
    ...
    "allParameters.count(byName type) >= 0 (structural non-negative guard)" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            val count = classpath.allParameters.count(p => p.declaredType.isInstanceOf[Tasty.Type.ByName])
            assert(
                count >= 0,
    ...
    "OpaqueType symbols present in classpath (structural guard)" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            val opaqueCount = classpath.symbols.count(_.isInstanceOf[Tasty.Symbol.OpaqueType])
            assert(
                opaqueCount > 0,
    ...
    // ─────────────────────────────────────────────────────────────────────────
    //  scala.reflect fully-qualified name documentation
    // ─────────────────────────────────────────────────────────────────────────
    ...
    "scala.reflect.Manifest canonical key present in fullNameIndex" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            val dollarKey = classpath.indices.byFullName.get("scala.reflect.Manifest$")
            dollarKey match
                case Maybe.Present(_) =>
                    val cleanKey = classpath.indices.byFullName.get("scala.reflect.Manifest")
                    assert(
                        cleanKey.isDefined,
                        "scala.reflect.Manifest$ is in fullNameIndex but canonical form scala.reflect.Manifest is absent; FullNameNormalizer dual-index should add both"
                    )
                    succeed
                case Maybe.Absent =>
                    succeed
    ...
    coldWarmEquiv("snapshot round-trip preserves MatchCase count in TypeAlias bodies") { classpath =>
        classpath.symbols.foldLeft(0) {
