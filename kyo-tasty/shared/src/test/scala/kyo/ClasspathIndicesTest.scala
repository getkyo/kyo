...
class ClasspathIndicesTest extends kyo.test.Test[Any]:
    ...
    "dictRoundTripOverSchema" in {
        ...
        val classpath = Classpath.make(
            ...
            fullNameIndex = Dict("test.Foo" -> SymbolId(0)),
        ...
        val knownFullName = "test.Foo"
        val encoded       = Json.encode(classpath)
        val decoded       = Json.decode[Classpath](encoded)
        decoded match
            case Result.Success(decodedCp) =>
                val orig     = classpath.indices.byFullName.get(knownFullName)
                val decoded2 = decodedCp.indices.byFullName.get(knownFullName)
                assert(orig == decoded2, s"byFullName.get($knownFullName) mismatch: orig=$orig decoded=$decoded2")
                succeed
    ...
    "dictAggregationsBuilderPreservesIds" in {
        ...
        val result = classpath.indices.bySimpleName.get("Bar")
        result match
