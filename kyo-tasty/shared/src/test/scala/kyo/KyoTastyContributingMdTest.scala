...
class KyoTastyContributingMdTest extends kyo.test.Test[Any]:
    ...
    "Leaf 3: CONTRIBUTING.md resource is loadable with >= 120 lines" in {
    ...
    "Leaf 3a: AllowUnsafe four-site list is present" in {
        val content = loadText("CONTRIBUTING.md")
    ...
    "Leaf 3b: closed TastyError ADT and wire-format-minor-bump rule" in {
        val content = loadText("CONTRIBUTING.md")
    ...
    "Leaf 3c: Tasty.Java.* namespace convention documented" in {
        val content = loadText("CONTRIBUTING.md")
        assert(content.contains("Tasty.Java"), "CONTRIBUTING.md must document the Tasty.Java.* namespace convention")
        succeed
    }

    "Leaf 3d: producer/consumer split (LoadingSymbol and DecodeContext) documented" in {
        val content = loadText("CONTRIBUTING.md")
    ...
    "Leaf 3e: cross-platform stance documented (JVM and in-memory)" in {
        val content = loadText("CONTRIBUTING.md")
        assert(content.contains("JVM"), "CONTRIBUTING.md must document JVM-primary cross-platform stance")
        succeed
    }

    "Leaf 3f: INV-IMMUTABLE-ADT documented" in {
        val content = loadText("CONTRIBUTING.md")
        assert(
    ...
    "Leaf 4: scaladoc bar citation (8-35 lines / CONTRIBUTING.md 434-455) is present" in {
        val content    = loadText("CONTRIBUTING.md")
