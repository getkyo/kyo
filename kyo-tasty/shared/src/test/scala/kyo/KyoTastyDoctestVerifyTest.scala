...
class KyoTastyDoctestVerifyTest extends kyo.test.Test[Any]:
    ...
    "isSubtypeOf is a pure Classpath instance method, not a companion shortcut" in {
        val ok = typeCheckErrors(
            "val _: kyo.Result[kyo.TastyError, kyo.Tasty.SubtypeVerdict] = (null: kyo.Tasty.Classpath).isSubtypeOf(kyo.Tasty.Type.Any, kyo.Tasty.Type.Nothing)"
        )
        assert(ok.length == 0, s"classpath.isSubtypeOf(tpe, other) should type-check; errors: ${ok.map(_.message).mkString("; ")}")
        val bad = typeCheckErrors(
            "given kyo.Frame = kyo.Frame.internal; kyo.Tasty.isSubtypeOf(kyo.Tasty.Type.Any, kyo.Tasty.Type.Nothing)"
        )
        assert(bad.length > 0, "Tasty.isSubtypeOf must not exist on object Tasty; it is a Classpath instance method")
        succeed
    }

    "Type.isSubtypeOf does NOT exist on Type" in {
        val bad = typeCheckErrors("(null : kyo.Tasty.Type).isSubtypeOf(kyo.Tasty.Type.Nothing)")
        assert(bad.length > 0, "Type.isSubtypeOf should not exist on Type (it is a Classpath instance method)")
        succeed
    }
    ...
    "paramLists is a pure Classpath instance method with return type Chunk[Chunk[Symbol.Parameter]]" in {
        val ok = typeCheckErrors(
            "val _: kyo.Chunk[kyo.Chunk[kyo.Tasty.Symbol.Parameter]] = (null: kyo.Tasty.Classpath).paramLists(null: kyo.Tasty.Symbol.Method)"
        )
        assert(ok.length == 0, s"classpath.paramLists should type-check with locked signature; errors: ${ok.map(_.message).mkString("; ")}")
        val notOnCompanion = typeCheckErrors(
            "given kyo.Frame = kyo.Frame.internal; kyo.Tasty.paramLists(null: kyo.Tasty.Symbol.Method)"
        )
        assert(notOnCompanion.length > 0, "Tasty.paramLists must not exist on object Tasty; it is a Classpath instance method")
        val bad = typeCheckErrors(
