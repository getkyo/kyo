...
/** Compile-time extraction of the dotted full name of `A`'s class symbol.
  *
  * Used by `Tasty.classFullName[A]`. Reads `TypeRepr.of[A].typeSymbol.fullName` so the result is the bare
...
private[kyo] object ClassFullNameMacro:
    ...
    def impl[A](using Quotes, Type[A]): Expr[String] =
        ...
        val typeSymbol = TypeRepr.of[A].dealias.typeSymbol
        if !typeSymbol.exists then
            report.errorAndAbort(s"Tasty.classFullName: type ${Type.show[A]} has no class symbol")
        Expr(typeSymbol.fullName)
    end impl

end ClassFullNameMacro
