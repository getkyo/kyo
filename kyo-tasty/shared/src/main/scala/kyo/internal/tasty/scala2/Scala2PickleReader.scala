...
object Scala2PickleReader:
    ...
    def readRaw(
        ...
        arena: TypeArena
    )(using Frame, AllowUnsafe): Result[TastyError, Scala2PickleResult] =
    ...
    def readScalaSig(
        ...
        val decoded = decodeCompact(attrBytes)
    ...
    def readScalaAttr(
        ...
        InflateHook.inflateUnsafe(attrBytes).flatMap { inflated =>
    ...
    private def parsePickle(
        ...
        if bytes.length < 2 then
            Result.Failure(TastyError.CorruptedFile("<Scala2Pickle>", 0L, s"pickle too short: ${bytes.length} bytes"))
        else
            val major = bytes(0) & 0xff
            val minor = bytes(1) & 0xff
            if major != MajorVersion then
                Result.Failure(TastyError.CorruptedFile(
                    "<Scala2Pickle>",
                    0L,
                ...
                if cursor.remaining <= 0 then
                    Result.Failure(TastyError.CorruptedFile("<Scala2Pickle>", 2L, "missing entry count"))
                else
                    val count = cursor.readNat()
    ...
    private def readEntryList(
    ...
    )(using Frame, AllowUnsafe): Result[TastyError, Chunk[PickleEntry]] =
        if idx >= total then Result.Success(acc)
            ...
            if cursor.remaining < dataLen then
                ...
                val data = cursor.readBytes(dataLen)
    ...
    private def buildResult(
        ...
        entries.iterator.zipWithIndex.foreach { case (entry, i) =>
            entry.tag match
                case CLASSsym =>
                    val symbol = decodeClassSym(entry.data, i, nameTable, scala2Flags)
                    symbols = symbols.appended(symbol)
                    if firstClass.isEmpty then
                        firstClass = Present(symbol)
                        classParen = Chunk(buildAnyRefParent())
                case MODULEsym =>
                    val symbol = decodeModuleSym(entry.data, i, nameTable, scala2Flags)
                    symbols = symbols.appended(symbol)
                case VALsym =>
                    val symbol = decodeValSym(entry.data, i, nameTable, scala2Flags)
                    symbols = symbols.appended(symbol)
                case ALIASsym =>
                    val symbol = decodeAliasSym(entry.data, i, nameTable, scala2Flags)
                    symbols = symbols.appended(symbol)
                case TYPEsym =>
                    val symbol = decodeTypeSym(entry.data, i, nameTable, scala2Flags)
                    symbols = symbols.appended(symbol)
                case EXTref =>
                    val symbol = decodeExtRef(entry.data, i, nameTable, entries, scala2Flags)
                    symbols = symbols.appended(symbol)
                case EXTMODCLASSref =>
                    val symbol = decodeExtModClassRef(entry.data, i, nameTable, entries, scala2Flags)
                    symbols = symbols.appended(symbol)
                // Carve-out: Scala 2 pickle tag-byte dispatch; non-symbol entries are skipped
                case _ => ()
        }
    ...
    /** VALsym data layout: nameRef(nat) ownerRef(nat) flags(longNat) [privateWithinRef(nat)] infoRef(nat)
      ...
      * `Type.Function(Chunk.empty, Named(symbol))` placeholder is stored as `declaredType`, representing a zero-argument method
    ...
    private def decodeValSym(
        ...
        // Self-referential Type.Named(symbol) is approximated with Absent.
    ...
    /** ALIASsym data layout: nameRef(nat) ownerRef(nat) flags(longNat) infoRef(nat)
      ...
      * `declaredType`. This means `symbol.declaredType` for a Scala 2 type alias will return a placeholder rather than the real aliased type.
      */
    ...
    /** EXTref data layout: nameRef(nat) [ownerRef(nat)]
      ...
      * indexes another entry (typically EXTref or EXTMODCLASSref) whose fully-qualified name forms the qualifier. The resulting symbol has
    ...
    private def decodeExtRef(
        ...
        val ownerFullName = ownerRefOpt.map(resolveExtFullName(_, nameTable, entries, Set.empty[Int])).filter(_.nonEmpty)
        val fullName      = ownerFullName.fold(symName)(q => q + "." + symName)
        // declaredType=Absent (Named(symbol) self-ref approximation).
        makePickleSym(SymbolKind.Class, baseFlags, fullName)
    end decodeExtRef
    ...
    private def decodeExtModClassRef(
        ...
        val ownerFullName   = ownerRefOpt.map(resolveExtFullName(_, nameTable, entries, Set.empty[Int])).filter(_.nonEmpty)
        val fullName        = ownerFullName.fold(moduleClassName)(q => q + "." + moduleClassName)
        // declaredType=Absent (Named(symbol) self-ref approximation).
        makePickleSym(SymbolKind.Class, baseFlags, fullName)
    end decodeExtModClassRef

    /** Resolve the fully-qualified name string for an EXTref or EXTMODCLASSref owner entry index.
      *
      ...
      * fully-qualified name without allocating new symbols. Returns empty string for unrecognized or missing entries.
      *
      ...
      */
    private def resolveExtFullName(
        entryIdx: Int,
        nameTable: scala.collection.mutable.HashMap[Int, String],
        entries: Chunk[PickleEntry],
        visited: Set[Int]
                ...
                case Absent =>
                    if entryIdx < 0 || entryIdx >= entries.length then ""
                        ...
                        if refEntry.tag == EXTref || refEntry.tag == EXTMODCLASSref then
                            ...
                            val ownerFullName = ownerRefOpt.map(resolveExtFullName(_, nameTable, entries, nextVisited)).filter(_.nonEmpty)
                            ownerFullName.fold(leafName)(q => q + "." + leafName)
                        else ""
                        end if
    end resolveExtFullName
