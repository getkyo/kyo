...
/** Structural key for Tasty.Type values used by TypeArena's hash-cons map.
  ...
  * except for Named(symbol) where symbol identity is used because symbols are canonical after finalizeMerge.
  *
    ...
    private def pushHashWork(t: Tasty.Type, work: mutable.ArrayDeque[AnyRef]): Unit =
        t match
            ...
            case Tasty.Type.Annotated(u, annotation) =>
                // 31 * hash(u) + annotation.hashCode
                work.append(new Combine1(annotation.hashCode))
                work.append(u)
        ...
        while result && work.nonEmpty do
            ...
            if !(x eq y) then
                (x, y) match
                    ...
                    // Carve-out: tuple cross-product over (Tasty.Type, Tasty.Type); cross-constructor pairs are structurally not equal
                    case _ =>
                        result = false
                end match
            end if
