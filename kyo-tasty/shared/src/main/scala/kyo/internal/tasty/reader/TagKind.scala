...
/** Exhaustive per-position tag enumerations for TASTy decode positions.
  ...
  *   - `TypePositionTag`: type nodes
  *   - `TreePositionTag`: statement-position tree nodes
  ...
  *
  * The enums serve as exhaustive documentation and as `from()` factories that throw an explicit
  * `UnknownTagInPosition` on an unrecognised byte rather than producing a sentinel symbol.
  *
