...
/** ADT for symbols in the loading phase (Pass A and Pass B of the classpath decoder).
  ...
  * fully-qualified name stored in `Classpath.Indices.unresolvedFullNameByNegId`. For fully-qualified name index entries that cannot be resolved at finalizeMerge time,
  * `TastyError.UnresolvedReference` is accumulated in `classpath.errors` (soft-fail mode) or raises `TastyError.ClasspathBuilding`
  * (fail-fast mode). Unresolvable parent-type references (where the parent fully-qualified name is absent from the loaded classpath) are filtered out
