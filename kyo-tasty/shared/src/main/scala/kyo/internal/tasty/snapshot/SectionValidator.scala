...
private[snapshot] object SectionValidator:
    ...
    /** Expected number of TastyError ADT variants in the closed enum (documentation invariant).
      *
      * Any variant addition must bump the snapshot minor version AND update this constant.
      * `TastyError` has non-singleton enum cases so `TastyError.values` is not available; this
      * constant serves as a documentation anchor (count the cases in TastyError.scala to verify).
      */
