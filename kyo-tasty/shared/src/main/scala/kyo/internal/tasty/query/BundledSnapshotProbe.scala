...
private[kyo] object BundledSnapshotProbe:
    ...
    /** Probe `root` for an embedded KRFL snapshot.
      ...
      *
      * The probe is a no-op for non-jar roots (directories, jrt:/) because `ZipHandle.open` returns `Maybe.Absent` for those paths.
      */
    def probe(
        root: String
    ...
    def mergePartialInto(
        ...
        // Merge byFullName: partial SymbolIds must be shifted.
        val mergedByFullName: Dict[String, SymbolId] =
            existing.indices.byFullName ++
                partial.indices.byFullName.map((fullName, id) => fullName -> SymbolId(id.value + offset))
        // Merge packageIndex.
        val mergedPkgIdx: Dict[String, SymbolId] =
            existing.indices.packageIndex ++
                partial.indices.packageIndex.map((fullName, id) => fullName -> SymbolId(id.value + offset))
    ...
    /** Remap all SymbolId fields and all Type-bearing fields in `symbol` by adding `offset`.
      *
      ...
      */
    private def remapSymbol(symbol: Tasty.Symbol, offset: Int): Tasty.Symbol =
        val newId      = SymbolId(symbol.id.value + offset)
        val newOwnerId = SymbolId(symbol.ownerId.value + offset)
        symbol match
    ...
    private def remapType(t: Tasty.Type, offset: Int): Tasty.Type =
        t match
            ...
            case Tasty.Type.Annotated(underlying, annotation) =>
                Tasty.Type.Annotated(remapType(underlying, offset), remapAnnotation(annotation, offset))
            ...
            case _: Tasty.Type.ConstantType | Tasty.Type.Nothing | Tasty.Type.Any => t
    end remapType
    ...
    private def remapAnnotation(annotation: Tasty.Annotation, offset: Int): Tasty.Annotation =
        annotation.copy(annotationType = remapType(annotation.annotationType, offset))
