...
/** Maps compiler-mangled full names to source-form full names, and identifies synthetic compiler-generated names.
  *
  ...
  *
  * The canonical source full name is used as the primary key in user-facing indexes (findClass, findTrait,
  * findObject). The binary full name is preserved as a secondary key for backward compatibility.
  *
  * Synthetic names (anon functions, proxies, trait forwarders, etc.) have no source representation
  * and are filtered out of user-facing indexes but kept in classpath.symbols for introspection.
  *
  ...
  */
private[kyo] object FullNameNormalizer:

    private val ValueClassFullNames: Set[String] = Set(
        "scala.Int",
        "scala.Long",
        "scala.Double",
        "scala.Float",
    ...
    /** Map a compiler-mangled binary name back to its source-form full name.
      *
      ...
      */
    def canonicalSourceFullName(binaryName: String): String =
        var s    = binaryName
        var i    = 0
        var prev = ""
        while i < 8 && s != prev do
    ...
    /** Return true if this full name is a compiler-synthetic name with no source representation.
      *
      * Synthetic names are kept in classpath.symbols (for introspection) but excluded from user-facing
      ...
      */
    def isSyntheticName(fullName: String): Boolean =
        fullName.contains("$anonfun$") ||
            fullName.contains("$proxy$") ||
            fullName.contains("$_trait_") ||
            fullName.contains("$$Lambda$") ||
            fullName.contains("$$anon$") ||
            fullName.contains("$$anonfun$") ||
            fullName.contains("$adapted$") ||
            fullName.contains("$default$")
    end isSyntheticName

    /** Return true if the given source full name corresponds to a known Scala value class.
      *
      ...
      */
    def isValueClass(sourceFullName: String): Boolean = ValueClassFullNames.contains(sourceFullName)

end FullNameNormalizer
