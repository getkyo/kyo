...
/** xxh64-based custom 64-bit content hash for classpath cache invalidation.
  *
  * Hashes the JAR central directory (CEN) CRC32 values with an xxh64-custom walk so the digest is stable
  * across machine boundaries and mtime-only copies.
  *
  * Algorithm note: the implementation uses xxh64 prime constants (0x9E3779B185EBCA87, etc.) with a bespoke
  * per-entry mix loop. This is NOT the full xxh3 streaming algorithm; it is a custom 64-bit hash built from
  * xxh64 primes and is not interoperable with conforming xxh3-64 implementations.
  *
  * `compute`: hashes jar roots via CEN-CRC walk (JVM) or path-hash fallback (JS/Native); directory roots use per-file
  * mtime+size via kyo.Path.stat; jrt:/ roots delegate to PlatformDigest.collectJrtStats. `computeParanoid`: hashes sorted
  * (path, content) tuples via kyo.Path.readBytes.
  *
...
object DigestComputer:
    ...
    /** Compute the content-addressed digest for a single root.
      ...
      * For non-jar roots this method returns 0L; callers that need non-jar digests should use compute(roots) instead.
      *
    ...
    def digestForRoot(root: String): Long =
        if root.toLowerCase.endsWith(".jar") then
            ...
            // Non-jar roots: not content-addressed at this call site; use compute(roots) for those.
            0L
    ...
    /** Compute a 64-bit digest of sorted (path, digestValue, 0L) tuples from the given roots.
      ...
      * jar roots on JS/Native, falls back to a path-only hash (not content-addressed for in-place jar mutation). For `jrt:/` roots,
      * delegates to PlatformDigest.collectJrtStats (JVM: walks the JRT filesystem; JS/Native: returns empty). For directory roots,
      * uses kyo.Path.list and kyo.Path.stat (mtime + size), which work cross-platform.
      *
      ...
      * array.
      */
    def compute(roots: Seq[String])(using Frame): Array[Byte] < (Sync & Abort[TastyError]) =
        collectAllStats(roots).map { stats =>
    ...
    /** Compute a 64-bit digest of sorted (path, content) tuples from the given roots.
      *
      * For jar roots, hashes `(jarPath, jarBytes)` by reading the entire jar file's raw bytes via kyo.Path.readBytes. For `jrt:/` roots,
      * delegates to PlatformDigest.collectJrtStats for path enumeration (JVM: JRT filesystem; JS/Native: empty). For directory roots,
      * reads each `.tasty` file via kyo.Path.readBytes.
      *
      * Slower than `compute` but detects content changes that leave mtime+size unchanged (e.g., in-place content edit with `touch -t`).
      */
    def computeParanoid(roots: Seq[String])(using Frame): Array[Byte] < (Sync & Abort[TastyError]) =
        collectAllFiles(roots).map { files =>
            ...
            Kyo.foreach(sorted) { path =>
                ...
                for (path, bytes) <- pairs do
                    ...
                    acc = xxh3Mix(acc, bytes.size.toLong)
                    var j = 0
    ...
    /** Collect (path, digestLong, 0L) for all roots, branching by root type.
      ...
      * jrt:/ roots delegate to PlatformDigest.collectJrtStats (JVM: walks the JRT filesystem; JS/Native: returns empty).
      * Directory roots contribute one triple per .tasty file via kyo.Path.stat (mtime + size).
      * All triples are collected into one flat sequence; the caller sorts globally before hashing.
      */
    private def collectAllStats(
        ...
        Kyo.foreach(roots) { root =>
            if root.startsWith("jrt:/") then
                PlatformDigest.collectJrtStats(root)
                ...
                collectStats(Seq(root))
        }.map(_.flatten.toSeq)

    /** Collect all file paths for `computeParanoid`, branching by root type.
      *
      * Jar roots contribute the jar path itself (to be read as raw bytes). `jrt:/` roots delegate to PlatformDigest.collectJrtStats
      * for path enumeration; the paths returned are then read via kyo.Path.readBytes. Directory roots contribute per-file `.tasty`
      * paths via kyo.Path.list.
      */
    private def collectAllFiles(
        ...
        Kyo.foreach(roots) { root =>
            if root.startsWith("jrt:/") then
                PlatformDigest.collectJrtStats(root).map(_.map(_._1))
            else if root.toLowerCase.endsWith(".jar") then
                Seq(root)
            else
                collectFiles(Seq(root))
        }.map(_.flatten.toSeq)

    /** Collect (path, mtime, size) for all .tasty files reachable from the given directory roots.
      *
      * Uses kyo.Path.stat to obtain lastModifiedMs and sizeBytes. FileReadException from stat is converted to TastyError.SnapshotIoError.
      */
    ...
    def longToBytes(v: Long): Array[Byte] =
        ...
        SnapshotFormat.writeInt64LE(bytes, 0, v)
        bytes
    end longToBytes
