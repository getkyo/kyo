...
/** Performance counters for cold-load timing instrumentation, backed by kyo.Stat (LongAdder-based).
  *
  * Counters are exposed as raw UnsafeCounter values so hot-path callers (JvmFileSource, ClasspathOrchestrator) can call .inc()/.add() with
  * zero allocation. All callsites must hold (using AllowUnsafe) per CONTRIBUTING.md SS870-SS897.
  *
