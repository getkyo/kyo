...
/** JS implementation: serves known fixture bytes from `Embedded` (classfile + .tasty fixtures) and known
  * text bytes from `EmbeddedText` (README.md, CONTRIBUTING.md, source files). Both are sbt-generated for
  * the JS target so tests do not need a classloader resource path.
  */
    ...
    def loadBytes(resourcePath: String): Array[Byte] =
        if embeddedTextKeys.contains(resourcePath) then
            ...
            resourcePath match
                ...
                case other =>
                    throw new RuntimeException(s"JS: resource not embedded: $other; add to Embedded.scala or EmbeddedText (build.sbt)")
