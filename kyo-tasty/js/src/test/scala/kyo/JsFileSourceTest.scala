...
/** T5 parity test for JsFileSource on JS.
  ...
  */
class JsFileSourceTest extends kyo.test.Test[Any]:

    private val sourceBytes: Array[Byte] = Array.tabulate(100)(i => (i & 0xff).toByte)

    "JsFileSource.read returns all bytes of a 100-byte file written by Node.js fs" in {
        Sync.defer:
            val os      = jsGlobal.require("os").asInstanceOf[sjs.Dynamic]
            val path0   = jsGlobal.require("path").asInstanceOf[sjs.Dynamic]
            val fs      = jsGlobal.require("fs").asInstanceOf[sjs.Dynamic]
            val tmpDir  = os.tmpdir().asInstanceOf[String]
            val tmpPath = path0.join(tmpDir, "kyo-jstasty-test-read-100.bin").asInstanceOf[String]
            val int8Arr = new sjs.typedarray.Int8Array(sourceBytes.length)
            var w       = 0
    ...
    "JsFileSource.read on missing path returns Abort[TastyError.FileNotFound]" in {
