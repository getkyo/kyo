...
/** JS-side cross-platform fixture helper.
  *
  * Provides the same `TestClasspaths.withClasspath` surface as the JVM version so shared test code can call it on all three platforms. On JS
  * there is no filesystem-based classpath discovery; the helper constructs a `MemoryFileSource` pre-loaded from `kyo.fixtures.Embedded` and
  * calls `ClasspathOrchestrator.init` directly, bypassing `JsFileSource` entirely.
  *
  * Design notes:
