...
class TestClasspathsJsTest extends kyo.test.Test[Any]:
    ...
    "js-symbols-non-empty: classpath.symbols non-empty from embedded fixtures" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
            assert(
    ...
    "js-cross-file-resolution: BaseClass and ChildClass both resolve by fully-qualified name" in {
        TestClasspaths.withClasspath()(Tasty.classpath).map { classpath =>
