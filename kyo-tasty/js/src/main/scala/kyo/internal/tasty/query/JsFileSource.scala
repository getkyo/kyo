...
/** JS implementation of `FileSource`.
  ...
  *
  * Browser path (no process object): all `read` and `list` calls return `Abort.fail(TastyError.FileNotFound("browser: use fromPickles"))`
  ...
  * the browser platform should use `Tasty.Classpath.fromPickles` directly.
  *
...
object JsFileSource extends FileSource:
    ...
    private val browserError: TastyError.FileNotFound = TastyError.FileNotFound("browser: use fromPickles")
    ...
    def write(path: String, bytes: Array[Byte])(using Frame): Unit < (Sync & Abort[TastyError]) =
        if !isNode then Abort.fail(TastyError.SnapshotIoError("browser: no filesystem"))
        else
            Sync.defer:
                try
                    val fs      = jsGlobal.require("fs").asInstanceOf[js.Dynamic]
