...
/** Integration tests for MySQL LOAD DATA LOCAL INFILE via stream upload.
  ...
  */
class LocalInfileIntegrationTest extends SqlContainerTest:
    ...
    // The leaves share one container, and several stream multi-megabyte LOCAL INFILE uploads (10 MB, 50 MB, an
    // unbounded one). Run concurrently, those uploads saturate that single container and the heaviest ones lose
    // their connection mid-transfer; serialising the suite keeps at most one upload on the wire at a time.
        ...
        Scope.run {
            withMyClient { client =>
                withLoadTable(client, "id INT, name VARCHAR(255)") { tbl =>
                    val sql =
                        s"LOAD DATA LOCAL INFILE 'tx.csv' INTO TABLE $tbl FIELDS TERMINATED BY ',' LINES TERMINATED BY '\\n' (id, name)"
                    // The rollback only reaches the load if the load ran on the transaction's own
                    // connection. A load that leased its own connection instead commits independently, and
                    // the two rows survive the rollback.
                    Abort.run[SqlException | String] {
                        client.transaction {
                            client.loadLocalInfile(sql, csvStream(Seq("1,alice\n", "2,bob\n"))).flatMap { affected =>
                                assert(affected == 2L, s"Expected 2 affected rows inside the transaction, got $affected")
                        ...
                        case Result.Failure(_: String) =>
                            client.query(s"SELECT COUNT(*) FROM $tbl").flatMap { result =>
                                result.head.decode[Long](0).map { count =>
                                    assert(count == 0L, s"Expected 0 rows after the rollback, got $count")
                                }
                            }
                        case other =>
                            fail(s"Expected the deliberate rollback to surface, got: $other")
                    }
                }
            }
        }
