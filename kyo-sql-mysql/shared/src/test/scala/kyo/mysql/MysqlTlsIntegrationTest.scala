                    ...
                    Scope.run {
                        openTlsClient(details).flatMap { client =>
                            client.query("SELECT VERSION()").map { rows =>
                                ...
                                // The patch level is genuinely unknown, the major.minor is not: the fixture takes
                                // ContainerPredef.MySQL.Config.default, whose image is pinned to mysql:8.0. Pinning
                                // the prefix also catches the fixture silently drifting to another image, which
                                // non-emptiness cannot.
                                assert(v.startsWith("8.0"), s"expected a mysql:8.0 server version, got '$v'")
                            }
                        }
                    }.flatMap { _ =>
