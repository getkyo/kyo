...
class RoundTripTest extends Test:
    ...
    // The MySQL zero date has no java.time representation, so it is refused rather than substituted.
    "binary DATETIME zero-length struct is refused (MySQL zero date)" in {
        ...
        result match
            ...
            case Result.Failure(e) => fail(s"Expected SqlDecodeTemporalException, got: $e")
            case Result.Panic(t)   => throw t
        end match
    }
    ...
    "binary DATETIME 4-byte struct (date only) decodes to midnight" in {
        ...
        val result = Abort.run[SqlDecodeException](
            MysqlTemporalDecoder.decodeDatetimeBytes(buf.toSpan)
        ).eval
        result match
    ...
    "binary TIME 12-byte struct decodes hours, minutes, seconds, microseconds" in {
        ...
        val result = Abort.run[SqlDecodeException](
            MysqlTemporalDecoder.decodeTimeBytes(buf.toSpan)
        ).eval
        result match
    ...
    "MysqlErrors.mkServerError produces same shape as former per-exchange copies" in {
        ...
        // Every field matches the shape every exchange shares through this one builder
