...
class InitTlsExchangeTest extends Test:
    ...
    "Connect failed built from Closed carries closed message without RuntimeException wrapping" in {
        ...
        // Call the factory InitTlsExchange.run calls, handing it the Closed instance directly.
        ...
        assert(
            Maybe(ex.getCause).map(_.getMessage).exists(_.contains(closed.getMessage)),
            s"cause message must contain closed.getMessage directly: ${Maybe(ex.getCause).map(_.getMessage)}"
        )
