...
/** Verifies that every name-resolution path reaches the SQL [[MysqlDialect]] emits: a query-scoped [[kyo.SqlNaming]] casing, the table-name
  * parameter, a per-field `@column` rename, and the combinations, each landing in the identifiers of the rendered statement.
  *
  * The casing governs columns only. A table with no name parameter is the lowercased type name whatever casing is in scope, which is why
  * under `SnakeCase`, `UserProfile` reads `user_profile` below; with no casing it is the lowercased type name, and a passed name wins.
  */
class MysqlDialectNamingRenderTest extends Test:
    ...
    "SnakeCase casing: all columns rendered as snake_case in SELECT" in {
        ...
        assert(rm.onlySql.get == "SELECT `u`.`id`, `u`.`first_name`, `u`.`created_at` FROM `user_profile` `u`")
    }
    ...
    "SnakeCase casing: column in WHERE renders as snake_case" in {
        ...
        val rm          = q.render(MysqlDialect)
        assert(rm.onlySql.get == "SELECT `u`.`first_name` FROM `user_profile` `u` WHERE (`u`.`created_at` > ?)")
    }
    ...
    "no casing in scope: the same columns render verbatim" in {
        val q  = Sql.from[UserProfile]("u")
        val rm = q.render(MysqlDialect)
        assert(rm.onlySql.get == "SELECT `u`.`id`, `u`.`firstName`, `u`.`createdAt` FROM `userprofile` `u`")
    }
    ...
    "table-name parameter: FROM uses the given table name, columns unchanged" in {
        ...
        val rm = q.render(MysqlDialect)
        assert(rm.onlySql.get == "SELECT `i`.`id`, `i`.`sku`, `i`.`qty` FROM `line_items` `i`")
    }
    ...
    "table-name parameter: table name survives WHERE + select chain" in {
        ...
        val rm = q.render(MysqlDialect)
        assert(rm.onlySql.get == "SELECT `i`.`sku` FROM `line_items` `i` WHERE (`i`.`qty` > ?)")
    }
    ...
    "rename beats casing: the annotated field uses its explicit name, others snake_case" in {
        ...
        val rm          = q.render(MysqlDialect)
        assert(rm.onlySql.get == "SELECT `e`.`id`, `e`.`dept_code`, `e`.`salary` FROM `employee` `e`")
    }
    ...
    "rename beats casing: the annotated field name is used in a WHERE predicate" in {
        ...
        val rm          = q.render(MysqlDialect)
        assert(rm.onlySql.get == "SELECT `e`.`salary` FROM `employee` `e` WHERE (`e`.`dept_code` = ?)")
    }
    ...
    "combined: table-name parameter + per-field rename + casing for the remaining fields" in {
        ...
        val rm          = q.render(MysqlDialect)
        assert(rm.onlySql.get == "SELECT `p`.`id`, `p`.`name`, `p`.`list_price` FROM `catalog` `p`")
    }
    ...
    "combined: WHERE + select use the given table name, the renamed and the snake_cased columns" in {
        ...
        val rm          = q.render(MysqlDialect)
        assert(rm.onlySql.get == "SELECT `p`.`name` FROM `catalog` `p` WHERE (`p`.`list_price` > ?)")
    }

end MysqlDialectNamingRenderTest
