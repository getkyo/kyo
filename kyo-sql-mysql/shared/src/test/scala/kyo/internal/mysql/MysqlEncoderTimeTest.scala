...
/** Tests for the MySQL TIME wire codec for [[java.time.Duration]].
  *
  * Verifies that [[MysqlEncoder.durationEncoder]] and [[MysqlTemporalDecoder.decodeDurationBytes]] produce and consume byte-exact wire
  * representations of the MySQL TIME binary protocol format (§14.7.4). The read leg calls the same function `MysqlRowReader` calls, since
  * this backend carries no per-type decoder table for a query path to route around it.
  *
...
class MysqlEncoderTimeTest extends Test:
    ...
    /** Decodes raw TIME struct body bytes (no Abort context, throws on failure).
      *
    ...
    private def decode(body: Array[Byte]): java.time.Duration =
        kyo.Abort.run(MysqlTemporalDecoder.decodeDurationBytes(Span.from(body))).eval match
