...
class MysqlBufferWriterTest extends Test:
    ...
    "writeLenencInt / readLenencInt round-trip across 0xFC boundary (value=251)" in {
        ...
        Abort.run[SqlDecodeException](reader.readLenencInt()).map {
            case Result.Success(v) => assert(v == 251L)
            case other             => fail(s"Expected Success(251), got: $other")
        }
    }
    ...
    "writeLenencInt / readLenencInt round-trip across 0xFD boundary (value=65536)" in {
        ...
        val reader = MysqlBufferReader(buf.toSpan)
        Abort.run[SqlDecodeException](reader.readLenencInt()).map {
            case Result.Success(v) => assert(v == 65536L)
            case other             => fail(s"Expected Success(65536), got: $other")
        }
    }
    ...
    "writeLenencInt / readLenencInt round-trip for large value (16777216)" in {
        ...
        val reader = MysqlBufferReader(buf.toSpan)
        Abort.run[SqlDecodeException](reader.readLenencInt()).map {
            case Result.Success(v) => assert(v == 16777216L)
            case other             => fail(s"Expected Success(16777216), got: $other")
        }
    }
