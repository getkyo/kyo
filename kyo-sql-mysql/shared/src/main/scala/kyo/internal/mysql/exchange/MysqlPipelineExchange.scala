...
/** Implements MySQL sequential pipeline execution.
  ...
  * ==Transaction context==
  *
  * [[runOnConnection]] takes whichever connection the caller already holds, whether that came from the pool for one batch or from a
  * transaction that pins it for longer. The distinction does not reach this exchange: the statements run the same way either way.
  */
