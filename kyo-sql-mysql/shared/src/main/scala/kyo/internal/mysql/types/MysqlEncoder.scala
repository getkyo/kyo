...
object MysqlEncoder:
    ...
    /** The dialect this wire layer encodes for. Every rejection of a value another dialect owns names it. */
    val dialectId: Idiom.Id = Idiom.Id("mysql")
    ...
    // --- Shared temporal struct writers ---
    //
    // MySQL's DATETIME/TIMESTAMP and TIME binary values are length-prefixed structs whose body is the same for every
    // encoder that writes one, so each struct's layout lives here once and the encoders below supply the fields.

    /** Writes the length-prefixed DATETIME/TIMESTAMP struct: 7 bytes without a microsecond part, 11 bytes with one. */
    private def writeDatetimeStruct(buf: MysqlBufferWriter, ldt: java.time.LocalDateTime, micros: Int): Unit =
        if micros != 0 then
    ...
    end writeDatetimeStruct

    /** Writes the length-prefixed TIME struct body: 8 bytes without a microsecond part, 12 bytes with one. The zero-length form a
      * `Duration.ZERO` writes stays with its caller, since only a duration reaches it.
      */
    private def writeTimeStruct(
        buf: MysqlBufferWriter,
        isNegative: Boolean,
        days: Int,
        hours: Int,
        minutes: Int,
    ...
    val instantEncoder: MysqlEncoder[kyo.Instant] = new MysqlEncoder[kyo.Instant]:
        ...
        def write(value: kyo.Instant, buf: MysqlBufferWriter): Unit =
            ...
            writeDatetimeStruct(buf, ldt, micros)
        end write
    ...
    val durationEncoder: MysqlEncoder[java.time.Duration] = new MysqlEncoder[java.time.Duration]:
        ...
        def write(value: java.time.Duration, buf: MysqlBufferWriter): Unit =
            ...
            if days == 0 && hours == 0 && minutes == 0 && seconds == 0 && micros == 0 then
                ...
                writeTimeStruct(buf, isNegative, days, hours, minutes, seconds, micros)
            end if
        end write
