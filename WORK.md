...
| 22 | item 5 (`nullBlockers` empty below 3 controls -> blocker -> breaks single-pair compare and default 2-leg chain). **Done item 6** (`9d10e9e2c8`): `Resolution.floorBound`/`Forecast.floorBound` carry which term bound the threshold (t*se vs ownError*mean); `.lever` names the right fix (more legs vs more iterations per fork); the report's resolution note names the widest row's lever and `bench plan`'s blind-row advice is per row, replacing the blanket 'more forks will not help, more legs' that was backwards for floor-bound rows. Tests in StatsTest and PlanTest | item 5: model/default changes |
