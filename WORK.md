...
harness as its oracle); recorded in INTERACTIONS.md. Both bench deliverables stay gated; note the
bench harness now has a second life staged: oracle of the first evolved skill. Re-armed 1800s.
...
| 27 | **core DONE** (`277f385d88`): the report enumerates the checks it could not run and the command for each (absence-names-its-remedy): `--evidence timing` -> no mechanism (`--evidence full`), subset run -> no whole-class statement (drop `--row`), <3 controls -> no A/A null (`--legs 5`); computable from the two runs and the leg count. The selection table's positive rows were already landed across items 4/6/3/1/5. Remaining (owner/design): item 4's "name the C3 mechanism" wording, the whole-class jit-provenance row (defect 30) | done (core) |
