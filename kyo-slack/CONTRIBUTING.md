...
(`Slack.run`, `Slack.receive`). The handler returns one `SlackAck`, and
...
anywhere on the `Slack` object or a `Slack` connection. Two failure modes are therefore
unrepresentable:
...
- **`ackDeadline` enforcement.** The handler is raced against `config.ackDeadline`
  ...
  `run[S]` / `receive[S]` carry the `Isolate[S, Abort[SlackException] & Async, S]`
...
layered so each layer depends only on the ones below it. `Slack` is both the opaque
connection type and its companion object (the entry methods and Web API hang on the
companion); `SlackTs` is a top-level opaque message-timestamp id.

| Layer | Type | Role |
|-------|------|------|
| Public surface | `Slack`, `SlackConfig`, `SlackTs`, `SlackAck`, `SlackEnvelope`, `SlackEvent`, `SlackInteraction`, `SlackCommand`, `SlackMessage`, `SlackView`, `SlackId`, `SlackToken`, `SlackException` | The bot author's whole vocabulary. |
...
| Handle carrier | `internal.SlackSocketHandle` | The struct behind the opaque `Slack` connection type. |

Entry points:

- `Slack.run[S](config)(handler)` is the default: `Scope`-managed, runs the loop
...
- `Slack.init(config)` returns the opaque `Slack` connection handle for the
...
`SlackId` holds ten opaque `String` types (`ChannelId`, `UserId`, `TeamId`, `AppId`,
`TriggerId`, `EnvelopeId`, `ViewId`, `BotId`, `ActionId`, `BlockId`), and the message
...
`ChannelId` passed where a `TriggerId` is required is a compile error. `SlackToken` holds
two distinct opaque types:
...
`SlackException` is a sealed base over `KyoException` with six final top-level leaves,
modeled on `HttpException` (a flat base plus top-level leaves like `HttpConnectException`;
six leaves do not earn an intermediate subcategory layer). The leaves: `SlackHandshakeException`, `SlackTransportException`,
