...
object SlackId:
    ...
    object Ts:
        def apply(s: String): Ts             = s
        extension (id: Ts) def value: String = id
        given Schema[Ts]                     = Schema.stringSchema.transform[Ts](apply)(_.value)
        given CanEqual[Ts, Ts]               = CanEqual.derived
    end Ts

end SlackId

/** Short top-level name for the opaque message-timestamp id, used pervasively in
  * event and message signatures.
  */
type SlackTs = SlackId.Ts
