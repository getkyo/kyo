...
/** Typed error hierarchy for kyo-slack: a sealed base extending `KyoException`
  ...
  * base plus top-level leaves (six leaves do not earn the intermediate subcategory
  * layer `HttpException`'s ~17 leaves do). The web-api and rate-limit leaves carry
  * typed fields (the Slack error code, the backoff), not a discriminator string.
  *
