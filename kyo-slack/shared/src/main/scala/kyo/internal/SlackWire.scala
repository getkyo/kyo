...
private[kyo] object SlackWire:
    ...
    private def decodeEvent(frame: String)(using Frame): SlackEvent < Sync =
        // TODO: free-form inbound JSON is recovered by hand-navigating the frame AST via
        // SlackRawJson.nestedJson (here and for payloadJson / view state below) because kyo-schema
        // cannot decode arbitrary JSON into a dynamic value: Json.decode needs a typed target, and
        // there is no JSON-string -> Structure.Value path. Remove once kyo-schema supports decoding
        // untagged dynamic JSON.
            ...
            case Result.Success(env) =>
                ...
                (w.channel, w.user, w.text, w.ts) match
                    case (Present(ch), Present(u), Present(t), Present(ts)) =>
                        SlackEvent.Message(SlackId.ChannelId(ch), SlackId.UserId(u), t, SlackTs(ts), w.thread_ts.map(SlackTs(_)))
                    case _ =>
    ...
    private def decodeAppMentionEvent(frame: String, eventJson: String)(using Frame): SlackEvent < Sync =
        Json.decode[SlackPayloadEnvelope[EventLeafEnvelope[WireAppMention]]](frame) match
            case Result.Success(env) =>
                ...
                (w.channel, w.user, w.text, w.ts) match
                    case (Present(ch), Present(u), Present(t), Present(ts)) =>
                        SlackEvent.AppMention(SlackId.ChannelId(ch), SlackId.UserId(u), t, SlackTs(ts))
                    case _ =>
    ...
    private def decodeReactionAddedEvent(frame: String, eventJson: String)(using Frame): SlackEvent < Sync =
        Json.decode[SlackPayloadEnvelope[EventLeafEnvelope[WireReactionAdded]]](frame) match
            case Result.Success(env) =>
                val w = env.payload.event
                (w.user, w.reaction, w.item.flatMap(_.channel), w.item.flatMap(_.ts)) match
                    case (Present(u), Present(r), Present(ch), Present(ts)) =>
                        SlackEvent.ReactionAdded(SlackId.UserId(u), r, SlackId.ChannelId(ch), SlackTs(ts))
                    case _ =>
                ...
                w.user.id match
                    case Present(u) =>
                        val actions = w.actions.map(a =>
                            SlackInteraction.Action(SlackId.ActionId(a.action_id), SlackId.BlockId(a.block_id), a.value)
                        )
                        SlackInteraction.BlockActions(
                            SlackId.UserId(u),
                            SlackId.TriggerId(w.trigger_id.getOrElse("")),
                            w.channel.id.map(SlackId.ChannelId(_)),
                            w.view.id.map(SlackId.ViewId(_)),
                            actions
                        )
                    case Absent =>
                ...
                w.user.id match
                    case Present(u) =>
                        SlackInteraction.MessageAction(
                            ...
                            SlackTs(w.message.ts.getOrElse(""))
                        )
                    case Absent =>
    ...
    /** Encode exactly one outbound ack frame from the handler's returned `SlackAck`.
      ...
      *
      * Building the native payload renders the message/view typed `blocks`, which can fail
      * with a typed `SlackDecodeException` on a malformed `SlackBlock.Raw`, so this is
      * effectful in `Abort`.
      */
    ...
    def decodeConnectionsOpen(frame: String)(using Frame): Result[SlackHandshakeException, String] =
        Json.decode[ConnectionsOpenResp](frame) match
            case Result.Success(r) if r.ok =>
                r.url match
                    case Present(u) => Result.succeed(u)
                    case Absent     => Result.fail(new SlackHandshakeException("apps.connections.open ok but no url"))
            case Result.Success(r) =>
                Result.fail(new SlackHandshakeException(s"apps.connections.open failed: ${r.error.getOrElse("unknown")}"))
            case other =>
                Result.fail(new SlackHandshakeException(s"apps.connections.open response decode failed: $other"))
