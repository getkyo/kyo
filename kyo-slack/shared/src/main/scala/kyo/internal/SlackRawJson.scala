...
/** Splices a raw Block Kit JSON string onto the Web API wire as a NATIVE JSON value
  ...
  * string surfaces as a typed `SlackDecodeException` at parse time, never
...
private[kyo] object SlackRawJson:
    ...
    def parse(raw: String, label: String)(using Frame): SlackRawJson < Abort[SlackException] =
        Parser(raw).parseValue match
            ...
            case Result.Failure(msg)   => Abort.fail(new SlackDecodeException(s"$label JSON did not parse: $msg"))
    ...
    /** A minimal recursive-descent JSON parser producing a native `Structure.Value`
      ...
      */
    // TODO: hand-rolled JSON parser. kyo-schema cannot parse free-form / untagged JSON into a
    // dynamic value: Json.decode requires a typed target Schema, and the derived Structure.Value
    // enum Schema reads and writes a tagged wrapper shape (e.g. {"Record":{...}}), not raw JSON.
    // This parser (with the native-write Schema above) bridges that gap so Block Kit can splice as
    // a real array and free-form inbound sub-objects can be recovered. Remove once kyo-schema can
    // decode/encode untagged dynamic JSON (Structure.Value <-> JSON string).
