...
final private[kyo] class SlackSocketEngine private[kyo] (
        ...
        def step: SlackReconnect.Reaction < (S & Async & Abort[SlackException]) =
            Abort.run[Closed](inbound.take).map {
                        ...
                        case SlackWire.Decoded.Envelope(env, ackable, responseUrl) =>
                            env match
                                case SlackEnvelope.Disconnect(SlackEnvelope.DisconnectReason.LinkDisabled) =>
                                    handler(
                                        env
                                    ).andThen(Abort.fail(new SlackException.SlackTerminalException("socket link_disabled (terminal)")))
                ...
                case Result.Panic(ex) =>
                    Abort.fail(new SlackException.SlackTransportException(s"receive loop panicked: ${ex.getMessage}", ex))
            }
        step
    ...
    private def deliverAndAck[S](
        ...
        env match
            case SlackEnvelope.Disconnect(SlackEnvelope.DisconnectReason.LinkDisabled) =>
                handler(env).andThen(Abort.fail(new SlackException.SlackTerminalException("socket link_disabled (terminal)")))
            case _ if !ackable =>
            ...
            case Present(id) =>
                val sendAck = SlackWire.encodeAck(id, ack).map { frame =>
                    Abort.recover[Closed] { (c: Closed) =>
                        Abort.fail(new SlackException.SlackTransportException(s"ack send failed: ${c.getMessage}", c))
                    } {
                        outbound.put(frame)
                    }
                }
    ...
    /** Final teardown (scope finalizer / `SlackConnection.close`): close the socket and
    ...
    private[kyo] def initUnscoped(
        ...
        for
            ...
            relay <- Fiber.initUnscoped {
                Abort.run[Throwable] {
                    ...
                    case Result.Failure(ex) =>
                        connectReady.complete(Result.fail(new SlackException.SlackTransportException(
            ...
            conn <- connRef.get.map {
                ...
                case Absent     => Abort.fail(new SlackException.SlackTransportException("connection not established after readiness"))
            }
            sender <- senderRef.get.map {
                case Present(s) => s: Fiber[Unit, Sync]
                case Absent     => Abort.fail(new SlackException.SlackTransportException("sender fiber not started after readiness"))
            }
