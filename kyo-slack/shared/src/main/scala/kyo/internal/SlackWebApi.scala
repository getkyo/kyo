...
/** The one canonical Web API request path plus the ambient bot-token `Local`. Every
  ...
  * `run`/`init` via `local.let` and read inside the handler's fiber
...
private[kyo] object SlackWebApi:
    ...
    private[kyo] def request[In: Schema, Out: Schema](method: String, body: In)(using Frame): Out < (Async & Abort[SlackException]) =
        local.use {
            case Absent =>
                Abort.fail(
                    new SlackHandshakeException("no Slack connection bound; call within Slack.run/init")
                )
            case Present(token) =>
                val headers = Seq(
                ...
                baseUrl.use { base =>
                    Abort.recover[HttpException] { (ex: HttpException) =>
                        Abort.fail(new SlackTransportException(s"$method transport failure: ${ex.getMessage}", ex))
                    } {
    ...
    private[kyo] def mapResponse[Out: Schema](
        ...
        if status == HttpStatus.TooManyRequests then
            ...
            Abort.fail(new SlackRateLimitException(retryAfter, s"$method rate limited"))
        else if envelope.ok then decodeOut[Out](rawBody, method)
        else
            Abort.fail(new SlackWebApiException(
    ...
    private[kyo] def postResponseUrl(url: String, message: SlackMessage)(using Frame): Unit < (Async & Abort[SlackException]) =
        Slack.messageBlocks(message).map { blocks =>
            ...
            Abort.recover[HttpException] { (ex: HttpException) =>
                Abort.fail(new SlackTransportException(s"response_url POST failed: ${ex.getMessage}", ex))
            } {
