...
private[kyo] object SlackTransport:
        ...
        new SlackTransport:
            private[kyo] def connect[A, S](url: String, config: HttpWebSocket.Config)(
                ...
                Abort.recover[HttpException] { (ex: HttpException) =>
                    Abort.fail(new SlackException.SlackTransportException(s"websocket connect/transport failure: ${ex.getMessage}", ex))
                } {
    ...
    /** The backend the connection entry points open over, defaulting to `live`. The
      * production `connect`/`connectUnscoped` path reads this default; cross-platform
      * tests bind an in-memory backend with `transport.let(...)` to drive the full
      * connect/receive path without a real socket.
      */
