...
class SlackAckCompileTest extends kyo.test.Test[Any]:
    ...
    "a handler whose body returns a non-SlackAck does not type-check" in {
        typeCheckFailure(
            "Slack.run(SlackConfig(SlackToken.AppLevel(\"x\"), SlackToken.Bot(\"y\")))((_: SlackEnvelope) => Slack.chatPostMessage(SlackMessage(SlackId.ChannelId(\"C\"), \"x\")))"
        )
    }

    "there is no public ack method on the Slack object" in {
        typeCheckFailure("""Slack.ack(SlackId.EnvelopeId("E1"))""")
    }

    "there is no public ack method on a Slack connection" in {
        typeCheckFailure("""(c: Slack) => c.ack(SlackId.EnvelopeId("E1"))""")
    }

    "there is no public sendAck method on a Slack connection" in {
        typeCheckFailure("""(c: Slack) => c.sendAck""")
    }
