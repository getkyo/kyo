        ...
        for
            ...
            neverTransport = new SlackTransport:
                private[kyo] def connect[B, S](u: String, cc: HttpWebSocket.Config)(
                    ...
                    Latch.init(1).map(_.await.andThen(Abort.fail(new SlackException.SlackTransportException("unreachable"))))
    ...
    "a handler that Abort.fails leaves the envelope unacked" in {
        for
            ...
            handler = (_: SlackEnvelope) =>
                Abort.fail(new SlackException.SlackTransportException("handler boom")): SlackAck < (Async & Abort[SlackException])
        ...
        yield
            result match
                case Result.Failure(ex: SlackException.SlackTransportException) =>
