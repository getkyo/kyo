...
class SlackWireTest extends kyo.test.Test[Any]:
    ...
    // the decoder must read item.channel/item.ts, not flat top-level fields. Regression
    // guard for the live-validation finding where it fell through to Unknown(reaction_added).
    "events_api reaction_added decodes the nested item channel/ts into a typed ReactionAdded" in {
        val frame =
            """{"type":"events_api","envelope_id":"E20","payload":{"type":"event_callback","event":{"type":"reaction_added","user":"U1","reaction":"rocket","item":{"type":"message","channel":"C1","ts":"1.2"}}}}"""
        SlackWire.decode(frame).map { d =>
            d match
                case SlackWire.Decoded.Envelope(SlackEnvelope.EventsApi(meta, event), ackable: Boolean, _) =>
                    assert(meta.envelopeId == SlackId.EnvelopeId("E20"))
                    assert(ackable)
                    event match
                        case SlackEvent.ReactionAdded(user, reaction, itemChannel, itemTs) =>
                            assert(user == SlackId.UserId("U1"))
                            assert(reaction == "rocket")
                            assert(itemChannel == SlackId.ChannelId("C1"), s"expected item.channel C1, got: $itemChannel")
                            assert(itemTs == SlackTs("1.2"), s"expected item.ts 1.2, got: $itemTs")
                        case other =>
                            assert(false, s"expected ReactionAdded event, got: $other")
                    end match
                case other =>
                    assert(false, s"expected EventsApi envelope, got: $other")
        }
        ...
        SlackWire.decode(frame).map { d =>
            d match
                case SlackWire.Decoded.Envelope(
                    ...
                    interaction match
                        case SlackInteraction.BlockActions(user, triggerId, channel, viewId, actions) =>
                            assert(user == SlackId.UserId("U1"))
                            ...
                            assert(viewId == Absent, s"a channel-message block_actions has no view id, got: $viewId")
                            assert(actions.size == 1)
                            assert(actions(0).actionId == SlackId.ActionId("a1"))
                            assert(actions(0).blockId == SlackId.BlockId("b1"))
    ...
    // A block_actions fired from inside a modal carries a nested `view` object; its id
    // must surface on BlockActions.viewId so a handler can drive Slack.viewsUpdate.
    "interactive block_actions from a modal captures the view id" in {
        ...
        SlackWire.decode(frame).map { d =>
            d match
                case SlackWire.Decoded.Envelope(SlackEnvelope.Interactive(_, interaction), _, _) =>
                    interaction match
                        case SlackInteraction.BlockActions(_, _, channel, viewId, _) =>
                            assert(viewId == Present(SlackId.ViewId("V9")), s"expected modal view id V9, got: $viewId")
                            assert(channel == Absent, s"a modal block_actions has no channel, got: $channel")
                        case other =>
                            assert(false, s"expected BlockActions, got: $other")
                    end match
                case other =>
    ...
    "encodeAck(ViewResponse(Update)) carries response_action update and view.blocks as a native array" in {
        val view = SlackView(SlackView.Type.Modal, blocks = Chunk(SlackBlock.Raw("""{"type":"section"}""")))
    ...
    "encodeAck(CommandResponse) carries the native message body with blocks as a native array" in {
        val msg = SlackMessage(SlackId.ChannelId("C1"), "hi", blocks = Chunk(SlackBlock.Raw("""{"type":"section"}""")))
    ...
    "encodeAck(CommandResponse) with malformed blocks aborts with SlackDecodeException" in {
        val msg = SlackMessage(SlackId.ChannelId("C1"), "hi", blocks = Chunk(SlackBlock.Raw("not json")))
    ...
    "decodeConnectionsOpen fails typed on ok:false" in {
        ...
        result match
            case Result.Failure(ex) =>
                assert(ex.isInstanceOf[SlackHandshakeException])
