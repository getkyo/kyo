        ...
        Abort.run[Closed](conn.inbound.safe.take).flatMap {
            ...
            case Result.Success(span) =>
                if span.isEmpty then
                    ...
                    responseByte match
                        case 'S' =>
                            // Server accepts TLS: upgrade the connection on the same socket. SNI-host injection and the
                            // transport failure mapping are shared with the MySQL path in TlsUpgrade.
                            TlsUpgrade.upgrade(conn, host, port, tls)
                        case 'N' =>
