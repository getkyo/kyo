...
//
// StartupMessage and CancelRequest are exceptions: they have no
...
final case class PasswordMessage(password: String) extends FrontendMessage:
    /** The one message whose default rendering would be a secret; a future debug log must not make it live. */
    override def toString: String = "PasswordMessage(***)"
end PasswordMessage
...
/** Sync, marks end of an extended-query message group; server responds with [[ReadyForQuery]].
  ...
  * Wire: 'S' | Int32(4)
  *
  * Named `SyncMessage` rather than `Sync` because `kyo.Sync` is the effect every file in this module works in. A `case object Sync` here
  * contributes a term that an `import kyo.internal.postgres.*` would shadow, so `Sync.defer` in such a file resolves to this message and
  * fails to compile while `Unit < Sync` still resolves to the effect. No protocol message is worth costing a reader that.
  */
case object SyncMessage extends FrontendMessage
