...
/** Big-endian byte reader for the PostgreSQL v3 wire protocol.
  ...
  */
final class PostgresBufferReader(spanBytes: Span[Byte]) extends SqlBufferReader(spanBytes):
    ...
    /** Reads a NUL-terminated UTF-8 string and advances the cursor past the NUL byte.
      *
      * The advance is guarded, as it is in `MysqlBufferReader.readNulTerminatedString`: a truncated payload whose last string runs to the
      * end of the span has no NUL to consume, and stepping past the end anyway would leave `pos` at `span.size + 1`, so `remaining` would
      * report -1 and every later bound check would read a negative count as though bytes were left.
      */
