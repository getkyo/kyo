...
class PostgresDialectDslRenderTest extends Test:
    ...
    "windowed SUM with partitionBy + orderBy" in {
        ...
        assert(
            sqlOf(q) ==
                """SELECT "p"."name", SUM("p"."age") OVER (PARTITION BY "p"."deptId" ORDER BY "p"."age" ASC NULLS LAST) FROM "person" "p""""
        )
    }
    ...
    "standalone rowNumber + rank window functions" in {
        ...
        assert(
            sqlOf(rn) ==
                """SELECT "p"."name", ROW_NUMBER() OVER (PARTITION BY "p"."deptId" ORDER BY "p"."age" DESC NULLS FIRST) FROM "person" "p""""
        )
