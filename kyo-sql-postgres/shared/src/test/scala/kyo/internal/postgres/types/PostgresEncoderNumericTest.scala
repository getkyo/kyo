...
class PostgresEncoderNumericTest extends kyo.Test:
    ...
    // ── Wire vectors, each hand-derived from the PG spec ──────────────────────
    ...
    "numeric (decoder) parses NaN as SqlDecodeException when decoded through SqlSchema" in {
        ...
        // The row's codec re-raises it through Abort.
        ...
        val dummyField = SqlRow.Column("n", 1700 /* OID_NUMERIC */ )
        val fakeRow    = new SqlRow(Chunk(Maybe.Present(nanBytes)), Chunk(dummyField), PostgresRowCodec(Format.Binary))

        // Decode through SqlSchema[BigDecimal], whose codec wraps every serializeRead throw into SqlDecodeException.
        Abort.run[SqlDecodeException](fakeRow.decode[BigDecimal]).map {
