...
/** Unit tests for Md5PasswordShared.
  *
  ...
  */
class Md5PasswordSharedTest extends kyo.Test:

    "Md5PasswordShared hash format, starts with 'md5' and is 35 characters total" in {
        val result = Md5PasswordShared.encode("anypassword", "anyuser", Span.from(Array[Byte](1, 2, 3, 4)))
    ...
    "Md5PasswordShared hash matches known vector, password=secret user=bob salt=0xABCDEF12" in {
        ...
        val password = "secret"
        val user     = "bob"
        val salt     = Span.from(Array[Byte](0xab.toByte, 0xcd.toByte, 0xef.toByte, 0x12.toByte))
        val expected = "md5a87eba369f4a6ff7fffa7aee32fa9e81"

        val actual = Md5PasswordShared.encode(password, user, salt)
        assert(actual == expected, s"Expected: $expected, got: $actual")
    }

    "Md5PasswordShared hash is lowercase hex, output contains only lowercase hex digits" in {
        val result  = Md5PasswordShared.encode("pass", "user", Span.from(Array[Byte](0, 1, 2, 3)))
    ...
    "Md5PasswordShared different salts produce different hashes" in {
        val h1 = Md5PasswordShared.encode("pass", "user", Span.from(Array[Byte](1, 2, 3, 4)))
        val h2 = Md5PasswordShared.encode("pass", "user", Span.from(Array[Byte](5, 6, 7, 8)))
        assert(h1 != h2, "Different salts must produce different hashes")
    }

    "Md5PasswordShared different passwords produce different hashes" in {
        val salt = Span.from(Array[Byte](0, 0, 0, 0))
        val h1   = Md5PasswordShared.encode("password1", "user", salt)
        val h2   = Md5PasswordShared.encode("password2", "user", salt)
