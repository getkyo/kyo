...
class MessageReaderTest extends Test:
    ...
    /** Verifies that [[MessageReader.readOne]] correctly decodes a valid [[ReadyForQuery]] frame.
      ...
      *
      * This test exercises the full [[MessageReader]] code path through [[MessageReader.readOne]] → [[decodeMessage]] → unmarshaller →
      * result.
      */
    ...
    "MessageReader readOne fails with SqlConnectionException when channel closes mid-read" in {
        ...
        Abort.run[Closed](conn.inbound.safe.put(header)).flatMap { _ =>
            ...
            // Unsafe: Connection.close is the unsafe-tier close, called here to shut the stub's channel without suspending.
    ...
    /** Verifies that a malformed BackendKeyData frame (body too short for the declared type) produces a [[SqlConnectionException]] rather
      ...
      * `SqlConnectionException`. A `readInt32()` that threw `ArrayIndexOutOfBoundsException` eagerly would do so outside
      * `Abort.run[SqlDecodeException]`'s try-block, bypass the `Result.Failure` arm entirely, and escape as an unhandled panic.
      */
    ...
    /** Verifies that [[MessageReader.readOneIfAvailable]] reports [[Absent]] over an incomplete frame and keeps the bytes it took.
      *
      * Only the 5-byte header is delivered. The poll must report [[Absent]] rather than suspend for the body, and the header must stay in
      * the pending buffer: a later [[MessageReader.readOne]] decodes the same frame once the body arrives. Dropping those bytes would
      * desynchronise the connection on the very next read, which is why the poll has to live inside the reader.
      */
    "MessageReader readOneIfAvailable reports Absent without losing the partial frame it buffered" in {
        val conn          = StubConnection()
        val reader        = new MessageReader()
        val unmarshallers = Unmarshallers.default
        val header        = Span.from(Array[Byte]('Z'.toByte, 0x00, 0x00, 0x00, 0x05))
        val body          = Span.from(Array[Byte]('I'.toByte))
        Abort.run[Closed](conn.inbound.safe.put(header)).flatMap { _ =>
            Abort.run[SqlException](reader.readOneIfAvailable(conn, unmarshallers)).flatMap {
                    ...
                    polled match
                        case Maybe.Absent =>
                            Abort.run[Closed](conn.inbound.safe.put(body)).flatMap { _ =>
                                Abort.run[SqlException](reader.readOne(conn, unmarshallers)).map {
                                    case Result.Success(ReadyForQuery('I')) => succeed
                                    case other => fail(s"the polled header must still be buffered for the next read, got: $other")
                                }
                            }
                        case Maybe.Present(msg) => fail(s"Expected Absent over an incomplete frame, got: $msg")
                case other => fail(s"Expected a successful poll, got: $other")
            }
        }
    }

    /** Verifies that [[MessageReader.readOneIfAvailable]] decodes a message already sitting in the pending buffer.
      *
      * Two frames arrive in ONE chunk, which is what a server answering inside a single TCP segment looks like. The first read takes the
      * whole chunk and leaves the second frame buffered, at which point the inbound channel is empty, so a poll of the channel would report
      * nothing over a message the reader is holding.
      */
    "MessageReader readOneIfAvailable decodes a buffered message with the channel already empty" in {
        val conn          = StubConnection()
        val reader        = new MessageReader()
        val unmarshallers = Unmarshallers.default
        // Two ReadyForQuery frames back to back: 'I' (idle) then 'T' (in transaction).
        val twoFrames = Span.from(Array[Byte](
            'Z'.toByte,
            0x00,
            0x00,
            0x00,
        ...
        Abort.run[Closed](conn.inbound.safe.put(twoFrames)).flatMap { _ =>
            Abort.run[SqlException](reader.readOne(conn, unmarshallers)).flatMap {
                case Result.Success(ReadyForQuery('I')) =>
                    Abort.run[SqlException](reader.readOneIfAvailable(conn, unmarshallers)).map {
                        case Result.Success(polled) =>
                            polled match
                                case Maybe.Present(ReadyForQuery('T')) => succeed
                                case other => fail(s"the second frame is buffered and must be polled out, got: $other")
                        case other => fail(s"Expected a successful poll, got: $other")
                    }
                case other => fail(s"Expected the first frame of the pair, got: $other")
            }
        }
    }
