...
/** Protocol-level integration tests for SCRAM-SHA-256-PLUS (channel binding).
  *
  * Driven through PostgresConnection.connectWithCertHashOverride:
  *   - SCRAM-SHA-256-PLUS over TLS succeeds and is verified as the selected mechanism.
  *   - Plaintext connect falls back to SCRAM-SHA-256 (no cert hash → no PLUS).
  *   - Wrong cert hash (MITM simulation) is rejected with SQLSTATE 28000 (invalid_authorization_specification).
  *   - Client with forced-Absent cert hash refuses PLUS even on TLS.
  *   - Client picks SCRAM-SHA-256 when no cert hash; server accepts.
  *   - Wrong password under SCRAM-PLUS produces sqlState=28P01; state does NOT leak.
  *
            ...
            Abort.run[FileStructureException](Path.tempDirUnscoped(prefix = "kyo-sql-scram-plus-")).flatMap {
                ...
                case Result.Success(tempDirPath) =>
                    ...
                    Abort.run[Throwable](Command(
                        ...
                        case Result.Success(_) =>
                            // 0700 from tempDirUnscoped is unreadable to the container that bind-mounts it, whose
                            // entrypoint then exits before the health check runs. Throwaway self-signed material.
            ...
            Abort.run[Throwable] {
                Scope.run {
                    Async.timeout(2.seconds) {
                        // Unsafe: Transport.connect is unsafe-tier and returns a bare Fiber.Unsafe, lifted here into Sync.
                        Sync.Unsafe.defer(kyo.net.NetPlatform.transport.connect(host, port).safe)
                            .flatMap(_.use(identity))
                            .flatMap { conn =>
                                // Unsafe: Connection.close is unsafe-tier, and it must run on every completion path
                                // (success, failure, abort, interrupt) or the probe FD leaks: `Transport.connect`
                                // returns a bare Fiber.Unsafe[Connection] with no automatic Scope tie-in, and the
                                // surrounding Async.timeout can interrupt this fiber mid-take, leaving the socket
                                // ESTABLISHED with a pending Read that never gets a close op submitted to the driver.
