...
class QueryMessagesTest extends Test:
    ...
    "PasswordMessage.toString never renders the secret" in {
        // The one frontend message whose default rendering would be a credential; a debug log of the outbound
        // message must print the mask, never the password.
        assert(PasswordMessage("hunter2").toString == "PasswordMessage(***)")
    }
