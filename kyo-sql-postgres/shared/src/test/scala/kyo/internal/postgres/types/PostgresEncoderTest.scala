...
class PostgresEncoderTest extends kyo.Test:
    ...
    "numeric binary encoder round-trip preserves base-10000 decomposition" in {
        // 123456789 in base 10000 is [1234, 5678, 9] (most-significant first). The binary encoder writes that
        // decomposition, so a round-trip through encode/decode is what pins `toBase10000` producing it.
    ...
    "textDecoder claims text, varchar, and bpchar" in {
        assert(PostgresDecoder.textDecoder.oids == Set(PostgresEncoder.OID_TEXT, 1043, 1042))
    }

    "scalar decoders claim exactly their own OID" in {
        assert(PostgresDecoder.bool.oids == Set(PostgresEncoder.OID_BOOL))
        assert(PostgresDecoder.int4.oids == Set(PostgresEncoder.OID_INT4))
        assert(PostgresDecoder.int8.oids == Set(PostgresEncoder.OID_INT8))
        assert(PostgresDecoder.numeric.oids == Set(PostgresEncoder.OID_NUMERIC))
        assert(PostgresDecoder.timestamptz.oids == Set(PostgresEncoder.OID_TIMESTAMPTZ))
    }
