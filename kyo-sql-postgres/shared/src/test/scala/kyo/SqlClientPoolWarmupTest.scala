...
/** Unit tests for connection-pool warm-up `minConnections`.
  ...
  */
class SqlClientPoolWarmupTest extends SqlContainerTest:
                    ...
                    Abort.run[SqlConnectionException](
                        ...
                        case Result.Success(client) =>
                            ...
                            openCount.get.flatMap { openedDuringWarmup =>
                                ...
                                Abort.run[SqlException](DB.run(client)(client.query("SELECT 1"))).flatMap { _ =>
                ...
                AtomicRef.initWith(0) { snapshotRef =>
                    Latch.initWith(minConns) { allActiveGate =>
                        kyo.internal.FakeServer.listenPort { conn =>
                            ...
                            // Snapping activeCount after the latch opens is last-write-wins and racy: on a
                            // scheduler that wakes the post-await fibers in sequence, each handler may
                            // decrement before the next one snaps, so the final snapshot reflects the LAST
                            // writer rather than the peak, and whether that shows up depends on the platform.
                            // snapshotRef = max(prev, current) at increment time is correct on every
                            // scheduler and captures the structural invariant under test.
    ...
    /** `warmUp` propagates the first connection failure and closes the connections that already succeeded, so no server-side socket is
      * leaked on a partial-failure init.
      *
      * The server accepts and completes the handshake for the first 2 connections, then immediately closes the 3rd (causing a
      * `SqlConnectionException`). The leaf asserts that:
      *   1. the error propagates (init fails with `SqlConnectionException`).
      *   2. the 2 successful connections are closed so the server-side sockets are released.
      *
      ...
      *
      * If the successful connections were not closed, the latch would never fire and the test would time out; when they are closed the
      * latch fires promptly.
      */
    "partial-failure warmUp (first 2 succeed, 3rd fails) closes the 2 successful connections" in {
        Scope.run {
