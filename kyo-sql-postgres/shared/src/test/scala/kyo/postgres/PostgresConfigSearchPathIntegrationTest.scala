...
class PostgresConfigSearchPathIntegrationTest extends SqlContainerTest:
    ...
    /** The same read on the simple protocol, for the one leaf that must not depend on the prepared-statement cache. */
            ...
            withServer { url =>
                ...
                SqlClient.init(url, config).map { client =>
                    DB.run(client) {
                        ...
                        //
                        // Read on the simple protocol, deliberately. `DISCARD ALL` also includes `DEALLOCATE ALL`, so
                        // an extended-protocol read here would additionally assert that the driver's per-connection
                        // prepared-statement cache survives a scrub, which is a different subsystem with its own
                        // coverage. Reading that way, this leaf reports a stale-statement `26000` and says nothing
                        // about the search path either way.
                        noteOnSimple(client).map(before => assert(before == "app", s"expected app before the reset, got $before"))
                            .andThen(client.reset)
                            .andThen(
                                noteOnSimple(client).map(after => assert(after == "app", s"expected app after the reset, got $after"))
                            )
                    }
                }
            }
