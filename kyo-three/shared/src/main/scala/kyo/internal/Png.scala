...
/** Pure-Scala PNG encoder for RGBA pixel buffers produced by WebGL readRenderTargetPixels.
  *
  * Encodes a flat RGBA `Uint8Array` (rows bottom-to-top, OpenGL convention) into a valid PNG byte stream
...
private[kyo] object Png:
    ...
    /** Builds the filtered-row buffer: one filter-byte (0x00 = None) plus 4 RGBA bytes per pixel per row,
      ...
      */
    private def filteredRows(pixels: Uint8Array, width: Int, height: Int): Array[Byte] =
        ...
        while pngRow < height do
            ...
            while px < rowStride do
                ...
                buf(dstOff + 1 + px) = pixels(srcOff + px).toShort.toByte
                px += 1
            end while
            pngRow += 1
        end while
    ...
    /** Encodes an RGBA pixel buffer into a valid PNG byte array.
      *
      * @param pixels a flat RGBA `Uint8Array` of length `width * height * 4`, rows bottom-to-top.
      * @param width  the image width in pixels.
      * @param height the image height in pixels.
      * @return a PNG byte array with rows top-to-bottom, color type 6 (RGBA).
      */
    def encode(pixels: Uint8Array, width: Int, height: Int): Array[Byte] =
