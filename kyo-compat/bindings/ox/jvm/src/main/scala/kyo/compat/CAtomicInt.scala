...
/** Underlying carrier is `java.util.concurrent.atomic.AtomicInteger`, a lock-free atomic int cell. `lift` and `lower` are identity since
  * the carrier is already a native `AtomicInteger`. Operations match the `java.util.concurrent.atomic` contract: reads, writes,
  * compare-and-swap, and numeric increments all execute without holding a lock.
  */
...
object CAtomicInt:
    ...
    /** Wraps a native `AtomicInteger` as a `CAtomicInt`. Identity on the carrier. */
    inline def lift(inline u: AtomicInteger): CAtomicInt = u

    extension (inline self: CAtomicInt)

        /** Unwraps to the native `AtomicInteger`. Identity on the carrier. */
