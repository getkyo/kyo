...
object CIO:
    ...
    /** Pairs acquisition with release that runs on every exit path of `use`, including Ox cancellation. Ox cancellation is thread
      * interruption, so a cancelled `use` exits via `InterruptedException` (which `NonFatal` classifies as fatal); the `use` catch matches
      * all `Throwable` so the interrupt does not skip release, and release runs inside `ox.uninterruptible` so a pending interrupt cannot
      * abort it before it completes. On `use`-failure + `release`-failure the release throwable is attached to `use`'s via `addSuppressed`.
      */
