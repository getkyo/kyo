...
/** Underlying carrier is `java.util.concurrent.atomic.AtomicLong`, a lock-free atomic long cell. `lift` and `lower` are identity since the
  * carrier is already a native `AtomicLong`. Operations match the `java.util.concurrent.atomic` contract: reads, writes, compare-and-swap,
  * and numeric increments all execute without holding a lock.
  */
...
object CAtomicLong:
    ...
    /** Wraps a native `AtomicLong` as a `CAtomicLong`. Identity on the carrier. */
    inline def lift(inline u: AtomicLong): CAtomicLong = u

    extension (inline self: CAtomicLong)

        /** Unwraps to the native `AtomicLong`. Identity on the carrier. */
