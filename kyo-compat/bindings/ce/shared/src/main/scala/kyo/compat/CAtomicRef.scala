...
object CAtomicRef:
    ...
    /** Wraps a native `cats.effect.kernel.Ref[IO, A]` as a `CAtomicRef`. Identity on the carrier. */
    inline def lift[A](inline u: Ref[IO, A]): CAtomicRef[A] = u

    extension [A](inline self: CAtomicRef[A])

        /** Unwraps to the native `cats.effect.kernel.Ref[IO, A]`. Identity on the carrier. */
