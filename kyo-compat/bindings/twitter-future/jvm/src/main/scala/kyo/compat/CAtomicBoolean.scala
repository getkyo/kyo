...
/** Underlying carrier is `java.util.concurrent.atomic.AtomicBoolean`, a lock-free atomic boolean cell. There is no `Frame` / `Trace` to
  * propagate. `lift` and `lower` are identity since the carrier is already a JDK atomic. Operations match the `java.util.concurrent.atomic`
  * contract.
  */
...
object CAtomicBoolean:
    ...
    /** Wraps a native `java.util.concurrent.atomic.AtomicBoolean` as a `CAtomicBoolean`. Identity on the carrier. */
    inline def lift(inline u: AtomicBoolean): CAtomicBoolean = u

    extension (inline self: CAtomicBoolean)

        /** Unwraps to the native `java.util.concurrent.atomic.AtomicBoolean`. Identity on the carrier. */
