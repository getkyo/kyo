...
/** Underlying carrier is `scala.collection.immutable.Vector[A]`. Twitter Future has no native bulk-collection type, so the binding uses
  * `Vector` as a portable backing. `lift` and `lower` are identity since the carrier is already a `Vector`. The portable accessor surface
  * (`toSeq`, `toIndexedSeq`, `apply`, `size`, `iterator`, `isEmpty`) is exposed on every backend.
  */
