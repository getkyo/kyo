...
/** Scala Native scheduling backend for `CIO.sleep`, `CIO.timeout`, and `CIO.cede`. Scala Native 0.5 supports
  * `java.util.concurrent.Executors.newScheduledThreadPool`, so this binding mirrors the JVM impl: a two-thread daemon
  * `ScheduledExecutorService`; tasks dispatched from `CompatScheduler.schedule` run on a `compat-scheduler-N` daemon thread.
  *
