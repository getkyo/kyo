...
abstract class UnsafeTraceSpan:
    def end(now: Instant)(using AllowUnsafe): Unit
    def event(name: String, a: Attributes, now: Instant)(using AllowUnsafe): Unit
...
object UnsafeTraceSpan:
    ...
    val noop: UnsafeTraceSpan =
        ...
        new UnsafeTraceSpan:
            def end(now: Instant)(using AllowUnsafe)                                     = ()
            def event(name: String, a: Attributes, now: Instant)(using AllowUnsafe)      = ()
