...
object KyoDoctestPlugin extends AutoPlugin {
    ...
    object autoImport {
        ...
        /** Directory used for the content-hash cache. Created if absent.
          ...
          *
          * Sharing across modules is safe because entries are content-addressed: the key already covers the block body, the classpath
          * fingerprint, the Scala version and the scalac options, so two modules can only collide on a key when all of those agree, in
          * which case reusing the entry is correct rather than a bug.
          */
