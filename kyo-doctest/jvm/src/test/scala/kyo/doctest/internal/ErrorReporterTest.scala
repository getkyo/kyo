...
class ErrorReporterTest extends kyo.test.Test[Any]:
    ...
    "two errors in the same block each produce their own location header" in {
    ...
    "warning diagnostic produces warning: label" in {
    ...
    "useAnsi=true produces ANSI escape codes" in {
    ...
    "Hidden carrier produces (<!-- hidden -->) prefix" in {
    ...
    "caret indicator is placed under the offending column" in {
