...
object KyoFfiPlugin extends AutoPlugin {
    ...
    /** `ffiPackagingCheckAll` minus the completeness half: every FFI project's staged natives must match
      * the platform directory they sit in, but no platform is required of anyone.
      *
      * Completeness can only be judged once every producer's artifacts are pooled, so the full command
    ...
    private def ffiPackagingFormatCheckAllCommand: Command = Command.command("ffiPackagingFormatCheckAll") { state =>
        ...
        if (failed.nonEmpty)
            sys.error(
                s"[kyo-ffi-plugin] ffiPackagingFormatCheckAll: the packaging check failed for " +
                    s"${failed.sorted.mkString(", ")}. See the errors above."
            )
        finalState
    }
    ...
    /** Runs `ffiPackagingCheck` on every FFI project, with the required platforms DERIVED per library
      ...
      *
      * A `publish / skip` project requires no platform, since it ships no jar that could have a hole in
      * it. Decided here rather than by a per-module `ffiReleasePlatforms := Nil`, which would be the
      * remember-to-edit pattern this command exists to remove.
      */
    private def ffiPackagingCheckAllCommand: Command = Command.command("ffiPackagingCheckAll") { state =>
        ...
        val (finalState, failed) =
            projects.foldLeft((state, Seq.empty[String])) { case ((st, bad), ref) =>
                // A project that publishes nothing cannot ship a jar with a hole in it, so no platform
                // is required of it: gating a release on kyo-ffi-it's test scaffolding would fail the
                // publish over a module no user can resolve. It still runs with an empty required set,
                // because the format/architecture half is free signal that the producer toolchain
                // names its artifacts truthfully.
                // `publish / skip` is a task, not a setting, so it is evaluated rather than read.
                val (afterSkip, skipped) = Project.runTask(ref / publish / skip, st) match {
                    case Some((next, Value(v))) => (next, v)
                    case Some((next, Inc(_)))   => (next, false)
                    case None                   => (st, false)
                }
                val ex           = Project.extract(afterSkip)
                val publishes    = !skipped
                val requiredKeys = if (publishes) (ref / ffiReleasePlatforms).get(ex.structure.data).getOrElse(Nil) else Nil
                if (requiredKeys.nonEmpty)
                    afterSkip.log.info(
                    ...
                    afterSkip.log.info(
                        s"[kyo-ffi-plugin] ffiPackagingCheckAll: ${ref.project} requires no platform " +
    ...
    /** The `<id> has no native for <key>` complaints for a required platform set, one per (declared
      * library, required key) pair that `have` does not cover.
      *
    ...
    /** Validate what this build actually staged, before it becomes a published artifact.
      ...
      *     object header, which is the one thing that cannot be renamed.
      ...
      *
      * The completeness half rests on an invariant: a library's absence of C sources is a declaration,
      * never a fallback. `ffiLibrariesResolved` is evaluated on the publishing host, so a library that
      * fell back to `cSources = Nil` when its vendored tree was missing would drop out of the required
      * set exactly when provisioning failed. Intentional absence is declared unconditionally
      * (`kyonet_openssl` on JVM/JS); a shim whose vendored dependency may be missing declares stub
      * sources instead (`kyonet_boringssl`), staying required so the library-state manifest catches it.
      *
    ...
    private def ffiPackagingCheckTask: Def.Initialize[Task[Unit]] = Def.task {
        ...
        val declared = ffiLibrariesResolved.value
        ...
        if (platform == "Native") ()
            ...
            if (required.nonEmpty) {
                ...
                // Requiredness comes from the declared libraries rather than the staged tree; see
                // missingRequiredNatives for why, and for the two rules it turns on.
                val checked = declared.filter(_.cSources.nonEmpty)
                val missing = missingRequiredNatives(declared, required, have)
                if (missing.nonEmpty)
                    sys.error(
                ...
                log.info(
                    s"[kyo-ffi-plugin] $module: ffiPackagingCheck passed, ${checked.size} declared librar(y/ies) " +
                        s"complete for ${required.sorted.mkString(", ")}."
                )
            } else
                log.info(
