...
/** IT spec for the struct-ABI self-check.
  ...
  *   2. Contract: the check surfaces `FfiAbiMismatch` with the expected shape when fed a synthetic divergence, the "what generated impls
...
class ItStructAbiTest extends ItTestBase:
    ...
    "synthetic FfiAbiMismatch" - {
        "a 16-vs-9 size divergence (forgot to pack) surfaces with a remediation hint" in {
            ...
            assert(ex.traitFqn == "kyo.ffi.it.ItStructsBindings")
        ...
        "a 9-vs-16 size divergence (over-packed) surfaces with a remediation hint" in {
            ...
            assert(ex.expectedSize == 9L)
            assert(ex.actualSize == 16L)
        }
    }
end ItStructAbiTest
