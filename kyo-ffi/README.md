...
`Ffi.load[T]` throws subtypes of `FfiLoadError`: `LibraryNotFound` (the native is not resolvable, or a per-module native manifest shows it is not bundled for the current `<os>-<arch>`; the message names the platform), `AbiMismatch` (the generated-impl ABI, or the bundled native's version against the module's `minRuntime` floor, does not match the runtime), `Unsupported` (32-bit host, browser Scala.js), `ImplNotFound` (no generated impl on the classpath). The manifest is read before the generated impl's class initializer runs, so a native missing for this platform surfaces here as a catchable `LibraryNotFound` rather than as a failure inside the initializer, which poisons the class. `Ffi.load[T]` then opens the library and binds its symbols, so no binding call pays for the load, and a native that is present but does not open fails here with the platform loader's own exception. A failed load is final for the process: a later `Ffi.load[T]` rethrows the same exception. Callers that want a single catch handler use `catch { case e: FfiLoadError => ... }`.
