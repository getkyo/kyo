    ...
    "tryCleanupStaleLock" - {
        "removes an abandoned .lck file with no live lock holder" in {
            freshDir().map { dir =>
                ...
                assert(lck.unsafe.exists().getOrThrow == true)

                val removed = NativeLoader.tryCleanupStaleLock(j(lck))

                assert(removed == true)
                assert(lck.unsafe.exists().getOrThrow == false)
            }
        }
    ...
    "writeAtomicRename" - {
        "atomically installs the full payload and leaves no .tmp-<uuid> residue on success" in {
            freshDir().map { dir =>
                ...
                assert(out.unsafe.exists().getOrThrow == true)
        ...
        "removes files newer than install epoch" in {
            freshDir().map { dir =>
                ...
                try
                    ...
                    assert(fresh.unsafe.exists().getOrThrow == false)
