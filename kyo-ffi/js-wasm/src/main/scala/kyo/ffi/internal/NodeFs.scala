...
/** Facade over Node's built-in `fs` module for the [[BufferFactory]] mmap fallback.
  ...
  * `readFileSync` backs the mmap fallback; `existsSync` backs the loader's env-path presence check.
  */
@js.native
...
private[ffi] object NodeFs extends js.Object:
    ...
    /** `true` when `path` exists on the filesystem. Used to gate the `KYO_FFI_<id>_PATH` override on a real file. */
