...
/** Parsers for the text SQLite hands back, each the exact inverse of the matching [[kyo.internal.SqlValueRender]] method.
  *
  * SQLite has no type of its own for any of these: a TEXT-affinity column stores the characters the driver wrote and returns them byte for
  * byte, so the only format in play is the one the renderer chose.
  */
private[sqlite] object SqliteText:
    ...
    /** Splits a trailing `+HH:MM[:SS]` off a value, or answers no offset.
      ...
      * would split `2026-08-25 10:00:00` in the middle of its date, and nothing about the answer would look wrong.
      *
