...
When `LspClient.init` returns, the handshake has already completed: `client.serverCapabilities`, `client.serverInfo`, and `client.positionEncoding` are populated, ready to read synchronously without a follow-up await.
...
val notifyOnLint =
    LspHandler.TextDocument.didChange { _ =>
        Lsp.server.map(_.showMessage(LspHandler.ShowMessageParams(
...
val onOpenLint =
    LspHandler.TextDocument.didOpen { params =>
        Lsp.documents.map(_.get(params.textDocument.uri)).map { maybeDoc =>
            ...
            Lsp.server.map(_.publishDiagnostics(LspHandler.PublishDiagnosticsParams(
...
`LspCapabilities.Name` is a typed discriminator used by `LspException.Dispatch.CapabilityNotAdvertised` to identify which capability the rejected method needed.
