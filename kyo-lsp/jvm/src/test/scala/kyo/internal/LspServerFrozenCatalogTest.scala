...
class LspServerFrozenCatalogTest extends Test:
    ...
    "LspServer companion has no add/remove/set handler methods" in {
    ...
    "LspServer.Unsafe has no add/remove/set handler methods" in {
    ...
    "LspServer init methods are all named init/initWith/initUnscoped/initUnscopedWith" in {
        val companionMethods = classOf[LspServer.type].getMethods.map(_.getName).toSet
