...
class SchemaSingletonTest extends Test:
    ...
    "WorkDoneProgressValue schema is a singleton" in {
        val s1 = summon[Schema[LspHandler.WorkDoneProgressValue]]
        val s2 = summon[Schema[LspHandler.WorkDoneProgressValue]]
        assert(s1 eq s2)
    }
    ...
    "DocumentDiagnosticReport schema is a singleton" in {
        val s1 = summon[Schema[LspHandler.DocumentDiagnosticReport]]
        val s2 = summon[Schema[LspHandler.DocumentDiagnosticReport]]
        assert(s1 eq s2)
    }

    "WorkspaceDocumentDiagnosticReport schema is a singleton" in {
        val s1 = summon[Schema[LspHandler.WorkspaceDocumentDiagnosticReport]]
        val s2 = summon[Schema[LspHandler.WorkspaceDocumentDiagnosticReport]]
        assert(s1 eq s2)
    }
