...
/** That every parser is the exact inverse of the renderer that produced the text.
  *
  * SQLite has no temporal or decimal type of its own: a TEXT-affinity column stores the characters the driver wrote and hands them back byte
  * for byte, so the only format in play is the one `SqlValueRender` chose. Each leaf therefore RENDERS a value and parses the result; a
  * literal typed here would pin what the author believed the renderer writes rather than what it does.
  */
class SqliteTextTest extends Test:
    ...
    "times round-trip, including the endpoints a LocalTime cannot hold" - {
        ...
        "a fraction whose trailing zeros the renderer drops is read back at full width" in {
            // The renderer writes `.5` for half a second, which read as an integer is 5 microseconds, so the parser
            // pads back to six digits. This is the one place the inverse is not a plain split.
    ...
    "timestamps round-trip, with the era trailing the offset" - {
        ...
        "an instant before the era boundary" in {
            // The era trails the OFFSET here, not the date and time, so the parser strips it FIRST: looking for the
            // offset sign inside a trailing ' BC' finds the date's hyphen instead.
