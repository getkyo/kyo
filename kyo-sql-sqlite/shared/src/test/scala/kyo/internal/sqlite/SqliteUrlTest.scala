...
/** What a `sqlite://` URL means, and in particular what it does NOT mean. Most of these leaves feed in a character that is a DELIMITER to
  * the shared network parser and an ordinary one in a path, which is read verbatim.
  */
class SqliteUrlTest extends Test:
    ...
    "a URI filename keeps everything SQLite itself will read" in {
        // SQLite reads `file:` names with their own query string, and the driver's split takes the first `?`, so a
        // caller spelling one gets the name intact and the rest as driver options.
