...
/** Covers the reads in [[SqliteRowReader]] that have to answer for something the engine does not.
  ...
  * complaint where the other engines refuse it at the server. A dialect could only reconcile that by wrapping every arithmetic expression in
  * a range guard, which would change the answer for legitimate 64-bit arithmetic, so the reconciliation happens at the read: a value that
  * does not fit the type being decoded is refused as OUT OF RANGE rather than as malformed text.
  */
        ...
        withDb { url =>
            SqlClient.init(url, SqlConfig(maxConnections = 1)).map { client =>
                DB.run(client) {
                    ...
                    yield
                        // Read at the width the value actually has, the sum is an ordinary 64-bit integer and no error was raised.
        ...
        withDb { url =>
            SqlClient.init(url, SqlConfig(maxConnections = 1)).map { client =>
                DB.run(client) {
                    ...
                    yield
                        // Out of range rather than a decode failure: the text IS a number, it just does not fit.
                        assert(
        ...
        withDb { url =>
            SqlClient.init(url, SqlConfig(maxConnections = 1)).map { client =>
                DB.run(client) {
                    ...
                    yield
                        // The other half of the split: one says the column never held numbers, the other that this number is too big.
                        assert(
