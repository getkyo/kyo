...
/** Consumes a SQLite row's columns positionally, decoding each from the text SQLite returned.
  *
  * One wire format, unlike the two the network backends carry. SQLite hands back a value's bytes and its storage class, and every type the
  * driver does not store natively is stored as the text the renderer wrote. So there is no binary arm here: the reader parses characters,
  * and [[SqliteText]] holds the parsers because the codec needs the same ones.
  *
...
final private[sqlite] class SqliteRowReader(
    ...
    private def nextBytes(): Span[Byte] =
        ...
        value match
            ...
            // Reached only when a caller read a value without asking isNil first, which the SPI allows and the
            // generated codecs do not do. Naming the column is what makes that caller findable.
    ...
    /** Runs a parse, turning whatever it throws into a decode failure naming the column it was reading.
      *
      * The parsers throw `NumberFormatException` and friends on malformed text, which is a per-column decode failure a caller can act on;
      * passing them through untyped would make the column invisible.
      */
    ...
    /** Refuses a column the DDL declared as something other than text.
      ...
      * holds. That is exactly why the check has to be here: reading an INTEGER column as `String` is a caller's mistake on every other
      * engine and returns `"42"` on this one, so without it the same program is type-checked by two engines and not by the third.
      *
      * A column traceable to no declared type stays permissive, which is the same answer the other engines give: an expression or a CAST
      * has no declared type anywhere, so refusing would reject code that is correct.
      */
    ...
    /** Refuses the column at the cursor when its DECLARED kind is one `accepts` rejects.
      *
      * A column traceable to no declared type is never refused, which is the same answer the other engines give: an expression or a CAST has
      * no declared type anywhere, so refusing one would reject code that is correct.
      */
    ...
    // NumberFormatException for text that is not a number and for a number that simply does not fit, and the two are
    // different failures to a caller: one is a malformed column, the other is a value out of range. SQLite reaches
    // the second on its own, since its integers are 64-bit and it widens an overflowing sum instead of refusing it,
    // so an Int-declared column can hand back a well-formed value larger than Int.
    ...
    /** Runs a numeric parse, reporting a failure as a NUMERIC decode error naming the text that would not parse.
      *
      * Distinct from [[parsing]] on purpose: text that is not a number is a different failure from a column whose declared type was never
      * numeric, and a caller looking at `Cannot parse numeric value: 'hello'` can see which one happened.
      */
    ...
    /** A duration is written as an interval, so it is read as one and folded back to a length of time.
      *
      * Months and days are not part of a duration and are refused rather than approximated: a month is not a fixed number of seconds, and
      * silently choosing one is how a value drifts.
      */
    ...
    override def nextDate(): java.time.LocalDate =
        ...
        parsing(text) { t =>
            ...
            // A proleptic year counts an era-BC year as 1 - year, which is what LocalDate holds and is NOT what the
    ...
    override def nextTime(): java.time.LocalTime =
        ...
        parsing(text) { t =>
            ...
            if hours > 23 || t.startsWith("-") then
                // The column is a signed SPAN and reaches past a day in both directions; a time of day does not. The
    ...
    // Arrays are JSON, which is the same choice MySQL made for the same reason: neither engine has an array type.
    ...
    override def decodeElement[A](column: SqlSchema.Column[A], bytes: Span[Byte], format: SqlCodec.Format): A =
        // One reader over the single nested column, so the element decodes through exactly the paths above. The token
        // is -1, which no declaration index reaches, so the nested column reads as having no declared type.
...
private[sqlite] object SqliteRowReader:
    ...
    /** Whether a column declared as `kind` may be read as a temporal value.
      ...
      * declared `TEXT` are both legitimate sources. What it rules out is the numeric and boolean kinds, where the stored value would parse
      * into something well-formed and wrong rather than failing.
      */
    ...
    /** Whether a column declared as `kind` may be read as a `String`.
      *
      * The textual kinds, and only those. `Unknown` never reaches here: a column with no declared type is not checked at all, because an
      * expression has no declared type on any engine and refusing one would reject correct code.
      */
