...
/** Decodes a SQLite result set, dispatching on each column's DECLARED type.
  *
  * SQLite has two type notions and only one of them answers the question a codec is asked. A value's STORAGE CLASS is per row and has five
  * members, so a date and a decimal are both `text` and a boolean and an integer are both `integer`: it cannot say that a column is a date.
  * The DECLARED type is per column, is whatever string the DDL wrote, and is exactly the vocabulary the type mapping chose. So `columnKind`
  * reads the declared type, and `typeToken` is the column's index into the declarations this result set reported.
  *
  * An index rather than a packed tag, because a declared type is a STRING with parameters: `DECIMAL TEXT(38,10)` carries a scale no Int
  * holds, and `typeName` is specified as the engine's own spelling for a human. A codec is built once per result set, which is where the
  * declarations live.
  *
  * A column traceable to no declared one answers [[Absent]]: every expression, every `CAST`, and every column declared with no type. Those
  * are [[SqlRow.ColumnKind.Unknown]], which is the honest answer, since an expression has no declared type on any engine either.
  *
  * @param declarations
...
final private[sqlite] class SqliteRowCodec(declarations: Chunk[Maybe[String]]) extends SqlPositionalRowCodec:
    ...
    /** One format. SQLite hands back a value's bytes, and every type it has no native storage for is held as the text the renderer wrote,
      * so there is no binary arm to choose between.
      */
    ...
    def newReader(sliced: SqlRow, matchesFieldAt: Maybe[(Int, String) => Boolean])(using Frame): SqlCodec.Reader =
        // The slice's columns travel with the reader so a read can be checked against what the column was DECLARED as. They carry the
        // token, which indexes the whole result set's declarations rather than the slice, so a decode of the second row type in a join
    ...
    /** A `FLOAT` column is four bytes wide even though SQLite stores it as a double.
      *
      * The width is not SQLite's, which has only one; it is the one the DDL declared, and it is what keeps 0.1 from being rendered at
      * double width after a round trip through a column that was asked to hold a Float.
      */
    ...
    /** The kinds the shared decode leaves to the backend, plus the one case SQLite has that no declared type covers.
      *
      * The shared implementation handles the kinds a neutral read spans and refuses the rest, because on a network backend they need the
      * wire itself. Here they need only the text, which [[SqliteText]] already parses, so each is answered rather than refused.
      *
      * `Unknown` is the interesting one. On a network backend it means a type with no neutral kind and is correctly unrenderable. Here it
      * means an EXPRESSION, which is an ordinary thing to select, and the server's own rendering is exactly what such a column carries. So
      * it answers [[SqlValue.ServerRendering]] rather than refusing.
      */
    override def columnValue(row: SqlRow, idx: Int)(using Frame): SqlValue < Abort[SqlDecodeException] =
        ...
        columnKind(typeToken) match
            ...
            case ColumnKind.Array =>
                // Arrays are JSON here, as on MySQL, neither engine having an array type. The elements come back as
                // their own text rather than re-parsed, which is what Elements carries.
                textOf(row, idx).map { text =>
...
private[sqlite] object SqliteRowCodec:
    ...
    /** The leading word of a declared type, upper-cased.
      ...
      * so a value that looks numeric is not rewritten on the way in. The KIND lives in the first word; the rest is affinity and parameters.
      */
    ...
    /** The temporal kinds, built from the wire FIELDS rather than from a `java.time` value.
      *
      * Fields because the neutral values hold what `java.time` cannot: a date counts its year within an era rather than as a negative
      * number, and a time is a signed span that reaches past a day in both directions.
      */
