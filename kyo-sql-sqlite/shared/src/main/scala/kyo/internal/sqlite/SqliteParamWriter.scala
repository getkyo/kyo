...
/** Collects the bind parameters for one statement, in the storage class SQLite will hold each value in.
  ...
  * converted rather than refused. The rule here is that a value the driver renders as text is BOUND as text, and the declared type in the
  * DDL is what makes the column hold it verbatim. Binding a decimal as a double, for instance, would lose its scale and its digits past a
  * double before the column ever saw it.
  *
  * So only the four kinds SQLite genuinely stores land in a non-text class: integers, floats, blobs, and booleans, which SQLite holds as
  * integers. Everything else is the rendering [[kyo.internal.SqlValueRender]] produces, which is also exactly what [[SqliteText]] reads
  * back.
  */
final private[sqlite] class SqliteParamWriter(frame0: Frame) extends SqlCodec.Writer(frame0):
    ...
    def double(value: Double): Unit =
        ...
        // would turn a value into an absent one silently, which is the failure this driver exists to avoid.
    ...
    /** Text, not a double. A decimal bound as a double loses its scale and every digit past a double before the column sees it.
      *
      * Always carrying a decimal point, because text is coerced by what it LOOKS like: `4` parses as an integer, and a column whose stored
      * text also looks integral then divides by it integrally, so a stored 10 over a bound 4 answers 2. `4.0` parses as a real and the same
      * division answers 2.5. The point costs nothing on any other path, since the value is read back through the column's declared scale.
      */
    ...
    // Arrays are JSON, the same choice MySQL made for the same reason: neither engine has an array type.
...
private[sqlite] object SqliteParamWriter:
    ...
    /** One bound value, in the storage class SQLite will hold it in.
      *
      * A class rather than bytes plus a tag, because the bind call differs per class: an integer and a blob of the same bytes are different
      * values, and choosing at the bind site is what keeps affinity from rewriting either.
      */
