...
/** The Scala 3 members of [[Platform]] on the JVM and Scala Native.
  ...
  * compile-time constant (platform identity, and `isWasm`, which is `false`), so it is an `inline if` and the untaken branch is never emitted.
  * On Scala.js the same call resolves `isWasm` at link time.
  */
trait PlatformStatic:
    ...
    /** `thenp` when `cond` holds and `elsep` otherwise, with the untaken branch left out of the output. `cond` may combine only `isJVM`,
      * `isJS`, `isNative`, and `isWasm`: those are the members known before run time on every platform.
      */
