...
/** A warm, 1:1-per-config presentation-compiler handle.
  ...
  * types, the neutral offset-based result types, and the [[CompilerError]] failure leaf.
  *
...
abstract class Compiler:
    ...
    def compile(uri: Uri, text: String)(using Frame): Chunk[Diagnostic] < (Async & Abort[CompilerError])

    /** Completions at an offset (empty Chunk = none). */
    def completions(uri: Uri, text: String, offset: Int)(using Frame): Chunk[Completion] < (Async & Abort[CompilerError])

    /** Hover information at an offset (Absent = none). */
    def hover(uri: Uri, text: String, offset: Int)(using Frame): Maybe[Hover] < (Async & Abort[CompilerError])

    /** Signature help at an offset (Absent = none). */
    def signatureHelp(uri: Uri, text: String, offset: Int)(using Frame): Maybe[Signature] < (Async & Abort[CompilerError])

    /** The symbol at an offset, with its live in-buffer definition span (Absent = none). */
    def symbol(uri: Uri, text: String, offset: Int)(using Frame): Maybe[SymbolInfo] < (Async & Abort[CompilerError])

    /** Drops the pc's per-uri cache. The result is `Unit`, but the op round-trips to the backend, so a
      * comms failure surfaces as `Abort[CompilerError]`.
      */
    def didClose(uri: Uri)(using Frame): Unit < (Async & Abort[CompilerError])
end Compiler
...
/** Drives the Scala 3 presentation compiler for IDE language intelligence.
  ...
  * types, the opaque [[Compiler.Uri]], and the [[CompilerError]] failure leaf. Everything
