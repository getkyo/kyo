...
/** The forked-worker backend: drives a per-config worker JVM over an aeron request/response session.
  ...
  * or a closed session maps to `CompilerError.Fatal` at this boundary so the op surface stays
  * `Abort[CompilerError]`; a broken session fails every pending op, and a stuck worker is reclaimed by
    ...
    def run(request: Request)(using Frame): Response < (Async & Abort[CompilerError]) =
        exchange(request)
            ...
            .map {
                ...
                case Result.Failure(TransportError(m))      => Abort.fail(CompilerError.Fatal(m))
                case Result.Failure(_)                      => Abort.fail(CompilerError.Fatal("worker session closed"))
...
private[kyo] object SpawnBackend:
    ...
    def init(
    ...
    )(using Frame): SpawnBackend < (Async & Abort[CompilerError]) =
        ...
        Abort.run[Throwable] {
            Abort.catching[Throwable]((t: Throwable) => t) {
                        ...
                        connect(aeron, reqStreamId, respStreamId).map { exchange =>
                            ...
                            Sync.ensure(
                                ...
                                Sync.defer(onSpawn(process))
                                    .andThen(ready(backend, readyTimeout))
                                    .andThen(Sync.defer { started.set(true); backend })
                            }
                        }
                    }
                }
            ...
            // Preserve the failure's stack in the log; the typed error stays concise for the wire.
            case Result.Failure(t) =>
                Log.error("worker backend failed to initialize", t)
                    .andThen(Abort.fail(CompilerError.InitializationFailed(t.getMessage)))
            case Result.Panic(t) =>
                Log.error("worker backend failed to initialize", t)
                    .andThen(Abort.fail(CompilerError.InitializationFailed(t.getMessage)))
        }
    ...
    /** A bounded readiness round-trip so a worker that cannot start (e.g. an unusable classpath, whose
      ...
      */
    private def ready(backend: SpawnBackend, readyTimeout: Duration)(using Frame): Unit < (Async & Abort[CompilerError]) =
        val probe = backend.run(Request.DidClose(Compiler.Uri("kyo-compiler-readiness-probe.scala")))
        Abort.run[CompilerError | Timeout](Async.timeout(readyTimeout)(probe)).map {
            case Result.Success(_)     => ()
            case Result.Failure(error) => Abort.fail(CompilerError.InitializationFailed(s"worker failed to start: $error"))
            case Result.Panic(t)       => Abort.fail(CompilerError.InitializationFailed(s"worker failed to start: ${t.getMessage}"))
        }
    end ready
    ...
    private[kyo] def spawnWorker(config: Compiler.Config, driver: MediaDriver, reqStreamId: Int, respStreamId: Int)(using
        Frame
    ): Process < (Sync & Abort[CompilerError]) =
        val targetClasspath =
        ...
        Abort.run[CommandException](Command(args*).inheritStderr.spawnUnscoped).map {
            ...
            case Result.Failure(e)    => Abort.fail(CompilerError.InitializationFailed(e.getMessage))
            case Result.Panic(t)      => Abort.fail(CompilerError.InitializationFailed(t.getMessage))
        }
    end spawnWorker
    ...
    /** Maps a `Topic` transport break (`Closed` or `Backpressured`) onto the Exchange's
      * `TransportError`, so a broken session fails every pending op with one typed error.
      */
    private def transportErrors[A, S](v: A < (S & Abort[Closed | Topic.Backpressured]))(using Frame): A < (S & Abort[TransportError]) =
        Abort.run[Closed | Topic.Backpressured](v).map {
            ...
            case Result.Panic(err)      => Abort.fail(TransportError(err.getMessage))
        }
end SpawnBackend
