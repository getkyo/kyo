...
private[kyo] object LocalBackend:
    /** Bridges a pc CompletableFuture into Async via `Async.fromCompletableFuture`, which cancels the
      ...
      * `Abort[Throwable]` rather than a panic. Every pc failure maps to `CompilerError.Fatal`: a
      ...
      */
    private[kyo] def bridge[A](cf: => CompletableFuture[A])(using Frame): A < (Async & Abort[CompilerError]) =
        Abort.catching[Throwable](t => CompilerError.Fatal(t.getMessage)) {
            Sync.defer(cf).map { future =>
                Abort.run[Throwable](Async.fromCompletableFuture(future)).map {
                    ...
                    case Result.Failure(error) =>
                        ...
                        Abort.fail(CompilerError.Fatal(cause.getMessage))
                    case Result.Panic(error) => Abort.fail(CompilerError.Fatal(error.getMessage))
                }
            }
        }

    /** Instantiates a version-matched pc for a config and wraps it in a LocalBackend. A pc that
      * cannot start surfaces as InitializationFailed on the caller's first op.
      */
    def init(config: Compiler.Config)(using Frame): Backend < (Async & Abort[CompilerError]) =
        Abort.run[Throwable] {
