...
final private[kyo] class CompilerPool(
            ...
            val metered =
                instance.mutex.run(globalSemaphore.run(instance.backend.run(request)))
                    ...
                    .map {
                        ...
                        case Result.Failure(_)                      => Abort.fail(CompilerClosedException())
                        case Result.Panic(err)                      => Abort.fail(CompilerExecutionException(err))
                    }
            Async.timeout(settings.stuckTimeout)(metered)
    ...
    private def resolve(config: Compiler.Config)(using Frame): Instance < (Async & Abort[CompilerException]) =
        Sync.Unsafe.defer {
            ...
            val promise = Promise.Unsafe.init[Instance, Abort[CompilerException]]().safe
            ...
            if (cached.asInstanceOf[AnyRef]) eq (promise.asInstanceOf[AnyRef]) then
                Sync.Unsafe.ensure {
                    ...
                    Abort.runWith[CompilerException](create(config)) {
                        case Result.Success(instance) =>
                            Sync.Unsafe.defer {
                                promise.unsafe.completeDiscard(Result.succeed(instance))
                                instance
                            }
                        case error: Result.Error[CompilerException] @unchecked =>
                            Sync.Unsafe.defer {
    ...
    /** Creates the per-config instance: chooses the backend by effectiveIsolate, builds it, and
      ...
      */
    private def create(config: Compiler.Config)(using Frame): Instance < (Async & Abort[CompilerException]) =
    ...
    /** The backend-selection rule: a forked worker whenever `isolate` resolves true OR the
      ...
      */
    private def backendFor(config: Compiler.Config)(using Frame): Backend < (Async & Abort[CompilerException]) =
        ...
        if effectiveIsolate then
            streamIdCounter.getAndIncrement.map(base => SpawnBackend.init(config, driver, base, readyTimeout = settings.readyTimeout))
        else LocalBackend.init(config)
    end backendFor

    // An internal invariant violation: the backend must return the Response kind that matches the
    // Request. It is a bug, not a tracked failure, so it panics rather than riding the Abort row.
    private def unexpected(response: Response)(using Frame): Nothing < Sync =
        Sync.defer(throw new IllegalStateException(s"unexpected worker response: $response"))
end CompilerPool
    ...
    def init(settings: Compiler.Pool.Settings)(using Frame): Compiler.Pool < (Sync & Scope) =
        Meter.initSemaphore(settings.maxConcurrentCompiles).map { globalSemaphore =>
            Cache.initWithFinalizer[Compiler.Config, Promise[Instance, Abort[CompilerException]]](
                ...
                Abort.run[CompilerException](promise.get).map {
