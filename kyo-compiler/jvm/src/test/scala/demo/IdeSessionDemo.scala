...
/** A self-contained, runnable demo of a code editor's language-server backend: the
  ...
  * Compiler.signatureHelp, Compiler.symbol, Compiler.didClose, CompilerError, Diagnostic,
...
object IdeSessionDemo extends KyoApp:
    ...
    run {
        for
            ...
            outcome <- Abort.run[CompilerError](withCompiler(c => flow(c).map(r => (r, validate(r)))))
    ...
    /** The language-intelligence session. Receives the warm per-config handle and drives the public ops;
      ...
      */
    def flow(compiler: Compiler)(using Frame): SessionReport < (Async & Abort[CompilerError]) =
    ...
    def withCompiler[A](f: Compiler => A < (Async & Abort[CompilerError]))(using
        Frame
    ): A < (Async & Scope & Abort[CompilerError]) =
        for
