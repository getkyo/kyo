...
class CompilerTest extends kyo.test.Test[Any]:
    ...
    "result ADTs round-trip through the upickle AsMessage codec with CanEqual" in {
        ...
        val errInit   = CompilerError.InitializationFailed("could not load scala3-presentation-compiler")
        val errInitRt = roundTrip(errInit)
        assert(errInitRt == errInit)

        val errFatal   = CompilerError.Fatal("internal error")
        val errFatalRt = roundTrip(errFatal)
        assert(errFatalRt == errFatal)
    }
