...
class LocalBackendTest extends kyo.test.Test[Any]:
    ...
    "the real bridge maps an exceptionally-completed pc future to a typed Fatal, never an escaped panic" in {
        for
            ...
            result <- Abort.run[CompilerError](LocalBackend.bridge(failed))
            _ = result match
                case Result.Failure(CompilerError.Fatal(msg)) =>
                    assert(msg.contains("boom"), s"expected the pc error message in the Fatal, got: $msg")
                case other => assert(false, s"expected a typed Fatal from an exceptionally-completed future, got: $other")
        yield ()
    }

    "a pc throw surfaces as a typed CompilerError, never an escaped throw (sub-case A and B)" in {
        for
            ...
            resultA <- Abort.run[CompilerError](runA)
            _ = resultA match
                case Result.Failure(CompilerError.Fatal(msg)) =>
                    assert(msg.nonEmpty, "expected non-empty error message in CompilerError.Fatal")
                case Result.Panic(_) =>
            ...
            resultB <- Abort.run[CompilerError](initB)
            _ = resultB match
