...
class SpawnBackendTest extends kyo.test.Test[Any]:
    ...
    /** The current JVM's own `java` launcher, used to spawn a throwaway child process when a test needs
      ...
      */
    private val javaBin =
        java.lang.System.getProperty("java.home") + java.io.File.separator + "bin" + java.io.File.separator + "java"

    /** Runs `f` against a fresh embedded MediaDriver, closed on scope exit. */
    private def withDriver[A](f: MediaDriver => A < (Async & Abort[CompilerError] & Scope))(using
        Frame
        ...
        Scope.run {
            Abort.run[Closed] {
                for
                    ...
                    proc <- Sync.Unsafe.defer(Command(javaBin, "-version").unsafe.spawn()).map {
                        case Result.Success(p) => p.safe
                    ...
                    fiber1  <- Fiber.initUnscoped(Abort.run[CompilerError](backend.run(Request.Compile(uri, "object A"))))
                    sentId1 <- sentCh.take
                    ...
                    fiber2  <- Fiber.initUnscoped(Abort.run[CompilerError](backend.run(Request.Completions(uri, "object B {}", 0))))
                    sentId2 <- sentCh.take
    ...
    "no thread leak after a kill: close kills the worker and every later op fails with a typed Fatal" in {
        withDriver { driver =>
            for
                ...
                r1 <- Abort.run[CompilerError](backend.run(Request.Compile(uri, "object A")))
                r2 <- Abort.run[CompilerError](backend.run(Request.Completions(uri, "object B {}", 0)))
                r3 <- Abort.run[CompilerError](backend.run(Request.DidClose(uri)))
                _ = List(r1, r2, r3).foreach { r =>
                    val ok = r match
                        case Result.Failure(CompilerError.Fatal(_)) => true
                        case _                                      => false
                    assert(ok, s"every op after a kill must fail with CompilerError.Fatal, got $r")
                }
            yield ()
        }
    }

    "a worker-comms failure surfaces didClose as a typed Fatal; a live worker yields Closed" in {
        withDriver { driver =>
            for
                backend <- SpawnBackend.init(spawnConfig(), driver, 0)
                uri = Compiler.Uri("DidClose.scala")

                liveResult <- Abort.run[CompilerError](backend.run(Request.DidClose(uri)))
                _ = liveResult match
                ...
                brokenResult <- Abort.run[CompilerError](backend.run(Request.DidClose(uri)))
                _ = brokenResult match
                    case Result.Failure(CompilerError.Fatal(_)) => succeed("broken didClose surfaced as a typed Fatal")
                    case other => assert(false, s"expected Failure(CompilerError.Fatal) from a broken worker, got $other")
            yield ()
        }
    }
        ...
        withDriver { driver =>
            for
                ...
                fiber <- Fiber.initUnscoped(
                    Abort.run[CompilerError](SpawnBackend.init(spawnConfig(), driver, 7, p => spawned.set(Present(p))))
                )
