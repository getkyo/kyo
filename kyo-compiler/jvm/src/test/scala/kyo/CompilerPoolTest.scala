...
class CompilerPoolTest extends kyo.test.Test[Any]:
    ...
    private def completed(instance: Instance): Promise[Instance, Abort[CompilerException]] =
        import AllowUnsafe.embrace.danger
        val p = Promise.Unsafe.init[Instance, Abort[CompilerException]]().safe
    ...
    private def makePool(
        ...
        stuckTimeout: Duration = Compiler.Pool.Settings.default.stuckTimeout
    ...
    "one instance per config: two concurrent ops on the same config reuse it; eviction triggers recreate" in {
        Scope.run {
            ...
            )(promise => Abort.run[CompilerException](promise.get).map { case Result.Success(i) => i.close; case _ => () }).map {
                instances =>
                    makePool(settings, instances).map { pool =>
                        for
                            // First op on cfg: creates the instance.
                            c1 <- pool.compiler(cfg)
                            _  <- Abort.run[CompilerException](c1.compile(uri, "object A"))

                            // Second op on the same config: reuses the cached instance.
                            c2   <- pool.compiler(cfg)
                            res2 <- Abort.run[CompilerException](c2.compile(uri, "object A"))
                            _ = assert(
                                !res2.isPanic,
                            ...
                            _   <- Abort.run[CompilerException](c2a.compile(uri, "object B"))
                            c3a <- pool.compiler(cfg3)
                            _   <- Abort.run[CompilerException](c3a.compile(uri, "object C"))

                            // After eviction, a third op on cfg must succeed via recreate (not fail).
                            c1b  <- pool.compiler(cfg)
                            res3 <- Abort.run[CompilerException](c1b.compile(uri, "object A"))
                            _ = assert(
                                !res3.isPanic,
                                        ...
                                        Fiber.initUnscoped {
                                            gate.await.andThen {
                                                pool.compiler(cfg).map { c =>
                                                    Abort.run[CompilerException](c.compile(Compiler.Uri("f.scala"), "object X"))
                                                }
                                            }
                                        }
                                    }
                        ...
                        for
                            ...
                            _ = resMismatch match
                                ...
                                case other =>
                                    assert(
                                        false,
                                        s"expected a CompilerInitializationFailure (worker could not start) for version mismatch, got $other"
                                    )
                                ...
                                makePool(settings, instances).map { pool =>
                                    for
                                        ...
                                        f1 <- Fiber.initUnscoped {
                                            gate.await.andThen {
                                                pool.compiler(cfg).map { c =>
                                                    Abort.run[CompilerException](c.compile(uri, "object A"))
                                                }
                                            }
                                        }
                                        f2 <- Fiber.initUnscoped {
                                            gate.await.andThen {
                                                pool.compiler(cfg).map { c =>
                                                    Abort.run[CompilerException](c.compile(uri, "object B"))
                                                }
                                            }
                                        }
                                        _ <- gate.release
                ...
                val backendA = new Backend:
                    def run(request: Request)(using Frame): Response < (Async & Abort[CompilerException]) =
                        Abort.run[Closed](parkA.take).andThen {
                            Abort.fail(CompilerExecutionException(new RuntimeException("A crashed")))
                        }
                    def close(using Frame): Unit < (Async & Abort[Throwable]) = ()
                ...
                instances =>
                    makePool(settings, instances).map { pool =>
                        for
                            ...
                            resFirst <- Abort.run[CompilerException](c.compile(Compiler.Uri("z.scala"), "object Z"))
                            _ = resFirst match
                            ...
                            instances.add(cfg, completed(Instance(hangingBackend, mutex))).andThen {
                                makePool(settings, instances, stuckTimeout).map { pool =>
                                    for
                                        ...
                                        resFresh <- Abort.run[CompilerException](cFresh.compile(uri, "object Fresh"))
                                        _ = resFresh match
                                            case Result.Success(diags) =>
                                                assert(
