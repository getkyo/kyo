...
object StaticSite extends KyoApp:
    ...
    case class FileInfo(name: String, size: Long, etag: String) derives Json
    case class FileList(files: List[FileInfo], directory: String) derives Json
    ...
    def handlers(store: AtomicRef[Map[String, Array[Byte]]]) =
        ...
        val serve = HttpRoute
            .getRaw("files" / Capture.Rest("path"))
            .filter(serverFilter)
        ...
        val head = HttpRoute
            .headRaw("files" / Capture.Rest("path"))
            .filter(serverFilter)
