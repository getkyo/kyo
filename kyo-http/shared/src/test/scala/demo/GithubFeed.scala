...
object GithubFeed extends KyoApp:
    ...
    def fetchEvents(url: String): Seq[GithubEvent] < (Async & Abort[HttpException]) =
    ...
    def pollStream(url: String): Stream[HttpSseEvent[FeedEvent], Async] < Async =
        for
            seenRef <- AtomicRef.init(Set.empty[String])
        yield Stream[HttpSseEvent[FeedEvent], Async] {
            Loop.foreach {
                for
                    _           <- Async.delay(10.seconds)(())
                    fetchResult <- Abort.run[HttpException](fetchEvents(url))
                ...
                yield Emit.valueWith(Chunk.from(newEvents.map { e =>
                    HttpSseEvent(
                        data = toFeed(e),
    ...
    run {
        ...
        HttpServer.init(HttpServerConfig().port(port))(publicFeed, repoFeedRoute, health).map { server =>
            for
                ...
                _ <- Console.printLine(s"  curl -N http://localhost:${server.port}/feed/scala/scala")
                _ <- server.await
            yield ()
        }
    }
