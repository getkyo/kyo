...
class GrowableByteBufferTest extends kyo.BaseHttpTest:
    ...
    "GrowableByteBuffer" - {
        ...
        // The doubling-on-grow growth strategy can overflow Int to a negative array length (CWE-190). The pre-fix code doubled in Int
        // arithmetic (`grow(newLen * 2)`): once the doubling target reaches 2^30 the next double is 2^31, which wraps to Int.MinValue and
        // feeds a negative length to `new Array[Byte](...)` (a NegativeArraySizeException), or, doubling again, to 0 (a non-terminating
        // growth loop / silent under-allocation). The fix computes the required capacity and the doubling target in Long and caps both at
        // MaxArrayLength. The guard's contract: any request strictly above MaxArrayLength fails fast with a clear, bounded
        // IllegalArgumentException and never asks the runtime for a negative or undersized array; the failure is in the length computation,
        // before any large allocation, so these leaves cost nothing.

        // Test 19: a capacity request strictly above MaxArrayLength fails with a clear bounded error, not a NegativeArraySizeException, and
        // without allocating. Int.MaxValue == MaxArrayLength + 8 > MaxArrayLength, so `required > MaxArrayLength` trips the guard up front
        // (no doubling loop, no allocation). The pre-fix Int doubling would instead wrap the length negative.
        ...
        // Test 20: a non-zero position plus a request whose sum exceeds MaxArrayLength still fails bounded: the guard sums `pos + needed`
        // in Long, so `pos` (here a few bytes already written) plus a `needed` of MaxArrayLength overflows the cap and trips the guard,
        // with no negative length and no allocation. This pins that the cap is on the sum (the real array size after the write), not on
        // `needed` alone.
        ...
        // Test 21: a legitimate large-but-safe growth (just over the previous power-of-two boundary, well under the cap) still succeeds
        // and preserves prior data. This pins that the overflow guard does not reject ordinary growth: it only caps the unrepresentable
        // case.
