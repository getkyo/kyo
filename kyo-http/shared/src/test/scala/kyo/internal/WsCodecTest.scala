...
class WsCodecTest extends kyo.Test:
    ...
    "writeFrame → readFrame roundtrip" - {
        "text unmasked" in run {
            ...
            WsCodec.writeFrame(writeConn, WebSocket.Payload.Text("hello"), mask = false).andThen {
                val readConn = new MockConn(writeConn.written)
                Abort.run[Closed](WsCodec.readFrame(readConn.read, readConn).map(_._1)).map { result =>
                    result match
                        case Result.Success(WebSocket.Payload.Text(text)) =>
                            assert(text == "hello")
                        case other =>
        ...
        "binary masked" in run {
            ...
            val writeConn = new MockConn(Array.empty[Byte])
            WsCodec.writeFrame(writeConn, WebSocket.Payload.Binary(data), mask = true).andThen {
                val readConn = new MockConn(writeConn.written)
                Abort.run[Closed](WsCodec.readFrame(readConn.read, readConn).map(_._1)).map { result =>
                    result match
                        case Result.Success(WebSocket.Payload.Binary(received)) =>
    ...
    "acceptUpgrade" - {
        "writes 101 with correct accept key" in run {
            ...
            WsCodec.acceptUpgrade(conn, headers, WebSocket.Config()).andThen {
        ...
        "fails on missing key" in run {
            ...
            Abort.run[HttpException] {
                WsCodec.acceptUpgrade(conn, HttpHeaders.empty, WebSocket.Config())
            }.map { result =>
                assert(result.isFailure)
            }
        }
    ...
    "requestUpgrade" - {
        "writes correct upgrade request" in run {
            ...
            val conn = new MockConn(Array.empty[Byte])
            Abort.run[HttpException] {
                WsCodec.requestUpgrade(conn, "localhost", "/ws", HttpHeaders.empty, WebSocket.Config())
            }.map { _ =>
