...
class SseDecoderTest extends Test:
    ...
    /** Helper to unwrap Result from decoder */
    private def unwrap[V](result: Result[HttpError.ParseError, Seq[HttpEvent[V]]]): Seq[HttpEvent[V]] =
        result match
            case Result.Success(v) => v
            case Result.Failure(e) => throw new AssertionError(s"Expected success but got failure: $e")
            case _                 => throw new AssertionError("Unexpected result")

    "SseDecoder" - {

        "parses LF-delimited events" in {
            val decoder = SseDecoder.init[Msg](Schema[Msg])
            val input   = "data: {\"text\":\"hello\"}\n\ndata: {\"text\":\"world\"}\n\n"
            val events  = unwrap(decoder.decode(Span.fromUnsafe(input.getBytes("UTF-8"))))
        ...
        "parses event with event and id fields" in {
            val decoder = SseDecoder.init[String](Schema[String])
        ...
        "handles partial events across chunks" in {
            ...
            val part1   = "data: {\"text\":\""
            val part2   = "hello\"}\n\n"
            val events1 = unwrap(decoder.decode(Span.fromUnsafe(part1.getBytes("UTF-8"))))
            assert(events1.isEmpty)
            val events2 = unwrap(decoder.decode(Span.fromUnsafe(part2.getBytes("UTF-8"))))
        ...
        "empty chunk returns no events" in {
            val decoder = SseDecoder.init[Msg](Schema[Msg])
            val events  = unwrap(decoder.decode(Span.fromUnsafe(Array.empty[Byte])))
        ...
        "multi-byte UTF-8 split across chunk boundary" in {
            ...
            val events1 = unwrap(decoder.decode(Span.fromUnsafe(chunk1)))
            assert(events1.isEmpty)
            val events2 = unwrap(decoder.decode(Span.fromUnsafe(chunk2)))
            assert(events2.size == 1)
        ...
        "many small events in a single chunk" in {
            ...
            val events = unwrap(decoder.decode(Span.fromUnsafe(sb.toString.getBytes("UTF-8"))))
