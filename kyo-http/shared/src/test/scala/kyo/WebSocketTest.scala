...
class WebSocketTest extends Test:
    ...
    "basic connectivity" - {
        ...
        "text echo" in run {
            withWsServer(HttpHandler.webSocket("ws/echo")(echo)) { url =>
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                    ws.put(WebSocket.Payload.Text("hello")).andThen {
                        ws.take().map(f => discard(assert(f == WebSocket.Payload.Text("hello"))))
                    }
                }
            }.andThen(succeed)
        }
        ...
        "binary echo" in run {
            ...
            withWsServer(HttpHandler.webSocket("ws/echo")(echo)) { url =>
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                    ws.put(WebSocket.Payload.Binary(bytes)).andThen {
                        ws.take().map {
        ...
        "multiple messages in order" in run {
            withWsServer(HttpHandler.webSocket("ws/echo")(echo)) { url =>
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                    Kyo.foreach(1 to 10)(i => ws.put(WebSocket.Payload.Text(s"msg$i"))).andThen {
                        Kyo.foreach(1 to 10)(i =>
                            ws.take().map(f => discard(assert(f == WebSocket.Payload.Text(s"msg$i"))))
                        ).unit
                    }
                }
            }.andThen(succeed)
        ...
        "empty text frame" in run {
            withWsServer(HttpHandler.webSocket("ws/echo")(echo)) { url =>
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                    ws.put(WebSocket.Payload.Text("")).andThen {
                        ws.take().map(f => discard(assert(f == WebSocket.Payload.Text(""))))
                    }
                }
            }.andThen(succeed)
        }
        ...
        "large text frame" in run {
            ...
            withWsServer(HttpHandler.webSocket("ws/echo")(echo)) { url =>
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                    ws.put(WebSocket.Payload.Text(text)).andThen {
                        ws.take().map(f => discard(assert(f == WebSocket.Payload.Text(text))))
                    }
                }
            }.andThen(succeed)
        }
        ...
        "large binary frame" in run {
            ...
            withWsServer(HttpHandler.webSocket("ws/echo")(echo)) { url =>
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                    ws.put(WebSocket.Payload.Binary(data)).andThen {
                        ws.take().map {
                            case WebSocket.Payload.Binary(d) => discard(assert(d.size == 65000))
                            case other                       => discard(fail(s"Expected Binary, got $other"))
                        }
                    }
                }
        ...
        "interleaved text and binary" in run {
            withWsServer(HttpHandler.webSocket("ws/echo")(echo)) { url =>
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                    ws.put(WebSocket.Payload.Text("a"))
                        .andThen(ws.put(WebSocket.Payload.Binary(Span.fromUnsafe(Array[Byte](1)))))
                        .andThen(ws.put(WebSocket.Payload.Text("b")))
                        .andThen(ws.take().map(f => discard(assert(f == WebSocket.Payload.Text("a")))))
                        .andThen(ws.take().map {
                            case WebSocket.Payload.Binary(d) => discard(assert(d(0) == 1.toByte))
                            case other                       => discard(fail(s"Expected Binary, got $other"))
                        })
                        .andThen(ws.take().map(f => discard(assert(f == WebSocket.Payload.Text("b")))))
                }
            }.andThen(succeed)
        }
        ...
        "binary all byte values" in run {
            ...
            withWsServer(HttpHandler.webSocket("ws/echo")(echo)) { url =>
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                    ws.put(WebSocket.Payload.Binary(allBytes)).andThen {
                        ws.take().map {
                            case WebSocket.Payload.Binary(d) =>
        ...
        "single byte binary" in run {
            withWsServer(HttpHandler.webSocket("ws/echo")(echo)) { url =>
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                    ws.put(WebSocket.Payload.Binary(Span.fromUnsafe(Array[Byte](42)))).andThen {
                        ws.take().map {
                            case WebSocket.Payload.Binary(d) =>
        ...
        "single char text" in run {
            withWsServer(HttpHandler.webSocket("ws/echo")(echo)) { url =>
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                    ws.put(WebSocket.Payload.Text("x")).andThen {
                        ws.take().map(f => discard(assert(f == WebSocket.Payload.Text("x"))))
                    }
                }
            }.andThen(succeed)
        }
    ...
    "close handshake" - {
        ...
        "client initiates close" in run {
            withWsServer(HttpHandler.webSocket("ws/echo")(echo)) { url =>
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                    ws.put(WebSocket.Payload.Text("hi")).andThen(ws.take()).andThen(ws.close())
                }
            }.andThen(succeed)
        }
        ...
        "server initiates close" in run {
            withWsServer(HttpHandler.webSocket("ws/close") { (_, ws) =>
                ...
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/close") { ws =>
                    ws.put(WebSocket.Payload.Text("trigger")).andThen {
        ...
        "close after exchange" in run {
            withWsServer(HttpHandler.webSocket("ws/echo")(echo)) { url =>
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                    ws.put(WebSocket.Payload.Text("a")).andThen(ws.take())
                        .andThen(ws.put(WebSocket.Payload.Text("b"))).andThen(ws.take())
                        .andThen(ws.close())
                }
            }.andThen(succeed)
        }
    ...
    "backpressure" - {
        ...
        "bidirectional concurrent exchange" in run {
            withWsServer(HttpHandler.webSocket("ws/echo")(echo)) { url =>
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                    ...
                    Abort.run[Closed] {
                        Async.gather(
                            Kyo.foreach(1 to count)(i => ws.put(WebSocket.Payload.Text(s"msg$i"))).unit,
        ...
        "handler that only sends" in run {
            withWsServer(HttpHandler.webSocket("ws/send-only") { (_, ws) =>
                Kyo.foreach(1 to 5)(i => ws.put(WebSocket.Payload.Text(s"server$i"))).andThen {
                    Abort.run[Closed](ws.take()).unit
                }
            }) { url =>
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/send-only") { ws =>
                    Kyo.foreach(1 to 5)(i =>
                        ws.take().map(f => discard(assert(f == WebSocket.Payload.Text(s"server$i"))))
                    ).unit
                }
            }.andThen(succeed)
        }
        ...
        "handler that only reads" in run {
            withWsServer(HttpHandler.webSocket("ws/read-only") { (_, ws) =>
                ...
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/read-only") { ws =>
                    Kyo.foreach(1 to 3)(i => ws.put(WebSocket.Payload.Text(s"msg$i"))).andThen {
    ...
    "disconnect scenarios" - {
        ...
        "multiple client connections" in run {
            withWsServer(HttpHandler.webSocket("ws/echo")(echo)) { url =>
                Async.gather(
                    HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                        ws.put(WebSocket.Payload.Text("a")).andThen(ws.take()).map(f => discard(assert(f == WebSocket.Payload.Text("a"))))
                    },
                    HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                        ws.put(WebSocket.Payload.Text("b")).andThen(ws.take()).map(f => discard(assert(f == WebSocket.Payload.Text("b"))))
                    },
                    HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                        ws.put(WebSocket.Payload.Text("c")).andThen(ws.take()).map(f => discard(assert(f == WebSocket.Payload.Text("c"))))
                    }
                ).unit
            }.andThen(succeed)
        }
        ...
        "5 concurrent clients" in run {
            withWsServer(HttpHandler.webSocket("ws/echo")(echo)) { url =>
                Async.foreach(1 to 5) { i =>
                    HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                        ws.put(WebSocket.Payload.Text(s"client$i")).andThen(ws.take()).map(f =>
                            discard(assert(f == WebSocket.Payload.Text(s"client$i")))
                        )
                    }
                }.unit
            }.andThen(succeed)
        ...
        "rapid connect disconnect cycles" in run {
            withWsServer(HttpHandler.webSocket("ws/echo")(echo)) { url =>
                Kyo.foreach(1 to 10) { i =>
                    Abort.run[HttpException] {
                        HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                            ws.put(WebSocket.Payload.Text(s"cycle$i")).andThen {
                                ws.take().map(f => discard(assert(f == WebSocket.Payload.Text(s"cycle$i"))))
                            }
                        }
                    }.unit
                }.unit
            ...
            Latch.init(1).map { serverDone =>
                withWsServer(HttpHandler.webSocket("ws/infinite") { (_, ws) =>
                    Sync.ensure(serverDone.release) {
                        Loop.foreach {
                            ws.put(WebSocket.Payload.Text("tick")).andThen(Loop.continue)
                        }
                    }
                }) { url =>
        ...
        "client sends then immediately closes" in run {
            withWsServer(HttpHandler.webSocket("ws/echo")(echo)) { url =>
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                    ws.put(WebSocket.Payload.Text("bye")).andThen(ws.close())
                }
            }.andThen(succeed)
        }
    }
    ...
    "HTTP integration" - {
        ...
        "websocket alongside http handlers" in run {
            ...
            withWsServer(httpHandler, HttpHandler.webSocket("ws/echo")(echo)) { url =>
                HttpClient.getText(s"${url.scheme.getOrElse("http")}://${url.host}:${url.port}/api/hello").map(text =>
                    ...
                    HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                        ws.put(WebSocket.Payload.Text("test")).andThen {
                            ws.take().map(f => discard(assert(f == WebSocket.Payload.Text("test"))))
                        }
                    }
                }
            }.andThen(succeed)
        ...
        "http after websocket on same server" in run {
            ...
            withWsServer(httpHandler, HttpHandler.webSocket("ws/echo")(echo)) { url =>
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                    ws.put(WebSocket.Payload.Text("a")).andThen(ws.take()).unit
                }.andThen {
        ...
        "multiple http and ws requests interleaved" in run {
            ...
            withWsServer(httpHandler, HttpHandler.webSocket("ws/echo")(echo)) { url =>
                HttpClient.getText(s"${url.scheme.getOrElse("http")}://${url.host}:${url.port}/api/count").andThen {
                    HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                        ws.put(WebSocket.Payload.Text("1")).andThen(ws.take()).unit
                    }
                }.andThen {
                    HttpClient.getText(s"${url.scheme.getOrElse("http")}://${url.host}:${url.port}/api/count")
                }.andThen {
                    HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                        ws.put(WebSocket.Payload.Text("2")).andThen(ws.take()).unit
                    }
                }
            }.andThen(succeed)
        }
        ...
        "path routing" in run {
            withWsServer(
                ...
                HttpHandler.webSocket("ws/echo2") { (_, ws) =>
                    ws.stream.map {
                        case WebSocket.Payload.Text(data) => WebSocket.Payload.Text(s"echo2:$data")
                        case other                        => other
                    }.foreach(ws.put)
                }
            ) { url =>
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo1") { ws =>
                    ws.put(WebSocket.Payload.Text("a")).andThen(ws.take()).map(f =>
                        discard(assert(f == WebSocket.Payload.Text("a")))
                    )
                }.andThen {
                    HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo2") { ws =>
                        ws.put(WebSocket.Payload.Text("b")).andThen(ws.take()).map(f =>
                            discard(assert(f == WebSocket.Payload.Text("echo2:b")))
                        )
                    }
                }
            }.andThen(succeed)
        ...
        "three different ws endpoints" in run {
            withWsServer(
                HttpHandler.webSocket("ws/a") { (_, ws) =>
                    ws.take().andThen(ws.put(WebSocket.Payload.Text("from-a")))
                        .andThen(Abort.run[Closed](ws.take()).unit)
                },
                HttpHandler.webSocket("ws/b") { (_, ws) =>
                    ws.take().andThen(ws.put(WebSocket.Payload.Text("from-b")))
                        .andThen(Abort.run[Closed](ws.take()).unit)
                },
                HttpHandler.webSocket("ws/c") { (_, ws) =>
                    ws.take().andThen(ws.put(WebSocket.Payload.Text("from-c")))
                        .andThen(Abort.run[Closed](ws.take()).unit)
                }
            ) { url =>
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/a") { ws =>
                    ws.put(WebSocket.Payload.Text("x")).andThen(ws.take()).map(f =>
                        discard(assert(f == WebSocket.Payload.Text("from-a")))
                    )
                }.andThen {
                    HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/b") { ws =>
                        ws.put(WebSocket.Payload.Text("x")).andThen(ws.take()).map(f =>
                            discard(assert(f == WebSocket.Payload.Text("from-b")))
                        )
                    }
                }.andThen {
                    HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/c") { ws =>
                        ws.put(WebSocket.Payload.Text("x")).andThen(ws.take()).map(f =>
                            discard(assert(f == WebSocket.Payload.Text("from-c")))
                        )
                    }
                }
            }.andThen(succeed)
        ...
        "request headers accessible" in run {
            withWsServer(HttpHandler.webSocket("ws/auth") { (req, ws) =>
                ...
                ws.put(WebSocket.Payload.Text(auth)).andThen {
                    Abort.run[Closed](ws.take()).unit
                }
            }) { url =>
                HttpClient.webSocket(
                    ...
                    ws.take().map(f => discard(assert(f == WebSocket.Payload.Text("Bearer token123"))))
                }
            }.andThen(succeed)
        }
    }
    ...
    "server handler patterns" - {
        ...
        "handler transforms messages" in run {
            withWsServer(HttpHandler.webSocket("ws/upper") { (_, ws) =>
                ws.stream.map {
                    case WebSocket.Payload.Text(data) => WebSocket.Payload.Text(data.toUpperCase)
                    case other                        => other
                }.foreach(ws.put)
            }) { url =>
        ...
        "handler sends greeting then echoes" in run {
            withWsServer(HttpHandler.webSocket("ws/greet") { (_, ws) =>
                ws.put(WebSocket.Payload.Text("welcome")).andThen {
                    ws.stream.foreach(ws.put)
                }
            }) { url =>
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/greet") { ws =>
                    ws.take().map(f => discard(assert(f == WebSocket.Payload.Text("welcome")))).andThen {
                        ws.put(WebSocket.Payload.Text("hi")).andThen {
                            ws.take().map(f => discard(assert(f == WebSocket.Payload.Text("hi"))))
                        }
                    }
                }
            }.andThen(succeed)
        ...
        "handler responds with count" in run {
            withWsServer(HttpHandler.webSocket("ws/count") { (_, ws) =>
                AtomicRef.init(0).map { counter =>
                    ws.stream.foreach { _ =>
                        counter.getAndUpdate(_ + 1).map { n =>
                            ws.put(WebSocket.Payload.Text(s"count:$n"))
                        }
                    }
                }
            }) { url =>
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/count") { ws =>
                    ws.put(WebSocket.Payload.Text("a")).andThen(ws.take()).map(f =>
                        discard(assert(f == WebSocket.Payload.Text("count:0")))
                    ).andThen {
                        ws.put(WebSocket.Payload.Text("b")).andThen(ws.take()).map(f =>
                            discard(assert(f == WebSocket.Payload.Text("count:1")))
                        )
                    }.andThen {
                        ws.put(WebSocket.Payload.Text("c")).andThen(ws.take()).map(f =>
                            discard(assert(f == WebSocket.Payload.Text("count:2")))
                        )
                    }
                }
            }.andThen(succeed)
    ...
    "server lifecycle" - {
        ...
        "server handles connection after previous ws closed" in run {
            withWsServer(HttpHandler.webSocket("ws/echo")(echo)) { url =>
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                    ws.put(WebSocket.Payload.Text("first")).andThen(ws.take()).map(f =>
                        discard(assert(f == WebSocket.Payload.Text("first")))
                    )
                }.andThen {
                    HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                        ws.put(WebSocket.Payload.Text("second")).andThen(ws.take()).map(f =>
                            discard(assert(f == WebSocket.Payload.Text("second")))
                        )
                    }
                }
            }.andThen(succeed)
        ...
        "three sequential connections" in run {
            withWsServer(HttpHandler.webSocket("ws/echo")(echo)) { url =>
                Kyo.foreach(1 to 3) { i =>
                    HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                        ws.put(WebSocket.Payload.Text(s"conn$i")).andThen(ws.take()).map(f =>
                            discard(assert(f == WebSocket.Payload.Text(s"conn$i")))
                        )
                    }
                }.unit
            }.andThen(succeed)
        ...
        "sequential then concurrent" in run {
            withWsServer(HttpHandler.webSocket("ws/echo")(echo)) { url =>
                // Sequential first
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                    ws.put(WebSocket.Payload.Text("seq")).andThen(ws.take()).unit
                }.andThen {
                    // Then concurrent
                    Async.gather(
                        HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                            ws.put(WebSocket.Payload.Text("par1")).andThen(ws.take()).unit
                        },
                        HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                            ws.put(WebSocket.Payload.Text("par2")).andThen(ws.take()).unit
                        }
                    ).unit
                }
            }.andThen(succeed)
    ...
    "edge cases" - {
        ...
        "server sends immediately on connect" in run {
            withWsServer(HttpHandler.webSocket("ws/eager") { (_, ws) =>
                ...
                ws.put(WebSocket.Payload.Text("eager1"))
                    .andThen(ws.put(WebSocket.Payload.Text("eager2")))
                    .andThen(ws.put(WebSocket.Payload.Text("eager3")))
                    .andThen(Abort.run[Closed](ws.take()).unit) // wait for client
            }) { url =>
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/eager") { ws =>
                    ws.take().map(f => discard(assert(f == WebSocket.Payload.Text("eager1"))))
                        .andThen(ws.take().map(f => discard(assert(f == WebSocket.Payload.Text("eager2")))))
                        .andThen(ws.take().map(f => discard(assert(f == WebSocket.Payload.Text("eager3")))))
                }
            }.andThen(succeed)
        }
            ...
            Latch.init(1).map { cleanupDone =>
                withWsServer(HttpHandler.webSocket("ws/cleanup") { (_, ws) =>
                    ...
                    HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/cleanup") { ws =>
                        ws.put(WebSocket.Payload.Text("hi")).andThen(ws.take()).unit
                    }.andThen {
        ...
        "messages before close are delivered" in run {
            withWsServer(HttpHandler.webSocket("ws/send-then-close") { (_, ws) =>
                ws.put(WebSocket.Payload.Text("msg1"))
                    .andThen(ws.put(WebSocket.Payload.Text("msg2")))
                    .andThen(ws.put(WebSocket.Payload.Text("msg3")))
                ...
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/send-then-close") { ws =>
                    ws.take().map(f => discard(assert(f == WebSocket.Payload.Text("msg1"))))
                        .andThen(ws.take().map(f => discard(assert(f == WebSocket.Payload.Text("msg2")))))
                        .andThen(ws.take().map(f => discard(assert(f == WebSocket.Payload.Text("msg3")))))
                }
            }.andThen(succeed)
        }
        ...
        "10 concurrent connections" in run {
            withWsServer(HttpHandler.webSocket("ws/echo")(echo)) { url =>
                Async.foreach(1 to 10) { i =>
                    HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                        ws.put(WebSocket.Payload.Text(s"c$i")).andThen(ws.take()).map(f =>
                            discard(assert(f == WebSocket.Payload.Text(s"c$i")))
                        )
                    }
                }.unit
            }.andThen(succeed)
                ...
                Async.foreach(1 to 10) { i =>
                    HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                        Kyo.foreach(1 to 5) { j =>
                            ws.put(WebSocket.Payload.Text(s"c${i}m$j")).andThen(ws.take()).map(f =>
                                discard(assert(f == WebSocket.Payload.Text(s"c${i}m$j")))
                            )
                        }.unit
                    }
                }.unit
        ...
        "binary echo preserves exact bytes" in run {
            ...
            withWsServer(HttpHandler.webSocket("ws/echo")(echo)) { url =>
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                    ws.put(WebSocket.Payload.Binary(data1))
                        .andThen(ws.put(WebSocket.Payload.Binary(data2)))
                        .andThen(ws.put(WebSocket.Payload.Binary(data3)))
                        .andThen(ws.take().map {
                            case WebSocket.Payload.Binary(d) =>
            ...
            withWsServer(httpHandler, HttpHandler.webSocket("ws/echo")(echo)) { url =>
                Async.foreach(1 to 5) { i =>
                    if i % 2 == 0 then
                        ...
                        HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/echo") { ws =>
                            ws.put(WebSocket.Payload.Text(s"ws$i")).andThen(ws.take()).map(f =>
                                discard(assert(f == WebSocket.Payload.Text(s"ws$i")))
                            )
                        }
                }.unit
            }.andThen(succeed)
        ...
        "server processes message with client ack" in run {
            withWsServer(HttpHandler.webSocket("ws/ack") { (_, ws) =>
                ws.take().andThen(ws.put(WebSocket.Payload.Text("ack")))
                    .andThen(Abort.run[Closed](ws.take()).unit)
            }) { url =>
                HttpClient.webSocket(s"ws://${url.host}:${url.port}/ws/ack") { ws =>
                    ws.put(WebSocket.Payload.Text("msg")).andThen {
                        ws.take().map(f => discard(assert(f == WebSocket.Payload.Text("ack"))))
                    }
                }
            }.andThen(succeed)
        }
