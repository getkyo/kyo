...
class Rfc7578Test extends BaseHttpTest:
    ...
    "Section 4.2 - Multiple parts with different content types".notNative in {
        ...
        withServer(ep) { port =>
            val parts = Seq(
                HttpRequest.Part("text", Absent, Present("text/plain"), Span.fromUnsafe("hello".getBytes("UTF-8"))),
                HttpRequest.Part("json", Absent, Present("application/json"), Span.fromUnsafe("{\"a\":1}".getBytes("UTF-8"))),
                HttpRequest.Part("img", Present("photo.jpg"), Present("image/jpeg"), Span.fromUnsafe(Array[Byte](1, 2, 3)))
            )
            send(
                port,
                route,
