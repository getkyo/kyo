    ...
    "WebSocket.connect" - {
            ...
            connectTest(
                ...
                ws =>
                    ws.put(WebSocket.Payload.Text("hello")).andThen(
                        ws.take().map(frame => discard(assert(frame == WebSocket.Payload.Text("hello"))))
                    )
            ).andThen(succeed)
        }
                ...
                ws =>
                    Kyo.foreach(1 to 10)(i => ws.put(WebSocket.Payload.Text(s"msg$i"))).andThen(
                        Kyo.foreach(1 to 10)(i =>
                            ws.take().map(frame => discard(assert(frame == WebSocket.Payload.Text(s"msg$i"))))
                        ).unit
                    )
            ).andThen(succeed)
        }
            ...
            connectTest(
                ...
                ws =>
                    ws.put(WebSocket.Payload.Text("")).andThen(
                        ws.take().map(frame => discard(assert(frame == WebSocket.Payload.Text(""))))
                    )
            ).andThen(succeed)
        }
        ...
        "empty binary" in run {
            connectTest(
                echo,
                ws =>
                    ws.put(WebSocket.Payload.Binary(Span.empty[Byte])).andThen(
                        ws.take().map {
                            case WebSocket.Payload.Binary(data) => discard(assert(data.size == 0))
                            case other                          => discard(fail(s"Expected Binary, got $other"))
                        }
                    )
            ).andThen(succeed)
            ...
            connectTest(
                ...
                ws =>
                    ws.put(WebSocket.Payload.Text(text)).andThen(
                        ws.take().map(frame => discard(assert(frame == WebSocket.Payload.Text(text))))
                    )
            ).andThen(succeed)
        }
        ...
        "binary with all byte values" in run {
            ...
            connectTest(
                echo,
                ws =>
                    ws.put(WebSocket.Payload.Binary(Span.fromUnsafe(allBytes))).andThen(
                        ws.take().map {
                            case WebSocket.Payload.Binary(data) =>
        ...
        "close reason preserved" in run {
            ...
            closeReasonRef.map { ref =>
                ...
                Channel.init[WebSocket.Payload](32).map { ch1to2 =>
                    Channel.init[WebSocket.Payload](32).map { ch2to1 =>
        ...
        "put after close" in run {
            connectTest(
                ...
                ws =>
                    ws.close().andThen(
                        Abort.run[Closed](ws.put(WebSocket.Payload.Text("fail"))).map(result =>
                            discard(assert(result.isFailure || result.isPanic))
                        )
                    )
            ).andThen(succeed)
        ...
        "concurrent bidirectional" in run {
            connectTest(
                ws =>
                    Async.gather(
                        Kyo.foreach(1 to 5)(i => ws.put(WebSocket.Payload.Text(s"a$i"))).unit,
                        Kyo.foreach(1 to 5)(_ => Abort.run[Closed](ws.take())).unit
                    ).unit,
                ws =>
                    Async.gather(
                        Kyo.foreach(1 to 5)(i => ws.put(WebSocket.Payload.Text(s"b$i"))).unit,
                        Kyo.foreach(1 to 5)(_ => Abort.run[Closed](ws.take())).unit
                    ).unit
            ).andThen(succeed)
        }
        ...
        "many small messages" in run {
            ...
            connectTest(
                ...
                ws =>
                    ...
                    val sender = Kyo.foreach(1 to count)(i => ws.put(WebSocket.Payload.Text(i.toString)))
                    val receiver = Kyo.foreach(1 to count)(i =>
                        ws.take().map(f => discard(assert(f == WebSocket.Payload.Text(i.toString))))
                    )
