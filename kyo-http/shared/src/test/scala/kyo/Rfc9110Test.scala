...
class Rfc9110Test extends BaseHttpTest:
    ...
    "Section 9.3.2 - HEAD response MUST NOT contain a message body" in {
    ...
    "Section 9.3.2 - HEAD response Content-Type MUST match GET" in {
    ...
    "Section 9.3.2 - HEAD response Content-Length MUST match GET body size" in {
    ...
    "Section 9.3.7 - OPTIONS MUST include Allow header" in {
    ...
    "Section 9.3.7 - OPTIONS Allow lists HEAD when GET registered" in {
    ...
    "Section 9.3.4 - POST success with 201 and Location header" in {
    ...
    "Section 9.3.5 - DELETE returning 202 Accepted" in {
    ...
    "Section 9.3.5 - DELETE returning 204 No Content" in {
    ...
    "Section 9.3.5 - DELETE returning 200 OK with status body" in {
    ...
    "Section 15.5.6 - 405 MUST include Allow header" in {
        val getRoute = HttpRoute.getRaw("resource").response(_.bodyText)
    ...
    "Section 15.5.6 - 405 Allow includes HEAD when GET registered" in {
        val route = HttpRoute.getRaw("resource").response(_.bodyText)
        val ep    = route.handler(_ => HttpResponse.ok("ok"))
        withServer(ep) { port =>
            send(port, rawRoute, HttpRequest.deleteRaw(HttpUrl.fromUri("/resource"))).map { resp =>
    ...
    "Section 15.5.6 - 405 Allow includes OPTIONS" in {
        val route = HttpRoute.getRaw("resource").response(_.bodyText)
        val ep    = route.handler(_ => HttpResponse.ok("ok"))
        withServer(ep) { port =>
            send(port, rawRoute, HttpRequest.deleteRaw(HttpUrl.fromUri("/resource"))).map { resp =>
    ...
    "Section 15.3.5 - 204 MUST NOT contain body" in {
    ...
    "Section 15.3.5 - 204 MUST NOT contain Content-Type" in {
        val route = HttpRoute.getRaw("empty").response(_.bodyText)
        val ep    = route.handler(_ => HttpResponse.halt(HttpResponse.noContent))
        withServer(ep) { port =>
            send(port, rawRoute, HttpRequest.getRaw(HttpUrl.fromUri("/empty"))).map { resp =>
    ...
    "Section 15.5.16 - wrong Content-Type MUST return 415" in {
    ...
    "Section 6.6.1 - Server MUST send Date header" in {
    ...
    "Section 8.3 - Content-Type text/plain for text body" in {
    ...
    "Section 8.3 - Content-Type application/json for JSON body" in {
    ...
    "Section 8.6 - Content-Length present for buffered responses" in {
        val route = HttpRoute.getRaw("sized").response(_.bodyText)
        val ep    = route.handler(_ => HttpResponse.ok("twelve chars"))
        withServer(ep) { port =>
    ...
    "Section 15.6.1 - 500 error MUST NOT leak internal details" in {
    ...
    "Section 15.4.4 - 303 See Other MUST change method to GET" in {
    ...
    "Section 15.4.8 - 307 MUST preserve method and body" in {
    ...
    "Section 15.4.9 - 308 MUST preserve method and body" in {
    ...
    "Section 15.4 - Redirect loop hits maxRedirects" in {
    ...
    "Section 15.4 - Relative Location header resolved against request" in {
    ...
    "Section 15.3.1 - 413 response body is readable" in {
    ...
    "Section 9.3.1 - GET returns 200 with body" in {
    ...
    "Section 9.3.1 - GET with query parameters" in {
    ...
    "Section 9.3.2 - HEAD on non-existent resource returns 404" in {
    ...
    "Section 9.3.4 - POST with JSON body round-trip" in {
    ...
    "Section 9.3.6 - PUT returning 200 OK with body" in {
    ...
    "Section 9.3.6 - PUT returning 201 Created" in {
    ...
    "Section 9.3.7 - OPTIONS Allow lists all registered methods" in {
    ...
    "Section 9.3.7 - OPTIONS response has no body" in {
    ...
    "Section 9.3.3 - PATCH with body" in {
    ...
    "Section 15.4 - Client followRedirects=false stops redirect" in {
    ...
    "Section 15.4 - maxRedirects=0 prevents any redirect" in {
    ...
    "Section 15.5.5 - 404 Not Found for unknown path" in {
        val route = HttpRoute.getRaw("exists").response(_.bodyText)
        val ep    = route.handler(_ => HttpResponse.ok("ok"))
        withServer(ep) { port =>
    ...
    "Section 15.5.6 - 405 Allow lists all registered methods for path" in {
    ...
    "Section 8.3 - Content-Type application/octet-stream for binary body" in {
    ...
    "Section 6.6.1 - Date header present on error responses" in {
    ...
    "Section 8.6 - Content-Length is 0 for empty body" in {
    ...
    "Section 8.6 - Content-Length matches actual bytes for UTF-8" in {
    ...
    "Section 15.6.1 - 500 response has valid Content-Type" in {
    ...
    "Section 15.5.16 - text Content-Type to JSON endpoint returns 415" in {
    ...
    "Section 15.5.1 - 400 for malformed path param" in {
    ...
    "Section 15.2.1 - 200 OK response with JSON body" in {
    ...
    "Section 15.2.2 - 201 Created with Location header" in {
    ...
    "Section 15.2.3 - 202 Accepted for async processing" in {
    ...
    "Section 15.5.4 - 403 Forbidden response" in {
    ...
    "Section 15.5.10 - 409 Conflict response" in {
    ...
    "Section 15.6.4 - 503 Service Unavailable response" in {
    ...
    "Section 15.4.4 - 303 from DELETE changes to GET" in {
    ...
    "Section 15.4 - Redirect with absolute URL in Location" in {
