...
/** RFC 9110 — HTTP Semantics compliance tests.
  ...
  */
class Rfc9110Test extends BaseHttpTest:
    ...
    "Section 9.3.2 - HEAD response MUST NOT contain a message body".notNative in {
    ...
    "Section 9.3.2 - HEAD response Content-Type MUST match GET".notNative in {
    ...
    "Section 9.3.2 - HEAD response Content-Length MUST match GET body size".notNative in {
    ...
    "Section 9.3.7 - OPTIONS MUST include Allow header".notNative in {
    ...
    "Section 9.3.7 - OPTIONS Allow lists HEAD when GET registered".notNative in {
    ...
    "Section 9.3.4 - POST success with 201 and Location header".notNative in {
    ...
    "Section 9.3.5 - DELETE returning 202 Accepted".notNative in {
    ...
    "Section 9.3.5 - DELETE returning 204 No Content".notNative in {
    ...
    "Section 9.3.5 - DELETE returning 200 OK with status body".notNative in {
    ...
    "Section 15.5.6 - 405 MUST include Allow header".notNative in {
        val getRoute = HttpRoute.getRaw("resource").response(_.bodyText)
    ...
    "Section 15.5.6 - 405 Allow includes HEAD when GET registered".notNative in {
        val route = HttpRoute.getRaw("resource").response(_.bodyText)
        val ep    = route.handler(_ => HttpResponse.ok("ok"))
        withServer(ep) { port =>
            send(port, rawRoute, HttpRequest.deleteRaw(HttpUrl.fromUri("/resource"))).map { resp =>
    ...
    "Section 15.5.6 - 405 Allow includes OPTIONS".notNative in {
        val route = HttpRoute.getRaw("resource").response(_.bodyText)
        val ep    = route.handler(_ => HttpResponse.ok("ok"))
        withServer(ep) { port =>
            send(port, rawRoute, HttpRequest.deleteRaw(HttpUrl.fromUri("/resource"))).map { resp =>
    ...
    "Section 15.3.5 - 204 MUST NOT contain body".notNative in {
    ...
    "Section 15.3.5 - 204 MUST NOT contain Content-Type".notNative in {
        val route = HttpRoute.getRaw("empty").response(_.bodyText)
        val ep    = route.handler(_ => HttpResponse.halt(HttpResponse.noContent))
        withServer(ep) { port =>
            send(port, rawRoute, HttpRequest.getRaw(HttpUrl.fromUri("/empty"))).map { resp =>
    ...
    "Section 15.4.5 - 304 MUST NOT contain body".notNative in {
    ...
    "Section 15.4.5 - 304 preserves ETag and Cache-Control".notNative in {
        val route = HttpRoute.getRaw("cached").response(_.bodyText)
        val ep = route.handler(_ =>
            HttpResponse.halt(HttpResponse.notModified.etag("\"abc123\"").cacheControl("max-age=3600"))
        )
    ...
    "Section 15.5.16 - wrong Content-Type MUST return 415".notNative in {
    ...
    "Section 6.6.1 - Server MUST send Date header".notNative in {
    ...
    "Section 8.3 - Content-Type text/plain for text body".notNative in {
    ...
    "Section 8.3 - Content-Type application/json for JSON body".notNative in {
    ...
    "Section 8.6 - Content-Length present for buffered responses".notNative in {
        val route = HttpRoute.getRaw("sized").response(_.bodyText)
        val ep    = route.handler(_ => HttpResponse.ok("twelve chars"))
        withServer(ep) { port =>
    ...
    "Section 15.6.1 - 500 error MUST NOT leak internal details".notNative in {
    ...
    "Section 15.4.4 - 303 See Other MUST change method to GET".notNative in {
    ...
    "Section 15.4.8 - 307 MUST preserve method and body".notNative in {
    ...
    "Section 15.4.9 - 308 MUST preserve method and body".notNative in {
    ...
    "Section 15.4 - Redirect loop hits maxRedirects".notNative in {
    ...
    "Section 15.4 - Relative Location header resolved against request".notNative in {
    ...
    "Section 15.3.1 - 413 response body is readable".notNative in {
    ...
    "Section 9.3.1 - GET returns 200 with body".notNative in {
    ...
    "Section 9.3.1 - GET with query parameters".notNative in {
    ...
    "Section 9.3.2 - HEAD on non-existent resource returns 404".notNative in {
    ...
    "Section 9.3.4 - POST with JSON body round-trip".notNative in {
    ...
    "Section 9.3.6 - PUT returning 200 OK with body".notNative in {
    ...
    "Section 9.3.6 - PUT returning 201 Created".notNative in {
    ...
    "Section 9.3.7 - OPTIONS Allow lists all registered methods".notNative in {
    ...
    "Section 9.3.7 - OPTIONS response has no body".notNative in {
    ...
    "Section 9.3.3 - PATCH with body".notNative in {
    ...
    "Section 15.4.2 - 301 Moved Permanently redirect followed".notNative in {
    ...
    "Section 15.4.3 - 302 Found redirect followed".notNative in {
    ...
    "Section 15.4 - Client followRedirects=false stops redirect".notNative in {
    ...
    "Section 15.4 - maxRedirects=0 prevents any redirect".notNative in {
    ...
    "Section 15.5.5 - 404 Not Found for unknown path".notNative in {
        val route = HttpRoute.getRaw("exists").response(_.bodyText)
        val ep    = route.handler(_ => HttpResponse.ok("ok"))
        withServer(ep) { port =>
    ...
    "Section 15.5.6 - 405 Allow lists all registered methods for path".notNative in {
    ...
    "Section 8.3 - Content-Type application/octet-stream for binary body".notNative in {
    ...
    "Section 8.3 - Handler-set Content-Type preserved".notNative in {
    ...
    "Section 6.6.1 - Date header present on error responses".notNative in {
    ...
    "Section 8.6 - Content-Length is 0 for empty body".notNative in {
    ...
    "Section 8.6 - Content-Length matches actual bytes for UTF-8".notNative in {
    ...
    "Section 15.6.1 - 500 response has valid Content-Type".notNative in {
    ...
    "Section 15.5.16 - text Content-Type to JSON endpoint returns 415".notNative in {
    ...
    "Section 15.5.1 - 400 for malformed path param".notNative in {
    ...
    "Section 15.2.1 - 200 OK response with JSON body".notNative in {
    ...
    "Section 15.2.2 - 201 Created with Location header".notNative in {
    ...
    "Section 15.2.3 - 202 Accepted for async processing".notNative in {
    ...
    "Section 15.5.4 - 403 Forbidden response".notNative in {
    ...
    "Section 15.5.10 - 409 Conflict response".notNative in {
    ...
    "Section 15.6.4 - 503 Service Unavailable response".notNative in {
    ...
    "Section 15.4.4 - 303 from DELETE changes to GET".notNative in {
    ...
    "Section 15.4 - Redirect with absolute URL in Location".notNative in {
    ...
    "Section 8.1 - Custom response headers preserved".notNative in {
