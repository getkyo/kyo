...
class HttpRouteTest extends Test:
    ...
    "Path" - {
        ...
        "concatenation" - {
            ...
            "capture / rest" in {
                val p = "items" / HttpPath.Capture[Int]("id") / Capture.Rest("rest")
    ...
    "Query parameters" - {
        ...
        "with default value" in {
            ...
            (r.request.fields(0): @unchecked) match
    ...
    "Request body" - {
        ...
        "JSON stores field name and json" in {
    ...
    "Filter" - {
        ...
        "filter with request field requirement rejected when route lacks field" in {
            ...
            typeCheckFailure("""HttpRoute.getRaw("users").filter(f)""")(
                """Found:    (f : kyo.HttpFilter.Request[("auth" : String) ~ String, Any, Nothing])"""
            )
        }
    ...
    "Bug probes" - {
        ...
        "andThen on wildcard filter loses first filter's ReqUse constraint" in {
