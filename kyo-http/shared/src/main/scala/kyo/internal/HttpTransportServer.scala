...
class HttpTransportServer(private[kyo] val transport: Transport)(using
    ...
    private def writeResponse(conn: transport.Connection, status: HttpStatus, headers: HttpHeaders, body: HttpBody)(using
        ...
        Sync.defer {
            val dated = headers.add("Date", ZonedDateTime.now(ZoneOffset.UTC).format(httpDateFormat))
            Http1Protocol.writeResponse(conn, status, dated, body)
        }
    ...
    def bind(
        ...
        Promise.initWith[Unit, Any] { done =>
            transport.listenWith(listenAddress, config.backlog, config.tls) { listener =>
                Fiber.initUnscoped {
                    listener.connections.foreach { conn =>
                        Fiber.initUnscoped {
                            ...
                            Sync.defer {
                                ...
                                fiber.onComplete { _ =>
                                    Sync.defer(discard(connFibers.remove(fiber)))
                                }.unit
                            }
                        }
                    }
                    ...
                    new HttpBackend.Binding:
                        val address: HttpAddress = listener.address
    ...
    private def serveConnection(
        ...
        Abort.run[HttpException] {
            Loop(conn.read) { stream =>
                Http1Protocol.readRequestStreamingWith(stream, config.maxContentLength) { (method, rawPath, headers, body, remaining) =>
    ...
    private def serveWebSocket(
        ...
        Channel.initUnscopedWith[WebSocket.Payload](wsHandler.wsConfig.bufferSize) { inbound =>
            Channel.initUnscopedWith[WebSocket.Payload](wsHandler.wsConfig.bufferSize) { outbound =>
