...
object Http1Protocol:
    ...
    inline def readRequestWith[A, S2](src: Stream[Span[Byte], Async], maxSize: Int)(
        inline f: (HttpMethod, String, HttpHeaders, HttpBody, Stream[Span[Byte], Async]) => A < S2
        ...
        ByteStream.readUntil(src, ByteStream.CRLF_CRLF, MaxHeaderSize).map { case (headerBytes, remaining) =>
            ...
            if lines.isEmpty then Abort.fail(HttpProtocolException("Empty request"))
                ...
                Abort.get(parseRequestLine(lines(0))).map { case (method, path) =>
                    ...
                    readBodyWith(remaining, headers, maxSize) { (body, remaining2) =>
                        f(method, path, headers, body, remaining2)
                    }
                }
            end if
        }
    ...
    /** Read a request head and streaming chunked body for use by the server when a streaming route is matched.
      ...
      */
    inline def readRequestStreamingWith[A, S2](src: Stream[Span[Byte], Async], maxSize: Int)(
        inline f: (HttpMethod, String, HttpHeaders, HttpBody, Stream[Span[Byte], Async]) => A < S2
    )(using inline frame: Frame): A < (S2 & Async & Abort[HttpException]) =
        ByteStream.readUntil(src, ByteStream.CRLF_CRLF, MaxHeaderSize).map { case (headerBytes, remaining) =>
            val headerStr = new String(headerBytes.toArrayUnsafe, Utf8)
            ...
            if lines.isEmpty then Abort.fail(HttpProtocolException("Empty request"))
                ...
                Abort.get(parseRequestLine(lines(0))).map { case (method, path) =>
                    ...
                    val isChunked = headers.get("Transfer-Encoding") match
                        ...
                        case Absent     => false
                    if isChunked then
                        readChunkedBodyStreamingWith(remaining) { (body, remaining2) =>
                            f(method, path, headers, body, remaining2)
                        }
                    else
                        readBodyWith(remaining, headers, maxSize) { (body, remaining2) =>
    ...
    end readRequestStreamingWith

    inline def readResponseWith[A, S2](src: Stream[Span[Byte], Async], maxSize: Int, requestMethod: HttpMethod = HttpMethod.GET)(
        inline f: (HttpStatus, HttpHeaders, HttpBody, Stream[Span[Byte], Async]) => A < S2
    )(using inline frame: Frame): A < (S2 & Async & Abort[HttpException]) =
        readResponseImplWith(src, maxSize, requestMethod, streaming = false)(f)

    inline def readResponseStreamingWith[A, S2](src: Stream[Span[Byte], Async], maxSize: Int, requestMethod: HttpMethod = HttpMethod.GET)(
        inline f: (HttpStatus, HttpHeaders, HttpBody, Stream[Span[Byte], Async]) => A < S2
    )(using inline frame: Frame): A < (S2 & Async & Abort[HttpException]) =
        readResponseImplWith(src, maxSize, requestMethod, streaming = true)(f)

    private def readResponseImplWith[A, S2](
        src: Stream[Span[Byte], Async],
        maxSize: Int,
        requestMethod: HttpMethod,
        streaming: Boolean
    )(f: (HttpStatus, HttpHeaders, HttpBody, Stream[Span[Byte], Async]) => A < S2)(using Frame): A < (S2 & Async & Abort[HttpException]) =
        ByteStream.readUntil(src, ByteStream.CRLF_CRLF, MaxHeaderSize).map { case (headerBytes, remaining) =>
            val headerStr = new String(headerBytes.toArrayUnsafe, Utf8)
            val lines     = headerStr.split("\r\n")
            if lines.isEmpty then Abort.fail(HttpProtocolException("Empty response"))
                ...
                Abort.get(parseStatusLine(lines(0))).map { status =>
                    ...
                    then
                        f(status, headers, HttpBody.Empty, remaining)
                    else
                        val isChunked = headers.get("Transfer-Encoding") match
                            case Present(v) => v.toLowerCase.contains("chunked")
                            case Absent     => false
                        if streaming && isChunked then
                            readChunkedBodyStreamingWith(remaining) { (body, remaining2) =>
                                f(status, headers, body, remaining2)
                            }
                        else
                            readBodyWith(remaining, headers, maxSize) { (body, remaining2) =>
                                f(status, headers, body, remaining2)
    ...
    def writeResponse(stream: TransportStream, status: HttpStatus, headers: HttpHeaders, body: HttpBody)(using Frame): Unit < Async =
        Sync.defer {
            val finalHeaders = addBodyHeaders(headers, body)
            val sb           = new StringBuilder(256)
    ...
    private inline def readBodyWith[A, S2](stream: Stream[Span[Byte], Async], headers: HttpHeaders, maxSize: Int)(
        inline f: (HttpBody, Stream[Span[Byte], Async]) => A < S2
    )(using inline frame: Frame): A < (S2 & Async & Abort[HttpException]) =
        val isChunked = headers.get("Transfer-Encoding") match
            case Present(v) => v.toLowerCase.contains("chunked")
            case Absent     => false

        if isChunked then readChunkedBody(stream).map { case (a, b) => f(a, b) }
        else
            headers.get("Content-Length") match
                case Present(lenStr) =>
                    Abort.get(
                        ...
                        else if len <= 0 then f(HttpBody.Empty, stream)
                        else
                            ByteStream.readExact(stream, len).map { case (data, remaining) =>
                                f(HttpBody.Buffered(data), remaining)
                            }
                    }
                case Absent => f(HttpBody.Empty, stream)
        end if
    end readBodyWith
    ...
    /** Read a chunked body lazily as a stream.
      ...
      */
    private inline def readChunkedBodyStreamingWith[A, S2](stream: Stream[Span[Byte], Async])(
        inline f: (HttpBody, Stream[Span[Byte], Async]) => A < S2
    )(using inline frame: Frame): A < (S2 & Async & Abort[HttpException]) =
        Promise.initWith[Stream[Span[Byte], Async], Any] { remainingPromise =>
            ...
            f(HttpBody.Streamed(lazyStream), remainingStream)
        }
    end readChunkedBodyStreamingWith
