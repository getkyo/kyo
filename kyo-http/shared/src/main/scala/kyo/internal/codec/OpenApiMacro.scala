...
private[kyo] object OpenApiMacro:
            ...
            innerType(left) match
                case '[l] =>
                    innerType(right) match
                        case '[r] =>
                            '{ ${ left.asExprOf[kyo.Record[l]] } & ${ right.asExprOf[kyo.Record[r]] } }.asTerm
        }.asExpr
    end generateRoutes
                    ...
                    case '[reqType] =>
                        innerType(respTerm) match
                            case '[respType] =>
                                ...
                                '{ kyo.Record.~($nameExpr)($route) }.asTerm
                end match
        end match
    end buildRoute
    ...
    private def buildRequestDef(using
        ...
        for param <- params do
            val codecType = resolveCodecType(param.json)
            ...
            case Some(json) =>
                ...
                codecType.asType match
                    case '[a] =>
                        ...
                        val tildeType   = TypeRepr.of[kyo.Record.~].appliedTo(List(nameType, TypeRepr.of[a]))
    ...
    private def buildCapture(paramName: String, pathParams: List[HttpOpenApi.Parameter], context: String = "")(using
        ...
        val json      = pathParams.find(_.name == paramName).map(_.json)
        val codecType = json.map(resolveCodecType).getOrElse(TypeRepr.of[String])
    ...
    private def resolveCodecType(json: HttpOpenApi.SchemaObject)(using Quotes): quotes.reflect.TypeRepr =
        import quotes.reflect.*
        (json.`type`, json.format) match
