...
object RouteUtil:
    ...
    def decodeBufferedResponse[In, Out, S](
        ...
        result match
            case Result.Error(_: HttpDecodeException) if !status.isSuccess =>
                Result.fail(HttpStatusException(status, method, url))
            case other => other
        end match
    ...
    def decodeStreamingResponse[In, Out, S](
        ...
        if !hasParams then
            bodyField.foreach(bf => discard(builder.add(bf.fieldName, decodeStreamBodyValue(bf.contentType, stream, headers, method, url))))
            Result.succeed(HttpResponse(status, headers, Record(builder.result())))
        else
    ...
    def decodeBufferedRequest[In, Out, S](
        ...
        val ctxUrl    = route.request.path.show
        val fields    = route.request.fields
        val bodyField = findBodyField(fields)
        val hasParams = fields.size > (if bodyField.isDefined then 1 else 0)
        val builder   = DictBuilder.init[String, Any]
                ...
                captures.get(wireName) match
                    case Present(raw) =>
                        c.codec.decode(raw)
                            ...
                            .mapFailure(e => HttpFieldDecodeException(wireName, "path", method, url, e))
                    case Absent =>
                        Result.fail(HttpMissingFieldException(wireName, "path", method, url))
                end match
            case r: HttpPath.Rest[?] =>
        ...
        @tailrec def loop(i: Int, cookieBuilder: Maybe[StringBuilder]): Maybe[StringBuilder] =
            if i >= fields.size then cookieBuilder
                ...
                val nextCookie = fields(i) match
                    ...
                    case _: Field.Body[?, ?] => cookieBuilder
                loop(i + 1, nextCookie)
        loop(0, Absent)
        ...
        raw match
            case Present(str) =>
                param.codec.decode(str)
                    ...
                    .mapFailure(e => HttpFieldDecodeException(wireName, fieldType, method, url, e))
            case Absent =>
                param.default match
                    case Present(d) =>
                    ...
                    case Absent =>
                        ...
                        else Result.fail(HttpMissingFieldException(wireName, fieldType, method, url))
        end match
    end decodeParam
    ...
    private def encodeBufferedBodyValueWith[A](ct: ContentType[?], value: Any)(f: (String, Span[Byte]) => A): A =
        ct match
    ...
    private def encodeStreamBodyValueWith[A](ct: ContentType[?], value: Any)(
        ...
        ct match
            ...
            case json: ContentType.Json[?] =>
                if !checkContentType(headers, "application/json") then
                    Result.fail(HttpUnsupportedMediaTypeException("application/json", headers.get("Content-Type"), method, url))
                else
                    json.json.decode(spanToString(bytes))
                        .mapFailure(msg => HttpJsonDecodeException(msg, method, url))
            case form: ContentType.Form[?] =>
            ...
            case ndjson: ContentType.Ndjson[?] =>
                ...
                splitLines(stream, "\n").mapPure { line =>
                    json.decode(line) match
                        ...
                        case Result.Failure(msg) => throw HttpJsonDecodeException(msg, method, url)
    ...
    private def parseMultipartBody(body: Span[Byte], headers: HttpHeaders, method: String, url: String)(using
        Frame
        ...
        boundaryOpt match
            case Absent =>
                Result.fail(HttpMissingBoundaryException(method, url))
