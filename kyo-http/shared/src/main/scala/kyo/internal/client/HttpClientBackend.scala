...
final private[kyo] class HttpClientBackend private (
    ...
    private inline def encodeAndSendDirectWith[In, Out, A](
        ...
        RouteUtil.encodeRequestWithBoundary(route, request, multipartBoundary)(
            onEmpty = (path, headers) =>
                PolicyClientBackend.unsendableField(path, hostHeader, headers) match
    ...
    def connectRaw(
        ...
        PolicyClientBackend.unsendableField(url.pathWithQuery, hostHeaderValue, headers) match
    ...
    /** Leases a pooled connection, or opens one, and sends the request over it. */
