...
object ByteStream:
    ...
    inline def readLineWith[A, S2](src: Stream[Span[Byte], Async], maxSize: Int = 8192)(
        inline f: (Span[Byte], Stream[Span[Byte], Async]) => A < S2
    )(using inline frame: Frame): A < (S2 & Async & Abort[HttpException]) =
        readLine(src, maxSize).map { case (a, b) => f(a, b) }
