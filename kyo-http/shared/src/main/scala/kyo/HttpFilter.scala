...
object HttpFilter:
    ...
    /** SPI for contributing HTTP filters that apply globally to all requests or responses, discovered via `java.util.ServiceLoader`.
      ...
      * pool). Return `Absent` from either method to skip installation, which lets factories be configured at runtime via system properties
      * or environment variables.
      *
      * IMPORTANT: Filter factories are loaded eagerly at first server or client use. Avoid side effects in the factory constructor itself;
      * put any initialization logic inside `serverFilter` or `clientFilter`.
      *
      * @see
      *   [[HttpFilter]] The filter type returned by each factory method
      * @see
      *   [[HttpHandler]] Applies the composed server filter on each request
      * @see
      *   [[HttpClient]] Applies the composed client filter on each outgoing request
      */
    trait Factory:
