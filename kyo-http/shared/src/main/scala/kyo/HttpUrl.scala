...
/** Parsed URL with structured access to scheme, host, port, path, and query parameters.
  *
  * Constructed via `HttpUrl.parse` (for full URLs like `https://example.com/path?q=1`) or `HttpUrl.fromUri` (for server-side request URIs
  * like `/path?q=1` with no host). Query parameters are lazily parsed from the raw query string on each call to `query(name)` or
  * `queryAll(name)` — results are not cached, so avoid calling these in tight loops. URL decoding is lenient: malformed percent-encoding
  * falls back to the raw value rather than throwing.
  *
  * The `baseUrl` property returns the URL without query parameters, which is safe for logging and error messages because query strings may
  * contain sensitive data (API keys, tokens, etc.). All `HttpException` messages use `baseUrl` internally for this reason.
  *
  * Note: `scheme` can be `Absent` for path-only URLs. The `ssl` property returns true when the scheme is "https" or the port is 443.
  *
  * Unix socket URLs use the `http+unix` or `https+unix` scheme following the convention from Python's urllib3:
  * `http+unix://%2Fvar%2Frun%2Fdocker.sock/v1.43/containers/json`. During parsing, the `+unix` suffix is consumed: `scheme` is normalized
  * to plain `http` or `https`, `host` defaults to `"localhost"`, and the decoded socket path is stored in `unixSocket`. The transport layer
  * checks `unixSocket` to decide between TCP and Unix domain socket connections.
  *
  * @see
  *   [[kyo.HttpRequest]] Carries an HttpUrl
  * @see
