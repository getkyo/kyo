...
/** Immutable HTTP headers backed by a flat interleaved `Chunk[String]`.
  *
  * Headers are stored as `[name0, value0, name1, value1, ...]` rather than a `Map[String, Seq[String]]`. This layout avoids per-header
  * allocation for multi-value headers (e.g., multiple `Set-Cookie`), supports zero-copy conversion from Netty's internal format on the JVM,
  * and handles case-insensitive lookup without key normalization. The tradeoff is O(n) lookup, but HTTP messages rarely have more than ~20
  * headers.
  *
  * All name lookups are case-insensitive per HTTP semantics. Header names preserve their original case for wire serialization. `add`
  * appends without deduplication (multi-value semantics, as needed for `Set-Cookie`). `set` removes all existing headers with the same name
  * before appending.
  *
  * Cookie methods operate on different headers depending on direction: request cookies are parsed from the `Cookie` header, while response
  * cookies are parsed from `Set-Cookie` headers. The `strict` parameter on cookie methods enables RFC 6265 validation of cookie names and
  * values.
  *
  * @see
  *   [[kyo.HttpRequest]] Carries headers on requests
  * @see
  *   [[kyo.HttpResponse]] Carries headers on responses
  * @see
  *   [[kyo.HttpCookie]] Typed cookie values with attributes
  */
