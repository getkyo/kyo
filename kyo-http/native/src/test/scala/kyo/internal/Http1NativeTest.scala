...
class Http1NativeTest extends kyo.Test:
    ...
    "GET 200 with body" in run {
        onMacOS { transport =>
            withServerClient(transport) { serverConn =>
                Http1Protocol.readRequestWith(serverConn.read, 65536) { (_, path, _, _, _) =>
                ...
                Http1Protocol.writeRequest(clientConn, HttpMethod.GET, "/hello", HttpHeaders.empty, HttpBody.Empty).andThen {
                    Http1Protocol.readResponseWith(clientConn.read, 65536) { (status, _, body, _) =>
    ...
    "POST 1KB JSON body" in run {
        onMacOS { transport =>
            ...
            withServerClient(transport) { serverConn =>
                Http1Protocol.readRequestWith(serverConn.read, 65536) { (_, _, _, body, _) =>
    ...
    "HEAD no body" in run {
        onMacOS { transport =>
            withServerClient(transport) { serverConn =>
                Http1Protocol.readRequestWith(serverConn.read, 65536) { (_, _, _, _, _) =>
                ...
                Http1Protocol.writeRequest(clientConn, HttpMethod.HEAD, "/resource", HttpHeaders.empty, HttpBody.Empty).andThen {
                    ...
                    Http1Protocol.readResponseWith(clientConn.read, 65536, requestMethod = HttpMethod.HEAD) {
                        ...
                        if chunk.isEmpty then Abort.panic(new Exception("No connection"))
                            ...
                            Loop(serverConn.read, 0) { (stream, count) =>
                                if count >= 5 then Loop.done(())
                                    ...
                                    Http1Protocol.readRequestWith(stream, 65536) { (_, _, _, _, remaining) =>
                                ...
                                Loop(clientConn.read, 0) { (stream, i) =>
                                    if i >= 5 then Loop.done(succeed)
                                        ...
                                        Http1Protocol
                                            ...
                                            .andThen {
                                                Http1Protocol.readResponseWith(stream, 65536) {
                                                    (status, _, body, remaining) =>
                                                        assert(status.code == 200)
                                                        body match
                                                            case HttpBody.Buffered(data) =>
                                                                assert(
                        ...
                        if chunk.isEmpty then Abort.panic(new Exception("No connection"))
                            ...
                            Loop(serverConn.read, 0) { (stream, count) =>
                                if count >= 2 then Loop.done(())
                                    ...
                                    Http1Protocol.readRequestWith(stream, 65536) {
                                        (method, _, _, body, remaining) =>
                                ...
                                Http1Protocol
                                    ...
                                    .andThen {
                                        Http1Protocol
                                            ...
                                            .andThen {
                                                Http1Protocol.readResponseWith(clientConn.read, 65536) {
                                                    (_, _, body1, remaining1) =>
                                                        body1 match
                                                        ...
                                                        Http1Protocol.readResponseWith(remaining1, 65536) {
                                                            (_, _, body2, _) =>
                                                                body2 match
                                                                    case HttpBody.Buffered(d) =>
                        ...
                        if count >= n then Loop.done(())
                            ...
                            connStream.splitAt(1).map { (chunk, rest) =>
                                if chunk.isEmpty then Loop.done(())
                                    ...
                                    Fiber.initUnscoped {
                                        Http1Protocol.readRequestWith(serverConn.read, 65536) {
                                            (_, path, _, _, _) =>
                                                val respBody =
                            ...
                            Scope.run {
                                Scope.ensure(transport.closeNow(conn)).andThen {
                                    Http1Protocol
                                        ...
                                        .andThen {
                                            Http1Protocol.readResponseWith(conn.read, 65536) { (status, _, body, _) =>
                                                assert(status.code == 200)
                                                body match
                                                    case HttpBody.Buffered(data) =>
                    ...
                    listener.connections.take(1).run.map { chunk =>
                        if chunk.isEmpty then Abort.panic(new Exception("No connection"))
                            ...
                            Abort.run[HttpException] {
                                Http1Protocol.readRequestWith(serverConn.read, 65536) { (a, b, c, d, e) => (a, b, c, d, e) }
                            }.map { result =>
                        ...
                        Scope.run {
                            Scope.ensure(transport.closeNow(clientConn)).andThen {
                                Http1Protocol
                                    ...
                                    .andThen {
                                        Abort.run[HttpException] {
                                            Http1Protocol.readResponseWith(clientConn.read, 65536) { (a, b, c, d) => (a, b, c, d) }
                                        }.map { result =>
    ...
    "1MB body arrives complete" in run {
        onMacOS { transport =>
            ...
            withServerClient(transport) { serverConn =>
                Http1Protocol.readRequestWith(serverConn.read, size + 1024) { (_, _, _, body, _) =>
                    Http1Protocol.writeResponse(serverConn, HttpStatus(200), HttpHeaders.empty, body)
                }
            } { (clientConn, _) =>
                ...
                Http1Protocol
                    ...
                    .andThen {
                        Http1Protocol.readResponseWith(clientConn.read, size + 1024) { (status, _, body, _) =>
                            assert(status.code == 200)
                            body match
                        ...
                        Scope.run {
                            Scope.ensure(transport.closeNow(clientConn)).andThen {
                                Http1Protocol
                                    ...
                                    .andThen {
                                        Abort.run[HttpException] {
                                            Http1Protocol.readResponseWith(clientConn.read, maxSize) { (a, b, c, d) => (a, b, c, d) }
                                        }.map { result =>
                                            result match
