...
final private[kyo] class CurlEventLoop(daemon: Boolean):
    ...
    def shutdown(): Unit =
        ...
        while iter.hasNext do
            ...
            failTransfer(state, HttpConnectException(host, port, new RuntimeException("Event loop shut down")))
            iter.remove()
        end while
    ...
    private def failTransfer(state: CurlTransferState, error: HttpException): Unit =
        state match
            case bs: CurlBufferedTransferState =>
    ...
    private def curlResultToError(code: CInt, state: CurlTransferState): HttpException =
        val (host, port) = state match
            case bs: CurlBufferedTransferState  => (bs.host, bs.port)
            case ss: CurlStreamingTransferState => (ss.host, ss.port)
...
private[kyo] object CurlEventLoop:
    ...
    private[kyo] def curlResultToError(code: Int, host: String, port: Int)(using Frame): HttpException =
        code match
            case 6 | 7 =>
                HttpConnectException(host, port, new RuntimeException(s"curl error $code: connection failed"))
            case 28 =>
                HttpConnectException(host, port, new RuntimeException(s"curl error $code: operation timed out"))
            case 35 | 51 | 53 | 54 | 58 | 59 | 60 =>
                HttpConnectException(host, port, new RuntimeException(s"curl error $code: SSL error"))
            case _ =>
                HttpConnectException(host, port, new RuntimeException(s"curl error $code"))
        end match
    end curlResultToError
