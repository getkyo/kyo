...
final class KqueueNativeTransport(
    ...
    def connect(address: HttpAddress, tls: Maybe[TlsConfig])(using Frame): KqueueConnection < (Async & Abort[HttpException]) =
        address match
    ...
    private def connectUnixPlain(path: String)(using Frame): KqueueConnection < (Async & Abort[HttpException]) =
        Sync.defer {
            val loop = group.next()
            val (fd, pending) = Zone {
    ...
    private def connectPlain(host: String, port: Int)(using Frame): KqueueConnection < (Async & Abort[HttpException]) =
        Sync.defer {
            val loop = group.next()
            val (fd, pending) = Zone {
                val outPending = alloc[CInt]()
    ...
    private def connectTls(host: String, conn: KqueueConnection, tlsCfg: TlsConfig)(using Frame): KqueueConnection < (Async & Abort[HttpException]) =
        import TlsBindings.*
    ...
    def listen(address: HttpAddress, backlog: Int, tls: Maybe[TlsConfig])(using Frame): TransportListener[KqueueConnection] < Async =
        address match
            case HttpAddress.Tcp(host, port) => listenTcp(host, port, backlog, tls)
            case HttpAddress.Unix(path)      => listenUnix(path, backlog, tls)

    private def listenTcp(host: String, port: Int, backlog: Int, tls: Maybe[TlsConfig])(using Frame): TransportListener[KqueueConnection] < Async =
        group.ensureStarted().andThen {
            Sync.defer {
    ...
    private def listenUnix(path: String, backlog: Int, tls: Maybe[TlsConfig])(using Frame): TransportListener[KqueueConnection] < Async =
        group.ensureStarted().andThen {
            Sync.defer {
                val (serverFd, _) = Zone {
                    val outPort = alloc[CInt]()
