...
/** Transfer state for a single curl easy handle in the backend.
  *
...
final private[kyo] class CurlBufferedTransferState(
    val promise: Promise.Unsafe[CurlBufferedResult, Abort[HttpException]],
...
final private[kyo] class CurlStreamingTransferState(
    val headerPromise: Promise.Unsafe[CurlStreamingHeaders, Abort[HttpException]],
