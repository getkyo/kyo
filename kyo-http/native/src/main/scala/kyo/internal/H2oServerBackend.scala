...
private[kyo] object H2oServerBackend:
    ...
    private def startServer(router: HttpRouter, config: HttpServerConfig): HttpBackend.Binding =
        ...
        if newServer == null then throw HttpBindException(config.host, config.port, new RuntimeException("h2o server returned null"))
            ...
            onStreaming = (status, headers, stream) =>
                if isHead then
                    ...
                    discard(Sync.Unsafe.evalOrThrow(Fiber.initUnscoped {
                        ...
                        }.handle(Abort.run[Throwable]).handle(Scope.run).map {
