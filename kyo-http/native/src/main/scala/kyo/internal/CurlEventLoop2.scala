...
import kyo.HttpStatus as Http2Status
...
/** Dedicated event loop thread running curl_multi_socket_action for the http2 backend.
  *
  ...
  * raw response data.
  */
final private[kyo] class CurlEventLoop2(daemon: Boolean):
    ...
    private val loopId: Long = CurlEventLoop2.nextId.getAndIncrement()
    discard(CurlEventLoop2.loops.put(loopId, this))

    // Set up multi callbacks
    locally {
        val loopPtr = fromRawPtr[Byte](Intrinsics.castLongToRawPtr(loopId))
        discard(curl_multi_setopt(multi, CURLMOPT_SOCKETFUNCTION, CurlEventLoop2.socketCallback))
        discard(curl_multi_setopt(multi, CURLMOPT_SOCKETDATA, loopPtr))
        discard(curl_multi_setopt(multi, CURLMOPT_TIMERFUNCTION, CurlEventLoop2.timerCallback))
        discard(curl_multi_setopt(multi, CURLMOPT_TIMERDATA, loopPtr))
    }

    // Start the event loop thread
    private val thread = new Thread(() => eventLoop(), "kyo-http2-curl-event-loop")
    ...
    def shutdown(): Unit =
        ...
        wakeUp()
        thread.join(5000)
        discard(CurlEventLoop2.loops.remove(loopId))
        ...
        while iter.hasNext do
            ...
            failTransfer(state, Http2Error.ConnectionError("Event loop shut down", new RuntimeException("Event loop shut down")))
            iter.remove()
        end while
    ...
    private def failTransfer(state: CurlTransferState, error: Http2Error): Unit =
        state match
            case bs: CurlBufferedTransferState =>
    ...
    private def curlResultToError(code: CInt, state: CurlTransferState): Http2Error =
        ...
        CurlEventLoop2.curlResultToError(code, host, port)
    end curlResultToError

    /** Parse raw header lines into http2 Http2Headers (flat interleaved Chunk). */
    private def parseHeaders(raw: String): Http2Headers =
        ...
        Http2Headers.fromChunk(builder.result())
    end parseHeaders
...
private[kyo] object CurlEventLoop2:
    import CurlBindings.*
    import scala.scalanative.runtime.Intrinsics

    private[kyo] val nextId = new AtomicLong(0)
    private[kyo] val loops  = new ConcurrentHashMap[Long, CurlEventLoop2]()

    /** Map a curl result code to the appropriate Http2Error. */
    private[kyo] def curlResultToError(code: Int, host: String, port: Int)(using Frame): Http2Error =
        code match
            case 6 | 7 =>
            ...
            case 28 => Http2Error.TimeoutError(Duration.Zero) // curl doesn't tell us the configured timeout
            case 35 | 51 | 53 | 54 | 58 | 59 | 60 =>
                Http2Error.ConnectionError(
                    s"SSL error connecting to $host:$port (curl error $code)",
                    new RuntimeException(s"curl SSL error $code")
                )
            case _ => Http2Error.ConnectionError(s"curl error $code for $host:$port", new RuntimeException(s"curl error $code"))
        end match
    end curlResultToError
