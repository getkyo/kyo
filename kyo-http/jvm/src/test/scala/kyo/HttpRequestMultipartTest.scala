...
class HttpRequestMultipartTest extends Test:
    ...
    "multipart constructor" - {
        "with single part" in {
            val parts   = Seq(Part("file", Present("test.txt"), Present("text/plain"), Span.fromUnsafe("content".getBytes)))
        ...
        "with multiple parts" in {
            val parts = Seq(
                Part("file1", Present("a.txt"), Present("text/plain"), Span.fromUnsafe("a".getBytes)),
                Part("file2", Present("b.txt"), Present("text/plain"), Span.fromUnsafe("b".getBytes))
            )
            val request = HttpRequest.multipart("http://example.com/upload", parts)
        ...
        "with headers" in {
            val parts = Seq(Part("data", Absent, Absent, Span.fromUnsafe(Array.empty[Byte])))
        ...
        "sets content-type to multipart/form-data" in {
            val parts   = Seq(Part("file", Absent, Absent, Span.fromUnsafe(Array.empty[Byte])))
            val request = HttpRequest.multipart("http://example.com/upload", parts)
        ...
        "sets method to POST" in {
            val request =
                HttpRequest.multipart("http://example.com/upload", Seq(Part("f", Absent, Absent, Span.fromUnsafe(Array.empty[Byte]))))
            assert(request.method == Method.POST)
        }
    ...
    "multipart parts parsing" - {
        "returns multipart parts" in {
            val parts = Seq(
                Part("file", Present("test.txt"), Present("text/plain"), Span.fromUnsafe("content".getBytes))
            )
            val request = HttpRequest.multipart("http://example.com/upload", parts)
        ...
        "preserves part order" in {
            val parts = Seq(
                Part("first", Absent, Absent, Span.fromUnsafe(Array.empty[Byte])),
                Part("second", Absent, Absent, Span.fromUnsafe(Array.empty[Byte])),
                Part("third", Absent, Absent, Span.fromUnsafe(Array.empty[Byte]))
            )
            val request = HttpRequest.multipart("http://example.com/upload", parts)
        ...
        "preserves filename" in {
            val parts = Seq(
                Part("doc", Present("report.pdf"), Present("application/pdf"), Span.fromUnsafe("data".getBytes))
            )
            val request     = HttpRequest.multipart("http://example.com/upload", parts)
            val parsedParts = request.parts
        ...
        "preserves content type" in {
            val parts = Seq(
                Part("image", Present("photo.png"), Present("image/png"), Span.fromUnsafe("pixels".getBytes))
            )
            val request     = HttpRequest.multipart("http://example.com/upload", parts)
            val parsedParts = request.parts
        ...
        "preserves content bytes" in {
            ...
            val parts   = Seq(Part("file", Present("test.txt"), Present("text/plain"), Span.fromUnsafe(content)))
            val request = HttpRequest.multipart("http://example.com/upload", parts)
            val parsed  = request.parts(0)
            assert(new String(parsed.data.toArrayUnsafe, "UTF-8") == "hello world")
        }

        "handles part without filename" in {
            val parts       = Seq(Part("field", Absent, Absent, Span.fromUnsafe("value".getBytes)))
            val request     = HttpRequest.multipart("http://example.com/upload", parts)
            val parsedParts = request.parts
            assert(parsedParts(0).name == "field")
            assert(parsedParts(0).filename == Absent)
            assert(new String(parsedParts(0).data.toArrayUnsafe, "UTF-8") == "value")
        }

        "handles part without content type" in {
            val parts       = Seq(Part("field", Present("data.bin"), Absent, Span.fromUnsafe(Array[Byte](1, 2, 3))))
            val request     = HttpRequest.multipart("http://example.com/upload", parts)
            val parsedParts = request.parts
        ...
        "handles binary content" in {
            ...
            val parts         = Seq(Part("bin", Present("data.bin"), Present("application/octet-stream"), Span.fromUnsafe(binaryContent)))
            val request       = HttpRequest.multipart("http://example.com/upload", parts)
            val parsed        = request.parts(0)
            assert(parsed.data.size == 256)
            assert(parsed.data.toArrayUnsafe.toSeq == binaryContent.toSeq)
        }

        "handles mixed form fields and file parts" in {
            val parts = Seq(
                Part("description", Absent, Absent, Span.fromUnsafe("A photo of sunset".getBytes)),
                Part(
                    "file",
                    Present("sunset.jpg"),
                    Present("image/jpeg"),
                    Span.fromUnsafe(Array[Byte](0xff.toByte, 0xd8.toByte, 0xff.toByte))
                ),
                Part("tags", Absent, Absent, Span.fromUnsafe("nature,sunset".getBytes))
            )
            val request     = HttpRequest.multipart("http://example.com/upload", parts)
            val parsedParts = request.parts
            ...
            assert(parsedParts(0).filename == Absent)
            assert(new String(parsedParts(0).data.toArrayUnsafe, "UTF-8") == "A photo of sunset")
            ...
            assert(new String(parsedParts(2).data.toArrayUnsafe, "UTF-8") == "nature,sunset")
        }

        "handles large content" in {
            val largeContent = new Array[Byte](10000)
            java.util.Arrays.fill(largeContent, 'X'.toByte)
            val parts       = Seq(Part("big", Present("large.dat"), Present("application/octet-stream"), Span.fromUnsafe(largeContent)))
            val request     = HttpRequest.multipart("http://example.com/upload", parts)
            val parsedParts = request.parts
            assert(parsedParts(0).data.size == 10000)
            assert(parsedParts(0).data.toArrayUnsafe.toSeq.forall(_ == 'X'.toByte))
        }
        ...
        "handles empty content" in {
            ...
            val request     = HttpRequest.multipart("http://example.com/upload", parts)
            val parsedParts = request.parts
            assert(parsedParts(0).data.isEmpty)
        }

        "handles content with CRLF bytes" in {
            val content     = "line1\r\nline2\r\nline3".getBytes("UTF-8")
            val parts       = Seq(Part("text", Present("lines.txt"), Present("text/plain"), Span.fromUnsafe(content)))
            val request     = HttpRequest.multipart("http://example.com/upload", parts)
            val parsedParts = request.parts
            assert(new String(parsedParts(0).data.toArrayUnsafe, "UTF-8") == "line1\r\nline2\r\nline3")
        }
        ...
        "handles many parts" in {
            ...
            assert(new String(parsedParts(0).data.toArrayUnsafe, "UTF-8") == "content1")
            assert(parsedParts(19).name == "field20")
            assert(new String(parsedParts(19).data.toArrayUnsafe, "UTF-8") == "content20")
        }
    }
    ...
    "Part validation" - {
        "rejects empty name" in {
            assertThrows[IllegalArgumentException] {
                Part("", Absent, Absent, Span.fromUnsafe(Array.empty[Byte]))
            }
        }
    }
