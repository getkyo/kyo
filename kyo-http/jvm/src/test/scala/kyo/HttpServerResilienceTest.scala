...
/** HTTP-level resilience of the shared `HttpClient` + `HttpServer` stack under request cancellation and server churn,
  ...
  *
  * The property under test: a per-connection failure must never mark the shared transport driver closed, because once it
  * is, every later call fails with "... is closed" and the process-shared driver wedges every unrelated caller. Each
  * scenario builds a client + server over a specific backend's transport (via the kyo-net `TestBackends` harness,
  * reachable through the `test->test` dependency), drives a failure/cancellation load, and asserts the shared client
  * still round-trips afterward. Fibers only: no real OS threads, no `runAndBlock`.
  */
class HttpServerResilienceTest extends BaseHttpTest:
    ...
    // ---- pooled client + firing request timeout + server restart churn (fibers) ------------------------------------

    /** Whether `e`'s message names both "NioIoDriver" and "is closed". This match is broad: every `Closed` the driver
      * emits renders as "NioIoDriver[...] created at ... is closed. <detail>", so it also matches ordinary per-connection
      * closes (an RST'd read, a cancelled connect), not only a whole-driver close. Each hit is therefore LOGGED
      * (`recordIfDriverClosed`) with its real exception type, message, and full cause chain, so the detail distinguishes a
      * per-connection close ("channel=... closed") from a genuine whole-driver close ("driver closed"). The post-load
      * liveness assert is the independent ground-truth check on whether the driver actually wedged.
      */
    ...
    /** If `e` matches the driver-closed signature, increment `counter` and log the first few hits' actual exception type,
    ...
    // ---- nio-specific: pooled client under request cancellation + server restart churn -----------------------------

    /** On nio, a per-connection dispatch failure must stay contained and never mark the shared driver closed, exercised
      * at the HTTP level with FIBERS.
      *
