...
/** MCP (Model Context Protocol) server implementing Streamable HTTP transport.
  ...
  * messages. Provides two tools:
  *   - "weather": fetches current weather from Open-Meteo (no API key needed)
  *   - "echo": returns the input back (for testing)
  *
  * Test with: curl -X POST http://localhost:3001/mcp -H "Content-Type: application/json" \ -d
...
object McpServer extends KyoApp:
    ...
    def fetchWeather(city: String): String < (Async & Abort[HttpError]) =
        ...
        HttpClient.get[WeatherResponse](url).map { w =>
    ...
    def handleRpc(req: JsonRpcRequest): JsonRpcResponse < (Async & Abort[HttpError]) =
        ...
        req.method match
            ...
            case "notifications/initialized" =>
                // Notification — no response needed, but we return one since caller checks
    ...
    val mcpGet = HttpHandler.streamSse[JsonRpcResponse]("mcp") { _ =>
    ...
    run {
        HttpFilter.server.cors().andThen(HttpFilter.server.logging).enable {
            HttpServer.init(
                HttpServer.Config.default.port(3001).openApi("/mcp/openapi.json", "MCP Demo Server")
            )(mcpPost, mcpGet).map { server =>
                for
                    _ <- Console.printLine(s"MCP server running on http://localhost:${server.port}/mcp")
                    _ <- Console.printLine("Test: curl -X POST http://localhost:3001/mcp -H 'Content-Type: application/json' \\")
                    _ <- Console.printLine(
                        """  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2025-06-18","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}'"""
                    )
                    _ <- Console.printLine(
                        """Tools: curl -X POST http://localhost:3001/mcp -H 'Content-Type: application/json' -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"weather","arguments":{"city":"tokyo"}}}'"""
                    )
                    _ <- server.await
                yield ()
            }
