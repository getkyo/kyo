package demo

import kyo.*

/** GitHub public event feed streamed as SSE.
  *
  * Polls GitHub's public events API and re-streams as typed SSE events. Demonstrates: SSE streaming, client filters (User-Agent), typed
  * JSON parsing, withConfig for baseUrl, stream transformation.
  *
  * Endpoints: GET /events - SSE stream of all public events GET /feed/:owner/:repo - SSE stream of repo events (uses route-based handler)
  */
object GithubFeed extends KyoApp:

    // GitHub event model (subset of fields)
    case class GithubEvent(id: String, `type`: String, actor: GithubActor, repo: GithubRepo, created_at: String) derives Schema
    case class GithubActor(login: String) derives Schema
    case class GithubRepo(name: String) derives Schema

    // Our simplified event for SSE output
    case class FeedEvent(id: String, kind: String, actor: String, repo: String, time: String) derives Schema

    def toFeed(e: GithubEvent): FeedEvent =
        FeedEvent(e.id, e.`type`, e.actor.login, e.repo.name, e.created_at)

    // Fetch events from GitHub
    def fetchEvents(url: String): Seq[GithubEvent] < (Async & Abort[HttpError]) =
        HttpFilter.client.addHeader("User-Agent", "kyo-http-demo").enable {
            HttpClient.withConfig(_.timeout(10.seconds)) {
                HttpClient.get[Seq[GithubEvent]](url)
            }
        }

    // Create a polling SSE stream that deduplicates events
    def pollStream(url: String): Stream[HttpEvent[FeedEvent], Async] < Async =
        AtomicRef.init(Set.empty[String]).map { seenRef =>
            Stream.init(Chunk.from(1 to 100)).mapChunk { _ =>
                Async.delay(10.seconds) {
                    Abort.run[HttpError](fetchEvents(url)).map {
                        case Result.Success(events) =>
                            seenRef.get.map { seen =>
                                val newEvents = events.filterNot(e => seen.contains(e.id))
                                seenRef.set(seen ++ newEvents.map(_.id)).andThen {
                                    Chunk.from(newEvents.map { e =>
                                        HttpEvent(
                                            data = toFeed(e),
                                            event = Present(e.`type`),
                                            id = Present(e.id)
                                        )
                                    })
                                }
                            }
                        case _ => Chunk.empty[HttpEvent[FeedEvent]]
                    }
                }
            }
        }

    // SSE handler for public events (uses convenience helper — string path only)
    val publicFeed = HttpHandler.streamSse[FeedEvent]("events") { _ =>
        pollStream("https://api.github.com/events?per_page=10")
    }

    // Route-based handler for repo events (supports path captures)
    val repoFeedRoute = HttpRoute
        .get("feed" / HttpPath.Capture[String]("owner") / HttpPath.Capture[String]("repo"))
        .response(_.bodySse[FeedEvent])
        .metadata(_.summary("SSE stream of repo events").tag("github"))
        .handle { in =>
            pollStream(s"https://api.github.com/repos/${in.owner}/${in.repo}/events?per_page=10")
        }

    val health = HttpHandler.health()

    run {
        HttpFilter.server.logging.enable {
            HttpServer.init(HttpServer.Config.default.port(3003))(publicFeed, repoFeedRoute, health).map { server =>
                for
                    _ <- Console.printLine(s"GitHub Feed running on http://localhost:${server.port}")
                    _ <- Console.printLine("  curl -N http://localhost:3003/events")
                    _ <- Console.printLine("  curl -N http://localhost:3003/feed/scala/scala")
                    _ <- server.await
                yield ()
            }
        }
    }
end GithubFeed
