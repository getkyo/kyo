...
object ApiGateway extends KyoApp:
    ...
    def fetchWeather(city: String): WeatherInfo < (Async & Abort[HttpError]) =
        cities.get(city.toLowerCase) match
            case None => Abort.fail(HttpError.InvalidResponse(s"Unknown city: $city"))
            case Some((lat, lon)) =>
                HttpClient.withConfig(_.timeout(5.seconds)) {
                    ...
                    HttpClient.get[MeteoResponse](url).map { r =>
    ...
    def fetchRates(base: String, symbols: String): RateInfo < (Async & Abort[HttpError]) =
        HttpClient.withConfig(_.timeout(5.seconds)) {
            ...
            HttpClient.get[FrankfurterResponse](url).map { r =>
    ...
    val weatherRoute = HttpRoute
        .get("weather" / HttpPath.Capture[String]("city"))
    ...
    val ratesRoute = HttpRoute
        .get("rates")
        .request(
            _.query[String]("base", default = Some("USD"))
                .query[String]("to", default = Some("EUR,GBP,JPY"))
        )
        .response(_.bodyJson[RateInfo].error[ApiError](HttpStatus.BadRequest))
        .metadata(_.summary("Exchange rates").tag("finance"))
        .handle { in =>
            fetchRates(in.base, in.to)
        }

    val travelRoute = HttpRoute
        .get("travel" / HttpPath.Capture[String]("city"))
        .request(
            _.query[Int]("budget", default = Some(1000))
                .query[String]("currency", default = Some("USD"))
        )
        ...
        .handle { in =>
            ...
            yield TravelInfo(weather, localCurrency, rate, budgetInLocal)
        }
    ...
    run {
        HttpFilter.server.logging
            ...
            .enable {
                HttpServer.init(
                    HttpServer.Config.default.port(3002).openApi("/openapi.json", "Travel API Gateway")
