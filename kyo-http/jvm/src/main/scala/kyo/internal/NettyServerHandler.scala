...
final private[kyo] class NettyServerHandler(
    router: HttpRouter,
    ...
    // BUFFERING state
    // Mutable vars are safe and intentional:
    // - Thread safety: this handler is not @Sharable and is instantiated per-channel in
    //   NettyServerBackend's ChannelInitializer.initChannel. Netty guarantees all events
    //   for a channel execute on the same EventLoop thread — no concurrent access.
    // - Performance: flat vars avoid allocation overhead of a sealed-trait state machine
    //   (which would allocate a case class per state transition on every HTTP chunk).
            ...
            fiber.unsafe.onComplete {
                case Result.Failure(e) =>
                    if ctx.channel().isActive then
                        val handlerError = HttpHandlerException(e)
    ...
    override def exceptionCaught(ctx: ChannelHandlerContext, cause: Throwable): Unit =
        import AllowUnsafe.embrace.danger // Override entry point — cannot use (using AllowUnsafe)
        val handlerError = HttpHandlerException(cause)
