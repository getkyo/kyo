...
final class NioTransport(
    ...
    def connect(address: HttpAddress, tls: Maybe[TlsConfig])(using Frame): NioConnection < (Async & Abort[HttpException]) =
        address match
    ...
    private def connectTls(host: String, port: Int, tlsCfg: TlsConfig)(using Frame): NioConnection < (Async & Abort[HttpException]) =
    ...
    private def connectUnixPlain(path: String)(using Frame): NioConnection < (Async & Abort[HttpException]) =
        Abort.recover[Exception](_ =>
    ...
    def listen(address: HttpAddress, backlog: Int, tls: Maybe[TlsConfig])(using Frame): TransportListener[NioConnection] < Async =
        address match
    ...
    private def listenTcp(host: String, port: Int, backlog: Int, tls: Maybe[TlsConfig])(using
        Frame
    ): TransportListener[NioConnection] < Async =
        group.ensureStarted().andThen {
            Sync.defer {
                ...
                val actualAddress = HttpAddress.Tcp(host, actualPort)
    ...
    private def listenUnix(path: String, backlog: Int, tls: Maybe[TlsConfig])(using Frame): TransportListener[NioConnection] < Async =
        group.ensureStarted().andThen {
            Sync.defer {
                ...
                val actualAddress = HttpAddress.Unix(path)
