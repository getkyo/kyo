...
final private[http2] class NettyConnection(
    ...
    private def sendStreamingWith[In, Out, A, S](
        ...
        Sync.Unsafe.defer {
            ...
            val headerPromise = Promise.Unsafe.init[NettyHttp2StreamingHeaderData, Abort[HttpError]]()
            val byteChannel   = Channel.Unsafe.init[Maybe[Span[Byte]]](32)
            discard(pipeline.addLast("response", new NettyHttp2StreamingResponseHandler(headerPromise, byteChannel)))
...
private[http2] case class NettyHttpResponseData(
    status: HttpStatus,
    headers: HttpHeaders,
    body: Span[Byte]
)

private[http2] case class NettyHttp2StreamingHeaderData(
    status: HttpStatus,
    headers: HttpHeaders
)

final private[http2] class NettyHttpResponseHandler(
...
final private[http2] class NettyHttp2StreamingResponseHandler(
    headerPromise: Promise.Unsafe[NettyHttp2StreamingHeaderData, Abort[HttpError]],
...
end NettyHttp2StreamingResponseHandler
