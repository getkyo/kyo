...
// By default, the server binds to 0.0.0.0 (all network interfaces) on port 0.
...
`getSseText` works the same way with `HttpSseEvent[String]` instead of a typed value. `SseEvent` also supports optional fields for the SSE protocol: `event` (event type name), `id` (event ID for reconnection), and `retry` (reconnection delay).
