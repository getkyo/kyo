...
final class JsTransport extends Transport:
    ...
    def connect(address: HttpAddress, tlsConfig: Maybe[TlsConfig])(using Frame): JsConnection < (Async & Abort[HttpException]) =
        address match
    ...
    private def connectTcp(host: String, port: Int, tlsConfig: Maybe[TlsConfig])(using
        ...
        Promise.initWith[JsConnection, Async & Abort[HttpException]] { promise =>
            Sync.defer {
    ...
    private def connectUnix(path: String)(using Frame): JsConnection < (Async & Abort[HttpException]) =
        Promise.initWith[JsConnection, Async & Abort[HttpException]] { promise =>
            Sync.defer {
    ...
    def listen(address: HttpAddress, backlog: Int, tlsOpt: Maybe[TlsConfig])(using frame: Frame): TransportListener[JsConnection] < Async =
        Promise.initWith[TransportListener[JsConnection], Any] { ready =>
            ...
            discard(server.on(
                ...
                { (socket: js.Dynamic) =>
                    ...
                    address match
                        case _: HttpAddress.Tcp => discard(socket.setNoDelay(true))
                        case _                  =>
            ...
            val connStream: Stream[JsConnection, Async] =
                Stream.unfold((), chunkSize = 1) { _ =>
                    ...
                    if connQueue.nonEmpty then
                        ...
                        Promise.initWith[JsConnection, Any] { p =>
            ...
            val closeListener: Unit < Sync = Sync.defer {
                import AllowUnsafe.embrace.danger
                        ...
                        discard(server.listen(
                            ...
                            { () =>
                                ...
                                discard(ready.unsafe.complete(Result.succeed(
                                    new TransportListener(HttpAddress.Tcp(host, actualPort), connStream, closeListener)
                                )))
                            }: js.Function0[Unit]
                        ))
                    case HttpAddress.Unix(path) =>
                        discard(server.listen(
                            ...
                            { () =>
                                import AllowUnsafe.embrace.danger
                                discard(ready.unsafe.complete(Result.succeed(
                                    new TransportListener(HttpAddress.Unix(path), connStream, closeListener)
                                )))
                            }: js.Function0[Unit]
                        ))
            }.andThen(ready.get)
        ...
        leftover match
            ...
            case Absent =>
                if conn.socket.destroyed.asInstanceOf[Boolean] then
                    ...
                    Promise.initWith[Int, Any] { promise =>
                        Sync.defer {
            ...
            Promise.initWith[Unit, Any] { promise =>
                Sync.defer {
                    ...
                    if flushed then
                        ...
                        // Using `once` ensures each callback fires at most once. removeAllListeners
                        // cleans up the remaining two when any one fires.
                        import AllowUnsafe.embrace.danger
                        def complete(): Unit =
                            discard(conn.socket.removeAllListeners("drain"))
                            discard(conn.socket.removeAllListeners("close"))
                            discard(conn.socket.removeAllListeners("error"))
                            discard(promise.unsafe.complete(Result.succeed(())))
                        end complete
                        discard(conn.socket.once("drain", (() => complete()): js.Function0[Unit]))
                        discard(conn.socket.once("close", (() => complete()): js.Function0[Unit]))
                        discard(conn.socket.once("error", ((_: js.Dynamic) => complete()): js.Function1[js.Dynamic, Unit]))
                    end if
                }.andThen(promise.get)
            }
