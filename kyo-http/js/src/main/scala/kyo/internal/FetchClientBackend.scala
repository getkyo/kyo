...
object FetchClientBackend:
    ...
    final class FetchConnection(
        ...
        /** Catches any Throwable (from Future rejection, network errors, etc.) and converts to typed HttpException via classifyError. */
        private def handleErrors[A, S](v: => A < (S & Abort[HttpException]))(using Frame): A < (S & Abort[HttpException]) =
            v.handle(Abort.recoverError[Throwable](e => Abort.fail(classifyError(e.failureOrPanic.asInstanceOf[Throwable]))))
        ...
        private def classifyError(e: Throwable)(using Frame): HttpException =
            e match
                ...
                case _ =>
                    ...
                    if !js.isUndefined(cause) && !js.isUndefined(cause.code) then
                        ...
                        HttpConnectException(host, port, new RuntimeException(codeStr, e))
                    else
                        HttpConnectException(host, port, e)
                    end if
        end classifyError
