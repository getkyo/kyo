...
- [kyo-sql] `SqlClient.reset`: the connection's prepared-statement cache is now discarded along with the session. `DISCARD ALL` includes `DEALLOCATE ALL`, so the next extended-protocol call reusing that SQL bound a name the server had dropped and the session answered `26000 prepared statement "s_..." does not exist`, permanently for that SQL.
