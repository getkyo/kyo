...
class AttributeUnpicklerTest extends Test:
    ...
    "absent Attributes section returns FileAttributes.default (all flags false)" in run {
        ...
        val result = Abort.run[ReflectError](AttributeUnpickler.read(view, Array.empty))
        result.map { r =>
            r match
    ...
    "synthesized Attributes section with EXPLICITNULLSattr sets explicitNulls=true" in run {
        ...
        val view  = ByteView(attrs)
        Abort.run[ReflectError](AttributeUnpickler.read(view, Array.empty)).map { result =>
            result match
                case Result.Success(fa) =>
                    assert(fa.explicitNulls)
                    assert(!fa.isJava)
    ...
    "SOURCEFILEattr decodes to Present(sourceFileName) when present" in run {
        ...
        val names: Array[Reflect.Name] = Array(Reflect.Name.wrap(entry))
        ...
        val view = ByteView(attrs)
        Abort.run[ReflectError](AttributeUnpickler.read(view, names)).map { result =>
            result match
                case Result.Success(fa) =>
