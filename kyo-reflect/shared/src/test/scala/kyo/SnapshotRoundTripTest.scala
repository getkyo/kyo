...
class SnapshotRoundTripTest extends Test:
    ...
    "DigestComputer.compute on jar root returns same digest for two successive calls" taggedAs jvmOnly in run {
    ...
    "DigestComputer.compute detects jar mtime change (+1 hour offset)" taggedAs jvmOnly in run {
        import java.io.FileOutputStream
        import java.nio.file.Files
    ...
    "DigestComputer.compute detects jar size change (rewrite with extra entry)" taggedAs jvmOnly in run {
        import java.io.FileOutputStream
        import java.nio.file.Files
        import java.nio.file.attribute.FileTime
        import java.time.Instant
    ...
    "DigestComputer.compute on mixed jar+directory roots is root-order independent" taggedAs jvmOnly in run {
        import java.io.FileOutputStream
        import java.nio.file.Files
        import java.util.zip.{ZipEntry, ZipOutputStream}
        ...
        .flatMap: jarPath =>
            Scope.ensure(Sync.defer(Files.deleteIfExists(jarPath): Unit)).andThen:
                ...
                .flatMap: _ =>
                    ...
                    // Combined source: delegates jar stat/list to PlatformFileSource, memory paths to MemoryFileSource
                    val platSrc = PlatformFileSource.get
