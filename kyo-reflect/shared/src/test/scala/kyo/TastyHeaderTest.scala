...
class TastyHeaderTest extends Test:
    ...
    "reading wrong magic 0xDEADBEEF produces CorruptedFile error" in run {
        ...
        Abort.run[ReflectError] {
            ...
            result match
                case Result.Failure(ReflectError.CorruptedFile(_, _, _)) =>
                    succeed
                case other =>
    ...
    "reading major=99 produces UnsupportedVersion error" in run {
        ...
        Abort.run[ReflectError] {
            ...
            result match
                case Result.Failure(ReflectError.UnsupportedVersion(found, supported)) =>
    ...
    "reading minor=9 experimental=0 produces UnsupportedVersion (forward incompatible)" in run {
        ...
        Abort.run[ReflectError] {
            ...
            result match
                case Result.Failure(ReflectError.UnsupportedVersion(found, _)) =>
    ...
    "reading truncated header produces MalformedSection error" in run {
        ...
        Abort.run[ReflectError] {
            ...
            result match
                case Result.Failure(ReflectError.MalformedSection(_, _)) =>
                    succeed
                case other =>
