...
class ClasspathOrchestratorPipelineTest extends Test:
        ...
        Scope.run:
            Abort.run[ReflectError](openFixtureClasspath(fixtureSource()).flatMap: cp =>
                ...
                case Result.Success(Present(sym)) =>
                    assert(
                        sym.kind == Reflect.SymbolKind.Class || sym.kind == Reflect.SymbolKind.Trait,
            ...
            Abort.run[ReflectError](
                openFixtureClasspath(fixtureSource()).flatMap: cp1 =>
                    openFixtureClasspath(fixtureSource()).flatMap: cp2 =>
                        val raw1 = Reflect.Classpath.unwrap(cp1)
                        val raw2 = Reflect.Classpath.unwrap(cp2)
                        Sync.defer:
    ...
    // T5: strict-fail file error - open with strict=true and a corrupted .tasty raises Abort[ReflectError].
    // The call must NOT hang.
    "T5: strict mode raises Abort[ReflectError] for corrupted tasty file without hanging" in run {
        val src = MemFileSource()
        ...
        Scope.run:
            Abort.run[ReflectError](openFixtureClasspath(src, strict = true)).map:
                case Result.Failure(_: ReflectError) =>
                    succeed
                case Result.Success(_) =>
                    fail("T5: Expected Abort[ReflectError] for corrupted tasty in strict mode, but got success")
                case Result.Panic(t) =>
                    throw t
    }
    ...
    "T6: pipeline completes successfully with 100+ entries at concurrency=2" in run {
        ...
        Scope.run:
            Abort.run[ReflectError](openFixtureClasspath(src, concurrency = 2).flatMap: cp =>
                Sync.defer(Reflect.Classpath.unwrap(cp).allSymbols.size)).map:
