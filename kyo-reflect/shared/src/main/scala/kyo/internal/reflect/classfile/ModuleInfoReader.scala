...
/** Reads a JVM 9+ module-info.class file and produces a Reflect.ModuleDescriptor.
  *
...
object ModuleInfoReader:
    ...
    /** Read a module-info.class from raw bytes.
      *
      * Returns a Reflect.ModuleDescriptor on success. Fails with ReflectError.ClassfileFormatError if: - The magic number is not
      * 0xCAFEBABE. - The major version is less than 53 (Java 9). - The Module attribute is missing or malformed. - Any required constant
      * pool index is out of range.
      *
    ...
    private[classfile] def readFrom(
        ...
        interner: Interner,
        path: String
    )(using Frame): Reflect.ModuleDescriptor < (Sync & Abort[ReflectError]) =
    ...
    private def readAttributesForModule(
        ...
        total: Int,
        idx: Int,
        found: Maybe[Reflect.ModuleDescriptor]
    )(using Frame): Reflect.ModuleDescriptor < (Sync & Abort[ReflectError]) =
        if idx >= total then
            found match
                case Present(desc) => desc
                case Absent =>
                    Abort.fail(ReflectError.ClassfileFormatError(path, "No Module attribute found in module-info.class"))
        else
    ...
    private def resolveVersion(
        ...
        idx: Int,
        path: String
    )(using Frame): Maybe[String] < (Sync & Abort[ReflectError]) =
    ...
    private def readRequires(
        ...
        total: Int,
        idx: Int,
        acc: Chunk[Reflect.ModuleRequires]
    )(using Frame): Chunk[Reflect.ModuleRequires] < (Sync & Abort[ReflectError]) =
        if idx >= total then acc
            ...
            resolveModuleName(pool, requiresIdx, path).flatMap: name =>
                resolveVersion(pool, requiresVerIdx, path).flatMap: version =>
                    ...
                    val req           = Reflect.ModuleRequires(name, version, isTransitive, isStaticPhase)
    ...
    private def readExports(
        ...
        total: Int,
        idx: Int,
        acc: Chunk[Reflect.ModuleExports]
    )(using Frame): Chunk[Reflect.ModuleExports] < (Sync & Abort[ReflectError]) =
        if idx >= total then acc
        else
    ...
    private def readOpens(
        ...
        total: Int,
        idx: Int,
        acc: Chunk[Reflect.ModuleOpens]
    )(using Frame): Chunk[Reflect.ModuleOpens] < (Sync & Abort[ReflectError]) =
        if idx >= total then acc
        else
    ...
    private def readProvides(
        ...
        total: Int,
        idx: Int,
        acc: Chunk[Reflect.ModuleProvides]
    )(using Frame): Chunk[Reflect.ModuleProvides] < (Sync & Abort[ReflectError]) =
        if idx >= total then acc
        else
            ...
            pool.classRef(providesIdx).flatMap: interfaceName =>
                readClassRefs(view, pool, path, providesWithCount, 0, Chunk.empty).flatMap: impls =>
                    val prov = Reflect.ModuleProvides(interfaceName.replace('/', '.'), impls)
