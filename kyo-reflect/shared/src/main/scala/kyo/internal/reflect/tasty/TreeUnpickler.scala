...
import kyo.*
import kyo.Reflect.Name.asString
...
object TreeUnpickler:
    ...
    private def readTree(view: ByteView, ctx: DecodeCtx): Reflect.Tree =
    ...
    private def readTreesUntil(view: ByteView, end: Int, ctx: DecodeCtx): Chunk[Reflect.Tree] =
        val buf = new mutable.ArrayBuffer[Reflect.Tree]()
    ...
    private def decodeTreeTag(tag: Int, startAddr: Int, view: ByteView, ctx: DecodeCtx): Reflect.Tree =
        tag match

            // ── Category 1: tag only (1-59) -- literal constants & modifiers ─────
            case TastyFormat.UNITconst  => Reflect.Tree.Literal(Reflect.Constant.UnitConst)
            case TastyFormat.FALSEconst => Reflect.Tree.Literal(Reflect.Constant.BooleanConst(false))
            case TastyFormat.TRUEconst  => Reflect.Tree.Literal(Reflect.Constant.BooleanConst(true))
            case TastyFormat.NULLconst  => Reflect.Tree.Literal(Reflect.Constant.NullConst)

            // ── Category 2: tag + Nat (60-89) ────────────────────────────────────

            case TastyFormat.SHAREDterm =>
                val addr = view.readNat()
                ctx.treeAddrCache.getOrElse(addr, Reflect.Tree.Unknown(TastyFormat.SHAREDterm, 0))

            case TastyFormat.TERMREFdirect =>
                val addr = view.readNat()
                val sym  = ctx.addrMap.getOrElse(addr, makeUnresolvedSym(s"termref@$addr", ctx.home))
                Reflect.Tree.Ident(sym.name, Reflect.Type.Named(sym))

            case TastyFormat.TERMREFpkg =>
                val nameRef = view.readNat()
                val name    = nameFromRef(nameRef, ctx)
                Reflect.Tree.Ident(name, Reflect.Type.Named(makeUnresolvedSym(name.asString, ctx.home)))

            case TastyFormat.BYTEconst =>
                Reflect.Tree.Literal(Reflect.Constant.ByteConst(view.readNat().toByte))
            case TastyFormat.SHORTconst =>
                Reflect.Tree.Literal(Reflect.Constant.ShortConst(view.readNat().toShort))
            case TastyFormat.CHARconst =>
                Reflect.Tree.Literal(Reflect.Constant.CharConst(view.readNat().toChar))
            case TastyFormat.INTconst =>
                Reflect.Tree.Literal(Reflect.Constant.IntConst(view.readInt()))
            case TastyFormat.LONGconst =>
                Reflect.Tree.Literal(Reflect.Constant.LongConst(view.readLongNat()))
            case TastyFormat.FLOATconst =>
                Reflect.Tree.Literal(Reflect.Constant.FloatConst(java.lang.Float.intBitsToFloat(view.readNat())))
            case TastyFormat.DOUBLEconst =>
                Reflect.Tree.Literal(Reflect.Constant.DoubleConst(java.lang.Double.longBitsToDouble(view.readLongNat())))
            case TastyFormat.STRINGconst =>
                val nameRef = view.readNat()
                Reflect.Tree.Literal(Reflect.Constant.StringConst(nameFromRef(nameRef, ctx).asString))
            ...
            case TastyFormat.THIS =>
                ...
                val sym = tpe match
                    case Reflect.Type.Named(s)    => s
                    case Reflect.Type.ThisType(s) => s
                    case _                        => makeUnresolvedSym("this-class", ctx.home)
                Reflect.Tree.This(sym)
            ...
            case TastyFormat.CLASSconst =>
                val tpe = readType(view, ctx)
                Reflect.Tree.Literal(Reflect.Constant.ClassConst(tpe))

            case TastyFormat.NEW =>
                val tpe = readType(view, ctx)
                Reflect.Tree.New(tpe)

            case TastyFormat.THROW =>
                val expr = readTree(view, ctx)
                Reflect.Tree.Throw(expr)
            ...
            case TastyFormat.SELECT =>
                ...
                Reflect.Tree.Select(
                    qualifier,
                    name,
                    Reflect.Type.Wildcard(
                        Reflect.Type.Named(makeUnresolvedSym("lo", ctx.home)),
                        Reflect.Type.Named(makeUnresolvedSym("hi", ctx.home))
                    )
                )
            ...
            case TastyFormat.TERMREF =>
                ...
                val name    = nameFromRef(nameRef, ctx)
                Reflect.Tree.Select(Reflect.Tree.Ident(name, prefix), name, prefix)

            case TastyFormat.NAMEDARG =>
                val nameRef = view.readNat()
                val value   = readTree(view, ctx)
                val name    = nameFromRef(nameRef, ctx)
                Reflect.Tree.NamedArg(name, value)
            ...
            case TastyFormat.TYPEDEF =>
                ...
                if view.position < end && (view.peekByte(view.position) & 0xff) == TastyFormat.TEMPLATE then
                    ...
                    view.goto(end)
                    Reflect.Tree.TypeDef(sym, rhs)
                end if

            case TastyFormat.IMPORT =>
                val end = view.readEnd()
                view.goto(end)
                Reflect.Tree.Unknown(TastyFormat.IMPORT, end - startAddr)
            ...
            case TastyFormat.TYPEAPPLY =>
                ...
                view.goto(end)
                Reflect.Tree.TypeApply(fun, typeArgs)
            ...
            case TastyFormat.IF =>
                ...
                view.goto(end)
                Reflect.Tree.If(cond, thenp, elsep)
            ...
            case TastyFormat.LAMBDA =>
                ...
                view.goto(end)
                Reflect.Tree.Lambda(method, tpe)
            ...
            case TastyFormat.MATCH =>
                ...
                view.goto(end)
                Reflect.Tree.Match(selector, cases)
            ...
            case TastyFormat.TRY =>
                ...
                view.goto(end)
                Reflect.Tree.Try(expr, cases, fin)
            ...
            case TastyFormat.INLINED =>
                ...
                view.goto(end)
                val (bindings, body) =
                    if remaining.isEmpty then (Chunk.empty[Reflect.Tree], Reflect.Tree.Unknown(TastyFormat.INLINED, 0))
                    else (remaining.dropRight(1), remaining.last)
                Reflect.Tree.Inlined(call, bindings, body)

            case TastyFormat.SELECTouter =>
                val end = view.readEnd()
                view.goto(end)
                Reflect.Tree.Unknown(TastyFormat.SELECTouter, end - startAddr)
            ...
            case TastyFormat.REPEATED =>
                ...
                Reflect.Tree.Apply(
                    Reflect.Tree.Ident(Reflect.Name("_repeated"), Reflect.Type.Named(makeUnresolvedSym("repeated", ctx.home))),
                    trees
                )
            ...
            case TastyFormat.UNAPPLY =>
                ...
                view.goto(end)
                Reflect.Tree.Unapply(fun, implicits, patterns)
            ...
            case TastyFormat.ANNOTATEDtype | TastyFormat.ANNOTATEDtpt =>
                ...
                view.goto(end)
                Reflect.Tree.Annotated(expr, ann)
    ...
    private def readTemplate(view: ByteView, ctx: DecodeCtx): Reflect.Tree.Template =
        // Consume tag + length prefix.
        val end                            = view.readEnd()
        val parents                        = new mutable.ArrayBuffer[Reflect.Tree]()
        var selfSym: Maybe[Reflect.Symbol] = Maybe.Absent
        val bodyBuf                        = new mutable.ArrayBuffer[Reflect.Tree]()
        ...
        view.goto(end)
        Reflect.Tree.Template(Chunk.from(parents.toSeq), selfSym, Chunk.from(bodyBuf.toSeq))
    end readTemplate
    ...
    /** Read a TEMPLATE body from a slice whose `end` is already known (no length prefix to consume).
      ...
      */
    private def readTemplateBody(view: ByteView, end: Int, ctx: DecodeCtx): Reflect.Tree.Template =
        val parents                        = new mutable.ArrayBuffer[Reflect.Tree]()
        var selfSym: Maybe[Reflect.Symbol] = Maybe.Absent
        val bodyBuf                        = new mutable.ArrayBuffer[Reflect.Tree]()
        var inParents                      = true
    ...
    private def readCaseDefs(view: ByteView, end: Int, ctx: DecodeCtx): Chunk[Reflect.Tree.CaseDef] =
        val buf = new mutable.ArrayBuffer[Reflect.Tree.CaseDef]()
        while view.position < end do
            val peek = view.peekByte(view.position) & 0xff
            if peek == TastyFormat.CASEDEF then
                ...
                val cd = Reflect.Tree.CaseDef(pattern, guard, body)
    ...
    private def readCaseDefGuardAndBody(view: ByteView, end: Int, ctx: DecodeCtx): (Maybe[Reflect.Tree], Reflect.Tree) =
        ...
        if view.position >= end then
            (Maybe.Absent, Reflect.Tree.Unknown(0, 0))
        else
    ...
    private def readUnapplyParts(
        ...
        ctx: DecodeCtx
    ): (Chunk[Reflect.Tree], Chunk[Reflect.Tree]) =
        val implicits = new mutable.ArrayBuffer[Reflect.Tree]()
        val patterns  = new mutable.ArrayBuffer[Reflect.Tree]()
        while view.position < end do
            val peek = view.peekByte(view.position) & 0xff
    ...
    private def readDefDefParamsAndTpt(
        ...
        ctx: DecodeCtx
    ): (Chunk[Chunk[Reflect.Tree]], Reflect.Type) =
        val paramss = new mutable.ArrayBuffer[Chunk[Reflect.Tree]]()
        var tpt     = Reflect.Type.Named(makeUnresolvedSym("unknown-tpt", ctx.home)): Reflect.Type
        while view.position < end do
            val peek = view.peekByte(view.position) & 0xff
            peek match
    ...
    private def readOneParamClause(view: ByteView, end: Int, ctx: DecodeCtx): Chunk[Reflect.Tree] =
        val buf = new mutable.ArrayBuffer[Reflect.Tree]()
        while view.position < end do
            val peek = view.peekByte(view.position) & 0xff
    ...
    private def readType(view: ByteView, ctx: DecodeCtx): Reflect.Type =
        TypeUnpickler.readTypeForTree(view, ctx.typeSession)
    end readType

    /** Read type nodes until position reaches end. */
    private def readTypesUntil(view: ByteView, end: Int, ctx: DecodeCtx): Chunk[Reflect.Type] =
    ...
    private def readTypeOrSkip(view: ByteView, end: Int, ctx: DecodeCtx): Reflect.Type =
        skipModifierTags(view, end)
        if view.position < end then
            val peek = view.peekByte(view.position) & 0xff
            if isTypeTag(peek) then
                ...
                Reflect.Type.Named(makeUnresolvedSym("unknown-tpt", ctx.home))
            end if
        else
            Reflect.Type.Named(makeUnresolvedSym("unknown-tpt", ctx.home))
        end if
    ...
    /** Read an optional rhs tree: the first remaining content after modifiers, or Absent.
      ...
      */
    private def readOptionalRhs(view: ByteView, end: Int, ctx: DecodeCtx): Maybe[Reflect.Tree] =
        skipModifierTags(view, end)
        if view.position < end then Maybe(readTree(view, ctx))
        else Maybe.Absent
    end readOptionalRhs
    ...
    private def extractPackageName(view: ByteView, ctx: DecodeCtx): Reflect.Name =
        val tag = view.readByte() & 0xff
        tag match
            case TastyFormat.TERMREFpkg =>
    ...
    private def nameFromRef(nameRef: Int, ctx: DecodeCtx): Reflect.Name =
        if nameRef >= 0 && nameRef < ctx.names.length then ctx.names(nameRef)
        else Reflect.Name(s"name@$nameRef")
    end nameFromRef

    private def makeUnresolvedSym(name: String, home: ClasspathRef): Reflect.Symbol =
