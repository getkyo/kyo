...
/** An index of TASTy section headers, mapping section name to byte range within the file.
  ...
  * }}}
  *
  * NameRefs in the section table are 0-based indices into the `Array[Reflect.Name]` produced by `NameUnpickler`.
  */
...
object SectionIndex:
    ...
    /** Read the section table from `view`, resolving section names via the already-decoded `names` array.
      ...
      */
    def read(view: ByteView, names: Array[Reflect.Name])(using Frame): SectionIndex < (Sync & Abort[ReflectError]) =
        val result =
            try Right(readSync(view, names))
            catch
                case _: ArrayIndexOutOfBoundsException =>
                    Left(ReflectError.MalformedSection("SectionIndex", "unexpected end while reading section headers"))
        result match
            case Right(idx) => Sync.defer(idx)
            case Left(err)  => Abort.fail(err)
    end read

    private def readSync(view: ByteView, names: Array[Reflect.Name]): SectionIndex =
