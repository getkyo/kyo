...
/** Decodes TASTy constant leaf nodes into `Reflect.Constant` values.
  *
  * TASTy constant tags:
  ...
  *
  * For `CLASSconst`: the type reference embedded in TASTy is decoded to produce `ClassConst(typeRef: Reflect.Type)` without resolving the
...
object Constant:
    ...
    private val classConstSentinel: Reflect.Symbol =
        Reflect.Symbol.make(
    ...
    /** Decode the constant whose tag byte has already been read. The `view` cursor is positioned immediately after the tag.
      ...
      */
    def fromTastyTag(tag: Int, view: ByteView, names: Array[Reflect.Name])(using Frame): Reflect.Constant < (Sync & Abort[ReflectError]) =
        val result =
            try Right(decodeConstant(tag, view, names))
            catch
                case _: ArrayIndexOutOfBoundsException =>
                    Left(ReflectError.MalformedSection("ASTs", s"unexpected end while reading constant tag $tag"))
        result match
            case Right(c)  => Sync.defer(c)
            case Left(err) => Abort.fail(err)
    end fromTastyTag

    private def decodeConstant(tag: Int, view: ByteView, names: Array[Reflect.Name]): Reflect.Constant =
        tag match
            case TastyFormat.UNITconst  => Reflect.Constant.UnitConst
            case TastyFormat.FALSEconst => Reflect.Constant.BooleanConst(false)
            case TastyFormat.TRUEconst  => Reflect.Constant.BooleanConst(true)
            case TastyFormat.NULLconst  => Reflect.Constant.NullConst
            case TastyFormat.BYTEconst =>
                val v = view.readNat()
                Reflect.Constant.ByteConst(v.toByte)
            case TastyFormat.SHORTconst =>
                val v = view.readNat()
                Reflect.Constant.ShortConst(v.toShort)
            case TastyFormat.CHARconst =>
                val v = view.readNat()
                Reflect.Constant.CharConst(v.toChar)
            case TastyFormat.INTconst =>
                val v = view.readInt()
                Reflect.Constant.IntConst(v)
            case TastyFormat.LONGconst =>
                val lo = view.readLongNat()
                Reflect.Constant.LongConst(lo)
            case TastyFormat.FLOATconst =>
                val bits = view.readNat()
                Reflect.Constant.FloatConst(java.lang.Float.intBitsToFloat(bits))
            case TastyFormat.DOUBLEconst =>
                val bits = view.readLongNat()
                Reflect.Constant.DoubleConst(java.lang.Double.longBitsToDouble(bits))
            case TastyFormat.STRINGconst =>
                val nameRef = view.readNat()
                Reflect.Constant.StringConst(names(nameRef).asString)
            case TastyFormat.CLASSconst =>
                ...
                Reflect.Constant.ClassConst(Reflect.Type.Named(classConstSentinel))
            case other =>
