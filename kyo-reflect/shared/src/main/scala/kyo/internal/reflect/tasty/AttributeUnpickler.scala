...
object AttributeUnpickler:
    ...
    /** Read the Attributes section from `view`.
      ...
      * this array.
      */
    def read(view: ByteView, names: Array[Reflect.Name])(using Frame): FileAttributes < (Sync & Abort[ReflectError]) =
        val result =
            try Right(readSync(view, names))
            catch
                case _: ArrayIndexOutOfBoundsException =>
                    Left(ReflectError.MalformedSection("Attributes", "unexpected end of attributes section"))
                case e: UnknownTagException =>
                    Left(ReflectError.MalformedSection("Attributes", s"Unknown attribute tag ${e.tag} at position ${e.pos}"))
        result match
            case Right(fa) => Sync.defer(fa)
            case Left(err) => Abort.fail(err)
    end read

    private def readSync(view: ByteView, names: Array[Reflect.Name]): FileAttributes =
