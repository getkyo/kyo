...
class WorkerTest extends AnyFreeSpec with NonImplicitAssertions with Eventually with org.scalatest.BeforeAndAfterEach {
    ...
    "checkAvailability" - {
        ...
        "a task that re-arms itself while running is not billed for that run" in {
            // The self-re-arming driver-loop shape: one reusable Task whose run() parks and then, as its last act, re-arms itself at the
            // fresh-task priority so the loop competes on equal terms with newly submitted work rather than sorting last forever. The park
            // happens INSIDE the measured run, which is exactly why the re-arm exists (see PollerIoDriver.reArm's scaladoc).
            //
            // runTask bills wall-clock time in a finally that runs AFTER run() returns, so without Task.rearm's release it lands on top of
            // the chosen priority and undoes it. WorkerQueue is a min-heap on LIVE runtime() reads, so the task would then sit in its next
            // queue keyed by the whole park while every fresh runtime-1 task sorts ahead of it.
            val worker     = createWorker(executor = executor)
            val parkMs     = 120L
            var self: Task = null
            val selfArming = TestTask(_run = () => {
                Thread.sleep(parkMs)
                // Exactly what a driver's reArm does as the last thing inside run().
                self.rearm()
                Task.Done
            })
            self = selfArming
            worker.enqueue(selfArming)
            // The billing finally runs only after run() returns, so reading the runtime right after the park would pass vacuously. Gate on a
            // second task instead: the worker runs its queue one task at a time on its carrier, so this one starts only once the billing
            // decision for the park has already been made.
            val next = TestTask()
            worker.enqueue(next)
            eventually(assert(next.executions == 1))
            assert(selfArming.executions == 1)
            assert(
                selfArming.runtime() <= 2,
                s"the in-run re-arm must survive: runtime is ${selfArming.runtime()}, i.e. the ~${parkMs}ms park was billed on top of " +
                    "it, so this task now sorts behind every freshly submitted one in whatever queue it was re-armed into"
            )
        }

        "a re-armed task is not billed even once a successor carrier is already running it" in {
            // The re-arm hands the successor to the scheduler from inside run(), so another carrier can start the next cycle before the
            // previous carrier reaches its billing finally. The release must survive that overlap: a signal the successor clears on entry
            // would already be cleared when the previous carrier checks, and the park would be billed after all.
            val workerA    = createWorker(executor = executor)
            val workerB    = createWorker(executor = executor)
            val parkMs     = 120L
            val successor  = new CountDownLatch(1) // the next cycle has begun on workerB
            val release    = new CountDownLatch(1) // lets that cycle finish, after the assertion
            val cycles     = new AtomicInteger(0)
            var self: Task = null
            val loop = TestTask(_run = () => {
                if (cycles.incrementAndGet() == 1) {
                    Thread.sleep(parkMs) // the park workerA would bill
                    self.rearm()
                    workerB.enqueue(self)
                    assert(successor.await(5, TimeUnit.SECONDS))
                    Task.Done
                } else {
                    successor.countDown()
            ...
            self = loop
            workerA.enqueue(loop)
            assert(successor.await(5, TimeUnit.SECONDS))

            // workerA's billing decision is made in the finally of the first cycle, so gate on a task that can only run after it.
            val probe = TestTask()
            workerA.enqueue(probe)
            eventually(assert(probe.executions == 1))
            assert(
                loop.runtime() <= 2,
                s"runtime is ${loop.runtime()}: the previous carrier billed the ~${parkMs}ms park to a task the next carrier already owns"
            )
            release.countDown()
            eventually(assert(loop.executions == 2))
        }
