...
/** Monitors worker threads for CPU stalls and dispatches thread interrupts when fibers are interrupted.
  *
  * A thread is "stalled" when its user CPU time stops advancing — it is blocked in a kernel operation (I/O, sleep, lock) rather than
  ...
  * same value, the thread is stalled. This catches all forms of blocking including cases where Thread.getState() is misleading — socket
  ...
  * (excluding kernel time) is used where available, which also identifies threads spinning in kernel locks (contended futex/mutex) as
  * stalled.
  *
  ...
  *
  * Stalled workers have their `blocked` flag set. The scheduler reads this in checkAvailability to stop routing new tasks to blocked
  * workers, transitioning them to Stalled state and draining their queues to other workers.
  *
  * ==Interrupt dispatch==
  *
  ...
  * worker's thread is stalled, Thread.interrupt() is dispatched to wake it from the blocking operation. The monitor re-interrupts on
  ...
  *
  *   - '''intervalNs''' (default 2ms, `-Dkyo.scheduler.cpuStallMonitorIntervalNs`): the periodic scan cadence for blocking compensation.
  *   - '''minIntervalNs''' (default 500μs, `-Dkyo.scheduler.cpuStallMonitorMinIntervalNs`): the minimum time between scans, controlling the
  *     fastest response to wake() for interrupt dispatch.
  *
  * Safety against false positives is handled by pressure-scaled stall thresholds, not by the scan interval. Each cycle, the monitor
  ...
  * worker threads are likely CPU-starved too, and flat CPU time doesn't mean "blocked." The stall threshold scales proportionally with this
  * pressure: under normal load (pressure ~1.0), the base threshold (default 2) applies and detection is fast. Under heavy load (e.g.,
  * pressure 5.0), the threshold scales to 10, requiring more consecutive idle samples before marking a worker as stalled. Truly blocked
  ...
  */
private[scheduler] class CPUStallMonitor(
    ...
    /** Test-only constructor for unit testing sample/isStalled without workers or a monitor thread. */
    ...
    private val intervalNs = Flag("cpuStallMonitorIntervalNs", 2000000)
    ...
    // pressure-scaled stall threshold handles load-dependent false positives on top of this.
    // Can be overridden via system property.
    private val minIntervalNs: Long = {
        val override_ = Flag("cpuStallMonitorMinIntervalNs", -1)
    ...
    // Base stall threshold: consecutive idle samples required before marking a worker as stalled.
    // Dynamically scaled by scheduling pressure (see effectiveStallThreshold).
    private val stallThreshold = Flag("cpuStallMonitorStallThreshold", 2)
    ...
    private val stalledFlags  = new Array[Boolean](maxWorkers)
    private val stallCounts   = new Array[Int](maxWorkers)
    ...
    // threads is expected (not a stall). The threshold scales proportionally so truly blocked
    ...
    private var effectiveStallThreshold: Int = stallThreshold
                    ...
                    () => {
                        ...
                        while (!thread.isInterrupted()) {
                            ...
                            if (elapsed >= minIntervalNs) {
                                // Measure scheduling pressure: ratio of actual delay to requested delay.
                                // If the OS couldn't schedule us on time, worker threads are likely
                                // CPU-starved too, and flat CPU time doesn't mean "blocked."
                                val pressure = elapsed.toDouble / intervalNs
                                effectiveStallThreshold = Math.max(stallThreshold, (stallThreshold * pressure).toInt)
                                cycle()
                                lastCycleNanos = System.nanoTime()
                                LockSupport.parkNanos(intervalNs)
                            } else {
                                // Woken too soon by wake() — park until minInterval elapses
    ...
    private def cycle(): Unit = {
        try {
            ...
            case ex if NonFatal(ex) =>
                bug(s"CPU stall monitor has failed.", ex)
        }
    }

    /** Samples CPU time and updates stall state for the given thread IDs. Used by tests to drive the monitor without workers. */
    ...
    private[scheduler] def isStalled(position: Int): Boolean =
        stalledFlags(position)
    ...
    // Single pass: detect stalls from CPU time samples, set blocked flags, dispatch interrupts.
    // Requires stallThreshold consecutive idle samples before marking a worker as stalled,
    ...
    @tailrec private def process(count: Int, i: Int): Unit =
        if (i < count) {
            ...
            val stalled = stallCounts(pos) >= effectiveStallThreshold
            val worker  = workers(pos)
            worker.blocked = stalled
            if (stalled) {
                val task = tasks(i)
    ...
    @tailrec private def detectOnly(count: Int, i: Int): Unit =
        if (i < count) {
            ...
            val lastTime = lastUserTimes(i)
            lastUserTimes(i) = userTime
            stalledFlags(i) = userTime >= 0 && lastTime >= 0 && userTime == lastTime
