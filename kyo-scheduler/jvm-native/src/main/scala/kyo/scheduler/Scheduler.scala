...
final class Scheduler(
    ...
    private val cpuStallMonitor = new CPUStallMonitor(workers, () => currentWorkers, maxWorkers, timerExecutor)

    /** Notifies the CPU stall monitor that a fiber was interrupted, triggering an immediate scan. */
    def notifyInterrupt(): Unit = cpuStallMonitor.wake()
