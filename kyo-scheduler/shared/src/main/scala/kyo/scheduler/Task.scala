...
trait Task {
    ...
    /** Marks this task for thread interrupt dispatch by the WorkerMonitor.
      *
      ...
      * the next effect boundary. The WorkerMonitor periodically checks this flag and, if the worker thread's CPU time is flat (thread is
...
object Task {
    ...
    /** Bit-packed task state encoding preemption, interrupt request, and runtime priority in a single Int.
      ...
      *   - '''bit 0''': needsInterrupt — whether the WorkerMonitor should dispatch Thread.interrupt() for this task
