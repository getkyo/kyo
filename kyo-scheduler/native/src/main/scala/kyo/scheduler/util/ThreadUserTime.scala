...
/** Queries per-thread CPU time for the CPUStallMonitor on Scala Native.
  *
  ...
  * CPUStallMonitor then passes these handles to userTimes() for cross-thread CPU time queries.
  *
  ...
  *     Returns user CPU time only (excludes kernel time), matching JVM behavior. Correctly detects kernel-level spinning as stalled.
  *   - '''Linux''': Uses `pthread_getcpuclockid` + `clock_gettime` to query the thread's CPU clock. Returns total CPU time (user + kernel)
  *     because cross-thread user-only time is not available without parsing `/proc/self/task/<tid>/stat`. This means kernel-level spinning
  *     (contended futex) won't be detected as stalled on Linux Native. This is a known limitation; kernel spinning is an uncommon case and
  *     the mechanism still correctly detects all I/O and sleep blocking.
  *
...
private[scheduler] object ThreadUserTime {
    ...
    /** Returns the native pthread handle for the current thread.
      ...
      * mount and stores the result as mountId for cross-thread queries by the CPUStallMonitor.
      */
    ...
    /** Linux: Get thread-specific CPU clock, then query clock_gettime. Returns total CPU time (user + kernel) — no cross-thread user-only
      * API available.
      */
