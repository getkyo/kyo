...
/** JVM-only static-site generator: emits the landing page, the per-version docs routes, the
  ...
  *   <outDir>/sitemap.xml     -- canonical-indexable route set
  *   <outDir>/robots.txt      -- allow-all + sitemap directive
...
object WebsiteGenerator:
    ...
    def emit(
        ...
        for
            // sitemap `<lastmod>` is the build date, computed ONCE here and threaded
    ...
    private def emitDocs(
        ...
        for
            ...
            // the bundle's first paint are byte-identical (hydration parity).
    ...
    private def emitVersion(
        ...
        // the current-latest version's own versioned tree (/v<X>/<slug>/) duplicates
    ...
    /** The version to serve as `latest`. The caller (`WebsiteMain.parseContent`) marks the CURRENT
      * version with `version.latest = true`, so that explicit flag wins (the forward-only build always
      * flags exactly the current version). When no version carries the flag, fall back to the newest
      * stable (non-pre-release) version, then to the newest pre-release. The content `Chunk` is ordered
      * oldest-first by semantic version (any appended snapshots first, the current version last), so
      * "newest" is the last matching entry.
      */
    private def pickLatest(content: Chunk[WebsiteContent]): Maybe[WebsiteContent] =
        ...
        if flagged.nonEmpty then flagged.lastMaybe
            ...
            // pre-release suffix. Reusing it keeps ONE definition of "stable" across the generator,
            // instead of a second substring-marker list that could drift.
    ...
    private def siteShell(versions: Chunk[WebsiteVersion], docsHome: String, body: UI)(using Frame): UI < Sync =
        for
            ...
            view <- SiteApp.view(
                ...
                Kyo.unit,
                // No version switch at build time: the static page renders the native <select> with the
                // current version pre-selected, and its change handler only fires in the live bundle.
                (_: String) => Kyo.unit,
    ...
    private def docOpts(
        ...
        // The current-latest version's own versioned module pages (/v<X>/<slug>/) duplicate
        // /latest/<slug>/, so they canonicalize to the /latest/ home instead of self. The /latest/
        // tree (prefix == "latest") and every other versioned/intro page stays self-canonical.
        val canonical =
    ...
    /** Build the page options for the intro/overview route `/<prefix>/`. The intro now renders the
      ...
      *     to `/latest/` (the same dedup rule the module pages follow) and stays out of the sitemap.
      *     A past-version intro is self-canonical.
      *
    ...
    /** Emit a styled `404.html` at the site root. GitHub Pages serves this file for any unknown
    ...
    /** The canonical-indexable route set: the landing root `/`, the `/latest/` overview (the
      * root-README intro, now real indexable content), plus each `/latest/<slug>/` for the
    ...
    /** Build the sitemap.xml document. `routes` are absolute-URL paths (each starting with `/`);
      * `lastmod` is a `YYYY-MM-DD` build date. Pure and deterministic so tests can
    ...
    /** Build the robots.txt document: allow all crawlers (no `Disallow`, so AI crawlers can read the
      * server-rendered HTML) and declare the sitemap URL. Pure.
      */
