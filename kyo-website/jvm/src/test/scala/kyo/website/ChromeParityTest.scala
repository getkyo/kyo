...
/** Verifies hydration parity for the unified `SiteApp` shell on BOTH a landing route and a
...
class ChromeParityTest extends WebsiteTest:
    ...
    "SiteApp landing-route shell: SSG vs RecordingBackend parity" in {
        for
        ...
        yield assert(
            ...
            "SSG (runRender) and mount (RecordingBackend) must produce identical HTML for the landing route shell"
        )
        end for
    }

    "SiteApp docs-route shell: SSG vs RecordingBackend parity" in {
        ...
        yield
            assert(
                normalize(ssg) == normalize(mount),
                "SSG (runRender) and mount (RecordingBackend) must produce identical HTML for the docs route shell"
            )
    ...
    "SiteApp intro/overview-route shell: SSG vs RecordingBackend parity" in {
        ...
        yield
            assert(
                normalize(ssg) == normalize(mount),
                "SSG (runRender) and mount (RecordingBackend) must produce identical HTML for the intro/overview route shell"
            )
    ...
    "parity holds for the header version <select> subtree under SiteApp" in {
        for
            body  <- LandingApp.body(docsHomeRoute)
            view  <- siteShell(versions2, docsHomeRoute, body)
            ssg   <- UI.runRender(view).take(1).run.map(_.headMaybe.getOrElse(""))
        ...
        yield
            ...
            // Both outputs must contain the version <select> markup for both version labels.
            ...
            // The version selector renders as a native <select> (not a div-based dropdown).
            val ssgSelStart   = ssgNorm.indexOf("<select")
            val mountSelStart = mountNorm.indexOf("<select")
            assert(ssgSelStart >= 0, "SSG must have a native <select> element")
            assert(mountSelStart >= 0, "mount must have a native <select> element")
            val closeTag    = "</select>"
            ...
            assert(ssgSel == mountSel, s"version <select> subtree must be identical between SSG and mount; ssg=$ssgSel mount=$mountSel")
        end for
    }

    // ---- Docs shell body parity ----
    ...
    "same Markdown gives byte-identical article in two runRenderPage calls" in {
        val src = "## Scope\n\n- item one\n- item two\n"
        for
        ...
        yield assert(
            html1 == html2,
            s"Two runRenderPage calls with the same Markdown must produce identical HTML"
        )
        end for
    }

    // SSG article HTML equals the client-injected article HTML.
    ...
    // injection is byte-identical to the SSG article string.
    "SSG article HTML equals injected article HTML (RecordingBackend rawHtml path)" in {
        val src = "# Title\n## Section\n\nCode: ```scala\nval x = true\nval n = null\n```\n"
        for
        ...
        yield assert(
            ...
            s"SSG article HTML and injected rawHtml must be byte-identical after normalization\nSSG: $ssgHtml\nInjected: $injectedHtml"
        )
        end for
    }
