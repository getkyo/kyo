...
/** The 2-pane documentation content body as a kyo-ui `UI` value.
  ...
  * (logo, search, version switcher) is owned by `SiteApp`, so `body` no longer renders its own
...
object DocsApp:
    ...
    /** Assemble the 2-pane docs content body for one route (header excluded, owned by `SiteApp`).
      ...
      * reactive without additional wrapping.
      *
      ...
      *   its own tree. The caller knows which physical tree it is
    ...
    private def sidebar(
        ...
        // viewports the base `docs-sidebar` rule already shows it and the class is a no-op.
        // Use UI.Ast.Reactive directly to avoid ambiguity with StringContext.render.
