...
/** Full-page wrapper for every kyo website route.
  ...
  * rendered to CSS, the Google Fonts preconnect and stylesheet links, and an
...
object WebsitePage:
    ...
    /** Per-route configuration for the page wrapper.
      *
      * `jsonLd` is the schema.org structured-data JSON for this page. When non-empty it is
      ...
      * empty by default. `noindex`, when true, emits `<meta name="robots" content="noindex">` so
      * thin pages are excluded from search indexes while remaining reachable.
      */
    ...
    private def pageHead(opts: Options)(using Frame): UI.PageHead =
        UI.PageHead(
            title = opts.title,
            // The stray `rel="alternate"` link is removed; it duplicated the canonical href
            ...
            links = (
                Seq("canonical" -> opts.canonical) ++
                    // Favicon: the site ships /kyo.png at the root; without this link every page
