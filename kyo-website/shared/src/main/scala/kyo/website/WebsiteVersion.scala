...
object WebsiteVersion:
    ...
    /** The newest tag by git creation timestamp (the max), tie-broken by [[tagOrdering]] so a tie
      * resolves to the higher semantic version. Pure and total: returns `Absent` for an empty
      * `tags`. Unlike the stable-preference [[tagOrdering]] max, a newer pre-release (a later
      * timestamp) wins over an older stable release here, which is the whole point of dating tags
      * by their git creation time rather than parsing the version string. A tag absent from
      * `timestamps` is treated as timestamp `Long.MinValue` (oldest), so it is selected only when no
      * dated tag is present, and then the semantic tiebreak applies.
      *
      * In the forward-only model this picks the CURRENT version label: the repository's most recent
      * release tag by git date (see `WebsiteMain.currentVersion`).
      */
    def pickLatestByTimestamp(tags: Chunk[String], timestamps: Map[String, Long]): Maybe[String] =
        if tags.isEmpty then Absent
        else
            val ordering: Ordering[String] =
                (a: String, b: String) =>
                    val ta = timestamps.getOrElse(a, Long.MinValue)
                    val tb = timestamps.getOrElse(b, Long.MinValue)
                    if ta != tb then java.lang.Long.compare(ta, tb)
                    else tagOrdering.compare(a, b)
            Maybe.fromOption(tags.toSeq.maxOption(using ordering))
        end if
    end pickLatestByTimestamp
