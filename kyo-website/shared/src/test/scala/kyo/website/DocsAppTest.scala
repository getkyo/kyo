...
class DocsAppTest extends WebsiteTest:
    ...
    // physical-tree-decoupling regression leaves pass an explicit prefix to assert that decoupling. The
    // body no longer renders a header, so there is no `versions` argument anymore.
    ...
    // sidebar groups match README order
    "sidebar groups match README order" in {
        for
            ...
            content = WebsiteContent(
                ...
                groups = Chunk(
                    WebsiteContent.Group(
                        "Foundation",
                        Chunk(WebsiteModule("kyo-data", "Foundation", "kyo-data", "", WebsiteModule.Platforms.everywhere))
                    ),
                    WebsiteContent.Group(
                        "Application runtime",
                        Chunk(WebsiteModule(
    ...
    // sidebar lists modules under each group with correct hrefs
    "sidebar lists modules under each group" in {
        for
            route <- fixedRoute("/latest/kyo-data/")
            content = WebsiteContent(
                intro = "",
                groups = Chunk(
                    WebsiteContent.Group(
                        "Foundation",
                        Chunk(
                            WebsiteModule("kyo-data", "Foundation", "kyo-data", "", WebsiteModule.Platforms.everywhere),
                            WebsiteModule("kyo-kernel", "Foundation", "kyo-kernel", "", WebsiteModule.Platforms.everywhere)
                        )
                    )
                ),
                version = WebsiteVersion("latest", "latest", true)
    ...
    private def coreContent(ver: WebsiteVersion = WebsiteVersion("latest", "latest", true))(using Frame): WebsiteContent =
        WebsiteContent(
            ...
            groups = Chunk(
                WebsiteContent.Group(
                    "Foundation",
                    Chunk(WebsiteModule("kyo-core", "Foundation", "kyo-core", "", WebsiteModule.Platforms.everywhere))
                )
            ),
            version = ver
        )

    // the active module's nested sections render one `#slug` link per section heading.
    // The level-1 page-title heading is the module link itself, so it is NOT repeated as a section link.
    "active module nested sections: one #slug link per section heading, level-1 skipped" in {
        val toc = Chunk(
    ...
    // static outline source: the active module renders the sections from the `outlines` map
    ...
    "active module renders the outline mapped to its own route, not another route's" in {
        val outlines = Map(
    ...
    // a NON-active module renders no nested sections (the outline collapses when not current).
    "non-active module renders no nested sections" in {
        val toc = Chunk(
        ...
        val content = WebsiteContent(
            ...
            groups = Chunk(
                WebsiteContent.Group(
                    ...
                    Chunk(
                        WebsiteModule("kyo-data", "Foundation", "kyo-data", "", WebsiteModule.Platforms.everywhere),
                        WebsiteModule("kyo-core", "Foundation", "kyo-core", "", WebsiteModule.Platforms.everywhere)
                    )
                )
            ),
            version = WebsiteVersion("latest", "latest", true)
    ...
    // the rail is one level deep: ONLY level-2 (`## `) sections render. The level-1 page title
    // is skipped (it is the module link), and level-3+ headings are dropped from the rail entirely.
    "rail renders only level-2 sections, dropping level-1 and level-3+" in {
        val toc = Chunk(
    ...
    // content area is route-reactive
    "content area is route-reactive" in {
        for
            route <- fixedRoute("/latest/kyo-core/")
            // DocsMarkdown.transpile is JVM-only after the split; use a constructed UI article
            // so this shared leaf stays cross-platform.
    ...
    // The version dropdown is verified in SiteAppTest: the dropdown is owned by the header
    ...
    // the docs body renders no header chrome
    "docs body renders no header, dropdown, or search input" in {
    ...
    // version banner shows for non-latest
    "version banner for non-latest version" in {
    ...
    // no banner for latest
    "no version banner for latest version" in {
    ...
    // the prev/next pager is gated on `contentLoading`. While content is loading (the bundle
    ...
    "prev/next pager is hidden while content is loading and shown once loaded" in {
        ...
        yield
            ...
            // Once loaded the pager carries the adjacent-module links exactly as when not loading. Scope the link
    ...
    "a module page lists where the module runs, and the overview and a loading article list nothing" in {
        ...
        def labels(html: String): Chunk[String] =
            Chunk.from("<li[^>]*class=\"docs-platform\"[^>]*>([^<]*)<".r.findAllMatchIn(html).map(_.group(1)).toSeq)
        for
            netRoute      <- fixedRoute("/latest/kyo-net/")
            dataRoute     <- fixedRoute("/latest/kyo-data/")
            overviewRoute <- fixedRoute("/latest/")
            netHtml       <- rendered(content, netRoute, Chunk.empty, UI.empty)
            dataHtml      <- rendered(content, dataRoute, Chunk.empty, UI.empty)
            overviewHtml  <- rendered(content, overviewRoute, Chunk.empty, UI.empty)
            loadingHtml   <- rendered(content, netRoute, Chunk.empty, UI.empty, contentLoading = true)
        yield
            assert(labels(netHtml) == Chunk("JVM", "JS on Node", "Native", "WASM on Node"), s"kyo-net: $netHtml")
            assert(
                labels(dataHtml) == Chunk("JVM", "JS on Node", "JS in a browser", "Native", "WASM on Node", "WASM in a browser"),
                s"kyo-data: $dataHtml"
            )
            assert(dataHtml.contains("aria-label=\"Runs on\""), s"the line names what it lists: $dataHtml")
            assert(!overviewHtml.contains("docs-platforms"), s"the overview is not a module: $overviewHtml")
            assert(!loadingHtml.contains("docs-platforms"), s"no line over an article still loading: $loadingHtml")
        end for
    }
    ...
    // intro-only content renders chrome with the Overview item but no module links
    "intro-only content renders shell with the Overview but no module links" in {
        for
            route <- fixedRoute("/latest/")
            html  <- rendered(emptyContent(), route, Chunk.empty, UI.empty)
        yield
    ...
    // on the intro/home route the Overview is the ACTIVE item and expands to its own h2 sections.
    "on the intro home the Overview is active and expands to its h2 sections" in {
    ...
    // Regression: the latest version is emitted under BOTH `latest/` and its
    ...
    "latest version under its own v<X> tree links within v<X> not latest (regression)" in {
        val modA = WebsiteModule("mod-a", "Foundation", "Mod A", "", WebsiteModule.Platforms.everywhere)
        val modB = WebsiteModule("mod-b", "Foundation", "Mod B", "", WebsiteModule.Platforms.everywhere)
        val modC = WebsiteModule("mod-c", "Foundation", "Mod C", "", WebsiteModule.Platforms.everywhere)
