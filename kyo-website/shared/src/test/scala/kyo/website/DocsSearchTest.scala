...
class DocsSearchTest extends WebsiteTest:
    ...
    "seed builds title-only entries; a module-name query returns the module page" in {
        val modules = Chunk(
            WebsiteModule("kyo-core", "Effects", "kyo-core", "", WebsiteModule.Platforms.everywhere),
            WebsiteModule("kyo-stream", "Effects", "kyo-stream", "", WebsiteModule.Platforms.everywhere)
        )
