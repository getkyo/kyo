...
class WebsiteStylesTest extends WebsiteTest:
    ...
    "sheet is a Stylesheet value; render is non-empty and contains :root vars and .feat-grid rule" in {
    ...
    "render contains at least one @media block with docs 2-pane rule and a :hover pseudo-state rule" in {
    ...
    "render contains callout-note, callout-caution, and code highlight token classes" in {
        val css = WebsiteStyles.sheet.render
