...
(render-from-tags) (`WebsiteContent.scala:5-7`).
...
- `latest/` is a duplicate emission, not a symlink (Pages serves files, not
  ...
  (`WebsiteGenerator.scala:191-193,361-384`).
...
(`ChromeParityTest.scala:55-66,11-15`). Parity and equality comparisons
...
- JVM tests locate the repo root by walking up from `user.dir` until `build.sbt` is
  ...
  `readFile` helper maps `FileSystemException` into `WebsiteEmitException` and
...
3. **Am I adding a JVM-only dependency or transpiler type?** It must live in `jvm/`,
   ...
   shared/JS (`DocsMarkdown.scala:15-16`). `WebsiteBuildGraphTest` enforces this with
   its source-placement, linked-output, and transpiler-surface checks
   (`WebsiteBuildGraphTest.scala:79-83,155-157,180-181`).
...
5. **Can my code fail at build time?** Abort over `WebsiteException` (a typed leaf
   ...
   Decide degrade vs abort per the degrade-not-fail rule (`WebsiteContent.scala:29-35`).
