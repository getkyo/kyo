        ...
        Scope.run {
            for
                ...
                _ <- Sync.defer {
                    ...
                    val collidingUriSnapshot = new FakeTransfer
                    collidingUriSnapshot.allow("copy")
                    collidingUriSnapshot.setData("TEXT/URI-LIST", "https://kyo.dev/first")
                    val uriCollisionEnter = dragEvent("dragenter", targetEl, collidingUriSnapshot, 11, 12)
                    discard(targetEl.dispatchEvent(uriCollisionEnter))
                    assert(uriCollisionEnter.defaultPrevented)
                    assert(runtime.stateName == "ExternalProbing")
                    collidingUriSnapshot.setData("text/uri-list", "https://kyo.dev/second")
                    collidingUriSnapshot.beginDrop()
                    val uriCollisionDrop = dragEvent("drop", targetEl, collidingUriSnapshot, 13, 14)
                    discard(targetEl.dispatchEvent(uriCollisionDrop))
                    assert(!uriCollisionDrop.defaultPrevented)
                    assert(runtime.stateName == "Idle")
                    assert(runtime.activeSessions == 0)
                    assert(runtime.fileTokens == 0)
                    assert(events.count(_.isInstanceOf[UIEvent.DragStart]) == 0)
                    assert(events.count(_.isInstanceOf[UIEvent.Drop]) == 0)
                    fixture.remove()
                }
            yield ()
        }
