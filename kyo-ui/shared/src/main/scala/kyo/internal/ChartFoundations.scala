...
/** Cross-cutting foundation helpers for the charts lowering.
  *
  * Houses the identity key for category values, the finite guard used at every extent fold, the per-chart id
  * prefix for `url(#id)`-bearing gradient defs (so each chart's defs do not collide with sibling charts), and
  * the `Chunk`-native ordered-distinct and group-by helpers.
  */
private[kyo] object ChartFoundations:

    /** Identity key for a category / series / group / x-key value.
      *
      * Pairs the raw value with the `ConcreteTag` of its static type so `1: Int` and `"1": String` stay distinct
      * (their `toString` collides) and two enum cases sharing a display label stay distinct (the enum case is its
      * own value). The tag is a stable platform-independent type identity (`ConcreteTag[Int]` is `IntTag`
      * everywhere), unlike a boxed runtime class which diverges between the JVM and Scala.js. Equality is
      * case-class structural, so two nulls under the same tag compare equal and the value is never dereferenced.
      */
    ...
    def categoryKey[C](tag: ConcreteTag[C], value: Any): CatKey =
        // Unsafe: widening `ConcreteTag[C]` to `ConcreteTag[Any]` is sound because the tag is only a structural
        // identity component of the key, never used to reconstruct or constrain `C`. A null `value` (from a typed
        // encoding accessor yielding a null reference) is fine: it is stored, never dereferenced.
    ...
    /** Insertion-ordered distinct over `rows` keyed by `key`, in O(n) with set-backed membership.
      *
      * Returns each first-seen `(CatKey, representativeRow)` in encounter order. Empty input fast-paths to empty.
      */
    ...
    /** Group `rows` by `key` in one pass, mapping each key to its rows in encounter order within the group.
      * Empty input fast-paths to an empty map. Does NOT sort by enum ordinal; callers needing ordinal order
      * must sort the keys separately (e.g. via `collectColorCategoriesWithRaw`).
      */
    ...
    /** Per-chart deterministic id prefix for `url(#id)`-bearing defs.
      *
      * Derived from the spec structural hash so two distinct charts on one page never alias the same `url(#id)`.
      * Structurally identical specs alias identical defs, which is benign.
      */
