...
class ChartSpecTest extends kyo.test.Test[Any]:
    ...
    private def rootOf[A](spec: Chart[A])(using Frame): Svg.Root < Sync =
        spec.lower
    ...
    "title(t) adds an Svg.Title child carrying the title text" in {
        rootOf(Chart(sales)(bar(x = _.month, y = _.revenue)).title("Revenue by month")).map { root =>
    ...
    "title(t) sets role=img on the root svg" in {
        rootOf(Chart(sales)(bar(x = _.month, y = _.revenue)).title("T")).map { root =>
    ...
    "desc(d) adds an Svg.Desc child carrying the description text" in {
        rootOf(Chart(sales)(bar(x = _.month, y = _.revenue)).desc("Monthly totals")).map { root =>
    ...
    "ariaLabel(l) sets aria-label on the root svg" in {
        rootOf(Chart(sales)(bar(x = _.month, y = _.revenue)).ariaLabel("chart")).map { root =>
    ...
    "no a11y configured -> no title, no desc, no role, no aria-label" in {
        rootOf(Chart(sales)(bar(x = _.month, y = _.revenue))).map { root =>
    ...
    "responsive uses width=100% and a viewBox with no fixed pixel height" in {
        rootOf(Chart(sales)(bar(x = _.month, y = _.revenue)).size(400, 300).responsive).map { root =>
            assert(
    ...
    "responsive(ratio) keeps width=100% and a viewBox and sets preserveAspectRatio" in {
        rootOf(Chart(sales)(bar(x = _.month, y = _.revenue)).responsive(16.0 / 9.0)).map { root =>
    ...
    "responsive and size are mutually exclusive with last-set-wins" in {
        ...
        for
            sizeWins <- rootOf(Chart(sales)(bar(x = _.month, y = _.revenue)).responsive.size(400, 300))
            _ = assert(
            ...
            _ = assert(sizeWins.svgAttrs.height == Present(Svg.Coord.Num(300.0)), "size-after-responsive must keep fixed height")
            // responsive after size -> responsive wins.
            respWins <- rootOf(Chart(sales)(bar(x = _.month, y = _.revenue)).size(400, 300).responsive)
        yield
    ...
    "margins(...) shifts the plot rectangle by the configured left/top margins" in {
        ...
        for
            (_, sc0) <- base.lowerWithScales
            (_, sc1) <- base.margins(_.left(120.0)).lowerWithScales
        yield
