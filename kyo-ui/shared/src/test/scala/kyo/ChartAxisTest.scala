...
class ChartAxisTest extends kyo.test.Test[Any]:
    ...
    "legend derives one swatch+label per Region enum case in enum order; colorScale assigns fills" in {
        ...
        (spec).lower.map { root =>
            ...
            assert(
                ...
                s"APAC swatch should be orange; got ${swatches(2).svgAttrs.fill}"
            )
        }
    }
        ...
        (spec).lower.map { root =>
            ...
            leftTickLabels.foldLeft(()): (_, t) =>
                assert(
                    ...
                    s"Left tick label should be palette(0) (blue) but was ${t.svgAttrs.fill}"
                )
    ...
    "yAxis(_.reverse) swaps the y range so a small value sits near the top, not the baseline" in {
        ...
        for
            rootNorm <- (normal).lower
            rootFlip <- (flipped).lower
        yield
            val rNorm = barsIn(rootNorm)
            val rFlip = barsIn(rootFlip)
    ...
    "xScale linear with pad insets the first datum from the plot edge (continuous x)" in {
        ...
        for
            rootNoPad  <- (noPad).lower
            rootPadded <- (padded).lower
        yield
    ...
    "yScale linear withClamp(true) clamps an out-of-range datum to the range; withClamp(false) extrapolates" in {
        ...
        for
            rootClamped   <- (clamped).lower
            rootUnclamped <- (unclamped).lower
        yield
            val yClamped   = numOf(barsIn(rootClamped)(0).svgAttrs.y)
            val yUnclamped = numOf(barsIn(rootUnclamped)(0).svgAttrs.y)
    ...
    "yScale log withPad widens the log domain so the smallest datum is inset from the baseline" in {
        ...
        for
            ...
            rootPadded <- (padded).lower
        yield
            val yNoPad  = numOf(barsIn(rootNoPad)(0).svgAttrs.y)
            val yPadded = numOf(barsIn(rootPadded)(0).svgAttrs.y)
    ...
    "bar+line combo emits the line path after all bar rects (z-order guard)" in {
        ...
        for
            root <- (spec).lower
            html <- kyo.internal.HtmlRenderer.render(root, Seq.empty)
        yield
    ...
    "x tick rotateTicks/anchor/font are applied through the shared helper" in {
        ...
        for
            rotRoot <- (rotSpec).lower
            ancRoot <- (ancSpec).lower
            fntRoot <- (fntSpec).lower
        yield
