...
class ImgTest extends UITest:
    ...
    "img src attribute" in run {
        withUI(UI.div(UI.img(ImgSrc.Path("photo.jpg"), "A photo").id("i"))) {
            Browser.assertAttributeSatisfies(Selector.id("i"), "src", "ignore")(_.contains("photo.jpg")).andThen(succeed)
        }
    }

    "img alt attribute" in run {
        withUI(UI.div(UI.img(ImgSrc.Path("photo.jpg"), "A photo").id("i"))) {
            Browser.assertAttribute(Selector.id("i"), "alt", "A photo").andThen(succeed)
        }
    }
    ...
    "img src signal initial" in run {
        ...
        withUI(app) {
            Browser.assertAttributeSatisfies(Selector.id("i"), "src", "ignore")(_.contains("initial.jpg")).andThen(succeed)
        }
    }
    ...
    "img long alt text" in run {
        val longAlt = "A" * 200
        withUI(UI.div(UI.img(ImgSrc.Path("photo.jpg"), longAlt).id("i"))) {
            Browser.assertAttributeSatisfies(Selector.id("i"), "alt", "ignore")(_.contains("AAAA")).andThen(succeed)
        }
    }

    "img inside anchor click bubbles" in run {
        val app: UI < Async =
    ...
    "img no alt set" in run {
        withUI(UI.div(UI.img(ImgSrc.Path("photo.jpg"), "").id("i"))) {
            Browser.assertAttributeSatisfies(Selector.id("i"), "src", "ignore")(_.contains("photo.jpg")).andThen(succeed)
        }
    }

    "img with style" in run {
        withUI(UI.div(UI.img(ImgSrc.Path("photo.jpg"), "photo").id("i").style(Style.width(100.px)))) {
            Browser.assertAttributeSatisfies(Selector.id("i"), "style", "ignore")(_.contains("width")).andThen(succeed)
        }
    }
