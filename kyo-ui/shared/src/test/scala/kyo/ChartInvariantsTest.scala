...
class ChartInvariantsTest extends kyo.test.Test[Any]:
    ...
    "AxisConfig.reverse flips pixel orientation (first datum at far range end)" in {
        ...
        val revSpec = Chart(rows)(bar(x = _.x, y = _.y)).xAxis(_.reverse)
        for
            (_, fwdSc) <- fwdSpec.lowerWithScales
            fwdA = fwdSc.x.toPixelCategory("a").getOrElse(fail("forward: band key 'a' must project"))
            fwdC = fwdSc.x.toPixelCategory("c").getOrElse(fail("forward: band key 'c' must project"))
            _    = assert(fwdA < fwdC, s"forward (no reverse): first band 'a' must be left of 'c', px(a)=$fwdA, px(c)=$fwdC")
            (_, sc) <- revSpec.lowerWithScales
            pa = sc.x.toPixelCategory("a").getOrElse(fail("reverse: band key 'a' must project"))
            pc = sc.x.toPixelCategory("c").getOrElse(fail("reverse: band key 'c' must project"))
        yield
    ...
    "interaction(_.highlightSelect) with no onSelect ref is a no-op: renders identically to the plain spec" in {
        ...
        for
            rootPlain   <- plain.lower
            rootWithInt <- withInt.lower
