...
class RawHtmlTest extends Test:
    ...
    "rawHtml emits verbatim, bypasses esc (INV-005)" in run {
    ...
    "text escapes (boundary side by side, INV-005)" in run {
    ...
    "rawHtml inside an element renders inside its tags" in run {
    ...
    "inline-HTML snippet round-trips byte-identical" in run {
    ...
    "rawHtml of empty string renders empty, no crash" in run {
    ...
    "rawHtml under the page path renders verbatim in body (cross-addition)" in run {
    ...
    "reactive rawHtml re-renders on change" in run {
