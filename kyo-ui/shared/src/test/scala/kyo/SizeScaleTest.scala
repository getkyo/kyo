...
/** Unit tests for SizeScale sqrt-area mapping.
  *
  * All tests are pure deterministic math.
  */
class SizeScaleTest extends kyo.test.Test[Any]:

    private val Tol = 1e-9

    "sqrt-area scale: r(magMin) == rMin and r(magMax) == rMax" in {
    ...
    "sqrt-area scale: mid-point is sqrt-proportional, not linear" in {
    ...
    "equal magnitudes yield rMin with no div-by-zero" in {
    ...
    "magMax <= magMin (inverted extent) is handled as degenerate, returns rMin" in {
        val s = SizeScale(10.0, 5.0, 2.0, 20.0) // magMax < magMin
        assert(s.radius(7.0) == 2.0, "Inverted extent must return rMin without crash")
    }

    "out-of-range magnitudes are clamped to [0,1] before sqrt" in {
    ...
    "DefaultRMin and DefaultRMax constants are 2.0 and 20.0" in {
    ...
    "radius is monotonically non-decreasing for increasing magnitudes" in {
