...
9. **Playwright hangs (mitigated)**: `web-fill`, `web-click`, and `web-text` now check `Browser.count(selector)` first and return `ERROR: no elements match '...'` instead of hanging. `web-exists` uses `Browser.count` (non-blocking). However, if a matched element is not *actionable* (e.g., hidden, disabled), `web-fill` can still hang. For fragile operations, prefer `web-js` with null checks.
10. **`#` selectors break in shell**: `./kyo-ui/ui-cmd.sh web-count #name` fails because `#` starts a shell comment. Use attribute selectors instead: `[id=name]`. Similarly, selectors with spaces need quoting at the call site, but shell consumes the quotes before `$*` sees them. For compound selectors, prefer `web-js` with `document.querySelector(...)`.
11. **web-js requires `return`**: Always prefix `web-js` expressions with `return`. Without it, Playwright's evaluate hangs waiting for a return value. Also wrap in try/catch to prevent null dereference from killing the session. Pattern: `web-js try { return document.querySelector('x').textContent } catch(e) { return 'ERROR: ' + e.message }`
