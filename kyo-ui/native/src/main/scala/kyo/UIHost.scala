...
extension (ui: UI.type)
    /** A host element: kyo-ui renders it once as a real DOM `<tag>` (default `canvas`) on
      ...
      *
      * Native-only copy: the arity-1 and arity-2 overloads live in the same extension block on
      * JS+Wasm (`UIHostMount.scala`). On Native there is no arity-2 overload (dom.Element is
