...
/** Stub implementation for the Native test compile. The SPA harness is JS-only by construction (it serves a Scala.js bundle to Chrome).
...
private[kyo] object SpaHarnessLocation:
    def bundleDir: String =
        sys.error("kyo.internal.spa.SpaHarnessLocation.bundleDir is JS-only; UITestSpa cannot run on Native")
