...
class ListAsyncShift[A](using Frame) extends IterableOpsAsyncShift[A, List, List[A]]:
    ...
    override def shiftedStateFold[F[_], S, R](c: List[A], monad: CpsMonad[F])(prolog: S, acc: (S, A) => F[S], epilog: S => R): F[R] =
        monad match
    ...
    override def filter[F[_]](c: List[A], monad: CpsMonad[F])(p: A => F[Boolean]): F[List[A]] =
        monad match
            case _: KyoCpsMonad[?] => Kyo.filter(c)(a => p(a))

    override def takeWhile[F[_]](c: List[A], monad: CpsMonad[F])(p: (A) => F[Boolean]): F[List[A]] =
        monad match
    ...
    override def foreach[F[_], U](c: List[A], monad: CpsMonad[F])(f: A => F[U]): F[Unit] =
        monad match
    ...
    override def collect[F[_], B](c: List[A], monad: CpsMonad[F])(pf: PartialFunction[A, F[B]]): F[List[B]] =
        monad match
            case _: KyoCpsMonad[?] => Kyo.collect(c)(liftPartial(pf))

    override def filterNot[F[_]](c: List[A], monad: CpsMonad[F])(p: A => F[Boolean]): F[List[A]] =
        monad match
            case _: KyoCpsMonad[?] => Kyo.filter(c)(a => p(a).map(!_))

    override def flatten[F[_], B](c: List[A], monad: CpsMonad[F])(implicit asIterable: A => F[IterableOnce[B]]): F[List[B]] =
        monad match
            case _: KyoCpsMonad[?] => flatMap(c, monad)(asIterable)

    override def groupBy[F[_], K](c: List[A], monad: CpsMonad[F])(f: A => F[K]): F[Map[K, List[A]]] =
        monad match
            case _: KyoCpsMonad[?] => Kyo.groupBy(c)(f)

    override def groupMap[F[_], K, B](c: List[A], monad: CpsMonad[F])(key: A => F[K])(f: A => F[B]): F[Map[K, List[B]]] =
        monad match
            case _: KyoCpsMonad[?] => Kyo.groupMap(c)(key)(f)

    override def partition[F[_]](c: List[A], monad: CpsMonad[F])(p: A => F[Boolean]): F[(List[A], List[A])] =
        monad match
            case _: KyoCpsMonad[?] => Kyo.partition(c)(p)

    override def partitionMap[F[_], A1, A2](c: List[A], monad: CpsMonad[F])(f: A => F[Either[A1, A2]]): F[(List[A1], List[A2])] =
        monad match
            case _: KyoCpsMonad[?] => Kyo.partitionMap(c)(f)

    override def scanLeft[F[_], B](c: List[A], monad: CpsMonad[F])(z: B)(op: (B, A) => F[B]): F[List[B]] =
        monad match
            case _: KyoCpsMonad[?] => Kyo.scanLeft(c)(z)(op)

    override def span[F[_]](c: List[A], monad: CpsMonad[F])(p: A => F[Boolean]): F[(List[A], List[A])] =
        monad match
            case _: KyoCpsMonad[?] => Kyo.span(c)(p)

    override def tapEach[F[_], U](c: List[A], monad: CpsMonad[F])(f: A => F[U]): F[List[A]] =
        monad match
