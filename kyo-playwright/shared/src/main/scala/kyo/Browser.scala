...
object Browser:
    ...
    final case class Image private (data: Array[Byte]):
        ...
        /** Attempts to render the image directly to the terminal.
          *
          * Supports iTerm2 and Kitty terminal emulators. The image will be displayed inline in the terminal output if the terminal supports
          * it.
          *
          * @param charsWidth
          ...
          *   images
          */
        def renderToConsole(charsWidth: Int = 0, charsHeight: Int = 0, consoleType: Maybe[Image.ConsoleType] = Image.ConsoleType.get)(
            using Frame
        ): Maybe[String] =
            val base64Image = base64
            consoleType.map {
                case Image.ConsoleType.iterm =>
                    val sizeSpec =
