...
object Safepoint:
    ...
    private[kernel] def enrich(ex: Throwable)(using self: Safepoint): Unit =
        self.enrich(ex)
    ...
    private[kernel] inline def eval[A](
        inline f: => A
    )(using inline frame: Frame, self: Safepoint): A =
        self.withNewTrace {
    ...
    private[kernel] inline def handle[V, A, B, S](value: V)(
        inline eval: => A,
