...
object Context:
    ...
    extension (context: Context)
        ...
        private[kernel] inline def contains[A, E <: ContextEffect[A]](tag: Tag[E]): Boolean =
            context.contains(tag.erased)

        private[kernel] inline def getOrElse[A, E <: ContextEffect[A], B >: A](tag: Tag[E], inline default: => B): B =
