        ...
        "catching" - {
            ...
            "with other effect" - {
                ...
                "success" in {
                    assert(
                        kyo2.Env.run(2)(
                            Abort.run[Ex1](Abort.catching[Ex1](test(kyo2.Env.get)))
                        ).eval ==
                            Result.success(5)
                    )
                }
            ...
            "interactions with Env" - {
                "should have access to the environment within Abort" in {
                    val env    = "test"
                    val result = kyo2.Env.run(env)(Abort.run[String](kyo2.Env.get[String]))
                    assert(result.eval == Result.success(env))
                }

                "should propagate Abort failures within Env" in {
                    val result = kyo2.Env.run("test")(Abort.run[String](Abort.fail("failure")))
            ...
            "interactions with Var" - {
                ...
                "should not modify state on Abort failures" in {
                    val result = kyo2.Var.run(42)(
                        Abort.run[String](
                            kyo2.Var.set[Int](24).map(_ => Abort.fail("failure"))
                        )
                    )
                    assert(result.eval == Result.fail("failure"))
