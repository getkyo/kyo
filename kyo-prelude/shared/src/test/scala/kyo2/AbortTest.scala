...
class AbortsTest extends Test:
    ...
    "pure" - {
        "handle" in {
            assert(
                kyo2.Abort.run[Ex1](kyo2.Abort.get[Ex1](Result.success(1))).eval ==
                    Result.success(1)
            )
        }
        "handle + transform" in {
            assert(
                kyo2.Abort.run[Ex1](kyo2.Abort.get[Ex1](Result.success(1)).map(_ + 1)).eval ==
                    Result.success(2)
            )
        }
        "handle + effectful transform" in {
            assert(
                kyo2.Abort.run[Ex1](kyo2.Abort.get[Ex1](Result.success(1)).map(i =>
                    kyo2.Abort.get[Ex1](Result.success(i + 1))
                )).eval ==
                    Result.success(2)
            )
        }
        "handle + transform + effectful transform" in {
            assert(
                kyo2.Abort.run[Ex1](kyo2.Abort.get[Ex1](Result.success(1)).map(_ + 1).map(i =>
                    kyo2.Abort.get[Ex1](Result.success(i + 1))
                )).eval ==
                    Result.success(3)
            )
        ...
        "handle + transform + failed effectful transform" in {
            val fail = Result.failure[Ex1, Int](ex1)
            assert(
                kyo2.Abort.run[Ex1](kyo2.Abort.get[Ex1](Result.success(1)).map(_ + 1).map(_ =>
                    kyo2.Abort.get(fail)
                )).eval ==
                    fail
            )
    ...
    "get" - {
        "either" in {
            assert(Abort.run(Abort.get(Result.failure(1))).eval == Result.failure(1))
    ...
    "effectful" - {
        "success" - {
            val v: Int < Abort[Ex1] = kyo2.Abort.get[Ex1](Result.success(1))
            "handle" in {
                assert(
            ...
            "handle + effectful transform" in {
                assert(
                    kyo2.Abort.run[Ex1](v.map(i => kyo2.Abort.get[Ex1](Result.success(i + 1)))).eval ==
                        Result.success(2)
                )
            }
            "handle + transform + effectful transform" in {
                assert(
                    kyo2.Abort.run[Ex1](v.map(_ + 1).map(i => kyo2.Abort.get[Ex1](Result.success(i + 1)))).eval ==
                        Result.success(3)
                )
            }
            "handle + transform + failed effectful transform" in {
        ...
        "failure" - {
            val v: Int < Abort[Ex1] = kyo2.Abort.get(Result.failure(ex1))
            "handle" in {
                assert(
                    kyo2.Abort.run[Ex1](v).eval ==
                        Result.failure(ex1)
    ...
    "Abort" - {
        ...
        "run" - {
            ...
            "super" in {
                ...
                val a: Int < Abort[Exception] = kyo2.Abort.failure(ex)
        ...
        "catching" - {
            ...
            "nested Abort effects" - {
                "should propagate the innermost failure" in {
                    val nested = kyo2.Abort.run[String](
                        kyo2.Abort.run[Int](
                            kyo2.Abort.failure[String]("inner").map(_ => kyo2.Abort.failure[Int](42))
                        )
                    )
                ...
                "should propagate the outermost failure if there are no inner failures" in {
                    val nested = kyo2.Abort.run(kyo2.Abort.run[String](
                        kyo2.Abort.run[Int](kyo2.Abort.get[Int](Result.success(42)))
                    ).map(_ => kyo2.Abort.failure("outer")))
            ...
            "interactions with Env" - {
                ...
                "should propagate Abort failures within Env" in {
                    val result = kyo2.Env.run("test")(kyo2.Abort.run[String](kyo2.Abort.failure("failure")))
            ...
            "interactions with Var" - {
                ...
                "should not modify state on Abort failures" in {
                    val result = kyo2.Var.run(42)(
                        kyo2.Abort.run[String](
                            kyo2.Var.set[Int](24).map(_ => kyo2.Abort.failure("failure"))
                        )
                    )
                    assert(result.eval == Result.failure("failure"))
            ...
            "short-circuiting with map" - {
                "should not execute subsequent operations on failure" in {
                    ...
                    val result = kyo2.Abort.run[String](
                        kyo2.Abort.failure("failure").map(_ => executed = true)
                    )
                    assert(result.eval == Result.failure("failure"))
                    assert(!executed)
                }
                ...
                "should execute subsequent operations on success" in {
                    var executed = false
                    val result = kyo2.Abort.run[Int](kyo2.Abort.run[String](
                        kyo2.Abort.get[Int](Result.success(42)).map(_ => executed = true)
                    ))
