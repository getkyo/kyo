...
class SchemaCompositionTest extends kyo.test.Test[Any]:
    ...
    "top-level wire shape per representative type" - {
        ...
        // Guards the CDP-shaped decode path: a `.discriminator("type")` sealed trait at a NESTED field
        // position, decoded from a flat Structure.Value.Record that carries wire fields beyond what the
        // matched variant declares (mirroring CDP's EvalResult(result: RemoteObject) with the `object`
        // variant and undeclared extras className/objectId/value). DiscriminatorReader buffers the
        // non-discriminator fields as captured StructureValueReader sub-readers, and the macro's by-name
        // read loop skips the undeclared extras; this asserts the path decodes correctly through
        // fromStructureValue and stays in parity with the Json wire shape.
        "sealed-trait-with-discriminator decodes from a nested Structure.Value with extra fields" in {
            val schema: Schema[DiscEnvelope] = summon[Schema[DiscEnvelope]]
            // Inner flat object: discriminator first, then a declared field, an extra (className), another
            // declared field, then another extra (objectId). The interleaving mirrors the real CDP payload.
            val inner = Structure.Value.Record(Chunk(
                "type"    -> Structure.Value.Str("object"),
                "subtype" -> Structure.Value.Str("node"),
                // Extra (undeclared) field whose VALUE is itself a Record, mirroring CDP's `value` payload
                // for a returnByValue object: an interleaved Record-valued extra that the by-name read loop
                // must skip without mistaking its sub-tree for a later declared String field.
                "value"       -> Structure.Value.Record(Chunk("nested" -> Structure.Value.Str("x"))),
                "className"   -> Structure.Value.Str("HTMLButtonElement"),
                "description" -> Structure.Value.Str("button#target"),
                "objectId"    -> Structure.Value.Str("0123")
            ))
            val outer    = Structure.Value.Record(Chunk("result" -> inner))
            val expected = DiscEnvelope(DiscObject.`object`(subtype = Maybe("node"), description = Maybe("button#target")))
            val dec      = schema.fromStructureValue(outer)
            assert(dec == Result.succeed(expected), s"Structure.Value decode failed: $dec")
            // Json must round-trip the same wire shape, so the two decode paths stay in parity.
            val innerJson =
                """{"type":"object","subtype":"node","value":{"nested":"x"},"className":"HTMLButtonElement","description":"button#target","objectId":"0123"}"""
            val outerJson = s"""{"result":$innerJson}"""
            val jsonDec   = Json.decode[DiscEnvelope](outerJson)(using summon[Json], schema)
            assert(jsonDec == Result.succeed(expected), s"Json round-trip regressed: $jsonDec")
        }
    ...
    "OrderedDict Schema given ergonomics" - {

        "a case class deriving Schema with an OrderedDict field resolves the correct given with zero explicit ceremony" in {
            val holder =
...
// Mirrors CDP's RemoteObject: a multi-variant sealed trait with `.discriminator("type")` whose
// target variant `object` is NOT the first variant (so the macro's matchField chain skips earlier
// variants), every variant carries all-optional String/primitive fields with defaults, and none
// declare the wire extras className/objectId/value that real CDP payloads include.
sealed trait DiscObject derives CanEqual
object DiscObject:
    final case class `string`(value: String, description: Maybe[String] = Maybe.empty)   extends DiscObject derives CanEqual, Schema
    final case class `number`(value: Double, description: Maybe[String] = Maybe.empty)   extends DiscObject derives CanEqual, Schema
    final case class `boolean`(value: Boolean, description: Maybe[String] = Maybe.empty) extends DiscObject derives CanEqual, Schema
    final case class `object`(subtype: Maybe[String] = Maybe.empty, description: Maybe[String] = Maybe.empty)
        extends DiscObject derives CanEqual, Schema
    final case class `function`(description: Maybe[String] = Maybe.empty) extends DiscObject derives CanEqual, Schema
    final case class `undefined`()                                        extends DiscObject derives CanEqual, Schema
end DiscObject
given Schema[DiscObject] = Schema.derived[DiscObject].discriminator("type")
// EvalResult analogue: the discriminated trait at a NESTED field position, with a trailing optional
// sibling field (mirroring EvalResult.exceptionDetails) that the outer reader must still read after
// the discriminated field has been consumed.
case class DiscEnvelope(result: DiscObject, extra: Maybe[String] = Maybe.empty) derives CanEqual, Schema
