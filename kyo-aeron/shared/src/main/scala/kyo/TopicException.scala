...
/** Base class for all kyo-aeron `Topic` errors, split into three sealed subcategories by failure mode:
  ...
  * `Topic.stream` fails with `Abort[TopicBackpressureException | TopicTransportException]`, since the offer-path
  * failures are unreachable on the subscribe path. Match on a subcategory to distinguish a retryable transient
  * stall from a terminal transport error, or on a concrete leaf for the specific condition.
  *
...
object TopicException:
    /** Renders the (uri, streamId) destination uniformly across leaf messages, mirroring
      * `HttpException.stripQuery` / `showRequest`.
      */
...
/** The retry schedule exhausted while a transient condition persisted: back-pressure (publish), no
  * fragments (stream), not-connected, or an admin action such as log rotation.
  */
...
/** The publication or its owning client was closed: offer sentinel -4, or the client was already
  * closed when the publication was added.
  */
...
/** The publication reached the maximum position its term-buffer layout allows (offer sentinel -5). A
  * fresh URI or session is required to continue.
  */
...
/** A single logical message exceeds the publication's `maxMessageLength`, which Aeron derives as
  * `min(termLength >> 3, 16 MiB)`.
  */
...
/** The Aeron driver rejected a publication or subscription registration (malformed URI, driver policy,
  * or a non-transient add failure). The rejection surfaces directly rather than retrying as
  * back-pressure.
  *
  * `errorCode` is the driver's positive error code, identical across platforms for a given rejection;
  * `detail` is the driver's accompanying message, or `"(no driver detail)"` when it supplies none.
  */
...
/** Registration of a publication or subscription did not complete within the add timeout, so the
  * driver may be unreachable or overloaded.
  */
