...
class AeronSentinelsTest extends Test:
    ...
    // Purely synthetic: no live Aeron driver required. transientSignal stands in for the
    // retry-exhausted outcome, so the retryable sentinels (-2/-1/-3) surface through it.
    "mapOfferResult maps the wired sentinels" - {
        ...
        // -1 is retryable so the offer-sentinel path stays consistent with the not-connected
        // pre-check in publish.
        ...
        // so -3 must not permanently fail a publish that would succeed on the next attempt.
        ...
        // Asserts the complete mapping in one pass.
    ...
    // With no subscriber the not-connected pre-check in Topic.publish short-circuits before offer
    // is ever called, so this exercises the pre-check exhaustion path rather than the
    // mapOfferResult(-1) arm the unit leaves above cover. The outcome must still be
    // TopicBackpressureExhaustedException, not a terminal TopicPublicationClosedException.
