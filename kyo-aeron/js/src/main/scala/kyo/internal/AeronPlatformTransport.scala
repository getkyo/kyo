...
// JS (Node): loads the FFI-backed AeronBindings via the koffi backend, starts the embedded
// C media driver, connects a C client, and returns an AeronRuntime whose transport is the
// shared FfiAeronTransport. This is the per-platform selection mechanism; the JVM path uses
// a separate file under jvm/. The Scala source is identical to the Native selector; the
// per-platform FFI backend difference (koffi vs @extern) is handled by kyo-ffi codegen.
...
private[kyo] object AeronPlatformTransport:
    ...
    // bridge each via .safe.get into Async. On JS (Node) the @Ffi.blocking downcall runs via
    // koffi's async dispatch on a libuv worker thread, so the event loop is never blocked during
    // the ~10 s connect. Both calls take the same unique dir.
        ...
        Sync.Unsafe.defer(Ffi.load[AeronBindings]).map { bindings =>
            ...
            yield new AeronRuntime:
                ...
                def close()(using AllowUnsafe): Unit =
                    ...
                    // clientClose/driverClose are plain (non-@Ffi.blocking) downcalls: on JS they
                    // run synchronously on the Node event loop, briefly freezing it during the
                    // bounded conductor pthread-join at teardown. This is the bounded one-shot
                    // scope-exit join, distinct from the unbounded ~10 s connect that @Ffi.blocking
                    // covers (see AeronBindings); leaving it synchronous is intentional.
    ...
    // driver-absent connect clientConnect returns NULL after the ~10 s driver-timeout, which the
    // generated binding surfaces as FfiNullPointer, recovered here and mapped to a uniform
    // Abort.fail(TopicTransportFailedException) eagerly and in-band. close() closes ONLY the client.
