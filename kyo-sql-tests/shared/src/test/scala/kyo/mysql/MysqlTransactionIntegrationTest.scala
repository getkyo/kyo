...
/** Integration tests for MySQL transactions.
  ...
  */
class MysqlTransactionIntegrationTest extends SqlContainerTest:
                    ...
                    yield
                        result match
                            case Result.Failure(e: SqlServerException) =>
                                assert(
                                    e.serverMessage == "force rollback",
                                    s"Expected the synthetic abort to surface, got: ${e.serverMessage}"
                                )
                            case other =>
                                fail(s"Expected the transaction to re-raise the synthetic abort, got: $other")
                        end match
            ...
            SqlSharedContainers.withFreshSchema(SqlSharedContainers.Backend.MySQL) { ctx =>
                initClient(ctx) { client =>
                    ...
                    yield
                        result match
                            case Result.Failure(e: SqlServerException) =>
                                assert(e.serverMessage == "outer rollback", s"Expected the outer abort to surface, got: ${e.serverMessage}")
                            case other =>
                                fail(s"Expected the transaction to re-raise the outer abort, got: $other")
                        end match
                        assert(count == 0L, s"Expected 0 rows after full rollback, got $count")
                    end for
                }
            }
        }
