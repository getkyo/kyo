...
/** Integration tests for the MySQL sequential pipeline API.
  ...
  */
class MysqlPipelineIntegrationTest extends SqlContainerTest:
    ...
    // ── Pipeline of 5 queries returns 5 results in order on MySQL ─────────────
    ...
    // ── Pipeline inside MySQL transaction commits atomically ──────────────────
                            ...
                            .andThen {
                                client.pipeline { p =>
                                    p.execute("INSERT INTO pip_err (id, val) VALUES (1, 'a')")               // success
                                        .andThen(p.execute("INSERT INTO pip_err (id, val) VALUES (2, 'b')")) // dup key error
                                        .andThen(p.execute("INSERT INTO pip_err (id, val) VALUES (3, 'c')")) // success
                                }.map { results =>
    ...
    // ── Empty pipeline returns empty Chunk ────────────────────────────────────
    ...
    // ── Affected-row count is reported per-statement ──────────────────────────
                        ...
                        .andThen {
                            client.pipeline { p =>
                                p.execute("UPDATE pip_aff SET val = 'updated' WHERE id = 1")                 // 1 row
                                    .andThen(p.execute("UPDATE pip_aff SET val = 'all' WHERE id IN (2, 3)")) // 2 rows
                            }.map { results =>
                                ...
                                assert(
                                    affectedCount(results(0)) == Present(1L),
                                    s"Statement 1 should affect 1 row, got ${affectedCount(results(0))}"
                                )
                                assert(
                                    affectedCount(results(1)) == Present(2L),
