    ...
    "an instant renders at UTC" - {
        forEachBackend() { (backend, client, _) =>
            ...
            for
                ...
                // The literal is engine-spelled, the assertion below is not: one engine refuses the offset form outright, and what the
                // DRIVER renders back is the neutral text every engine must agree on, which is the property under test.
                _        <- client.executeRaw(s"INSERT INTO tzrender VALUES (${backend.instantLiteral("2026-08-25 10:00:00")})")
                extended <- client.query(select)
                simple   <- client.simpleQuery(select)
    ...
    "a mid-session zone change does not change what an instant reads as" - {
        forEachBackend(where = b => b.instantWireCarriesOffset && b.sessionZoneStatements.nonEmpty) { (backend, client, _) =>
            ...
            for
                ...
                _        <- client.executeRaw(s"INSERT INTO tzshift VALUES (${backend.instantLiteral("2026-08-25 10:00:00")})")
