...
/** Integration tests for the PostgreSQL COPY protocol (`copyIn` / `copyOut`).
  ...
  *   11. 1 M row COPY FROM completes within the per-test timeout and reports correct count
  */
class CopyIntegrationTest extends SqlContainerTest:

    override def timeout: Duration = 20.minutes

    // client.query uses the extended protocol (binary format); columnDecoded[Long] resolves its decoder here.
    private given PostgresDecoder[Long] = PostgresDecoder.int8
    ...
    /** Asserts a literal probe's result carries exactly its literal, in exactly one row.
      *
      ...
      * without proving the protocol state was clean enough to carry a correct result. A desynchronised connection can return a row while
      * returning the wrong row, which is precisely what a post-COPY probe exists to catch, so the value is what separates the two.
      */
    private def assertProbeRows(rows: Chunk[SqlRow], literal: Int)(using
        Frame,
        kyo.test.AssertScope
    ): Unit < Abort[SqlDecodeException] =
    ...
    private def assertProbe(client: SqlClient, literal: Int = 1)(using
        ...
        client.query(s"SELECT $literal").flatMap(rows => assertProbeRows(rows, literal))
    ...
    /** Builds a byte stream carrying `data` as a single chunk. */
    private def byteStream(data: Span[Byte]): Stream[Byte, Any] =
        Stream[Byte, Any]:
            Emit.valueWith(Chunk.from(data.toArray))(())

    /** Builds a byte stream that emits one chunk per span, in order. */
    private def byteStreams(spans: Chunk[Span[Byte]]): Stream[Byte, Any] =
        Stream[Byte, Any]:
            def loop(remaining: Chunk[Span[Byte]]): Unit < Emit[Chunk[Byte]] =
                if remaining.isEmpty then ()
                else Emit.valueWith(Chunk.from(remaining.head.toArray))(loop(remaining.tail))
            loop(spans)
        ...
        Scope.run {
            withPg { client =>
                ...
                client.executeRaw("CREATE TABLE copy_t1 (id INT, val TEXT)").flatMap { _ =>
                    val data = byteStream(csvRows(1, n))
        ...
        Scope.run {
            withPg { client =>
                ...
                client.executeRaw("CREATE TABLE copy_t2 (id INT, val TEXT)").flatMap { _ =>
                    ...
                    val inData = byteStream(csvRows(1, n))
                    ...
                    val data = Stream[Byte, Abort[SqlException]] {
                        Emit.valueWith(Chunk.from(csvRow(1, "a").toArray)) {
                            Emit.valueWith(Chunk.from(csvRow(2, "b").toArray)) {
                                Emit.valueWith(Chunk.from(csvRow(3, "c").toArray)) {
                                    Abort.fail(err)
                                }
                            }
                        }
        ...
        Scope.run {
            withPg { client =>
                ...
                client.executeRaw("CREATE TABLE copy_t4 (id INTEGER)").flatMap { _ =>
                    ...
                    val data       = byteStream(binaryData)
                ...
                client.executeRaw("CREATE TABLE copy_t5 (id INT, val TEXT)").flatMap { _ =>
                    ...
                    client.copyIn("COPY copy_t5 (id, val) FROM STDIN WITH (FORMAT CSV)", inData).flatMap { _ =>
                        ...
                        Abort.run[SqlException] {
                            ...
                            // Critical assertion: the SAME pool must be usable for a follow-up SELECT, and
                            // must carry the right answer over it.
                            assertProbe(client)
                        }
                    }
                }
            ...
            withPg { client =>
                ...
                client.executeRaw("CREATE TABLE copy_t7 (id INT PRIMARY KEY, val TEXT)").flatMap { _ =>
                    ...
                    client.executeRaw("INSERT INTO copy_t7 VALUES (5, 'existing')").flatMap { _ =>
                        ...
                        val goodAndBad = "1,a\n2,b\n3,c\n4,d\n5,DUPLICATE\n6,e\n"
                        val data       = byteStream(Span.from(goodAndBad.getBytes(StandardCharsets.UTF_8)))
                                            ...
                                            resumed.release.flatMap { _ =>
                                                ...
                                                fiber.get.flatMap { _ =>
                                                    chunksSeen.get.flatMap { seen =>
                                                        ...
                                                        // and still carrying correct results.
                                                        assertProbe(client)
                                                    }
                                                }
                                            }
                    ...
                    client.copyIn("COPY copy_t9 (id, val) FROM STDIN WITH (FORMAT CSV)", inData).flatMap { _ =>
                        ...
                        Abort.run[SqlException] {
                            ...
                            Abort.run[SqlException] {
                                ...
                                // The rows already in hand are asserted directly rather than through
                                // assertProbe, so the leaf keeps its named failure message for a probe that
                                // aborts and still pins the value for a probe that answers.
                                case Result.Success(rows) =>
                                    assertProbeRows(rows, 42)
                                case Result.Failure(e) =>
        ...
        Scope.run {
            withPg { client =>
                ...
                client.executeRaw("CREATE TABLE copy_t10 (id INT, val TEXT)").flatMap { _ =>
                    ...
                    // client.copyIn is transaction-aware: inside client.transaction it uses the bound
                    // connection, the same physical connection as the BEGIN.
                        ...
                        case Result.Failure(_: String) =>
                            ...
                            client.query("SELECT COUNT(*) FROM copy_t10").flatMap { rows =>
                                Abort.run[SqlDecodeException] {
                                    if rows.isEmpty then 0L
                                    else PostgresRowCodec.columnDecoded[Long](rows(0), 0)
                                }.map {
    ...
    "1 M row COPY FROM completes within the per-test timeout and reports correct count" in {
        Scope.run {
            withPg { client =>
                val totalRows = 1_000_000
                val chunkRows = 10_000 // rows per stream chunk
                val numChunks = totalRows / chunkRows

                client.executeRaw("CREATE TABLE copy_t11 (id INT, val TEXT)").flatMap { _ =>
                    // Build a lazy stream that emits numChunks chunks,
                    // each containing chunkRows CSV rows, without allocating everything up front.
                    val rowsStream: Stream[Byte, Any] =
                        byteStreams(Chunk.tabulate(numChunks)(chunkIdx => csvRows(chunkIdx * chunkRows + 1, chunkRows)))
