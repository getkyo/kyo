...
/** Integration tests verifying that evicted prepared statements are closed server-side via `Close 'S' <name>`.
  ...
  *
  * Test strategy: set `preparedStatementCacheSize=2`, execute 10 queries with distinct SQL strings so each occupies a different cache slot.
...
class PreparedStmtEvictionIntegrationTest extends SqlContainerTest:
    ...
    private def serverStmtCount(client: PostgresClient)(using
        Frame
