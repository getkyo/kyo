...
/** Integration test verifying that the NUMERIC round-trip works correctly against a real PostgreSQL server.
  *
  * Uses the Postgres extended-protocol via `SqlClient.query` and `SqlClient.execute`. Parameters are sent using `Sql.BoundValue` +
  ...
  */
class NumericBinaryIntegrationTest extends SqlContainerTest:

    // The extended protocol returns NUMERIC in binary, which PostgresDecoder.numeric reads; columnDecoded[BigDecimal] resolves it here.
    private given PostgresDecoder[BigDecimal] = PostgresDecoder.numeric
                            ...
                            Kyo.foreach(values.zipWithIndex) { case (v, i) =>
                                ...
                                Kyo.foreach(values.zipWithIndex) { case (expected, i) =>
                                    client.query(
                                        ...
                                        val row = rows(0)
                                        Abort.run[SqlDecodeException](PostgresRowCodec.columnDecoded[BigDecimal](row, 0)).map {
