...
/** Cross-backend conformance for [[SqlClient.withAdvisoryLock]]: on every backend that HAS advisory locks, the lock excludes a concurrent
  ...
  * locked session.
  *
  * Keyed advisory locks are a capability, not a given: an engine whose concurrency is one writer over the whole database has no per-key lock
  * to hand out. So the leaves split on `hasAdvisoryLocks` rather than run everywhere, and the final leaf claims the other side, asserting
  * that such an engine refuses the acquire with a typed failure. Every backend is covered by exactly one of the two.
  *
...
class SqlClientAdvisoryLockTest extends SqlBackendTest:
    ...
    "an engine without advisory locks refuses the acquire rather than blocking" - forEachBackend(where = !_.hasAdvisoryLocks) {
        (_, client, _) =>
            // The complement of every leaf above, and what makes gating them honest: an engine with no per-key lock has to REFUSE, typed
            // and immediately. Running the body anyway would hand back a lock that excludes nobody, and blocking would stall any caller
            // waiting on something the body releases, which is how this surfaced.
            val key = 5150L
