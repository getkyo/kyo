...
object SqlTestBackends:
    ...
    /** The descriptors to run this pass: those the registry carries that can also be exercised here.
      *
      * Reachability is asked of each descriptor rather than of the host, because it is not one fact. A server engine needs a container
      * daemon and an embedded one needs nothing, so a single probe standing in for "can this run" silently excludes every backend that
      * never needed a daemon. That probe is still what a container-backed descriptor answers with; see [[SqlTestBackend.reachable]].
      */
