...
/** Integration tests for MySQL connection, handshake, and text-protocol query.
  ...
  */
class MysqlConnectionTest extends SqlContainerTest:
                        ...
                        Scope.ensure(conn.close).andThen {
                            Abort.run[SqlException](conn.simpleQuery("SELECT * FROM nonexistent_table_xyz")).map {
                                case Result.Failure(e: SqlServerException) =>
                                    // The leaf is named for the error code and the SQLSTATE, so both are pinned.
                                    // A missing table is ER_NO_SUCH_TABLE, which the engine fixes: any other
                                    // failure carrying some SQLSTATE would satisfy a non-emptiness assertion.
                                    ...
                                    assert(
                                        e.serverMessage.contains("nonexistent_table_xyz"),
                                        s"the server's message must name the table that is missing, got '${e.serverMessage}'"
                                    )
        ...
        Scope.run {
            SqlSharedContainers.withFreshSchema(SqlSharedContainers.Backend.MySQL) { ctx =>
                MysqlConnection
                    ...
                    .flatMap { conn =>
                        Scope.ensure(conn.close).andThen {
                            conn.simpleExecute("CREATE TABLE mid_result_t (id INT)").andThen {
                                Kyo.foreachDiscard(1 to 5)(i => conn.simpleExecute(s"INSERT INTO mid_result_t VALUES ($i)")).andThen {
                                    // The correlated scalar subquery returns one row for t.id = 1 and more than one
                                    // from t.id = 2 on, so the server sends the first row and then replaces the next
                                    // row packet with ERR 1242. That is what makes this leaf mid-result rather than a
                                    // rejection before execution: a missing table or a syntax error never reaches a
                                    // row, and the leaf above already covers that case.
                                    val sql =
                                        "SELECT (SELECT s.id FROM mid_result_t s WHERE s.id <= t.id) FROM mid_result_t t ORDER BY t.id"
                                    Abort.run[SqlException](conn.simpleQuery(sql)).flatMap {
                                        case Result.Failure(e: SqlServerException) =>
                                            assert(
                                                e.extra.get("code").contains("1242"),
                                                s"expected MySQL error 1242 (ER_SUBQUERY_NO_1_ROW), got ${e.extra}"
                                            )
                                            assert(
                                                e.sqlState == "21000",
                                                s"expected SQLSTATE 21000 (cardinality violation), got '${e.sqlState}'"
                                            )
                                            // The ERR terminated the partially delivered result set, so the buffering
                                            // simpleQuery path must have drained to a clean wire rather than leaving
                                            // the undelivered rows in front of the next command.
                                            conn.simpleQuery("SELECT 'mid_result_ok'").map { r =>
                                                val str =
                                                    new String(r(0).column(0).get.toArray, java.nio.charset.StandardCharsets.UTF_8)
                                                assert(str == "mid_result_ok", s"the session must survive a mid-result error, got '$str'")
                                            }
                                        case other => fail(s"Expected SqlServerException, got: $other")
                                    }
                                }
