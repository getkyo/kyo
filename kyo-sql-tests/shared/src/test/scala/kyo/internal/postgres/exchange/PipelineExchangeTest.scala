...
/** Integration tests for [[PipelineExchange]] internal instrumentation.
  ...
  */
class PipelineExchangeTest extends SqlContainerTest:

    // Unsafe: reads and resets the transport-local write counter, an AtomicInt.Unsafe living outside any effect context.
    import AllowUnsafe.embrace.danger

    // Relies on SqlContainerTest.globallySequential: writeCount is one process-global counter and the other
    // postgres pipeline suites (PipelineIntegrationTest, SqlPipelineConformanceTest) increment it too, so only
    // cross-suite serialization keeps a concurrent pipeline write from landing between a reset and its assert.
        ...
        Scope.run {
            withPg { client =>
                ...
                client.executeRaw("CREATE TABLE pip_rtt (id INT PRIMARY KEY, val TEXT)").andThen {
                    client.pipeline { p =>
                        // Registration only buffers; the batch is flushed in ONE TCP write once the body returns.
            ...
            withPg { client =>
                ...
                client.query("SELECT 1").flatMap { rows =>
                    ...
                    rows(0).decode[Int](0).map { v =>
                        assert(v == 1, s"the probe query must return 1, got $v")
                        assert(
