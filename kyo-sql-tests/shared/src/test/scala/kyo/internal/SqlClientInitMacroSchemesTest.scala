...
class SqlClientInitMacroSchemesTest extends Test:
    ...
    // The scheme below is deliberately not the name of an engine anyone might ship. These two leaves assert what happens to an UNCLAIMED
    // scheme, so one picked because no backend claims it today silently stops testing anything the day a backend does: it was `sqlite`
    // until this repository shipped a SQLite backend, at which point both leaves were asserting against a scheme that resolves.
