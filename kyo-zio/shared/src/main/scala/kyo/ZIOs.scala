...
object ZIOs:
    ...
    private object internal:
        extension [E, A](exit: zio.Exit[E, A])
            def toResult(using frame: Frame, ce: CanEqual[E, E]): Result[E, A] =
                exit match
        ...
        extension [E](cause: zio.Cause[E])
            def toError(using frame: Frame, ce: CanEqual[E, E]): Result.Error[E] =
                ...
                loop(cause).getOrElse(Result.Panic(new Exception("Unexpected empty cause at " + frame.parse.position)))
            end toError
        end extension
    end internal
